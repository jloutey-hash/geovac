...
**Version:** v5.5.0 (September 3, 2026)
...
*Status:* **PROVEN — unconditional (2026-06-10; constant re-priced 2026-09-03).** Paper 38: the discrete truncations converge to the round-S³ spectral triple in van Suijlekom's state-space GH distance at rate (c + o(1))·log n_max/n_max, on the truthful CH substrate (translation-seminorm metrization; frozen falsifier `tests/test_p38_action_seminorm.py`). The constant is c = 4/π in the **dual-Coxeter** normalisation (Cas(ad) = h∨), which is the rule Paper 40 uses for the whole semisimple class and under which the geodesic distance on SU(2) is the rotation angle — so P38 is P40's rank-1 case and 4/π is canonical, not a convention artifact (settled 2026-09-03; `tests/test_p38_metric_convention.py`). On the unit S³ the same bound reads 2/π. The "SU(2) = twice the circle" reading is the metric scale. The Lorentzian extension (Papers 45–49) is DESCOPED (P45 annihilation theorem); repair path = Toeplitz temporal compressions (see WH7).
