...
- **M3 period-map rank (2026-07-08, v4.74.0):** Paper 56 cosmic-Galois M3 column = rank 2 not 1 via χ₋₄ vertex parity — resolves the named rank-≥2 follow-on, capped at 2 (not faithful); 3 stale "uncertain outcome" claims fixed; headline rank-1→2 flip pending PI χ₋₄-admission call. See debug/sprint_m3_parity_rank_memo.md.
...
Before forming or reporting a verdict — **or proposing a new connection or direction from corpus state** (e.g. an /ahha generative pass) — on any question, *especially* when resuming a thread from a `debug/` memo, verify the CURRENT state, not the snapshot. The papers (the section that *owns* the question) + CHANGELOG since the memo's date are canonical; `debug/` memos are dated snapshots the corpus moves past, and a memo's "open question / next step" may already be closed. First move when picking up a thread = read the owning paper section + post-memo CHANGELOG, THEN conclude *or propose*. (Added 2026-06-14: twice in one session a verdict was formed from an 8-day-stale synthesis memo — an already-proven theorem was reported as a future sprint, and an already-settled A-vs-B question was re-answered wrong; both answers were live in Paper 56. Widened 2026-07-08: generation is not exempt — an /ahha pass pitched a connection as novel that the Marcolli–vS chemistry arc had already established a month earlier; the `/ahha` skill now grounds-then-verifies (Steps 0/2). Standing rule: `memory/feedback_verify_current_state.md`.)
