...
**Version:** v5.15.24 (September 23, 2026)
...
*Status:* REGISTERED (2026-06-10); **PRIOR ART FOUND + a technical hole opened, 2026-09-06.** Load-bearing leg is **(i)**. Prior art (verified): Dolce's Elementary Cycles (arXiv:1707.00677; Ann. Phys. 327, 1562) already postulates compact/cyclic Minkowski time as *the* quantization condition, unifying Matsubara + KK + particle-in-a-box — ontological, not observer-relative, which is now WH7's **only** distinguishing content and must be stated explicitly. Rovelli's RQM Postulate 1 ("relevant information is finite for a system with compact phase space" ⇒ "the characteristic discreteness of quantum theory", IJTP 35, 1637) anticipates the compactness→discreteness chain generally. **Technical hole (aim the falsifier here):** a compact *interval* is not a compact *circle* — a finite real-time window with no boundary condition gives a broadened but still CONTINUOUS spectrum; 2π/β spacing requires periodic identification. And the Matsubara/KMS circle is *imaginary* time while an observer's lifetime is a *real*-time interval; WH7 identifies the two silently. Unverified but flagged: bounded-region (diamond) algebras are Type III₁ with continuous modular spectrum, and the one rigorous compact-window computation (Martinetti–Rovelli, CQG 20, 4919) yields a temperature but *no* discrete spectrum — if that holds it is fatal to the strong form. Honest cap is itself published: Kempf, NJP 12, 115001 (2010), continuous↔discrete equivalent under bandlimitation — cite, do not present as an internal caveat. Prior verdicts unchanged (Toeplitz "weakens-to-convention"; Lorentzian chase structurally closed; μ₄/CM re-reading). **Claim-level restatement is a PI call.** Scans: `debug/lit_scan/{compactness_discreteness,info_theoretic_compactness}_memo.md`. Superseded text: `docs/wh_register_history.md`.
...
**Organizing observation — compactness *forces* discreteness; the converse is false (corrected 2026-09-06, PI-directed).** Compact groups have discrete spectra (Peter-Weyl), and that is the mathematical content of the S³ graph: the discrete, bit-exact skeleton is the closed/compact regime. **Do not state this as an identity.** "Discreteness *is* compactness" is false as a biconditional over spaces — Maz'ya–Shubin (Ann. of Math. 162, 919) give purely discrete spectra on non-compact ℝⁿ from a confining potential alone, and the corpus's own Paper 24 harmonic oscillator is that counterexample. The correct general statement is at the level of the **operator**: discrete spectrum with finite multiplicities ⇔ **compact resolvent** (already the definitional axiom in WH1's spectral triple). Compactness of the group or manifold is a *sufficient* mechanism and the one operative for S³; it is not necessary. **Credit:** the physics reading is not ours — Liu–Noui (CQG 34, 135008) state it causally for LQG's SU(2), Rovelli's RQM Postulate 1 (IJTP 35, 1637) derives discreteness from compact phase space, and Dolce (arXiv:1707.00677) states it for time. **What remains ours is the accounting**, not the mechanism: each released axis carries an identifiable transcendental price, so the skeleton is rational/forced and calibration (continuum, un-packed data) is transcendental/free. Captured in Paper 18 §III (with the attribution paragraph and this correction) and Paper 35 (temporal compactification injects π). The "second packing axiom" framing is retired; the 2026-05-30 confinement reframing is archived as an organizing reading (2026-05-31).
...
- **The "marriage" DELIVERED — RI-free multibody r₁₂ on a multi-det reference, cross-validated LiH energy −8.0372 Ha (2026-09-23, v5.15.24, PI-directed, 5 phases):** the exact RI-free bridge/triangle reductions run on a 74-det σ CI reference (not the welded 2-MO one). Phase 1 G-leaf PASS — the 4-body bridge reproduces brute-force MC on real non-isotropic densities (the audit's weakest link; general bound 2(l_bridge+l_leaf) confirmed by a nonzero l=4 channel). Phase 2 G1 PASS — 7 enumerators generalised to tensor contractions, reproduce the 2-MO banked energies to <0.1 mHa. Phase 3 GO — marriage E_R12 = −8.0372 (linexp/exp agree 0.15 mHa; variational; beats the −8.032 ceiling), VMC-cross-checked to ≤0.31 mHa. New: exact ordered-integral Neumann operator (`geovac/lih_r12ci/neumann_exact.py`, opt-in, legacy bit-identical). **NOT corpus-best (−8.047 VMC stays best; σ-only reference caps it, no π); methodological milestone.** Follow-ons: M=16 π reference → −8.047 window; r₁₂t² 3-body → −8.062. READ FIRST `memory/lih_marriage_state_of_play.md`; memo `debug/sprint_lih_marriage_memo.md`; Paper 19 [MARRIAGE], Paper 12. See CHANGELOG v5.15.24.
- **Bridge factorization re-tiered + "marriage is blocked" REVERSED (2026-09-22, v5.15.23, PI-directed, two audits):** the N=4 "bridge addition theorem" is a VALIDATED REDUCTION, not a theorem (no derivation; bound is 2(l_bridge+l_leaf), the quoted 2·l_bridge is the s-leaf case — L=4 nonzero at 24σ for p-leaves; π/δ on a model density; N≥5 conjectural). The analytic-r12 ⟷ multi-det-reference marriage is NOT a wall — UNDONE/tractable (every reduction takes a pair-density product as argument; reference = coefficient lists); the marriage IS the factorization's generality test (gate G-leaf). Expect ≈ −8.047 (pairwise cap), −8.062 needs an r₁₂t² term. READ FIRST `memory/lih_marriage_state_of_play.md`; plan `debug/lih_marriage_build_plan.md`; Paper 12 [AUDITED] ¶ + Paper 19; register/matrix/memos corrected at source. See CHANGELOG v5.15.23.
- **Rigorous LiH energy via VMC-over-FCI: E = −8.047 ± 0.001 (beats −8.032 ceiling by 18 mHa, variational) (2026-09-22, v5.15.22, PI-directed):** built + validated a from-scratch VMC-over-FCI stack (`debug/lih_vmc.py`): real-space analytic-orbital Ψ_CI (bit-matches the engine; gate-6 VMC(J=1)==FCI on both real M=6 and complex-π M=16) × 2-body Padé Jastrow (exact cusp), standard finite-variance local energy (NOT the plan's gradient form — infinite node variance). Padé γ=1.5 → **−8.04731 ± 0.00073**. **Tempers v5.15.21's "−8.062 CONFIRMED"**: a 2-body Jastrow recovers ~19 of 41 mHa; −8.062 needs 3-body (e-e-n) correlation. Decision gate = BORDERLINE. `tests/test_lih_vmc.py` (5 gates); Paper 19 updated. See `debug/sprint_lih_vmc_progress.md`.
...
- **Route C energy follow-on — LiH energy is determinant-walled, not radial (2026-09-22, v5.15.17):** the radial ladder reaches H2 chemical accuracy (99.66%), but LiH caps ~58 mHa above exact (4e determinant wall + core-core correlation from the single-orbital analytic core); cure = explicit r₁₂ (correlation-frugal), NOT more radial functions. Conditioning fix = float64 Laguerre×Legendre re-basing. PI-directed next: lean on the r₁₂ arc (`geovac/lih_r12ci`) for the energy. See debug/track_logs/prolate_native_lih.md.
- **Prolate all-electron LiH R_eq = +0.2% (π-converged), best in corpus (2026-09-22, v5.15.16):** Route C — an R-accurate analytic atom-centered core cures the +5.5% frozen-core drift (chain: breathing NO → polarization NO → the frozen-core *approximation* is the culprit → C1 core 1e-50 → C2 ERIs 1e-28 → C3 binds +1.9% → C4 π converges +0.2%). Geometry paper-grade; energy PoC (~60 mHa, single-ζ radial). All debug/; Paper 19 + tests owed. See debug/track_logs/prolate_native_lih.md.
- **LiH R12-CI item (2) DONE — engine migrated to `geovac/lih_r12ci/` + regression tests (2026-09-21, v5.15.15):** the debug PoC engine (10-module acyclic DAG) is now a deterministic `geovac` subpackage with an `energy(geminal)` API reproducing both validated energies (linexp −7.9168 bit-exact; exp −7.9418, 0.2 mHa vs debug) — 6 passing @slow tests (`tests/test_lih_r12ci.py`); intricate math preserved verbatim, imports rewritten, no MC in the energy path. debug/ drivers retained (MC ground truths + chronicle). See CHANGELOG v5.15.15.
- **LiH R12-CI item (1) CLOSED — linexp E_R12=−7.917 fully analytic/RI-free/deterministic (2026-09-21, v5.15.14):** the one owed grid-limited Yukawa covariance was the `psi_yuk` dressing (~0.5% off exact `yukawa_pot_iso` on the isotropic aa/bb blocks, F̄=3.57-amplified), NOT the grid — fixed by exact-isotropic dressings (mirrors the Coulomb exact-Hartree+Neumann-ab split): Cov[F,Y] −0.079→−0.1025 (MC −0.1018), E_R12(linexp) −7.9210→−7.9168 (dE −33.2→−29.0 mHa, VMC −29.5). ab,ab de-MC'd (deterministic). Exp −7.942 left as-is (validated cancellation). **Owed: geovac/ migration + tests (item 2).** See CHANGELOG v5.15.14.
- **LiH R12-CI — COMPLETE: 3-body triangle reduced RI-free, g_Vee/g_T done, full analytic 2×2 → E_R12 assembled (2026-09-21, v5.15.13, PI-directed):** the owed pieces all closed. **Triangle** `⟨f_{ik}f_{jk}/r_{ij}⟩` (the one non-separable class) reduced RI-free — non-separable H₂ → azimuthal Fourier `H₂^{(m)}` → low-rank eigh → mode-by-mode general-m prolate Neumann Coulomb; both placements (intra/inter coul) validated vs MC (T1 0.2σ, T2 0.4σ). **g_Vee=+0.53834** (VMC +0.53605, 0.4%) via a 216-triple unified reducer (separable→m=0 Coulomb/Yukawa, 3-cycle→triangle; reproduces E2 exactly; all 6 groups match MC ~1e-3; grid-consistent cancellation). **g_T=+1.39419** (VMC +1.38640, 0.56%) via an IBP collapse — gT2+gT3 → scalar Yukawa cov, **no vector ∇f dressing needed**; only gT1 (drift² over the KD gradient density) is new. **Capstone 2×2 → E_R12=−7.94200 Ha** (dE −54.2 mHa, variational), matching the same-geminal VMC 2×2 to **0.79 mHa** — the full two-center 4e explicit-r₁₂ CI energy now assembled analytically/RI-free, incl. the 3-body triangle (the boundary classic F12 treats with RI). Done for BOTH geminals (framework geminal-agnostic, only kernels change): exp f=e^{−γr} → E_R12=−7.942 (matches VMC 0.79 mHa); **cusp-correct f=r·e^{−γr} → E_R12=−7.921/dE−33 mHa, reproducing the vmc −7.932 headline** — every linexp piece <1% except one grid-limited Yukawa covariance Cov[F,Y_sum] (§1.3 long-range ill-conditioning, F̄=3.57), which at converged precision → −7.917/dE−29 mHa = vmc. Files `debug/lih_r12ci_{triangle_gate,gVee_analytic,gT_analytic,assemble,linexp}.py`; CHANGELOG v5.15.13 + `debug/lih_r12_build_plan.md`.
- **LiH R12-CI — full off-diagonal h ANALYTIC/RI-free; g_Vne done; 4-body bridge inside the energy validated (2026-09-21, v5.15.12, PI-directed):** analytic RI-free **h_T=+0.7495** (VMC +0.7485; Part A gradient densities G_pq + Part B = IBP-collapse of ∇F·∇Φ₀ → scalar Yukawa Ȳ) and **h_Vee=+0.3098** (VMC +0.3082; reusable `cov_FA_FB` engine self-checked vs σ² + two-center Coulomb `neumann_potential`) ⟹ **h=+0.129** (VMC +0.1268 ✓). Diagonal g: **g_Vne=−2.852** VALIDATED (block decomp F=S+I; ⟨F²V⟩=⟨S²V⟩+2⟨SIV⟩+⟨I²V⟩, A/B/C all MC-checked); the **RI-free 4-body bridge INSIDE the energy** `⟨f₁₂f₃₄coul₁₃⟩=¼I_coul[S^f,S^f]`=0.0328 vs MC 0.0328 (0.0σ) — the showpiece — + Yukawa-field trick `Ψ^Y=Ψ^coul−Ψ^smooth`. **Owed:** rest of g_Vee = a two-center **3-body triangle** reduction (H₂ low-rank/SVD + per-mode Neumann) + g_T + 2×2→E_R12 vs VMC −7.932. Files `debug/lih_r12ci_{hT,hVee}_analytic.py`, `{gVne_analytic,gVee_groundwork}.py`; CHANGELOG v5.15.12 + `debug/lih_r12_build_plan.md`.
- **LiH R12-CI PoC energy PASSED + RI-free analytic σ²/h_Vne (2026-09-21, v5.15.11, PI-directed):** two-center E₀=−7.888 (analytic); explicit-r₁₂ VMC E_R12=−7.932 (dE=−29.5 mHa, ~36% corr, variational, >exact); the RI-free analytic reduction (no MC/RI) reproduces VMC for σ²=0.137 & h_Vne=−0.930 (0.03σ) — the "ahead-of-F12" machinery inside real correlation quantities. h_T/h_Vee/g owed with VMC targets banked. See CHANGELOG v5.15.11 + `debug/lih_r12_build_plan.md`.
- **LiH (two-center) N=4 explicit-r₁₂ 4-body integral gate (σ + π/δ) — the Be soft-wall reduction CARRIES TO TWO CENTERS, ALL m (2026-09-21, v5.15.10, PI-directed):** the 4-body bridge `⟨ρ₁ρ₂ρ₃ρ₄ f₁₂f₃₄/r₁₃⟩` reduces **exactly, RI-free** to a 1-D leaf dressing + a two-center **prolate Neumann** Coulomb between dressed densities. σ (m=0) anchored to **closed-form 5α/8**, full reduction == 12-D MC rel 1.5e-4; π (m=1)/δ (m=2) via an azimuthally-modulated bridge density → general-m Neumann (assoc. Legendre `P_l^m/Q_l^m`) pinned to 6-D MC **rel 3.9e-6**, full 4-body == 12-D MC rel 2.2e-5, m=0 part reproduces σ bit-for-bit. Stronger anchors than the Be gate (closed-form + near-exact MC). Scalar-Coulomb model orbitals, **NOT** a LiH energy — energy assembly (ansatz B on the 4e FCI Φ₀) the remaining owed step. Engines `debug/lih_r12_4body_integral.py` (σ) + `debug/lih_r12_4body_pi_channel.py` (π/δ); memo `debug/sprint_lih_r12_4body_integral_memo.md`; CHANGELOG v5.15.10.
- **Be R12-CI energy — 4-body RI-free reduction demonstrated inside a real Be correlation energy (2026-09-21, v5.15.9, PI-directed):** E_R12 = −14.5572 Ha, correlation **−18.2 mHa (19% of Be's 94 mHa), variational**, from a short-range explicit-r₁₂ geminal. Key: the linear-geminal energy is **ill-conditioned** (basis 99.8% parallel; the coupling h is a 0.03 residual of ~71-magnitude numbers → needs ~1e-4 precision, beyond MC), fixed by a short-range geminal + a **noise-free analytic block-reduction engine** (overlaps + H₀₁ exact, validated 0.15% vs MC; **h and σ² exact analytic**; well-conditioned g by MC). The 4-body content lives in gV=E[(F−F̄)²V_ee]. PoC-level (single geminal, minimal basis; not spectroscopic). Engine `debug/be_r12ci_full.py`; chronicle memo §7b + CHANGELOG v5.15.9.
- **N=4 explicit-r₁₂ "wall" is SOFT for the scalar Coulomb term (2026-09-21, v5.15.8, PI-directed diagnostic):** the disjoint-pair `f₁₂f₃₄/r₁₃` bridging integral (the only genuinely-4-body term in `⟨Φ|F H F|Φ⟩`; kinetic gradients are pair-local, V_ne one-body → only the two-body Coulomb bridges) is exact, RI-free, terminates at L=2·l_bridge, and reduces to a 2-electron Slater-Condon integral — **reduced==brute 0.02%** on the Be-relevant integral (2p bridge / 1s leaves). Refines the documented "N=4 = THE WALL, no ≤3-body reduction": true as a *term* claim, but exact-no-RI does NOT stop at N=4; on this axis ahead of Gaussian-F12 (which uses RI here). NOT a Be energy; the hard walls (quantum-encoding 4-body Pauli; non-Hermitian TC; kinetic-vector 4-body untested) are separate axes. See `debug/sprint_n4_wall_diagnostic_memo.md`.
- **All-electron variational-core prolate LiH — cure CONFIRMED (bracketed), clean number blocked by a NUMERICAL wall (2026-09-20, v5.15.7, PI-directed):** built + validated a multi-exponent all-electron prolate FCI with generalized-m (π) ERIs (`debug/prolate_allelectron_fci.py`; HF==eckart, μ-kernels==numerical, H₂ converges). σ(+π) LiH collapses INWARD — mirror of the frozen-core +5.5% OUTWARD drift; two review cycles pinned the cause to a *numerical* wall (isolated Li²⁺ core swings 0.36 Ha with R = grid resolution tied to r_A=(R/2)(ξ+η), NOT BSSE; confirmed by refinement) with a co-dominant *two-center-basis-completeness* half no grid fixes. PI's cancellation hunch → fragment subtraction over-corrects to +22%; graded grid marginal (+15%); found+fixed a real `_attraction_matrix` 1/r-at-focus bug. **Verdict: frozen-core=culprit / variational-core=cure STANDS** (bracketed inward↔+5.5%↔+22% + standard QC); bespoke clean R_eq needs a dedicated atom-centered core (scoped future build). `debug/track_logs/prolate_native_lih.md`; guards `tests/test_prolate_allelectron_fci.py`. See CHANGELOG v5.15.7.
- **Frozen-core prolate LiH — VERDICT: the FROZEN CORE is the culprit, not missing r₁₂ (PI's bet RIGHT) (2026-09-20, v5.15.6, PI-directed):** built the discriminating test — r₁₂-coupled core-shield V_H + Huzinaga projector (`debug/lih_r12_coupled.py`, p=0-reduction validated to 5.6e-4/1.2e-4 vs the factorized refs) so LiH runs WITH r₁₂ (apples-to-apples with the HeH⁺ PoC). Result: WITH r₁₂ the R_eq drift STILL points outward and STILL grows with angular basis — **+3.4% (l=2) → +5.5% (l=3)** — the OPPOSITE of HeH⁺'s convergence (−4.5%→−0.5%) at the identical (2,3) basis; energies variational, λ-plateau-verified (flat λ=30..10⁴) so NOT a projector artifact. r₁₂ absorbs part of the over-polarization (+7.1%→+5.5% at l=3) but can't reach a core d.o.f. The finite-extent rigidity (v5.15.2 mechanism) lives in the FROZEN CORE, not the valence bond orbital; a variational valence + r₁₂ is insufficient. **Cure = all-electron (N=4) variational-core build** (hits the N=4 explicit-r₁₂ wall). Prolate frozen-core (+5.5%) improves on the *balanced* recipe (+8.8%, Paper 19) but does NOT beat *composed* l-dep-PK (5.3%, Paper 17) — this was a mechanism diagnostic, not a new best LiH number; the distinguishing feature is HeH⁺'s demonstrated convergence (no frozen core). Paper 19 sharpened. `debug/track_logs/prolate_native_lih.md`; `debug/data/lih_r12_req_l{2,3}.log`. See CHANGELOG v5.15.6.
- **HeH⁺ prolate PoC captured + reference verified (2026-09-20, v5.15.3, PI-directed):** the prolate explicit-r₁₂ engine reproduces heteronuclear *geometry*, not just energy — HeH⁺ R_eq converges −4.5%→−0.5% (l 2→3, the angular lever), energy 0.9 mHa off the verified Kolos-Peek reference (R_e=1.463 a₀, D_e=16455.64 cm⁻¹ → E_BO=−2.9787 Ha). This is the validated precedent for the frozen-core prolate LiH build (`debug/track_logs/prolate_native_lih.md`); Paper 12 HeH⁺ paragraph extended + first `tests/` backing. See CHANGELOG v5.15.3.
- **LiH R_eq-drift mechanism PINNED → WALL registered (2026-09-20, v5.15.2, PI-directed):** the balanced-LiH 8.8% drift is the *finite-extent screening* of the fixed Z_orb=1 bond orbital — cross-attraction R-slope→point-charge limit +Z/R² (= the required force) as it contracts, but contraction is inexpressible in the parameter-free construction (h1=−Z_orb²/2n² locks Z_orb to Z_nuc). Named→demonstrated; Paper 19 sharpened, test-backed, wall registered. Next: prolate-native (H2-style) LiH scoping. See CHANGELOG v5.15.2.
- **/qa DELTA (Papers 12/14 + 13/20/synthesis) = DEFECTS, remediated (2026-09-20, v5.15.1, PI-invoked):** the record numbers are SOUND; fixed an unverifiable Tao–McCurdy–Rescigno attribution (P12 §gap/§impl/§concl + P15), a P20 pair-diagonal zombie (→127,855/42,535), synthesis H₂ currency (→99.97%/0.053 mHa), and a stale group4 DoD (1.69→1.774). All deterministic gates PASS; all papers compile. See CHANGELOG v5.15.1.
- **v5.15.0 checkpoint (minor, PI-directed, 2026-09-20):** rolls up the explicit-r₁₂ arc — exact-algebraic engine (H₂ 0.053 mHa), N≤3-generalizes / N=4-wall boundary, QA-prep cleanup. Detail in the v5.14.8–.11 bullets below. See CHANGELOG v5.15.0.
- **Exact-algebraic r₁₂ generalizes to N=3; wall is N=4 (2026-09-20, v5.14.11, PI-directed B probe):** transfers to a 2nd center (HeH⁺, new heteronuclear V_ne, validated 6.1e-5) and a 3rd electron (Li, existing engine) — exact, no RI up to N=3 (triangle rule δ_abc/(2a+1)²); N=4 (Be) first needs 4-body. RETRACTED: HeH⁺'s gap is basis-size, NOT single-α (two-block gains ~0.3 mHa; multi-exponent is multi-shell, not heteronuclear). Trenev cc-pVDZ counts VERIFIED vs primary source. See CHANGELOG v5.14.11.
- **Explicit-r₁₂ H₂ = 0.053 mHa (99.97% D_e), exact integrals, 6× past re-based CI; odd routing 65× faster (2026-09-20, v5.14.10, PI-approved capture):** r₁₂ powers reduce to the same A·K0−B·K1 Neumann kernels as V_ee (no quadrature; p0×p1 kinetic IBP-collapses to −½∫r₁₂ g_v ∇²g_u). Plateaus at p≤1/single-α (space near-spanned); µHa needs p≥2. mpf solve load-bearing past cond 1e19. See CHANGELOG v5.14.10.
- **Li's residual = 88% angular, not cusp (2026-09-19, v5.14.9):** naive cusp gain 4.5x-inflated by basis redundancy (4.53→1.00 mHa at the converged basis); Li's lever is l>0 orbitals, opposite of H2's cusp. See CHANGELOG v5.14.9.
- **LiH R_eq drift = one term (2026-09-19, v5.14.8, PI-approved):** cross-center V_ne R-slope 8.8% too weak; all other terms bit-exactly R-independent; closes to the R-polarization wall. Paper 19 sharpened, test-backed. See CHANGELOG v5.14.8.
- **Fixed /checkpoint's stale remote model + clarified version-bump scope (2026-09-19, v5.14.7, PI-directed):** the skill + a §2 bullet claimed "no GitHub origin"; `main` in fact tracks a GitHub `origin`, so a `push` publishes there. Bump for CLAUDE.md / behavioral-md changes (`memory/feedback_version_bump_scope.md`). See CHANGELOG v5.14.7.
...
- **/checkpoint un-blocks main push (2026-09-17, v5.13.7, PI-directed):** the command hard-prohibited `git push origin main`, assuming a PR workflow this repo doesn't use — it commits directly to `main`. Step 4 now pushes the current branch (main included) to its upstream under explicit PI direction; force-push and GitHub-Release prohibitions unchanged. (Corrected 2026-09-19, v5.14.7: `main` tracks a GitHub `origin` — `github.com/jloutey-hash/geovac` — alongside the local mirror `old`; the earlier "no GitHub origin" note was stale, and a `push` publishes to GitHub.) See CHANGELOG.
...
- **Paper 12 synced to the closed-form B-seeds (2026-09-17, v5.13.5):** corrected 9 loci that still called the V_ee "quadrature-seeded / not fully quadrature-free" — now documents the `{E₁,γ,ln}` closed form (§sec:Bl_closed), fully quadrature-free; LaTeX cycle clean. A V_ee X-table test un-gated from `@slow` (0.2s now). See CHANGELOG.
- **V_ee B-seeds closed-form → `vee_mp` 38× faster, energy bit-identical (2026-09-17, v5.13.4):** the `_seed_B` mpmath quadrature (95% of `vee_mp`) is replaced by an {E₁,γ,ln} closed form — the obstruction dissolves because s≥m always polynomializes the (ξ−1)^{−k} poles under the intact (ξ²−1)^s, leaving one Q₀ log-moment. General-m V_ee now quadrature-free (§12 registry → algebraic). 176–268× on `_B_table`; may warrant a minor (PI). See CHANGELOG + debug/direct_vee_bseed_closedform.py.
- **V_ee corr 2D l-recurrence PROVEN; B-seeds are the V_ee wall (2026-09-17, v5.13.3):** the ordered-integral corr reproduces the mpf X-table to 1e-27 (σ/π/δ; 2 independent ground truths); float64 recipe = mpf Q-rows + float64 P-axis. But `_seed_B` mpmath quadrature is 95% of `vee_mp` AND 92% of the recurrence build — float64-fast V_ee is gated on the B-seeds, not corr; that is the next target. See debug/sprint_direct_build_memo.md.
- **V_ee z-recurrence + σ assembly validated; corr = the hard core (2026-09-17, v5.13.2):** #3 V_ee follow-on — first-kind Neumann moment A_l[a] float64-exact via the z-Laguerre l-recurrence (2e-16); σ-sector V_ee assembly machine-exact vs mpf (2.6e-15). Correct, not yet faster; the remaining speed piece = the IBP-corr term, pinned as a 2D ordered integral coupling d^mP_l/d^mQ_l → needs a 2D l-recurrence (a focused sprint). **Consolidated for context reset — see the "RESUME HERE" block atop debug/sprint_direct_build_memo.md.**
- **H₂→Level 2; one-body direct engine (2026-09-17, v5.13.0):** PI moved H₂ to Level 2 (prolate, 99.77%) in §5; #3 built the complete float64 one-body engine (S+T+V_ne, all μ) validated to 7e-16, ~135× faster than mpf; V_ee float64 path diagnosed (shortcuts ruled out, z-recurrence identified). See debug/sprint_direct_build_memo.md.
- **H₂ re-conditioning productionized; Paper 12 corrected (2026-09-17, v5.12.9):** `geovac/prolate_recondition.py` + `tests/test_paper12_recondition.py` reproduce 99.767% of D_e at (5,5)+δ (0.41 mHa); Paper 12's "99.1% cap" corrected to a conditioning wall. Gegenbauer basis = same span, ~1000× better-conditioned (NOT fewer functions; a float64 re-basing *of the monomial matrices* breaks for both families — which does not bar a direct float64 build in the orthogonal basis, see v5.13.8). See debug/sprint_h2_recondition_memo.md.
...
- **P60 floor is the scale lock, not l_max (2026-09-08, v5.10.12):** metric-free iff lambda = p_kappa, so the span was never the limit; ground-state pathology (4.28x chem vs 1.08x for 2^1S). See CHANGELOG v5.10.12.
- **/qa 58/59/60 FULL + group1 DELTA = FAIL, remediated (2026-09-07, v5.10.10):** P60's `K^0.84` was a WINDOW fit and five gates were examining nothing. Its replacement mechanism was ITSELF retired next day -- current values in the v5.10.12 bullet above. See debug/sprint_qa_papers_58_59_60_memo.md.
...
- **Decompactification is continuous (2026-09-06, v5.10.2):** two-center front set by decay length n/Z, linear in R; correlation moves only the coherence front, via antibonding occupation. Papers 58/60/11; CHANGELOG v5.10.2.
- **/qa trunk DELTA #10 = DEFECTS (2026-09-05, v5.10.1): the claim-impact discipline paid off on run one.** Trunk items (P1 L246 'faithfully reproduces the Coulomb degeneracy' -- a 'reproduces' paraphrase my 'recover'-keyed grep missed; P32 sqrt2->2) fixed -> trunk clean. The claim-impact reviewer, reading the IMPACT SET (not the diff), surfaced a substantial GROUP1 stale-echo cluster no byte-diff/grep would reach: the withdrawn L5 assembly still credited as the live proof in the group1 synthesis (+false height_P=0) and enumerated across P42/44/47/51; the Lorentzian descope never reached the P42/P43 abstracts; P40's Pythagorean-route future-work. Owed as its own group1 pass. See carryforward Part U.
- **Semantic-diff DELTA + FULL #6 remediation (2026-09-05, v5.10.0, PI-directed minor):** the DELTA scope is redefined from a byte-diff to a **claim-impact set** (nominated by \cite-graph + topical-vocab + rests-on, swept by a claim-impact reviewer; generalizes/retires the owner-moved dispatch). Measured driver: DELTA #9 CLEAN but FULL #6 found 2 LARGE zombies outside the diff -- clean byte-delta != certifiable corpus; only a claim-impact delta or the FULL run reaches the pre-existing surface. All FULL #6 findings (2 LARGE + 6 SMALL) remediated gate-first at claim-scope; two instrument defects fixed (C16 +-5-window -> two-tier +-2; Paper-39->32 cited_by). See carryforward Part T.
- **/qa trunk FULL run #6 = FAIL (2026-09-05, v5.9.7, PI-invoked):** the certifying run; NOT a PASS. 2 LARGE + 6 SMALL, all the retracted/refuted-claim-left-live class (P7 L127 verbatim gate-escaped; P32 L1453 Paper-39 refuted-mechanism = cited_by miss; +6). Mathematics SOUND (keystones re-derived, C7/C8/K clean, restricted-eval all PASS, citations grounded). Key lesson: DELTA #9 was CLEAN but FULL #6 found pre-existing zombies -- a delta verifies only the diff; 'clean delta' != certifiable corpus. Two instrument defects: C16 +-5-window escape; Paper-39->32 cited_by miss. See carryforward Part S.
- **/qa trunk DELTA #9 = CLEAN-DELTA (2026-09-05, v5.9.6):** the v5.9.5 remediation held (9/9 loci + owner-moved synthesis clean). 3 NITs swept, incl. a self-catch: my v5.9.5 Lorentzian relabel was locus-by-locus, leaving sibling sigma_2pi verdicts inconsistent -- the owner-moved dispatch caught the churn class *in my own fix*. FULL #6 unlocked. See carryforward Part R.
- **/qa trunk FULL run #5 = FAIL, CALIBRATED (2026-09-05, v5.9.5, PI-invoked seeded):** the re-measure. 11 reviewers + completeness-critic; 10/12 seeds caught, 0 FP; both priority classes addressed (S10 mirror-staleness caught by the new owner-moved mandate; restricted-evaluation covered organically). 9 genuine MATERIAL-SMALL defects, all the recurring locus-by-locus-remediation class; every keystone SOUND under independent re-derivation. 2 seeds void (S9/S11, my construction errors). See carryforward Part Q.
- **Trunk FULL re-measure prep (2026-09-05, v5.9.4, PI direction):** seed catalog gains S10 (mirror-staleness) + S11 (restricted-evaluation), the live-distribution classes the old catalog never tested; trunk deterministic layer 12/12 green, profile unchanged since DELTA #7 -> ready for `/qa trunk seeded`. See CHANGELOG v5.9.4.
- **QA process meta-critique -> tooling (2026-09-05, v5.9.3):** pre-commit hook fires the fast gates (C19/C16/C17-scoped/`py_compile`) at introduction; suspiciously-clean heuristic -> code-reviewer restricted-evaluation mandate; DELTA owner-moved-synthesis dispatch rule; `rests on:` inversion = durable mirror mechanism. Arguably a minor (PI). See CHANGELOG v5.9.3.
- **/qa trunk DELTA #8 (2026-09-05, v5.9.2):** DEFECTS, remediated (1 LARGE synthesis staleness + a few reconciliation leftovers; 40 -> ~5 material, none mathematical). Paper 39 discharge settled. See carryforward Part P.
- **Paper 39 discharged CONDITIONAL -> [INTERNAL THEOREM] (2026-09-05, v5.9.1):** lifted-state route (constant 1, reach_P unnecessary) critique-confirmed SOUND; limit = Dabrowski-Dossena tensor CH triple, J_ab KO-6 verified. Possible minor (PI). See carryforward O.5e/f.
...
| LiH | R_eq +0.2% (best); energy −8.047 rigorous (VMC-over-FCI, variational; −8.062 = additive-F12 est. needing 3-body) | Route C prolate all-electron, analytic core (geom); VMC(Ψ_CI × 2-body Padé Jastrow) (energy). Composed l-dep-PK = 5.3% | 19/17 |
...
| R-adaptive shared bond exponent Z_orb(R) to heal the balanced-LiH R_eq drift (2026-09-20) | 1 | Non-variational in the parameter-free construction: h1=−Z_orb²/2n² locks Z_orb to Z_nuc, so Z_orb>1 on the Z=1 H centre drives E below exact (−9.6 vs −8.07). The drift's cure needs a variational (prolate-native) basis, not a knob on the composed recipe. See `debug/sprint_balanced_reqdrift_mechanism_memo.md`. |
...
| 2 | H2 (2-center, 2e) | Prolate spheroid | 99.97% D_e (0.053 mHa) via explicit-r₁₂ CI with exact algebraic integrals (v5.14.10); 99.81% (0.32 mHa) at α=1.40 and 99.77% (0.41 mHa) fixed-α — re-based CI (v5.13.9) | 12 |
...
*Level 4 note (added 2026-09-14, PI-directed):* **Level 4's placement for H2 was motivated in part by a claim that is now withdrawn, and on accuracy it is no longer the leading geometry for this system.** Paper 12 read its 92.4% prolate-spheroidal residual as the electron-electron cusp being a coordinate singularity in (xi, eta) that demands a coordinate change; Papers 13 and 15 inherited that motivation. The residual was instead the sigma-only restriction — a phi-independent basis spans only m1 = m2 = 0, while a 1Sigma_g+ state constrains only the total M. Restoring the azimuthal channels in the *same* prolate-spheroidal basis reaches **99.1% of D_e**, and re-conditioning that basis (v5.12.9, `geovac/prolate_recondition.py`) climbs it to **99.81% (0.32 mHa, chemical accuracy) at the variational optimum α=1.40** — 99.77% (0.41 mHa) at the fixed-α=1.0 ladder endpoint the table prints (v5.13.9) — against Level 4's 96.0% at l_max = 6 with a Schwartz cusp correction; an independent Gaussian route gives 99.10%, and accurate grid-based prolate-spheroidal two-electron H2 methods exist in these same coordinates (the FE-DVR treatment of Tao-McCurdy-Rescigno, *Phys. Rev. A* **82**, 023423 (2010); the specific "99.97%/0.05 mHa" ground-state figure once quoted here could not be verified against that photoionization paper and was dropped 2026-09-20 — see CHANGELOG v5.14.11). What Level 4 retains, independently of the withdrawn claim, is *structural*: it is the exact N-electron generalization (SO(3N), S_N antisymmetry) that Level 4N and the composed Level 5 build on, and its cusp sits at a geometry-independent location. Those are architecture properties, not an accuracy advantage, and the hierarchy should not be read as asserting one here. **Decided (PI, 2026-09-17): H2's natural geometry is Level 2 (prolate spheroidal), alongside H2+, where its best result (99.97% of D_e / 0.053 mHa via explicit-r₁₂ CI with exact algebraic integrals, Paper 12 Sec. "Explicit correlation"; 99.81% / 0.32 mHa via re-based CI at α=1.40) is obtained; H2 is now listed primarily at Level 2.** Level 4 is retained as the structural N-electron (SO(3N), S_N antisymmetry) generalization that Level 4N and Level 5 build on; its 96.0% hyperspherical H2 result is documented as that structural treatment, not H2's leading geometry. See CHANGELOG v5.11.18 / v5.12.9, Paper 12 Sec. "Restoring the Azimuthal Channels" and "The monomial cap is conditioning, not a ceiling".
...
**Guard-writing is a separate, separately-reviewed activity — not a step in the remediation (hard, added 2026-09-04, PI direction).** Do not write a regression guard in the same pass as the fix it protects. Complete the fix, then write the guard as its own piece of work, and review it by asking **what wrong answer would this accept?** rather than *does it pass?* Every new or rebuilt guard is fire-tested against the specific wrong answer its claim excludes (`python debug/qa/fire_test.py`), and a guard whose rejected answer cannot be named is not a guard. *Why, measured:* about ten guards in this corpus could not fail, found across DELTA #4, FULL #4, DELTA #5 and DELTA #6. Several were **replacements for guards that could not fail**, written by an author who had just been shown the failure mode; one was `test_cb_norm_is_one`, still `1 == 1` after two rebuilds; one was the `fire_test.py` selftest added to prove a cache fix worked, which could not detect that fix being reverted. The cause is structural rather than careless — a guard written in the same pass as the fix inherits the fix's reasoning, so it encodes the belief instead of testing it. Note the asymmetry in the same record: every substantive *mathematical* claim of that cycle survived independent re-derivation, while nearly every *guard* written to protect one did not.
...
| 5 (Natural Geometry Hierarchy) | Numeric result cells ONLY (narrowed 2026-09-18, PI-directed) | Update a MEASURED result in the table's result column, or the same number where a note restates it (e.g. 99.77% → 99.81% when the optimum is re-measured). NEVER add/remove/renumber levels, change a coordinate system, or edit the framing of §5's notes — those remain prohibited by the §13.5 preamble above and §13.8 below. |
