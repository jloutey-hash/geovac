...
| Paper 31 | `paper_31_universal_coulomb_partition.tex` | Universal/Coulomb partition of the GeoVac spectral triple. A/D decomposition: universal sector (angular sparsity, composed scaling, selection rules) lives in algebra A and transfers to any spherical-fermion system without re-derivation; potential-specific sector (spectrum, calibration κ, transcendental content, rigidity, α-conjecture) depends on D and requires per-system rederivation. Three-layer Coulomb/HO asymmetry from Sprint 5 (spectrum-computing role of L_0; calibration π; non-abelian Wilson gauge). Frames Papers 22+23+24+25 in spectral-triple language; structural reorganization, not new theorem. |

(Paper 2 moved to Observations 2026-05-02; see `papers/observations/paper_2_alpha.tex`.)
...
| Paper 4 | `Paper_4_Universality.tex` | **Archived 2026-05-03.** Universal holographic central charge / muonic-hydrogen / proton-radius-puzzle resolution claim. Archived because (1) the proton radius puzzle is now resolved as a measurement artifact in older electronic spectroscopy (PDG 0.8409(4) fm; April 2026 review "the puzzle is no more"), and (2) the holographic / spectral-dimension claims inherit the machinery retracted from Paper 3. Salvageable kernel (mass as unit-fixing projection, topology mass-independent) absorbed into the Klein-Gordon / observation-projection thread (Paper 34 + chat memo). |
...
| Paper | File | Key Result |
|:------|:-----|:-----------|
| Paper 2 | `paper_2_alpha.tex` | α observation: K = π(B + F − Δ) matches 1/α at 8.8×10⁻⁸. Three structurally independent spectral homes for B, F, Δ derived. Combination rule itself is a numerical observation, not derived. Statistical validation p = 5.2×10⁻⁹ from 1.92e9 formula search. Moved core → observations 2026-05-02 per audit memo. |
...
| Paper 34 | `paper_34_projection_taxonomy.tex` | **Living catalogue.** Two-layer framework: bare combinatorial graph (Layer 1, no physics) + finite family of named projections (Layer 2, each adds specific variables, dimensions, transcendentals). **Fifteen projections** documented with three-axis tagging (variable / dimension / transcendental). The 13th (Drake-Swainson asymptotic subtraction, sprint LS-4 May 2026) sits in Paper 18's flow tier. The 14th and 15th (rest-mass projection and observation/temporal-window projection, sprint KG May 2026 / Paper 35) split the previously implicit mass-vs-observation content along the transcendental axis: rest-mass is ring-preserving, observation injects 2π via Matsubara. Empirical matches catalogue (§V) with companion off-precision matches and error-source classification (§V.B): T=truncation, B=basis quality, A=approximation order, C=calibration mismatch, S=structural floor. Falsifiable prediction §VI: error compounds with projection depth. **Maintenance protocol:** when a sprint produces a new machine-precision match against standard physics, append a row to §V; when a sprint produces an off-precision result, append a row to §V.B with error-source code; when a new projection mechanism is identified with literature support, append to §III/§IV. PMs may make these appends autonomously per §13.8; do not assert structural identifications beyond what the producing sprint verified. |
| Paper 35 | `paper_35_time_as_projection.tex` | Klein-Gordon spectrum on S³ × ℝ is π-free in ℚ[√d for d square-free] for every rational m² (KG-1 sprint, 200 cases verified symbolically). π enters the spectrum at exactly one step: temporal compactification, via the Matsubara mode 2πk/β; first π-bearing eigenvalue is the (n=0, k=1) mode at ω² = 4π² (KG-2). Spatial Casimir on unit S³ for the conformally coupled massless scalar = 1/240 EXACT RATIONAL, no transcendental at any step (KG-3); Stefan-Boltzmann π²/90 in the high-T limit on S³ × S¹_β enters only via the Matsubara sum. Spatial Dirac Casimir = 17/480 EXACT RATIONAL in full-Dirac convention, matching Camporesi-Higuchi 1996 to 40 dps (KG-5, spinor sector confirmed). Proposes the Paper 34 dictionary split into rest-mass projection (ring-preserving) and observation/temporal-window projection (injects 2π). Falsifiable prediction: a GeoVac observable contains π iff its evaluation includes a continuous integration over a temporal/spectral parameter promoted from the discrete graph spectrum. K-CC sprint (May 2026) closed possibility (i) of §VII.2 (K is single CC spectral-action coefficient) with a clean triple negative — T9 forces ζ_{D²}(s) at integer s into √π·ℚ ⊕ π²·ℚ, F = π²/6 cannot appear at any integer s. Sprint provenance: debug/kg{1,2,3,5}_*.py + debug/kcc_*.py + memos (May 2026). |
| Paper 36 | `paper_36_bound_state_qed.tex` | Bound-state QED on Dirac-S³ — one-loop closure of the hydrogen Lamb shift. Final result 1052.19 MHz vs experimental 1057.845 MHz, residual −5.65 MHz / −0.534%, no fits, no calibration against multi-loop literature. Consolidates the LS-1..LS-7 sprint sequence: §III architecture (Dirac-S³ + Sturmian projection at λ=Z/n), §IV Bethe log (velocity vs acceleration vs Drake-Swainson), §V Eides §3.2 convention reconciliation (identifies LS-1 Uehling-kernel double-counting; the +27.13 MHz shift is exactly 4/15 × α³Z⁴/πn³, the Uehling kernel constant), §VI result and component breakdown including LS-7 reframed residual decomposition (~+1.20 MHz multi-loop + ~+4.4 MHz non-loop), §VII LS-7 outlook with Proposition 1 sharpened (multi-loop test target ~+1.20 MHz; LS-8a strong test). Sub-percent closure confirms the framework's internal completeness for one-loop QED on S³ even more cleanly than originally framed. Sprint provenance: debug/ls{1,2,3,4,5,6a,7}_*.md. |
...
(Paper 4 moved to Archive 2026-05-03; see entry in §Archive table above. Reason: proton radius puzzle resolved by measurement; holographic claims inherit Paper 3 retraction.)
...
| Projection taxonomy (fifteen projections, three-axis tagging) | 34 | Sec III-IV | Observation |
