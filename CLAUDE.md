...
- S_min irreducible constant confirmed (v2.19.4): the CG-weighted two-loop sunset sum S_min = Σ_{k≥1} T(k)² ≈ 2.47954 (100 digits computed at 150 dps) is confirmed irreducible — 15 PSLQ attempts across 47 basis elements (including polylogarithms, Euler sums, double Hurwitz zeta, polygamma, and cross-products of all known constants) ALL FAILED. S_min is a depth-2 multiple Hurwitz zeta at half-integer shift a=3/2, intrinsic to the Dirac spectrum on S³. Lives at the intersection of all three Paper 18 taxonomy axes (operator order + vertex topology + CG weighting). Key files: `debug/smin_identification.py`, `debug/smin_identification_memo.md`, `debug/data/smin_identification.json`.
- Three-loop QED on S³ infrastructure (v2.19.4): new module `geovac/qed_three_loop.py`. Iterated sunset topology: three electron lines connected by two photon lines in chain, with SO(4) vertex selection at each vertex. O(n_max^5) computation. Unrestricted factorizes as D(4)³ (purely π^{even}). CG-weighted sum EXCEEDS unrestricted at n_max≥15 (W=2 amplification). Vertex-restricted converges slowly (78% of D(4)³ at n_max=30). PSLQ identification not yet possible (insufficient convergence). Depth-3 MZV prediction structurally motivated but computationally unverified — needs n_max≈50+ or analytical decomposition. 18 tests (15 fast, 3 slow) in `tests/test_qed_three_loop.py`. Key files: `geovac/qed_three_loop.py`, `debug/qed_three_loop_memo.md`.
...
| Paper 28 | On-topic | QED on S³: operator-order × bundle × vertex-topology transcendental taxonomy, S_min irreducible constant, Catalan G and Dirichlet β from two-loop parity |
...
| QED three-loop | `geovac/qed_three_loop.py` | `three_loop_sunset_s3()`, `three_loop_unrestricted()`, `three_loop_vertex_restricted()`, `three_loop_cg_weighted()`, `three_loop_convergence_table()`, `decompose_three_loop()` |
