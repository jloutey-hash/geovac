...
- **`/qa group1` FIRST WHOLE-GROUP run (chunked panel) FAIL→remediated (2026-06-23, v4.48.0):** one-invocation cert of all 14 group1 papers + synthesis via the new granularity rule (deterministic whole-group layer FIRST — all 7 PASS; then code 1/paper, claims/citation chunked, synthesis + completeness-critic; ~21 agents in waves, no rate-limit failures). Calibrated **10/11 sens, spec clean** — the one miss is the **SYNTH plant** (the synthesis reviewer read past an "established bound" lead-in, reassured by a retraction 12 lines below ⇒ that dimension UNDER-CALIBRATED; re-cert needs a sharpened synthesis prompt, the run-#4 K-sentence lesson re-applied). FAIL on one recurring CLASS + 2 code zombies, all remediated: the withdrawn **√(1−1/n_max) "C3^op" envelope constant** (v4.40/4.41 collapsed it to C₃=1 in p45/p46 but never transported) live in **p47** (Thm `inner` + Thm `g2_metric`) + **synthesis** `eq:p46_main` → C₃=1 corpus-wide (+ p47 g2_metric rate mass-max→Berezin); **p53 plane-test docstring** mis-attributed the −0.13 positivity-fail to Cesàro → heat weight; **p45 `lorentzian_theorem_statement()`** returned the retracted weak-form convergence theorem + a pinning test → rewrote to the K⁺-annihilation headline + guard. **C16 extended** with `withdrawn-c3op-envelope-sqrt` (the √ form its Pythagorean pattern missed — exactly why the class re-surfaced). Compiles clean; C16 + tests green; whole-group orchestration validated. Re-cert precondition: a clean confirmation run with the sharpened synthesis reviewer. See CHANGELOG + debug/qa/group1_whole_run_notes.md. **(v4.48.1 re-cert prep:** p29 Cor `int_alg` keystone-precision gap backfilled with `tests/test_paper29_int_alg.py`; **claims-reviewer** synthesis rule sharpened — enumerate-and-quote every descope/retraction status clause, a nearby retraction does NOT cure a live-sounding lead-in. Flavor-B `debug/`-path migration deferred — a 13-driver/8-test `wh7_b*` cluster = dedicated sprint, NIT not a cert blocker.)** **(v4.48.2 CONFIRMATION re-cert = FAIL→remediated, still not certified:** deterministic all-PASS; calibration 9/10, spec clean; **the SYNTHESIS calibration RECOVERED** — the sharpened reviewer caught its lead-in seed despite the retraction below (v4.48.1 fix worked). But 3 new latent SMALL defects (fresh-adversary lesson): p45 module-docstring inner-block retracted-thm-as-live → inline WITHDRAWN; p29 int_alg cited a non-existent factor 4s²+1 (live S⁵ factor is (2s²+1)², verified) → corrected + test tied to the real factor; p47 §1 G1 "closed"→"descoped". **claims-C MISSED its p46 descope seed** (per-paper C14 had the synthesis-only sharpening) → claims-reviewer rule generalized to per-paper C14; C16 extended to scan the geovac lorentzian code-docstring (blind spot code-45 named). Convergence: zero-LARGE, all-SMALL, two calibration/blind-spot mechanisms closed. See CHANGELOG + debug/qa/group1_recert_run_notes.md.)** **(v4.48.3 re-cert #2, hardened detectors = FAIL→remediated, still not certified:** **calibration PERFECT 10/10** — claims-C CAUGHT the p46 descope seed it MISSED last run (generalized per-paper C14 rule works) + synthesis recovery held; **claims/synthesis/citation dimensions MATERIAL-CLEAN** (first time). The 4 residual defects are ALL the same SMALL code-docstring/cross-paper-consistency "stale-prose one-scope-over" class (p45 sibling docstrings, p29 test docstring, gh_convergence "propinquity"→state-space-GH labels, p48 T5 Π_W-odd→mixed-parity) + 1 test-hygiene (test_paper50 global-dps pollution) → all swept comprehensively; gates PASS, 47 tests green. **Trajectory across 3 whole-group runs: severity LARGE→SMALL→SMALL-docstring-only; calibration 9/10→9/10→10/10; LLM-judgment dimensions → clean.** Cert-bar decision now with PI (iterate-to-zero vs set bar at calibrated/zero-LARGE/judgment-clean/deterministic-green + code-docstring-stale-prose-as-NIT). See CHANGELOG + debug/qa/group1_rc3_run_notes.md.)**
- **/qa hardening — C16 retracted-claims deterministic gate + enumeration-forced reviewer doctrine (2026-06-23, v4.47.0):** the fix for the recurring lesson (each cert kept surfacing latent zombies the prior panel walked past). Diagnosis: every recurring miss was a **mechanical** drift in a low-salience region (table cells, docstrings, footnotes, one abstract clause) — NOT a judgment failure (the withdrawn Pythagorean C₃<1 form survived **three** incomplete sweeps). Fix moves those classes out of judgment-mode LLM review into cheap deterministic checks: **(1)** new `debug/qa/check_retracted_terms.py` (C16) + a `REGISTRY` of every retired phrase (withdrawn Pythagorean, S⁷ structural-negative, Batch-2 false-closure, propinquity-as-metric) with per-entry withdrawal-flag exemptions — exhaustive, zero-variance, runs every /qa for ~0 tokens; **it caught 2 residual Pythagorean comment-zombies on its first run** (gh_convergence_tensor.py l.1283/1742, fixed). **(2)** qa.md + criteria.md: enumeration-forced reviewer prompts (every table row/bibitem/tier-term/docstring, no sampling) + a one-agent completeness-critic pass + loop-until-dry reserved for the final convergence run. **Maintenance rule:** when a /qa run retires a claim, add its phrase to the C16 registry ([[feedback_deferral_is_churn]]). Backed by tests/test_retracted_terms_check.py (non-vacuity locked). All 6 gates PASS group1. See CHANGELOG.
- **`/qa group1` Batch 3 confirmation FULL re-cert (post-v4.45.0) FAIL→remediated — fresh adversaries caught a NEW S⁷ zombie (2026-06-23, v4.46.0):** the clean re-cert the v4.45.0 preconditions unblocked. 16 reviewers + fresh synthesis re-dispatch + 5 gates; **calibrated 5/5 sens, spec clean** (5 fresh seeds; the v4.44/4.45 fixes held as M1–M6 controls; the synthesis seed was missed by the first synthesis reviewer, caught by a focused re-dispatch). FAIL on a **NEW LARGE** the first cert missed: **(1) the S⁷ zombie** — Paper 50's catalogue table (l.1031 "PSLQ fails/UNKNOWN") + wall-list (l.1253) AND the synthesis (l.1709) carried a withdrawn S⁷ "structural negative" that Paper 50's own §8 Erratum retracted (false negative; ladder generates S³–S¹¹ at ≥200 dps) → fixed corpus-wide (catalogue→DONE in-ring R₇, wall→squashed-S³, synthesis→"S⁷–S¹¹ ladder closure"). Plus SMALL: **(2) code-39** 3 more residual "Pythagorean" mechanism docstrings in gh_convergence_tensor.py (v4.44.0 sweep still incomplete) → triangle+WITHDRAWN; **(3) claims-40** main-thm 4.13(ii) inline tier + L3-interior "all ranks" split (PRV rank-uniform vs INT-leg F₄–E₈ named follow-on); **(4) claims-50** "provably outside the ring"→numerical-disjointness. Compiles clean; gates PASS; topo 18/18. **Lesson: fresh adversaries each pass catch latent zombies the prior panel walked past.** Batch 3 needs one more clean full re-cert to certify. See CHANGELOG + debug/sprint_qa_group1_batch3_cert_memo.md.
- **`/qa group1` Batch 3 re-cert preconditions RESOLVED (2026-06-23, v4.45.0):** all four named blockers from the v4.44.0 FULL-cert FAIL fixed. **(A, LARGE) p40 general-G test-backfill** — migrated the backing drivers out of prunable `debug/qa/_resurrected/` → permanent `tests/rank2_rate_support/`; **wired the previously-uncalled `verify_dirac_triangle` into a DEFAULT general-G C₃≤1 test** (SU(3)/Sp(2)/G2 reduced panels, fail_count=0) + un-slowed Haar; kept big panels `@slow`; matrix 113 NO-TEST→BACKED-SOUND. Surfaced an honest-scope nuance (all-σ triangle is panel-bounded; the validated `@slow` test uses group-specific Casimir bounds; PRV/max-σ bound is what C₃ needs). **(C) p29 algebraic-integer math** (verified MATERIAL) — the Ihara *zeros* are algebraic over ℚ but NOT algebraic integers (non-monic Bass det; `4s²+1`→±i/2); the **reciprocal** zeros (Hashimoto T-eigenvalues, `s²+4`→±2i) are — corrected Cor `int_alg` + 2 synthesis spots. **(B) p29 title** reframed (PI choice) → "Finite-Size Graph-RH for the GeoVac Hopf Graphs: …" + corpus-wide bibitem sweep (C11 PASS ALL groups). **(D) p40 "Vinberg's lemma"** re-attributed → standard Weyl-dominance + Bourbaki. Compiles clean; all gates PASS; topo 18/18 + p40 3/3 default. Batch 3 ready for a clean full re-cert. See CHANGELOG + debug/sprint_qa_group1_batch3_cert_memo.md.
- **`/qa group1` Batch 3 FIRST FULL cert (P29/39/40/50/52 + synth) FAIL→remediated — defects in ALL FOUR LLM dimensions (2026-06-23, v4.44.0):** first all-5-dim cert of Batch 3. 16 reviewers + 5 gates, calibrated **6/6 sens, spec clean** (one seed per dimension). FAIL — genuine non-seed MATERIAL defects across code/claims/citation/synthesis. **Fixed:** (1) **synthesis fabricated Paper-29 "Theorems 5.1–5.2/Cor 5.3" citations** + Observation→theorem promotion (Paper 29 has NO theorems; S³ crosses at n=5) → demoted to finite-size Observation; + p40 universality tier-caveat; (2) **p29 Observation 1 over-scope** (contradicted its own §7) → scoped to small cutoffs; (3) **completed the v4.43.5 code-side-zombie sweep in `gh_convergence_tensor.py`** (module/L3_T/class/epsilon_cross docstrings withdrawn-Pythagorean + "Latrémolière propinquity" → triangle √2 + state-space GH + WITHDRAWN) **+ the zombie-pinning test** (`assert "Pythagorean" in L3_T` → triangle/WITHDRAWN + guard); (4) citations (p40 Leimbach-vS propinquity→state-space GH, p39 aguilar venue). **Raised to PI (named re-cert preconditions):** **p40 general-G universality test-backfill** (LARGE, confirmed v4.43.5 Flavor-B — the general-G C₃=1 verifier's `run_panel` is never called, rank-2 tests `@slow`-skipped, per-group table values untested/G2 live 1.695≠paper 1.177); p29 title (editorial); p29 "algebraic integer" math point; p40 Vinberg missing ref. Compiles clean; gates PASS; topo 18/18. See CHANGELOG + debug/sprint_qa_group1_batch3_cert_memo.md.
- **`/qa group1` Batch 2 CODE-DIMENSION-only validation — v4.43.6 fixes HELD, code MATERIAL-clean (2026-06-23, v4.43.7):** PI-directed fresh adversarial pass on the two v4.43.6 code-touching fixes (the §11 honest cap — they hadn't been through fresh review). 4 code-reviewers (P42/43/44/53), RAN the tests; **validation pass, NOT a cert** (1 of 5 dims ⇒ INCONCLUSIVE-for-cert). Calibrated **4/4 sens** (4 fresh seeds caught, one vacuous-tol/tautology per paper), **spec clean** (M1–M5 respected). **Both v4.43.6 fixes confirmed sound under fresh review:** code-43 affirmed the mixed-parity §5.2 proof (temporal γ⁰ odd / spatial $\DGV^W$ diagonal-$\Pi_W$-even-sign-paired) matches §5.3; code-53 **recomputed** and confirmed the Cesàro-preserves-positivity / heat-fails / obstruction-is-the-non-decaying-rate framing. **Zero genuine non-seed MATERIAL defects** — every reviewer fix-rec targeted a seed; real corpus carries the correct tight tolerances at all 4 sites. NITs logged (production verdict-gate looseness backstopped by the integer-spectrum test; six-witness test checks scalar-not-Δ_L/K_TT [claim verified true]; 18-cell panel from an archived driver; M3 tautology mirrors a weak observation; wedge Prop 9.2 slow-only). No edits this run. See CHANGELOG + debug/sprint_qa_group1_batch2_cert_memo.md (§12).
- **`/qa group1` Batch 2 rc2 confirmation re-cert FAIL→remediated — the CODE dimension caught two paper-BODY proof/claim defects (2026-06-23, v4.43.6):** full 5-dim re-cert from v4.43.5, hoping the v4.43.4 sweeps left a PASS. 13 reviewers, calibrated **5/5 sens, spec clean** (fresh C5 K-seed → claims-42; M1–M5 respected). FAIL on **2 genuine MATERIAL defects, both caught by CODE** (running the operators / both weights), both accepted by claims+citation+synthesis: **(1) Paper 43 §5.2 Theorem 5.7 Input I2** ("$\DL^W$ is $\Pi_W$-odd") is FALSE for the spatial Dirac — $\DGV^W$ is diagonal/$\Pi_W$-**even** (eigenvalue $\chi(n+\tfrac12)$), contradicting I2 + the paper's own §5.3; orthogonality holds via chirality sign-pairing (temporal $\gamma^0$ IS odd) → corrected I2 to mixed parity + new `lem:pairing_HS`, transported to synthesis; **(2) Paper 53 §4 positivity leg** — "Cesàro weight fails positivity ($\min g≈-0.13$)" is FALSE: Cesàro (Def 4.1, s≥2) PRESERVES positivity ($\min g>0$), the $-0.13$ is the **heat** weight (Gibbs); genuine obstruction = non-decaying rate (weight-robust) → corrected all positivity statements + rewrote the test (now 2 tests: Cesàro-positive/heat-fails + rate-no-decay). Both in the paper-body layer the prior claims/citation-heavy runs never ran against live machinery. citation-43 BBB-sign flag = ar5iv artifact (ε=+1 forced by code-verified $J^2=+I$), no edit. Compiles clean; gates PASS; 35 tests green. See CHANGELOG + debug/sprint_qa_group1_batch2_cert_memo.md (§11).
- **`/qa group1` Batch 3 CODE-DIMENSION validation pass — new flavor "code-side zombies" caught + swept corpus-wide (2026-06-22, v4.43.5):** PI-directed code-only run (Papers 29/39/40/50/52) to catch new sweepable flavors. 5 code-reviewers, calibrated **4/4 sens**, spec clean; **validation pass, NOT a cert** (1 of 5 dims). **NEW FLAVOR A (headline): code-side withdrawn-claim zombies** — withdrawn/false claims living in production-code function STRINGS/DOCSTRINGS (invisible to tests), the code analog of the Batch-2 paper zombies: `gh_tensor_theorem_statement()` returned the withdrawn Pythagorean "C₃<1→1" + Paper-38-as-"Latrémolière propinquity" mislabel. Swept the contained `gh_convergence{,_tensor}.py` pair (5 sites: theorem-statement fns + L3/L5 docstrings → triangle bound ≥1→√2, state-space GH) **+ a sub-flavor: 2 tests that ASSERTED the zombie keyword's presence** (`assert "Latremoliere" in s`) → corrected to assert "state-space"/"van Suijlekom" + guard the zombie can't re-enter. **Flavor B (flagged): load-bearing backing on a prunable `debug/qa/_resurrected/` path with skip-on-absence** (p40 rank-2 universality + the general-G C₃=1 verifier whose `run_panel` is never called → genuine coverage gap) — named backfill follow-on. Flavor C (p52 "61+ digits") dissolved on reconcile (true measured claim; test gate just loose). Strings/docstrings only — no logic change; affected tests green. See CHANGELOG + debug/sprint_qa_group1_batch3_code_memo.md.
- **`/qa group1` Batch 2 FRESH re-cert (full 5-dim) FAIL→remediated — two defect CLASSES swept corpus-wide (2026-06-22, v4.43.4):** confirmation re-cert from v4.43.3. 13 reviewers, calibrated (5/5 sens, spec clean). 2 genuine material defects, both **the same classes re-surfacing in new locations**: (1) the false-closure framing ("$D_{CH}$ would not produce the bit-exact closure") found AGAIN in the **p42 abstract** + **Paper 32** (after v4.43.0 fixed the §5.5(II) body and v4.43.3 fixed the synthesis) → **corpus-wide grep sweep, now CLEAN**; (2) p43 §5.2 closed-form $r^2=\kappa_g^2 S(n)/4\pi^2+D(n)$ + $1/\pi^2$ + PSLQ had no test (the v4.43.2 backfill tested only ⟨H,D⟩=0/Pythagorean) → new `test_pythagorean_residual_closed_form` (verifies $\|H\|^2\cdot4\pi^2=S(n)$, $\|D\|^2=D(n)$ rational). NITs drained: connes_rovelli end-page, witness-pair ~38% magnitude pinned (was `>1e-6`), cross-witness named-test caveat. Compiles clean; gates PASS. **Lesson: fix defect CLASSES corpus-wide, not instances — re-certs kept finding the same class elsewhere.** See CHANGELOG + debug/sprint_qa_group1_batch2_cert_memo.md (§rc-fresh).
- **`/qa group1` Batch 2 RE-CERT (post-backfill, full 5-dim) FAIL→remediated — code dimension clean, claims/citation/synthesis carried it (2026-06-22, v4.43.3):** first full cert after the v4.43.2 backfill. 13 reviewers, calibrated (5/5 sens, spec clean); the backfill HELD (code surfaced no new defects beyond seeds). 4 genuine material fixes: **(1) synthesis false-closure** "$e^{i2\pi n}=1$ would fail with $D_{CH}$" → operator-level $-I$ vs $+I$ distinction (the v4.43.0 §5.5(II) sweep had missed the synthesis); (2) p53 descoped Paper 45→40 as compact-carrier precedent (×2); (3) p42 van den Dungen missing `\bibitem` added; (4) p42 "Prop. 4.2"→"§4" (consistency w/ p44). NITs drained: p43 theorem residual 4e-16→7e-15 (matched its tables), p42 §5.4 propinquity→state-space GH, **code-42 §5.5(II) coverage gap CLOSED** (new operator-level-lift test). Compiles clean; gates PASS; 81 passed/4 slow-skip. See CHANGELOG + debug/sprint_qa_group1_batch2_cert_memo.md (§re-cert).
- **group1 Batch 2 coverage-gap test-backfill — all 4 code gaps closed, BACKED-SOUND (2026-06-22, v4.43.2):** PI-directed backfill (the named re-cert precondition) before the Batch-2 re-cert. Recompute-from-framework tests, validated against the live machinery (not hardcode-and-assert): `tests/test_paper53_disk_obstruction.py` (disk positivity-fails min g<−0.10 at Λ=200 + rate slope>0 no-decay; prop2 O² fills $M_N$ at 4 cells via live `operator_system_dim`); `tests/test_paper43_pythagorean_hs_orthogonality.py` (§5.2 ⟨H,D⟩=0 + Pythagorean + chirality pairing, 7 cells); p53 plane test **de-slowed** (runs by default ~1.5s); p42 cross-witness collapse **hardened** (bit-identical ρ/Δ/K_TT across distinct-β witnesses + β-cancellation negative control, 9-order separation). Paper citations repointed debug/→tests/; claim_test_matrix Batch-2 rows → BACKED-SOUND. C13 PASS; p43/p53 compile clean; 80 passed/4 slow-skip. See CHANGELOG + debug/sprint_qa_group1_batch2_cert_memo.md (§backfill).
- **`/qa group1` Batch 2 CODE-DIMENSION re-validation — v4.43.0 fixes HOLD, code MATERIAL-clean beyond known gaps (2026-06-22, v4.43.1):** PI-directed narrow re-run of only C1/C2 (4 code-reviewers, 4 fresh vacuous-tolerance seeds, one per backing test); **validation pass, NOT a cert** (1 of 5 dimensions ⇒ INCONCLUSIVE-for-cert). Calibrated **4/4 sens**; specificity confirmed the v4.43.0 fixes hold — code-42 **independently recomputed §5.5(II)** and verified the corrected prose MATCHES the computation ($e^{i2\pi D_W}=-I$, conjugation flow closes ~1e-16); p44 prop=2 confirmed genuinely computed (not hardcoded); p43 deleted-debug citation gone. **Zero new MATERIAL code defects** beyond the 4 seeds + the already-logged coverage gaps (p43 §5.2 Pythagorean panel; p53 disk-negatives + prop2 + slow-gate). One NEW NIT drained: a spurious β in the v4.43.0 §5.5(II) formula ($e^{i2\pi\beta D_W}$→$e^{i2\pi D_W}$, caught by code-42's recompute). Compiles clean. The coverage-gap backfill remains the named re-cert precondition. See CHANGELOG + debug/sprint_qa_group1_batch2_cert_memo.md (§code-validation).
- **`/qa group1` Batch 2 FULL cert (P42/43/44/53 + synth) FAIL→remediated — code dimension carried it (2026-06-22, v4.43.0):** first all-5-dimension cert of the Lorentzian-arc FOUNDATION papers (prior rc2/rerun2 were claims+citation only). 13 reviewers, calibrated (5/5 sens, specificity clean). The CODE dimension (first full exercise on Batch 2) carried the FAIL — and **overturned a control**: p42 §5.5(II) "$D_W$ does not close the period closure" is FALSE (conjugation by scalar $-I$ closes; the real distinction is operator-level $-I$ vs $+I$ double-cover, not flow-closure) → corrected. Plus p43 §5.2 deleted-debug citation neutralized; p42 verch2001 descriptor + p44 Connes–vS "Prop 4.2/4.3"→"§4" citation fixes; p42 l.495/1650 propinquity→state-space GH NITs. **3 coverage gaps logged** (p43 Pythagorean numeric panel; p53 disk-negatives + prop2; p53 slow-gate) → test-backfill sprint is the named re-cert precondition. Compiles clean; gates PASS. See CHANGELOG + debug/sprint_qa_group1_batch2_cert_memo.md.
- **`/qa group1` rem3 CODE-DIMENSION-ONLY validation pass — first MATERIAL-clean dimension (2026-06-22, v4.42.1):** PI-directed narrow re-validation of ONLY the code/test-backing dimension (code has been the high-yield dimension twice — caught the p45 fabrication in rem1 + the production wiring in rem2 that claims accepted both times). 5 code-reviewers, 4 vacuous-tolerance seeds (one per paper's backing test); **validation pass, NOT a cert** (1 of 5 dimensions ⇒ INCONCLUSIVE-for-cert). Calibrated **4/4 sens** (graded by materiality — NIT where a sibling assertion backstops, MATERIAL where not); specificity held — **the rem2 code fixes confirmed holding** (production `c_lipschitz_joint = 1.0`, √-formula unwired, envelope-max tight). **ZERO new MATERIAL code defects — the first pass MATERIAL-clean on its dimension.** Drained 3 recurring test-hygiene NITs (gamma_su2_residual tautology comment [3rd flag], `paper_38_bound`→`per_harmonic_bound` √-mis-attribution rename, asymptotic_rate docstring C₃=1). Logged for the next claims pass: p49 "joint propinquity rate" label = SU(2) factor. Tests pass; C5/C13/C14 PASS. Three laps (rem1→rem2→rem3) converged the code dimension to MATERIAL-clean, each narrower than the last. cert6 (full, w/ citation) certifies. See CHANGELOG + debug/sprint_qa_group1_cert_rerun1_memo.md (§rem3).
- **`/qa group1` rem2 second TARGETED remediation pass — rem1 Paper-45 transport found INCOMPLETE (2026-06-22, v4.42.0):** PI-directed 2nd targeted re-run (claims+code+synthesis+deterministic; citation dropped again); **remediation pass, NOT a cert**. 11 agents, calibrated 4/4 sens, specificity held (rem1 p46 fix + cert4/5 fixes confirmed holding). Validated the rem1 Paper-45 C3 transport and found it **incomplete — 3 residual sites**: p45 L4(d) proof (l.1131 `C_3^(N)`→C₃=1), p45 Table 1 (l.1570–1575 + caption → C₃=1; the same Table-1 class rem1 fixed in p46 but never checked in p45), and the production `c_lipschitz_joint` wiring (build() stored the √-value → now C₃=1; test → ==1.0). Plus 3 pre-existing descope-zombies prior sweeps missed: synthesis Headline-4 ("propinquity satisfies [→0]" lead-in, retraction not inline), p46 `rem:limit_identification_op`, p47 `rem:bit_exact_g2` (metric-value remarks w/o inline descope). +NIT: p46 p38-test-citation overstatement softened. The **CODE dimension again caught what claims accepted** (the production wiring). Compiles clean; gates PASS; test 33/2. cert6 (full, w/ citation) still certifies; next targeted pass re-validates rem2. See CHANGELOG + debug/sprint_qa_group1_cert_rerun1_memo.md (§rem2).
- **`/qa group1` rem1 TARGETED remediation pass (claims+code+synthesis, citation dropped) — 2 cross-paper cert5 misses caught (2026-06-22, v4.41.0):** PI-directed targeted re-run (the live defects all cluster in claims/code/synthesis; citation converged clean post-v4.31–4.33). 11 agents; **remediation pass, NOT a cert** (dropping citation ⇒ INCONCLUSIVE-for-cert by the all-dimensions rule). Calibrated 4/4 sens, specificity held; all cert5 fixes confirmed holding. Caught two genuine misses from the cert5 p46 rework: **(1) p46 Table 1 column** still printed √(1−1/n_max) (cert5 swept prose/eqs, missed the table) → fixed to C₃=1; **(2) Paper 45 carries the IDENTICAL C3^op fabrication** (eq:C3_joint_bound √→√(1−1/n_max) "Paper 38 L3 verbatim" + false-positive test) — never swept (cert5 was p46-only). code-45 caught it by running the machinery while **claims-45 accepted the prose as sound** (the two-dimension save — why keeping CODE in the targeted set mattered). Transported the p46 fix to Paper 45 (C₃=1 corpus-wide: eq + rem:envelope_v2 withdrawal + Q4 + production `c3_joint_panel_sup` docstring + test reframe). Compiles clean; gates PASS; tests green. cert6 (full, w/ citation) still certifies. See CHANGELOG + debug/sprint_qa_group1_cert_rerun1_memo.md (§rem1).
- **`/qa group1` cert5 Batch 1 (P45–49 + synth) FAIL→remediated — body layer + the p46 C3^op fabrication (2026-06-22, v4.40.0):** 16-agent panel, calibrated 5/5 sens, 5/5 spec. cert5 hypothesis confirmed — remaining zombies were in the paper BODY (the abstract/§1 sweeps couldn't reach them). SMALL fixes: p47 §5.4 (descoped inner-arrow panel "confirms the theorem at full machine precision" → rate-formula/degeneracy tag); p48 §1.2 (B1–B4 "theorem-grade" → partitioned); p48 §1-roadmap+cor (substrate-vs-rate T/4); synthesis (Umegaki-vs-D_max misattribution). **LARGE: p46 C3^op fabrication** — diagnostic (`debug/cert5_p46_c3op_diagnostic.py`) CONFIRMED the per-harmonic √((N−1)/(N+1)) / envelope √(1−1/n_max) is operator-norm-false (ratios 1.74/2.74/3.14, envelope-max monopole COMMUTES) + mis-attributed to Paper 38 L3 (actually C₃=1, gradient seminorm). PI-directed rework: collapsed C₃^op→C₃=1 corpus-wide in p46 (Lemma L3 / abstract / thm:main / L4 / L5-appendix / Appendix A), kept the envelope RANGE N≤2n_max−1 (true), archived the pure-sympy test → `_archive/dead_ends/`, added operator-system `test_paper46_c3_operator_system.py` (13/13). Feeds the DESCOPED rate; convergence unaffected. Theorem + abstract/§1 + body layers all now swept. Compiles clean; C5/C11/C13/C14/C15 PASS. **cert6 = first real shot at a clean PASS.** See CHANGELOG + debug/sprint_qa_group1_cert_rerun1_memo.md (§cert5).
- **group1 prose/abstract descope sweep + CLAUDE.md compaction round 4 (2026-06-22, v4.39.0):** cert4 follow-on draining the layer the v4.37.0 theorem-statement sweep didn't cover. Read all five P45–49 abstracts + §1 Introductions in full; **2 genuine fixes of 10 regions** — p48 abstract B2 (listed flatly under "theorem-grade rigor" though its own §1 calls B2 "structurally vacuous off-orbit" → B1/B3 structural vs B2 structural/vacuous vs B4 descoped) + p49 §1 honest-scope bullet ("Bridge Theorem holds at theorem-grade rigor" → per-leg "structural+thermal-stack survive; B4′ descoped"). P45/46/47 abstracts + P45/46/48 §1 + P49 abstract already clean. Both the theorem-statement (round 1) and prose/abstract (round 2) layers now swept; any remaining zombie is paper-body. Compiles clean; C5/C11/C13/C14/C15 PASS. **CLAUDE.md 165→135 KB** (round-4 §2 compaction: granular cert/historical one-liners → docs/development_frontier_archive.md). cert5 is the fresh-adversary validation. See CHANGELOG + debug/sprint_qa_group1_descope_sweep_memo.md (§Round 2).
- **`/qa group1` CERTIFYING cert4 Batch 1 (P45–49) FAIL→remediated — descope sweep HELD, prose/abstract zombies remain (2026-06-22, v4.38.0):** calibrated 5/5 sens, 5/5 spec; the v4.37.0 theorem-statement descope tags HELD (no theorem zombie re-surfaced). cert4 found the NEXT class — descope zombies in **§1-Introduction prose** (p47 program restatement: "establishes…converges in the propinquity"→descoped) + **abstract enumerations** (p48 "Main theorem" B4 item, distinct from cert3's §1.2 fix); a **thm:compact_agreement substrate-vs-rate over-reach** ("reduces bit-exactly" — but `gamma_l1(N_t=1)=gamma_su2+T/4`; only substrate+SU(2) factor bit-exact, joint rate carries the U(1) T/4 offset); a **p49 dangling driver** (`q1prime_phase2b3_panel_compute.py`→`tests/test_lorentzian_propinquity_foundation.py`); +NIT docstring retraction banner. **che_perales venue OVER-FLAG dropped** (web-confirmed published DGA 103 (2026)). Compiles clean; C5/C11/C13/C14/C15 PASS. **Still not a clean PASS — prose/abstract descope layer is the remaining tail; cert5 next.** See CHANGELOG + debug/sprint_qa_group1_cert_rerun1_memo.md (§cert4).
- **group1 Batch-1 cert arc cert1–cert3 + descope sweep (v4.27.0–v4.37.0, 2026-06-19→22):** cert re-runs cert1 (v4.30.0), the rerun-1b/cert C4 citation sweeps (v4.31.0/4.35.0), cert2 (v4.35.1), cert3 (v4.36.0), and the v4.37.0 inline theorem-statement descope-tagging sweep — all FAIL→remediated, all calibrated (≈5/5 sens, 5/5 spec), prior fixes held each pass. Convergence is **by class** (citations → theorem-statement descopes → prose/abstract descopes), not yet a clean PASS. Per-run chronicle: CHANGELOG + debug/sprint_qa_group1_cert_rerun1_memo.md + debug/sprint_qa_group1_descope_sweep_memo.md.
- **group1 Batches 2 & 3 re-runs FAIL→remediated + coverage backfill (v4.28.0–v4.34.0, 2026-06-19→21):** Batch 2 (P42/43/44/53) + Batch 3 (P29/39/40/50/52 + synth) re-cert re-runs incl. folded-in exhaustive C4 citation sweeps + a 4-gap recompute-from-framework coverage-test backfill (v4.34.0: p29 block-decomp/Alon–Boppana, p50 wedge-KMS/CHM). Each batch still needs its own clean cert re-run to certify. CHANGELOG + debug/sprint_qa_group1_rerun{2,3}_memo.md.
...
- **group1 bites A/B1/B2/B3 + Paper 29 all FAIL→remediated (2026-06-17/18, v4.21.3–v4.25.0):** calibrated panels per bite; recurring patterns — fabricated/wrong-ID math.OA citations, C7 Paper-38 "propinquity"→state-space-GH relabels, "Status-note-only descope" zombies (45–49); bite-B3 rescues (p39 C₃→√2, convergence survives; p53 boundaryless-plane). CHANGELOG + per-bite memos debug/sprint_qa_group1_bite*_memo.md.
...
- **QA gate + deterministic checks C5/C11/C13/C14/C15 + standing policies built across trunk + group3 cert arcs (v4.13.0–v4.22.1, 2026-06-14→17):** the /qa certification gate (pre-registered criteria + blind calibration seeds + fresh reviewers → PASS/FAIL/INCONCLUSIVE), the five deterministic gates, the resurrect-pruned + debug-citation policies, the corpus-wide internal-title audit, and the trunk QA (κ / 4·π / K-observation downgrades). Chronicle: docs/development_frontier_archive.md (round-4 §2 index) + CHANGELOG.
...
Seven slash commands defined in `.claude/commands/`:
...
The first four are genuine triggers (no corresponding memory rule, fired by PI at a specific moment). The last three are force-fire backups (their primary mechanism is a memory rule that should fire automatically; the slash command exists to manually invoke the discipline when the rule failed to trigger).
