...
- Vector-photon QED on the Fock graph (v2.26.1, May 2026): Adding explicit photon quantum numbers (q, m_q) with Wigner 3j vertex coupling recovers **7/8 continuum QED selection rules** — up from 1/8 (scalar graph), 4/8 (Dirac graph Rule B), and 5/8 (plaquette GS zero). Construction: photon modes (q=1..q_max, m_q=-q..q), propagator G_γ(q) = 1/[q(q+2)] (vector Laplacian on S³), vertex V(a,b,q,m_q) = 3j × parity(l_a+l_b+q odd) × R=1. Self-energy Σ(a,c) = Σ_{b,q,m_q} V·G_e·G_γ·V. **Pre-investigation:** L₁ = B^T B does NOT block-diagonalize by (q, m_q) — photon quantum numbers are transition labels (difference between endpoint labels), not conserved at vertices. Cross-sector Frobenius fraction: 20% (Fock), 57% (Dirac Rule B). Vector photon structure must be added from outside. **Selection rule census (7/8):** Gaunt/CG sparsity PASS (built into 3j), vertex parity / GS zero PASS (l_a+l_b+q odd; GS: 2l=even≠odd), SO(4) channel count PASS (triangle inequality), Δm/Δm_j conservation PASS (3j m-rule), spatial parity PASS (E1), Ward identity PASS at n_max=2 / FAIL at n_max=3 (truncation), charge conjugation PASS (Σ Hermiticity). **Furry's theorem FAIL** — sole irreducible obstruction, requires second-quantized C-symmetry (particle-antiparticle field structure), not just state labels. Dirac graph's earlier Furry "recovery" was a topological accident (no self-loops from Δl=±1), not the physical mechanism. **Scalar vs Dirac electrons give SAME 7/8** — CG coefficients do all angular momentum work; spinor labels provide no additional selection rule recovery beyond CG. Dirac gives 5× better Ward ratio at n_max=3 (0.61 vs 3.1) due to linear Dirac spectrum. **GS structural zero:** Σ(GS,GS) = 0 exactly at all n_max (triangle + parity forces 2l_int odd, impossible). **Calibration exchange constant:** l=1 self-energy at n_max=2 is EXACTLY 1/(4π) — the S² Weyl exchange constant. The scalar graph QED is π-free (ℚ[√2,√3,√6,...]); the vector photon introduces exactly one transcendental per loop: π, through the S² solid angle normalization. **The 1-7-8 partition:** 1 rule from graph topology (always survives) → 6 rules from angular momentum conservation (CG + E1 + triangle, costs exactly 1/(4π) per loop) → 1 rule from second-quantized field theory (Furry, requires C-symmetry of the Dirac field). Papers 28 and 18 updated. Key files: `geovac/vector_qed.py`, `tests/test_vector_qed.py` (99 tests), `debug/vector_qed_sprint_memo.md`, `debug/l1_photon_block_diag.py`, `debug/data/l1_photon_block_diag.json`.
...
| Vector QED Dirac+vector 7/8 rules | 7 of 8 PASS | Dirac electron + vector photon |
...
| Vector QED Furry FAIL | nonzero odd-q diagonal | Irreducible obstruction (needs C-symmetry) |
| Vector QED scalar = Dirac rules | identical 7/8 | CG does all angular work |
...
| Furry's theorem irreducible obstruction | 28 | §vector_photon_qed | Observation |
| Scalar vs Dirac electron equivalence (vector QED) | 28 | §vector_photon_qed | Observation |
