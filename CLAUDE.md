...
- **`/qa synthesis` in progress: 1st cert INCONCLUSIVE (setup error) → citation delta = DEFECTS→remediated (2026-07-04, v4.67.2) — re-delta pending.** 1st FULL run INCONCLUSIVE (worktree-from-HEAD while the field guide had uncommitted PI edits → claims dim reviewed stale text); field-guide re-run on correct text CLEAN (6/6 + 2/2 seeds). Citation-dim delta (PI-directed, clean=pass) = **DEFECTS**, calibrated 2/2: surfaced genuine wrong-arXiv/venue cites in the **certified group1 synthesis** its branch cert missed — `hekkelman2022` (2206.13744=a black-hole paper→2111.13865), `hekkelman_mcdonald2024` (2403.18619=an OpenMP paper→removed, flagged), `latremoliere_metric_st_2017` (Adv.Math 415/2023→404/2022), + 5 title/label SMALLs; all remediated (v4.67.2). Earlier: `latremoliere2018`→TAMS 368/2016, `glanois`→JNT 160/2016, D6 Paper-49 scoping, κ→Observation (v4.67.1). Re-run the citation delta next (clean=cert pass). See docs/qa/synthesis.done.md.
...
- **group5 Tier-2 backing sprints COMPLETE (2026-07-03, v4.65.0):** Furry derivation ported (21 symbolic tests); census de-tautologized; XCWG migrated (24 witness tests); P51 headlines pinned at paper scale + 27 tier tags; P30 Prop 3 corrected (co-exact completion, not ∝L1); CP² 24.98%→40.8% superseded; Bochner 2==2 → real operator identity; P2/P36 exact pins + Lamb chain ported; 6 claim corrections. Follow-up named: hodge1/P28 eigenvalue labeling. See debug/sprint_group5_1stcert_memo.md.
- **group5 delta-1 CLEAN (2026-07-03, v4.64.1):** 7/7 seeds + 2 cross-catches, specificity clean, zero genuine MATERIAL in the remediation diff; 12/12 new bibliographic facts verified GROUNDED; ~510k tokens. Certifying FULL run earned — PI timing vs the open Tier-2 backing sprints. See debug/sprint_group5_1stcert_memo.md.
- **group5 1st cert FAIL (calibrated 18/18+6/6) → Tier-1 remediated (2026-07-02/03, v4.64.0):** conjecture-idiom sweep (P25 ×14 + P33/P30/P2); P28 Wald factor-of-2 + parker repoint + petermann sign; P36 phantom bibitems + retracted-reading conclusion; P51 propinquity ×2 + citation layer; mp.dps hermeticity bug (624-green); graph_qed n_max≥4 crash fixed; fabricated test cite → real 1/(4π) pin; C13/C16 hardened. Tier-2 named; delta run next. See debug/sprint_group5_1stcert_memo.md.
...
- **WH8 registered + Born-measure probe NEGATIVE (PI-directed solo sprint, 2026-07-01/02, v4.61.0):** the skeleton's counting measure = Born only on the flat locus (exact-rational); TV=0.86 vs the He CI ground state; locus dynamically unstable (t²). Born = the observation projection's exchange constant (Gleason-unique); Class-1 upgraded classification→tested-negative; complements the 2026-05-26 derivation-direction closure. Falsifier tests/test_wh8_born_probe.py. See debug/sprint_wh8_born_probe_memo.md.
- **group4 6th cert FAIL→remediated; recert HELD per PI (2026-07-01, v4.60.0):** calibrated 8/8, 6/6 (P16 code needed a 2nd reviewer — #1 missed its seed, discarded per protocol). 3 genuine MATERIALs fixed: Z=1–56 propagation (5 loci), ℓ-parity spectrum claim now genuinely tested (new H₂ test <1e-10), P14 organics zombie + C16 entry. λ 33.3→32.6 (0.95×). Both PI-flags resolved v4.60.1 (PI-directed): §1.5 retired to near-parity; MPO drivers → tests/mpo_rank_support/ + test_paper14_mpo_rank.py, whose recompute caught + fixed a profile overclaim (first-cut 4-vs-5, memo-known, drafting-lost). See debug/sprint_group4_6thcert_remediation_memo.md.
- **group4 M-A market-direction call (PI-delegated, 2026-06-29, v4.59.0):** demoted the LiH-vs-STO-3G "2.7× fewer Pauli" headline (rests on the un-reduced raw 907 + pair-diagonal rule; vs the strongest baseline, reduced 276, it's near parity at more qubits). Market test now LEADS honest parity; decisive wins foregrounded = O(Q^2.5) scaling + 190× vs cc-pVDZ. P20/synthesis/§2; numbers unchanged. Per §1.5 benchmarking rule. See CHANGELOG v4.59.0.
- **group4 5th cert FAIL→remediated; closest-to-PASS; recert HELD per PI (2026-06-29, v4.58.0):** calibrated 8/8, 6/6; **6 of 8 dimensions clean-except-seed**, every v4.55–4.57 fix confirmed accepted (Trenev reversal triple-confirmed incl. citation PDF read). Thin framing/attribution layer fixed: #2 GH-metric-rate-not-energy-bound (P20 §VI), M-A raw-907 carved out of the Trenev attribution (my v4.57.0 over-blanket), M-C Z=1–36→Z=1–56 (BaH Z=56), + §1.5/CF-1/mislabel SMALL + NaH worked-example. PI-flagged: M-A market-direction (raw-2.7× vs reduced-parity). See debug/sprint_group4_5thcert_remediation_memo.md.
- **group4 4th cert (PASS-confirmation) FAIL→remediated; recert HELD per PI (2026-06-29, v4.57.0):** calibrated 8/8, 6/6 (panel confirmed every v4.55/v4.56 fix). **Headline: the Trenev attribution was reversed** — a "diagnose-first" web-check found Trenev Table 5 (Appendix B) DOES tabulate the electronic Gaussian counts (all 6 exact); the v4.54/v4.55 "GeoVac's own recompute / Trenev vibrational-only" was a double mistake (missed App. B), **the code was right**. Reverted corpus-wide (no numbers changed). Plus F2 P20 "/balanced" zombie, S1 synth R_eq propagation, matched-qubit wording, 2 new coverage tests (library=37, Dirac §VI). See debug/sprint_group4_4thcert_remediation_memo.md.
...
- **`/qa group2` re-cert#5 FAIL→remediated (2026-06-28, v4.50.4):** PERFECT calibration again (11/11, 6/6; rs8 4th catch) — thin-residual asymptote: 2 thin genuine residuals (P11 HeH⁺ secondary stale −1.475→−1.512 [recomputed]; P17 H2O 19.4%-correct but provenance-note cause wrong) + recurring synth −0.6025→−0.6026 → all fixed; CERTIFIED v4.51.0 via the NIT carve-out. See debug/qa/group2_recert5_run_notes.md.
- **`/qa group2` re-cert#4 FAIL→remediated (2026-06-28, v4.50.3):** calibration PERFECT (11/11 sens, 6/6 spec; cs3/P17 scored via the fast-test carry-forward; rs8-class caught a 3rd time) — closest run yet; only 2 thin framing-syncs left: synthesis cusp-0.004%-"variational"→non-variational (3 loci, run-#2 fix not propagated to synthesis) + fci_molecules "graph-Laplacian-eigenvalues=−Z²/2n²"→hydrogenic/continuum-image (C6). Full-class swept. Run #5 next. See debug/qa/group2_recert4_run_notes.md.
- **`/qa group2` re-cert#3 FAIL→remediated (2026-06-27, v4.50.2):** calibrated 10/11 (rs8-class fix CONFIRMED via cs5; cs3 unscored — P17 reviewer over-ran); the **cross-paper "second-locus" propagation layer** — run-#1/#2 corrections (H∝S→two-term/SO(4)-congruence, HeH⁺ 93.1%→adiabatic-caveat+5% 2D-var, cusp non-variational, H2O 26→19.4%, FCI-A LiH vertical/no-equilibrium, H2+ 0.0002%) synced to ~25 stale echoes across 10 files + 1 genuine fix (FCI-A "Li beats HF"→corrected). Det. gates green. Run #4 next. See debug/qa/group2_recert3_run_notes.md.
- **`/qa group2` re-cert#2 FAIL→remediated (2026-06-27, v4.50.1):** calibrated 8/9; new defect layer — TC §3-reassertion reframed (P15/P17, +C16 entry), HeH⁺ "parity" recomputed 93.1%→**5.3%** (2D-var), P8 Sturmian theorem re-derived (SO(4)-congruence; guardrail preserved), cusp non-variational + 7 SMALL; rovib confirmed reproducible. Run #3 next. See debug/qa/group2_recert_run_notes.md.
- **`/qa group2` whole-group run#1 FAIL→remediated (2026-06-27, v4.50.0):** calibrated 10/10; **no paper headline wrong** — v2.7.0-dropped code restored (P11/P15 spectral, MolecularLatticeIndex→_archive), BeH₂ regression fixed (11.7% recovered) + H2O improved 26%→19.4%, all coverage gaps backfilled. See debug/qa/group2_whole_run_notes.md.
