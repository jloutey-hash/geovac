...
**Quantum computing status: ACTIVE FRONTIER (Paper 14).** O(Q^2.5) composed Pauli scaling, 51x-1712x vs Gaussian, ecosystem export (OpenFermion/Qiskit/PennyLane). PK classical partitioning gives 78x 1-norm reduction for H₂O. Market test: LiH 0.97× 1-norm vs STO-3G with 2.7× fewer Pauli — **doubly convention-sensitive** (group4 v4.55.0 D): (CF-1) composed Pauli uses the pair-diagonal ERI rule (physical global-M_L re-prices 2.51× main-group / 3.25× d-block, scaling unchanged), AND it is raw-vs-raw (334@Q30 vs 907@Q12) — narrows toward parity vs the 2-qubit-reduced STO-3G (276@Q10). 37-system `hamiltonian()` library (35 composed molecules + He + H2). See Paper 14, Paper 20, CHANGELOG Tracks AW-CA.
