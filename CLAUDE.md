...
**Version:** v5.15.5 (September 20, 2026)
...
- **Frozen-core prolate LiH — build works, but the geometry result is INCONCLUSIVE pending r₁₂ (2026-09-20, v5.15.4-5, PI-directed; PoC):** built the frozen-core prolate LiH valence Hamiltonian (HeH⁺ engine Z=3/Z=1 + validated core-shield V_H + Huzinaga projector + core-valence exchange; bookkeeping validated to −8.070 at the expected valence energy; exchange small, −0.9 mHa). First R_eq +2% at (2,2)/p=0 looked ~5× better than composed (+8.8%), **BUT it does NOT survive angular refinement**: (2,3) gives +7.1%, approaching composed. **Confound:** the LiH scans ran p=0 (NO r₁₂) while the HeH⁺ PoC that CONVERGED (−4.5%→−0.5%) had r₁₂ — LiH was forced to p=0 because V_H/projector are p=0-factorized. So the l=3 divergence is likely the missing r₁₂ (not a real failure), but the frozen core is an unruled-out suspect. **Do NOT quote +2% as the result — unconverged.** CRITICAL next: r₁₂-coupled V_H + projector (apples-to-apples with HeH⁺). `debug/track_logs/prolate_native_lih.md`; drivers `debug/lih_{frozen_core_first,projector_test,exchange,req_scan}.py`. See CHANGELOG v5.15.4-5.
