...
| Paper 34 | On-topic | Living catalogue: two-layer framework (bare graph + projection family); thirteen projections × three-axis tagging (variable/dimension/transcendental); empirical matches catalogue with off-precision rows tagged by error source (T/B/A/C/S). Load when discussing where physics enters the graph, projection mechanisms, error-source classification, or when adding a new match to the framework's record. 13th projection (Drake-Swainson asymptotic subtraction, May 2026) sits in Paper 18 flow tier. |
...
| Projection taxonomy (thirteen projections, three-axis tagging) | 34 | Sec III-IV | Observation |
