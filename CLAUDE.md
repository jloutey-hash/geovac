...
- **`/qa group1` Bite B sub-bite 3 (P39/52/53) FAIL→remediated — COMPLETES group1 (2026-06-18, v4.25.0):** calibrated 9-agent panel; two SERIOUS findings remediated **rescue-first** (PI): **p39** C₃<1 keystone FALSE + footnote fabricated (real-operator probe: Pyth-ratio 2.0 max-violated / triangle-ratio 1.0 tight) → C₃→√2, convergence SURVIVES, "<1" + "k-INDEPENDENT master theorems"→√k-sketch downgraded; **p53** disk positivity/Λ^{−1.30} refuted/phantom (min eig −0.13, Λ^{+0.07}) → rescue SUCCEEDS via boundaryless plane (Λ^{−0.6..−0.9}). Bonus: **Lorentzian-propinquity rescue probe** (PI question) — P45 K⁺ annihilation is spatial-multiplier-only (Toeplitz temporal survives K⁺=ω_q) but signature-blind robustly → WH7 weakens-to-convention confirmed. See CHANGELOG + debug/sprint_qa_group1_biteB3_memo.md + debug/sprint_lorentzian_toeplitz_kplus_probe_memo.md.

- **`/qa group1` Bite B sub-bite 2 (P45–49) FAIL→remediated full sweep (2026-06-18, v4.24.0):** calibrated 13-agent panel (5/5 sens, 5/5 spec), heaviest FAIL; defining pattern **"Status-note-only descope"** — all 5 descoped/partial papers had honest Status notes but re-asserted descoped Lorentzian-metric convergence as established throughout theorem bodies/abstracts/appendices (~20 zombies); propagated each paper's own descope framing + fixed citations (devastato/hekkelman/bizi-confabulated + the nonexistent Latrémolière Thm 5.5); theorem-numbers verified GROUNDED. See CHANGELOG + debug/sprint_qa_group1_biteB2_memo.md.
- **`/qa group1` Bite B sub-bite 1 (P42/43/44+synth) FAIL→remediated (2026-06-18, v4.23.0):** calibrated 10-agent panel (5/5 sens, 5/5 spec); **two reviewer LARGE findings overturned by PM verification** (§9 reconcile) — p42 "derived finding false" (reviewer omitted the β factor; finding CORRECT) + §10 "descoped-zombie" (over-flag; genuine finite-cutoff Krein closure). Genuine fixes: 6 wrong-ID/fabricated cites (hekkelman_mcdonald2024 ×3, zhu_casini authors, latremoliere2018 ×3, devastato, …) + Paper-38 propinquity→state-space-GH labels + P39 zombie + synthesis P45 degeneracy framing; van den Dungen/Nieuviarts theorem-numbers verified GROUNDED. See CHANGELOG + debug/sprint_qa_group1_biteB1_memo.md.
