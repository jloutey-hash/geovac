...
| Two-electron V_ee (Neumann, general m) | algebraic-pending (recurrence-stable, quadrature-seeded) | Paper 12. σ (m=0): `compute_Xl` from the A_l/B_l tables (B_l via a 1D quadrature seed). General m (μ>0): the A_l^{m,s}/B_l^{m,s}/X_l^{m,s} tables generalised to associated Legendre via the forward l-recurrence (l−m+1)B_{l+1}=(2l+1)B_l(p+1)−(l+m)B_{l−1}, weight (ξ²−1)^s kept intact; **stable at μ=2 (δ) where the d^m-differentiation route diverges (non-variational ≈−14.5 Ha)**. Low-l B_l seeds still use a 1D quadrature (as σ does), so recurrence-stable not fully quadrature-free; the closed-form {e^a·E₁(a), ln, γ} seeds are the quadrature-free ideal. `geovac/neumann_vee_general_m.py`, `tests/test_paper12_general_m_neumann.py` (v5.12.7) |
