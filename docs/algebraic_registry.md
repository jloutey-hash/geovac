...
| Two-electron V_ee (Neumann, general m) | **algebraic** (quadrature-free, v5.13.4) | Paper 12. General m (μ≥0): A_l^{m,s}/B_l^{m,s}/X_l^{m,s} tables via the forward l-recurrence (l−m+1)B_{l+1}=(2l+1)B_l(p+1)−(l+m)B_{l−1}, weight (ξ²−1)^s intact; **stable at μ=2 (δ) where the d^m-differentiation route diverges (non-variational ≈−14.5 Ha)**. The low-l B_l seeds are now CLOSED FORM (`_seed_B_closed`), retiring the 1D quadrature that was 95% of `vee_mp`: s≥m always (physical) polynomializes every pole term d^kQ_0~(ξ−1)^{−k} of d^mQ_l under the intact (ξ²−1)^s, leaving one log-moment against Q_0 whose primitive L_n(c)=∫ξ^n Q_0 e^{−cξ} is closed-form in the minimal transcendental set {E₁(2c), γ, ln c} (`_L_moments`). vee_mp ~38× faster, H2 energy bit-identical. `geovac/neumann_vee_general_m.py`, `tests/test_paper12_general_m_neumann.py` (v5.13.4) |
