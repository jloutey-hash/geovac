...
| 2 | Unit-S³ graph eigenvalues are −(n²−1); kinetic scale κ = −1/16 derived from the Fock projection (not fitted) | Papers 0, 7 | SYMBOLIC PROOF | same test files; κ derivation v2.26.1 |
...
| 17 | Master Mellin engine / π-source case-exhaustion: every π entering the framework's projections arises from Tr(D^k e^{−tD²}) at k ∈ {0,1,2} | Papers 18, 32 §VIII | INTERNAL THEOREM | case-checked over the Paper 34 projection catalogue; status caveat in Paper 32 §VIII applies to the GH-theorem entry |
