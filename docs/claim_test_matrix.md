...
| 43 | §5.2 Pythagorean HS-orthogonality $\langle H_{\mathrm{local}},D_W\rangle=0$ + Pythagorean corollary $\|H-D\|^2=\|H\|^2+\|D\|^2$ + chirality pairing | `test_paper43_pythagorean_hs_orthogonality.py` (**v4.43.2**, 7 cells: Riemannian n_max=2,3,4 + Lorentzian (2,1),(3,1),(2,3),(3,3); HS-IP <1e-12, pairing max_h_diff/max_d_sum <1e-14, Pythagorean residual <1e-10) | `modular_hamiltonian_lorentzian.py` | BACKED-SOUND (**v4.43.2**) | recompute-from-framework (ports the archived verifier); in-paper Lemma 5.5 proof remains self-contained backstop. (The separate closed-form $r^2=\kappa_g^2 S(n)/4\pi^2+D(n)$ / PSLQ refinement is not load-bearing for the §5.2 orthogonality and stays proof-by-argument) |
