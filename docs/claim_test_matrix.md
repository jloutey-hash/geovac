...
| 42 | $H_{\mathrm{local}}=K_\alpha^W$ not $D_W$ (HS-orthogonal; distinct operators) | `test_modular_hamiltonian.py` (norm $\|H_{\mathrm{local}}-D_W\|_F$), `test_modular_hamiltonian_lorentzian.py` (thm:h_local_sig_independent) | `modular_hamiltonian.py` | BACKED-SOUND | **v4.43.0 prose fix:** the justification "$D_W$ would not close the period closure" was FALSE (conjugation by scalar $-I$ closes; verified $<10^{-13}$) → corrected to the *operator-level* lift ($-I$ double-cover vs $+I$); conclusion survives |
