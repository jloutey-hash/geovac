...
Calibrated panel (sensitivity 4/4, specificity 5/5). One genuine code bug fixed with a new regression test; coverage gaps below are verified-true claims with weak/no test — **raised to PI** (disposition: dedicated test-backfill candidate, not remediated this round).
...
| 29 | Observation 2: P₁₂, P₂₂ are the m_ℓ→−m_ℓ ℤ₂ block decomposition (det M_sym=(s²−1)P₂₂, det M_antisym=P₁₂; 22+24 Dirac-B analog) | — (only `debug/hopf_u1_block_test_memo.md`) | `ihara_zeta.py` | **NO-TEST** | coverage gap (raise to PI) — the P₁₂·P₂₂ *factorization* IS tested (`test_ihara_zeta.py` slow); the block-*origin* identity is not |
| 29 | Alon–Boppana bound-crossing sweep (three of four families cross at modest V, signed deviation linear) | `test_alon_boppana_sweep.py` (schema/arithmetic smoke on frozen JSON) | `debug/compute_alon_boppana_sweep.py` (generator, not pytest) | **BACKED-WEAK** | coverage gap (already logged rc3) — test checks internal arithmetic consistency of a static JSON, does not recompute spectra |
| 50 | Wedge KMS entropy: slope=2=dim(∂W), constant coth1−log(2sinh1)=0.458448, PSLQ-disjoint from F-ring | — (only `debug/bh_phase0` memo) | — | **NO-TEST** | coverage gap (raise to PI) — Sec-3 headline; quantitative/falsifiable but no `tests/` backing |
| 50 | CHM identification (Thm 4.2): spec(K_α)={1,3,…,2n_max−1}, 2π-period closure = canonical CHM spectrum | — (Paper-42 infra tests only) | `modular_hamiltonian.py` | **NO-TEST** | coverage gap (raise to PI) — Paper-50-specific integer-spectrum sub-claim is trivially testable, untested; prose hedged ("operator-system-level analog", convergence "inherited") |
