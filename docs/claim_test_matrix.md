...
### Group 1 — Batch 2 (Papers 42, 43, 44, 53, 2026-06-22, v4.43.0)

First *full* (all-5-dimension) cert run of the Lorentzian-arc foundation papers; FAIL→remediated. Calibrated panel (sensitivity 5/5, specificity clean). The CODE dimension — never fully exercised in the prior Batch-2 runs (rc2/rerun2 were claims+citation) — carried the verdict: it overturned a control (p42 "$D_W$ does not close" was *false*; conjugation by the scalar $-I$ closes — prose corrected this run) and surfaced 3 coverage gaps (NO-TEST), recorded here rather than silently omitted (C1 deliverable). Coverage gaps queued for a dedicated test-backfill sprint (the v4.34.0 Batch-3 precedent).
...
| 42 | Cross-witness collapse (six witnesses → same $\Delta$, $K_{\mathrm{TT}}$) | `test_modular_hamiltonian.py` (`verify_cross_witness_collapse`, consistency $<10^{-14}$) | `modular_hamiltonian.py` | BACKED-WEAK | collapse is by-construction ($\rho_W=e^{-K_\alpha^W}/Z$ $\beta$-independent); test compares bit-identical paths — **test-hardening** queued (instantiate each witness from its own $\beta$-dependent $H_{\mathrm{local}}$) |
...
| 43 | §5.2 Pythagorean HS-orthogonality $\langle H_{\mathrm{local}},D_W\rangle=0$ + closed form $r^2=\kappa_g^2 S(n)/4\pi^2+D(n)$ + 18-cell panel | in-paper proof (Lemma 5.5 chirality-pairing, self-contained); **no `tests/` numerical regression** | `modular_hamiltonian_lorentzian.py` | **NO-TEST** | coverage gap; deleted `debug/h_local_orthogonality_formal_proof.py` citation **neutralized this run** (→ CHANGELOG + in-paper proof); numerical-panel backfill queued |
...
| 53 | Plane Bochner–Riesz convergence + positivity (boundaryless $\mathbb{R}^2_\alpha$, rate $\Lambda^{-0.6..-0.9}$) | `test_paper53_plane_bochner_riesz.py` (binding thresholds, Hankel pairs verified) | (plane Berezin) | BACKED-SOUND (**@slow**) | the genuinely-convergent claim is sound, but the only cited test is `@pytest.mark.slow` (skipped by default) — fast-gate or de-slow queued |
| 53 | Finite-disk OBSTRUCTION negatives (min g≈−0.13; interior rate $\Lambda^{+0.07}$ no decay) + prop:prop2 ($O_\Lambda^2$ fills $M_N$) | `debug/archive/` drivers + JSON only (pruning-scheduled) | — | **NO-TEST** | coverage gap; the negative-result half + the one numbered Proposition have no permanent backing — resurrect the `debug/archive/` drivers into `tests/` (per [[feedback_resurrect_pruned_artifacts]]) queued |
