...
| 53 | Finite-disk OBSTRUCTION negatives (min g≈−0.13; interior rate $\Lambda^{+0.07}$ no decay) + prop:prop2 ($O_\Lambda^2$ fills $M_N$) | `test_paper53_disk_obstruction.py` (**v4.43.2**; obstruction: positivity fails min g<−0.10 at Λ=200, rate slope>0 no-decay; prop2: O² fills $M_N$ at (2,2),(2,3),(3,2),(3,3) via live `operator_system_dim`) | `operator_system.py` | BACKED-SOUND (**v4.43.2**) | resurrected the `debug/archive/` driver computations into `tests/` (per [[feedback_resurrect_pruned_artifacts]]); the negative-result half + the Proposition now have permanent backing |
