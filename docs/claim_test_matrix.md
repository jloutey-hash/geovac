...
| 40 | General-G Lipschitz comparison C₃(G)=1 (PRV summand / Brauer–Klimyk, "5641/5641 cells") | `dirac_triangle_extended_verify.py::run_panel` (imported by `test_paper40_universal_rate.py` for its constructors but **never called**); rank-2 backing sits under prunable `debug/qa/_resurrected/` behind `skipif` | (driver) | **NO-TEST** (flagged v4.43.5) | code-dimension Flavor B: general-G C₃=1 has no `tests/` assertion (test_r25 backs only S³/SU(2)); if `debug/` is pruned the rank-2 tests silently skip. → backfill: reduced-panel C₃(G)=1 assertion + migrate drivers to a permanent home / harden the skip |
