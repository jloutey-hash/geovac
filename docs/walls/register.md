...
- **Status → SOFTENED · Kind → SOFT (cost/encoding, not RI/decidability).** The only genuinely-4-body-connected term in `⟨Φ|F H F|Φ⟩` (F=Σf_pq multiplicative) is the scalar Coulomb **chain** `f₁₂·f₃₄·(1/r₁₃)` — kinetic gradients are pair-local (`∇_k f₁₂·∇_k f₃₄ = 0` for disjoint pairs) and V_ne is one-body, so only the two-body Coulomb can bridge two disjoint correlation edges. That term is **exact, RI-free, terminating** (the bridging-Coulomb multipole sum truncates at `L ≤ 2·l_bridge` — measured s→0, p→2, d→4) and **reducible** (factorizes across the bridge via the Legendre addition theorem into a finite (L,M) contraction of 3-body vertex kernels — the chain analogue of the N=3 triangle contraction).
- **Confirmed with numbers:** reduced == brute-force 12-D MC at **0.02%** (2p bridge / 1s leaves, exercises L>0) and **0.25%** (with the antisymmetric Be determinant — exchange chains; `debug/r12ci_4e_be_integral.py`, `be_r12ci_4body_exchange.py`). And demonstrated **inside a Be R12-CI energy**: E_R12 = −14.5572 Ha, corr −18.2 mHa (19% of Be's 94 mHa), variational, with the 4-body content in `gV = E[(F−F̄)²V_ee]` (`debug/be_r12ci_full.py`; the ill-conditioned coupling h and σ² done analytically-exact).
- **What stays HARD (separate axes — do NOT read this softening as global):** (i) **quantum encoding** — an explicit 4-body correlation operator is a 4-body Pauli string (§3 TC 2nd-quant plateau 3.4%, TC angular-gradient 2.66× Pauli); real wall for the quantum-simulation product. (ii) **non-Hermitian TC** (vector legs, non-abelian Gaunt/6j; §3 2026-08-23) — a different, harder operator. (iii) **kinetic-vector 4-body** — argued non-bridging (pair-local gradients), UNTESTED.
- **On this narrow axis GeoVac is ahead of Gaussian-F12** (which uses RI for exactly these 3-/4-electron integrals). The general resolution: the bridge addition theorem, likely extending to N≥5 (more bridges, same per-bridge factorization).
