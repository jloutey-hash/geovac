...
| Three-center ERI period on the Legendre / Γ(2) modular family (λ=1−ρ exact); CM-fibre periods are Γ-values — a concrete elliptic cosmic-Galois period at the Q(i) CM point (same as the Dirac Kramers doublet) | 59, 56 | Paper 59 `sec:modular`; Paper 56 `rem:paper59_cm` | Observation |
| Integrated three-center observable = a Γ(2) multiple modular value (elliptic polylogarithm); explicit iterated-Eisenstein realization and the generic (non-CM) Hain–Brown locus remain open | 59, 56 | Paper 59 `sec:modular`, `sec:scope`; Paper 56 `rem:paper59_cm` | Open |
...
| T2 fibre = 2D Euclidean two-propagator correlator (Feynman parameters = Källén–Lehmann spectral masses; D = Euclidean-time displacement; j₀ = spatial box smearing); Stokes location = squared complexified Euclidean interval | 59 | Paper 59 `sec:reduction` (kinematics), `sec:modular` (light cone / two-axis) | Measured |
| T2's modular home is X₀(2): the s↔t density exchange = ρ↦1/ρ = the T-coset; co-area fold = the involution; Γ₀(2)'s Z[i] elliptic point unphysical; physical fixed locus = pure-Tate cusp | 59, 56 | Paper 59 `sec:modular` (X₀(2) paragraph); Paper 56 `rem:paper59_cm` | Symbolic + Measured |
| Resurgent-skeleton pattern (5/5): Stokes data algebraic up to singularity-type-fixed π-power across E₁ seed / hybrid ERI / exchange class / N(D) / T2 cusp; integer charges + γ-as-bookkeeping in the exchange class; boundary period forced at weight ≤1, free at genus ≥1 | 59, 18, 34 | Paper 59 `sec:modular` [OBSERVATION] paragraph | Observation (5 objects) |
...
| T2 certified to 66 digits via the (k,w) factorization (double integral = square of a 1-D integral); β(2)/Catalan DECISIVE-NEG at h≤10 (64 digits) — no conductor-4 content in the physical value; closed form still open | 59, 56 | Paper 59 `sec:modular` eq:kw; Paper 56 `rem:paper59_cm` | Measured (decomposed certification) |
