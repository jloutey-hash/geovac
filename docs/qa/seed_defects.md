...
| S11 | **Restricted-evaluation / suspiciously-clean — a test passes because the evaluation object was restricted** (Paper 24 `if N_bra != N_ket: return 0.0` zeroed couplings ~20× larger than the diagonal it kept, for four months) | back a trunk claim with a test whose result is exactly-0 / exactly-invariant / exactly-a-named-constant via a hidden restriction — a frozen sector, a zeroed block, a trivialized outer factor, a truncated kernel | `code-reviewer` (the restricted-evaluation mandate, step 4a) |
