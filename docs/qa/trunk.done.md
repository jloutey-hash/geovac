...
- **C13 — Paper↔test reference integrity (deterministic).** Every test cited *inline* in a paper in the certified scope resolves to a **real, live** test file under `tests/` (not renamed, not typo'd, not archived) — certified by `debug/qa/check_paper_test_refs.py` (exit 0; also `tests/test_paper_test_refs.py`), scoped to the target (default = trunk; `--gate <branch>` per branch). Existence is the **gate** (a paper citing a non-existent test is the real failure mode); matrix↔paper *coverage* and non-trunk stale refs are reported **advisory** (a curated matrix tracks fewer tests than papers cite, and not every matrix claim has a clean prose anchor, so requiring full membership would over-fail). Keeps the inline test references honest against the suite and `docs/claim_test_matrix.md`.
...
| Deterministic | C10, C11, C12, C13 | scripts (`check_internal_titles.py`, `check_k_label.py`, `check_paper_test_refs.py`, compile) | not an LLM reviewer |
