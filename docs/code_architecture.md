...
| Sturmian metric-free isoenergetic secular (Paper 60 eq:secular) | `geovac/sturmian_secular.py` | `build_M`, `build_Tprime`, `build_S`, `solve`, `solve_with_metric`, `gen_configs` (||M||_1 ~ K^0.82 on a window; `solve_with_metric` is a COST diagnostic -- the L² posing is dearer to encode, not ill-conditioned. K^0.84 and the cond(S) 4->3673 divergence both retired [retracted 2026-09-07: p60-l2-metric-diverges]) |
...
| Sturmian SW sigma-spectrum law (Paper 60 eq:sigma_law) | `geovac/sturmian_sigma_law.py` | `sw_cross_block(s,nmax)`, `sigma_spectrum`, `cond_from_sigma`, `commutator_from_sigma/direct`, `gerade_constant()` (N^2 law -- **Kac-Murdock-Szego PRIOR ART, not derived here**; ours is the identification of the SW metric as such a finite section; collapse pi^2/24) |
