...
4a. **Restricted-evaluation mandate (required, not optional).** For every backing test whose result is *exactly* 0, *exactly* invariant under a parameter, or *exactly* a named constant: (i) enumerate what the evaluation object EXCLUDES — zeroed blocks, a frozen particle-number sector (`if N_bra != N_ket: return 0.0`), trivialized outer factors, a truncated kernel, a single-block ground state; and (ii) exhibit the claim on the UNRESTRICTED object, or state why you cannot. Report **FALSE-POSITIVE** if the clean value depends on the restriction. This is the class that voids a keystone silently — five Paper-24 tests passed for four months against a hard `S=0` that zeroed couplings ~20× larger than what it kept, and the corroborating "E₀ is basis-independent" was the symptom, not the confirmation. A suspiciously clean number is a reason to ask *what was restricted*, never a strong result on its own.
