...
5. **Hard prohibitions (§13.5) — enumerate, do not sample.** For the K = π(B+F−Δ) tripwire, `grep` **every** line mentioning the combination rule (`B + F`, `combination rule`, `K =`, `\alpha^{-1}`, the `K`-status remarks/theorems), QUOTE each, and classify each individually. Never conclude "all appearances are Observation" from a subset — one prohibited tier word ("derived", "a theorem", "proven", "conjecture/conjectural") applied to K is a MATERIAL violation **even when a neighboring sentence hedges or contradicts it** (a self-contradicting passage still FAILS — the prohibited-label sentence is the violation). Distinguish the compliant *non-selection theorem* ("no morphism derives K" — SOUND, do not flag) from a prohibited promotion ("K is now derived as a theorem" — FLAG). [Run-#4 lesson: a reviewer that sampled K-appearances missed a planted "K is now derived as a theorem" because the next clause said "does not promote K to a theorem"; exhaustive enumerate-and-quote catches it.] Also flag any fitted/empirical parameter and any suppressed negative result.
