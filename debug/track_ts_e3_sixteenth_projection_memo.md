...
**Context (read for this memo):** `debug/track_ts_d_paper35_triple_test_memo.md` (definitions of M1/M2/M3 and the candidate-theorem statement); `papers/group6_precision_observations/paper_34_projection_taxonomy.tex` (fifteen projections, three-axis dictionary, off-precision catalogue); `papers/group6_precision_observations/paper_35_time_as_projection.tex` (Prediction 1, KG sprint, TX-B panel); `papers/group1_operator_algebras/paper_32_spectral_triple.tex` §VI (K-decomposition into three sectors), §VII (Coulomb/HO asymmetry); `papers/group5_qed_gauge/paper_25_hopf_gauge_structure.tex` (Hopf-measure identification); `papers/group5_qed_gauge/paper_28_qed_s3.tex` (Seeley–DeWitt and vertex parity); `papers/group3_foundations/paper_18_exchange_constants.tex` §III.7, §IV (Mellin engine, motivic-weight axis, Chern–Simons / K-theory cross-reference).
