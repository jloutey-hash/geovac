...
- **Named open follow-ons:** (1) full external-citation audit of the *other* group syntheses (groups 2–6; PI-deferred) — now looking less optional (see §8); (2) ~~`hekkelman_mcdonald2024` + `farsi_latremoliere2024` bibliographic confirmation~~ **CLOSED (v4.67.6, §8): both were PHANTOM citations, resolved**; (3) the standing debug/-refs debt (§9 / C14 advisory).
...
## 8. Follow-on #2 + `/qa group1` citation delta (v4.67.6; the phantom-citation wave)

The synthesis cert (§2–3) flagged two group1-synthesis cites "pending confirmation." Resolving #2 revealed **both were PHANTOM — fabricated papers that do not exist** (the second wave of citation drift in the *certified* group1 bib, after v4.67.2's two wrong-arXiv-IDs):

- `hekkelman_mcdonald2024` "Spectral truncations of $\T^d$ and quantum metric geometry" — **no such paper**. Hekkelman's own publication list confirms her only McDonald collaboration is arXiv:2412.00628, *already* cited as `hekkelman_mcdonald2024b`. The phantom bibitem was **removed**; its two `\cite` usages repointed — the lineage list keeps the real `…2024b`, and the L670 Berezin–Toeplitz-torus claim now cites the real **Connes–van Suijlekom** operator-systems paper (`connes_vs2021` = CMP 383/2021 = arXiv:2004.14115, which genuinely treats the torus truncation).
- `farsi_latremoliere2024` "Continuity of spectral propinquity for spectrally truncated spheres" — **no such paper**. Repointed to the real 2024 Farsi–Latrémolière paper "**Collapse in noncommutative geometry and spectral continuity**," **arXiv:2404.00240**.

**Verification (the PI-directed `/qa group1` citation-dimension delta):** a full re-scan of the group1-synthesis bibliography (1 Sonnet reviewer, sharpened to verify each cite *resolves to a real paper*, 2 seeds). **Sensitivity 2/2, specificity clean; 35 of 37 external cites GROUNDED (the 2 defects were exactly the planted seeds); ZERO genuine non-seed defects** — no phantoms, no wrong-IDs, no fabricated titles remain. All v4.67.1–.6 fixes confirmed correct; the phantom `hekkelman_mcdonald2024` confirmed correctly absent. The group1-synthesis bibliography is now clean after two remediation waves + this confirming delta. group1 synthesis recompiles ERRORS=0, no undefined-citation warnings; worktree removed, no seed leaked.

**Standing lesson (reinforced):** a **branch cert is not a permanent guarantee for its bibliography**. The group1 branch cert (v4.49.0) passed over BOTH a wrong-arXiv wave AND a phantom-citation wave — only the cross-cutting synthesis re-scan (with a *resolves-to-a-real-paper* prompt, not metadata-only) caught them. This strengthens the case for follow-on #1 (a full external-citation re-scan of the groups 2–6 syntheses with the same sharpened prompt).
