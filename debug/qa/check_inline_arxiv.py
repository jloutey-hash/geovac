...
def main() -> int:
    ...
    # Scope resolution via the shared resolver (debug/qa/qa_scopes.py): the
    # old substring test is what let C19 examine zero trunk papers while
    # reporting PASS, and the local TRUNK name-set here was a second copy of
    # a declaration that now lives once, keyed to docs/qa/<target>.done.md.
    if gate_corpus:
        gated_files = list(files)
        _scope_warnings = []
        scope_label = "whole corpus"
    else:
        gate_value = gate_substr or "trunk"
        resolved, _scope_warnings = qa_scopes.resolve(
            gate_value, [str(f) for f in files])
        gated_files = [pathlib.Path(f) for f in resolved]
