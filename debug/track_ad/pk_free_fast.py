"""
Track AD Part 1 (fast): PK-free diagnostic — adiabatic only, l_max 1-3.
"""
...
for pk_mode in ['none', 'ab_initio']:
    for l_max in [1, 2, 3]:
        label = f"pk={pk_mode}, adiabatic, l_max={l_max}"
        print(f"\n{'='*60}")
        print(f"  {label}")
        print(f"{'='*60}")
...
# Summary
print(f"\n\n{'='*70}")
print("SUMMARY: PK-free vs ab_initio PK (adiabatic)")
print(f"{'='*70}")
print(f"{'pk_mode':>12s} {'l_max':>5s} {'R_eq':>8s} {'err%':>8s} "
      f"{'D_e':>8s} {'time':>6s}")
print("-" * 70)
for r in results:
    print(f"{r['pk_mode']:>12s} {r['l_max']:>5d} "
