"""
Convergence study of the three-point vertex correction B on S^3.

Tracks B = L(+1/2) - L(-1/2) as a function of n_max and compares
to the Schwinger value alpha/(2*pi).
"""
...
def compute_vertex_3pt_single_nint(n_ext, j_ext, mj_ext, n_int, q_probe=1):
    """Contribution from a single n_int level to the 3-pt vertex correction."""
...
def main():
    ...
    print("=" * 70)
    print("Three-point vertex correction convergence on S^3")
    print(f"External: n_CH={n_ext}, j={j_ext}")
    print(f"Schwinger target: alpha/(2*pi) = {schwinger:.6e}")
    print("=" * 70)

    results = []
    cumulative_up = S.Zero
    cumulative_dn = S.Zero
    ...
    for n_int in range(6):
        ...
        cumulative_up += contrib_up
        cumulative_dn += contrib_dn

        B_contrib = float(contrib_up - contrib_dn)
        B_cumul = float(cumulative_up - cumulative_dn)

        elapsed = time.time() - t0

        if B_contrib != 0:
            print(f"  n_int={n_int}: B_contrib={B_contrib:+.6e}, "
                  f"B_cumul={B_cumul:+.6e}, "
                  f"B/Schwinger={B_cumul/schwinger:.4f}, "
                  f"({elapsed:.1f}s)")
        else:
            print(f"  n_int={n_int}: zero (forbidden) ({elapsed:.1f}s)")

        results.append({
            'n_int': n_int,
            'B_contrib': B_contrib,
            'B_cumul': B_cumul,
            'ratio_to_schwinger': B_cumul / schwinger if schwinger != 0 else 0,
            'time': elapsed,
        })

    print(f"\n  Final B = {float(cumulative_up - cumulative_dn):.6e}")
    print(f"  B / [alpha/(2*pi)] = {float(cumulative_up - cumulative_dn)/schwinger:.4f}")
    ...
    with open('debug/data/alpha_g_minus_2_convergence.json', 'w') as f:
        json.dump(output, f, indent=2)
    print("\nSaved to debug/data/alpha_g_minus_2_convergence.json")
