"""Paper 12's prolate-spheroidal H2 CI, extended to |m| >= 1 channels.

Paper 12 (`paper_12_algebraic_vee.tex`) computes H2 in prolate spheroidal
coordinates with a basis that is entirely phi-INDEPENDENT --

    phi = e^{-a(xi1+xi2)} xi1^j xi2^k eta1^l eta2^m + (1<->2)

-- and a Coulomb kernel projected onto its m = 0 Neumann component.  That is a
sigma-only ansatz: a ^1Sigma_g^+ state has TOTAL M = m1 + m2 = 0, not m_i = 0,
so every pi^2 / delta^2 configuration is structurally absent.  Paper 12
plateaus at 92.4% of D_e and attributes the residual to the electron-electron
cusp; `debug/sprint_tmr_method_memo.md` argues it is the missing channels, and
`debug/p12_m_channel_probe.py` confirms that independently in a Gaussian basis.

This module adds the azimuthal quantum number in Paper 12's OWN basis.  The
one-electron factor becomes

    u_{j,l,mu}(xi,eta) = xi^j eta^l (xi^2-1)^{mu/2} (1-eta^2)^{mu/2} e^{-a xi}

and a basis function pairs m1 = +mu with m2 = -mu through cos(mu*(phi1-phi2)):
the M = 0, ^1Sigma_g^+ combination (pi_x pi_x + pi_y pi_y).  mu = 0 reduces to
Paper 12's basis exactly.

Integrals
---------
S, T, V_ne are EXACT: every integrand is a polynomial in xi and eta times
e^{-2a xi}, evaluated against
    A_n = int_1^inf xi^n e^{-2a xi} dxi     (three-term recurrence)
    E_q = int_-1^1 eta^q d eta              (2/(q+1) or 0)
All three are diagonal in mu -- the azimuthal integrals force mu_i = mu_j.

V_ee uses the general-m Neumann expansion

  1/r12 = (2/R) sum_l sum_m (-1)^m (2l+1) [(l-|m|)!/(l+|m|)!]^2
              P_l^|m|(xi_<) Q_l^|m|(xi_>) P_l^|m|(eta1) P_l^|m|(eta2) e^{i m dphi}

which COUPLES different mu -- that coupling is the physics Paper 12 discards.
eta integrals stay exact polynomial moments (the kernel's (1-eta^2)^{|m|/2} and
the basis's (1-eta^2)^{mu/2} always combine to an integer power).  The ordered
xi integral is computed on a geometrically graded Gauss-Legendre panel grid
with spectral (Legendre-series) antiderivatives, tabulated once per
(l, m, s, P1, P2), and is validated against the corpus's exact
`geovac.neumann_vee` machinery in the mu = 0 sector.

Run:  python debug/prolate_ci_general_m.py            (validate + experiment)
"""
...
def _kin(ja, la, jb, lb, mu, alpha, mom):
    ...
    if mu == 0:
        ...
        def ny(lx):
            t = P.polynegative if False else shift(np.array([-float(mu)]), lx + 1)
            if lx > 0:
...
def legendre_deriv_poly_safe(m):
    return np.array([1.0])
