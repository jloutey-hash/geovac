"""Final scan: check all files, allow long timeout, identify what's still broken."""
...
RESULTS_PATH = ROOT / "debug" / "data" / "sprint_test_cleanup_final_scan.json"
...
def run_pytest_on_file(test_file: str) -> dict:
    ...
    try:
        ...
        return {
            "file": test_file, "status": "TIMEOUT_20min", "returncode": -1,
...
def main():
    files = [
        # Originally broken files we touched + the originally-timeout files
    ...
    print(f"Final scan: {len(files)} files in parallel...")
    results = []
    with ProcessPoolExecutor(max_workers=3) as ex:
        futs = {ex.submit(run_pytest_on_file, f): f for f in files}
        done = 0
        for fut in as_completed(futs):
            r = fut.result()
            results.append(r)
            done += 1
            print(f"[{done}/{len(files)}] {r['status']:18} {r['file']:60} F={r['failed']} E={r['errors']} P={r['passed']} S={r['skipped']}")
