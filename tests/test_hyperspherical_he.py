"""
Tests for the hyperspherical adiabatic solver for helium (live tests only).
...
also tested spectral and algebraic-Laguerre code paths (TestSpectralRadialSolver,
TestSpectralCoupledChannel, TestAlgebraicLaguerreMatrices) that depended on
...
kwargs. All of these were removed from geovac/hyperspherical_radial.py in v2.7.0
(commit 8d692a0). The simplified hyperspherical_radial.py in current code is
pure FD. Those test classes were extracted to
tests/_archive/superseded/test_hyperspherical_he_spectral.py. Live API tests
(Gaunt integrals, angular solver, adiabatic curve, helium solver, radial solver)
preserved here.
"""
