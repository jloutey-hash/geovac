...
class TestH2SSpec:
    ...
    def test_block_count(self):
        from geovac.molecular_spec import h2s_spec
        spec = h2s_spec(max_n=2)
...
class TestPH3Spec:
    ...
    def test_block_count(self):
        from geovac.molecular_spec import ph3_spec
        spec = ph3_spec(max_n=2)
...
class TestSiH4Spec:
    ...
    def test_block_count(self):
        from geovac.molecular_spec import sih4_spec
        spec = sih4_spec(max_n=2)
...
@pytest.mark.skip(
    reason="_v_cross_frozen_core was a private helper in an earlier "
    "balanced-coupled architecture and has been removed.  Cross-center "
    "V_ne is now computed via the balanced_coupled multipole machinery "
    "(geovac.shibuya_wulfman.compute_cross_center_vne).  These tests "
    "validate a removed primitive; the equivalent physics is exercised "
    "by tests/test_balanced_coupled_*.py."
)
class TestFrozenCoreCrossNuclear:
    """Test _v_cross_frozen_core function (REMOVED from production code)."""
