...
def make_kernel_Y_full(gam):
    ...
    NOTE (2026-09-22): the isotropic aa/bb dressings here use psi_yuk (Neumann-minus-smooth),
    which carries a ~0.5% grid error vs the exact closed-radial yukawa_pot_iso. In the LINEXP
    (cusp-correct) geminal this bias, amplified by the Fbar=3.57 cancellation, was the sole >1%
    miss in Cov[F,Y_sum] and is fixed there by using the exact isotropic dressings (see
    lih_r12ci_linexp.py). It is DELIBERATELY left as psi_yuk here for the EXP geminal: the exp
    E_R12=-7.942 is validated end-to-end against the VMC 2x2 (-7.94121, 0.79 mHa) via a
    documented partial cancellation of piece-level deviations (g_T +0.008 high, g_Vne -0.006
    low); switching this dressing alone moves total g -0.919 -> -0.927 and E_R12 to ~-7.9435,
    DEGRADING the validated agreement to ~2.3 mHa. Fixing it would require re-checking g_Vne/
    g_Vee (separate machinery, own errors) too. Left as-is by design; flagged for the geovac
    migration where all pieces get re-derived on one consistent grid."""
    K = make_kernel_Y(gam)
