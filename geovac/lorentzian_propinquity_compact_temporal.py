...
def lorentzian_theorem_statement() -> str:
    """Return the CURRENT Paper 45 headline statement (the K^+ annihilation
    theorem). The earlier K^+-restricted weak-form convergence theorem is
    WITHDRAWN (retracted 2026-06-09); see the body for the historical form."""
    return (
        "Theorem (Paper 45 §5, headline -- the K^+ ANNIHILATION theorem). "
        "On the truncated S^3 x S^1_T Krein spectral triple "
        "T_L_truncated = (O^L, K, D_L, J), Krein self-adjointness of i*D "
        "forces {J, D_GV (x) I} = 0, so the K^+ compression annihilates the "
        "spatial Dirac:\n\n"
        "  P_+ (D_GV (x) I) P_+ = 0   (bit-exact),\n\n"
        "and the K^+-restricted Lipschitz seminorm vanishes identically on the "
        "whole operator system.  Hence Lambda^L := Lambda(.|_{K^+}, .|_{K^+}) "
        "is DEGENERATE -- it is not a quantum-metric distance.\n\n"
        "WITHDRAWN (retracted 2026-06-09): the earlier K^+-restricted weak-form "
        "Lorentzian Latremoliere propinquity CONVERGENCE theorem "
        "(Lambda^L <= C_3^joint * gamma^joint -> 0) is false as a metric "
        "convergence -- the seminorm is degenerate (above), and the constant "
        "C_3^joint = sqrt(1 - 1/n_max) was operator-norm-false (the correct "
        "Paper 38 Lemma L3 value is C_3 = 1, the gradient/translation "
        "seminorm).  Do NOT cite the pre-retraction convergence form.\n\n"
        "What SURVIVES (signature-agnostic, NOT a Lorentzian claim): the "
        "product-carrier S^3 x S^1 truncations converge to the continuum in "
        "van Suijlekom STATE-SPACE Gromov-Hausdorff distance at the additive "
        "rate gamma_SU(2) + gamma_U(1) = O(log n_max / n_max + T/N_t), the "
        "SU(2) factor's leading constant being 4/pi (Paper 38 L2 Stein-Weiss); "
        "this is a Euclidean product-carrier statement, and reduces to "
        "Paper 38's Riemannian SU(2) state-space GH bit-exactly at N_t = 1.\n\n"
        "Honest scope: the strong-form Latremoliere metric on Krein-signature "
        "spectral triples (without K^+ restriction) remains the open NCG-math "
        "problem; the Lorentzian signature is metrically invisible to any "
        "K^+-compressed Lipschitz seminorm (weakens-to-convention)."
    )
