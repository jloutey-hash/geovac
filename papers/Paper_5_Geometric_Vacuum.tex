...
This is not a fit. It is the \emph{measured resistance} of the lattice to electromagnetic flux, derived from helical photon geometry (spin-1 helicity) rather than planar circular paths.
...
\begin{table}[h]
\centering
\caption{Molecular geometry from vacuum constants. Bridge parameters $\mathcal{A} = \alphainv \cdot |\mathcal{K}|$ and $\lambda = \mu = 1/4$ are derived, not fitted. $R_{\text{eq}}$ is the equilibrium bond length at the minimum of $E(R)$.}
\label{tab:bond_validation}
\begin{tabular}{lccc}
\toprule
System & $R_{\text{eq}}$ (Bohr) & $R_{\text{exp}}$ (Bohr) & Error \\
\midrule
H$_2$   & 1.25 & 1.40 & 11\% \\
LiH     & 2.75 & 3.015 & 9\% \\
\bottomrule
...
Both systems exhibit bound states with equilibrium geometries within $\sim$10\% of experiment---using \emph{zero free parameters}. The remaining deviation is consistent with finite-basis discretization effects (Section~IV.C.4) and diminishes with increasing $n_{\text{max}}$.
