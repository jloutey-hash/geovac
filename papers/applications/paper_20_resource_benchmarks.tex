...
at 30 qubits.  The library covers 30 molecules across three rows of the periodic table
($Z=1$--36), including multi-center systems
(CO, N$_2$, F$_2$, CH$_2$O, C$_2$H$_2$, C$_2$H$_6$), mixed
frozen-core molecules (NaCl), and transition metal hydrides (ScH, TiH),
...
count is exactly linear in qubit count (coefficient $\approx 11.11$
for main-group molecules; $9.27$ for $d$-orbital blocks, which are
...
Table~\ref{tab:molecules} lists all 30 molecules currently available
...
Table~\ref{tab:transition_metals} lists the two transition metal
hydrides added in v2.4, which use [Ar] frozen cores and isolated
$d$-orbital valence blocks.
...
\begin{table}[h]
\caption{Transition metal hydrides added in v2.4 at $n_{\max}=2$.
  Both use [Ar] frozen cores (18 electrons) with $d$-orbital valence
  blocks ($l_{\min} = l_{\max} = 2$).  The Pauli/$Q$ ratio of~9.27
  falls below the main-group coefficient of~11.11, reflecting
  sparser $d$-orbital ERIs (4.0\% density vs.\ 8.9\% for $s$+$p$).}
\label{tab:transition_metals}
\begin{ruledtabular}
\begin{tabular}{llrrrr}
Molecule & Core & $Q$ & Pauli (comp.) & Pauli/$Q$ & QWC \\
\colrule
ScH & Ar & 30 & 278 & 9.27 & --- \\
TiH & Ar & 30 & 278 & 9.27 & --- \\
\end{tabular}
...
\emph{exactly linear} in the qubit count $Q$ across all 30 molecules:
\begin{equation}
  N_{\text{Pauli}} \approx 11.11 \times Q\,.
  \label{eq:linear}
\end{equation}
...
\emph{below} the 11.11 coefficient: ScH and TiH both have
Pauli/$Q = 9.27$.  This reflects the sparser ERI tensor of $d$-orbital
blocks (4.0\% density vs.\ 8.9\% for $s$+$p$), which produce fewer
Pauli terms per qubit.  The 11.11 coefficient is therefore an upper
...
Transition metal hydrides (ScH, TiH) have been demonstrated with
[Ar] frozen cores and isolated $d$-orbital valence blocks.
$d$-Orbital blocks are structurally sparser than $s$/$p$ blocks
(4.0\% vs.\ 8.9\% ERI density), producing a lower Pauli-per-qubit
ratio.  General transition metal classification ($Z = 23$--30) is
not yet automated.  Main-group third-row atoms
($Z=19$--20 and $Z=31$--36) are supported via [Ar]
and [Ar]3$d^{10}$ frozen cores~\cite{GeoVac_ScopeBoundary}.
The library currently includes 30 molecules.
...
of the periodic table (30~molecules, $Z = 1$--36), including
multi-center systems with multiple heavy atoms and initial transition
metal hydrides (ScH, TiH) whose $d$-orbital blocks are sparser than
...
$\approx 11.11$), meaning that adding atoms to a molecule increases
...
active-space reduction techniques, and (iv)~automated atomic classification for transition metals
($Z = 21$--30) and extension to heavier main-group atoms
