...
single, physically motivated discretization.  Under the proposed
...
claim that the Paper~2 Observation belongs inside an established
...
Separately, Paper~2 proposes that the Hopf fibration
...
(Phases 4B--4I) have identified each of $B$, $F$, $\Delta$ as
...
instance of that framework at the data level (with the continuum
limit of the original gauge networks, as corrected by the
Perez-Sanchez Comment~\cite{PerezSanchez2025Comment},
being Yang--Mills without Higgs --- the quiver
extension~\cite{PerezSanchez2024} obtains Yang--Mills--Higgs for its
own enlarged construction);\ the forced-graph claim
(Bertrand $\times$ Fock projection) and the master Mellin engine
are GeoVac-original structural extensions not predicted by the MvS
scaffolding.

We do not claim a new derivation of $\alpha$.  We do claim that the
Paper~2 observation and its proposed interpretation can be placed
inside an established mathematical framework.  In that framework, $B$ is a Casimir-weighted node-trace
...
the three bundle components.  What remains open is not the
existence of these quantities---they are computable and have been
computed in Phases 4B--4G~\cite{Paper2}---but their additive
combination (an Observation, not a derivation; \S 13.5 standing
label) and its proposed physical interpretation.
...
open components of Paper~2 (the observed combination rule and its
proposed interpretation) inside an established mathematical
...
\paragraph*{What is open and what is sharp.} The
...
(electron/photon/gauge) is Paper~2's proposed interpretation, flagged
throughout as such.  The interpretive gap is deliberate: we do not
assert the interpretation is correct.  We assert that if it is correct, the
...
On a manifold, the full Hodge--1 Laplacian is
$\Delta_1 = d_0 d_0^* + d_1^* d_1$, where the first term is the
\emph{exact} (longitudinal) part and the second is the
\emph{co-exact} (transverse) part.  On a bare graph (simplicial
1-complex without 2-cells), only the exact part
$L_1 = B^\top B = d_0 d_0^*$ is available.  The co-exact part requires
faces (2-cells) to define a boundary operator $d_1$;
Paper~30~\cite{paper30} provides precisely these faces via plaquette
enumeration, giving the co-exact Laplacian
$L_1^{\mathrm{co}} = d_1 d_1^\top$.  Paper~28~\cite{paper28} shows
that the transverse propagator $(d_1 d_1^\top)^+$ recovers the
continuum self-energy structural zero that $L_1^+$ alone cannot,
establishing a concrete QED role for the face structure.
...
the $S^3$ graph).  The quotient construction and the exact spectrum
are pinned live in \texttt{tests/test\_paper25\_s2\_quotient.py}
(sympy-exact: characteristic polynomial $x^3(x-1)(x-3)(x-6)$;
crossing-count weights $1, 1, 3$ on the s-chain and $p$-block radial
bundles, $d$-chain sector isolated).
...
\end{equation}
The two zero modes confirm $\beta_1 = 2$.  (The full spectrum is
pinned sympy-exactly, live from \texttt{GeometricLattice}, in
\texttt{tests/test\_paper25\_s2\_quotient.py}, 2026-07-04.)  The
golden-ratio
...
\item The Hopf quotient (collapsing $m_l$-fibers) yields a
  ...
  $\alpha$-D~\cite{Paper2}; pinned exactly in
  \texttt{tests/test\_paper25\_s2\_quotient.py}).
...
The three structural ingredients of Paper~2's combination
...
\label{obs:framework}
Under the proposed Paper~2 identification of $S^1 \to S^3 \to S^2$ with
...
(Paper~2)~\cite{Paper2}, and its status as an Observation is unchanged.
\end{observation}
...
lattice-gauge language.  It does not sharpen the observation; it does
...
\item Derive the fine-structure constant $\alpha$ from first
  principles.  Paper~2 is, and remains, an Observation with a
  proposed interpretation---not a derivation.  The present paper
  shifts that content into gauge-theoretic language without
  removing any of its undetermined inputs.
\item Prove the combination rule $K = \pi(B + F - \Delta)$.  Phases
  4B--4I plus two follow-on sprints eliminated twelve specific
  derivation mechanisms; the
...
within the GeoVac pipeline required the Paper~2 observation, which in
...
\subsection{Gauge structure on the Bargmann--Segal $S^5$ lattice: answered (mixed)}
\label{sec:s5_bargmann_open}
...
fibration.  The question posed in the first draft of this paper was
whether the Bargmann--Segal graph carries an analogous gauge
structure, with three candidate outcomes: (a) abelian $U(1)$ analog,
(b) non-abelian $\mathrm{SU}(3)$ analog, or (c) no gauge structure.
Sprint~5 Track~S5 (April~2026) resolved the question with a
\emph{mixed} verdict: (a) is confirmed, (b) is not natural, and (c)
holds on the spectral side (the $m_l$-quotient is not a
$\mathbb{CP}^2$ discretization).  The data sharpen Paper~24's
Coulomb/HO asymmetry rather than bridge it.  Paper~24
Corollary~4 (the gauge-structure-reduction corollary added by the
same sprint) documents the result in the Paper~24 vocabulary; we
summarize here in the Paper~25 vocabulary.

\paragraph*{Positive transfer: the abelian $U(1)$ Wilson--Hodge
structure is universal.} At $N_{\max} = 5$, the Bargmann graph has
56~nodes and 165~edges; the signed incidence matrix~$B$, node
Laplacian $L_0 = B B^{\top}$, and edge Laplacian
$L_1 = B^{\top} B$ are well-defined; the first Betti number
$\beta_1 = E - V + c = 110$ counts independent cycle
classes (graph counts at $N_{\max} = 2$--$5$ and the plaquette
censuses pinned live in \texttt{tests/test\_su3\_wilson\_s5.py},
class \texttt{TestPaper25S5Pins}).  Under a node-local $U(1)$ gauge transformation
$\psi_v \to e^{i\chi_v}\psi_v$, the dipole edge amplitudes transform
covariantly, exactly as in Section~III.C for the $S^3$ case.  The
combinatorial Hodge decomposition and the SVD identity between
nonzero spectra of $L_0$ and $L_1$ apply unchanged.  In short: the
combinatorial Wilson--Hodge vocabulary of Section~II generalizes
verbatim to the $S^5$ Bargmann graph (and, more generally, to any
finite triangle-free simplicial 1-complex with a chosen orientation).

\paragraph*{Negative: no natural non-abelian $\mathrm{SU}(3)$
lattice structure.} A Wilson lattice gauge theory requires each
link variable to live in a fixed group acting on a fixed matter
irrep.  The Bargmann graph has \emph{different} $\mathrm{SU}(3)$
irreps on different shells---the dimension of $(N, 0)$ grows as
$(N+1)(N+2)/2$---and the dipole operator intertwines
$(N, 0) \otimes (1, 0) \to (N+1, 0)$ as a Clebsch--Gordan
coupling, not as an $\mathrm{SU}(3)$ group element.  The natural
gauge group of the Bargmann graph is therefore $U(1)$, not
$\mathrm{SU}(3)$.  An SU(3)-covariant reformulation
(replacing $(N, l, m_l)$ labels by full weight-space labels of
$(N, 0)$ and promoting edges to SU(3) link variables) would be a
different construction, and would not correspond to a Wilson
lattice on the $S^5$ Hardy space in any natural sense.

\paragraph*{Negative: the $m_l$-quotient is not a $\mathbb{CP}^2$
discretization.} Collapsing the $m_l$-fibers at fixed $(N, l)$
yields a 12-sector graph at $N_{\max} = 5$ with sectors
$\{(0,0), (1,1), (2,0), (2,2), (3,1), (3,3), (4,0), (4,2), (4,4),$
$(5,1), (5,3), (5,5)\}$ and fiber dimensions $(2l+1) = 1, 3, 1, 5,
3, 7, 1, 5, 9, 3, 7, 11$.  Its Laplacian eigenvalues are
$\{0, 2.22, 4.87, 4.94, 11.65, 12.11, 18.74, 28.09, 33.58, 50.61,
63.77, 99.43\}$ (12 distinct values, multiplicity scheme).  The
scalar Laplace--Beltrami spectrum on $\mathbb{CP}^2$ with the
Fubini--Study K\"ahler metric of constant holomorphic sectional
curvature~$K = 4$ is
$\lambda_k = 4k(k+2)$ with degeneracy $(k+1)^3$, giving
$\{0, 12, 32, 60, 96, 140, \ldots\}$ with multiplicities
$\{1, 8, 27, 64, 125, 216, \ldots\}$.  A least-squares fit of the
eleven nonzero quotient eigenvalues to the $\mathbb{CP}^2$ sequence
under a single linear rescaling leaves a $40.8\%$ maximum relative
residual (measured against the fitted values); the ratios
$\lambda_{\text{quot},k} / \lambda_{\mathbb{CP}^2, k}$ range
between~$0.082$ and~$0.185$, so \emph{no} single rescaling can bring
the maximum relative residual below the sharp two-point floor
$(r_{\max} - r_{\min})/(r_{\max} + r_{\min}) \approx 38\%$ under
either normalization (a fortiori ${\geq}33\%$ against the data;
deviation \emph{factor} $\sqrt{r_{\max}/r_{\min}} \approx 1.50$) ---
there is no uniform fit under any convention.  (An earlier version
quoted a $24.98\%$ residual from an unrecorded fit convention, and a
first corrected printing overstated the against-the-fit floor as
${\approx}50\%$ --- the one-sided minimax is the $38\%$ two-point
floor above; both are superseded by the pinned computation in
\texttt{tests/test\_su3\_wilson\_s5.py}, class
\texttt{TestPaper25CP2Quotient}, which also pins the twelve-sector
spectrum above.)  The quotient is a
combinatorial SO(3)-labeled graph, not a spectral discretization
of the Kähler $\mathbb{CP}^2$.

\paragraph*{Structural interpretation: two-layer Coulomb/HO
asymmetry becomes three-layer.} Paper~24 established the
Coulomb/HO asymmetry at the level of the \emph{spectrum} (on
$S^3$, $L_0 = D - A$ yields the Coulomb spectrum in the continuum
limit via the $\kappa = -1/16$ projection; on $S^5$, $L_0$ is spectrally inert and the HO
spectrum is in the diagonal).  The present sprint adds a third
layer at the level of \emph{gauge-structure content}: the abelian
$U(1)$ Wilson vocabulary is universal, but its physical content is
not.  On $S^3$, the dictionary (matter propagator, photon
propagator) $= (L_0, L_1)$ is nontrivial---both sides carry
physical spectrum and physical gauge content.  On $S^5$, the
dictionary is trivial on the matter side (the HO spectrum is not
in $L_0$) and only combinatorial on the gauge side ($L_1$
generates cycle classes, but there is no ``photon propagator on
$S^5$'' interpretation tied to the HO physics).

An immediate consequence is that the Paper~2 combination rule
$K = \pi(B + F - \Delta)$ has no natural $S^5$ analog.  A candidate
Fock-degeneracy Dirichlet analog $F_{S^5} = \sum_{N \geq 1}
g_N^{\text{HO}} / N^{d_{\max}}$ with
$g_N^{\text{HO}} = (N+1)(N+2)/2$ and packing exponent
...
projection.  The analogs of $B$ and $\Delta$ are
spectrum-computing-tied (both rely on the Coulomb projection),
which Paper~23's Fock projection rigidity theorem restricts to
$S^3$ alone.  Together with Phase~4H's SM-running negative
result~\cite{Paper2} for alternative origins of $\Delta$, the
combined picture is that the $S^5$ graph is a lattice gauge
structure with abelian $U(1)$ content but without $\alpha$-like
spectral content.  This is consistent with Paper~24's operator-order
transcendental discriminant: first-order complex-analytic
operators give linear spectra, linear projections, and $\pi$-free
lattices; second-order Riemannian operators give quadratic
spectra, nonlinear projections, and $\pi$ from spectral zeta
regularization.  Only the second-order Coulomb case carries
Wilson-structure content of the $\alpha$-relevant kind.

\paragraph*{What remains open.} The sprint answers the three
candidate outcomes listed above; one narrow follow-up remains, but
is well outside the Paper~25 frame: \emph{if} the Bargmann graph
were rebuilt in a genuinely SU(3)-covariant basis (not the SO(3)
adaptation of Paper~24), with nodes labeled by SU(3) weight-space
coordinates and edges carrying SU(3) intertwiners rather than
$U(1)$ phases, \emph{would} a different gauge-structure dictionary
emerge?  The answer is technically yes (this is classical
representation theory), but the construction is no longer a Wilson
lattice in the natural sense, and it has no known tie to the
harmonic oscillator's physical spectrum.  We record this as a
speculative direction for future work and do not pursue it here.
Data backing the Sprint~5 verdict: \texttt{debug/s5\_bargmann\_segal\_graph.py},
\texttt{debug/s5\_edge\_laplacian\_analysis.py},
\texttt{debug/data/s5\_graph\_spectrum.json},
\texttt{debug/s5\_gauge\_structure\_memo.md}.

\paragraph*{Sprint~ST-SU3 update: pure-gauge SU(3) Wilson on
Bargmann is well-defined; matter coupling rediscovers the CG
obstruction.}  Sprint~ST-SU3 (May~2026,
\texttt{debug/st\_su3\_wilson\_memo.md}, with module
\texttt{geovac/su3\_wilson\_s5.py} and tests
\texttt{tests/test\_su3\_wilson\_s5.py} 49/49 pass, two slow-marked)
revisits the non-abelian question with a structurally different
construction: link variables $U_e \in \mathrm{SU}(3)$ as external
$3\times 3$ matrices in the \emph{fundamental} representation,
independent of the shell index~$N$.  This is categorically distinct
from Sprint~5's adjacency-preserving SU(3) action on the $(N, 0)$
tower (which the negative above ruled out): Wilson links do not
require an action on shell labels.  The construction is well-defined
on the Bargmann graph as a Wilson lattice gauge theory: link
variables in SU(3), plaquette holonomies in SU(3), action
gauge-invariant under node-local SU(3) (verified to machine
precision at $N_\text{max} = 2, 3$), Haar-normalizable partition
function.  The maximal-torus reduction yields a coupled
$U(1)\times U(1)$ Wilson action --- the rank-2 sibling of the present
paper's $U(1)$ on $S^3$ --- with character
$(1/3)\bigl(\cos a_P + \cos b_P + \cos(a_P + b_P)\bigr)$ verified at
machine precision.  The weak-coupling kinetic coefficient is
$1/12 = 1/(4 N_c)$ per plaquette per generator component, the
$N_c = 3$ specialization of Paper~30's universal SU($N$) formula.
\emph{However}, coupling Wilson SU(3) to matter on the Bargmann
shells (in the natural $(N, 0)$ irreps) requires a CG intertwiner
$\textbf{3} \otimes (N, 0) \to (N+1, 0)$ --- exactly the Sprint~5
Track~S5 obstruction reappearing on the matter-coupling side.
Wilson SU(3) on Bargmann is therefore a self-consistent
\emph{pure-gauge} theory; coupling it to natural matter requires the
same CG structure that the strict Wilson formalism does not
accommodate.

The structural reading that emerges from combining Sprint~5,
Paper~30, and Sprint~ST-SU3 is that Paper~24's Coulomb/HO asymmetry
sharpens from three layers to \emph{four}: the Wilson--Hodge
combinatorial vocabulary is universal (transfers to $S^5$); the
spectrum-computing role of $L_0$, the calibration~$\pi$, and Wilson
gauge \emph{with natural matter coupling} are all Coulomb-specific.
The previous version of \S~\ref{sec:s5_bargmann_open} (Paper~24
Corollary~4 \& the three-layer reading) has Wilson gauge and matter
coupling bundled together; the present update splits them: Wilson
gauge alone is universal; only Wilson gauge with natural matter
coupling on the shell tower is Coulomb-specific.  The corresponding
update to Paper~24 \S~V is documented at the same memo (\S~5.4).
...
observation---previously implicit in Paper~2 but never
...
Under the Paper~2 proposed interpretation, the three components
...
rule itself remains an Observation---observed, not derived---just as
in Paper~2; what changes
...
base graph.  What remains a proposal is the physical interpretation of
the bundle components and of the additive combination rule.  The
...
Paper~2's proposed interpretation.
...
\bibitem{Paper0}
J.~Loutey, ``Angular Momentum as Information Geometry: Isotropic
Packing and the Universal Angular Cross-Section,'' GeoVac Paper~0 (2025).

\bibitem{Paper2}
J.~Loutey, ``The Fine Structure Constant as a Spectral Coincidence on
the Hopf Fibration,'' GeoVac Paper~2 (2025).
...
\bibitem{Paper22}
J.~Loutey, ``Potential-Independent Angular Sparsity for Qubit
Hamiltonians in Spherical Fermion Systems,'' GeoVac Paper~22 (2026).
