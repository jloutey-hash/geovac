...
% remaining -5.65 MHz residual decomposes per LS-7 into ~+1.20 MHz
% multi-loop plus ~+4.4 MHz non-loop pieces (the original "genuine
% alpha^5 multi-loop ceiling" reading was revised by LS-7; see the
% revised-decomposition subsection).
...
$-0.534\%$ ($-5.65$~MHz) is roughly three orders of magnitude looser
than the precision-QED frontier, which routinely reaches sub-kHz on
the same observable with multi-loop self-energy and recoil
corrections~\cite{eides2001, yerokhin2024_two_loop_se}. The claim
here is therefore not that the GeoVac framework competes with
precision-QED compilations on the absolute number --- it does not
--- but that the framework's bare combinatorial graph plus three
standard projections (Fock, Camporesi--Higuchi spinor lift,
Drake--Swainson asymptotic subtraction) plus the canonical
Eides~\S\,3.2 one-loop convention reproduces the Lamb shift at
sub-percent without any GeoVac-specific input. The interest is not
in the absolute number --- one-loop QED on flat-space hydrogen also
gives sub-percent --- but in \emph{how} the framework reproduces
it: which projections are required, which projections turn out to
be unnecessary, and what specific structural pieces had to be added
during the LS-1 through LS-6a sprint sequence to close the
calculation.
...
The hydrogen Lamb chain is continuously verified by
\texttt{tests/test\_paper36\_lamb\_chain.py} with the Sturmian
machinery ported durably to \texttt{tests/paper36\_lamb\_support/}
(2026-07-03); the original sprint evidence is in
...
and ln $k_0(1S) = 2.924$ (reference 2.98413; $-2.0\%$ at $N = 50$).
...
resolve it.  The standard literature treatment regularizes the ratio by
asymptotic-subtraction techniques: the integral-representation
technique originates with Schwartz~\cite{schwartz1961} (for the
helium ground state) and was extended to hydrogen $p$-states by
Drake--Swainson~\cite{drake_swainson}.
...
$3.3\times$ tighter).
\item $\ln k_0(1S) = 3.002$, residual $+0.60\%$ --- at $N = 20$, not
$N = 40$: the 2026-07-02 recompute
(\texttt{tests/test\_paper36\_lamb\_chain.py}) showed the $1S$
acceleration-form value is a \emph{crossing point} of a
non-converging drift, not a converged value (by $N = 40$ it has
drifted to $3.268$, $+9.5\%$).  The $2S$ value at $N = 40$ is the
converged production number; the $1S$ channel is quoted at its
crossing for reference only.
\end{itemize}
...
$-0.030017$.
\item $\ln k_0(3D) = -0.005236$, residual $+0.07\%$ vs $-0.005232$
(reference corrected 2026-07-03: an earlier printing quoted a
mistranscribed $-0.005249$ against the same table).
\end{itemize}
...
\Delta_{\text{LS-1} \to \text{LS-6a}} = +\frac{4}{15} \cdot
...
\paragraph{Continuous verification vs archived record.}
The chain above is continuously verified (added 2026-07-02):
\texttt{tests/test\_paper36\_lamb\_chain.py} recomputes the full
component table closed-form from the Drake--Swainson tabulated Bethe
logs (LS-1 $1025.06$, LS-6a $1052.19$, shift $+27.13$, residual
$-5.65$~MHz / $-0.534\%$, and the exact identity
$10/9 - 38/45 = 4/15$ in exact rational arithmetic), and
(slow-marked) recomputes the from-scratch Sturmian Bethe logarithms
of \S\ref{sec:drake} at $N = 40$ ($2P = -0.0307$, $3D = -0.005236$,
$2S$ acceleration $= 2.786$, plus the $1S$ $N = 20$ crossing and its
$N = 40$ drift) from the machinery ported durably to
\texttt{tests/paper36\_lamb\_support/}.  The remaining
\S\ref{sec:drake} numbers --- the LS-2 velocity-form values and the
$N = 55$ 2$P$ value $-0.0304$ ($|{-}1.24\%|$, re-validated
2026-07-02 at $62$~s runtime) --- stay archived-measured in the LS
sprint record.
...
of the dimensionless bracket coefficient $C_{2S} = +3.63$ (current
independent state-of-the-art two-loop SE computations at low~$Z$ are
reported by Yerokhin, Harman, and
Keitel~\cite{yerokhin2024_two_loop_se}) from the
...
adequate: \texttt{geovac/two\_loop\_self\_energy.py} (backed by
\texttt{tests/test\_two\_loop\_self\_energy.py}) computes the two-loop
SE bracket; the separate \texttt{geovac/qed\_two\_loop.py} computes
connected double Dirac Dirichlet sums in closed form via
half-integer Hurwitz, and
...
twenty-eight) handle the \emph{finite} parts of one-loop bound-state QED
...
Antognini~2013~\cite{antognini2013}: $<1$ ppm} against the canonical literature value
$205.0074$ meV.
...
\midrule
Framework $+$ literature & $+202.17$ & $+202.16$ \\
\midrule
...
(PDG~2024~\cite{pdg2024}), so this sprint is a \emph{framework calibration check} on
...
Sprint provenance: the driver is preserved durably at
\texttt{tests/paper36\_precision\_support/sprint\_mh\_track\_a.py}
(resurrected 2026-07-04 from the pruned
\texttt{debug/sprint\_mh\_track\_a.py}; archived-measured, not yet
regression-gated --- re-validation is a named follow-up), with the
original sprint memo and JSON in the \texttt{debug/} clean-room
record.
...
\begin{tabular}{lcc}
\toprule
Component & Framework (MHz) & Karshenboim 2005~\cite{karshenboim2005} (MHz) \\
\midrule
...
$0.071$~MHz / $68$~ppm of the value (corrected 2026-07-03: an earlier
printing misstated this as $7$~ppm).  This is the sharpest verification
...
across the full $e/\mu/p$ swap matrix.  Sprint provenance: the driver
is preserved durably at
\texttt{tests/paper36\_precision\_support/precision\_catalogue\_muonium\_lamb.py}
(resurrected 2026-07-04; archived-measured, not yet regression-gated),
with the memo and JSON in the \texttt{debug/} record.
...
Dirac~$=2$ or full CODATA values~\cite{codata2018} (the difference being $a_e + a_\mu$
...
The driver is preserved durably at
\texttt{tests/paper36\_precision\_support/precision\_catalogue\_muonium\_hfs.py}
(resurrected 2026-07-04; archived-measured, not yet regression-gated),
with the memo and JSON in the \texttt{debug/} record.
...
The remaining $-5.65$~MHz residual decomposes, per the corrected
reading of \S 5.2, into $\approx +1.20$~MHz of genuine multi-loop QED
plus $\approx +4.4$~MHz of non-loop sectors (the earlier
``Table~7.4 multi-loop band'' attribution was a misreading of which
Eides table carries the multi-loop total).  Sprint LS-7/LS-8a
(\S 6.1) tested the two-loop leg and returned a WEAK verdict at the
renormalization wall; the residual remains the standing target for
Paper~35's Prediction~1 in the multi-loop sector.
...
of 2S--2P transition frequencies of muonic hydrogen,''
\textit{Science}\ \textbf{339}, 417--420 (2013).

\bibitem{karshenboim2005}
S.~G.~Karshenboim, ``Precision physics of simple atoms: QED tests,
nuclear structure and fundamental constants,''
\textit{Phys.\ Rep.}\ \textbf{422}, 1--63 (2005).
...
physics,'' \textit{Phys.\ Rev.\ D}\ \textbf{110}, 030001 (2024).
...
G.~W.~F.~Drake and R.~A.~Swainson, ``Bethe logarithms for hydrogen
up to $n=20$, and approximations for two-electron atoms,''
\textit{Phys.\ Rev.\ A}\ \textbf{41}, 1243--1246 (1990).

\bibitem{yerokhin2024_two_loop_se}
V.~A.~Yerokhin, Z.~Harman, and C.~H.~Keitel,
``Two-loop electron self-energy for low nuclear charges,''
Phys.\ Rev.\ Lett.\ \textbf{133}, 251803 (2024); arXiv:2411.12459.
