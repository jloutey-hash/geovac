...
but shows the literal Sommerfeld--Cheeger $(1/12)(1/\alpha - \alpha)$
extraction sits below the bulk-subtraction discretization floor at
sprint scale, identifying G4-5 (discrete replica method) as the
proper target. The architectural scoping for G4-4 (warped Dirac on the
discrete substrate, with load-bearing falsifiers F1/F2/F3) is closed.
End-to-end commitment for the remaining G4-4 through G4-6 program is
estimated at $\sim 9$--$16$ months.
\end{abstract}
...
\paragraph{Q1.} The discrete-substrate program is now closed at
G4-3 sub-sprint scale (G4-3a/cleanup/b/c/d/UV all done, five of seven
sub-sprints). The remaining multi-month commitment G4-4
through G4-6 is open. The G4-4 architectural scoping (warped Dirac on
the discrete substrate, with three load-bearing falsifiers F1, F2, F3
named) is closed; first-move sub-sprint G4-4a (constant-warp Dirac +
falsifier verification) is 4--8 weeks. End-to-end G4-4 through G4-6 is
estimated at $\sim 9$--$16$ months.

The literal Sommerfeld--Cheeger $(1/12)(1/\alpha - \alpha)$ extraction
is now identified as a G4-5 target (discrete replica method), not a
sprint-scale sub-sprint, based on the G4-3c-proper wedge-lattice
diagnosis: the bulk subtraction at sprint-scale discretization
($N_0 = 120$) leaves a residual with $\alpha < 1$ UV-asymmetry that is
structurally larger than the SC tip term. This is a discretization
floor, not an algorithm bug.
