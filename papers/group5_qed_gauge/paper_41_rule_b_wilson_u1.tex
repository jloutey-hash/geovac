...
(\texttt{tests/wilson\_rule\_b\_support/xcwg\_u1\_wilson\_rule\_b\_pilot.py};
the kernel dimensions and spectra below are pinned by
\texttt{tests/test\_paper41\_wilson\_witnesses.py}):
...
quadratic kinetic operator. We verified that
$\mathrm{rank}(K)=\mathrm{rank}(d_1)=\beta_1$, matching the Hodge theorem on
the 2-complex ($n_\text{max}=2$ in the frozen suite,
\texttt{tests/test\_paper41\_wilson\_witnesses.py}; $n_\text{max}=3,4,5$
in the archived driver run).
...
$\beta_\text{eff}\approx\beta/k$, approached as $O(1/\beta)$: $8.6\%$ at
$\beta=50$ and $\le 2\%$ at $\beta=200$ for $k=8.8$. (The July~2026
durability migration corrected an earlier ``$\le 2\%$ at $\beta=50$''
misstatement against the archived \texttt{beta\_eff\_table}; the recompute
is pinned by \texttt{tests/test\_paper41\_wilson\_witnesses.py}.)
\end{itemize}
...
The earlier reading of the same numerical data (in the May~2026 XCWG-C
sprint pilot memo) interpreted the
...
higher-order character corrections (technical, beyond sprint scale) or a direct
...
(3) $\langle P\rangle=0$ within 2$\sigma$ at every non-degenerate $(\beta,N_t)$:
...
Rule B's monopole sites (the size-3 closed 2-cycles of the XCWG-F
construction; 60 at $n_\text{max}=2$); edges connect sites sharing a
plaquette. The construction gives $V_\text{dual}=60$,
...
The seven independent diagnostics give (the table values are pinned by the
frozen regression suite \texttt{tests/test\_paper41\_wilson\_witnesses.py}:
witnesses 1--4 and all graph-structural facts recomputed from scratch, the
seeded Monte-Carlo fit values of witnesses 5--7 pinned against the archived
JSON in \texttt{tests/wilson\_rule\_b\_support/data/} together with
reduced-statistics fixed-seed smoke runs):
...
2 (\S\ref{sec:mk_rg}) & MK $\beta_\text{eff}$ flow at $n_\text{max}=2,3$ &
  ...
  strong $\le 0.25\%$ ($\beta{=}0.03$); weak $\le 2\%$ ($\beta{=}200$) \\
\midrule
...
which caps the correlator's dynamic range). Each is a bounded amount
of focused effort.
...
seven-witness diagnostic suite. This is well beyond
...
are reproducible from the driver scripts and JSON output files preserved
in the permanent \texttt{tests/wilson\_rule\_b\_support/} directory of the
GeoVac repository (migrated there from the transient \texttt{debug/}
clean-room in July~2026), including
\texttt{xcwg\_nlo\_character\_expansion.py} and
\texttt{data/xcwg\_nlo\_character\_expansion.json} for the v2 NLO
results; \texttt{xcwg\_monopole\_density.py} together with
\texttt{data/xcwg\_monopole\_density.json} and
\texttt{data/xcwg\_monopole\_density.png} for the v3 monopole-density
results; \texttt{xcwg\_full\_mc\_wilson\_loops.py} together with
\texttt{data/xcwg\_full\_mc\_wilson\_loops.json} and
\texttt{data/xcwg\_full\_mc\_wilson\_loops.png} for the v4 XCWG-G
full-MC Wilson-loop results; and \texttt{xcwg\_polyakov\_loop.py}
together with \texttt{data/xcwg\_polyakov\_loop.json} for the v4
XCWG-H Polyakov-loop results. The cheaply recomputable witness values and
the archived seeded Monte-Carlo fits are pinned by the frozen regression
suite \texttt{tests/test\_paper41\_wilson\_witnesses.py}. Per
CLAUDE.md~\S 1.5, no ontological
