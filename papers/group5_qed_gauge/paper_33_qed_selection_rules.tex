...
construction.  All claims are verified by 100 tests in
...
\texttt{tests/test\_vector\_qed.py} (100 tests).
...
$\Sigma(\mathrm{GS})$.  Continuously verified at $n_{\max} = 2$
(exact rational arithmetic) in
\texttt{tests/test\_graph\_qed\_self\_energy.py}; the original sprint
driver additionally swept $n_{\max} = 2$--$5$ (archived measurement).
The closed form itself holds for all $n_{\max}$ by the path-graph
Laplacian identity above.
\end{proof}
...
The partition is structural and verified by 100 tests in
