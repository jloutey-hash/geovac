...
We tested LiH at $l_{\max} = 3$ and $l_{\max} = 4$ with
$\sigma{+}\pi$ ($m_{\max} = 1$) channels, expecting the $l_{\max}$
...
\begin{tabular}{lccc}
\hline
Configuration & $R_{\rm eq}$ (bohr) & Error \\
\hline
$l_{\max}=2$, $\sigma$ (manual PK) & 2.97 & 1.5\% \\
$l_{\max}=2$, $\sigma$ (ab initio PK) & 3.21 & 6.4\% \\
$l_{\max}=3$, $\sigma{+}\pi$ (ab initio PK) & 3.52 & 16.9\% \\
$l_{\max}=4$, $\sigma{+}\pi$ (ab initio PK) & 4.00 & 32.8\% \\
\hline
\end{tabular}
\end{center}

The root cause is that the Phillips-Kleinman pseudopotential is
channel-blind: it was derived from the $1s^2$ core (pure $l=0$), so
it applies the same repulsive barrier to all angular channels.
Higher-$l$ channels add correlation energy preferentially at large
$R$ (where the valence cloud is diffuse and benefits most from
angular flexibility) rather than at short $R$ (where the cloud is
compact).  This shifts the PES minimum outward monotonically.
The $l_{\max}$ convergence pattern from Paper~15 does not transfer
to core-valence systems because the pseudopotential breaks the
mechanism.

The path forward is an $l$-dependent or $m$-dependent
pseudopotential that modulates the core--valence barrier by angular
channel, or a multi-channel adiabatic correction (DBOC) that
captures non-adiabatic coupling between the angular states.
