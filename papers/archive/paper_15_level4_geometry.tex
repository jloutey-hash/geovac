...
The double-adiabatic fiber bundle structure---nuclear base, hyperradial
base, angular fiber---generalizes the single-adiabatic bundle of Paper~13.
\end{abstract}
...
The $\theta$-integrals are computed by Gauss--Legendre quadrature
(typically 24--32 points), avoiding the Legendre expansion of $V_i$
which diverges when $\rho/s_i > 1$ (electron between midpoint and
nucleus).
...
The Level~4 geometry realizes a \emph{double-adiabatic fiber bundle},
extending the single-adiabatic bundle of Paper~13:

\medskip
\noindent
\begin{tabular}{ll}
\textbf{Outer bundle:} & Base $= R$ (nuclear),
  Fiber $=$ electronic states \\
\textbf{Inner bundle:} & Base $= R_e$ (hyperradial),
  Fiber $=$ angular channels \\
\textbf{Connection:} & $P_{\nu\mu}(R_e) =
...
\medskip
\noindent
The inner connection encodes the non-adiabatic coupling
between angular channels as $R_e$ varies.  Since the angular problem
depends on $R_e$ only through $\rho$, this connection can be computed
...
Future work: (i)~$l_{\max} \ge 5$ with $\sigma{+}\pi$ channels
...
with nuclear dynamics; (iv)~heteronuclear systems beyond HeH$^+$
(e.g., LiH, LiHe$^+$).
