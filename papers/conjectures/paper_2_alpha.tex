...
($\Delta = 1/40$). The resulting $\alpha^{-1} = 137.035987$ agrees
with experiment to $8.8 \times 10^{-8}$ relative error with zero free
parameters. A combinatorial search over $1.92 \times 10^9$ formulas
built from the same spectral ingredients yields a $p$-value of
...
$N(n_{\max}) = \sum_{n=1}^{n_{\max}} n^2$. We observe that the ratio
...
The ratio crosses through~3 only at $n_{\max} = 3$: below this shell
$B/N < 3$, and above it $B/N > 3$. We emphasize that this condition
is \textit{empirically observed}, not derived from a variational
principle. The base manifold invariant is
...
2 & Spectral data $\to$ $B$, $F$, $\Delta$ & $\circ$ \\
...
\textbf{Link~2} (partial): The three spectral invariants $B$, $F$,
$\Delta$ are well-defined quantities associated with the bundle
components. The selection principle $B/N = 3$ uniquely fixes
$n_{\max} = 3$. However, the \textit{choice} of which spectral
quantity to extract from each component is not derived from the bundle
geometry.
...
$\det'(\Delta_{S^2}) = e^{-4\zeta'_R(-1)}$, and
...
\item \textit{Understand the residual $8.8 \times 10^{-8}$.} Higher-order
  ...
  contributions could account for the discrepancy.

\item \textit{The other cubic roots.} The two non-physical roots of
  Eq.~(\ref{eq:cubic}) may relate to the weak mixing angle or the
...
invariants of the Hopf fibration. The formula contains no fitted
parameters and achieves $8.8 \times 10^{-8}$ relative agreement with
experiment. A combinatorial search over $1.92 \times 10^9$ formulas
yields a $p$-value of $5.2 \times 10^{-9}$, establishing that this
precision is very unlikely to be accidental. The cubic form is the
...
cubic form is natural, but their assembly into $K$ awaits a
...
material~\cite{phase2_audit}. Computational implementation and
documentation drafting were performed with the assistance of large
language models (Anthropic Claude) under the author's scientific
direction. All results were validated against analytical benchmarks.
\end{acknowledgments}
