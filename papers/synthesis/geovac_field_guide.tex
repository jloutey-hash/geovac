...
produced something resembling the periodic table.  Over the course
of the investigation, it acquired a precise structural identity:\ a
...
of reader toward the right entry into the corpus.  As the project's \emph{grand
synthesis} it now sits atop a complete layer of six audience-targeted
group syntheses---one per branch (Groups~1--6), each digesting its
branch in a few pages---and composes them into a single front-to-back
reading that routes each reader to the right branch digest.  The
one-sentence
...
This graph is conformally equivalent to a continuous operator on the
unit three-sphere (the Laplace--Beltrami operator;\ see
\S\ref{sec:fock}), whose eigenvalues are
\begin{equation}
\label{eq:graph_eigs}
\lambda_n = -(n^2 - 1), \qquad n \in \N_{\ge 1},
\end{equation}
with multiplicity $n^2$ --- the spectrum of the hydrogen atom, to
within an overall scale.  The shape of the periodic table fell out of
the packing.
...
corpus~\cite{loutey_paper7} provides $18$ symbolic proofs
of this equivalence (\texttt{tests/test\_fock\_projection.py} and
\texttt{tests/test\_fock\_laplacian.py}).  The universal scaling
constant relating the two is $\kappa = -1/16$;\ its value
\emph{coincides} with a conformal-geometry quantity --- a numerical
observation, not a derivation
(Paper~0~\cite{loutey_paper0}, Paper~7~\cite{loutey_paper7}).
...
2 & H$_2^+$ (2-center, 1e) & Prolate spheroid & machine precision & 11 \\
3 & He (1-center, 2e) & Hyperspherical & $0.004\%$ (cusp-corrected;\ raw variational $0.022\%$;\ $0.216\%$ graph-native CI) & 13 \\
4 & H$_2$ (2-center, 2e) & Mol-frame hyperspherical & $96.0\%$ $D_e$ (cusp-corrected;\ prolate spheroidal reaches $99.97\%$ / $0.053$~mHa via explicit-$r_{12}$ CI, Paper 12) & 15 \\
...
``multi-focal-composition wall'' --- the project's name for a
structural obstruction established by multiple independent negative
results --- documented across six independent diagnostic
investigations).  The framework's most natural quantum-simulation
application --- Pauli-operator counts exactly linear in the qubit
count $Q$ at fixed basis ($N_{\mathrm{Pauli}} = 27.90 \times Q$) for
the core--valence ``composed'' encoding, with a
sub-quadratic $O(Q^{1.77})$ $1$-norm, benchmarked across a $37$-system
library~\cite{loutey_paper14} --- shipped at the end of Phase~2.  The
absolute Pauli multipliers are convention-sensitive (near parity with
minimal-basis STO-3G, $76\times$ sparser than cc-pVDZ);\ the
load-bearing advantage is the scaling, not any single market-test
ratio.
...
Perez-Sanchez clarification~\cite{perez_sanchez2024, perez_sanchez2025}
that the continuum limit is
...
This claim was checked, not assumed --- and then re-checked.  A
Gromov--Hausdorff convergence theorem on truncated
...
\end{equation}
with the constant $4/\pi$ (dual-Coxeter normalisation; $2/\pi$ on the
unit $S^3$) reported universal across compact connected semisimple Lie
groups (rigorous at rank~1, numerically pinned at higher ranks;
the rank-uniform proof is a named gap)~\cite{loutey_paper40}.
Paper~38's theorem is unconditional in
van~Suijlekom's state-space framework, via a translation-seminorm
metrization (the kernel condition is proved on the truthful
substrate;\ the dual direction uses an exact-fit spinor lifted state,
and both almost-inverse defects are Fej\'er smoothings at the same
moment).  See Paper~38~\S~1 and the claims register,
\texttt{docs/claims\_register.md}.
The rate constant reads $4/\pi = \mathrm{Vol}(S^2)/\pi^2$ in the
dual-Coxeter normalisation and $2/\pi = \mathrm{Vol}(S^2)/\mathrm{Vol}(\mathrm{SU}(2))$
on the unit $S^3$ --- the $k = 0$ volume factor of the master Mellin
engine (historically named ``Hopf-base measure'', though it is a quotient of
unit-sphere volumes and not the fibration's base-to-total ratio), its value
fixed by the metric normalisation, classified in
\S\ref{sec:periods} below.

Following Paper~38, the project produced a math.OA arc extending the
framework along the Lorentzian axis (Papers~42--49)~\cite{loutey_paper42, loutey_paper43, loutey_paper44,
...
\emph{metric-level} strong-form and bridge convergence claims built on
it (Papers~46, 48, 49), is withdrawn;\ those three and Paper~47 were
...
of Paper~49 (whose operator-system Lorentzian pre-length-space bridge
stands at the algebra level).  The repaired Lorentzian target --- a Toeplitz temporal
multiplier algebra and a non-compressing Krein-compatible seminorm
--- is stated as an open problem in Paper~45.
...
$\sfive$ Hardy lattice (Paper~24), and on the $37$ molecular and
atomic species in the qubit Hamiltonian library (Paper~14).
...
Early in its development the project produced transcendentals through ad-hoc
...
\begin{itemize}
  ...
  \item The Standard Model gauge group $U(1) \times SU(2) \times
    ...
    structural saturation (Paper~32~\S~VIII).
  \item The dimension of the inner Dirac moduli space is
    $\dim \mathcal{M}(D_F) = 32$ at one generation (matter-sector
    projection rank $16$; $8$ in the diagonal slice) --- corrected
    2026-09-03 from a degenerate-sample $128$.  Bertrand-shaped
    Forced-Count Theorem (Paper~32~\S~VIII).
\end{itemize}
...
\begin{itemize}
  ...
  \item The Yukawa coupling values.  A 162-cell PSLQ sweep against
    low-coefficient pure-Tate periods returned zero hits.  The Yukawa Dirichlet ring lives
  ...
  \item The generation count $N_{\mathrm{gen}} = 3$.  The
    ...
    shortcut for forcing $N_{\mathrm{gen}}$:\ the ``3 algebra factors'' and the
    ``3 generations'' are different $3$s in the standard SM
    representation.  A deep structural obstruction;\ no known line
    of attack.
...
\begin{itemize}
  \item The Forced-Count Theorem (Paper~32~\S~VIII):\ the gauge structure
    of GeoVac is forced.
  \item The Yukawa Non-Selection Theorem (Paper~32~\S~VIII):\ the framework's
...
relations are forced.  This is the
project's most operationally useful structural reading.

\paragraph{Theorem-bounding the seam (June 2026).}  The forced/free seam
discipline above was sharpened into a set of named structural theorems
in June 2026.  Paper~57
catalogues sixty named entries across nine domains (35 forced, 1
admitted-not-forced, 24 calibration) and tests a
``packing-reachability'' discriminator (P5);\ its $98.3\%$ agreement
with the forced/free labelling is an internal-consistency check, not an
independent discriminator --- the packing-reachable tag is the
forced/free label by construction (Paper~57 \S6.1), and the genuine
discriminator signal is the two-family (multi-focal composition
vs.\ inner-factor input-data) decomposition.  Paper~32~\S~VIII
consolidates eight theorem-grade non-selection results (Forced-Count,
H1 Yukawa, $N_{\mathrm{gen}}$, KO-dimension, single-cutoff spectral
action, spatial-composition wall, no-single-mechanism-$K$, and the
cutoff function as external test-function data) across four sectors:
inner-factor structural skeleton, multi-focal composition,
$\alpha$-combination, and gravity.  The seam's individual entries are
thus theorem-bound;\ but Paper~57 remains an observations paper --- the
meta-theorem that would decide forcedness \emph{without} first locating
a witness derivation is still open, with a partial Bohr-site answer
(Paper~57's Bohr-site dichotomy: forced $=$ site property, calibration
$=$ valuation, admitted $=$ site degeneracy).
...
archive and a synthesis folder.  Each group is described
paper-by-paper in the repository's \texttt{papers/INDEX.md}.  This
section gives one-line summaries.

\begin{itemize}
  \item \textbf{Group~1 --- Operator algebras / NCG} ($16$ papers).
    Math.OA arc.  Headline:\ unconditional SU(2) state-space
    Gromov--Hausdorff convergence with explicit $4/\pi$ rate
    (Paper~38, translation-seminorm metrization);\ the Lorentzian degeneracy theorem
    ...
    for descoped metric-level claims.
  ...
  \item \textbf{Group~3 --- Foundations} ($11$ papers).
    Headline:\ Paper~0 (packing), Paper~7
  ...
  \item \textbf{Group~4 --- Quantum computing} ($4$ papers).
    Qubit-encoding resource estimates.  Headline:\ exactly linear
    composed Pauli scaling in $Q$ at fixed basis
    ($N_{\mathrm{Pauli}} = 27.90 \times Q$) and a sub-quadratic
    $O(Q^{1.77})$ $1$-norm across a $37$-system library
    (Papers~14/20);\ absolute Pauli
    multipliers are convention-sensitive (near parity with STO-3G,
    $76\times$ vs cc-pVDZ).
  \item \textbf{Group~5 --- QED / gauge / SM} ($8$ papers).
    Headline:\ hydrogen Lamb shift at $-0.534\%$ one-loop closure
    (Paper~36);\ SU(2) Wilson lattice gauge structure with
    maximal-torus reduction to the U(1) of Paper~25 (Paper~30);\ QED on
    $\sthree$ with five theorems and the two-loop constant $S_{\min}$
    identified in closed form (Paper~28).
...
this field guide together with a complete layer of \emph{six}
group-level syntheses---one per audience branch (Groups~1--6), each
digesting its branch in four to six pages.  This field guide is the
grand synthesis that composes them;\ a reader who wants a single
branch should read that branch's group synthesis directly.
...
\begin{itemize}
  \item \textbf{The $\alpha$-derivation}.  Paper~2's combination rule
    ...
    eliminated for a single-principle derivation;\ the project's
    standing internal reading is that $\alpha$ is a projection
    constant, not a derivable number.
  \item \textbf{The Yukawa values}.  A dedicated PSLQ search
  ...
  \item \textbf{$N_{\mathrm{gen}} = 3$}.  A deep structural
    obstruction;\ no line of attack is known.
  \item \textbf{The cosmological constant problem}.  Standard fine-tuning
    ...
    proposed.
  \item \textbf{The chemistry correlation wall (W1e)}.  Multi-determinant FCI correlation
    in second-row and heavier diatomic binding is below the
    framework's structural accuracy ceiling.  Six independent
    diagnostic investigations identified the obstruction as
    multi-focal-composition rather than a missing single mechanism.
  \item \textbf{The M2/M3 motivic Galois unification (Q5')}.  Both M2 and
    ...
    (Paper~55~\S~7.6).  A deep frontier.
  \item \textbf{Deligne--Milne pro-finite reconstruction}.
    ...
    (Deligne--Milne 1982 Theorem~2.11) is the named deep
    frontier;\ every intermediate step nonetheless closed readily at
    finite cutoff, suggesting the pro-finite assembly may prove
    similarly tractable once the right transition-map machinery
    is in place.
  \item \textbf{Non-commutative Mondino--S\"amann (Q2')}.  Paper~49
    ...
    diamonds) is the remaining substantial NCG-research target.
  \item \textbf{Inhomogeneous extensions}.  The full
    ...
    Paper~51~\S G4-3 (named follow-on).
  \item \textbf{Full Standard Model spectral triple}.  The Connes
    Standard Model on $\sthree$ alone was closed in May~2026 ---
    positive, but structurally thin;\ the genuine cross-manifold
    construction $\sthree \otimes \mathrm{Hardy}(\sfive)$ is blocked
    at the Coulomb-versus-harmonic-oscillator asymmetry of
    Paper~24~\S~V at layer~4 (modular Hamiltonian of the wedge KMS
    state).  Resolution requires NCG-framework extension.
\end{itemize}
...
should start.  Each audience now has a one-document digest---its
\emph{group synthesis}---which is the recommended first stop before
the individual papers:\ Group~1 (operator algebras), Group~2
(quantum chemistry), Group~3 (foundations), Group~4 (quantum
computing), Group~5 (QED / gauge / gravity), and Group~6 (precision
observations).  The per-paper entries below are the second step.
...
\begin{itemize}
  \item \textbf{The mathematician (NCG, math.OA, spectral triples)}.
    ...
    Papers~38, 45, and 32.  Paper~38 is the unconditional SU(2)
    state-space Gromov--Hausdorff convergence theorem
    (translation-seminorm metrization, compression/lifted-state
    proof);\ Paper~45 is the Lorentzian degeneracy theorem (its
    History remark records a withdrawn earlier convergence
    claim);\ Paper~32 is the explicit spectral triple
    ...
    identification;\ Paper~47's norm-resolvent arrow stands;\
    Papers~46, 48, and 49 carry Status notes.
  \item \textbf{The mathematician (periods, motives, Tannakian)}.
    ...
    pro-finite limit is the named deep frontier.  Open
    M2/M3 unification question (Q5') discussed in Paper~55~\S~7.6
    and Paper~56's open-questions section.
  ...
  \item \textbf{The physicist (gravity)}.  Read Paper~51.  Spectral
    action approach, two-term exact on $\sthree$;\ the
    $S_{\mathrm{BH}} = A/4$ replica derivation was closed by the
    replica-weight-harmless theorem.
...
descents,'' arXiv:1411.4947 (2015);\ \textit{J.\ Number Theory}~\textbf{160},
334--384 (2016).
...
J.~Loutey, ``Paper 38: State-space Gromov--Hausdorff convergence of truncated
Camporesi--Higuchi spectral triples,'' GeoVac corpus (2026);\
released to Zenodo 2026-05-07.

\bibitem{loutey_paper40}
J.~Loutey, ``Paper 40: State-space Gromov--Hausdorff convergence of
...
theorem,'' GeoVac corpus (2026).

\bibitem{loutey_paper46}
J.~Loutey, ``Paper 46: Strong-form Lorentzian propinquity on truncated
SU(2)$\times$U(1)$_T$ Krein spectral triples,'' GeoVac corpus (2026).
