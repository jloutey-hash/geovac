...
application --- Pauli-operator counts scaling as $O(Q^{2.5})$ in the
qubit count $Q$ for the core--valence ``composed'' encoding, with a
sub-quadratic $O(Q^{1.69})$ $1$-norm, benchmarked across a $37$-system
...
minimal-basis STO-3G, $\sim 190\times$ sparser than cc-pVDZ);\ the
...
\begin{itemize}
  ...
  \item \textbf{Group~4 --- Quantum computing} ($4$ papers).
    ...
    composed Pauli scaling and a sub-quadratic $O(Q^{1.69})$ $1$-norm
    across a $37$-system library (Papers~14/20);\ absolute Pauli
    multipliers are convention-sensitive (near parity with STO-3G,
    $\sim 190\times$ vs cc-pVDZ).
