...
% Status: Project archive synthesis, revised August 2026 (Paper 58 promotion)
...
arc---the twelve Group~2 papers spanning one-electron atoms, the
two-center one-electron ion, the two-electron atom, the two-center
two-electron molecule, the core--valence and polyatomic
extensions, the genuine two-center tensor census, the transcendence
frontier at the third center, and the isoenergetic Sturmian secular
equation---into a single
coherent narrative organized by the
...
\emph{Second}, two structural negative theorems delimit the
design space:\ in the single-$n$ $D$-matrix \emph{approximation} to a
single-center molecular Sturmian basis with a shared momentum scale, the
one-electron Hamiltonian and overlap obey the
...
$R$-dependent~\cite{loutey_paper8}.  (Paper~8's own scope remark is
explicit that this delimits that approximation and is \emph{not} a no-go
for Sturmian molecular binding:\ the genuine cross-$n$ Coulomb--Sturmian
overlap breaks the shared congruence and supplies the $R$-dependence
algebraically---see the Paper~60 entry below, where the method binds.)
And a graph
...
potential-energy surface with no equilibrium~\cite{loutey_fci_mol}.  A
later genuine two-center census sharpens where the sparsity lives:\ evaluated
over orbital products that span two nuclei the Gaunt $l$-selection rule
fails and only the axial $m$-rule survives, so the angular sparsity is an
\emph{atomic-sector} property---the abelian residue of the molecular
point group~\cite{loutey_paper58}.
\emph{Third}, honest accuracy ceilings:\ the framework reaches
machine precision for $\mathrm{H}_2^+$, $0.022\%$ (properly variational, raw
$l_{\max}=7$) down to $0.004\%$ (a non-variational cusp extrapolation)
for He, $99.97\%$ of $D_e$ ($0.053$~mHa) for $\mathrm{H}_2$ via an
explicit-$r_{12}$ extension of the prolate spheroidal engine with exact
algebraic integrals ($99.81\%$ / $0.32$~mHa via re-conditioned CI at the
variational optimum $\alpha = 1.40$)---and $96.0\%$ (cusp-corrected) in
the molecule-frame hyperspherical treatment---two routes to the same
system, not a matched pair, and $\mathrm{H}_2$'s natural geometry is the
prolate spheroid---and---in the
...
2 & $\mathrm{H}_2^+$ (2, 1) & prolate spheroid & machine precision (spectral Laguerre) & \cite{loutey_paper11} \\
3 & He (1, 2) & hyperspherical & $0.022\%$ (2D var.\ raw) to $0.004\%$ (cusp-extrapolated, non-variational); $0.216\%$ (graph-native CI) & \cite{loutey_paper13} \\
4 & $\mathrm{H}_2$ (2, 2) & molecule-frame hyperspherical & $96.0\%$ of $D_e$ ($l_{\max}=6$, 61 channels, cusp-corrected) & \cite{loutey_paper15} \\
4$^{\ast}$ & $\mathrm{H}_2$ (2, 2) & prolate spheroidal & $99.97\%$ of $D_e$ ($0.053$~mHa) via explicit-$r_{12}$ CI (exact integrals);\ $99.81\%$ ($0.32$~mHa) via re-based CI at $\alpha_{\rm opt}=1.40$ & \cite{loutey_paper12} \\
...
follows.  In the molecular Sturmian basis~\cite{avery2006} with a shared momentum scale
...
definition an $R$-dependent lowering of the energy---so \emph{within this
approximation} binding is excluded.  Paper~8 sharpens
...
\emph{built on the single-$n$ $D$-matrix approximation} (nested
hyperspherical, unified-basis, single-$p_0$ Sturmian CI), and one
resolution---reintroduce the $R$-dependent spectrum by
...
\paragraph{Scope correction (Paper~8 \texttt{rem:overlap\_imposed}).}
The theorem's backing test \emph{imposes} $S_{AB}=f\,D^{(n)}$ by
construction, so what it verifies is a property of the bond-sphere
$D$-matrix model, not of the genuine cross-$n$ Coulomb--Sturmian basis
(where, e.g., $\langle\chi^A_{1s}|\chi^B_{2s}\rangle=-0.47$ exceeds the
within-$n$ diagonal $+0.35$ at $R=3.015$).  A cross-$n$ overlap breaks the
shared congruence, so binding is \emph{not} excluded for the genuine
basis:\ Paper~8's \texttt{cor:binding} records that the $R$-dependence is
then supplied \emph{algebraically} by the Shibuya--Wulfman overlap
(a function of $s=p_0R$), needing no continuous two-center PDE, and
Paper~60 exhibits the binding directly.  Prolate spheroidal geometry
remains the route this arc took; it is not the only one the theorem
permits.
...
$E_{\min} = -0.602634214494936$~Ha against the reference
...
$250\times$ dimension reduction and a $270\times$ speedup, and reaches
machine precision, against a finite-difference baseline
...
Legendre moments satisfy exact three-term recurrences, so the
...
numerically unstable at the base case.  A
...
eliminated~\cite{loutey_paper12}.  The remaining $7.6\%$ gap is a
...
correlation---are absent by construction.  Restoring them at
$|m| \le 1$ in the same basis, with the same Neumann kernel, reaches
$99.1\%$ of $D_e$ in the original monomial basis---a gain of
$11.6$~mHa against the $0.34$~mHa
available from near-tripling the $\sigma$ basis.  Re-conditioning that
same span (Laguerre $\times$ Legendre, high-precision assembly) then
climbs to $99.81\%$ ($0.32$~mHa, inside chemical accuracy) at the
variational optimum $\alpha = 1.40$, and an explicit-$r_{12}$ extension of
the same engine---every $r_{12}$ matrix element reduced to the same
algebraic Neumann kernels, no quadrature---reaches $99.97\%$
($0.053$~mHa)~\cite{loutey_paper12}.  The
$|m| = 2$ sector was numerically unstable in the differentiation-based
radial engine, so the $\delta$ contribution (${\approx}0.5$~mHa) was
first fixed by an independent Gaussian-basis route;\ the quoted figure
carries a stability envelope, the basis being strongly linearly
dependent at that size.  Paper~12 has since generalised the same
$A_l$, $B_l$, $X_l$ recurrences to the associated Legendre functions,
giving a moment-recurrence engine that is stable in the $\delta$
sector---where the differentiation route drove the energy far below
the variational floor---and reproduces the sound $\sigma$ and
$|m| \le 1$ results;\ the $\mu > 0$ channels are therefore now
recurrence-based
(recurrence-stable and---since the $B$-seeds were closed in
$\{E_1, \gamma, \ln\}$---fully quadrature-free).
...
natural geometry.  Paper~12 has withdrawn both, and the correction reached
...
Paper~13~\cite{loutey_paper13} introduces hyperspherical coordinates
...
integrals and reaches $0.216\%$ at $n_{\max}=7$ with no free
...
\emph{independent of molecular geometry}---a structural difference from
...
into an accuracy advantage---and the angular eigenvalue problem depends on $(R, R_e)$
...
solver that avoids the adiabatic approximation, recovers $96.0\%$ (cusp-corrected;\ ${\sim}95\%$ pure-variational) of
...
channels and the $92.4\%$ does not.  At matched $m_{\max}$ no comparison is
available:\ Paper~12's own basis with $|m| \le 1$ reaches $99.1\%$ in its
original monomial basis and $99.81\%$ re-conditioned,
but those figures and the $96.0\%$ differ in truncation, cusp treatment and
solver class alike.  Neither paper claims a coordinate-system advantage
...
angular correlation, and neither geometry gets it without them.  The framework extends to heteronuclear
...
construction reaches $19.4\%$ (improved from $26\%$ by general solver evolution---\emph{not}
by the PK-consistency fix, which the uncoupled PES does not invoke) with a correct qualitative
...
contributing negligibly~\cite{loutey_paper17}.  This wall is later
relocated from the correlation treatment to the Hamiltonian specification
itself, and given a group-theoretic mechanism, in
Sec.~\ref{sec:pkfree_fci}.
...
the energy axis converges through $n_{\max}=3$ while the geometry drifts---but
not that the energy axis is simply the good one:\ at $n_{\max}=4$ the model
crosses \emph{past} exact (over-binding), so the fixed-geometry error is not
monotonically improvable either~\cite{loutey_paper19}.  The balanced coupled
Hamiltonian is therefore suited to single-point quantum simulation at fixed
geometries rather than to full potential-energy surfaces, with that
non-monotonicity disclosed.  Paper~19 also records a clean
...
reduces but does not close~\cite{loutey_paper19}.  That NaH wall is
crossed in Paper~58 (next subsection), by evaluating the integrals over
genuine two-center orbital products rather than atomic-sector
surrogates---at the cost of the label-generated
sparsity~\cite{loutey_paper58}.

\subsection{The abelian residue:\ why the sparsity is atomic-sector (Paper 58)}

Paper~58~\cite{loutey_paper58} closes the question the balanced-coupled
work leaves open---whether the label-generated sparsity and the genuine
cross-center physics can coexist---and the answer is group-theoretic
rather than technical.  Every prior molecular builder evaluates its
...
through a surrogate (the composed builder's $Z_{\mathrm{eff}}$ screening;
the balanced builder's displaced-nucleus one-body term, with both
orbitals still on one center).  Because neither ever forms a genuine
two-center orbital product, the Gaunt $l$-selection rule is exact for
them \emph{by construction}, and the sparsity follows.

Evaluated instead over orbital products that genuinely span two nuclei,
$\langle \chi^A | \cdot | \chi^B \rangle$, in exact rational arithmetic
(Mulliken auxiliary functions for the overlaps; a hydrogenic eigen-trick
for the cross-center one-body term, certified entry-wise by an exact
bra/ket identity), the census measures that the Gaunt $l$-selection rule
does \emph{not} survive while the axial $m$-rule does, and the
symmetry-permitted electron-repulsion density inflates $13.8\times$ over
the builder at $n_{\max}=2$.  An internal theorem supplies the mechanism.
The label $m$ names the abelian symmetry both centers share---rotation
about the internuclear axis is a single global one-parameter
group---while $l$ names irreducible representations of a rotation group
attached to one center.  The intersection of the two centers' rotation
groups is exactly the axial $U(1)$, whose character group is $\mathbb{Z}$
by Pontryagin duality; abelian selection rules are additive charge
conservation and survive, whereas non-abelian Clebsch--Gordan selection
needs the full sphere and does not.  \emph{The surviving cross-center
sparsity is the abelian residue of the molecular point group, readable in
advance.}

Two re-readings follow.  First, the sparsity previously charged to
L\"owdin orthogonalization (a $17.9\times$ Pauli inflation) was largely a
bill already owed:\ the genuine tensor's own permitted density inflates by
the same order ($\sim\!15\times$) before any transformation, so
orthogonalization is not, in the bulk, the cause of the density blamed on
it.  Second, four earlier relocations of the non-orthogonality each moved
the cost without reducing it---the cost measures the physical asymmetry
of a two-center problem---and equivalent-atom-swap tapering \emph{reduces}
the qubit saving while inflating the Pauli count, because identifying
blocks by permutation destroys the per-block abelian labels that generate
the sparsity.  The sparsity-versus-binding trade is a conservation law,
not a defeat.

The corresponding gain is a panel-verified demonstration.  All-electron
twelve-electron NaH---the framework's hardest documented binding failure,
which six Phillips--Kleinman modifications, an explicit-core
Hartree--Fock treatment, and a DMRG cross-check had each left
unbound---binds at $R_{\mathrm{eq}} = 3.736\,a_0$ ($+4.8\%$ versus
experiment), recovering $91.1\%$ of the in-basis FCI binding from three
fragment-native determinants, and with \emph{fewer} correlation degrees
of freedom than the runs that failed---which is what carries the
inference.  This localizes W1e to the Hamiltonian specification rather
than the correlation treatment:\ the wall was the atomic-sector
construction, not the determinant expansion.  The demonstration
deliberately forfeits the sparsity, because the sparsity and the
atomic-sector evaluation are the same choice.

A subsequently supplied two-center electron-repulsion engine closes all
four integral classes in exact form (at transcendence weight one,
$\pi$-free over $\{E_1, \ln, \gamma\}$), which upgrades the $(AA|BB)$
block of the census from \emph{counted} to \emph{decided}:\ all $195$
symmetry-permitted entries are proven genuinely nonzero by
Lindemann--Weierstrass separation, with no accidental zeros and none
missed by the counting.  For the three genuinely cross classes---$79.5\%$ of the tensor---Paper~58
states a three-rung position that is \emph{materially better than counted}:\
the seed functions $\{1, e^{-\lambda R}, E_1(\mu R), \ln R\}$ are linearly
independent over $\mathbb{Q}(R)$ by Liouville--Rosenlicht, so every permitted
entry is \emph{functionally} nonvanishing (unconditional) and generically
nonzero (measured);\ but it is pointwise \emph{decidable} only conditional on
a Schanuel-type independence hypothesis for the $E_1$ and $\gamma$ values.
The block is therefore blocked from \textsc{decided} by open transcendence,
\emph{not} by missing labour---$E_1$-value independence at algebraic points is
an open problem and $\gamma$ is not known even to be irrational~\cite{loutey_paper58}.
Weight one is not the same as decidable.

\subsection{The transcendence frontier at the third center (Paper~59)}

Paper~58 closes the two-center tensor in closed form;\ Paper~59 asks what
happens at the third.  Read in momentum space, the three-center
electron-repulsion integral is a two-scale Bessel moment whose two Fock
scales define a quartic curve:\ the two-center case is genus zero
(elementary, weight-one $\{E_1,\ln,\gamma\}$), and the third center
raises the genus to one.  Its period is a complete elliptic integral, it
satisfies an explicit fourth-order Picard--Fuchs equation, and the
connection is \emph{irreducible} (a Fourier--Laplace transform of an
irreducible rank-one connection), so no closed form arises through
factorization~\cite{loutey_paper59}.  The collinear observable is
certified to $66$ digits and guarded integer-relation searches are
decisively negative through weight three at clean heights (the companion
Paper~61~\cite{loutey_paper61});\ the finite
closed form remains open, and the paper says so.

Two consequences for this arc.  The polyatomic question below is thereby
sharpened from ``can the genuine-integral route be extended?'' to a
statement about transcendence class:\ the extension is not blocked by
missing algebra but by a genus jump, and any closed-form polyatomic
tensor is all-or-nothing (a part-closed-form, part-fitted tensor is not
decidable at all).  And the same obstruction gates explicitly-correlated
(F12) evaluation, because the difficulty of a two-electron integral lives
in the densities, not in the operator between them.

That density-not-operator reading is borne out constructively at two
centers, where the genus is zero:\ Paper~12 carries out the
explicit-$r_{12}$ (James--Coolidge) CI of a two-center \emph{four-electron}
reference \emph{without} a resolution of identity.  The genuinely four-body
disjoint-pair bridge $f_{12}f_{34}\,r_{13}^{-1}$ and the three-body triangle
$f_{ik}f_{jk}\,r_{ij}^{-1}$---the two integral classes that force an RI in
Gaussian $F_{12}$ theory---both reduce by the \emph{same} Neumann machinery,
screened-Coulomb (Yukawa) kernels included, which is exactly the
operator-independence at work;\ the full analytic $2\times2\to E_{R12}$ is
then assembled and validated against variational Monte-Carlo for both an
$e^{-\gamma r}$ and a cusp-correct $r\,e^{-\gamma r}$ geminal
($E_{R12}=-7.942$ and $-7.917$~Ha, variational)~\cite{loutey_paper12}.  This
is a proof of concept (an ionic single-$\zeta$ reference, not a spectroscopic
LiH):\ the deliverable is the RI-free reduction, not the correlation energy
recovered.  The gate is therefore specifically the three-center genus jump,
not explicit correlation as such.

\subsection{The isoenergetic secular equation as a quantum algorithm (Paper~60)}

Paper~60 carries the Sturmian thread into the quantum-algorithm setting
and, in doing so, exhibits the binding the scoped guardrail above does
not forbid.  Naively encoding shared-scale Coulomb--Sturmians in $L^2$
inflates the block-encoding $1$-norm (the non-orthogonal overlap must be
L\"owdin-inverted, and $S^{-1/2}$ is dense);\ Avery's \emph{isoenergetic} (potential-weighted)
reformulation removes the metric \emph{for atoms} entirely---a metric-free
standard eigenproblem whose eigenvalues are the scaling parameters
$p_\kappa=\sqrt{-2E}$, from which the energies follow
directly~\cite{loutey_paper60}.  Its $1$-norm grows more slowly than the
matrix dimension over every basis the paper can compute ($\|M\|_1\sim
K^{0.82}$ fitted on $K=74$--$164$, helium), and \emph{increasingly} so:\ the
local slope falls monotonically to $0.766$ by $K=514$, so no window fit is
stable and the object is closer to $K^{p}\log K$ than to a power law.  The
nuclear diagonal $T^0=Z\sum_\nu R_\nu$ is the slowest-growing block and the
only one derived rather than measured --- $Z\sqrt{2K}\ln(K/2)$, asymptotic
exponent $1/2$ --- while neither grouping of the pure-number block $T'$ is
superlinear ($K^{0.88}$ full, $K^{0.94}$ off-diagonal).  \emph{The
substantive finding is that the advantage belongs to the basis-growth rule,
...
carries an accuracy
floor bracketed at $[6.47,6.62]$~mHa --- the bracket the paper quotes, since its fit family approaches the floor from below and a single value would be a lower estimate rather than a central one.  \emph{The mechanism of that floor is now pinned, and it is
...
from $K^{0.72}$ to $K^{2.75}$ on the $s$-only comparison ladder
($K=21$--$136$, not the headline window).  Metric-free posing and accuracy floor are one
fact rather than two, both being the scale lock;\ the sublinearity is not part
of that identity, since it belongs to the basis-growth rule.  The floor is moreover a
...
encode --- and at $K=452$ --- the largest basis where both roots were computed, full
...
remove.)  (Superseded 2026-09-07:\ the earlier $K^{0.84}$,
``drifts upward'', $T^0$-at-$K^{0.70}$ and $T'$-superlinear-at-$K^{1.05}$ [retracted 2026-09-08: p60-tprime-superlinear]
readings were measured on a truncated radial domain whose error grew with
the fit variable.)  That split is what decides the molecular case.  For molecules the Shibuya--Wulfman metric returns, and the paper
measures rather than assumes its cost:\ the conditioning grows
...
frame the same band is worse than no treatment at all), and a direct block-encoding of the preconditioned metric takes the
penalty from $n^3$ to $n$.  Established on $s$-sector shared-scale bases at
$M=2$ and non-collinear $M=3$;\ the collinear case is open.  What remains standing is the \emph{locality} cost,
capped by the symbol's other pole, and the $\ell$-block sparsity loss, which is
structural rather than a conditioning effect.  The $N$-electron $1$-norm question is answered in the
negative:\ the atomic sublinearity is a property of the isoenergetic
secular matrix's single-center diagonal, not of the basis, and it does not
survive two centers.

For the guardrail above this is the operative datum:\ the method
\emph{binds}---$\mathrm{H}_2^+$ at $1.3\%$ $s$-only underbinding and a
minimal-basis $\mathrm{H}_2$ at $E_{\rm tot}\approx-1.09$~Ha, in a
common-scale basis---because the genuine overlap supplies the
$R$-dependence the single-$n$ approximation lacked.  What it does not do
is recover GeoVac's angular sparsity, which is the trade Paper~58 states
...
$0.26\%$ at $n_{\max}=5$ (grid-based) and $0.216\%$ at $n_{\max}=7$ with
...
the Sturmian structural theorem of Paper~8---rules out every
single-center Sturmian and bond-sphere alternative \emph{of the
single-$n$ $D$-matrix form} as a repair; a
...
single-center basis in the single-$n$ $D$-matrix approximation; one
concatenated graph) cannot bind in the forms analysed.  The
shared-exponent leg is the scoped one:\ the genuine cross-$n$ basis
escapes it algebraically (Paper~8 \texttt{cor:binding}; Paper~60), at the
cost of the qubit-encoding sparsity that motivated the restriction.
...
Prolate spheroidal $V_{ee}$ ($\sigma$) & algebraic (one seed) & Neumann recurrences $+$ 1-D quadrature for $B_l$ \cite{loutey_paper12} \\
...
re-derived.  Paper~58~\cite{loutey_paper58} sharpens the scope of that
payoff:\ the $l$-selection component of the sparsity is an atomic-sector
property, exact for the composed encoding precisely because it evaluates
per center, so the favorable scaling characterizes small systems
evaluated in the atomic sector rather than genuine multi-center
integration.  This synthesis establishes the chemistry-side foundation of
...
$l_{\max}=2$ operating point.  Whether a composition rule could close
W1e without sacrificing the $144\times$ angular compression---i.e.\
without reverting to the exact-but-expensive Level-4N encoding---was the
central open chemistry question of the classical-solver arc; Paper~58
answers it in the negative and gives the reason~\cite{loutey_paper58}.
The $l$-selection sparsity is exact only in the atomic sector---it is the
abelian residue that survives genuine two-center evaluation---so closing
the bond requires forming the two-center orbital products that destroy
it.  W1e is thereby relocated from the correlation treatment (a DMRG
cross-check reproduces FCI bit-identically without binding) to the
Hamiltonian specification itself, and the sparsity-versus-binding trade
is a conservation law rather than a missing integral.  What remains open
is quantitative:\ how much of the residual a genuine-integral
construction recovers across a range of systems, of which NaH ($+4.8\%$)
...
Hamiltonian hit an orthogonalization wall for second-row systems such as
NaH~\cite{loutey_paper19}; the genuine two-center construction of
Paper~58 crosses it (NaH binds at $+4.8\%$), at the cost of the
sparsity~\cite{loutey_paper58}.  The polyatomic $\mathrm{H}_2\mathrm{O}$
construction validates only bond--bond coupling, with bond--lone and
lone--lone couplings unphysical at the composed
level~\cite{loutey_paper17}; extending the genuine-integral route past
two centers is the open polyatomic question---now sharpened by Paper~59
from an algebra question to a transcendence one:\ the third center raises
the genus from zero to one, the resulting connection is irreducible, and
the finite closed form is open~\cite{loutey_paper59}.  Both remain named
...
for $\mathrm{H}_2^+$ (machine precision)~\cite{loutey_paper11}, hyperspherical
coordinates for He ($0.022\%$ variational raw / $0.004\%$ non-variational
cusp, $0.216\%$ graph-native)~\cite{loutey_paper13}, molecule-frame hyperspherical
coordinates for $\mathrm{H}_2$ ($96.0\%$ of $D_e$,
cusp-corrected)~\cite{loutey_paper15}---though $\mathrm{H}_2$'s natural
geometry is the prolate spheroid, where an explicit-$r_{12}$ CI reaches
$99.97\%$ ($0.053$~mHa)~\cite{loutey_paper12}---and composed fiber bundles for
...
bounded by two structural negative theorems:\ a single-center
shared-$p_0$ Sturmian basis cannot bind a molecule \emph{in the single-$n$
$D$-matrix approximation}~\cite{loutey_paper8} (the genuine cross-$n$
basis does bind, algebraically, at the cost of the
sparsity---\cite{loutey_paper60}), and a
concatenated atom-centered graph has no equilibrium~\cite{loutey_fci_mol}.
Those theorems are why the hierarchy is the route this arc took;\ the
first is a scoped guardrail rather than an unconditional no-go.
...
\bibitem{neumann1878}
C.~Neumann,
\textit{Theorie der Elektricit\"ats- und W\"arme-Vertheilung in einem Ringe}
(Halle, 1864).
...
\bibitem{avery2006}
J.~E.~Avery and J.~S.~Avery,
\textit{Generalized Sturmians and Atomic Spectra}
