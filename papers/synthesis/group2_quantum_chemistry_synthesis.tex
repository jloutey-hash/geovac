...
shared momentum scale the one-electron Hamiltonian is proportional to
the overlap matrix, so its eigenvalues are independent of the
internuclear distance and cannot bind~\cite{loutey_paper8}; and a graph
...
$0.0002\%$ for $\mathrm{H}_2^+$, $0.004\%$--$0.022\%$ (properly
variational) for He, $96.0\%$ of $D_e$ for $\mathrm{H}_2$, and---in the
...
3 & He (1, 2) & hyperspherical & $0.004\%$--$0.022\%$ (2D variational); $0.19\%$ (graph-native CI) & \cite{loutey_paper13} \\
4 & $\mathrm{H}_2$ (2, 2) & molecule-frame hyperspherical & $96.0\%$ of $D_e$ ($l_{\max}=6$, 61 channels) & \cite{loutey_paper15} \\
4N & LiH (2, 4) & full mol-frame ($SO(12)$) & $R_{\mathrm{eq}}\approx 64\%$ (unbound $D_e$, no PK) & \cite{loutey_paper17} \\
...
eigenvalue of orbital~$j$.  Because $H \propto S$, the generalized
eigenvalues $E_k = \tfrac{1}{2}p_0^2(1/\beta_k - 1)$ are independent of
...
$E_{\min} = -0.6025$~Ha against the exact $-0.6026$~Ha~\cite{bates1953}---a $0.0002\%$
...
$0.022\%$ raw at $l_{\max}=7$ and $0.004\%$ with a Schwartz cusp
correction at $l_{\max}=4$, both genuine variational upper
bounds~\cite{loutey_paper13}.  A separate, fully zero-parameter route
...
$l_{\max}=4$ with $\sigma+\pi$ channels~\cite{loutey_paper15}.
...
exact rational Slater integrals; lithium $1.07\%$ and beryllium
$0.90\%$ at $n_{\max}=4$ (the latter with $487{,}635$
...
Together the two negative theorems---Sturmian structural ($H \propto S$
$\Rightarrow$ $R$-independent eigenvalues, Paper~8) and graph
...
coordinates for He ($0.004\%$--$0.022\%$ variational, $0.19\%$
graph-native)~\cite{loutey_paper13}, molecule-frame hyperspherical
