...
\textbf{(H1)} \textbf{[SYMBOLIC PROOF]} The discrete Fock graph's continuum
limit is conformally equivalent to the unit $S^3$ at the level of the
Laplace--Beltrami operator (Paper~7~\cite{loutey_paper7});
\textbf{[MEASURED]} the graph's convergence to that limit is numerical
(through $n_{\max} = 30$, no rate proven), while
...
graph Laplacian lifts the $s/p$ degeneracy at finite cutoff (up to $37\%$
at $n_{\max} = 5$ on the binary lattice, below $1\%$ by $n_{\max} = 30$).
...
\textbf{[MEASURED]} The graph Laplacian on the GeoVac lattice converges, in the continuum
...
in Paper~7 (verified to $n_{\max} = 30$, without a rate), remains
numerical for the graph Laplacian itself; what is now a theorem is the
...
Laplacian spectrum converges to the pure rational numbers $n^2 - 1$ of
the continuum $S^3$ operator, the
...
\textbf{[SYMBOLIC PROOF]} The discrete Fock graph's continuum limit is conformally equivalent to the
unit $S^3$ Laplace--Beltrami operator (Paper~7~\cite{loutey_paper7}),
the graph's convergence to that limit being numerical rather than
proved, while the state-space GH convergence of the spinor spectral
