...
Paper~56~\cite{loutey_paper56} closes the cosmic-Galois comparison $U^*_{\mathrm{GV}}
\hookrightarrow \mathcal{U}_4^{\mathrm{ab}} \rtimes SL_2$, and Paper~57~\cite{loutey_paper57}
...
(through $n_{\max} = 30$, no rate proven).  \textbf{(H2)} \textbf{[INTERNAL THEOREM]} The angular sparsity
...
theorem:\ \textbf{[INTERNAL THEOREM]} for the spinor (Dirac) triple it is an unconditional
...
universal across compact Lie groups (Paper~40~\cite{loutey_paper40}); the scalar
...
within the series;\ the results quoted in this section were re-run
and reproduce, and their regression backing is live in
\texttt{tests/test\_rabi\_oscillation.py}.  Until 2026-09-01 that
backing existed but was never executed by any test run---the file
was named so that collection skipped it, and its cases were wired
as a script rather than as tests.  The graph Hamiltonian $H$ is
sparse with $O(V)$
...
operator: $(D - A)$ carries the hydrogenic spectrum $n^2 - 1$ through
its conformal identification with the continuum $S^3$ Laplace--Beltrami
operator.  On
...
that Paper~56 reaches for---a Hain--Brown mixed-elliptic-motive target---came
...
proved; \textbf{[INTERNAL THEOREM]} the
