...
arc---nine papers covering the packing construction, the discrete
...
Bargmann--Segal lattice, and the universal/Coulomb partition---into a
single coherent narrative.  Four headline structural results frame the
...
The Group~3 (foundations) arc comprises nine papers.  Paper~0
\cite{loutey_paper0} states the packing axiom and derives the
angular quantum numbers as packing indices.  Paper~1
\cite{loutey_paper1} sets up the spectral graph theory machinery and
demonstrates exact reproduction of the Rydberg spectrum.  Paper~6
\cite{loutey_paper6} extends the framework to time evolution at
$O(V)$ cost.  Paper~7~\cite{loutey_paper7} proves the conformal
equivalence with the unit $S^3$ via 18 independent symbolic
verifications and extends to the multi-electron $S^{3N-1}$ setting.
Paper~18~\cite{loutey_paper18} classifies transcendental content in
GeoVac observables via a six-tier exchange-constant taxonomy and
identifies a master Mellin engine on the spectral side.  Paper~21
\cite{loutey_paper21} synthesizes the proof chain and frames the
atomic spectra research program.  Paper~22~\cite{loutey_paper22}
states the potential-independent angular sparsity theorem.
Paper~24~\cite{loutey_paper24} constructs the Bargmann--Segal lattice
for the harmonic oscillator and establishes the four-layer
Coulomb/HO asymmetry.  Paper~31~\cite{loutey_paper31} states the
A/D universal/Coulomb partition in spectral-triple language.
...
in keeping with the project rule that the combination rule remains
conjectural at the rule level despite three independent spectral homes
for $B$, $F$, and $\Delta$.
...
dimensionless graph eigenvalues $\lambda_n = -(n^2 - 1)$ to the
physical Rydberg spectrum via the energy-shell focal length
$p_0 = Z/n$.  Recent analysis~\cite{loutey_paper18} has clarified that
$\kappa = -1/16$ is itself derivable from the Fock conformal projection
rather than fitted: the squared coupling between adjacent $n$-shells
in the Gegenbauer eigenbasis is $c^2(n, l) = (1/16)[1 - l(l+1)/(n(n+1))]$,
giving $c^2(n, 0) = 1/16$ universally for $l = 0$.  Equivalent
readings identify $1/16$ as the squared Chebyshev transition
amplitude, as the inverse Fock Jacobian $1/\Omega^4(0)$, and as the
$l = 0$ base rate of the Casimir decomposition.  Paper~18 reclassifies
$\kappa$ accordingly from \emph{calibration} to \emph{conformal} in the
exchange-constant taxonomy.
...
$S^3$ Laplace--Beltrami operator.  Imposing the energy-shell
...
\subsection*{The four-layer Coulomb/HO asymmetry}

A central structural finding of Paper~24, sharpened by subsequent
analysis in Paper~25 (synthesis group) and a recent sprint on the
wedge KMS state of the framework's Lorentzian extension, is that the
Coulomb and HO discretizations differ along four independent layers.
...
operator-algebra-side reading of the four-layer asymmetry; for full
details see the operator-algebra arc (Group~1).
\end{enumerate}

The first three layers were established in Paper~24 and Paper~25
(synthesis group, S$^5$ gauge structure paper); the fourth layer was
identified in a 2026 sprint on the wedge KMS state and folded into
Paper~24's \S{}V as an addendum.  All four layers stratify along the
same operator-order divide: the Coulomb side uses a second-order
Riemannian Laplacian with a nonlinear stereographic projection, and
the HO side uses a first-order complex-analytic Euler operator with a
linear-affine Bargmann projection.
...
itself remains conjectural at the rule level: there is no
...
question and does not assert priority for the conjectural identification.
...
the foundations arc, Paper~24's four-layer Coulomb/HO asymmetry
...
foundations arc.  The $\alpha$ combination rule remains conjectural;
...
C.~I.~P\'erez-S\'anchez,
``On the gauge networks of Marcolli--van Suijlekom: Yang--Mills without Higgs,''
...
J.~Loutey,
``Fine Structure Constant from the Hopf Bundle on the Fock-Projected $S^3$ Graph,''
...
J.~Loutey,
``Projection Taxonomy and Empirical Matches Catalogue,''
