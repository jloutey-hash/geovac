...
% Status: Project archive synthesis (living document; corrected in place -- version record in git/CHANGELOG)
...
program: \textbf{[SYMBOLIC PROOF]} (a) the conformal chain \emph{from the
continuum operator onward} --- the unit-$S^3$ Laplace--Beltrami operator, its
integer spectrum, and the energy-shell projection --- verified by a suite of 18
symbolic checks;\ \textbf{[OBSERVATION]} that the discrete Fock graph's
continuum limit \emph{is} that operator is a reading with numerical
indications, not an established convergence (corrected 2026-09-04:\ this
headline tagged the whole statement \textbf{[SYMBOLIC PROOF]}, which the
document itself denies at three later loci, and the 18 checks do not touch the
graph-side step) (ten of the eighteen limiting-case or
definitional rather than logically independent, Paper~7); \textbf{[INTERNAL THEOREM]} (b) the angular sparsity of two-body matrix elements
...
about spherical-fermion bases; \textbf{[PANEL-VERIFIED]} (c) the three-dimensional isotropic
...
arithmetic at every finite cutoff; \textbf{[OBSERVATION]} (d) the framework's results split
...
candidate fine-structure-constant combination of Paper~2~\cite{loutey_paper2}.  \textbf{[OBSERVATION]} A structural reading we make explicit in this synthesis is that
...
conformal field theory embodies at
the physics level (cf.\ the F-theorem of~\cite{klebanov_pufu_safdi2011}).  GeoVac provides a scale-free mathematical
substrate; CFT is a scale-free physics theory living on such a
substrate.  This is not a claim that GeoVac is a CFT; it is a
structural reflection consistent with the framework's reproduction of
...
concrete computational advantages---$O(V)$ sparsity, angular momentum
selection rules baked into the basis, bit-exactly rational matrix
elements on the Bargmann--Segal lattice, and the universal sparsity
bound---are independent of that interpretive question. (The integer
spectrum $n^2 - 1$ is a property of the continuum operator, not of the
graph, whose closed-form spectrum is bounded in $[0, 8]$;\ an earlier
version of this sentence listed it among the discrete advantages.)
\end{abstract}
...
co-workers~\cite{aquilanti_caligiana2002}, and others working in the
...
focal length.  The framework's central reading, recorded in Paper~7 as an
Observation (re-priced 2026-09-03), is that this discrete construction
converges to the unit $S^3$ Laplace--Beltrami operator in the continuum
limit; what is measured is the spectral-bound saturation and the $s/p$
proxy decay [retracted 2026-09-04: sp-splitting-as-convergence-evidence], not the operator convergence --- and the
degeneracy-recovery leg is itself an artifact of cutoff divisibility
($\lambda_{2s} = 3$ exactly iff $n_{\max} \equiv 0 \bmod 3$;\ Paper~1 \S III,
2026-09-03), so the saturation of the spectral bound is the only leg that
carries weight.  The discrete and
...
it physical, verifying the conformal equivalence with the unit $S^3$ via
a suite of 18 symbolic checks (ten of the eighteen limiting-case or definitional)
...
graph theory and reproduces the Rydberg quantum-number structure (the
energies follow once the matched scale $\kappa$ is applied);
...
Bargmann--Segal lattice and the seven-layer Coulomb/HO asymmetry;
...
statement, \S\ref{sec:reconvergence}):\ Paper~54~\cite{loutey_paper54} derives two-body
selection rules from the tensor-product triple, Paper~55~\cite{loutey_paper55} classifies
every GeoVac period as cyclotomic mixed-Tate at level $\le 4$,
Paper~56~\cite{loutey_paper56} constructs the cosmic-Galois comparison map $U^*_{\mathrm{GV}}
\to \mathcal{U}_4^{\mathrm{ab}} \rtimes SL_2$ (a homomorphism, not a closed immersion), and Paper~57~\cite{loutey_paper57}
...
\textbf{(H1)} \textbf{[SYMBOLIC PROOF]} The conformal chain from the
unit-$S^3$ Laplace--Beltrami operator to the Schr\"odinger equation is
verified symbolically (Paper~7~\cite{loutey_paper7});
\textbf{[OBSERVATION]} that the discrete Fock graph's continuum limit is
that operator is a reading supported by one proved spectral statement
--- the saturation of the bipartite bound, \textbf{[INTERNAL THEOREM]}
closed form with rate $C/n_{\max}^2$ (Paper~0~\S VI) --- and by nothing
else (the $s/p$-lift decay [retracted 2026-09-04: sp-splitting-as-convergence-evidence]
was withdrawn as evidence on 2026-09-03, its size tracking the residue of
$n_{\max}$ modulo $3$), not an
established convergence (re-priced 2026-09-03), while
\textbf{[INTERNAL THEOREM]} the state-space Gromov--Hausdorff
convergence of the spinor spectral truncations is proved
unconditionally with rate $(\gvq{l2_rate_4_over_pi}{4/\pi})\log n_{\max}/n_{\max}$ (dual-Coxeter normalisation; $2/\pi$ on the unit $S^3$, $2\sqrt2/\pi$ under the field-standard Kac form)
(Paper~38~\cite{loutey_paper38}).  \textbf{(H2)} \textbf{[INTERNAL THEOREM]} The angular sparsity
...
\textbf{(H3)} \textbf{[PANEL-VERIFIED]} The 3D harmonic oscillator admits a discrete graph
encoding on the Hardy sector of $S^5$ that is bit-exactly $\pi$-free
(Paper~24~\cite{loutey_paper24}).  \textbf{(H4)} \textbf{[OBSERVATION]} The framework's
...
\textbf{[SYMBOLIC PROOF]} Applied to concentric isotropic shells in two dimensions, these axioms
...
\textbf{[OBSERVATION]} The principal quantum number~$n$ enters as a cumulative grouping of
...
algebra~\cite{barut1967,biedenharn1981} are imposed as edges:
\begin{align}
...
$n$-shells at fixed $l$.  \textbf{[PANEL-VERIFIED]} Paper~1~\cite{loutey_paper1} demonstrates
that the Casimir invariants built from the ladder operators reproduce
the Rydberg quantum-number structure ($n$ and $l(l+1)$) exactly; the
energies $E_n = -1/(2n^2)$ follow only once the matched kinetic scale
$\kappa = -1/16$ is applied, which the corpus records as an Observation
(Paper~7).  At the algebraic level the discrete and continuous
descriptions therefore agree on the quantum-number skeleton, while the
graph Laplacian's node-amplitude proxy for the $s/p$ gap is large at finite
cutoff (up to $37\%$ at $n_{\max} = 5$ on the binary lattice;\ $0.39\%$ at
$n_{\max} = 30$, which sits on the $n_{\max} \equiv 0 \bmod 3$ branch, against
$1.65\%$ and $2.58\%$ at $28$ and $29$; the $\ell$-blocks are disconnected,
so this is a proxy and not a spectral gap --- Paper~1 \S III).

\subsection*{The universal kinetic constant $\kappa = -1/16$}

\textbf{[OBSERVATION]} A second invariant of the packing geometry is the universal kinetic
...
eigenbasis is $c^2(n, l) = (1/4)[1 - l(l+1)/(n(n+1))]$ (corrected
2026-09-03 from a printed $1/16$ prefactor; the Chebyshev amplitude is
$1/2$), giving $c^2(n, 0) = 1/4$ for $l = 0$; the quantity that
coincides with $1/16$ is the inverse Fock Jacobian $1/\Omega^4(0)$,
equivalently $c^2(n,0)/4$.  That the energy-calibration constant
...
\textbf{[OBSERVATION]} The discrete lattice exists \emph{a priori} from the packing
...
structure---the Rydberg quantum-number structure, the chemical
periodicity of multi-electron atoms (Paper~16~\cite{loutey_paper16}, outside the
foundations group), nuclear magic numbers under a harmonic-oscillator
basis (Paper~23~\cite{loutey_paper23}, applications group)---is therefore evidence that
...
established in Paper~7~\cite{loutey_paper7}, between the \emph{continuum}
unit-$S^3$ Laplace--Beltrami operator and the Fock projection.  That the
discrete Fock graph's continuum limit is that operator is a separate and
\emph{unestablished} step (corrected 2026-09-04:\ this sentence named the
graph-to-operator equivalence and called it established, which Paper~7
denies).  This section
states the result and explains its operational role.

\subsection*{The Fock projection and the chordal distance identity}

\textbf{[SYMBOLIC PROOF]} Fock's 1935 construction~\cite{fock1935} writes the momentum-space
...
\textbf{[SYMBOLIC PROOF]} Paper~7~\cite{loutey_paper7} verifies the conformal-equivalence chain
through a suite of 18 symbolic checks implemented in \texttt{sympy}
(Paper~7 flags several of them as limiting-case or definitional
substitutions rather than logically independent theorems).
...
status as machine-verified algebraic identities pins the
continuum-side conformal chain; the discrete-to-continuum step itself
lies outside their scope (Paper~7 states this explicitly).

\subsection*{Schr\"odinger recovery in the continuum limit}

\textbf{[OBSERVATION]} The graph Laplacian on the GeoVac lattice is read as
converging, in the continuum limit $n_{\max} \to \infty$ with appropriate
rescaling, to the round $S^3$ Laplace--Beltrami operator.  This reading,
recorded in Paper~7 with one proved indication --- the closed-form
saturation of the spectral bound, \textbf{[INTERNAL THEOREM]}
(Paper~0~\S VI) --- (the $s/p$-lift decay [retracted 2026-09-04: sp-splitting-as-convergence-evidence] was
withdrawn as evidence on 2026-09-03; no operator-convergence
statement), remains unestablished for the graph Laplacian itself; what is now a theorem is the
companion statement:\ \textbf{[INTERNAL THEOREM]} for the spinor (Dirac)
spectral truncations it is an unconditional
van~Suijlekom state-space Gromov--Hausdorff convergence at rate
$(4/\pi)\log n_{\max}/n_{\max}$ (dual-Coxeter normalisation; $2/\pi$ on the unit $S^3$, $2\sqrt2/\pi$ under the field-standard Kac form) under the
translation-seminorm metrization (Paper~38~\cite{loutey_paper38},
operator-algebra arc), with the $4/\pi$ constant
universal across compact connected semisimple Lie groups
(Paper~40~\cite{loutey_paper40}: rigorous at rank~1, a rank-uniform
analytical proof a named gap; a circle factor carries $2/\pi$); the
scalar analogue of that theorem, for the Peter--Weyl spectral
truncations of $L^2(S^3)$, is the prior-art state-space GH result of
Gaudillot-Estrada--van~Suijlekom~\cite{gaudillot_vs2025}.  Paper~7's
18 symbolic checks (ten of the eighteen limiting-case or definitional) target the
conformal-transformation chain, not a convergence statement, and the
graph-side \emph{operator} convergence has no theorem behind it. The
saturation of the bipartite bound $\lambda_{\max} \to 2 d_{\max} = 8$
\emph{is} one --- \textbf{[INTERNAL THEOREM]} closed form with rate
$C/n_{\max}^2$, $C = \gvq{saturation_c}{42.7397}\ldots$ (Paper~0~\S VI) --- and the frozen
test \texttt{tests/test\_paper7\_graph\_convergence.py} pins it together
with the block structure of the lattice Laplacian;\ neither is operator
convergence (re-scoped 2026-09-03;\ tier corrected 2026-09-04).
Imposing the energy-shell
...
\textbf{[OBSERVATION]} The separation between dimensionless topology and dimensional projection
...
committing to a specific physical value---and outputs a graph whose nodes carry the labels of the eigenspaces of the
continuum $S^3$ operator (the pure rational eigenvalues $n^2 - 1$ belong
to that continuum operator; the graph spectrum is not a level-by-level
copy of it, Paper~7), the
...
in the open-questions section below) is consistent with the
substrate-side scale-freedom established here; no mechanism is claimed.
The substrate is the right kind of mathematical object to host
scale-free physics, which is a reading of the parallel, not an
explanation of it.
...
\textbf{[SYMBOLIC PROOF]} A particular consequence of the $S^3$ projection is the closed-form
single-center two-electron repulsion integral
$F^0(\text{1s}, \text{1s}) = \gvq{slater_f0_1s}{5Z/8}$, derived from the node-overlap
formula for $V_{ee}$ on $S^3$ in Paper~7 for $s$-orbital pairs.  More
generally, all single-center Slater $F^k$ integrals reduce to exact
rationals in arithmetic over $\mathbb{Q}$ (the hypergeometric Slater
engine, \texttt{geovac/hypergeometric\_slater.py}, which is not part
of Paper~7's derivation).  The single-center two-electron problem
...
contributions are: (a) the proved spectral-bound saturation
(\textbf{[INTERNAL THEOREM]}, closed form, Paper~0~\S VI) and the
$s/p$-proxy decay [retracted 2026-09-04: sp-splitting-as-convergence-evidence] through $n_{\max} = 30$ --- the latter now known to
track the residue of $n_{\max}$ modulo $3$ rather than convergence, and so
withdrawn as evidence, 2026-09-03) that the discrete GeoVac
graph converges to the continuous $S^3$ Laplace--Beltrami operator,
recorded as an Observation; (b)
...
exactly $Q$-linear composed Pauli-term count
($N_{\mathrm{Pauli}} = 27.90 \times Q$ at fixed basis, Paper~14~\cite{loutey_paper14}) at
the qubit-encoding level.  The
theorem can be stated cleanly without reference to the Coulomb
potential or to any specific radial wavefunction.

\medskip
\noindent\textbf{Theorem (Paper~22).}  \textbf{[INTERNAL THEOREM]} \emph{For any $N$-fermion
...
\begin{table}[t]
\centering
\caption{Pair-diagonal angular ERI density $D_{\mathrm{pd}}(l_{\max})$
from Paper~22 (per-pair $m_a=m_c$, $m_b=m_d$), which holds only for
axially-symmetric or $m$-decoupled bases.  \emph{The composed pipeline
realizes the physically-correct global-$M_L$ Coulomb density
$D(l_{\max})$, not this one} --- $\sim\!4\times$ larger ($8.52\%$ at
$l_{\max}=2$, $6.06\%$ at $l_{\max}=3$; Paper~22 Table).  The
``realizes'' attribution was corrected 2026-08-30:\ the production
enumerator returned $D_{\mathrm{pd}}$ only because of the wrong-sign-$q$
coefficient bug fixed on 2026-08-29.  Both are universal across Coulomb,
harmonic oscillator, Woods--Saxon, square well, and Yukawa
potentials.}
\label{tab:eri_density}
\begin{tabular}{c c}
\toprule
$l_{\max}$ & $D_{\mathrm{pd}}(l_{\max})$ \\
\midrule
$0$ & $100\%$ \\
$1$ & $7.81\%$ \\
$2$ & $2.76\%$ \\
...
\textbf{[PANEL-VERIFIED]} The bound has been verified numerically across five qualitatively
...
oscillator bases (used in nuclear shell models~\cite{Suhonen2007}) impose the same
...
\textbf{[PANEL-VERIFIED]} Paper~22's spinor extension shows that the angular sparsity bound
...
Hamiltonians competitive for VQE/NISQ resource estimation.
\textbf{[MEASURED]} The
quantum-computing applications papers (Group~4) report Pauli-term
counts exactly linear in the qubit count for the composed-geometry
architecture across the molecular library
($N_{\mathrm{Pauli}} = 27.90 \times Q$ at fixed basis)---a
$54\times$ to $317\times$ equal-qubit advantage in Pauli-term count
over published Gaussian baselines (H$_2$O, $n_{\max} = 1$--$3$; the
retired pair-diagonal rule gave $O(Q^{2.5})$ and
$51\times$--$1712\times$).  The structural reason for this advantage is the
...
to time evolution.  \emph{Provenance note:}\ Paper~6 is archived
within the series.  Of the results quoted in this section, the
unitarity benchmark and the resonant Rabi oscillation (item (b)
below) were re-run and reproduce, with regression backing live in
\texttt{tests/test\_rabi\_oscillation.py};\ until 2026-09-01 that file
was \texttt{tests/rabi\_oscillation.py}, named so that collection
skipped it, with its cases wired as a script rather than as tests.
Items (a), (c) and (d) below are Paper~6's archived measurements with
no live regression test (a named coverage gap in the claim matrix).
The graph Hamiltonian $H$ is
sparse with $O(V)$
...
matrix-vector products plus iterative linear solves.  \textbf{[MEASURED]} Unitarity is
preserved to $10^{-10}$ (measured $1.1 \times 10^{-13}$) over $10^3$
time steps in the live hydrogen regression ($n_{\max} = 10$, 385 states);\ Paper~6's original
$\mathrm{H}_2$ benchmark reported machine precision over $10^4$ steps
(archived measurement).

\textbf{[MEASURED]} The applications demonstrated in Paper~6 are: (a) broadband molecular
...
coherent population transfer (live hydrogen run, $n_{\max} = 4$,
30 states); (c) ab initio molecular dynamics with
...
thermal vibrations at 300~K.  Only (b) and the unitarity benchmark
carry live regression backing (see the provenance note above).
...
ladder operators $T_\pm, L_\pm$ reproduce the Rydberg quantum-number
structure, Paper~1) and the dynamical statement
...
matrix elements.  \textbf{[PANEL-VERIFIED]} At $N_{\max} = 5$ the graph has 56 nodes and 165
...
\medskip
\noindent\textbf{Theorem (Paper~24, HO rigidity).}  \textbf{[CONDITIONAL]} \emph{The 3D
...
$H^2(S^5)$ restricted to symmetric $SU(3)$ irreps $(N, 0)$.}  Paper~24
states the named gap explicitly:\ the converse half---that
$\mathrm{SU}(3)$ closure \emph{forces} $V \propto r^2$---is an imported
dynamical-symmetry result the paper flags as unverified at the
pointer level.
...
\subsection*{The Coulomb/HO asymmetry: a seven-layer stratification}

\emph{Conventions.}\ Sprint and track designations below are anchors into the
project's internal chronicle (\texttt{CHANGELOG.md} in the repository); ``wall''
is project shorthand for a structural obstruction established by multiple
independent negative results.

\textbf{[OBSERVATION]} A central structural finding of Paper~24, sharpened progressively by
the $S^5$ gauge-structure analysis (Paper~25~\cite{loutey_paper25}, QED/gauge group), a
sprint on the wedge KMS state of the framework's Lorentzian extension,
the spectral-action gravity arc (Paper~51~\cite{loutey_paper51}), and the master Mellin
engine, is that the Coulomb and HO discretizations differ along
\emph{seven} independent layers, identified in stages across 2025--2026.
The asymmetry as currently catalogued reads:

\begin{enumerate}
\item \textbf{Spectral-bound role of $L_0$.}  On the Coulomb $S^3$
graph, the graph Laplacian $L_0$ carries the bipartite bound whose
saturation $\lambda_{\max} \to 2 d_{\max} = 8$ is the one convergence
indication that survives scrutiny --- \textbf{[INTERNAL THEOREM]} closed
form, at rate $(C + o(1))/n_{\max}^2$ (Paper~0~\S VI). Its spectrum is
bounded in $[0, 8]$ and does \emph{not} approach the continuum
$n^2 - 1$ [retracted 2026-09-04: graph-spectrum-attribution] (the sentence printed here until 2026-09-04
said it did, ``numerically, Paper~7'';\ Paper~7 says the opposite:\ the
spectrum of $\kappa L$ is confined to $[-1/2, 0]$ and no level-by-level
Rydberg ladder is reproduced). The hydrogenic ground energy follows only
under the matched kinetic scale $\kappa$, an Observation.  On
...
\item \textbf{Calibration $\pi$.}  \textbf{[PANEL-VERIFIED]} The Coulomb $S^3$ projection
...
\item \textbf{Spectral-action gravity termination.}  \textbf{[SYMBOLIC PROOF]} The
...
\item \textbf{Master Mellin engine chirality-parity.}  \textbf{[PANEL-VERIFIED]} On the
...
\item \emph{CM / Hodge-circle arithmetic of the compact flow.}  On
$S^3$ the compact KMS/Bisognano--Wichmann circle has $\pm$-symmetric
integer generator spectrum, so its quarter-period point \emph{is} a
$\mathbb{Q}$-rational complex structure $J$, $J^2=-1$, making the
fundamental doublet a polarized weight-one $\mathbb{Q}$-Hodge structure
with complex multiplication by $\mathbb{Q}(i)$ (Paper~56).  The $S^5$
Hardy phase circle is equally compact and even carries a faithful
$\mu_4$ (the metaplectic zero-point $\tfrac32$), yet admits no CM Hodge
structure---its generator is strictly positive, the quarter point is
$\mathbb{Q}$-rational only where it squares to $+1$, and the conjugate
half a weight-one structure needs is exactly what the holomorphic-sector
restriction discards.  Conductor-four \emph{period} content, by
contrast, does transfer (Paper~24's companion non-layer).
\end{enumerate}
...
fifth (spectral-action gravity) comes from Paper~51; the sixth
(master Mellin chirality-parity) from a June~2026 sprint; and the seventh
(CM/Hodge-circle arithmetic) from the August~2026 seam arc.  The first six
...
\textbf{[OBSERVATION]} The bit-exact $\pi$-freeness of the Bargmann--Segal lattice is a
...
\textbf{[OBSERVATION]} Paper~18~\cite{loutey_paper18} catalogues where transcendental
...
\textbf{[OBSERVATION]} The taxonomy is falsifiable: the claim is that the transcendental
...
\textbf{[CONDITIONAL]} A structural sharpening of the taxonomy is the master Mellin engine
...
unit-radius sphere-volume quotient $2\,\text{Vol}(S^2)/\text{Vol}(S^3) = 4/\pi$ (numerically $\text{Vol}(S^2)/\pi^2$; not the Hopf fibration's base-to-total ratio, corrected 2026-09-03).  This is the
signature of all calibration-$\pi$ content in the Coulomb sector and
appears as the leading constant in the GeoVac state-space Gromov--Hausdorff convergence
rate.
...
characteristic domain of observable: M1 governs state-space Gromov--Hausdorff
...
\textbf{[OBSERVATION]} A particularly clean operational consequence of the taxonomy is the
...
\textbf{[OBSERVATION]} Paper~31~\cite{loutey_paper31} states the cleanest single
...
exactly $Q$-linear Pauli-term count of composed-geometry qubit
encodings ($N_{\mathrm{Pauli}} = 27.90 \times Q$ at fixed basis)
depends on the Gaunt selection rules.
...
(group~5, QED/gauge) on the SU(2) Wilson lattice gauge structure of the
...
\textbf{[MEASURED]} Gauging via the inner fluctuation of the truncated operator-system
algebra produces a genuine, non-factorizable two-body interaction
(connected fraction $77\%$ at $n_{\max} = 2$, falling to $32\%$ at $n_{\max} = 3$---
cutoff-sensitive, like the radial Pearson coefficient below).  \textbf{[PANEL-VERIFIED]} Its angular structure is
...
$\mathcal{M}[\mathrm{Tr}(D^k e^{-t D^2})]$.  \textbf{[INTERNAL THEOREM]} Paper~55 completes this
...
Fathizadeh--Marcolli classification~\cite{fathizadeh_marcolli2016}
specialized to the static $S^3$ sub-case), and $M_3 \subset \mathcal{MT}(\mathbb{Z}[i, 1/2])$ at level
$\le 4$ (cyclotomic mixed-Tate, via Hurwitz zeta at quarter-integer
shifts and the Deligne--Glanois descent~\cite{glanois2015}; the vertex-parity-as-$\chi_{-4}$
correspondence of Paper~28~\cite{loutey_paper28}).  This is the period-theoretic completion of
...
natural categorical home is a motivic Galois group.  \textbf{[PANEL-VERIFIED]} Paper~56 assembles,
...
$\mathrm{Aut}^{\otimes}(\omega)$.  \textbf{[INTERNAL THEOREM]} The load-bearing result is the
\emph{cosmic-Galois injection}:\ a homomorphism (not a closed
immersion --- the M3 column is rank~2, its parity-blind per-sector restriction
rank~1, both $\ll N$, under Reading~A; Paper~56)
$U^*_{\mathrm{GV}} \rightarrow \mathcal{U}_4^{\mathrm{ab}}
...
Eskandari--Murty--Nemoto 2025 place Catalan $G$ at level~4 (mixed Tate
over $\mathbb{Q}(i)$).  That $G$ is \emph{not} a period of $\mathrm{MT}(\mathbb{Z})$
is expected but not proven, so this motivates the refinement rather than
forcing it.
...
content is the named deep forward question.

\emph{The elliptic layer (Papers~59 and~61~\cite{loutey_paper59,loutey_paper61}).}  The injection above lands in the
\emph{mixed-Tate} (genus-zero) cosmic Galois;\ the elliptic (genus-one) layer
that Paper~56 reaches for---a Hain--Brown mixed-elliptic-motive
target~\cite{hain2014,brown2014_mmv}---came
back negative on the abstract $\mathrm{Sym}^2$ panel, structurally because
GeoVac's periods are $\mathbb{Q}(i)$-CM-pure rather than generically modular.
\textbf{[MEASURED]} Paper~59 supplies a concrete elliptic period from the chemistry side:\ the
three-center electron-repulsion integral, read in momentum space, is a two-scale
Bessel moment on the Legendre / $\Gamma(2)$ family;\ the modular and
cosmic-Galois structure that follows---the family arithmetic, the $X_0(2)$
descent, the $66$-digit certification and the guarded searches---is developed
in the companion Paper~61~\cite{loutey_paper61}.  The load-bearing arithmetic
is \emph{fibre-independent}:\ the fibre period is $K=\tfrac{\pi}{2}\theta_3^2$
and $\theta_3^2$ is the theta series of $\mathbb{Z}[i]$, so the family carries
conductor-$4$ arithmetic at every fibre---the same $\mathbb{Q}(i)$ the Dirac
Kramers doublet of Paper~56 realizes, by a \emph{different} forcing (level
rather than spin):\ the two routes are convergent, not common-cause, and no
comparison map between them has been exhibited.  The physical density exchange
acts on the modulus as $\rho\mapsto1/\rho$, so the observable descends to
$X_0(2)=X(2)/\langle\iota\rangle$;\ its physically distinguished fibre is the
coincident-scale \emph{pure-Tate cusp} $\rho=1$, while the $\tau=i$ CM fibre is
a visited member of the exchange orbit $\{\tfrac12,2\}$, not a pinned one.  The
integrated observable is an Eisenstein / complex-multiplication \emph{Bessel
moment}---an irregular (exponential) period, \emph{not} a cusp-form $L$-value,
of which a $\Gamma(2)$ multiple modular value is only the regular $D\to0$
shadow.  Its value is now certified to $66$ digits, and the guarded searches
are decisive:\ no low-height classical combination through weight three,
including the Catalan-completed ring at height $\le10$---so no measured
conductor-$4$ content exists in the physical value at reachable heights (the
arithmetic lives in the family and the shadow), and its explicit finite
realisation remains open.
Two honest bounds keep
this in proportion.  First, the level is $\Gamma(2)$, the classical
\emph{universal} Legendre level (below the two-loop sunrise's $\Gamma_1(6)$):\
landing on the Legendre family is expected of any four-branch-point curve, so the
informative content is the rational modulus map $\lambda = 1-\rho$, the
$X_0(2)$ descent, and the physical Bessel twist's level promotion
$\Gamma(2)\to\Gamma(4)$, not an exotic level.  Second, whether the observable
reaches the \emph{generic} (non-CM, full-$SL_2$, $E_4/E_6/\Delta$) Hain--Brown
locus is open.  \textbf{[SYMBOLIC + MEASURED]} What the irreducibility \emph{does} certify is structural:\
because the rank-four connection is the Fourier--Laplace transform of an
irreducible rank-one connection---hence simple, with no invariant
\emph{coordinate} subspace in its monodromy frame---no closed form arises
through any factorization, so the period is not reducible to the mixed-Tate
layer by factorization (the genus-one label itself is set by the curve's two
distinct Fock scales).  On the physical contour the two layers demonstrably
touch at the pure-Tate diagonal cusp;\ the seam's two upgrade tests---a natural
comparison map, and a measured $\beta(2)$ in the physical value---are
respectively tautology-blocked and answered negative at clean heights.  The
elliptic layer is thus a concrete, irreducible genus-one \emph{instance} that
corroborates rather than lifts the Hain--Brown negative---not filled to the
generic modular locus.

\subsection{The forced/free seam (Paper~57)}
\textbf{[OBSERVATION]} The reconvergence closes with an observations paper that consolidates,
...
truncation plus Bertrand).  The packing-reachable tag is the F/C label
by construction, so its $98.3\%$ agreement across the $60$-entry
catalogue is an internal-consistency check, not an independent
discriminator (Paper~57~\S6.1);\ the genuine discriminator signal is
the two-family decomposition below.  The free side splits into two structurally
...
period classification (Paper~55) and the Tannakian injection (Paper~56)
place theorem-grade boundaries around the empirical forced/free
observation---with the $\mathcal{A}/D$ partition (Paper~31) supplying the
spectral-triple language rather than a further theorem---
though Paper~57 holds the forcing-iff-property principle itself open as
a preliminary observation (it is an observations paper, not a theorem
paper).  The
...
The foundations arc as catalogued here leaves five named structural
questions open.
...
\textbf{[OPEN]} The packing construction of Paper~0 generates the angular sector
...
of $S^5$ respectively.  \textbf{[OPEN]} A natural question is whether a coupled system
...
the foundations arc, Paper~24's seven-layer Coulomb/HO asymmetry
...
\textbf{[OPEN]} The framework's composition rules for multiple Fock-style projections
...
\subsection*{The elliptic layer of the cosmic Galois}

\textbf{[OPEN]} Paper~56's cosmic-Galois injection lands in the mixed-Tate (genus-zero) group
$\mathcal{G}_4$;\ the elliptic (genus-one) layer it reaches for---the
Hain--Brown mixed-elliptic-motive target over $M_{1,1}$---came back negative,
because GeoVac's periods are $\mathbb{Q}(i)$-CM-pure rather than generically
modular.  Paper~59 now instantiates that layer with a concrete, irreducible
genus-one period (the three-center integral's elliptic Bessel moment on the
Legendre/$\Gamma(2)$ family, descending to $X_0(2)$), carrying the same
$\mathbb{Q}(i)$ arithmetic at the \emph{family} level ($\theta_3^2$, the
$\mathbb{Z}[i]$ theta series) by level-forcing rather than spin-forcing---%
convergent with the Dirac route, not common-cause.  The open question is whether
any GeoVac observable reaches the \emph{generic} (non-CM, full-$SL_2$)
Hain--Brown locus---a lift that would unify the genus-zero and genus-one layers
(the Hain--Brown mixed-elliptic completion).  On the physical contour the two
layers demonstrably meet at the pure-Tate cusp;\ no natural comparison map has
been exhibited, and the measured seam test ($\beta(2)$ in the physical value)
is negative at clean heights---a lift beyond convergence is unestablished.
...
non-relativistic quantum mechanics on natural geometries.
\textbf{[SYMBOLIC PROOF]} The conformal chain from the unit-$S^3$
Laplace--Beltrami operator onward is verified symbolically
(Paper~7~\cite{loutey_paper7}); \textbf{[OBSERVATION]} the discrete Fock
graph's convergence to that operator is a reading supported by the
proved saturation of the spectral bound, not an established result, while
\textbf{[INTERNAL THEOREM]} the state-space GH convergence of the spinor
spectral truncations is proved with explicit rate in
Paper~38~\cite{loutey_paper38}; \textbf{[INTERNAL THEOREM]} the
...
\textbf{[CONDITIONAL]} master Mellin engine at three operator orders as the generator of the
$\pi$-content of the projection chains case-checked so far (Paper~32~\cite{loutey_paper32}'s
case-exhaustion theorem, over Paper~34 \S III.1--15, with its
(i)$\Leftrightarrow$(ii) leg resting on empirical verification); the
packing axiom of
...
fundamental.  $O(V)$ sparsity, angular momentum selection rules baked
into the basis, bit-exactly rational matrix elements on the
Bargmann--Segal lattice, and the universal sparsity bound at fixed
$l_{\max}$ are statements about the discrete graph (the integer spectrum
$n^2 - 1$ is \emph{not}:\ it belongs to the continuum operator, the
graph's own spectrum being closed form and bounded in $[0, 8]$ ---
Paper~0~\S VI, Paper~7); their continuum counterparts are theorems about the
...
wall is an active research frontier; and the elliptic (genus-one) layer of the
cosmic Galois is instantiated but not lifted---the generic Hain--Brown locus
stays open.  Each of these is a frontier-of-field
...
Int.\ Math.\ Res.\ Not.\ IMRN \textbf{2025}, no.~13, rnaf197 (2025); arXiv:2310.14733.
...
Comm.\ Math.\ Phys.\ \textbf{356}, 641--671 (2017); arXiv:1611.01815.

\bibitem{glanois2015}
C.~Glanois,
``Motivic unipotent fundamental groupoid of $\mathbb{G}_m \setminus \mu_N$ for $N = 2, 3, 4, 6, 8$ and Galois descents,''
J.\ Number Theory \textbf{160}, 334--384 (2016); arXiv:1411.4947.

\bibitem{hain2014}
R.~Hain,
``The Hodge-de Rham theory of modular groups,''
in \emph{Recent advances in Hodge theory} (LMS Lecture Notes 427, Cambridge Univ.\ Press, 2016) 422--514; arXiv:1403.6443.

\bibitem{brown2014_mmv}
F.~Brown,
``Multiple modular values and the relative completion of the fundamental group of $M_{1,1}$,''
arXiv:1407.5167 (2014; v4, 2017).
