...
degeneracy recovery [retracted 2026-09-04: sp-splitting-as-convergence-evidence], not the operator convergence --- and the
...
that operator is a reading supported by numerical indications through
$n_{\max} = 30$ (spectral-bound saturation alone;\ the $s/p$-lift decay [retracted 2026-09-04: sp-splitting-as-convergence-evidence]
...
cutoff (up to $37\%$ at $n_{\max} = 5$ on the binary lattice, below $1\%$ by
$n_{\max} = 30$; the $\ell$-blocks are disconnected, so this is a proxy and
not a spectral gap --- Paper~1 \S III).
...
recorded in Paper~7 with its numerical indications through $n_{\max} =
30$ (spectral-bound saturation alone;\ the $s/p$-lift decay [retracted 2026-09-04: sp-splitting-as-convergence-evidence] was
...
graph-side convergence itself has no theorem behind it: what the frozen
test \texttt{tests/test\_paper7\_graph\_convergence.py} pins is the
saturation of the bipartite bound $\lambda_{\max} \to 2 d_{\max} = 8$ and
the block structure of the lattice Laplacian, not operator convergence
(re-scoped 2026-09-03).
...
contributions are: (a) the numerical indications (spectral-bound saturation and
degeneracy recovery [retracted 2026-09-04: sp-splitting-as-convergence-evidence] through $n_{\max} = 30$ --- the latter now known to
...
graph reproduces the Rydberg quantum-number structure) and the dynamical statement
...
graph, the graph Laplacian $L_0$ is the spectrum-generating
operator: the spectrum of $(D - A)$ approaches the continuum $S^3$
Laplace--Beltrami spectrum $n^2 - 1$ as $n_{\max} \to \infty$
(numerically, Paper~7), and the hydrogenic energies follow with the
matched kinetic scale.  On
...
graph's convergence to that operator is a reading with numerical
indications, not an established result, while the state-space GH convergence of the spinor spectral
