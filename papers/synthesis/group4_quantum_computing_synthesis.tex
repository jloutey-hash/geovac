...
(H$_2$O), and the range they bracket, are GeoVac's own recomputation via
the OpenFermion/Qiskit-Nature pipeline (the resource-estimation
methodology follows Trenev et al.~\cite{trenev2025}, a vibrational-spectra
study that reports no electronic Gaussian scaling exponents).
...
All Gaussian baselines are validated against published integral
values~\cite{trenev2025} and against analytic references (Szabo and
...
qubit Hamiltonians spanning $Z=1$--$36$ across three periodic-table rows,
...
reference at $12$ qubits.  The market-test comparison ``$334$ versus
$907$ STO-3G Pauli terms'' is stated under the pair-diagonal rule; under
the exact global-$M_L$ rule LiH re-prices to $838$ terms, i.e.\ near
parity ($0.92\times$) with STO-3G's $907$---so the broad
``one-to-three-orders-of-magnitude sparser'' headline rests on the
scaling gap and the multi-center block structure, while the specific
small-system market-test line is sensitive to the ERI rule and is
reported with that caveat.  The \emph{balanced coupled} (PK-free)
architecture trades sparsity for chemistry:\ it binds LiH at the
experimental equilibrium geometry $R_{\mathrm{eq}}=3.015$~bohr with $878$
Pauli terms at $30$ qubits and a $0.20\%$ single-point energy error at
the well minimum when solved exactly (number-projected FCI).
...
structural sparsity with quantitative truncation-error bounds
...
the framework's binding wall is structurally inseparable from its
...
Hamiltonians with quantitative truncation-error bounds---a research
...
[Cited for the $Q^{3.9}$--$Q^{4.3}$ Gaussian-baseline scaling range and
resource-estimation methodology; the specific LiH/H$_2$O electronic-structure
exponents are GeoVac's own recomputation in that pipeline, per Paper~14.]
