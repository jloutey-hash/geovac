...
analysis.  Three of the five lemmas are now closed: L1' (offdiag
Camporesi--Higuchi operator system substrate, every cross-pair
finite, sprint~R3.5, 2026-05-04;
\texttt{debug/wh1\_r35\_full\_dirac\_memo.md}); L2 (existence of
...
2026-05-04; \texttt{debug/r25\_l2\_proof\_memo.md}); and L3
(Lipschitz comparison $\|[D_{\mathrm{CH}}, M_f]\|_{\mathrm{op}} \le
C_3 \|\nabla f\|_{L^\infty}$ with $C_3 = 1$ on the natural Avery
test panel at $n_{\max} \in \{2,3,4\}$, sprint~R2.5/L3,
2026-05-04; \texttt{debug/r25\_l3\_proof\_memo.md}).  The
``torsion-and-curvature correction'' anticipated in the Track-A
master memo is identified concretely as the spectral-gap
quantization $|n - n'|$ of the truthful Camporesi--Higuchi Dirac:
the per-multiplier ratio for $M_{NLM}$ is bounded by $N - 1$ from
the SO(4) selection rule, and the corresponding Lipschitz scaling
$\|\nabla Y^{(3)}_{NLM}\|_{L^\infty} \sim \sqrt{N^2 - 1}$ gives the
asymptotic ratio $(N-1)/\sqrt{N^2-1} \nearrow 1$. The
$N=3$ theoretical bound $1/\sqrt 2 \approx 0.7071$ matches the
observed $C_3$ at $n_{\max} = 3$ to 4 decimals.  The remaining two
lemmas (L4 Berezin reconstruction, L5
