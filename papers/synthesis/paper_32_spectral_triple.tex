...
program~\cite{connes_vs2021,hekkelman2022,hekkelman_mcdonald2024}.  Every
...
The structural reading is that G4b is the four-layer Coulomb/HO
asymmetry of Paper~24~\S V resurfacing at the spectral-triple level.
The first three layers (spectrum-computing role of $L_0$;\
calibration~$\pi$;\ gauge-with-natural-matter,
Sec.~\ref{sec:unified_gauge}) are surface manifestations.  G4b is
the fourth layer:\ the Coulomb $S^3$ side carries a Riemannian
spectral triple in the standard NCG sense, the Bargmann $S^5$ side
does not, and the Connes tensor-product framework cannot bridge them
as currently published.  Closing G4b is therefore not a sprint-scale
GeoVac task;\ it requires either an extension of NCG to handle
Riemannian--Hardy mixed tensor products, or the identification of a
unifying ambient manifold whose GeoVac sub-structure contains both
$S^3$ Coulomb and $S^5$ Bargmann as natural sub-objects.  Both
directions are recorded as honest open structural questions, not
as sprint targets.
...
alone with $M_3(\mathbb{C})$) is a 1--2~month sprint with predicted
positive-thin outcome;\ G4b (genuine cross-manifold) is blocked at
the NCG-framework level by the Coulomb/HO structural asymmetry.
...
\paragraph{Files.}  Memo
\texttt{debug/g4\_cross\_manifold\_scoping\_memo.md}
($\sim 5\,100$ words);\ no production module created (consistent
with the FAR verdict and the §1.5 rhetoric rule).  G4a sprint is
queued as the follow-up;\ G4b is recorded as an open structural
question.
...
outlined in Paper~43 (\texttt{papers/standalone/paper\_43\_lorentzian\_extension\_outline.md},
...
``Towards a noncommutative geometry of the Standard Model with neutrino
mixing,''
