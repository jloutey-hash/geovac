...
theory, its natural matter coupling obstructed by a Clebsch--Gordan
intertwiner wall (a sharpening of the Coulomb/harmonic-oscillator
...
non-abelian theory.  The second-order weak-coupling expansion returns
Paper~25's $L_1=B^{\top}B$ as the kinetic term, with non-abelian
couplings first entering at order $A^4$.  The kinetic normalization is
universal,
\begin{equation}
...
$m_\ell$-quotient to $\mathbb{CP}^2$ fails uniformly ($\approx25\%$
residual).  This is the gauge-side face of the Coulomb/HO asymmetry
...
vertex topology.  Three results anchor the arc, each at theorem tier:
\begin{itemize}
...
and is recovered only once the vector-photon angular labels of Paper~33
are restored.  A graph-native anomalous moment
...
and the per-component invariants (Link~2) are established, while the
...
open, multi-month program.  Represented at its true tier, this is a
