...
case-exhaustion theorem of Paper~32~\S\,VIII.  Three further closures land subsequent to the May~22, 2026 drafting
of this synthesis and are recorded in
\S\ref{sec:closures_post_47}--\S\ref{sec:closures_post_49}:\
Paper~48 constructs the Krein-pointed proper quantum metric spaces
and the bridge to Mondino--S\"amann Lorentzian pre-length spaces,
giving the first quantitative pLGH-convergence panel in the
synthetic-Lorentzian literature from operator-algebraic input;
Paper~49 closes the strong-form bridge on the enlarged operator
system via the Connes--Rovelli thermal-time stack across distinct
KMS states, identifying off-orbit super-additivity as the
operator-algebraic dual of the twin paradox quantified by
Uhlmann's relative-entropy monotonicity; and Paper~50 demonstrates
bit-exact reproduction of CFT-on-sphere partition functions on
...
framework.  This synthesis is corrected at
...
(\texttt{docs/claims\_register.md}) are authoritative.
...
Lorentzian arc.  Paper~46~\cite{paper46} closes this question on the
natural chirality-doubled scalar-multiplier substrate of
Paper~44~\cite{paper44} via the operator-norm Lipschitz seminorm
...
\paragraph{Strong-form Lorentzian propinquity (Q1) --- CLOSED by
Paper~46 (\S\ref{sec:closures_paper46}).}
...
own right (Paper~45~\S 1.4 G1), is now closed by
Paper~46~\cite{paper46} via the operator-norm Lipschitz seminorm
$L_{\mathrm{op}}(a) = \opnorm{[\DL, a]}$ on the natural
chirality-doubled scalar-multiplier substrate of Paper~44.  The
``free-upgrade'' reading holds:\ $\Lambda^{\mathrm{strong}}$ matches
the $K^{+}$-weak-form bound bit-exactly at every panel cell, with
$C_{3}^{\mathrm{op}}(\nmax) = \sqrt{1 - 1/\nmax} \to 1^{-}$.
Paper~46~Appendix~B extends the closure to the enlarged operator
system with chirality-flipping generators via a gradient-norm
absorption mechanism, leaving the propinquity bound bit-exactly
preserved.
