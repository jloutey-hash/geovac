...
master theorem (\S\ref{sec:closures_post_45}) subsumes Papers~38,~39,
and~40 as special cases:\ for arbitrary $k \ge 1$ and arbitrary
compact connected Lie groups $G_{1}, \ldots, G_{k}$, the k-fold
tensor product propinquity converges at the universal $4/\pi$ rate
with $k$-INDEPENDENT Lipschitz comparison constant.  The constant
...
case-exhaustion theorem of Paper~32~\S\,VIII.  Three further papers (48, 49, 50) extend the arc and are recorded in
\S\ref{sec:closures_post_47}--\S\ref{sec:closures_post_49};\ two of
their metric-level claims were subsequently descoped with the Paper~45
retraction and are reported here in corrected form.
...
\textbf{(iii) k-fold extension --- $k$-independent constant.}  The
sprint memo
\texttt{debug/sprint\_paper39\_k\_fold\_extension\_memo.md}
established the k-fold theorem at SU(2) factors with closed-form
$C_{3}^{(k)} \le \sqrt{(N_{*} - 1)/(N_{*} + 1)}$ where $N_{*} :=
\max_{j} n_{j}$ --- \textbf{$k$-INDEPENDENT, identical to the
single-factor Paper~38 L3 constant}, asymptotic $\to 1^{-}$.  The
$O(\sqrt{k})$ estimate that the GeoVac internal record had carried
(triangle-inequality bound) was strictly improved by the k-fold
Pythagorean Leibniz identity $\opnorm{\sum_{j} X_{j}}^{2} = \sum_{j}
\opnorm{X_{j}}^{2}$ on the Connes-Marcolli graded module --- the $k$
summands of $D^{(k)}$ pairwise anti-commute, so the cross-terms vanish
and the sum-of-magnitudes bound is replaced by the sum-of-squares
identity.  The improvement of $\sqrt{k}$ comes from precisely the
Pythagorean refinement of the triangle inequality.

\textbf{(v) Master theorem --- universal compact-Lie-group factors.}
Combining the k-fold extension with the universal $4/\pi$ rate of
Paper~40~\cite{paper40_unified} produces the master theorem of the
math.OA arc:
...
\;\le\; \sup_{j} C_{3}^{G_{j}}(n_{j}) \cdot \max_{j}
\gamma^{G_{j}}_{n_{j}}\;}
\end{equation}

for arbitrary $k \ge 1$ and arbitrary compact connected Lie groups
$G_{1}, \ldots, G_{k}$ with bi-invariant metrics.
$C_{3}^{(k)}$ is $k$-INDEPENDENT (bounded by the worst-factor
Paper~40 L3 constant, $\to 1^{-}$).  The joint rate inherits the
universal $4/\pi$ asymptotic from Paper~40's
Plancherel-weight $\times$ Vandermonde-Jacobian cancellation.

The two extensions are \emph{structurally orthogonal}:\ the k-fold
mechanism (Pythagorean Leibniz) acts on the factor-counting
dimension; the general-$G$ mechanism (PRV-summand Weyl harmonics) acts
on the per-factor structure.  Each preserves the other's content
cleanly.
