...
with $C_3 = 1$ and asymptotic constant $4/\pi$ (Paper~38; rotation-angle
...
$\sthree \to S^{2}$, $\Vol(S^{2}) = 4\pi$ and
$\pi^{2} = \Vol(\SU(2))/2$ is half the Haar volume of
$\SU(2) \cong \sthree$, so that $4/\pi = 2\Vol(S^{2})/\Vol(\SU(2))$ in
the rotation-angle normalisation; on the unit $\sthree$ the constant is
...
rotation-angle normalisation and equal to it on the unit $\sthree$, so
