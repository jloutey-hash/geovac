...
$(m,n) = (4,6)$, the originally-claimed $K^{+}$-restricted weak-form
Lorentzian propinquity bound (\emph{retracted}; see below) was
\begin{equation}
...
An earlier draft reported the $S^{7}$ scalar value as a structural
non-match (``no PSLQ relation in the tested bases'').  Paper~50's
Erratum (\S 8) identifies this as a \emph{false negative} from an
under-resolved ($30$-digit) search:\ at $\geq 200$ digits the genuine
$S^{7}$ scalar relation has integer coefficients of order $10^{4}$ and
lies in the substrate-natural ring $\mathcal{R}_{7}$ (reconstruction
error $\approx 2.4\times10^{-202}$, reproduced by the Door-1 driver at
$260$ dps).  The ladder therefore \emph{generates}:\ every odd rung
...
\newblock [Bibliographic details pending confirmation --- title/arXiv
ID not independently verified; retained as prior-art lineage credit.]
...
\newblock [arXiv ID pending author confirmation --- the previously
listed 2403.18619 was a mis-citation and has been removed.]
