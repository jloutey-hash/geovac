...
\title{Chemical Periodicity as $S_N$ Representation Theory\\
on $S^{3N-1}$}
...
C~(alkali), D~(alkaline earth)---that reproduces the periodic table's
group structure.  The periodic law itself is the repetition of the
$S_N$ irrep sequence $\mathrm{C} \to \mathrm{D} \to \cdots \to
\mathrm{B}$ within each period.  In the per-pair limit, $\mufree$
...
follows from the representation theory of the symmetric group $S_N$
acting on the spatial part of the $N$-electron wavefunction on the
...
\begin{enumerate}
  ...
  \item The shape of the $S_N$ irrep classifies atoms into chemical
    types that reproduce the group structure of the periodic table
    (Sec.~\ref{sec:classification}).
  \item The periodic law is the repetition of $S_N$ irrep sequences
    (Sec.~\ref{sec:periodic_law}).
...
%% ========================================================================
%% II. THE HYPERSPHERICAL FRAMEWORK
%% ========================================================================
...
This is the periodic law in group-theoretic language:
\begin{quote}
...
\emph{chemical classification} of atoms.  Where Paper~0 explained
\emph{why} each shell holds $2n^2$ electrons, the present analysis
explains why filling those shells produces the periodic pattern
...
\begin{itemize}
  ...
  \item \textbf{The periodic law} itself is the statement that the
    $S_N$ irrep sequence repeats whenever a new principal shell
    begins to fill.
\end{itemize}
...
\begin{enumerate}
  ...
  \item The shape of the $S_N$ spatial irrep classifies atoms into
    types (A/B/C/D/E) that reproduce the chemical group structure.

  \item The periodic law is the repetition of the irrep sequence
    $\mathrm{C} \to \mathrm{D} \to \mathrm{E} \to \mathrm{B}$
    within each period.
...
$\alpha$, Coulomb integrals).  This separation explains why the
periodic table has the structure it does (topology) and why individual
elements have the properties they do (metric).
