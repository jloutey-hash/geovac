...
\begin{table*}[tb]
  ...
  \begin{tabular}{lrl}
    ...
    \colrule
    Rabi period error & 0.41\% & BS-corrected $T_R$ \\
    ...
    \colrule
    PES equilibrium $R_{\mathrm{eq}}$ & 1.293 Bohr & expt.\ 1.401 bohr ($-8\%$) \\
...
\begin{table}[tb]
  ...
  \begin{flushleft}
  \footnotesize
  ${}^\dagger$Graph eigenvalue, max\_n$=20$, $N_\text{basis}=2870$.
  ${}^\ddagger$Variational $Z_\text{eff}$, max\_n$=5$, $N_\text{basis}=55$.
  ${}^\S$Full CI, max\_n$=8$, $N_\text{basis}=408$.
  \end{flushleft}
\end{table}
