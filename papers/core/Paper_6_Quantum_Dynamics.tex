...
% - Rabi coherence (0.46% period error, 99.98% population transfer)
...
analytically expected period to 0.46\% with 99.98\% coherent population
transfer. Coupling quantum forces to classical nuclear kinematics via
...
In this work, we demonstrate that spectral graph theory provides an
alternative computational substrate for quantum dynamics that achieves
$O(V)$ scaling per time step, where $V$ is the number of graph vertices
encoding the quantum states. The key insight is that atomic and molecular
...
In the graph framework, $E(R)$ is obtained by solving the Full
Configuration Interaction (CI) problem on the tensor product of
atomic lattices connected by distance-dependent bridge edges with
weights
\begin{equation}
  W_{\mathrm{bridge}}(R) = A \, e^{-\lambda R},
  \label{eq:bridge-weight}
\end{equation}
where $\lambda$ is the bridge decay rate. The force is evaluated
...
Full CI solves (at $R$, $R \pm \delta R$), each completing in
$\sim$0.03\,s for a $3600$-dimensional CI space
($\mathrm{max}_n = 4$, two atoms).
...
\begin{figure}[tb]
  ...
  transfer with a Rabi period error of 0.46\% relative to the
  analytical prediction $T_R = 2\pi/\Omega_R$.}
  \label{fig:rabi}
\end{figure}
...
population transfer efficiency and reproduces the analytically
expected Rabi period to 0.46\% accuracy. The norm is conserved to
machine precision ($|\braket{\psi|\psi} - 1| < 10^{-14}$) throughout
the propagation, confirming the exact unitarity of the Crank--Nicolson
scheme on the graph Hamiltonian.
...
the Full CI ground-state energy at each geometry. The resulting
curve exhibits the characteristic Morse-like shape with a well
minimum at $R_{\mathrm{eq}} = 1.30$~Bohr (experimental:
$1.401$~Bohr), representing a ``topological contraction'' consistent
with the discrete nature of the graph lattice.
...
\begin{figure}[tb]
  ...
  Full CI on the graph-topological Hamiltonian. Top: electronic
  energy $E_{\mathrm{CI}}(R)$ shows a Morse-like well with minimum
  at $R = 1.30$~Bohr. Bottom: binding energy
  $D_e(R) = 2E(\mathrm{H}) - E_{\mathrm{CI}}(R)$, with the
  experimental dissociation energy marked for comparison.}
  \label{fig:pes}
\end{figure}

Gradient descent on this surface converges to
$R_{\mathrm{eq}} = 1.293$~Bohr in 47 steps (3.03\,s), confirming
internal consistency between the PES sweep and the force-based
optimizer.
...
motion
\begin{equation}
  \mu \ddot{R} = F(R) = -\frac{\partial E_{\mathrm{CI}}}{\partial R},
  \label{eq:newton}
\end{equation}
where $\mu = m_p/2 = 918.08$~a.u.\ is the reduced mass. Over 600
steps ($\Delta t = 1.0$~a.u.), the molecule executes two full
vibrational periods with:

\begin{itemize}
  \item Vibrational period: $T = 7.15$~fs (expt.\ $\sim$8.1~fs)
  \item Vibrational frequency: $4666~\mathrm{cm}^{-1}$
    (expt.\ $4161~\mathrm{cm}^{-1}$)
  \item Maximum energy drift:
    $\Delta E_{\mathrm{tot}} = 7.1 \times 10^{-6}$~Ha (0.0003\%)
\end{itemize}

The 12\% frequency overestimate is consistent with the topological
contraction of the equilibrium distance: a narrower potential well
implies higher curvature and correspondingly faster oscillation.
...
\begin{table}[tb]
  ...
  \begin{tabular}{lrl}
    ...
    \colrule
    Rabi period error & 0.46\% & vs.\ analytical $T_R$ \\
    ...
    \colrule
    PES equilibrium $R_{\mathrm{eq}}$ & 1.30 Bohr & expt.\ 1.401 \\
    ...
    Vibrational frequency & 4666\,cm$^{-1}$ & expt.\ 4161 \\
...
The Full CI electronic energy $E_{\mathrm{CI}}(R)$ does not
exactly recover the dissociation limit $2E(\mathrm{H})$ at large
internuclear separations. Cross-nuclear attraction matrix elements
$V_{en}^{\mathrm{cross}}$ decay slowly with $R$ due to the
$-Z/n^2$ node-weight structure, producing an artificial residual
binding at $R > 5$~Bohr. This artifact does not affect the
equilibrium geometry or vibrational dynamics (which occur at
$R \sim 1.3$~Bohr) but would need to be addressed for quantitative
...
($R_{\mathrm{eq}} = 1.29$ vs.\ 1.40~Bohr, $\sim$8\% contraction)
...
(i)~extension to polyatomic molecules by connecting multiple atomic
lattices in arbitrary topologies; (ii)~nonadiabatic dynamics via
coupled-surface propagation on the Full CI manifold;
(iii)~periodic systems where the graph topology naturally encodes
