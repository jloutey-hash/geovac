...
\end{equation}
where $\kappa = -1/16$ is the universal kinetic scale---the unique constant that maps the dimensionless graph eigenvalues to the physical hydrogen spectrum, consistent with the information-theoretic packing constraints of \cite{loutey_paper0}.
...
The mathematical evidence supports a precise structural observation: \textit{the Schr\"odinger equation for hydrogen is mathematically equivalent, under Fock's conformal projection, to the Laplace-Beltrami operator on a dimensionless, scale-invariant topology (unit $S^3$)}. The continuous equation and its dimensionful energy levels can be viewed as emergent properties of projecting this topology into observational coordinates. This is a mathematical equivalence under conformal projection, not a proof of physical priority---whether the graph or the differential equation is ``more fundamental'' is a question of physical interpretation, not of theorem.
...
\textit{Universal kinetic scale:} The constant $\kappa = -1/16$ is the unique kinetic scale consistent with the Rydberg formula and the graph topology. It is not independently derived from topology alone---it is the bridge between the dimensionless graph and dimensionful physics, determined by requiring the graph eigenvalues to reproduce the hydrogen spectrum via the energy-shell projection mechanism. Its universality across all nuclear charges $Z$ (via $Z^2$ scaling) confirms that the graph topology correctly encodes the \textit{relative} spectral structure.
