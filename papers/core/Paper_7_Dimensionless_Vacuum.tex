...
\end{equation}
where $\kappa = -1/16$ is the kinetic scale factor---the unique constant that maps the dimensionless graph eigenvalues to the physical hydrogen spectrum, determined by matching the Rydberg formula and consistent with the information-theoretic packing constraints of \cite{loutey_paper0}.
...
\textit{Kinetic scale factor:} The constant $\kappa = -1/16$ is the kinetic scale factor that bridges the dimensionless graph and dimensionful physics. It is not derived from topology alone---it is determined by requiring the graph eigenvalues to reproduce the hydrogen spectrum via the energy-shell projection mechanism. Its universality across all nuclear charges $Z$ (via $Z^2$ scaling) confirms that the graph topology correctly encodes the \textit{relative} spectral structure.
