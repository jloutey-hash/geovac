...
abelian or K\"ahler structures; the most natural physically motivated
non-K\"ahler compact case --- the round $\sthree = \SU(2)$ with a
Dirac spectral triple --- has remained open.
...
the Connes--van~Suijlekom $S^1$ Fejér reconstruction.
...
\textbf{(d)} By linearity of the commutator and using
\eqref{eq:per_multiplier_bound} from L3 with $\widehat{K}(N) \le 1$:
\begin{equation*}
   \opnorm{[\DCH, B_{\nmax}(f)]}
   \;\le\; \sum_{N, L, M} \widehat{K}(N)\,|c_{NLM}|\,\opnorm{[\DCH, \Mat_{NLM}]}
   \;\le\; \sum_{N, L, M}|c_{NLM}|\,\norm{\nabla Y^{(3)}_{NLM}}_{L^\infty}
   \;\le\; \norm{\nabla f}_{L^\infty}.
\end{equation*}
\end{proof}
...
\item $\mathrm{height}_B$, the contractivity envelope of $B_{\nmax}$,
\item $\mathrm{height}_P = 0$ for the orthogonal projection.
\end{enumerate}

By L4(c), $\mathrm{reach}_B \le \gamma_{\nmax} \cdot 1 = \gamma_{\nmax}$
on the unit Lipschitz ball $\{\Lipnorm{f} \le 1\}$, where we have
used L3 to convert $\norm{\nabla f}_{L^\infty} \le \Lipnorm{f}$ with
constant $C_3 = 1$. The dual reach is bounded by the same
$\gamma_{\nmax}$ by the symmetric L2(c) cb-norm equality
$\opnorm{T_K}_{\mathrm{cb}} = \norm{\widehat{K}}_{\ell^\infty}
= 2/(\nmax+1)$ on the central subalgebra. The height contributions
are bounded constants:\ $\mathrm{height}_B \le \pi$ from L4(b), and
$\mathrm{height}_P = 0$ exactly.
...
\begin{equation*}
   ...
   \;\le\; \max\bigl(\gamma_{\nmax}, \gamma_{\nmax}, \pi, 0\bigr).
\end{equation*}
The constant $\pi$ does not depend on $\nmax$ and contributes only at
$\nmax = 1$. The rate-controlled $\nmax$-dependent bound is
\begin{equation*}
   \Lambda(\Tcal_{\nmax}, \Tcal_{\sthree})
   \;\le\; C_3 \cdot \gamma_{\nmax}
   \;=\; \gamma_{\nmax} \;\to\; 0 \;\;\text{as}\;\; \nmax \to \infty.
