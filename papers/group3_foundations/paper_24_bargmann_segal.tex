...
operator.  The kinetic constant $\kappa = -1/16$ then maps these
continuum $S^3$ eigenvalues $n^2 - 1$ to physical Rydberg energies
...
Observation~\cite{paper2_alpha} and for the precision of nuclear
...
discusses implications for the $\alpha$ Observation, for nuclear
...
$\alpha$ Observation of Paper~2~\cite{paper2_alpha}.
...
multiplicative normalizations~\cite{follandstein1974}.  A
genuinely spherical harmonic-oscillator analogue --- one whose
configuration space is the sphere itself and which, unlike the
flat-space oscillator, does not factor into one-dimensional modes
--- has been studied independently in recent mathematical
work~\cite{higgs_pickrell2025}, which identifies the
two-dimensional case as critical.  That construction is distinct
from the flat-space isotropic oscillator discretized here via the
$S^5$ Hardy sector; we note it only as an independent recent
appearance of harmonic-oscillator dynamics with sphere symmetry.
For the
...
Laplacian $D - A$ on the $(n, l, m)$ lattice converges to the continuum
$S^3$ Laplace--Beltrami operator, whose eigenvalue $n^2 - 1$ on the
$n$-th shell the universal kinetic constant $\kappa = -1/16$ converts to
Rydberg energies.  There the
...
\emph{Note on the layer count.}\ The Coulomb/HO asymmetry has been
refined to seven layers (enumerated in \S\ref{sec:asymmetry_layer4}) across Sprint~5 Track~S5 (gauge-content),
Paper~30 (SU(2) Wilson on $S^3$), Sprint~ST-SU3 (SU(3) Wilson on
$S^5$), Sprints~L2-E/L2-F.1 (modular wedge), Paper~51 (gravity termination),
Sprint~Q5$'$-S5 (Mellin parity), and the Hodge-circle
arithmetic computation (August~2026).  The first three Coulomb-specific
layers are: (i)~spectrum-computing role of $L_0$;
(ii)~calibration~$\pi$; (iii)~Wilson gauge with natural matter
coupling --- with the universal Wilson--Hodge combinatorial vocabulary
as the shared (\textbf{universal}) residue, not itself one of the seven.  Sprint~ST-SU3 split
the Wilson layer into ``gauge alone'' (universal) versus ``gauge
with natural matter coupling'' (Coulomb-specific): on $S^3$,
$S^3 = \mathrm{SU}(2)$ means the matter spinor lives in the same
representation as the Wilson links, and gauge-matter coupling is
natural; on $S^5$, the matter representations $(N, 0)$ have
dimensions that grow with~$N$, so a uniform link-rep matter coupling
is not available, and any coupling reverts to a Clebsch--Gordan
intertwiner --- the same obstruction Sprint~5 identified.  See
\texttt{debug/st\_su3\_wilson\_memo.md}~\S~5.4.
...
Paper~18~\cite{paper18} classifies exchange constants into a
six-tier taxonomy: \emph{intrinsic} (internal to the discrete
graph), \emph{conformal/calibration} (from projecting graph $\to$
physical spectrum), \emph{embedding} (from embedding the graph into
a continuous space for measurement), \emph{algebraic-implicit}
(parameter-dependent functions with inherited transcendental
content), \emph{composition} (the cost of factorizing into composed
coordinates), and \emph{inner-factor input data} (calibration data
of the almost-commutative inner factor, supplied externally).
...
%================================================================
\subsection{Fourth layer: modular-Hamiltonian Pythagorean
orthogonality}
\label{sec:asymmetry_layer4}
%================================================================

The note at the head of Section~\ref{sec:asymmetry} records
the original three layers of the Coulomb/HO asymmetry: the spectrum-computing
role of $L_0$ (Coulomb-specific), the calibration~$\pi$
(Coulomb-specific), and Wilson gauge with natural matter coupling
(Coulomb-specific).  Sprints L2-E + L2-F.1 (May~2026) identified a
fourth layer at the modular-Hamiltonian / wedge level:

\begin{itemize}\setlength{\itemsep}{2pt}
\item Layer~1: spectrum-computing role of $L_0$
  (Coulomb-specific).
\item Layer~2: calibration~$\pi$ (Coulomb-specific).
\item Layer~3: Wilson lattice gauge with natural matter coupling
  (Coulomb-specific, via Papers~25, 30; $S^5$ is gauge-only per
  Sprint~ST-SU3).
\item Layer~4: modular-Hamiltonian Pythagorean
  orthogonality on the hemispheric wedge (Coulomb-specific; $S^5$
  Hardy sector has no clean analog).
\item Layer~5: spectral-action gravity termination
  (Paper~51). The CH Dirac spectral action on $S^{2m+1}$ terminates
  at exactly $(m{+}1)$ power-law terms; only $S^3$ ($m=1$) gives
  \emph{pure Einstein gravity} (2~terms: cosmological $+$ EH, no
  $R^2$ or higher-curvature corrections). $S^5$ ($m=2$) terminates at
  3~terms (includes $R^2$ Gauss--Bonnet). The mechanism is the
  Bernoulli identity for odd polynomials at half-integer shifts.
\item Layer~6 \emph{(new)}: master Mellin engine chirality-parity
  selection rule (Sprint Q5'-S5-falsifier, June~2026;
  \texttt{debug/sprint\_q5p\_s5\_falsifier\_memo.md}).  On the
  Camporesi--Higuchi $S^3$ spectral triple at every finite
  $n_{\max}$, the Mellin source $\mathrm{Tr}(D^k\,e^{-tD^2})$
  exhibits a bit-exact $\mathbb{Z}/2$ chirality-parity factorization
  (Sprint~Q5'-CH-1):\ plain trace vanishes identically at odd $k$,
  supertrace vanishes identically at even $k$.  On the
  Bargmann--Segal Hardy sector at every finite $N_{\max}$ this rule
  fails on both load-bearing structural conditions:\ the Hardy
  spectrum is strictly positive (no $\pm$-partner) and no
  half-integer-weight chirality grading exists on the $(N, 0)$
  tower (the \emph{angular} weight;\ the zero-point $\tfrac32$ is a
  different, metaplectic half-integer --- see the Layer-7 companion
  note below).  This propagates the Coulomb/HO asymmetry from the
  modular-Hamiltonian level (Layer~4) and the spectral-action level
  (Layer~5) to the master Mellin engine level, naming a
  CH-substrate-specific structural prerequisite for the cosmic-Galois
  $U^*$ enrichment program (Paper~55~\S~subsec:open\_m2\_m3).
\item Layer~7 \emph{(new)}: CM/Hodge-circle arithmetic of the compact
  flow (August~2026;\ the flow-level identification is Observation-tier in
  Paper~56, while the $S^5$ obstructions below are exact;\
  \texttt{tests/test\_paper24\_hardy\_circle\_arithmetic.py}).  On
  $S^3$ the compact KMS/Bisognano--Wichmann circle has
  $\pm$-symmetric integer generator spectrum, so its $\beta/4$
  quarter-period point \emph{is} a $\mathbb{Q}$-rational complex structure
  $J$, $J^2=-\mathbf 1$, making the fundamental doublet a polarized
  weight-one $\mathbb{Q}$-Hodge structure with CM by $\mathbb{Q}(i)$ (Paper~56).  The
  $S^5$ Hardy phase circle --- the $U(1)$ generated by the Euler
  operator --- is equally compact and even carries a \emph{faithful}
  $\mu_4$ (the metaplectic zero-point $\tfrac32$ lifts the graph's
  bipartition parity $(-1)^N$ to order four), yet admits \emph{no} CM
  Hodge structure, for three independent exact reasons:\ (i) the
  generator $N+\tfrac32$ is strictly positive, while
  $\det(\cos t\,\mathbf 1+\sin t\,J)=1$ forces a traceless (i.e.\
  $\pm$-paired) generator on any Hodge $2$-plane;\ (ii) on the
  substrate's own $\mathbb{Q}$-form (the $\pi$-free $(N,l,m)$ basis) the
  quarter point acts by $i^N$ (projectively;\ the metaplectic $\tfrac32$
  contributes a global phase $e^{3\pi i/4}$), $\mathbb{Q}$-rational exactly where it squares
  to $+\mathbf 1$;\ (iii) a weight-one structure needs the conjugate
  half $(0,N)$ --- precisely the antiholomorphic sector the Hardy
  construction discards, whereas the Dirac's chirality-doubled
  $\pm(n+\tfrac32)$ spectrum carries it for free.  Layer~7 is a new
  consequence level (arithmetic/Tannakian:\ the field of definition
  of the flow's Hodge group), standing to Layers~4--6 as those stand
  to Layers~1--3.
\end{itemize}

A companion \emph{non}-layer belongs beside Layer~7, because it is the half of
the $S^3$ arithmetic that does \emph{not} separate the two sides:\
conductor-$4$ \emph{period} content transfers.  The Bargmann graph is bipartite
(every edge has $|\Delta N|=1$), so $(-1)^N$ is a substrate-natural $\mathbb{Z}_2$
grading, and the graded Hardy spectral zeta evaluates in closed form to
$Z_-(s)=\sum_N(-1)^N g_N\,(N+\tfrac32)^{-s}=2^{s-3}\!\left(\beta(s)-
\beta(s-2)\right)$ --- exactly one quarter of the Dirac-tower value of
Paper~28's $\chi_{-4}$ theorem, because both towers carry the same
``index${}+d/2$'' eigenvalues at $d=3$ and their degeneracies differ by $4$
(at $s=4$:\ $Z_-(4)=2(\beta(4)-G)$ with $G$ Catalan's constant).  The
$\chi_{-4}$ is produced by the half-integer index shift times the $\mathbb{Z}_2$
grading --- spin on $S^3$, the \emph{metaplectic} zero-point on $S^5$, no spin
required --- while the ungraded Hardy partition function
$\sum_N g_N q^N=1/(1-q)^3$ (the Hilbert series of $\mathbb{C}[z_1,z_2,z_3]$) is pure
Tate, conductor $1$.  So the sharp split is:\ an order-$2$ datum lifted to
order $4$ suffices for conductor-$4$ \emph{periods} on either side;\ the CM
\emph{Hodge structure} of Layer~7 additionally needs the $\pm$-paired
$\mathbb{Q}$-rational carrier, which only the spinor side has.  (Honest cap:\ on the
Dirac side the $\beta$-values are measured in a computed observable, while
$Z_-$ is not yet attached to any HO/nuclear observable --- the identity is
exact, its physical significance an observation.  And the HO shift can be
normal-ordered away, killing the grading's $\chi_{-4}$, whereas the Dirac
shift is chirality-forced --- the residual asymmetry \emph{inside} the
period statement.)

The fourth-layer statement is structural rather than numerical.
On the truncated Camporesi--Higuchi spectral triple at signature
$(3, 1)$ (the Lorentzian Krein-space lift of the $S^3$ Coulomb
discretisation; Paper~43~\cite{paper43}), the
Bisognano--Wichmann-aligned local Hamiltonian $H_{\mathrm{local}} :=
K^{\alpha, W}_L / \beta$ and the wedge-restricted Lorentzian Dirac
$D_W^L$ live in mutually-orthogonal Hilbert--Schmidt subspaces of
the wedge operator algebra:
\[
   \langle H_{\mathrm{local}}, D_W^L \rangle_{\mathrm{HS}} \;=\; 0
\]
bit-exactly at every tested $n_{\max}$ and $N_t$, across all six
modular witnesses (Bisognano--Wichmann + Hartle--Hawking $\times$
2 + Sewell + Unruh $\times$ 2).  This holds because
$H_{\mathrm{local}}$ is diagonal in the unfolded wedge spinor basis
(eigenvalues $\mathrm{two\_m}_j > 0$, the rotation generator around
the Hopf axis) and $D_W^L$ is off-diagonal in the same basis (it
intertwines $\pm m_j$ chirality partners via the wedge basis
change).  The closed-form residual lives entirely in the master
Mellin engine M1 ring (Paper~32~\S~VIII):
\[
   \|H_{\mathrm{local}} - D_W^L\|_F^2
   \;=\; \kappa_g^2 \cdot \frac{S(n_{\max})}{4\pi^2}
       \;+\; D(n_{\max}),
\]
with $S$ and $D$ pure rationals in $n_{\max}$ (cumulative wedge
Casimir traces).

\paragraph{Why the fourth layer fails on the $S^5$ Hardy sector.}
The Pythagorean structure requires the $S^3$ confluence of
first-order $D_{\mathrm{GV}}$ \emph{and} second-order
$\Delta_{\mathrm{LB}}$ coexisting on a parallelizable spinor
bundle with half-integer Hopf-axis wedge.  On the Bargmann--Segal
Hardy sector $H^2(S^5)$ this confluence is structurally absent.
Four named obstructions:

\begin{enumerate}\setlength{\itemsep}{2pt}
\item \emph{No spinor sector on the $(N, 0)$ tower.}  The
  symmetric irreducible representations of $\mathrm{SU}(3)$ are
  bosonic (integer $m_l$); the Hardy sector has no half-integer
  weight content, hence no spinor lift compatible with the
  Bargmann transform.
\item \emph{No second-order vs.\ first-order distinction with a
  separate Dirac.}  The HO's spectrum-computing role is played by
  the first-order Euler operator $\hat{N} + 3/2$ on the Hardy
  sector, which \emph{is} the Hamiltonian.  There is no
  ``$D_W^L$'' sitting beside a Laplacian the way $D_{\mathrm{GV}}$
  sits beside $\Delta_{\mathrm{LB}}$ on $S^3$; the operands of the
  Pythagorean comparison collapse to two diagonal operators in
  the same subspace, and the inner product is generically
  nonzero.
\item \emph{No half-integer wedge.}  The natural BW-analog
  generator $K^{\alpha, W} = \mathrm{diag}(2 m_l > 0)$ has
  \emph{even}-integer spectrum, so the modular period closes at
  $\beta = \pi$ rather than $\beta = 2\pi$.  The four-witness
  Wick-rotation theorem at $\beta = 2\pi$ is half-integer-spectrum
  specific.
\item \emph{Coulomb/HO category mismatch resurfaces.}  The
  layer-1/2/3 asymmetry already documented above propagates to
  the modular-Hamiltonian level: every structural ingredient
  Pythagorean orthogonality requires (spinor bundle + half-integer
  wedge + second-order $\Delta$ alongside first-order $D$) is the
  Coulomb-side outcome of layers~1--3.
\end{enumerate}

These four obstructions are the same blocker that prevents the
cross-manifold tensor product $\mathcal{T}_{S^3} \otimes
\mathcal{T}_{\mathrm{Hardy}(S^5)}$ at the AC-extension level
(Paper~32~\S~VIII.C G4b), now showing up at the modular-Hamiltonian
level as a sibling obstruction rather than a new one.  The
seven-layer Coulomb/HO asymmetry is consistent across spectrum
computation, calibration, gauge-matter coupling, modular structure,
spectral-action gravity termination, master-Mellin chirality
factorization, and Hodge-circle arithmetic.

\paragraph{Honest scope.}  The HS-orthogonality is empirically
demonstrated and structurally explained via the diagonal /
off-diagonal subspace decomposition.  A formal proof of the
subspace decomposition at general $(n_{\max}, N_t)$ is a named
follow-on; the bit-exact numerical evidence at $n_{\max} \in
\{1, \ldots, 5\}$ and across the 18-cell six-witness panel is
strong evidence the orthogonality is structural, not coincidental.
Supporting memos:
\texttt{debug/h\_local\_residual\_pslq\_memo.md},
\texttt{debug/six\_witness\_hs\_orthogonality\_memo.md},
\texttt{debug/pythagorean\_extension\_scoping\_memo.md}.
...
\emph{Step 1 (Jauch--Hill, 1940; uniqueness sharpening
Wybourne / Iachello).}  Jauch and Hill~\cite{jauchhill1940}
proved the forward direction: the three-dimensional isotropic
harmonic oscillator possesses an $\mathrm{SU}(3)$ dynamical
symmetry, constructing the explicit $\mathfrak{su}(3)$ generators
as bilinears in the position and momentum operators (the angular
momentum $\mathbf{L}$ and the quadrupole tensor
$Q_{ij} = \frac{1}{2}(p_i p_j / m\omega + m\omega x_i x_j)$) and
showing that they close into $\mathfrak{su}(3)$ under commutation.
The converse --- that $V(r) \propto r^2$ is the \emph{only}
central-field potential whose generators close into
$\mathfrak{su}(3)$, so that the closure \emph{forces}
$V(r) \propto r^2$ --- is the uniqueness sharpening established in
the later dynamical-symmetry literature; we take the biconditional
\[
  V(r) = \tfrac{1}{2} m\omega^2 r^2
  \;\;\Longleftrightarrow\;\;
  \mathrm{SU}(3)\text{ dynamical symmetry}
\]
in the form standard in the dynamical-symmetry literature~\cite{wybourne1974,iachello2006}
(Iachello, Chapters~11--12 on spectrum-generating and degeneracy algebras).
\emph{Provenance caveat:} we have confirmed the forward direction (Jauch--Hill
1940) against its primary source, but not the \emph{converse}---that su(3)
closure forces $V \propto r^{2}$---against a specific numbered statement in
either monograph.  Theorem~\ref{thm:rigidity}'s converse half therefore rests on
an imported result we flag as unverified at the pointer level.
...
sharpening of Step~1~\cite{wybourne1974, iachello2006}: the
algebraic closure of the angular momentum and quadrupole
generators into $\mathfrak{su}(3)$ \emph{forces} $V \propto r^2$
(the forward implication is the original Jauch--Hill
result~\cite{jauchhill1940}).  This is the HO analog of the
corresponding biconditional Fock--Bargmann result for the Coulomb
problem (the Runge--Lenz closure forces $V \propto -1/r$).  Both
biconditional results are standard classical group
theory~\cite{wybourne1974, iachello2006}.
...
\subsection{Two-fermion entanglement rigidity --- \textsc{retracted}}
\label{sec:entanglement-rigidity}

\textbf{[RETRACTED 2026-08-22.]}  Earlier versions of this section carried a
corollary of the HO rigidity theorem stating that \emph{the spatial 1-RDM von
Neumann entropy of the closed-shell two-fermion ground state on the
Bargmann--Segal graph is identically zero for any central two-body interaction
$V(r_{12})$}, on the mechanism that any central $V(r_{12})$ preserves the total
HO quantum number $N_{\mathrm{tot}}$.  \textbf{Both the claim and the mechanism
are withdrawn.}

\emph{The mechanism is false.}  The Moshinsky--Talmi \emph{bracket} conserves
$N$ --- that is a property of the harmonic-oscillator coordinate
transformation.  The \emph{potential matrix element} does not.  A central
$V(r_{\mathrm{rel}})$ is diagonal in the centre-of-mass quantum numbers and in
$l_{\mathrm{rel}}$, but it \emph{couples different relative-$n$}.  Expanding
the element, the bra bracket forces
$2n + l + 2N_{CM} + \Lambda = N_{\mathrm{bra}}$ and the ket bracket forces
$2n' + l + 2N_{CM} + \Lambda = N_{\mathrm{ket}}$, so
$n - n' = (N_{\mathrm{bra}} - N_{\mathrm{ket}})/2$:\ the $N$-changing elements
are exactly those with $n \neq n'$, and they are nonzero.  For the Minnesota
singlet at $b=1$,
$\langle n_{\mathrm{rel}}{=}0, l{=}0 \lvert V \rvert n_{\mathrm{rel}}{=}1,
l{=}0 \rangle = +17.3$~MeV against a diagonal
$\langle 0,0 \lvert V \rvert 0,0 \rangle = -0.55$~MeV.  (Magnitude caveat,
2026-08-29:\ this contrast is convention-sensitive --- the $b{=}1$ diagonal
sits near a zero crossing, and at the production convention
($b = 2.0364$, $\hbar\omega = 10$~MeV) the coupling is $0.48\times$ the
diagonal, \emph{smaller} rather than larger.  The load-bearing evidence for
non-conservation is convention-independent:\ the measured commutator
$\lVert[H_{HO},V]\rVert_F/\lVert H_{HO}\rVert_F = 0.74$ and the nonzero
corrected entropies below, not any single matrix-element ratio.)

The section's own contrast should have exposed this:\ it argued that the
Coulomb case is nontrivial because $1/r_{12}$ ``does not preserve any HO-like
total-quanta quantum number'', but $1/r_{12}$ \emph{is} a central potential.
Both statements cannot hold.

\emph{The zero was a code artifact.}  The published $S_{\mathrm{full}} = 0$ and
$\lVert[H_{HO},V]\rVert_F / \lVert H_{HO}\rVert_F < 10^{-15}$ were produced by
an undocumented guard in the two-body matrix-element routine that returned
zero whenever the bra and ket total quantum numbers differed, discarding the
couplings above.  With the guard removed the measured values are:

\begin{center}
\begin{tabular}{rrrr}
\hline
$N_{\max}$ & $E_{0}$ (MeV) & $S_{\mathrm{full}}$ &
$\lVert[H_{HO},V]\rVert_F / \lVert H_{HO}\rVert_F$ \\
\hline
2 & 21.6538 & 0.0671 & 0.74 \\
3 & 21.6279 & 0.0716 & 0.63 \\
4 & 21.5442 & 0.0833 & 0.67 \\
\hline
\end{tabular}
\end{center}

\emph{So the HO two-fermion ground state is not a single Slater determinant for
a central two-body interaction, and $H_{HO}$ and $V$ do not commute.}  The
$\pi$-freeness results of Section~\ref{sec:pi-free} are unaffected --- they
concern the graph's eigenvalues and adjacency weights, not the interacting
two-fermion state --- but the claim that $\pi$-freeness extends ``also to
two-fermion entanglement'' does not survive, and the asserted structural
duality with Paper~27's nonzero Coulomb scaling is withdrawn with it:\ the two
bases are not opposites on this axis.

\emph{Methodological note, recorded because it is the transferable part.}  The
corroborating evidence was itself a symptom.  The result was reported as robust
partly because $E_{0}$ came out independent of $N_{\max}$ --- but that
independence was produced by the same guard, which froze the ground state
inside a single $N_{\mathrm{tot}}$ block.  Under the corrected computation
$E_{0}$ decreases with basis size, as a variational calculation must.  A
quantity that comes out \emph{exactly} zero, or \emph{exactly} invariant, where
the physics predicts only ``small'' or ``weakly dependent'', is a reason to ask
...
The HO rigidity theorem of Section~\ref{sec:rigidity}
and the Coulomb/HO asymmetry of Section~\ref{sec:asymmetry}
established that calibration~$\pi$ (even-zeta content) enters from
second-order Riemannian operators on scalar bundles---which the Coulomb
system supplies via the Fock projection to $-\Delta_{\mathrm{LB}}$
on~$S^3$, and which the HO does \emph{not}:\ its natural operator is the
first-order, complex-analytic Bargmann--Segal Euler operator on
$H^2(S^5)$, and its projection carries no calibration~$\pi$ at all.
...
\subsection{Gauge-structure corollary: reduced Wilson content on $S^5$}
\label{sec:gauge-structure-reduced}

Paper~25~\cite{paper25} observes that the GeoVac $S^3$ Coulomb graph
at finite~$n_{\max}$ is simultaneously a Fock discretization of
$S^3$, a triangle-free simplicial 1-complex with discrete Hodge
decomposition, and an abelian Wilson-type $U(1)$ lattice gauge
structure with matter on nodes and a $U(1)$ connection on edges.
The same three structural layers are available on the
Bargmann--Segal graph of the present paper, but with reduced
physical content.  This corollary documents the transfer pattern.

\paragraph{Positive transfer (abelian $U(1)$ Wilson--Hodge
structure).}
The Bargmann graph at $N_{\max} = 5$ has 56 nodes, 165 edges, one
connected component, and Euler characteristic $V - E = -109$.  The
signed incidence matrix $B\in\mathbb{Z}^{56\times 165}$, oriented
along the creation-operator direction ($N\to N+1$), gives a discrete
exterior derivative $d_0 = B^\top$ with node Laplacian $L_0 = BB^\top$
and edge Laplacian $L_1 = B^\top B$.  The first Betti number is
$\beta_1 = E - V + c = 110$, counting the independent cycle classes
that carry Wilson-loop holonomies in the abelian interpretation.
Under a node-local $U(1)$ gauge transformation
$\psi_v \mapsto e^{i\chi_v}\psi_v$, the dipole edge amplitudes
$\langle w\,|\,z_q\,|\,v\rangle$ transform covariantly as
$U_{vw}\mapsto e^{-i\chi_v}U_{vw}e^{i\chi_w}$.  The combinatorial
Hodge decomposition
$\mathbb{R}^{165} = \mathrm{im}\,d_0 \oplus \ker L_1$ and the
SVD identity between nonzero spectra of $L_0$ and $L_1$ both hold.
At this level of abstraction, the Bargmann graph is a lattice
gauge structure in exactly the Paper~25 sense.

\paragraph{Reduced content (no spectrum-computing role for $L_0$).}
On $S^3$, the node Laplacian $L_0 = D - A$ is conformally identified
with the continuum $S^3$ Laplace--Beltrami operator, whose eigenvalues
$n^2 - 1$ are mapped to the Coulomb spectrum by
the kinetic constant $\kappa = -1/16$.  The Wilson dictionary in
Paper~25 identifies $L_0$ with the matter propagator's inverse
(covariant Laplacian $-\nabla^2 + m^2$), and this identification has
spectral content: the matter spectrum is the spectrum of the continuum
operator $L_0$ is conformally identified with, up to $\kappa$-scaling.  On $S^5$, by contrast, the Bargmann
node Laplacian is spectrally inert for the physical HO spectrum: the
HO energies $\hbar\omega(N + 3/2)$ live in the diagonal
(Eq.~\eqref{eq:H-graph}), not in~$L_0 = D - A$.  The graph adjacency
$A$ encodes dipole transition amplitudes only, and Corollary~\ref{cor:gauge-structure-reduction}
(item~1) shows that the node Laplacian $L_0$ computed from $A$ plays no role
in reconstructing the spectrum.

\paragraph{Reduced content (no Wilson SU(3) structure).}
The complex Hopf fibration $S^1\to S^5\to\mathbb{CP}^2$ of
Eq.~\eqref{eq:s5-fibration} suggests a potential non-abelian
$\mathrm{SU}(3)$ gauge structure, with link variables valued in the
fundamental rep $\mathbf{3}$ and node matter in the symmetric
irreps $(N, 0)$.  This does not materialize.  The dipole operator
$z_q$ is the fundamental $\mathbf{3}$ and intertwines
$(N, 0)\otimes(1, 0)\to(N+1, 0)$ as a Clebsch--Gordan coupling, not
as a group element of $\mathrm{SU}(3)$.  A Wilson lattice gauge
structure requires the link variables to live in a fixed group
(the same $\mathrm{SU}(3)$ on every edge) acting uniformly on a
fixed matter irrep.  The Bargmann graph has \emph{different}
$\mathrm{SU}(3)$ irreps on different shells (the representation
dimension grows as $(N+1)(N+2)/2$), which is structurally a
representation tower, not a lattice matter-gauge system.  The
natural gauge group of the Bargmann graph is therefore $U(1)$,
inherited from the phase ambiguity of the node wavefunctions,
not $\mathrm{SU}(3)$.

\paragraph{Reduced content (no $\mathbb{CP}^2$ base spectrum).}
The natural discrete Hopf quotient collapsing the $m_l$-fibers of
the (SO(3)-adapted) Bargmann basis at fixed $(N, l)$ gives a
12-sector graph at $N_{\max} = 5$:
\[
\begin{aligned}
...
The scalar Laplace--Beltrami spectrum on $\mathbb{CP}^2$ with the
Fubini--Study K\"ahler metric of constant holomorphic sectional
curvature~$K = 4$ is~\cite{berger_gauduchon_mazet}
\begin{equation}
  \lambda_k = 4k(k+2),\qquad k = 0, 1, 2, \ldots,
\end{equation}
with degeneracy $(k+1)^3$ (the dimension of the $(k, k)$
$\mathrm{SU}(3)$ irrep).  The 12-sector quotient Laplacian under
the natural multiplicity weighting has eigenvalues
$\{0, 2.22, 4.87, 4.94, 11.65, 12.11, 18.74, 28.09, 33.58, 50.61,
63.77, 99.43\}$.  The best single-scale rescaling against the
analytic CP$^2$ sequence $\{0, 12, 32, 60, 96, 140, \ldots\}$ has
a maximum relative residual of $24.98\%$; the ratios
$\lambda_{\mathrm{quot},k}/\lambda_{\mathrm{CP}^2,k}$ vary between
$0.08$ and $0.19$ across the 12 pairs, with no uniform rescaling.
The $m_l$-quotient is therefore a combinatorial SO(3)-labeled graph
on 12 sectors, not a spectral discretization of $\mathbb{CP}^2$.
At this $N_{\max}$, there are simply too few sectors to represent
the four-dimensional $\mathbb{CP}^2$ Laplacian, and the weighting
scheme in the quotient is inherited from SO(3) branching, not from
the K\"ahler volume form on $\mathbb{CP}^2$.

\begin{corollary}[Gauge-structure reduction on the Bargmann graph]
\label{cor:gauge-structure-reduction}
The abelian $U(1)$ Wilson--Hodge structure of the $S^3$ Coulomb
graph (Paper~25) transfers to the Bargmann--Segal $S^5$ graph at
the combinatorial level (incidence matrix, node/edge Laplacians,
discrete Hodge decomposition, $\beta_1$ Wilson-loop classes), but
the physical content of the structure is strictly reduced:
\begin{enumerate}
\item The node Laplacian $L_0 = D - A$ carries the Coulomb spectrum on
  $S^3$ through its conformal identification with the continuum
  Laplace--Beltrami operator ($\kappa = -1/16$) but is spectrally inert on $S^5$ (the
  HO spectrum is in the diagonal).
\item The natural gauge group of the graph is $U(1)$; no Wilson
  $\mathrm{SU}(3)$ lattice structure emerges from the $(N, 0)$
  symmetric irreps, because transitions between distinct irreps
  are Clebsch--Gordan intertwiners, not group elements.
\item The $m_l$-fiber-collapse quotient does not produce a
  spectral discretization of $\mathbb{CP}^2$ at finite $N_{\max}$.
\end{enumerate}
\end{corollary}

The corollary extends the Coulomb/HO asymmetry of
Theorem~\ref{thm:asymmetry} from a two-layer statement
(spectrum: yes $S^3$, no $S^5$; calibration~$\pi$: yes $S^3$,
no $S^5$) to a three-layer statement adding gauge-structure
content (Wilson interpretability: full on $S^3$, reduced on $S^5$); this
is the third of the seven layers catalogued in \S\ref{sec:asymmetry_layer4}.
The universal content --- that the Wilson--Hodge vocabulary applies
combinatorially to any simplicial 1-complex --- is preserved, but
the Coulomb-specific content --- that the graph Laplacian
simultaneously carries the physical spectrum (through its conformal
identification with the continuum Laplacian) and supports a
nontrivial matter-gauge double --- is not.

A practical consequence for the Paper~2 fine-structure-constant
Observation~\cite{paper2_alpha}: the $S^5$ analog of the Paper~2
combination rule $K = \pi(B + F - \Delta)$ does not exist in
natural form.  A candidate $F$-analog (the Dirichlet series
$\sum_{N\geq 1} g_N^{\mathrm{HO}} / N^{d_{\max}}$ with
$g_N^{\mathrm{HO}} = (N+1)(N+2)/2$ and packing exponent
$d_{\max} = \dim\mathbb{C}^3 = 6$) evaluates to
$\tfrac{1}{2}[\zeta_R(4) + 3\zeta_R(5) + 2\zeta_R(6)]$, a mixed
even/odd zeta combination with no clean $\zeta_R(2) = \pi^2/6$
projection; and Paper~23's Fock projection rigidity theorem
establishes that the spectrum-computing projection with
calibration~$\pi$ is unique to $S^3$ (not $S^5$), so the $B$ and
$\Delta$ analogs (both tied to the Coulomb projection) also do not
lift.  The $S^5$ graph is thus a lattice gauge structure with
abelian $U(1)$ content but without $\alpha$-like spectral content,
consistent with the operator-order transcendental discriminant of
Section~\ref{sec:spinor-lichnerowicz}.  Cross-reference: Paper~25
\S VII.1 (the open question answered here); debug memo
\texttt{debug/s5\_gauge\_structure\_memo.md}; numerical data
\texttt{debug/data/s5\_graph\_spectrum.json}.
...
\subsection{For the fine-structure constant Observation}
\label{sec:alpha}

Paper~2~\cite{paper2_alpha} observes that the fine-structure
constant satisfies the cubic equation $\alpha^3 - K \alpha + 1 = 0$
...
rule $K = \pi(B + F - \Delta)$ is an empirical Observation.
...
themselves.  Paper~2's combination rule remains an Observation; its
...
and the Gaunt integral; the ERI density bound (at $l_{\max}=3$:
$6.06\%$ under the full-Gaunt global-$M_L$ rule, $1.44\%$ under the
stricter pair-diagonal restriction---independent of $V(r)$ either way); the block-diagonal
structure of two-body operators; and the exactly $Q$-linear composed
Pauli count, $N_{\mathrm{Pauli}} = 27.90 \times Q$ at fixed basis
(the retired pair-diagonal rule gave $O(Q^{2.5})$).  These are the
Paper~22 results.
...
$\alpha$ Observation; the kinetic constant $\kappa = -1/16$ from
...
the biconditional uniqueness of $\mathrm{SU}(3)$ dynamical
symmetry (forward direction due to Jauch--Hill, converse to
Wybourne and Iachello), the Borel--Weil identification of $(N, 0)$
...
fine-structure constant Observation: any geometric derivation of
...
\bibitem{paper25}
J.~Loutey,
\textit{The Hopf Graph as a Lattice Gauge Structure: A Framework
Observation}, Paper~25 of the Geometric Vacuum series
(synthesis, 2026).

\bibitem{paper32}
J.~Loutey,
\textit{The GeoVac spectral triple}, Paper~32 of the Geometric
Vacuum series (synthesis, 2026).

\bibitem{paper43}
J.~Loutey,
\textit{Lorentzian extension of the four-witness Wick-rotation
theorem at finite cutoff}, Paper~43 of the Geometric Vacuum
series (standalone, 2026).

\bibitem{berger_gauduchon_mazet}
M.~Berger, P.~Gauduchon, and E.~Mazet,
\textit{Le Spectre d'une Vari\'et\'e Riemannienne},
Lecture Notes in Mathematics \textbf{194}, Springer (1971);
see also Ch.~III (\emph{Le spectre d'une vari\'et\'e riemannienne}) for the scalar Laplace spectrum on
$\mathbb{CP}^n$ with the Fubini--Study metric.

\bibitem{higgs_pickrell2025}
V.~Higgs and D.~Pickrell,
``Spherical Harmonic Oscillators,''
arXiv:2503.23549 (2025).
