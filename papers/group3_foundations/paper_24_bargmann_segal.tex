...
summarizes the comparison.\footnote{The Coulomb/HO asymmetry has been
refined to six layers (enumerated in \S\ref{sec:asymmetry_layer4}) across Sprint~5 Track~S5 (gauge-content),
Paper~30 (SU(2) Wilson on $S^3$), Sprint~ST-SU3 (SU(3) Wilson on
$S^5$), and Paper~51 (gravity termination).  The first three Coulomb-specific
...
as the shared (\textbf{universal}) residue, not itself one of the six.  Sprint~ST-SU3 split
...
The footnote at the head of Section~\ref{sec:asymmetry} records
...
\item Layer~6 \emph{(new)}: master Mellin engine chirality-parity
  ...
  tower.  This propagates the Coulomb/HO asymmetry from the
...
six-layer Coulomb/HO asymmetry is consistent across spectrum
computation, calibration, gauge-matter coupling, modular structure,
spectral-action gravity termination, and master-Mellin chirality
factorization.
...
\]
in the form given by Wybourne~\cite{wybourne1974}, Chapter~16, and
Iachello~\cite{iachello2006}, Chapter~4.
...
Section~\ref{sec:rigidity} has a direct
...
spectral content: the matter spectrum is literally the spectrum of
$L_0$ up to $\kappa$-scaling.  On $S^5$, by contrast, the Bargmann
...
$A$ encodes dipole transition amplitudes only, and Theorem~3 below
shows that the node Laplacian $L_0$ computed from $A$ plays no role
...
is the third of the six layers catalogued in \S\ref{sec:asymmetry_layer4}.
...
see also Ch.~5 for the scalar Laplace spectrum on
