...
multiplicative normalizations~\cite{follandstein1974}.  The
$\mathrm{SO}(d)$-invariant harmonic-oscillator construction on
$d$-spheres has been developed independently in recent
mathematical work~\cite{higgs_pickrell2025}; their open question
for $d \ge 3$ is exactly the $d = 5$ case used here as the
$\mathrm{SO}(6)$-invariant Hardy-sector substrate.  For the
...
refined to five layers across Sprint~5 Track~S5 (gauge-content),
Paper~30 (SU(2) Wilson on $S^3$), Sprint~ST-SU3 (SU(3) Wilson on
$S^5$), and Paper~51 (gravity termination):
(i)~spectrum-computing role of $L_0$
(\textbf{Coulomb-specific}); (ii)~calibration~$\pi$
(\textbf{Coulomb-specific}); (iii)~Wilson gauge with natural matter
coupling (\textbf{Coulomb-specific}); (iv)~universal Wilson--Hodge
combinatorial vocabulary (\textbf{universal}).  Sprint~ST-SU3 split
...
three layers of the Coulomb/HO asymmetry: the spectrum-computing
...
On $S^3$, the node Laplacian $L_0 = D - A$ is precisely the operator
whose eigenvalues $n^2 - 1$ are mapped to the Coulomb spectrum by
...
\begin{enumerate}
\item The node Laplacian $L_0 = D - A$ is spectrum-computing on
  $S^3$ (via $\kappa = -1/16$) but spectrally inert on $S^5$ (the
...
content (Wilson interpretability: full on $S^3$, reduced on $S^5$).
...
simultaneously computes the physical spectrum and supports a
nontrivial matter-gauge double --- is not.

A practical consequence for the Paper~2 fine-structure-constant
conjecture~\cite{paper2_alpha}: the $S^5$ analog of the Paper~2
