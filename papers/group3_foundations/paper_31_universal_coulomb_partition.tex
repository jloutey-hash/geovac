...
\item A \emph{universal sector}, depending only on $\mathcal{A}$ and
  ...
  of Paper~22~\cite{paper22} --- the angular ERI density is
  potential-independent (global-$M_L$ Coulomb $D=6.06\%$ at $l_{\max}=3$;
  pair-diagonal realized $D_{\mathrm{pd}}=1.44\%$), bit-identical across
  Coulomb, harmonic oscillator, Woods--Saxon, square well, and Yukawa
  potentials --- is the canonical example.
...
the result is exact: the continuum $S^3$ Laplace--Beltrami spectrum
$\lambda_n = -(n^2 - 1)$ --- to which the graph Laplacian converges ---
...
calibration $\kappa = -1/16$ is itself rational (the $\pi$-free bridge
to the $\pi$-laden continuum), its value \emph{coinciding} with the
$1/16$ of the Fock projection Jacobian as an Observation, not a
derivation (Paper~7); the $\alpha$-combination $K = \pi(B + F -
...
Coulomb/HO asymmetry has three independent discriminators, not two
(within the seven-layer catalogue of Paper~24~\cite{paper24}), and gives
the predictive checklist that follows.
...
number.  Theorem~2 of Paper~22 then states that the ERI density is a function
only of the angular momentum cutoff.  The composed pipeline realizes the
\emph{pair-diagonal} density $D_{\mathrm{pd}}$ (per-pair $m_a=m_c$,
$m_b=m_d$):
\begin{equation}
  D_{\mathrm{pd}}(l_{\max}) =
  \begin{cases}
...
\end{equation}
The physically-correct global-$M_L$ Coulomb density $D(l_{\max})$
(selection rule $m_a+m_b=m_c+m_d$) is $\sim\!4\times$ larger
($14.84/8.52/6.06/4.83/3.99\%$ at $l_{\max}=1\ldots5$; Paper~22 Table);
$D_{\mathrm{pd}}$ is the figure the composed qubit pipeline realizes.
Paper~22 verified the potential-independence numerically across five
potentials (Coulomb, harmonic oscillator, Woods--Saxon, square well,
Yukawa) at $l_{\max} = 2$, finding bit-identical zero patterns
($D_{\mathrm{pd}} = 2.76\%$) in every case.
...
\item The composed-block Pauli count of Paper~14~\cite{paper14},
  exactly linear in the qubit count across molecules at fixed basis
  ($N_{\mathrm{Pauli}} = 27.90 \times Q$; the retired pair-diagonal
  rule gave $O(Q^{2.5})$), which depends on the block-diagonal
  structure of two-body matrix elements in a composed orbital basis
  and the Gaunt selection rules.  Verified across the 35 composed
  molecules in the GeoVac library.
...
\item The integer eigenvalue spectrum $\lambda_n = -(n^2 - 1)$ of
  the continuum $S^3$ Laplace--Beltrami operator (the graph-Laplacian
  limit) does not coincide with the rescaled energy spectrum.
...
$\kappa = -1/16$, the $\alpha$-combination Observation
...
$\mathrm{Vol}(S^2)/4 = \pi$); this calibration $\pi$ enters the
energy mapping, not the kinetic constant: $\kappa = -1/16$ is itself
$\pi$-free, coinciding with the geometric $1/16$ as an
Observation~\cite{paper7,paper24}.
...
$\kappa = -1/16$~\cite{paper7} & no ($-1/16$ rational; $\pi$ enters at the energy map) \\
Spectral zeta $\zeta_{D^2}(s)$~\cite{paper28} & yes ($\pi^{2k}$) \\
Hopf-bundle measure $\mathrm{Vol}(S^2)/4$~\cite{paper2_alpha} & yes \\
$\alpha$-combination Observation $K$~\cite{paper2_alpha} & yes \\
\hline
...
%=================================================================
\section{The Layer Sharpening (Sprint~5)}
...
(spectrum-computing and calibration $\pi$) to three independent
discriminators, the third being the Wilson physical content of the
gauge sector.  We summarize the potential-specific layers here as the
predictive checklist for extending the framework to any new central
potential (the full asymmetry is catalogued as seven layers in
Paper~24~\cite{paper24}).

\subsection{The structural layers}

\begin{description}
\item[Layer 1: Spectrum-computing role of the node Laplacian $L_0$.]
  In the Coulomb $S^3$ construction, the node Laplacian $L_0 = D - A$
  on the Fock graph is conformally identified with the continuum $S^3$
  Laplace--Beltrami operator, whose spectrum $\lambda_n = -(n^2 - 1)$
  (energy normalization) maps to $E_n = -1/(2n^2)$ via the Fock projection.  The graph
...
\subsection{The layer table}

\begin{table}[h]
\caption{Coulomb/HO partition by structural layer.  Rows~1--3 are the
three independent discriminators established here (Sprint~5
sharpening~\cite{paper25}); rows~4--5 are further potential-specific
layers from Papers~24 and~51~\cite{paper24,paper51}, part of the
seven-layer asymmetry catalogued in Paper~24~\cite{paper24}.
$\checkmark$~means the feature is present; ``--'' means it is absent or
not natural.}
...
Gravity termination (Paper~51~\cite{paper51} G4-5a) & $\checkmark$ (pure Einstein) & -- \\
\hline
...
The features above the divide in Table~\ref{tab:five_layers} are the
potential-specific layers --- the three independent discriminators
established here (rows~1--3) together with two further layers
catalogued in Papers~24 and~51; the features below are the
universal-sector features that hold on either sphere.

\subsection{Why three independent discriminators}
...
new central potential, the layer table provides a checklist
...
\item Composed-block exactly $Q$-linear Pauli
  count~\cite{paper14} ($N_{\mathrm{Pauli}} = 27.90 \times Q$ at
  fixed basis).  The structural sparsity pattern behind the composed
  architecture's advantage over Gaussian baselines ($54\times$ to
  $317\times$ at equal qubit counts for the Coulomb library) carries
  over to any spherical-fermion system.
...
\begin{itemize}
\item The continuum $S^3$ spectrum $\lambda_n = -(n^2 - 1)$ that the
  graph Laplacian converges to (no analog unless the system has
...
\item The fine-structure-constant Observation
...
\item \textbf{Potential-specific sector} (substantive $D$):
  ...
  the $\alpha$-combination Observation.  Per-system rederivation required.
\end{enumerate}
...
Sections~\ref{sec:universal} or
\ref{sec:specific}.  Its value is clarifying:
...
van~Suijlekom state-space Gromov--Hausdorff distance used in
Paper~38~\cite{paper38} for the WH1 PROVEN result.  The signature partition is the structural sibling
...
\begin{description}
\item[B1\@.] \emph{Lorentzian propinquity.}  The van~Suijlekom
  state-space Gromov--Hausdorff distance used in Paper~38~\cite{paper38}
  for the WH1 PROVEN result---and the Latr\'emoli\`ere propinquity
  family more broadly---is purely Riemannian as of May~2026
  (Latr\'emoli\`ere, Toyota, Hekkelman--McDonald,
  Leimbach--van~Suijlekom).  Constructing
  a Lorentzian propinquity is original NCG-mathematics work at the
  long-range scale.  The Nieuviarts~2025
...
exactly $Q$-linear Pauli count, the Wigner~$3j$ / $D$ algebra, and
the selection rules.  Adding the signature axis to the partition
...
\begin{description}
\item[Sprint L0 (sprint-scale, this).]  Documentation only.  The
...
\item[Sprint L1 (beyond sprint scale, MEDIUM-HIGH priority).]
  ...
  algebra.
\item[Sprint L2 (a substantial program).]  BBB Krein lift and Connes axiom audit
...
\item[Sprint L3 (long-range).]  Lorentzian propinquity construction
...
\item Four \textsc{wick-map free} projections close at the
operator-system level on the \emph{Riemannian} side (Sprint L1;
Paper~42 \S\S~5--8):\ the M1 sub-mechanism of the master Mellin engine
realizes the four-witness Wick-rotation theorem (Hartle--Hawking
$+$ Sewell $+$ Bisognano--Wichmann $+$ Unruh) as literal identification
at the operator-system level in Riemannian signature.  The
\emph{Lorentzian} side is structural correspondence, \emph{not}
literal identification:\ the Sprint L2-E Krein period closure
(Paper~32 \S~VIII.E.E) is signature-blind at finite cutoff --- the
truncated Bisognano--Wichmann boost is compact ($e^{2\pi i K}=I$ on
the integer modular spectrum), so the closed period is the Euclidean
KMS $\beta = 2\pi$ circle and the Lorentzian signature is metrically
invisible (Euclidean/convention).  The strong ``literal
identification'' reading of Sprint L2-E (2026-05-17) was withdrawn
2026-06 (see Paper~34~\S~III.29 and Paper~45).
\item Five \textsc{euclidean specific} projections were carried to the
Krein operator-system level at finite cutoff via Sprint L2-B/C/D/E
(Paper~32 \S~VIII.E.B/C/D/E):\ the Krein space, Lorentzian Dirac
$D_L = i^t \, \not{\!\!D}_{g_r}$ per van~den~Dungen 2016 Prop~4.1, and
Connes axiom audit at BBB $(m, n) = (4, 6)$ are constructed, and the
Krein period-closure identities close bit-exactly at every tested
$(n_{\max}, N_t) \in \{1, 2, 3\} \times \{1, 11, 21\}$.  These
identities are \emph{signature-blind}, however (the same compact-boost
mechanism as above), so they do \emph{not} constitute a Lorentzian
literal identification;\ the genuine Lorentzian propinquity extension
(blocker B1) remains open at Sprint L3 scope.
...
direct operational payoff of the Sig/Op framing introduced here:\ the
$28/28$ figure certifies the \emph{bucket classification} (which
projections transfer freely, which require system-specific
re-derivation), not a Lorentzian closure.
...
classification of GeoVac's projection inventory.  At the
operator-system / finite-cutoff level the \emph{Riemannian} closure is
complete;\ the \emph{Lorentzian} closure is \emph{not} --- the
truncated modular flow is the compact KMS $\beta = 2\pi$ circle, so
Lorentzian signature is metrically invisible at any finite cutoff
(Euclidean/convention), and the genuine Lorentzian extension (Sprint
L3, blocker B1) remains open as major NCG-mathematics work (see
Paper~34~\S~III.29, Paper~45).  The headline structural finding of
...
The fine-structure constant Observation
...
essentially uncorrelated ($|r| < 0.05$ at $n_{\max}=3$, and well below
the Dirac resolvent's correlation at every cutoff) because the $N = 1$
zero mode of $\Delta_{S^3}$ excludes the dominant $(1s,1s)$ contribution.
...
the rigidity theorems, and the $\alpha$-combination Observation --- depends on
...
isolates three independent discriminators, not two: spectrum-computing
role of $L_0$, calibration $\pi$, and Wilson physical content of the
non-abelian gauge sector, all stratifying along the same
operator-order divide; Paper~24~\cite{paper24} catalogues the full
asymmetry as seven structural layers.
...
following from the layer table
...
as exposited by J.-P.~Serre, S\'eminaire Bourbaki, expos\'e 100 (1954).
...
G.~Nieuviarts,
``Emergence of Time from a Twisted Spectral Triple in
