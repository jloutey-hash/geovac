...
digit printed.  Off that residue class the value is three to six times larger
...
\begin{table}[h]
\centering
\caption{\textbf{Historical values --- not reproduced by any current construction} (flagged 2026-09-03).  These figures are retained to document the original calculation;\ the production binary lattice gives $D_{2s} \approx 1.9$, $D_{2p} \approx 3.8$ (\S\ref{sec:geometry} QA note), and the CG construction gives $0.7071$, $1.9319$, $3.8284$, $2.9568$, $5.4096$, $6.1237$ --- of which only the $3d$ row here reproduces.  What survives is the \emph{qualitative} ordering $D_{2p} > D_{2s}$, which both constructions share.  Weighted node degrees for select quantum states. The non-integer values arise from weighted sums of transition matrix elements (see Appendix~\ref{app:berry}).}
\begin{tabular}{cccc}
\hline
