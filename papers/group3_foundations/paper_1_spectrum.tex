...
\begin{abstract}
We present a discrete geometric framework for the hydrogen atom based on finite information capacity. The hydrogen state space is modeled as a paraboloid lattice dual to the SO(4,2) conformal algebra. This single-particle model exhibits a fundamental algebra-geometry duality: \textbf{(i) Algebraic exactness:} \textbf{[PANEL-VERIFIED]} Transition operators $T_\pm, L_\pm$ reproduce the exact Rydberg quantum-number structure ($n$ and $\ell(\ell+1)$) from operator eigenvalues; the energies $E_n = -1/(2n^2)$ follow once the matched kinetic scale is applied (an Observation, Paper~7). \textbf{(ii) Geometric artifacts:} \textbf{[MEASURED]} A node-amplitude proxy for the $s/p$ gap is nonzero at finite lattice cutoff ($37\%$ at $n_{\max}=5$, oscillating through $15.6\%$ at $8$ and $1.7\%$ at $10$ to $0.39\%$ at $30$ on the binary production lattice; see the \S III QA note).  \textbf{[INTERNAL THEOREM]} Its size tracks the residue of $n_{\max}$ modulo $3$ --- exactly closed form on $n_{\max} \equiv 0$, and generically larger off it (the earlier $n_{\max}/(\sqrt{3}\,\pi)$ asymptotic is [retracted 2026-09-04: sp-ratio-false-precision], read off three sampled cutoffs) --- so the quoted endpoint sits on a favourable branch and the sequence is not evidence of convergence (2026-09-03;\ the earlier wording, ``the graph Laplacian lifts $s/p$ degeneracy \ldots\ through differential weighted connectivity'', both contradicted the clause below and named the weighted construction's mechanism for binary-lattice numbers, where every edge weight is~$1$). The cutoff sweep from $n_{\max}=5$ to 30 shows this is a discretization artifact---a residue-controlled truncation effect, \emph{not} spectral aliasing [retracted 2026-09-04: sp-splitting-aliasing-mechanism] (that reading was withdrawn 2026-09-03: the $l$-blocks are disconnected and carry no degeneracy to alias)---that oscillates and reads $0.39\%$ at $n_{\max}=30$ on the $n_{\max}\equiv 0 \pmod 3$ branch ($1.65\%$ and $2.58\%$ at $28$ and $29$): a proxy value, not evidence of recovering the exact Coulomb degeneracy [retracted 2026-09-04: sp-splitting-aliasing-mechanism], which belongs to the continuum operator (the graph's $l$-blocks are disconnected, so the graph has no $s/p$ degeneracy of its own). \textbf{(iii) Geometric phase structure:} \textbf{[SYMBOLIC PROOF]} The Berry phase vanishes identically for the real-valued transition operators; the edge-weight log-holonomy $\Theta(n) = -2\ln((n{+}1)/n) \sim n^{-1}$ measures the curvature of the weight function across the lattice. \textbf{Crucially, this model describes only the electron manifold}---electromagnetic interactions and the fine structure constant $\alpha$ cannot emerge from a single-particle lattice and require coupling to photon degrees of freedom, which we address in a companion paper.
\end{abstract}
...
\item \textbf{The Geometric Flesh:} The lattice \textit{topology}---the pattern of connections---generates physical corrections. High angular momentum states occupy higher-degree nodes at finite truncation (read before 2026-09-03 as differential weighted connectivity [retracted 2026-09-04: sp-splitting-aliasing-mechanism] lifting the $s/p$ degeneracy;\ withdrawn --- the $l$-blocks are disconnected components). The edge-weight log-holonomy provides a measure of lattice curvature that decays as $n^{-1}$.
\end{itemize}

In this paper, we present computational evidence for this dual framework. Section~\ref{sec:algebra} demonstrates spectral exactness. Section~\ref{sec:geometry} analyzes the $s/p$ node-amplitude proxy, showing it is a discretization artifact whose size tracks the residue of $n_{\max}$ modulo $3$ --- not a graph degeneracy (the $\ell$-blocks are disconnected) and not a continuum-convergence signal. Section~\ref{sec:relativity} analyzes the geometric phase structure, showing that the Berry phase vanishes while the log-holonomy decays as $n^{-1}$. Section~\ref{sec:discussion} discusses the algebra-geometry duality and its connection to wave-particle complementarity. Section~\ref{sec:conclusion} summarizes our findings.
...
\label{eq:ham_graph}
\end{equation}
where $V_{ii} = -1/n_i^2$ is a diagonal Coulomb node weight and $\beta$
a scaling factor.  \textbf{Eq.~\eqref{eq:ham_graph} is a general form, not the
object measured below and not the production object} (clarified 2026-09-03):\
the splitting measurements of \S\ref{sec:geometry} are taken on the bare
graph Laplacian $L = D - A$, and \texttt{AtomicSolver} builds
$H = K_{\mathrm{vac}} L$ with node weights never applied.  Under
Eq.~\eqref{eq:ham_graph} with $V_{ii} = -1/n_i^2$ the same twelve cutoffs give
$44.7$, $2.2$, $27.5$, $10.6$, $31.5$, $620.9\%$ and above --- a different
quantity entirely.
...
\end{align}
\textbf{[PANEL-VERIFIED]} States with higher angular momentum $l$ have \textit{more connections}---more available transitions to neighboring states. This higher connectivity translates to higher on-site energy in the Laplacian formulation;\ it does not couple the $l$-blocks, which are disconnected components, so it is not a mechanism for the $s/p$ proxy (that reading withdrawn 2026-09-03;\ \S III).

\subsection{The Splitting as Discretization Artifact}

We performed exact diagonalization of the bare graph Laplacian
$L = D - A$ for lattices with $n_{\max}$ ranging from 5 to 30 (corrected
2026-09-03:\ this paragraph read ``of $H$ (Eq.~\ref{eq:ham_graph})'', which is
not the operator measured --- see the note there).  Using a full dense
eigendecomposition, we identified the eigenstates with maximal overlap with
$|2,0,0\rangle$ (2s) and $|2,1,0\rangle$ (2p) and computed the
relative splitting $\Delta E_{\mathrm{rel}} = |E_{2p} - E_{2s}|/|E_{2s}|$.

At $n_{\max} = 10$ (385 nodes), the \emph{originally reported} splitting was:
\begin{align}
\lambda_{2s} &= 0.0202, \\
\lambda_{2p} &= 0.0238, \\
\Delta E &= \lambda_{2p} - \lambda_{2s} = 0.0035,
\end{align}
quoted as a $16\%$ relative splitting (arithmetically $17.3\%$).
\textbf{[MEASURED]} \emph{These are not the bare Laplacian's values} and are
retained only as the historical record:\ on the operator named above,
$L = D - A$ at $n_{\max} = 10$ gives $\lambda_{2s} = 3.1756$,
$\lambda_{2p} = 3.1206$ (corrected 2026-09-04 from a transposed $3.2306$ --- its mirror about $\lambda_{2s}$, which is why the ratio was unaffected) and a $1.73\%$ ratio (see the QA correction below,
and the twelve-cutoff list in \S\ref{sec:geometry}).  Flagged inline
2026-09-04:\ the method sentence above was corrected to name $L = D - A$ while
this display still carried the legacy figures, so the corrected sentence
appeared to vouch for them. However, this degeneracy
...
aliasing [retracted 2026-09-04: sp-splitting-aliasing-mechanism] in finite-element methods, an analogy retired
for the $s/p$ proxy on 2026-09-03 (the $\ell$-blocks are disconnected,
...
at $n_{\max} = 5, 6, 7, 8, 9, 10, 12, 15, 18, 20, 25, 30$.  Six of those twelve
cutoffs are multiples of~$3$, including the endpoint, and on that residue class
the value is exactly $\bigl(2-2\cos\frac{\pi}{n_{\max}-1}\bigr)/3$
(Eq.~\eqref{eq:lambda2s_residue});\ the neighbouring cutoffs $28$ and $29$ give
$1.65\%$ and $2.58\%$.  What \emph{is}
...
recur---Table~I (the abstract, Appendix~B and the convergence list
below now print the binary-lattice measurements).  (The
placeholder figure environments that also restated them were removed
2026-09-01.)}

\emph{Conditioning caveat (2026-09-03).}  The $l$-blocks of the graph Laplacian are \emph{disconnected} components (no edge changes $l$), so there is no $2s/2p$ degeneracy in the graph spectrum to lift:\ the quantity reported here selects, for each of the nodes $|2,0,0\rangle$ and $|2,1,0\rangle$, the eigenvector of maximal amplitude at that node, and that selection is a near-tie (top-two overlaps within $1.2\%$ at $n_{\max} = 30$, with selected eigenvalues $3.0$ and $0.011$).  The percentages below reproduce exactly but are \emph{not robust} to the mode-selection rule;\ they are a node-amplitude proxy, not a spectral gap (\texttt{tests/test\_paper1\_block\_spectrum.py}).
...
\begin{itemize}
  ...
  \item At $n_{\max} = 8$: $15.6\%$ (a secondary peak); by
...
see the conditioning caveat above;
\texttt{tests/test\_trunk\_qa\_splitting.py} pins the qualitative content
(bounded, non-monotone, downward trend, sub-percent at $n_{\max} = 30$),
\texttt{tests/test\_paper1\_block\_spectrum.py} the conditioning and the
CG-construction non-decay; the list printed before
2026-09-03---$13\%$, $16\%$, $0.3\%$, $0.005\%$---did not reproduce at
$n_{\max} = 5$, $20$ or $30$.)

The splitting does not decay monotonically; it \emph{oscillates} at
intermediate resolutions.  \textbf{[INTERNAL THEOREM]} \emph{The oscillation
is the residue of $n_{\max}$ modulo $3$, and the sequence is closed form}
(2026-09-03).  The $\ell = 0$ block is the path $P_{n_{\max}}$, whose
eigenvalues are $2 - 2\cos(j\pi/n_{\max})$;\ the mode selected by maximal
overlap with the $2s$ node has $j = \operatorname{round}(2n_{\max}/3)$, so
\begin{equation}
  \lambda_{2s} = 3 \ \text{\emph{exactly}} \quad\Longleftrightarrow\quad
  n_{\max} \equiv 0 \ (\mathrm{mod}\ 3),
  \label{eq:lambda2s_residue}
\end{equation}
and on that branch the reported ratio is exactly
$\bigl(2 - 2\cos\frac{\pi}{n_{\max}-1}\bigr)/3$ --- $2.7005$, $1.6715$,
$1.1351$, $0.8208$, $0.6209$, $0.4861$ and $0.3908\%$ at
$n_{\max} = 12, 15, 18, 21, 24, 27, 30$, matching the measured values to every
digit printed.  \emph{At neighbouring cutoffs} the off-branch value is generically larger --- asymptotically by $n_{\max}/(\sqrt{3}\,\pi)$ [retracted 2026-09-04: sp-ratio-false-precision], but far from that at small cutoff: $4.2\times$ and $6.6\times$ at $n_{\max} = 28, 29$ against $30$ ($1.65\%$, $2.58\%$ vs $0.39\%$), where the formula gives $5.5$;\ $20$--$23\times$ at $118, 119$ against $120$ ($22.1$);\ $550$--$552\times$ at $2998, 2999$ against $3000$ ($551.3$). Below $n_{\max} \approx 12$ the ordering can invert ($0.65\%$ at $7$ against $12.7\%$ at $6$). (Two earlier statements of this ratio --- ``three to six times'' and ``tracking $n_{\max}/(\sqrt{3}\,\pi)$ to within $1\%$'' [retracted 2026-09-04: sp-ratio-false-precision] --- were each read off three sampled cutoffs and stated as theorems;\ both are withdrawn.)  \textbf{The endpoint quoted throughout
this paper, $0.39\%$ at $n_{\max} = 30$, therefore lies on the favourable
branch}, a selection this paper did not disclose before 2026-09-03.

The decay is consequently $O(n_{\max}^{-2})$ along $n_{\max} \equiv 0$ and
$O(n_{\max}^{-1})$ generically --- a proved rate, replacing the earlier
statement that none was proven.  \emph{The mechanism previously printed here
--- ``spectral aliasing\ldots\ standing-wave reflections that differentially [retracted 2026-09-04: sp-splitting-aliasing-mechanism]
shift eigenvalues in distinct angular momentum sectors'' --- is withdrawn:}
the $\ell$-blocks are disconnected components
(\texttt{tests/test\_paper1\_block\_spectrum.py}), so there is no $s/p$
degeneracy present to alias and no cross-sector reflection to shift it.  Both
selected eigenvalues approach $3 = 2 - 2\cos(2\pi/3)$ because the target node
is vertex~$2$ of a path in each block.  As $n_{\max}$ grows the two proxy
eigenvalues coincide (a path-graph coincidence), which does \emph{not}
reproduce the continuum $SO(4)$ Coulomb degeneracy (the $\ell$-blocks are
disconnected, so the graph has no $s/p$ degeneracy of its own).

\subsection{Physical Interpretation}

\textbf{[PANEL-VERIFIED]} The 2p state has the higher weighted degree
($D_{2p} > D_{2s}$; the ``$4 \times$'' ratio came from the
non-reproducing Table~\ref{tab:degrees} figures --- see the \S III QA
note). \emph{Read before 2026-09-03 as:} the higher-degree 2p node is
more sensitive to the lattice boundary than the 2s state, the finite-$n_{\max}$
truncation acting as an absorbing wall that differentially perturbs
high-connectivity nodes [retracted 2026-09-04: sp-splitting-aliasing-mechanism]. \emph{Withdrawn for the $s/p$
proxy (2026-09-03):} its size is set by the residue of $n_{\max}$ modulo
$3$, not by a boundary effect;\ the ``geometric boundary effect'' reading printed here until 2026-09-04 is withdrawn with it, and it is not an emergent centrifugal barrier either.
...
or replace electromagnetic interactions, and its finite-lattice $s/p$ proxy
does \emph{not} reproduce the continuum $SO(4)$ Coulomb degeneracy (the
$\ell$-blocks are disconnected components;\ the continuum limit that would
carry that degeneracy is a reading, not established by any repository test).  \textbf{[INTERNAL THEOREM]} The $s/p$
node-amplitude proxy is a characterisable discretization artifact with a
closed-form value on $n_{\max} \equiv 0 \ (\mathrm{mod}\ 3)$ and a proved
rate --- $O(n_{\max}^{-2})$ there, $O(n_{\max}^{-1})$ generically
(Eq.~\eqref{eq:lambda2s_residue};\ the earlier ``no convergence rate is
proven'' is superseded).  Its magnitude depends on the residue of $n_{\max}$
modulo $3$ rather than on proximity to a continuum limit, so it does not
function as evidence of convergence;\ see the \S III QA note.
...
\item \textbf{Geometric Structure (Particle-like):} The graph Laplacian $L = D - A$ encodes \textit{perturbations}---corrections arising from discrete connectivity. The weighted degree matrix $D$ differs between angular momentum sectors at finite truncation (read before 2026-09-03 as differential connectivity [retracted 2026-09-04: sp-splitting-aliasing-mechanism] lifting the $s/p$ degeneracy;\ withdrawn --- the sectors are disconnected components). The edge-weight log-holonomy decays as $n^{-1}$ (Section~\ref{sec:relativity}), measuring the curvature of the weight function. This represents the discrete aspect of the lattice formulation.
\end{enumerate}
...
tested, not proved for all $n_{\max}$---while the Frobenius-norm ratio
converges to $2\sqrt{6}/3 = 1.632993\ldots$, its exact limit (algebraic closed form;\ the frozen \texttt{test\_trunk\_qa\_gearing.py} panel verifies only the monotone approach to $n_{\max}=30$, ratio $1.701$, not the limit) from
$\|L_+\|_F^2/\|T_+\|_F^2 = 4(N+2)(2N+1)/(3N^2-2) \to 8/3$ (corrected
2026-09-03 from ``drifts through $\approx 1.77$ without converging'':\ the
sequence is monotone decreasing and $1.77$ is a finite-$n_{\max}$ value, not a
non-limit)
...
on the binary lattice it decays as $n_{\max} \to \infty$ (not on the
CG-weighted construction of \S III) and exhibits the oscillatory
decay characteristic of finite-size aliasing [retracted 2026-09-04: sp-splitting-aliasing-mechanism] ---
a characterisation withdrawn for this quantity on 2026-09-03.
...
\item \textbf{Understood discretization artifacts:} \textbf{[INTERNAL THEOREM]} The graph Laplacian's node-amplitude proxy for the $s/p$ gap is closed form on $n_{\max} \equiv 0 \ (\mathrm{mod}\ 3)$ and is set by that residue rather than by proximity to a continuum limit (2026-09-03), and is large at finite $n_{\max}$ (up to $37\%$ at $n_{\max}=5$ on the binary production lattice, $15.6\%$ at $8$, $1.7\%$ at $10$), and is not spectral aliasing [retracted 2026-09-04: sp-splitting-aliasing-mechanism] (withdrawn 2026-09-03: the $l$-blocks are disconnected and carry no degeneracy to alias) nor an emergent physical force. The proxy oscillates ($4.1\%$ at $n_{\max}=20$) and reads $0.39\%$ at $n_{\max}=30$ on the $n_{\max}\equiv 0 \pmod 3$ branch ($1.65\%$, $2.58\%$ at $28$, $29$) --- a proxy value, not a recovery of the exact Coulomb degeneracy [retracted 2026-09-04: sp-splitting-aliasing-mechanism], which belongs to the continuum operator; on the CG-weighted construction of \S III the lift does \emph{not} decay ($129$, $68$, $84$ and $139\%$ at $n_{\max} = 8, 10, 15, 20$; \texttt{tests/test\_paper1\_block\_spectrum.py}), so the artifact-decay claim is scoped to the binary lattice, and both figures carry the conditioning caveat of \S III.
...
\item \textbf{Kinematic completeness:} \textbf{[OBSERVATION]} The lattice encodes the $\alpha$-independent single-electron structure---the Coulomb quantum-number structure, angular momentum structure, and geometric phase effects---while cleanly separating boundary artifacts from physical content.
\end{itemize}
...
\begin{table}[h]
\centering
\caption{\textbf{Historical values --- not reproduced by any current construction} (flagged 2026-09-03).  These figures are retained to document the original calculation;\ the production binary lattice gives \emph{integer} degrees --- $D_{2s} = 2$ and $D_{2p} = 3$ exactly, every edge weight being $1$ --- while the CG construction gives $0.7071$, $1.9319$, $3.8284$, $2.9568$, $5.4096$, $6.1237$, of which only the $3d$ row here reproduces.  (Corrected 2026-09-04:\ this caption assigned the CG values $1.9$ and $3.8$ to the binary lattice, inverting the separation it was written to enforce.)  What survives is the \emph{qualitative} ordering $D_{2p} > D_{2s}$, which both constructions share.  Weighted node degrees for select quantum states. The non-integer values arise from weighted sums of transition matrix elements (see Appendix~\ref{app:berry}).}
\begin{tabular}{cccc}
\hline
