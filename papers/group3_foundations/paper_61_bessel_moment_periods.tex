...
$\Gamma(1/8)\Gamma(3/8)$ are Chowla--Selberg~\cite{chowla_selberg1967} periods attached to the
...
periods are $\Gamma$-values by Chowla--Selberg~\cite{chowla_selberg1967} \textbf{[MEASURED, $\sim\!10^{-51}$;
...
its source-and-scale Borel singularity---the Stokes-data \emph{location}---is
...
$\sqrt{c_{1}}-ib$.  The extracted amplitude approaches this closed form
\emph{monotonically from below} as the fitting window advances---a $1/n^{2}$
finite-order deficit whose Richardson limit is $1$ to $\sim\!4\times10^{-4}$---and
an integer-relation search excludes any algebraic factor but $1$
\textbf{[SYMBOLIC + MEASURED at $(c_{1},b)=(0.2,0.5),\,(0.1,0.5),\,(0.3,0.5),\,(0.2,0.7)$;
\texttt{tests/test\_paper61\_stokes\_amplitude.py}]}.  This is the two-scale analog
of the algebraic-Stokes closure of $N(D)$ in Paper~59, \emph{on the small-$b$ side
of a Stokes horizon}:\ $z^{*}$ is the dominant Borel singularity only while
$|z^{*}|=c_{1}+b^{2}$ stays below the source-independent pure-scale action, and for
$b\gtrsim\sqrt{3c_{1}}$---bracketed by the convergent $b=0.7$ ($|z^{*}|=0.69$) and the
factorially divergent $b=1.0$ ($|z^{*}|=1.2$) extractions at $c_{1}=0.2$;\ the competing
distance reads naturally as the pure-scale $|z|\approx4c_{1}$ but is not cleanly
separated here---a nearer singularity dominates and the single-$z^{*}$ amplitude no
longer controls the leading large-order growth.  The median-Borel resummation across
the $z^{*}$ pair is therefore unambiguous only below that horizon.  \textbf{[OPEN]} The residual step to the finite value is the
integration of this algebraic-Stokes trans-series over the Feynman family (the inner scale) and the
modulus---the concrete form the two-scale Broadhurst--Dorigoni resummation takes here.  That
integration sweeps $b$ across the $\sqrt{3c_{1}}$ horizon above, so it is a \emph{two-singularity}
resummation---the source-and-scale $z^{*}$ pair together with the pure-scale action---not the
single-$z^{*}$ closure the small-$b$ regime alone would suggest.  Brute
