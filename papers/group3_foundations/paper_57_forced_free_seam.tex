...
\begin{enumerate}\setlength{\itemsep}{1pt}
  ...
  \item \textbf{The B5 correspondence} (MEASURED, order-invariant
    level):\ $\mathcal{C}(M_2(\C))$ and $\mathcal{C}(\HH)$ have
    \emph{identical} order invariants (height-$1$ fan;\ one point
    stratum plus one $2$-parameter family of maximal elements), while
    $\mathcal{C}(M_3(\C))$ differs (height $2$).  The site is blind
    exactly at the catalogue's lone admitted-not-forced entry (B5,
    $\HH$ vs $M_2(\C)$, Door~4c) and sighted exactly at $k = 3$ where
    the Hurwitz fallback forces $M_3(\C)$:\ the catalogue's third
    value corresponds to the degenerate dimension of site
    reconstruction, decided without consulting any witness derivation.
...
and the valuation-freedom dimensions --- all decided from site
properties without locating witnesses (the non-circularity P5 could
not achieve).  The multi-focal family (Family~1, $17$ entries) is formalized at
\emph{mechanism} level on both corpus-faithful instances (item~5:\ the
mixed-exponent frame pair;\ item~6:\ the two-center pair, confirming
the pre-registered $m$-grading prediction);\ the extension to the full
$17$-entry census and a general-parameter form of the vanishing lemma
remain open follow-ons, as are the full $60$-entry classification and
the relation to general site-reconstruction theorems at the
exceptional dimension $k = 2$ (literature verification pending).  Nothing here
touches the Observation status of the combination rule $K$:\ its
\emph{value} classifies as an external valuation, consistent with and
not upgrading that label.
