...
\begin{itemize}
  ...
  \item \S\ref{sec:principles} --- formal probe of five candidate discriminators (Sprint C2 finding:  packing-reachability is the operational principle at 98.3\% accuracy).
...
\textbf{Verdict.}  98.3\% accuracy across the 60-entry catalogue (59 correct, 1 ambiguous on an entry already structurally flagged). P5 catches both the multi-focal composition family and the inner-factor input data family. The single misclassification is a falsifiable prediction:  if the Hopf-base identification is established, I3 graduates from calibration to forced and P5's accuracy goes to 100\%.
...
\begin{table}[h]
\centering
\caption{Candidate principles after Sprint C2.  Accuracy is true-positive + true-negative over total entries on the 60-entry catalogue.  ``Family 1'' = multi-focal composition (17 calibration entries); ``Family 2'' = inner-factor input data (7 calibration entries).}
...
P5 (packing-reachability) is the operational principle. The 98.3\% accuracy on the 60-entry catalogue, with a single misclassification on the one entry currently flagged as structurally open, is the strongest available verdict at sprint scale.
...
The honest interpretation:  the forced/free seam IS the packing-reachable / packing-unreachable boundary.  Whether P5 is the "right" principle in a deeper sense — whether it admits a structural derivation lifting it from observation to theorem — is the open question of \S\ref{sec:open}.

%====================================================================
\section{Open questions}
\label{sec:open}

\subsection{Is P5 the principle, or tautological?}
\label{sec:open_family2}

Sprint C2 (\S\ref{sec:p5}) identified packing-reachability as the 98.3\%-accurate surface principle.  The honest question this leaves open is whether P5 is the structural principle or whether it is definitionally equivalent to the F/C label.

\paragraph{The tautology concern.} The packing-reachable tag for each entry was determined by examining the corpus for witness derivations (forced entries) and explicit non-selection theorems / negative-search memos (calibration entries).  If "packing-reachable" is operationally just "we have a derivation," then P5 = F/C label by construction, and the 98.3\% accuracy is a tautology.

\paragraph{Three reasons to think P5 is non-trivial.}
\begin{enumerate}
  \item \textbf{The tagging procedure works on new entries.}  For any candidate physical input not yet in the catalogue, the question "does a witness derivation from $\{$Paper~0 + standard NCG + Upgrade~B$\}$ exist?" is well-defined and empirically determinable.  The procedure does not require a pre-existing F/C label;  it produces one.
  \item \textbf{Predictive content.}  P5 makes specific predictions:  I3 graduates to forced if the Hopf-base identification of the Higgs direction is established;  a sibling-axiom direction emitting $\Ngen$ would re-classify F2 and other family-2 entries under the augmented axiom set;  any new physical observation (precision-physics result, new SM extension, candidate dark-sector parameter) admits an a~priori F/C prediction.
  \item \textbf{The failure-mode decomposition matches independent structural arguments.}  The two families that P5's failure modes decompose into are exactly the families named in independent prior work --- the multi-focal-composition wall pattern from the Sprint HF cluster, and the inner-factor input-data tier from the $\eta$-trivialization theorem in Paper~18~\S IV.6.  P5 was not constructed to match these;  it inherits the split as a corollary.
\end{enumerate}

\paragraph{The remaining open question.} Whether P5 graduates from observation to theorem.  A meta-theorem of the form ``any observable on the GeoVac spectral triple is forced iff packing-reachable" would establish P5 as a structural truth, not merely an empirical generalization.  Such a theorem is multi-year, NCG-research-grade, and named here as the sharpest follow-on of the Sprint C2 finding.
...
\begin{enumerate}
  \item \textbf{Forced-Count Theorem} (Paper~32~\texttt{thm:forced\_count}):  $\dim \mathcal{M}(D_F) = 128$ per generation;  the moduli space and its dimension are forced.
