...
\begin{enumerate}\setlength{\itemsep}{1pt}\setcounter{enumi}{6}
  \item \textbf{No-single-mechanism $K = \pi(B + F - \Delta)$} (Paper~32~\texttt{thm:no\_single\_mechanism\_K}, Sprint E6):  no morphism in $\mathcal{A}$ produces the empirical combination rule for $\alpha^{-1}$ as a single-mechanism output;  three spectral homes live in two distinct Mellin sub-rings;  twelve mechanisms empirically eliminated (Phases 4B--4I + Sprint A $\alpha$-SP + Sprint K-CC).  Combination rule preserved as conjectural per CLAUDE.md \S 13.5.
