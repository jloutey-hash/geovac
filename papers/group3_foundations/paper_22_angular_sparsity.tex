...
\texttt{tests/test\_paper22\_density.py}: the global-$M_L$ $D$ via the
production Gaunt routine \texttt{casimir\_ci.\_gaunt\_ck}, and the
pair-diagonal $D_{\mathrm{pd}}$ realized by
\texttt{angular\_zero\_count} in
\texttt{geovac/nuclear/potential\_sparsity.py}.}
...
radial integral structure.  The ERI-density column is the pair-diagonal
$D_{\mathrm{pd}}$ (the density the composed pipeline realizes); the
global-$M_L$ Coulomb density $D$ is $\sim\!3\times$ larger (Theorem~3).}
\label{tab:sparsity}
\begin{ruledtabular}
\begin{tabular}{lccc}
...
\item The ERI density is $2.76\%$ for all potentials, matching the
  pair-diagonal $D_{\mathrm{pd}}$ column of Theorem~3 at $l_{\max}=2$
  (the global-$M_L$ density there is $D = 8.52\%$).
\end{itemize}
