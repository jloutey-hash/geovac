...
density $D(l_{\max})$ under the Coulomb selection rule (conservation
of total magnetic quantum number, $m_a+m_b=m_c+m_d$): $14.84\%$ at
$l_{\max}=1$, $8.52\%$ at $l_{\max}=2$, $6.06\%$ at $l_{\max}=3$,
$4.83\%$ at $l_{\max}=4$, and $3.99\%$ at $l_{\max}=5$.  At
$l_{\max}=2$, $91.48\%$ of all possible two-body matrix elements are
guaranteed to vanish by the Wigner--Eckart theorem before any
property of the potential is invoked.  A stricter pair-diagonal
restriction ($m_a=m_c$, $m_b=m_d$), applicable to axially-symmetric
or $m$-decoupled bases, gives a further reduction ($2.76\%$ at
$l_{\max}=2$, $1.44\%$ at $l_{\max}=3$).  The bound is independent of the number of radial quantum
...
feature of the GeoVac framework---namely the convergence of the
one-body graph Laplacian spectrum to the pure integers of the
continuum $S^3$ Laplace--Beltrami operator---is
...
\medskip
\noindent\textbf{Theorem 2} (Potential-Independent Sparsity).\label{sec:theorem}
...
\toprule
$l_{\max}$ & Shells & $N_{\mathrm{orb}}$ & $N_{\mathrm{orb}}^4$ & $D(l_{\max})$ & $D_{\mathrm{pd}}(l_{\max})$ \\
\midrule
...
\textit{$D(l_{\max})$ is the physical Coulomb density (conservation
of total magnetic quantum number $M_L = m_a + m_b = m_c + m_d$);
$D_{\mathrm{pd}}$ is the stricter pair-diagonal restriction
($m_a = m_c$, $m_b = m_d$ per pair) that holds only for
axially-symmetric or $m$-decoupled bases.  These values are obtained
by exact enumeration of the $N_{\mathrm{orb}}^4$ four-index
combinations, applying the conditions of the definition above.  Both
conventions are verified by exact enumeration in
\texttt{tests/test\_paper22\_density.py}, both via the production Gaunt
routine \texttt{casimir\_ci.\_gaunt\_ck}.  The production enumerator
\texttt{angular\_zero\_count} in
\texttt{geovac/nuclear/potential\_sparsity.py} computes the global-$M_L$
$D$; until the 2026-08-29 wrong-sign-$q$ fix it silently returned
$D_{\mathrm{pd}}$ instead, because the buggy coefficient zeroed every
$m$-changing multipole and made the global constraint redundant.}
...
radial integral structure.  The ERI-density column is the physical
global-$M_L$ Coulomb density $D$, which is what the composed pipeline
realizes.  \emph{Re-measured 2026-08-30}:\ this table previously read
$97.24 / 0.00 / 2.76$, the stricter pair-diagonal $D_{\mathrm{pd}}$,
because the production enumerator collapsed to it before the
2026-08-29 wrong-sign-$q$ fix (Theorem~3 note).}
\label{tab:sparsity}
\begin{ruledtabular}
\begin{tabular}{lccc}
Potential & Angular & Radial & ERI \\
          & zeros (\%) & zeros (\%) & density (\%) \\
\midrule
Coulomb        & 91.48 & 0.00 & 8.52 \\
Harmonic osc.  & 91.48 & 0.00 & 8.52 \\
Woods--Saxon   & 91.48 & 0.00 & 8.52 \\
Square well    & 91.48 & 0.00 & 8.52 \\
Yukawa\footnotemark[1] & 91.76 & 0.00 & 8.24 \\
\end{tabular}
\footnotetext[1]{Yukawa binds only $17$ of the $18$ states at the tested
screening parameters, so its row is not a matched-orbital-count
comparison; the density shown is its own-count value.  The angular
\emph{pattern} is still bit-identical on the shared orbitals --- this is
why the invariance test groups by orbital count
(\texttt{tests/test\_nuclear\_sparsity.py}).  The previous table
silently listed Yukawa at $18$.}
\end{ruledtabular}
\end{table}
...
\item Radial zeros contribute exactly $0.00\%$ additional sparsity.
  ...
  potentials tested.
\item The ERI density is $8.52\%$ for the four potentials that bind
  all $18$ states, matching the global-$M_L$ $D$ column of Theorem~3 at
  $l_{\max}=2$ exactly.  (The stricter pair-diagonal $D_{\mathrm{pd}}$
  there is $2.76\%$; the table reported that value until 2026-08-30, for
  the reason given in its caption.)
\end{itemize}
...
cutoff $l_{\max}$.  At $l_{\max} = 2$, at most $8.52\%$ of all
possible two-body terms are nonzero; at $l_{\max}=3$, at most
$6.06\%$.  This bound applies identically to:
...
\subsection{Composed architectures: exactly $Q$-linear Pauli counts
for arbitrary spherical potentials}
...
of Theorem~2.  The resulting Pauli count measured across the
composed molecular library is exactly linear in the qubit count at
fixed basis, $N_{\mathrm{Pauli}} = 27.90 \times Q$ (main group;
$30.03$ for $d$-block systems)~\cite{GeoVac_Paper14}, with the
coefficient exact rather than fitted (the retired pair-diagonal
rule gave $O(Q^{2.5})$ with a fitted exponent; corrected 2026-08-29).

\medskip
\noindent\textbf{Corollary 1} (Potential-Independent
...
the block decomposition, the exactly $Q$-linear Pauli count
$N_{\mathrm{Pauli}} = 27.90 \times Q$ observed for
the Coulomb interaction in
Ref.~\cite{GeoVac_Paper14} reflects the set of surviving ERIs, which
is potential-independent (Theorem~2): the radial potential affects
only the magnitudes of the surviving ERIs, not which ERIs survive.
The coefficient $27.90$ ($= 279/10$, exact rather than fitted) is
the value for the Coulomb composed library and is expected, but not
here proven, to carry over to other central interactions.}
...
number of terms and the linear coefficient match.  \hfill$\square$

This corollary means that the angular selection-rule sparsity
\emph{pattern} of the GeoVac composed architecture---the density that
yields the $54\times$--$317\times$ fewer Pauli terms measured at
equal qubit counts against published Gaussian baselines for
electronic structure (H$_2$O, $n_{\max} = 1$--$3$; the retired
pair-diagonal rule gave $51\times$--$1{,}712\times$)---is
present for nuclear shell model Hamiltonians, quantum dot
Hamiltonians, and any other composed block architecture built from
spherical harmonic bases.  The specific $54\times$--$317\times$
ratios are measured Coulomb-vs-Gaussian numbers, expected but
not here proven to carry over quantitatively.  The angular selection
rule structure is a computational resource that belongs to the
geometry of angular momentum, not to the Coulomb potential.

\subsection{Z$_2$ tapering from Gaunt parity}
\label{sec:ell_parity_consequence}

The Gaunt parity selection rule
$(\ell_a + \ell_c + k)$ even AND $(\ell_b + \ell_d + k)$ even
in Theorem~2 forces $\ell_a + \ell_b + \ell_c + \ell_d$ to be even
on every nonzero ERI element.  Therefore the operator
$P_\ell^{(i)} = (-1)^{N_{\ell\text{-odd}}^{(i)}}$, counting electrons
in $\ell$-odd orbitals of sub-block $i$, commutes with the two-body
Coulomb sector exactly, providing a $\mathbb{Z}_2$ qubit-tapering
stabiliser independent of the Hopf $m_\ell \to -m_\ell$ reflection.
Per-sub-block, this saves $\Delta Q = n_{\rm sb}$ qubits beyond the
v3.52.0 Hopf-only tapering and reduces the Pauli term count by
$\sim 3\%$ across the production hydride library, verified bit-exact
across LiH ($+3$), HF ($+6$), BeH$_2$ ($+5$), H$_2$O ($+7$), NH$_3$
($+8$), CH$_4$ ($+9$).  The relativistic jj-coupled analog has no
such parity protection (the half-integer-$j$ sum is not forced
even by jj-coupled selection rules), structurally explaining why
the relativistic Tier 2 chemistry construction admits no direct
$m_j$-parity $\mathbb{Z}_2$ tapering analog.  Production:\
\texttt{geovac/extended\_tapering.py} (v3.89.0,
\texttt{tapered='extended'}); detailed analysis and panel data in
Paper~14 (the $\ell$-parity-tapering and
Gaunt-parity-protection sections).
...
\item Reduction from the generic single-center $O(Q^4)$ two-body
  term count to the exactly $Q$-linear composed Pauli count
  (Corollary~1; the retired pair-diagonal rule gave $O(Q^{2.5})$).
...
The explicit angular ERI densities (physical Coulomb selection rule,
global $M_L$ conservation) obtained by exact enumeration are
$14.84\%$ at $l_{\max}=1$, $8.52\%$ at $l_{\max}=2$, $6.06\%$ at
$l_{\max}=3$, $4.83\%$ at $l_{\max}=4$, and $3.99\%$ at $l_{\max}=5$.
...
The universal--specific partition is sharp.  Angular sparsity and
radial--angular factorization are universal across spherical fermion
systems;\ the exactly $Q$-linear Pauli count of composed block
architectures ($N_{\mathrm{Pauli}} = 27.90 \times Q$ at fixed basis)
is expected, but not here proven, to carry over beyond
the Coulomb composed library (Corollary~1).  The
$S^3$ Fock projection, the $\pi$-free graph Laplacian,
...
(Dyall--Faegri~\cite{Dyall}, Grant~\cite{Grant}),
\[
...
give quantitatively similar ratios; both scalar conventions now
appear in Paper~22 \S\ref{sec:theorem} ($D$ = full-Gaunt physical
density, $D_{\mathrm{pd}}$ = pair-diagonal), and the
$d_{\mathrm{sc}}^{\mathrm{FG}}$ column here reproduces the headline
$D(l_{\max})$ bit-exactly. All densities are exact rationals
...
$Q_{\mathrm{spinor}} = 2(l_{\max}+1)^2$. Both conventions are
tabulated in Paper~22 \S\ref{sec:theorem}: the full-Gaunt convention
is the physically correct Coulomb selection rule (the headline
$D(l_{\max})$), and the pair-diagonal-$m$ convention is the stricter
axial sub-case ($D_{\mathrm{pd}}$).}
\label{tab:d_spinor}
\begin{ruledtabular}
...
Paper~22 \S\ref{sec:universal} identified angular sparsity
...
Both spinor columns of Table~\ref{tab:d_spinor} are pinned to exact
nonzero counts and 4-decimal percentages by the regression test
\texttt{tests/test\_paper22\_spinor\_density.py}, which re-enumerates
the $jj$-coupled selection rule in exact \texttt{sympy} arithmetic over
the full single shell ($l_{\max}=0,\dots,5$; the four heaviest tuples
are \texttt{@slow}-marked). All selection-rule decisions are sympy
symbolic (\texttt{wigner\_3j(...) == 0} is a symbolic boolean); no
floating point enters the zero/nonzero decision. The discovery-mode
driver (Dirac-on-$S^3$ Tier 2 Track T0) is archived at
\texttt{debug/archive/spectral\_triple\_arc/tier2\_t0\_spinor\_density.py},
and its data dump --- integer numerators and denominators for every
density --- remains at
\texttt{debug/data/tier2\_t0\_spinor\_density.json}.

The full-Gaunt scalar column $d_{\mathrm{sc}}^{\mathrm{FG}}$ of
Table~\ref{tab:d_spinor} reproduces the headline density $D(l_{\max})$
pinned by \texttt{tests/test\_paper22\_density.py} bit-exactly, so the
scalar/spinor boundary is mutually consistent across the two tests and
the selection-rule implementation is validated against the existing
reference.
...
\textbf{Numerical result: the angular sparsity bound extends to rank-2 (verified at $Z=4$, $l_{\max} \le 5$; formal proof deferred).}
...
The proposed generalized statement (numerically supported across the
cases below; formal proof deferred): \emph{ERI density in the $S^3$
...
basis is deferred to a formal write-up.
...
content for cross-$n$ (see Paper~18 for the
...
The $Z=4$ rank-0/rank-1/rank-2 density table above (rows $l_{\max} \le 3$)
is regression-pinned in \texttt{tests/test\_paper22\_breit\_angular.py},
which recomputes the Coulomb, spin-other-orbit, and spin-spin angular
densities from the Wigner-$3j$ selection rules and checks the
potential-independence (kernel-truncation invariance) of the rank-0
pattern.
...
\bibitem{Dyall}
K.~G.~Dyall and K.~Faegri~Jr.,
\textit{Introduction to Relativistic Quantum Chemistry}
(Oxford University Press, 2007).

\bibitem{Grant}
I.~P.~Grant,
\textit{Relativistic Quantum Theory of Atoms and Molecules}
(Springer, 2007).
