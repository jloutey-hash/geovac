...
\textbf{[SYMBOLIC PROOF]} construction yields per-shell angular capacities $2k-1$ for shell~$k$, which is
...
\textbf{[OBSERVATION]} forced by the axioms alone.  Similarly, a factor of~2 arises from the
...
\textbf{[PANEL-VERIFIED]} sector constructed here recurs as a universal structural factor inside
...
imposed by hand: it follows from the symmetry each geometry retains
...
\textbf{[SYMBOLIC PROOF]} where an audit of 18~symbolic checks validates the conformal geometry
...
\item[$\times$] Time evolution and dynamics.  These require the
  Hamiltonian as an operator, developed in Paper~6~\cite{paper6}.
