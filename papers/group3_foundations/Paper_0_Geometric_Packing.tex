...
ground-state eigenvalue spectrally converges to the hydrogen
ground-state energy, as shown in subsequent papers; the full per-shell
spectrum $E_n = -Z^2/(2n^2)$ requires an additional node-weight term
(Paper~18~\S\,\textsc{iii}).  This angular cross-section recurs as a universal
...
Paper~15~\cite{paper15} recovers 96.0\% of the H$_2$ dissociation
...
units.  This constant is derivable from the Fock projection
(Paper~7~\cite{paper7}): the squared coupling between adjacent
$n$-shells in the Gegenbauer eigenbasis of the $S^3$ Laplacian is
$c^2(n,l) = \tfrac{1}{16}[1 - l(l+1)/(n(n+1))]$, which equals
$1/16$ universally for all $l = 0$ (s-wave) transitions.
Equivalently, $1/16 = 1/\Omega^4(0)$ where $\Omega(0) = 2$ is the
stereographic conformal factor at the projection center.  The packing
construction fixes the graph topology; the Fock projection then
determines the kinetic scale without any fitting.
