...
\textbf{[OBSERVATION]} number~$n$ corresponds to a cumulative grouping of the first~$n$
...
$n$ or $m$, never $\ell$), and \textbf{[MEASURED]} its top eigenvalue
saturates the bipartite bound $\lambda_{\max} \to 2 d_{\max} = 8$
...
failed is the spectral bound, not the target the scale is matched to; the full per-shell
spectrum $E_n = -Z^2/(2n^2)$ requires an additional node-weight term
(Paper~18~\cite{paper18}~\S\,\textsc{iii}).  \textbf{[OBSERVATION]} This angular cross-section recurs as a universal
...
bundles for multi-electron molecules---making it a foundational
...
\textbf{[OBSERVATION]} doubling.  The identification of this doubling with
...
\textbf{[SYMBOLIC PROOF]} This is not a coincidence that must be checked case by case: it is an
...
\textbf{[SYMBOLIC PROOF]} construction therefore generates the complete angular momentum basis
...
\textbf{[OBSERVATION]} The identification of this $\mathbb{Z}_2$ doubling with the
...
\caption{The $(\ell, m)$ angular cross-section inside each natural
  \textbf{[OBSERVATION]} geometry of the GeoVac series.  In every case, the full coordinate
...
Paper~7~\cite{paper7} shows that the continuum limit of the graph
Laplacian on the $2n^2$ packing is conformally equivalent to this
$S^3$ operator (the convergence itself is only indicated numerically, not established,
through $n_{\max} = 30$).
...
\textbf{[OBSERVATION]} The invariance of the angular sector across all five levels is not
imposed by hand: it tracks the symmetry each geometry retains
($m$ exactly wherever axial symmetry survives; $\ell$ exactly
wherever the full SO(3) does---the two-center problem breaks SO(3)
to axial SO(2), replacing $\ell$ by a separation index, as \S V
records).  The packing construction provides the combinatorial
...
\textbf{[OBSERVATION]} comment.  The same discrete quantum number skeleton
...
The conformal equivalences between the continuum limit of the graph
Laplacian and each continuum operator are established case by case:
\textbf{[SYMBOLIC PROOF]} Paper~7~\cite{paper7} provides 18~symbolic proofs of the Fock
projection chain for the $S^3$ embedding (the graph-to-continuum
convergence itself is only indicated numerically, through $n_{\max} = 30$,
and is not established), and the
subsequent papers verify the angular-sector recurrences and benchmark
each geometry in the hierarchy numerically.
...
isospectral).  \textbf{[OBSERVATION]} The present result is stronger in a specific sense:
the geometries in Table~\ref{tab:cross-section} share a labeled node
structure---quantum number assignments and per-shell degeneracies,
with the $(\ell, m)$ selection rules shared exactly wherever the
corresponding symmetry survives ($m$ in every row; $\ell$ in every
row except the two-center Level~2, where SO(3) breaks to axial
SO(2)).  The spectra themselves differ across rows; what is shared is
...
Laplacian decomposes into independent $\ell$-blocks ($m$ is mixed within a
block by the azimuthal edges; the production graph has no $\ell$-changing
edges --- corrected 2026-09-03 from ``$(\ell, m)$ blocks'').  This
...
units.  \textbf{[OBSERVATION]} This constant's value $1/16$ \emph{coincides} with a conformal-geometry
...
\textbf{[MEASURED]} The edges change $n$ (radial, $T_\pm$) or $m$
...
\begin{equation}
  ...
  \;=\; \frac{C}{n_{\max}^{2}} + o\bigl(n_{\max}^{-2}\bigr),
  \qquad
  C \;=\; \frac{\pi^{2}}{4}\bigl(2 + 2^{1/3}\bigr)^{2}\bigl(1 + 2^{-2/3}\bigr)
  \;=\; 42.7397\ldots,
  \label{eq:saturation_rate}
\end{equation}
attained at $\ell/n_{\max} \to 1 - 2/(2 + 2^{1/3}) = 0.386488$ (measured argmax $\ell = 11$ at $n_{\max} = 30$, $27$ at $70$).  Every finite sample lies \emph{below} $C$ (verified at all $591$
integer cutoffs $10 \le n_{\max} \le 600$) --- $C_{n} = 42.61$ at
$n_{\max} = 320$, $42.70$ at $1000$, $42.73$ at $4000$ --- so a finite sample
always understates $C$.  The approach is \emph{not} monotone, however:\ there
are $195$ decreasing steps in that range, e.g.\ $C_{20} = 40.7285 >
C_{21} = 40.6470$ (corrected 2026-09-04;\ ``monotone from below'' was asserted
from a doubling grid that happens to be monotone).  It is the one-sided bound,
not monotonicity, that makes every sample an understatement, and the value printed here before 2026-09-03, $42.6$, was the $n_{\max} = 320$ sample rather than the limit (\texttt{tests/test\_paper1\_block\_spectrum.py}).  
...
The conformal equivalence between the continuum limit of this graph
Laplacian and the Laplace--Beltrami operator on the unit~$S^3$ (via
Fock's stereographic projection) is developed in Paper~7~\cite{paper7},
\textbf{[SYMBOLIC PROOF]} where an audit of 18~symbolic checks (several limiting-case or definitional) validates the conformal geometry
(the convergence itself is only indicated numerically, not established, through
$n_{\max}=30$; the rate is a separate result).  The energy
eigenvalues and dynamics built on this graph are developed in
Paper~1~\cite{paper1} and the archived Paper~6~\cite{paper6}.
...
\item[$\times$] Energy eigenvalues $E_n = -1/(2n^2)$.  These require
  the graph Laplacian together with the matched kinetic scale
  $K_{\mathrm{vac}}$, whose value is an Observation rather than a
...
\item[$\times$] Time evolution and dynamics.  These require the
  Hamiltonian as an operator, developed in the archived Paper~6~\cite{paper6}.
...
by de~Broglie as an integer number of wavelengths around the orbit).
...
shows that the continuum limit of this graph Laplacian is conformally
equivalent to the
Laplace--Beltrami operator on~$S^3$ (the convergence itself is only
\emph{indicated} numerically through $n_{\max} = 30$, not established
--- corrected 2026-09-04 to match Paper~7 and \S VI above, both of which
already stated it correctly), with the $1/r$ Coulomb potential
...
\textbf{[MEASURED]} LiH, and BeH$^+$, with a saturation deficit of $0.57\%$ for the hydrogen ground-state
bound on the production graph at $n_{\max} = 30$ (a by-construction
quantity, \S VI; $0.11\%$ at $n_{\max} = 70$,
\texttt{tests/test\_paper7\_graph\_convergence.py}; the continuum chain
itself is exact) and $5.3\%$ for the LiH equilibrium
geometry (BeH$^+$ is markedly worse: $R_{\rm eq}$ 3.25 vs ${\sim}\,2.48$ bohr with the ab-initio
pseudopotential; HeH$^+$ is treated at a fixed bond length and recovers
93\% of $D_e$ adiabatically but only ${\sim}\,5\%$ in the consistent
2D-variational solver; Paper~17), benchmarked against
