...
as a graph, its Laplacian decomposes into $(\ell, m)$ sectors whose
\textbf{[MEASURED]} ground-state eigenvalue reaches the hydrogen ground-state energy under
a single matched scale $K_{\mathrm{vac}} = -1/16$, as shown in
subsequent papers---matched, not derived: the content that could have
failed is the spectral bound $\lambda_{\max} \to 8$, not the target the
scale is matched to; the full per-shell
spectrum $E_n = -Z^2/(2n^2)$ requires an additional node-weight term
(Paper~18~\cite{paper18}~\S\,\textsc{iii}).  \textbf{[PANEL-VERIFIED]} This angular cross-section recurs as a universal
...
\caption{The $(\ell, m)$ angular cross-section inside each natural
  \textbf{[PANEL-VERIFIED]} geometry of the GeoVac series.  In every case, the full coordinate
...
$S^3$ operator (the convergence itself is demonstrated empirically
through $n_{\max} = 30$).
...
\textbf{[PANEL-VERIFIED]} The invariance of the angular sector across all five levels is not
...
\textbf{[PANEL-VERIFIED]} comment.  The same discrete quantum number skeleton
...
convergence itself is empirical, through $n_{\max} = 30$), and the
subsequent papers verify operator convergence for each geometry in
the hierarchy.
...
\textbf{[PANEL-VERIFIED]} isospectral).  The present result is stronger in a specific sense:
...
\textbf{[SYMBOLIC PROOF]} This spin-doubled vertex count is verified by
\texttt{tests/test\_dirac\_lattice.py}.
...
$S^3$ Laplacian is $c^2(n,l) = \tfrac{1}{16}[1 - l(l+1)/(n(n+1))]$, equal
to $1/16$ for all $l = 0$ (s-wave) transitions, equivalently
$1/16 = 1/\Omega^4(0)$ with $\Omega(0) = 2$.  This numerical coincidence
...
As $n_{\max} \to \infty$, the largest Laplacian eigenvalue converges
to $\lambda_{\max} \to 8 = 2 d_{\max}$, twice the coordination number;
with $H = K_{\mathrm{vac}} L$ and $K_{\mathrm{vac}}$ fixed by the match
...
(the convergence itself is demonstrated empirically through
...
\textbf{[MEASURED]} LiH, and BeH$^+$, with errors ranging from ${<}\,0.1\%$ for hydrogen
to $5.3\%$ for the LiH equilibrium geometry (BeH$^+$ is markedly
worse: $R_{\rm eq}$ 3.25 vs ${\sim}\,2.48$ bohr with the ab-initio
