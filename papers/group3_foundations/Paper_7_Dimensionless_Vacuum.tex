...
At finite $n_{\max}$, the graph Laplacian exhibits discretization artifacts: the $2s$/$2p$ node-amplitude proxy reaches $37\%$ at $n_{\max} = 5$ on the binary production lattice (the splitting is strongly non-monotone: $12.7\%$, $0.65\%$, $15.6\%$, $5.1\%$ and $1.7\%$ at $n_{\max} = 6, 7, 8, 9, 10$, then $4.1\%$ at $20$ and $0.39\%$ at $30$; measured 2026-09-03), set by the residue of $n_{\max}$ modulo $3$ \cite{loutey_paper1};\ the ``differential connectivity between pole ($l = 0$) and equatorial ($l > 0$) nodes'' account printed here before 2026-09-04 is withdrawn on the same ground as the ``spectral aliasing'' account it restated --- the $l$-blocks are disconnected, so there is no connectivity between them to be differential. However, this splitting oscillates and decays systematically:
\begin{equation}
\Delta E_{2s\text{-}2p}(n_{\max}) \to 0 \quad \text{as} \quad n_{\max} \to \infty,
\end{equation}
recovering the exact Coulomb degeneracy.  \textbf{[INTERNAL THEOREM]} The rate is closed form and is governed by the residue of $n_{\max}$ modulo $3$, not by proximity to a continuum limit:\ $\lambda_{2s} = 3$ exactly when $n_{\max} \equiv 0 \ (\mathrm{mod}\ 3)$, where the ratio is $\bigl(2-2\cos\frac{\pi}{n_{\max}-1}\bigr)/3 = O(n_{\max}^{-2})$, while off that class it is $O(n_{\max}^{-1})$, so at neighbouring cutoffs the off-branch value is larger by a factor that \emph{grows without bound}, tracking $n_{\max}/(\sqrt{3}\,\pi)$ to within $1\%$:\ $6.6\times$ at $n_{\max} = 30$ ($1.65\%$ at $28$, $2.58\%$ at $29$ against $0.39\%$ at $30$), $23\times$ at $120$ and $552\times$ at $3000$.  \emph{The ``spectral aliasing on a compact manifold'' reading printed here before 2026-09-03 is withdrawn:} the $\ell$-blocks are disconnected, so no degeneracy is present to alias (Paper~1 \S III).  \textbf{This leg accordingly carries no evidential weight for the continuum reading}, which now rests on the $\lambda_{\max}$ saturation alone. \textbf{[MEASURED]} At $n_{\max} = 30$, the splitting has decayed to well under 1\%, continuing toward the exact Coulomb degeneracy.
...
(the saturation in (i) does have a proved closed-form rate;\ see
Limitations; the operator-system rate results of Paper~38~\cite{loutey_paper38} concern the truncated spectral triple, whose identification with this graph Laplacian is not established in this paper).
