...
\begin{abstract}
Fock showed in 1935 that the hydrogen Schr\"odinger equation is mathematically equivalent to the Laplace-Beltrami operator on the unit three-sphere $S^3$ via stereographic projection. We demonstrate that the GeoVac discrete graph Laplacian converges to this same operator in the continuum limit, establishing a precise mathematical bridge between the discrete graph formulation and the known continuous structure. The discrete paraboloid lattice---whose nodes encode quantum numbers $(n, l, m)$ and whose edges encode transition amplitudes---possesses a continuum limit that is conformally equivalent to the unit $S^3$. The continuum $S^3$ topology (the limit of the positive-semidefinite graph Laplacian) has pure integer eigenvalues $\lambda_n = -(n^2 - 1)$ with no intrinsic physical scale. The familiar Rydberg energy levels $E_n = -1/(2n^2)$ emerge only upon imposing an energy-shell constraint $p_0^2 = -2E$ that acts as a ``stereographic focal length,'' projecting the curved topological information into flat $\mathbb{R}^3$ observational coordinates. We verify these claims through a suite of 18 symbolic checks (an algebraic audit of the transformation chain, several of them limiting-case or definitional) confirming the algebraic geometry of the projection, the operator equivalence under conformal rescaling, and the eigenvalue-to-energy mapping.
\end{abstract}
...
\begin{enumerate}
\item \textbf{Discrete graph convergence.} We demonstrate that the GeoVac discrete paraboloid lattice---with nodes $(n, l, m)$ and edges generated by $L_\pm, T_\pm$ operators---converges in the continuum limit to the Laplace-Beltrami operator on unit $S^3$. This connects a concrete computational framework (sparse graph eigenvalue problems) to the known continuous structure.

\item \textbf{Explicit symbolic verification.} The 18 symbolic proofs (Appendix~\ref{app:tests}) provide a complete, machine-verified algebraic audit of the conformal transformation chain. While the underlying identities are known, this systematic verification in \texttt{sympy} is new and provides a reproducible computational check.

\item \textbf{Scale-invariance framing.} We articulate the ``dimensionless vacuum'' perspective: the observation that the conformal factor $\Omega$ always produces the \textit{unit} $S^3$ regardless of $p_0$, and that all physical dimensions enter exclusively through the energy-shell constraint. While implicit in Fock's original treatment, this framing---and its implications for the GeoVac computational approach---has not been stated explicitly in the literature.
...
\end{equation}
where $\kappa = -1/16$ is the kinetic scale factor---the constant that maps the dimensionless graph eigenvalues to the physical hydrogen spectrum (the scale matching the binary graph's $\lambda_{\max} \to 8$ to the ground-state energy $-\tfrac12$).  This matching value \emph{coincides} with two independent conformal-geometry quantities at $1/16$ (Section~\ref{sec:discussion}): the universal $s$-wave Fock coupling $c^2(n,0) = (1/4)^2$ and the inverse Fock Jacobian $1/\Omega^4(0)$ with $\Omega(0) = 2$.  We record this as an \emph{observation}: the matching $\kappa$ and the geometric $1/16$ agree numerically, but are computed from disjoint inputs and no mechanism is known that derives one from the other.
...
\end{equation}
recovering the exact Coulomb degeneracy at a rate consistent with spectral aliasing on a compact manifold. At $n_{\max} = 30$, the splitting has decayed to well under 1\%, continuing toward the exact Coulomb degeneracy.
...
$N = 1$ case.  The generalization to $N > 1$ is expected by the same
...
\end{enumerate}
We record this as an \emph{observation}, not a derivation:\ the
...
term in Paper~2's fine-structure-constant formula
$K = \pi(B + F - \Delta)$~\cite{loutey_paper2}.  This connects the
highest-$l$ Fock coupling at the Paper~2 cutoff to the Dirac
degeneracy $g_3^{\mathrm{Dirac}} = 40$ identified in Phase~4H
(verified by \texttt{tests/test\_trunk\_qa\_c2\_delta.py}, which reaches
$1/40$ by two independent routes).
...
The principle that the graph Laplacian should discretize the Laplace-Beltrami operator on the natural conformal manifold extends beyond single-center systems.  For diatomic molecules, the two-center Coulomb problem separates in prolate spheroidal coordinates $(\xi, \eta, \varphi)$, just as the single-center problem separates in spherical coordinates (leading to $S^3$ via Fock's projection).  Discretizing the Laplace-Beltrami operator on the prolate spheroid yields a molecular lattice with $R$-dependence built into the metric---no fitted parameters are required.  For H$_2^+$, this approach achieves $R_{\mathrm{eq}} = 2.001$~bohr (0.21\% error) and $E_{\min} = -0.598$~Ha (0.70\% error) with zero free parameters~\cite{loutey_paper11}.  This suggests a general principle: every separable quantum system has a natural lattice defined by the coordinate system in which separation occurs.
...
energy is $-2.8983$~Ha (0.19\% error vs.\ exact $-2.903724$~Ha),
with a convergence floor estimated at $\sim$0.14\%.
...
with the diagonal hydrogenic operator degrades the result to 1.41\%
...
$< 3 \times 10^{-15}$~Ha), confirming that the convergence floor
...
$E = -Z^2/2 + 5Z/8 = -11/8$~Ha for $Z = 2$, an exact rational.
The variationally optimized single-exponent result $k^* = 27/16$,
