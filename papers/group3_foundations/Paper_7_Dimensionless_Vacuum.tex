...
The central results of this paper are one spectral-bound statement (proved in closed form), one observation (its continuum reading) and two symbolic ones:
\begin{enumerate}
\item \textbf{[INTERNAL THEOREM]} The top Laplacian eigenvalue of the discrete graph saturates its bipartite bound, $\lambda_{\max} \to 2 d_{\max} = 8$, at the closed-form rate $(C + o(1))/n_{\max}^2$ (Paper~0~\S VI;\ measured through $n_{\max} = 30$ and $70$).  \textbf{[OBSERVATION]} We read this as a numerical indication of a continuum limit conformally equivalent to the \textit{unit} $S^3$;\ the operator convergence and the identification of the limit manifold are \emph{not} established here (see New Contributions item~1).  \emph{Corrected 2026-09-03/04:} this item previously asserted both as \textbf{[MEASURED]}, contradicting four other loci in this paper.
...
At finite $n_{\max}$, the graph Laplacian exhibits discretization artifacts: the $2s$/$2p$ node-amplitude proxy reaches $37\%$ at $n_{\max} = 5$ on the binary production lattice (the splitting is strongly non-monotone: $12.7\%$, $0.65\%$, $15.6\%$, $5.1\%$ and $1.7\%$ at $n_{\max} = 6, 7, 8, 9, 10$, then $4.1\%$ at $20$ and $0.39\%$ at $30$; measured 2026-09-03), \textbf{[INTERNAL THEOREM]} set by the residue of $n_{\max}$ modulo $3$ \cite{loutey_paper1};\ the ``differential connectivity between pole ($l = 0$) and equatorial ($l > 0$) nodes'' account printed here before 2026-09-04 is withdrawn [retracted 2026-09-04: sp-splitting-aliasing-mechanism] on the same ground as the ``spectral aliasing'' account --- the sectors' degrees do differ ($D_{2p} > D_{2s}$), but the $l$-blocks are disconnected components, so no operator acts across them to lift a degeneracy, and a degree difference is not a mechanism for the proxy. However, this splitting oscillates and decays systematically:
\begin{equation}
\Delta E_{2s\text{-}2p}(n_{\max}) \to 0 \quad \text{as} \quad n_{\max} \to \infty,
\end{equation}
the two proxy eigenvalues coinciding in the limit --- a path-graph coincidence, \emph{not} a reproduction of the continuum $SO(4)$ Coulomb degeneracy (the $\ell$-blocks are disconnected).  \textbf{[INTERNAL THEOREM]} The rate is closed form and is governed by the residue of $n_{\max}$ modulo $3$, not by proximity to a continuum limit:\ $\lambda_{2s} = 3$ exactly when $n_{\max} \equiv 0 \ (\mathrm{mod}\ 3)$, where the ratio is $\bigl(2-2\cos\frac{\pi}{n_{\max}-1}\bigr)/3 = O(n_{\max}^{-2})$, while off that class it is $O(n_{\max}^{-1})$, so at neighbouring cutoffs the off-branch value is generically larger --- asymptotically by $n_{\max}/(\sqrt{3}\,\pi)$ [retracted 2026-09-04: sp-ratio-false-precision], but far from that at small cutoff: $4.2\times$ and $6.6\times$ at $n_{\max} = 28, 29$ against $30$ ($1.65\%$, $2.58\%$ vs $0.39\%$), where the formula gives $5.5$;\ $20$--$23\times$ at $118, 119$ against $120$ ($22.1$);\ $550$--$552\times$ at $2998, 2999$ against $3000$ ($551.3$). Below $n_{\max} \approx 12$ the ordering can invert ($0.65\%$ at $7$ against $12.7\%$ at $6$) (Paper~1~\S III for the full table).  \emph{The ``spectral aliasing on a compact manifold'' reading printed here before 2026-09-03 is withdrawn [retracted 2026-09-04: sp-splitting-aliasing-mechanism]:} the $\ell$-blocks are disconnected, so no degeneracy is present to alias (Paper~1 \S III).  \textbf{This leg accordingly carries no evidential weight for the continuum reading}, which now rests on the $\lambda_{\max}$ saturation alone. \textbf{[MEASURED]} At $n_{\max} = 30$ the proxy is $0.39\%$, on the $n_{\max} \equiv 0 \pmod 3$ branch;\ at $28$ and $29$ it is $1.65\%$ and $2.58\%$.
...
\begin{equation}
  ...
  = \gvq{slater_f0_1s}{\frac{5Z}{8}} \,. \quad\checkmark
\end{equation}
...
Stages 2 and 3 are verified by independent symbolic computation: the 18 passing tests constitute an algebraic audit of the conformal transformation chain (ten of the eighteen are definitional, limiting-case or non-discriminating checks; the load-bearing identities are items 1, 6, 7, 8, 10, 11, 16 and 17), covering the unit sphere constraint, the conformal factor identities, the projection limits, the chordal distance identity, the dot-product form, the volume Jacobian, the inverse projection, the connection term, the conformal Laplacian of $\Omega^2$, the zero-mode annihilation, the eigenvalue equation for $n = 2$ and $n = 3$, the conformal decomposition, and the eigenvalue-to-energy mapping.  Stage 1---the graph-to-continuum convergence itself---is only indicated numerically through $n_{\max} = 30$ (spectral-bound saturation only, the $s/p$ leg having been withdrawn as evidence on 2026-09-03; the operator convergence itself is not established) with no rate proven here for \emph{that} convergence
(the saturation of New Contributions item~1(i) has a closed-form rate proved in Paper~0~\S VI;\ see Limitations, which distinguishes the two; the operator-system rate results of Paper~38~\cite{loutey_paper38} concern the truncated spectral triple, whose identification with this graph Laplacian is not established in this paper).
...
$1 - 12/20 = 2/5$, and $(2/5) \times 1/\Omega^4(0) = \gvq{delta_dirac}{1/40} = \Delta$, the
...
\textbf{[MEASURED]} energy is $-2.89746$~Ha (0.216\% error vs.\ exact $-2.903724$~Ha),
