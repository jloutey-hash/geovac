...
\begin{enumerate}
\item \textbf{Discrete graph convergence.} \textbf{[OBSERVATION]} We read the GeoVac discrete paraboloid lattice---with nodes $(n, l, m)$ and edges generated by $L_\pm, T_\pm$ operators---as a discretization whose continuum limit is the Laplace-Beltrami operator on unit $S^3$, connecting a concrete computational framework (sparse graph eigenvalue problems) to the known continuous structure. What is \textbf{[MEASURED]} through $n_{\max} = 30$ (\texttt{tests/test\_paper7\_graph\_convergence.py}; \texttt{tests/test\_trunk\_qa\_splitting.py}, Paper~1) is narrower than that reading: (i) the top Laplacian eigenvalue of the production lattice saturates the bipartite bound, $\lambda_{\max} \to 2 d_{\max} = 8$ (6.618 at $n_{\max} = 5$, 7.954 at 30), so that with the matched scale $\kappa = -1/16$ the extremal eigenvalue $\kappa\lambda_{\max}$ approaches $-1/2$ \emph{by construction}---the content is the saturation, not the target; (ii) the $2s/2p$ lift on the binary lattice decays, oscillating, to $0.39\%$ at $n_{\max} = 30$. Two measured facts bound the reading: the lattice's edges never change $l$, so $L$ splits into $n_{\max}$ blocks (one per $l$) with an $n_{\max}$-dimensional kernel of block constants, and the $\lambda_{\max}$ mode lives in a mid-$l$ block with negligible weight on the $1s$ node (the $l = 0$ block alone saturates only $\lambda_{\max} \to 4$); and the spectrum of $\kappa L$ is confined to $[-1/2, 0]$, so no level-by-level Rydberg ladder is reproduced. The operator convergence $L \to \Delta_{S^3}$ and the identification of the limit manifold are not established here (coverage gap; 2026-09-03 correction of a 2026-09-02 backing note that presented the matched target $-1/2$ as the measured quantity, reversing Paper~0's own ``matched, not derived'' scope).
...
The recovered degeneracy is the hallmark of $SO(4)$ symmetry---the isometry group of $S^3$. \textbf{[OBSERVATION]} A continuum limit carrying $SO(4)$ isometry is consistent with this, and $S^3$ is its natural candidate (up to discrete quotients); the identification is a reading, not a measurement---no test in the repository establishes the limit manifold, and the production graph is disconnected into $n_{\max}$ components (one per $l$), a structure the reading must eventually account for.
...
with \emph{zero free parameters} beyond the universal matched scale $\kappa$: no orbital exponent, no variational
