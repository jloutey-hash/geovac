...
\end{equation}
where $\kappa = -1/16$ is the kinetic scale factor---the unique constant that maps the dimensionless graph eigenvalues to the physical hydrogen spectrum.  As shown in Section~\ref{sec:discussion}, this value is derivable from the conformal geometry of the Fock projection: the $s$-wave Fock coupling is $c^2(n,0) = 1/16$ \emph{universally for all $n$} (the squared Chebyshev/Gegenbauer three-term recurrence amplitude $(1/4)^2$, independent of $p_0$), equivalently the inverse Fock Jacobian $1/\Omega^4(0)$ with $\Omega(0) = 2$ at the projection center.
...
dimensionful physics.  It is derivable from the Fock projection
itself.  The Gegenbauer eigenfunctions $C^1_{n-1}(\cos\chi)$ of
...
\end{enumerate}
Thus $\kappa$ is not a fitted constant: it is determined by the
conformal geometry of the $S^3$ projection.  Its universality across
all nuclear charges $Z$ (via $Z^2$ scaling) confirms that the graph
topology correctly encodes the \textit{relative} spectral structure.
