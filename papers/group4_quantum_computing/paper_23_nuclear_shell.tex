...
qubit encoding for the deuteron ($1p+1n$, $16$ qubits, $688$ non-identity
Pauli terms, $1$-norm $\approx 384$~MeV) using the Minnesota
nucleon--nucleon potential and Moshinsky--Talmi brackets, and extend the
architecture to helium-4 ($2p+2n$, $16$ qubits, $828$ Pauli terms) with
...
deuteron has $16$ qubits and $688$ Pauli terms, while He-4 has the
same $16$ qubits but only $828$ Pauli terms despite a $12.25\times$
...
to their determination. The recovered gaps are not uniform in
magnitude:\ within the physically meaningful region (cumulative
occupancy $\leq 126$; boundaries above this sit at the $n_{\max}=7$
truncation edge, where the missing $N=7$ intruder levels distort the
spectrum), the six lower magic numbers $2, 8, 20, 28, 50, 82$ are the
six \emph{largest} gaps, while $126$ appears as a clear but
non-dominant gap ($\approx 0.11\,\hbar\omega$), tied with the known
sub-shell boundary at $40$ and below several intermediate sub-shell
gaps ($14, 56, 104, 118$) --- the expected granularity of the
two-parameter Mayer--Jensen form at this $n_{\max}$ (gap structure
pinned by \texttt{tests/test\_paper23\_magic\_gaps.py}). Table~\ref{tab:levels}
...
structure of the two-body interaction tensor, and the exactly
$Q$-linear composed Pauli count that follows~\cite{Paper22}. For nuclear shell models,
...
%% =====================================================================
\section{The Deuteron: First Two-Species Nuclear Qubit Hamiltonian}
\label{sec:deuteron}
%% =====================================================================
...
term count is $688$, with a $1$-norm of $\approx 384$ MeV dominated by
the HO diagonal and the Minnesota spin-triplet contribution. Of the
$688$ terms, $80$ are $Z$-only (number-operator products) and the
remaining $608$ carry at least one $X$ or $Y$, reflecting the
...
the qubit Hamiltonian; the FCI validation is performed on the matrix
representation (the source of truth), while the Pauli-string export is
validated structurally, subject to the off-diagonal sign limitation
disclosed in Sec.~\ref{subsec:known_limitation}. Recent
...
is that the Pauli count grows by only $20\%$ ($688 \rightarrow 828$)
...
$\approx 512$ MeV (without Coulomb) and $\approx 507$ MeV (with
...
count on the full register is $710$ non-identity terms, of which
$688$ come from the nuclear block, $10$ from the electronic block
...
$\sim 5 \times 10^5$ Ha (after conversion from MeV to
...
leading-order term of the Pachucki--Patkó\v{s}--Yerokhin 2018
Foldy--Wouthuysen-reduced two-particle
Hamiltonian~\cite{Pachucki2018}.
...
cross-register $V_{eN}$ at $n_{\max} = 1$ matches Pachucki~2018
...
\begin{equation}
  ...
  = \frac{3}{2} \times \frac{4}{3}\,
    \frac{g_d^{\mathrm{atomic}}}{2}\, \alpha^2
    \left(\frac{m_e}{m_p}\right) \mathrm{Ha},
\label{eq:d-hfs-bf}
\end{equation}
...
factor $2I$); the divisor restoring $\mu_I/(I\mu_N)$ is $2$, not $2I$
--- the two coincide only at $I=1$.  The
mass factor is $m_e/m_p$ for every nucleus, because the nuclear
magneton $\mu_N = e\hbar/(2m_p)$ is defined with the proton mass.
With CODATA constants this yields $327.2350$~MHz vs the
Wineland--Ramsey~1972~\cite{WinelandRamsey1972} measurement of
$327.384352522(2)$~MHz, a residual of $-456$~ppm.  (Corrected
2026-08-28: this equation previously carried $m_e/m_d$ in place of
$m_e/m_p$ with no factor-of-$2$ divisor.  Since $m_d/m_p = 1.99900750 \approx 2$
the two errors nearly cancelled, leaving the baseline high by
$496.5$~ppm; $I=1/2$ nuclei were unaffected, so the H and T
cross-checks did not expose it.)

The leading-order Zemach correction with $r_Z(D) = 2.593$~fm
(Friar--Payne~1997~\cite{FriarPayne1997}) gives
...
Sec.~\ref{ssec:omega-magn}. The cumulative chain
(BF + recoil + Schwinger $a_e$ + leading Zemach) lands at $327.3153$~MHz
with $-211$~ppm residual against experiment.  The chain sits below the
Pachucki--Yerokhin / Karshenboim total theory ($327.339$~MHz), the
expected direction for a chain omitting the
...
The Bohr--Fermi residual of $-456$~ppm is the cleanest framework-native
...
at $-456$~ppm BF strict and $-211$~ppm full chain), Paper~34~\S V.C.X
...
\begin{itemize}
  \item $V_{eN}$: leading-order Bethe--Salpeter recoil verbatim
    ...
    Pachucki~2018). Multi-shell $n_{\max} \geq 2$ on both registers is
...
register is established in Paper~38~\cite{Paper38}, the van Suijlekom
state-space Gromov--Hausdorff convergence for the truncated
...
Pachucki~2018~\cite{Pachucki2018} and lift the basis-truncation
...
\begin{itemize}
  \item \textbf{Universal (applies to HO, Woods--Saxon, NN potentials).}
    ...
    integral; the angular-sparsity reduction of single-center Pauli
    scaling below the naive $O(Q^4)$ (measured $O(Q^{3.77})$ for
    Coulomb atoms --- a narrow margin under the exact global-$M_L$
    rule) and the exactly linear composed Pauli scaling in $Q$ at
    fixed basis; the
    block structure of the two-body tensor; the
...
\bibitem{Paper39}
J.~Loutey, ``GeoVac Paper 39: State-space Gromov--Hausdorff convergence
of truncated tensor-product Camporesi--Higuchi spectral triples on
$S^3 \times S^3$,'' technical report, Zenodo (2026).
...
\bibitem{FriarPayne1997}
J.~L.~Friar and G.~L.~Payne,
``Higher-order nuclear-size corrections in atomic hydrogen,''
\emph{Phys.~Rev.~A} \textbf{56}, 5173 (1997); deuteron Zemach radius
$r_Z(D) = 2.593(16)$~fm.
