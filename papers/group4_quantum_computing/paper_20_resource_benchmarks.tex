...
\caption{Quantum resource comparison: GeoVac vs.\ Gaussian raw JW.
  ...
  and re-price toward parity under the exact rule.  The Gaussian Pauli
  counts were computed independently via the OpenFermion/Qiskit-Nature
  pipeline (Trenev et al.~\cite{Trenev2025}---a vibrational-spectra
  resource study---is cited for the resource-estimation methodology and the
  $Q^{3.9\text{--}4.3}$ Gaussian-scaling range, not for these specific
  electronic-structure counts).  ``---'' indicates data not available.}
\label{tab:resources}
\begin{ruledtabular}
...
use the pair-diagonal ERI rule, Sec.~\ref{sec:eri_rule}; near parity under
the exact rule), with a nearly identical
...
molecular spec topology. Swain ratio = $N_{\mathrm{Pauli}}^{\mathrm{rel}} / 47\,099$,
the single calibrated Swain RaH-18q cell~\cite{Swain2022}.}
\label{tab:paper20_tier2}
\begin{ruledtabular}
\begin{tabular}{cccccccc}
Molecule & $n_{\max}$ & $Q$ & $N_{\mathrm{Pauli}}^{\mathrm{rel}}$ & $N_{\mathrm{Pauli}}^{\mathrm{sc}}$ & $\lambda_{\mathrm{ni}}$ (Ha) & QWC & Swain ratio \\
\hline
...
\hline\hline
\multicolumn{8}{l}{\footnotesize Swain RaH-18q~\cite{Swain2022}: 47\,099 Pauli (rel), 12\,556 Pauli (non-rel), $Q = 18$.} \\
...
\multicolumn{8}{l}{\footnotesize Swain et al.\ SI Tables S1--S3 (per-molecule matched-$Q$ data): accessibility \textbf{deferred}.} \\
\end{tabular}
\end{ruledtabular}
\end{table*}
...
\begin{itemize}
  \item At $n_{\max}=2$, all GeoVac molecules sit at
    $0.02$--$0.03\times$ the Swain RaH-18q Pauli count at native
  ...
  \item At $n_{\max}=3$, the absolute GeoVac Pauli count overtakes
    Swain et al.\ at matched $Q$ (LiH/BeH reach 1.9$\times$ of the RaH-18q
  ...
  \item The $\lambda_{\mathrm{ni}}$ column is non-inflated by
    ...
    favorable relative to Swain et al.'s DC-UCCSD 1-norm (not reported
    in Swain et al.'s main text).
...
\textbf{Deferred Swain et al.\ cells.} The per-molecule Pauli counts, 1-norm,
and QWC values for BeH, MgH, CaH, SrH, BaH at $Q = 18$ are reported
in Swain et al.~\cite{Swain2022} Supplemental Material Tables
...
Sprint 4 v2.15.0), which is $0.020\times$ Swain et al.'s RaH-18q ($50\times$
...
from Paper~14. Swain et al.'s DC-UCCSD is
