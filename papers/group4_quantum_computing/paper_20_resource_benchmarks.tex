...
The balanced coupled architecture (PK-free) binds LiH at its experimental
equilibrium geometry $R_{\mathrm{eq}} = 3.015$~bohr with 878 Pauli terms at
30 qubits, achieving 0.20\% single-point energy error at the well minimum
when solved exactly.  Chemistry-accuracy scope at $n_{\max}=2$ is
...
truncation-error bounds (Paper~38 propinquity), not energy-accuracy
parity with CCSD(T) on second-row systems.  The library covers 38 molecules across three rows of the periodic table
...
\begin{table}[h]
  ...
  \begin{tabular}{lcc}
    ...
    balanced (frozen-core, Path A) & yes ($R_{\mathrm{eq}}=3.015$) & no \\
    ...
    explicit-core HF (12 electrons NaH; 4 e LiH cross-check) & yes ($R_{\mathrm{eq}}=3.015$) & no \\
    \bottomrule
  \end{tabular}
\end{table}
...
1-norm bounds, and propinquity error bounds are correct in either case
...
triple~\cite{GeoVac_Paper38} (Latrémolière propinquity
$\Lambda(T_{n_{\max}}, T_{S^3}) \le C_3 \gamma_{n_{\max}}$ with
...
\paragraph{Propinquity-derived basis-truncation error bound.}
Every \texttt{GeoVacHamiltonian} returned by the public API
carries a metadata field \texttt{propinquity\_bound} that
evaluates the closed-form
$\Lambda \le C_3 \cdot \gamma_{n_{\max}}$ from
\cite{GeoVac_Paper38} (Paper~38, the SU(2)
propinquity convergence theorem) at the cutoff $n_{\max}$
...
commodity hardware); the per-sub-block Z$_2$ Hopf-U(1)
tapering of \cite{GeoVac_Paper14} (saving $\sim 12.6\%$ of
Pauli terms across the 37-molecule library) is a natural
intermediate but does not by itself close the $Q = 30$ scale.
...
(LiH composed/balanced and NaH balanced at production
...
to 190$\times$ fewer terms against cc-pVDZ.  For first-row hydrides
...
quantitative truncation-error bounds (Paper~38 propinquity, exposed as
