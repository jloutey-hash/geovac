...
across a range that depends strongly on the comparison:\ from
$0.28\times$ (H$_2$O against minimal-basis STO-3G, i.e.\ \emph{denser})
through $55$--$76\times$ against cc-pVDZ, to $317\times$ at the most
favourable equal-qubit point.  (Corrected 2026-08-30:\ the retired
pair-diagonal rule supported a blanket ``one to three orders of
magnitude''; it does not survive at either end.)
The Hamiltonians are constructed from
...
the composed LiH Hamiltonian is near parity with minimal-basis STO-3G on
Pauli count (\gvq{lih_composed_pauli}{838} vs.\ STO-3G's \gvq{lih_sto3g_raw}{907} raw; $3.0\times$ more than the 276
reduced) while using more qubits; the decisive resource advantages are the
exact linearity of the Pauli count in~$Q$ across molecules at fixed basis
and a 76$\times$ reduction versus cc-pVDZ.  (All counts corrected
2026-08-29 to the exact global-$M_L$ ERI rule; see
...
3.015~bohr) at $n_{\max}=2$ (\gvq{lih_balanced_pauli}{2{,}726} Pauli terms, 30 qubits); a single-point
...
truncation-convergence rates (Paper~38 state-space GH convergence), not energy-accuracy
parity with CCSD(T) on second-row systems.  The \texttt{hamiltonian()}
registry covers 37 systems (35 composed molecules spanning
$Z=1$--56, H through Ba, plus the He and H$_2$ reference atoms),
...
with the within-molecule basis exponent $3.17$ identical across all
three rows (small-basis fit, $n_{\max} \le 3$; local slope rising
toward $\sim 3.8$)---and, on the two-point $n_{\max}=1,2$ fit, an exponent
of $2.8163$ identical across all six molecules to the digits shown,
\emph{forced} by the exact $Q$-linearity at each basis rather than
fitted---compared to
$O(Q^{3.9\text{--}4.3})$ for Gaussian baselines.  The composed Pauli
count is exactly linear in qubit count (coefficient $\gvq{composed_coeff}{27.90}$, exact,
for main-group molecules; $30.03$ for $d$-orbital blocks --- both under
the exact global-$M_L$ ERI rule, Sec.~\ref{sec:eri_rule}),
...
The result is a Pauli count exactly linear in~$Q$ across molecules at
fixed basis (within-molecule basis exponent $3.17$), with
$54$--$317\times$ fewer terms than Gaussian raw
Jordan-Wigner baselines at comparable qubit counts~\cite{GeoVac_Paper14}
(exact-rule numbers, 2026-08-29).
...
op = h.to_qiskit()    # \gvq{lih_composed_pauli}{838} Pauli terms
...
\end{equation}
This produces the sparsest qubit operator (\gvq{lih_composed_pauli}{838} Pauli terms for LiH)
...
This produces a larger operator (\gvq{lih_balanced_pauli}{2{,}726} Pauli terms for LiH at
...
\caption{LiH balanced coupled FCI energies at $n_{\max}=2$
  ($Q=30$, \gvq{lih_balanced_pauli}{2{,}726} Pauli terms, analytical V$_{ne}$ integrals).
...
for composed with PK.  The balanced 1-norm ($323.4$~Ha) is 14\% lower
than composed with PK in-Hamiltonian ($374.9$~Ha; re-measured 2026-09-01
at BeH$_2$'s own $R = 2.502$~bohr --- the 2026-08-30 pass had evaluated
it at LiH's $3.015$, giving $328.1$ and ``12\%''
under the exact global-$M_L$ rule --- the retired figures were $306.4$ /
$373.4$ / ``18\%'', and an earlier $354.9$ came from the deprecated legacy
BeH$_2$ builder path; the live values are pinned by
\texttt{tests/test\_paper20\_library.py}),
demonstrating that the PK-free architecture is cheaper in the QPE
regime, where PK must be encoded; with PK partitioned classically (the
VQE default), the composed electronic-only 1-norm ($68.8$~Ha,
Table~\ref{tab:resources}) is far lower.

For H$_2$O ($Q=70$), FCI is infeasible (10 electrons), but the
Hamiltonian metrics are available: $19{,}742$ Pauli terms,
$1{,}612.0$~Ha 1-norm, $1{,}494$ QWC groups (re-measured 2026-08-30; the
retired pre-exact-rule figures were $5{,}798$ / $1{,}511$ / $168$, the
Pauli count low by $3.4\times$).  The balanced 1-norm is $0.057\times$ the
composed total (which is dominated by the PK barrier at $Z_{\text{eff}}=6$),
and $4.3\times$ the composed electronic-only 1-norm.
...
overestimates the experimental value (0.092~Ha) by 75\%.  Users
...
overbinding (the 0.20\% single-point / $D_e$ overestimated by
$\sim$75\% vs.\ experiment pattern documented in
...
1-norm bounds, and GH convergence rates are correct in either case
...
\caption{Quantum resource comparison: GeoVac vs.\ Gaussian raw JW.
  ...
  convention:} the LiH STO-3G count (\gvq{lih_sto3g_raw}{907}, $Q=12$) is the \emph{raw} JW
  Hamiltonian, matched against the raw (un-tapered) GeoVac composed count
  (838, $Q=30$); the larger-basis LiH counts and all H$_2$O Gaussian counts
  ...
  (qubit-reduced STO-3G LiH is 276 at $Q=10$ vs.\ GeoVac
  composed 838, i.e.\ $3.0\times$ \emph{more}); the raw-vs-raw and
  reduced-vs-reduced comparisons bracket
  the genuine position: near parity raw, unfavorable reduced.  The \emph{2-qubit-reduced} Gaussian Pauli counts
  (LiH 6-31G/cc-pVDZ and all H$_2$O entries) are the published values of
  Trenev et al.~\cite{Trenev2025} (Table~5, Appendix~B; Jordan--Wigner with
  2-qubit reduction; their reduced STO-3G LiH value is 276 at $Q=10$); the
  raw STO-3G LiH count (\gvq{lih_sto3g_raw}{907}, $Q=12$) used as the matched-raw market-test
  baseline is the raw-JW value, not a Trenev-reduced count.  The
  $Q^{3.9\text{--}4.3}$ Gaussian-scaling exponents are GeoVac's own log-log
  fit of the published reduced counts.
...
\colrule
\multirow{5}{*}{LiH}
 & GeoVac composed & 30 & 838 & 34.0 & 64 & --- \\
 & GeoVac balanced & 30 & \gvq{lih_balanced_pauli}{2{,}726} & 78.2 & 629 & --- \\
 & Gaussian STO-3G & 12 & \gvq{lih_sto3g_raw}{907} & 34.3 & 273 & $1.08\times$ Pauli, $4.3\times$ QWC \\
 & Gaussian 6-31G  & 20 & 5{,}851 & --- & --- & $7.0\times$ Pauli \\
 & Gaussian cc-pVDZ & 36 & \gvq{lih_ccpvdz_pauli}{63{,}519} & --- & --- & $76\times$ Pauli \\
\colrule
\multirow{4}{*}{H$_2$O}
 & GeoVac composed & 70 & 1{,}954 & 372 & 64 & --- \\
 & GeoVac balanced & 70 & 19{,}742 & 1{,}612 & 1{,}494 & --- \\
 & Gaussian STO-3G & 12 & 551 & --- & --- & $0.28\times$ Pauli \\
 & Gaussian cc-pVDZ & 46 & \gvq{h2o_ccpvdz_pauli}{107{,}382} & --- & --- & $55\times$ Pauli \\
\colrule
\multirow{2}{*}{BeH$_2$}
 & GeoVac composed & 50 & 1{,}396 & 68.8 & 64 & --- \\
...
\emph{Convention (declared 2026-08-30):} the GeoVac Pauli counts and
1-norms in this table \emph{include} the identity term, matching the raw
JW totals the Gaussian baselines are quoted at --- which is what makes the
LiH $34.0$ vs.\ $34.3$~Ha comparison like-for-like.  The balanced 1-norms
were previously printed non-identity in this same column ($75.2$ / $286.9$
/ $1{,}440$), mixing two conventions in one column; they are now
identity-included ($78.2$ / $323.4$ / $1{,}612$).  For the
simulation-relevant non-identity $\lambda_{\mathrm{ni}}$ see
Table~\ref{tab:molecules}.}
\end{table*}

For the small-molecule minimal-basis case, the composed LiH Hamiltonian is
near parity with Gaussian STO-3G on Pauli count---\gvq{lih_composed_pauli}{838} terms vs.\ STO-3G's
\gvq{lih_sto3g_raw}{907} (raw JW), $3.0\times$ more than the 276 (qubit-reduced), with a nearly
identical electronic
1-norm (34.0 vs.\ 34.3~Ha)---while using more qubits (30 vs.\ 10--12).  The
favorable $2.7\times$/$13\times$ multipliers hold only against the
un-reduced raw-\gvq{lih_sto3g_raw}{907} baseline and under the pair-diagonal ERI rule
(Sec.~\ref{sec:eri_rule}); they are not the framework's load-bearing
advantage.  That advantage appears against larger Gaussian bases, where the
gap widens decisively: $76\times$ fewer Pauli terms than cc-pVDZ
(exact rule; the retired pair-diagonal figure was $190\times$).

For H$_2$O, the composed Hamiltonian has \emph{more} Pauli terms than
STO-3G (1{,}954 vs.\ 551, advantage $0.28\times$) because the GeoVac
encoding uses 70 qubits (35 spatial orbitals $\times$ 2 spins) while
STO-3G uses only 12 qubits (7 spatial orbitals
with frozen core).  Against cc-pVDZ (46 qubits vs.\ GeoVac's 70---GeoVac
uses $1.5\times$ more; not matched accuracy either, as the composed
H$_2$O carries a 26\% $R_{\rm eq}$ error), GeoVac has $55\times$
fewer Pauli terms (corrected 2026-08-30: the retired $138\times$ was
$107{,}382/778$, i.e.\ computed against the retired pair-diagonal count).
The measured GeoVac exponent over this range is $2.816$ against the
Gaussian $3.9$--$4.3$, but that gap should \emph{not} be read as a
basis-size trend:\ $2.816$ is a two-point $n_{\max} = 1, 2$ fit and the
local slope steepens toward $\sim\!3.8$ by $n_{\max} = 4$, which closes
most of the gap.  See Sec.~\ref{sec:scaling_exponents}, where the
extrapolation this comparison used to license is withdrawn.

\subsection{Scaling exponents}
\label{sec:scaling_exponents}
...
\caption{Within-molecule Pauli scaling: $N_{\text{Pauli}} \propto Q^{\alpha}$.
  ...
  progression.  Corrected 2026-08-29 under the exact Coulomb rule
  (Paper~\cite{GeoVac_Paper14}, exact ERI selection rule); the retired pair-diagonal rule
  gave $2.18$--$2.24$, mean $2.21 \pm 0.02$.  The exact-rule exponent is
  \emph{identical} across all six molecules, not merely close:\ see the
  text.}
\label{tab:scaling}
\begin{ruledtabular}
\begin{tabular}{lcc}
...
LiH    & 2.816 & 3.9--4.3 \\
BeH$_2$ & 2.816 & --- \\
H$_2$O  & 2.816 & 3.9--4.3 \\
HF     & 2.816 & --- \\
NH$_3$  & 2.816 & --- \\
CH$_4$  & 2.816 & --- \\
\colrule
Mean   & 2.816 $\pm$ 0.000 & 3.9--4.3 \\
\end{tabular}
\end{ruledtabular}
\end{table}

The GeoVac composed scaling exponent is $2.816$ for every one of the
six molecules---identically, to the digits shown, not merely nearly
constant.  This is forced rather than empirical.  The composed Pauli
count is exactly linear in~$Q$ at each fixed basis
(Paper~\cite{GeoVac_Paper14}, universal composed linearity):\ $N_{\mathrm{Pauli}} = 1.5\,Q$ at
$n_{\max}=1$ and $27.90\,Q$ at $n_{\max}=2$, while $Q$ itself grows by
the same factor of~5 for every molecule.  The two-point exponent is
therefore
\begin{equation}
  \alpha = 1 + \log_5\!\left(\frac{27.90}{1.5}\right)
         = 1 + \log_5 18.6 = 2.8163\ldots,
  \label{eq:alpha_exact}
\end{equation}
independent of the molecule.  This two-point value is not comparable with
the wider-range within-molecule exponent of $3.17$ quoted in
Paper~\cite{GeoVac_Paper14}:\ the exponent is not constant in $n_{\max}$
(the local slope steepens toward $\sim\!3.8$ by $n_{\max}=4$), so a
$n_{\max}=1,2$ fit and a small-basis fit over a longer range measure
different things and legitimately differ.  The Gaussian scaling exponent of
$\sim 4$ reflects the dense four-index ERI tensor.  The GeoVac advantage still
grows with system size---but the within-molecule exponent is the wrong
instrument for saying by how much, and the retired
$(100)^{4.1}/(100)^{2.21} \approx 6{,}000\times$ extrapolation is
withdrawn rather than re-priced.  Two reasons.  First, this exponent is
not constant in $n_{\max}$ (above), so substituting the local
$\sim\!3.8$ instead of $2.816$ moves the same formula from
$\approx 370\times$ to $\approx 4\times$---two orders of magnitude, which
is the signature of an extrapolation outside its range.  Second, and more
basic:\ $Q = 100$ is reached in this library by \emph{adding blocks}, not
by enlarging the basis, and along that axis the composed count is exactly
linear in~$Q$ (Eq.~\ref{eq:linear})---exponent~1, measured, not
extrapolated.  The linearity is the claim; the ratio at a hypothetical
$Q$ is not.
...
group count. Chawla ratio = $N_{\mathrm{Pauli}}^{\mathrm{rel}} / \gvq{chawla_rah}{12\,556}$,
the single calibrated Chawla RaH-18q cell~\cite{Chawla2024}.
\emph{Re-measured 2026-08-30} under the corrected Condon--Shortley angular
factor order (retired values: $1{,}413$ / $942$ / $89{,}226$ / $59{,}484$
and $\lambda = 40.59$ / $143.96$ / $18.68$ / $218.78$ / $413.90$ /
$125.36$).  The former ``scalar'' column is \emph{removed}, not re-priced:\
it evaluated the relativistic spec with the spinless builder, which returns
the same count as the relativistic builder, so it was never a scalar
comparator and the ratios drawn from it measured nothing.}
\label{tab:paper20_tier2}
\begin{ruledtabular}
\begin{tabular}{ccccccc}
Molecule & $n_{\max}$ & $Q$ & $N_{\mathrm{Pauli}}^{\mathrm{rel}}$ & $\lambda_{\mathrm{ni}}$ (Ha) & QWC & Chawla ratio \\
\hline
LiH & 2 & 30 &  1\,501 &  39.53 &     142 & 0.120$\times$ \\
LiH & 3 & 84 & 90\,114 & 208.79 & 11\,938 & 7.177$\times$ \\
BeH & 2 & 30 &  1\,501 & 142.52 &     142 & 0.120$\times$ \\
BeH & 3 & 84 & 90\,114 & 400.01 & 11\,948 & 7.177$\times$ \\
CaH & 2 & 20 &     998 &  17.91 &     142 & 0.079$\times$ \\
CaH & 3 & 56 & 60\,060 & 119.35 & 11\,925 & 4.783$\times$ \\
SrH & 2 & 20 &     998 &  17.91 &     142 & 0.079$\times$ \\
BaH & 2 & 20 &     998 &  17.91 &     142 & 0.079$\times$ \\
\hline\hline
\multicolumn{7}{l}{\footnotesize Chawla RaH-18q~\cite{Chawla2024}: 12\,556 Pauli (rel), 2\,740 Pauli (non-rel) (47\,099 / 4\,249 two-electron integrals), $Q = 18$.} \\
\multicolumn{7}{l}{\footnotesize SrH, BaH added Sprint 3 (v2.14.0) via [Kr] and [Xe] frozen cores (\texttt{geovac/neon\_core.py}).} \\
\multicolumn{7}{l}{\footnotesize CaH/SrH/BaH at $Q=20$ bit-identical ($N_{\mathrm{Pauli}}^{\mathrm{rel}} = \gvq{cah_rel_n2_pauli}{998}$, $\lambda_{\mathrm{ni}} = 17.91$, QWC $= 142$): isostructural invariance.} \\
\multicolumn{7}{l}{\footnotesize Sprint 4 Track TR (v2.15.0): rel Pauli counts corrected 1.76$\times$ (n=2) / 1.92$\times$ (n=3) upward via missing jj reduced-matrix-element phase fix.  A second correction (2026-08-30) fixed the Condon--Shortley factor order; the values above are post-both.} \\
\multicolumn{7}{l}{\footnotesize Chawla et al.\ SI Tables S1--S3 (per-molecule matched-$Q$ data): accessibility \textbf{deferred}.} \\
\end{tabular}
\end{ruledtabular}
\end{table*}
...
\begin{itemize}
  \item At $n_{\max}=2$, all GeoVac molecules sit at
    ...
    $Q \in \{20, 30\}$, an $8.4$--$12.6\times$ advantage.
    \emph{The former matched-$Q$ projection to $17\times$--$32\times$ is
    withdrawn} (2026-08-30), in step with Paper~14: it combined the retired
    O($Q^{2.5}$) composed scaling law with a $\sim 4.2\times$ rel/scalar
    multiplier drawn from the ``scalar'' column this table's caption
    removes as never having been a scalar comparator.  Both inputs are
    gone and neither has a corrected replacement, so no projection is
    reported.  The surviving comparison is at native $Q$, and it is not
  ...
  \item At $n_{\max}=3$, the absolute GeoVac Pauli count overtakes
    ...
    number at $Q = 84$; CaH sits at $4.783\times$ at $Q = 56$). This
  ...
  \item \emph{The scalar-vs-relativistic $\lambda_{\mathrm{ni}}$
    comparison is withdrawn} (2026-08-30).  It read
    ``$170.59 \to 218.78$~Ha ($+28\%$) for LiH, $96.45 \to 125.36$~Ha
    ($+30\%$) for CaH,'' but all four figures are gone: the scalar side
    came from the removed column that evaluated the relativistic spec
    with the spinless builder (so it was never a scalar comparator), and
    the relativistic side is in this caption's retired list.  Producing
    an honest version needs a genuinely spinless comparator at matched
    basis and geometry, which the Tier~2 path does not currently build;
    until it does, the $\lambda_{\mathrm{ni}}$ column should be read as
    an absolute relativistic 1-norm and not as a relativistic
    \emph{inflation} factor.
...
CaH/SrH/BaH relativistic Hamiltonians give $\gvq{cah_rel_n2_pauli}{998}$ Pauli terms
(re-measured 2026-08-30 under the corrected factor order; retired: $942$),
which is $0.079\times$ Chawla et al.'s RaH-18q ($12.6\times$
...
molecules plus the He and H$_2$ reference atoms), spanning
$Z = 1$--56 (H through Ba); Tables~\ref{tab:molecules}--%
...
\caption{GeoVac Hamiltonian library at $n_{\max}=2$.  First-row
  ...
  the Pauli column, by contrast, \emph{includes} the identity term.
  Isostructural pairs (rows 2--3) produce identical Pauli counts --- but
  \emph{not} identical QWC group counts (MgH$_2$ $903$ vs.\ CaH$_2$ $898$;
  SiH$_4$ $1{,}566$ vs.\ GeH$_4$ $1{,}562$; only NaH/KH tie).  The greedy
  grouper sorts terms by descending $|$coefficient$|$ before insertion
  (\texttt{geovac/measurement\_grouping.py}), so partners with identical
  Pauli \emph{support} but different coefficients are visited in different
  orders.  The invariance is a property of the support, and QWC grouping
  is not a function of the support alone.
  The \texttt{hamiltonian()} API currently returns composed encoding
  for all rows; balanced data shown here are from Paper~19's
  \texttt{build\_balanced\_hamiltonian()} (every cell pinned by
  \texttt{tests/test\_paper20\_balanced\_lambda.py}; the
  $\lambda_{\text{ni}}$ column was re-synced to the live builder
  2026-07-02; all cells re-measured under the exact global-$M_L$ rule
  2026-08-29, Sec.~\ref{sec:eri_rule} --- Pauli-count isostructural
  invariance survives exactly (NaH $=$ KH, HCl $=$ HBr, \dots).
  \emph{Each row is evaluated at its own equilibrium bond length $R$,
  now stated in the table.}  Corrected 2026-08-31: the balanced
  $\lambda_{\text{ni}}$ and QWC columns had been computed at
  $R = 3.015$~bohr --- LiH's bond length --- for \emph{every} molecule,
  because the balanced builder defaulted to that value and ignored the
  spec's geometry.  The largest error was KH at $-12.7\%$
  ($32.2 \to 28.1$), its bond length being furthest from the substituted
  one.  The Pauli column was unaffected, and that is not luck: the count
  is fixed by the angular selection rules and is bit-identical across
  geometries, whereas $\lambda_{\text{ni}}$ and the greedy QWC count are
  both projections through the geometry
  (\texttt{tests/test\_paper20\_geometry\_independence.py}).  This also
  reverses the 2026-08-30 note that had ``corrected'' NaH
  $19.6 \to 20.6$ and HCl $866.1 \to 869.7$: those two cells were right
  before that pass, which had re-measured them at the wrong geometry.)}
\label{tab:molecules}
\begin{ruledtabular}
\begin{tabular}{llrrrrr}
Molecule & Core & $Q$ & $R$ (bohr) & Pauli & $\lambda_{\text{ni}}$ (Ha) & QWC \\
\colrule
LiH      & He   & 30 & 3.015 &    838 &  27.7 & 64 \\
BeH$_2$  & He   & 50 & 2.502 & 1{,}396 &  54.3 & 64 \\
H$_2$O   & He   & 70 & 1.809 & 1{,}954 & 186.8 & 64 \\
HF       & He   & 60 & 1.733 & 1{,}675 & 232.7 & 64 \\
NH$_3$   & He   & 80 & 1.912 & 2{,}233 & 158.0 & 64 \\
CH$_4$   & He   & 90 & 2.050 & 2{,}512 & 135.4 & 64 \\
\colrule
NaH      & Ne   & 20 & 3.566 &    575 &  19.6 &    69 \\
MgH$_2$  & Ne   & 40 & 3.261 & 4{,}861 & 111.8 &   903 \\
HCl      & Ne   & 50 & 2.409 & 9{,}824 & 866.1 & 1{,}173 \\
H$_2$S   & Ne   & 60 & 2.534 & 13{,}863 & 873.6 & 1{,}264 \\
PH$_3$   & Ne   & 70 & 2.680 & 18{,}854 & 889.3 & 1{,}427 \\
SiH$_4$  & Ne   & 80 & 2.800 & 24{,}745 & 909.2 & 1{,}566 \\
\colrule
KH       & Ar   & 20 & 4.243 &    575 &  28.1 &    69 \\
CaH$_2$  & Ar   & 40 & 3.807 & 4{,}861 & 123.8 &   898 \\
HBr      & Ar$d$ & 50 & 2.670 & 9{,}824 & 875.3 & 1{,}176 \\
H$_2$Se  & Ar$d$ & 60 & 2.760 & 13{,}863 & 883.4 & 1{,}264 \\
AsH$_3$  & Ar$d$ & 70 & 2.820 & 18{,}854 & 885.2 & 1{,}468 \\
GeH$_4$  & Ar$d$ & 80 & 2.870 & 24{,}745 & 874.8 & 1{,}562 \\
\end{tabular}
\end{ruledtabular}
\end{table}
...
\caption{Multi-center molecules added in v2.2 at $n_{\max}=2$.
  ...
  frozen cores (Na [Ne] + Cl [Ne]).  Pauli counts are non-identity terms
  and QWC groups are greedy; both re-measured 2026-08-29 under the exact
  Coulomb rule (the retired pair-diagonal rule gave $778$/$1{,}111$/$556$
  terms and 21 groups).  Every row is $\gvq{composed_coeff}{27.90} \times Q$ exactly.}
...
N$_2$    & He   & 14 & 7 & 100 & 2{,}790 & 64 \\
F$_2$    & He   & 18 & 9 & 100 & 2{,}790 & 64 \\
...
\caption{First-row transition metal hydrides ($Z = 21$--30) at $n_{\max}=2$
  ...
  blocks ($l_{\min} = 2$).  Under the exact global-$M_L$ rule the
  Pauli/$Q$ ratio is \gvq{composed_coeff_d}{30.03}, slightly above the main-group coefficient
  of~27.90 (the retired pair-diagonal rule inverted this, giving 9.23
  vs.\ 11.10 --- an artifact of dropping more $m$-swap integrals at higher
  angular momentum; Sec.~\ref{sec:eri_rule}).
...
\begin{equation}
  N_{\text{Pauli}} = \gvq{composed_coeff}{27.90} \times Q\,.
  \label{eq:linear}
\end{equation}
Here $N_{\text{Pauli}}$ counts \emph{non-identity} terms.  Table
\ref{tab:multicenter} is quoted in that convention and matches
Eq.~\eqref{eq:linear} exactly; Table~\ref{tab:molecules} includes the
identity and therefore reads $27.90\,Q + 1$.  The relation is exact in
both---the two differ by the one constant term---but the tables are not
...
qubits and $\sim$\gvq{lih_composed_pauli}{837} Pauli terms, regardless of molecular context.
This linear nuclear scaling is a stronger result than the
within-molecule basis scaling, which describes how cost grows when
expanding the angular basis on a \emph{fixed} molecule.  That
within-molecule exponent is $3.17$ over the small-basis range; note it is
\emph{not} the $2.816$ of Table~\ref{tab:scaling}, which is the two-point
$n_{\max}=1,2$ value---the two measure different ranges and legitimately
differ (Sec.~\ref{sec:scaling_exponents}).

\subsection{The retired pair-diagonal ERI rule: a sign error, not an approximation}
\label{sec:eri_rule}

All Pauli counts and multipliers in this library are realized under the
\emph{exact} global-$M_L$ Coulomb selection rule
($m_a + m_b = m_c + m_d$), including the \emph{$m$-swap} two-electron
integrals.

\emph{Correction (2026-08-29).}  Earlier versions of this library were
built under a \emph{pair-diagonal} restriction ($m_a = m_c$,
$m_b = m_d$) described as a deliberate sparsifying approximation.  An
independent re-derivation traced it to a sign error in the production
Gaunt coefficient ($q = m_c - m_a$ where the Wigner-3j bottom row
requires $m_a - m_c$), which silently zeroed every $m$-changing
multipole; the error was arbitrated against direct quadrature of the
angular factor from its definition.  The earlier ``constant factor,
scaling unchanged'' re-pricing claim was a single-point artifact: the
ratio is $2.51\times$ uniformly across main-group molecules at
$n_{\max} = 2$ but grows along the basis axis
($1.00\times/2.51\times/5.40\times$ at $n_{\max} = 1/2/3$), moving the
within-molecule exponent from the retired $2.5$ to $3.17$.  What
survives exactly is the cross-molecule linearity
$N_{\mathrm{Pauli}} = c\,Q$ at fixed basis, with $c = \gvq{composed_coeff}{27.90}$
(main-group) and $30.03$ ($d$-block); the retired pair-diagonal
coefficients were $11.10$ and $9.23$, and the apparent $d$-block
sparsity advantage inverts (higher angular momentum has \emph{more}
$m$-swap integrals for the buggy rule to drop).  The realized rule is
pinned by \texttt{tests/test\_paper14\_eri\_rule.py}; Paper~14 carries
...
# 559, 171.99  (composed API: 558 non-identity + identity; FULL 1-norm
#  incl. the identity coefficient -- a different builder and convention
#  from the BALANCED Table rows)
\end{lstlisting}

\noindent The within-molecule basis exponent $3.17$ is confirmed across both
rows (identical to all reported digits --- a consequence of the exact
factorization $N_{\mathrm{Pauli}} = c(n_{\max})\,Q$).  Second-row molecules use fewer qubits than their first-row
...
Transition metal hydrides (Table~\ref{tab:transition_metals}) sit
slightly \emph{above} the \gvq{composed_coeff}{27.90} coefficient: all ten first-row hydrides have
Pauli/$Q = \gvq{composed_coeff_d}{30.03}$ under the exact rule (the retired pair-diagonal rule
inverted this ordering, giving 9.23; see Sec.~\ref{sec:eri_rule}).
That lower ratio is an \emph{artifact} of the
...
\paragraph{GH-convergence-derived basis-truncation rate.}
...
with asymptotic rate $4/\pi = \mathrm{Vol}(S^2)/\pi^2$, the $k = 0$
volume signature of the master Mellin engine M1
(see~\cite{GeoVac_Paper18}).  The constant is a quotient of
\emph{unit-radius} sphere volumes and is \emph{not} the Hopf
base-to-total ratio (misnomer retired 2026-09-03);\ it is quoted here in
the dual-Coxeter normalisation of the $\mathrm{SU}(2)$ metric, and reads
$2/\pi$ on the unit round $S^3$.  This is, to our
knowledge, the first quantitative basis-truncation \emph{convergence-rate}
estimate exposed at a chemistry-consumer API---a proven state-space GH
metric rate, \emph{not} a direct energy-error bound (the operator-to-energy
lift is a loose heuristic that overshoots by 3--4 orders of magnitude; see
the Trotter-budget footnote).
The rate is system-independent (it is a property of the
spectral-triple truncation, not of the chemistry content),
so chemistry consumers reading the metadata see the same
``the operator is within state-space GH distance $\gamma_{n_{\max}}$ of the
$n_{\max}\to\infty$ limit'' message (a convergence rate, not a direct
energy-error bound) regardless of which molecule they are running.
...
across LiH, BeH$_2$, H$_2$O, NaH, CO, ScH, H$_2$ gives
relative error below $10^{-10}$ across all integrals
(\texttt{tests/test\_ecosystem\_export.py}).  This
...
$84$ qubits and $127{,}855$ Pauli terms (balanced coupled) or
$42{,}535$ (composed) under the exact global-$M_L$ rule.  The $n_{\max}=3$ basis approaches
...
$d$-Orbital blocks have a slightly higher Pauli-per-qubit ratio
(30.03 vs.\ 27.90) under the exact global-$M_L$ rule
(Sec.~\ref{sec:eri_rule}; the retired pair-diagonal rule inverted this).
...
H$_2$---$Z = 1$--56), including multi-center systems with multiple heavy
atoms and the full first transition series as hydrides (ScH through ZnH;
the $d$-block Pauli/$Q$ ratio is slightly higher than main-group,
Sec.~\ref{sec:eri_rule}).  The
structural sparsity---Gaunt selection
rules with a universal within-molecule exponent of 3.17---is confirmed
across all three rows and all molecular
...
$27.90$), meaning that adding atoms to a molecule increases
quantum cost linearly.
For LiH, this yields near parity on Pauli terms
($1.08\times$ fewer, raw-vs-raw) and 4.3$\times$ fewer measurement
groups than Gaussian STO-3G (QWC 64 vs.\ 273), growing
to 76$\times$ fewer terms against cc-pVDZ.  All multipliers
are realized under the exact global-$M_L$ rule
(Sec.~\ref{sec:eri_rule}); the LiH
small-molecule count is near parity with STO-3G (838 vs.\ \gvq{lih_sto3g_raw}{907}), while the
...
quantitative truncation-convergence rates (Paper~38 state-space GH rate, exposed as
...
NaH $=$ KH at $Q = 20$).
The within-molecule scaling exponent (3.17) is universal across all three
