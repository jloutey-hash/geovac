...
framework~\cite{mondino_samann2025_pointed}. The bridge is a contravariant
functor
\[
...
(Theorem~\ref{thm:bridge}) holds at theorem-grade rigor:\ (B1)
structural correspondence on $15$ rows, (B2) reverse triangle inequality
via on-orbit modular-flow equality plus an M-diagonal topography
trivialization off-orbit, (B3) pre-compactness inheritance via
Paper~44~\cite{paper44} propagation-number bound, (B4) convergence
transport via Plancherel-nested Berezin compatibility with the
Mondino--S\"amann Def~3.6 axioms.
...
\textbf{T6}:\ a \emph{$\mathbf{G2}$ metric-level closure for the GeoVac
wedge}, supplying a wedge-specific workaround for the published-open
non-compact Latr\'emoli\`ere propinquity problem (Paper~45~\cite{paper45}
\S~1.4 $\mathrm{G2}$). The synthetic pLGH-convergence rate panel
inherits the bit-exact numerical values
$\{2.0746, 1.6101, 1.3223\}$ from Paper~45's
Krein-side $\Lprop$. To the author's knowledge this is the
first quantitative pLGH-convergence panel in the Mondino--S\"amann
literature derived from operator-algebraic input.
...
metric-level closure of the published-open G2 problem of
Paper~45~\cite{paper45} \S~1.4. The strong-form bridge on the
...
states that all four bridge properties hold at theorem-grade rigor:
\begin{itemize}
...
$=2$ cardinality bound.
\item[(B4)] \emph{Convergence transport}\ — Krein-side propinquity
convergence under the Latr\'emoli\`ere hypertopology induces
Mondino--S\"amann pLGH convergence, with Plancherel-nested Berezin maps
matching the MS Def~3.6 axioms one-to-one.
\end{itemize}
...
the first quantitative pLGH-convergence panel in the MS literature
derived from operator-algebraic input;
...
\item \textbf{T6} (\textbf{G2 metric-equivalent closure for the
GeoVac wedge}) — the substantive deepest content:\ the
de-compactification limit $T \to \infty$ closes at the pLGH-equivalent
(metric-level-equivalent) level on the synthetic side for the GeoVac
wedge specifically. A workaround for the published-open non-compact
propinquity problem (Paper~45~\cite{paper45} \S~1.4 $\mathrm{G2}$) at
the GeoVac-wedge-specific level;
...
For the GeoVac wedge, the substantive \textbf{T6} closure of
$\mathrm{G2}$ is wedge-specific:\ it closes the de-compactification
...
in which all nine Latr\'emoli\`ere axioms transport at theorem-grade
rigor.
...
Paper~38~\cite{paper38} SU(2) Riemannian propinquity hypertopology.
...
internal refinements; the former extends $\ell$-convergence on
metric measure spaces, the latter establishes a Lorentzian coarea
inequality. We cite both as adjacent works in the synthetic-Lorentzian
convergence literature.

The Hekkelman--McDonald papers~\cite{hekkelman_mcdonald2024,
hekkelman_mcdonald2024b} on Connes spectral triples and
...
Paper~45~\cite{paper45} establishes the K$^{+}$-restricted weak-form
Lorentzian propinquity convergence:\
\[
\Lprop(\Tcal^{L}_{\nmax, \Nt, T}, \Tcal^{L}_{\Manifold})
\;\le\; \Cthreejoint \cdot \gammajoint_{\nmax, \Nt, T} \;\to\; 0
\]
along admissible scaling $(\nmax, \Nt) \to (\infty, \infty)$ at fixed
$T$, with bit-exact panel values $\Lprop(2,3) = 2.0746$,
$\Lprop(3,5) = 1.6101$, $\Lprop(4,7) = 1.3223$. Paper~46~\cite{paper46}
extends to a strong-form (no K$^{+}$ restriction) on the natural
chirality-doubled scalar-multiplier substrate, with bit-identical
$\Lambda^{\mathrm{strong}} = \Lprop$ ("free upgrade"), and to a further
enlarged substrate (Appendix~B) with chirality-flipping generators
$M^{\mathrm{flip}}$ satisfying $\{\JL, M^{\mathrm{flip}}\} = 0$.
...
When restricted to $\Nt = 1$, the Krein hypertopology on the class of
Krein PPQMS reduces bit-exactly to the Paper~38~\cite{paper38} SU(2)
Riemannian propinquity hypertopology.
\end{theorem}
...
Sub-Sprint~D~\S~5 Riemannian-limit recovery,
$\Lprop_{\nmax, 1, T} = \Lambda^{\mathrm{Paper~38}}_{\nmax}$
bit-exact. The pointed-propinquity reduction follows.
\end{proof}
...
the Krein-pointed propinquity hypertopology is a strict extension of
Paper~38, not a parallel construction.
\end{remark}
...
\begin{theorem}[Convergence transport]\label{thm:convergence_transport}
...
the synthetic-side counterpart of Paper~38's SU(2) Riemannian
propinquity:\ a trivial LPLS with no Lorentzian content (modular flow
...
Mondino--S\"amann pLGH sense to a covered LPLS limit.
\end{theorem}
...
bit-identical to the Paper~45 Krein-side $\Lprop$ values. This is the
\textbf{first quantitative pLGH-convergence panel in the
Mondino--S\"amann literature derived from operator-algebraic input.}
\end{theorem}
...
\begin{theorem}[T6:\ G2 metric-level closure for the GeoVac
hemispheric wedge]\label{thm:g2_wedge_closure}
...
This is a \textbf{GeoVac-wedge-specific closure of} $\mathrm{G2}$
(de-compactification limit, Paper~45~\cite{paper45} \S~1.4) at the
\textbf{metric-equivalent level on the synthetic side} that the
general non-compact Latr\'emoli\`ere problem does not have at the
operator side.
\end{theorem}
...
The wedge application is unconditional in the sense that the bridge
...
three-carrier). The headline content (T6 G2 metric-equivalent closure
for the wedge) is established at theorem-grade rigor for the wedge
specifically; the general non-compact Latr\'emoli\`ere problem
...
wedge}\label{sec:g2_wedge_closure_unpacked}

The deepest substantive new content of the paper is
Theorem~\ref{thm:g2_wedge_closure} (T6). We unpack the structural
significance:
...
well-defined). The synthetic-side closure exists; the bridge
transports it back to the operator side.
...
side via the Wick-rotation functor. The substantive headlines (T3
quantitative panel, T6 G2 closure) are genuinely new content for the
Mondino--S\"amann literature.
...
Theorem~\ref{thm:g2_wedge_closure} (T6) closes G2 metric-level for the
GeoVac wedge specifically via the bridge image on the synthetic side.
...
P.~Bertozzini, R.~Conti, W.~Lewkeeratiyutkul, ``Modular spectral
triples in noncommutative geometry and physics,'' (2009).
[Note: arXiv:0901.4031 was previously cited but that ID is actually
a tridiagonal-matrices paper; the intended Bertozzini-Conti-Lewkeeratiyutkul
modular-spectral-triples work is a programme of writings (~2008--2013) of
which the canonical reference is to be supplied by the PI.]
...
[Replaces an earlier bibitem pointing to arXiv:2411.04566, which is
actually a BosonSampling paper. An intended second Hekkelman--McDonald
2024 work could not be located; this entry duplicates
\texttt{hekkelman\_mcdonald2024} pending PI clarification.]
...
\bibitem[Kostant(1965)]{kostant1965}
...
A.~Mondino, M.~Ryborz, C.~S\"amann, ``Stability of synthetic
timelike curvature bounds,'' arXiv:2605.03172 (May~2026).
...
J.~Loutey, ``The Bargmann--Segal lattice for the 3D harmonic oscillator
and the Coulomb/HO asymmetry,'' GeoVac internal preprint Paper~24
(2026), at
...
spectral triple,'' GeoVac internal preprint Paper~38 (2026),
