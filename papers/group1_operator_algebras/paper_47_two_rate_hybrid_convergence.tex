...
Reed--Simon~\cite{reed_simon_iv}\,\S~XIII.16 compact-to-non-compact-domain
resolvent machinery on the outer arrow.
\end{abstract}
...
Papers~45~\cite{paper45}--46~\cite{paper46} \emph{set out to establish}
that the finite-cutoff Lorentzian Krein spectral triple
...
at fixed temporal radius $T$. \emph{That metric/propinquity-level
convergence is descoped} --- it rests on Paper~45's degenerate $\Kplus$
seminorm (see the Status note); only the norm-resolvent / spectral leg
survives, which is what the present paper closes. The natural canonical choice is the
...
$\Kplus$-restriction (\emph{descoped} on the natural substrate in
Paper~46~\cite{paper46} --- rate-formula only, metric-level convergence
open as that seminorm is degenerate; strict-strong-form on the
truly-enlarged substrate also open),
...
substantial open NCG-mathematics program, beyond sprint scale, and not
addressed here (\S~\ref{sec:open}).
...
Sprint L2-C\footnote{Sprint/track designations are anchors into the project's internal chronicle (\texttt{CHANGELOG.md} in the repository).} construction), the Lorentzian Dirac operator on each
Krein space is
\begin{align}
...
\begin{theorem}[Inner arrow --- \emph{descoped}]\label{thm:inner}
\emph{(Descoped:} the bound below consumes Paper~45's $\Lprop$, a
degenerate seminorm, so it is a rate-formula statement, not an
established propinquity/metric convergence; only the outer
norm-resolvent arrow survives --- see the Status note.\emph{)}
...
\;\le\; \Cthreejoint(\nmax)\,\gammajoint\bigl(\nmax, \Nt, T(\Nt)\bigr),
\end{equation}
with $\Cthreejoint(\nmax) = 1$ (Paper~38 Lemma~L3, gradient/translation
seminorm; the earlier operator-norm envelope form $\sqrt{1 - 1/\nmax}$ is
\emph{withdrawn} as operator-norm-false, see Paper~46~App.~A.3), $T$- and
$\Nt$-independent, and
...
bound at that cell. Four of the five lemmas L1' / L2 / L3 / L4
transport without modification;\ the fifth is no longer available (see
the scope note at the end of this list):
\begin{itemize}
...
\item \textbf{L2 (Plancherel-mass bound, Paper~45~\S~5):}
the relevant constant $2/(\nmax+1)$ is the maximum of the Plancherel
mass distribution (the smoothing map itself is UCP with cb-norm $1$;
see the Status note), depending only on
...
$a_{t}$. The constant $\Cthreejoint(\nmax, \Nt) = 1$ (Paper~38 L3; the earlier
$\sqrt{1 - 1/\nmax}$ operator-norm form is withdrawn) is $T$- and
$\Nt$-independent.
...
\textbf{[SCOPE NOTE, 2026-09-07 --- this item is not currently
available.]}\ The L5 step above transports a \emph{height} bookkeeping,
and Paper~38's Lemma~L5 was withdrawn on 2026-09-03 when precisely that
leg was refuted:\ a finite-band witness gives $\mathrm{height}_B \equiv
1$, so the bound fails for every $\nmax \ge 6$.  Paper~38's own
convergence theorem is unaffected because its unconditional proof (the
lifted-state construction) uses no Berezin map, no partial inverse and
no height term --- but that means the repair here is not a
substitution of constants:\ the cell-wise argument must be rebuilt on
the lifted-state route, which has not been done.  Until it is, the
transport claim for this item is \textbf{open}.  This is a
\emph{second}, independent reason to read Theorem~\ref{thm:inner} as
descoped, on top of the degenerate-seminorm reason already recorded in
its statement:\ even granting $\Lprop$, this item's lemma is withdrawn.
...
\begin{remark}[Status of the two contributions]
The $R^{\mathrm{outer}}$ (norm-resolvent) contribution is established.
The $R^{\mathrm{inner}}$ contribution rests on the inner-arrow
propinquity quantity $\gammajoint$ (Theorem~\ref{thm:inner}), which is
\emph{descoped}:\ it consumes Paper~45's $\Lprop$, a degenerate
seminorm. The composite is therefore established at the
\emph{norm-resolvent / spectral} level (via $R^{\mathrm{outer}}$);\ the
\emph{metric-level} reading of the $R^{\mathrm{inner}}$ term is
descoped (see the Status note). This theorem and
$R^{\mathrm{outer}}$ are analytic results (proof-by-argument,
Reed--Simon \S XIII.16);\ the $T\to\infty$ de-compactification is not
...
\subsection{Rate-formula panel along admissible scalings (the inner
arrow is descoped)}\label{sec:numerical_panel}
...
scalings confirms the \emph{rate formula} of the (descoped)
Theorem~\ref{thm:inner} --- not a metric-level convergence:\ the
$\Nt$-independence below is the structural signature of the degenerate
$\Kplus$ seminorm (Paper~45), not a propinquity limit:
...
The empirical panel confirms this rate-formula identity at full machine
precision;\ it is \emph{not} evidence of a metric-level inner-arrow
convergence, which is descoped (Theorem~\ref{thm:inner}).
...
\begin{proof}[Proof (analytic, proof-by-argument; like
Theorem~\ref{thm:outer}, the $T\to\infty$ de-compactification is not
computationally implemented)]
...
Hekkelman--McDonald~2024~\cite{hekkelman_mcdonald2024} construct a
noncommutative integral on spectrally truncated spectral triples (with
a quantum-ergodicity application). Their setting is the compact
truncation, Riemannian (positive-definite metric), not the non-compact
de-compactification limit of the present outer arrow, and not the
Krein / Lorentzian-signature setting; the present paper's outer-arrow
resolvent machinery is the standard Reed--Simon~\S~XIII.16 comparison,
not theirs. To our knowledge the present paper is the first to make the
explicit identification of the non-compact
...
farsi_latremoliere2025} is defined for
...
\end{itemize}
Either route is a substantial program of original NCG mathematics,
beyond sprint scale, and is not addressed here.
...
% =====================================================================
\section{Propinquity-level closure via temporal-Lipschitz-invisibility (descoped)}\label{sec:g2_metric}
% =====================================================================
...
``substantial open program'' assessment in the original Q1
formulation. GeoVac's
...
\begin{theorem}[G2-metric route on the natural substrate (descoped --- rests on the degenerate Paper~45 seminorm)]\label{thm:g2_metric}
...
\begin{equation}\label{eq:g2_metric_bound}
   ...
   \qquad \forall\, r > 0,
\end{equation}
where $\Cthreeop(\nmax) = 1$ (Paper~38 Lemma~L3; the earlier
$\sqrt{1 - 1/\nmax}$ operator-norm envelope form is \emph{withdrawn},
Paper~46~App.~A.3), $\gamma_{\nmax} = O(\log\nmax/\nmax)$ (the Paper~45 L2
Berezin approximate-identity rate; note $2/(\nmax+1)$ is the Plancherel
mass-distribution maximum, not the rate, and the smoothing map has cb-norm
$1$), and $\epsilon(T) \to 0$ as $T \to \infty$.
\end{theorem}
...
$\Lprop(4, 7) = 1.3223$ are inherited as the asymptotic non-compact
\emph{rate-formula} values. \emph{(Descoped: these are the
degenerate-$\Kplus$ seminorm rate-formula values of Paper~45, not
established metric-level propinquity limits --- the inner arrow is
descoped; see the Status note and \S\ref{sec:numerical_panel}.)} This is
the ``free upgrade'' reading extended from the compact to the non-compact
carrier at the rate-formula level only.
\end{remark}

\begin{remark}[Scope: natural substrate only]%
\label{rem:g2_scope}
Theorem~\ref{thm:g2_metric}'s construction is restricted to the
\emph{natural substrate}
(chirality-block-diagonal multipliers commuting with $\JL$, Paper~44
Prop.~5.1) --- and even there the metric-level closure is
\emph{descoped} (it inherits Paper~45's degenerate seminorm; only the
norm-resolvent/spectral level is established). On the \emph{enlarged substrate}
