...
% the norm-resolvent / spectral level, NOT at the propinquity / metric
% level. Propinquity on a non-compact carrier remains open (G1-genuine).
%
...
%
% Honest scope: norm-resolvent at the outer arrow, propinquity at the
% inner arrow. The two rates do not compose into a single uniform
% Latremoliere bound. Metric-level closure of G2 on the non-compact
% carrier requires a non-compact extension of Latremoliere propinquity
% (multi-month NCG-math, named G1-genuine).
% =====================================================================
...
verbatim, at coupled cells); the outer arrow is norm-resolvent
...
program~\cite{mondino_samann2024,mondino_samann2025},
...
Mondino and S\"amann
\cite{mondino_samann2024,mondino_samann2025} (and the
...
The construction of this paper closes G2 (Paper~45~\S~1.4) at the
norm-resolvent level. The following named open questions remain:

\begin{enumerate}
\item[(Q1)] \emph{Propinquity-level closure on a non-compact carrier
(G2-metric).} The natural-substrate metric extension of the present
two-rate result to Latr\'emoli\`ere distance on $\Tcal^{L}_{S^{3}
\times \R_{t}}$ requires a non-compact extension of Latr\'emoli\`ere's
propinquity. This is multi-month original NCG-math. Routes outlined
in \S~\ref{sec:vs_latremoliere}.
Latr\'emoli\`ere~\cite{latremoliere2025_hypertopology} (December 2025)
provides the Riemannian C*-algebra side of this problem via the
pointed-proper-QLCMS hypertopology;\ the natural Krein spectral-triple
follow-on combines that pointed-proper machinery with the operator-system
substrate of Papers~44--46~\cite{paper44, paper45, paper46} to define a
Lorentzian pointed-proper propinquity directly on the non-compact
carrier $\sthree \times \R_{t}$. This is the explicit named NCG-math
program for closing G2 at the metric level.

\item[(Q2)] \emph{Enlarged-substrate parametric stability
(L3c-$\beta$).} The inner arrow of Theorem~\ref{thm:main} holds on
the natural substrate of Paper~45 / Paper~46 main theorem; on the
enlarged substrate of Paper~46 Appendix~B (chirality-flipping
generators with $\{\JL, M^{\flip}\} = 0$), the gradient norm picks up
an $O(T)$ term that does not vanish under the joint limit. A refined
gradient norm or an alternative propinquity construction would be
needed.

\item[(Q3)] \emph{Joint convergence with cross-manifold extension
(G3).} The cross-manifold setting $\sthree \otimes
\mathrm{Hardy}(S^{5})$ is blocked at the NCG-framework level by the
Coulomb/HO category mismatch (Paper~24~\cite{paper24}\,\S~V four-layer
asymmetry). The combination with G2 closure (this paper) would
require addressing G3 separately.
...
signature,'' \emph{Class. Quant. Grav.} \textbf{35} (2018), 175004.
arXiv:1611.07062.
...
R.~Che, R.~Perales, C.~Sormani, ``Synthetic Lorentzian
Gromov--Hausdorff convergence,'' \emph{Adv. Math.} (2025), arXiv:2510.13069.
...
non-compact spectral triples,'' arXiv:2411.xxxxx (2024).
...
A.~Mondino, C.~Sämann, ``Synthetic Lorentzian Gromov--Hausdorff
convergence,'' \emph{Adv. Math.} (2024), arXiv:2403.xxxxx.

\bibitem[Mondino \& Sämann(2025)]{mondino_samann2025}
A.~Mondino, C.~Sämann, ``Synthetic curvature bounds for Lorentzian
length spaces,'' (2025), arXiv:2502.xxxxx.
...
J.~Loutey, ``Universal vs Coulomb-specific partition of the GeoVac
spectral triple,'' GeoVac internal preprint Paper~31 (2026), at
