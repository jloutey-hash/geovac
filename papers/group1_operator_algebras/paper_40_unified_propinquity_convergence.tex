...
hypothesis is the named gap), but the two forward estimates the
state-space GH bound needs --- the good-kernel estimate and the
Lipschitz distortion --- are the forward reach and forward height of
that would-be tunnel, so we organise the proof by those four terms.
...
$\Tcal_G$ and $\Tcal_\Lammax$, and yields the state-space GH bound
...
\begin{proof}
We organise the estimate by the four quantities a Latr\'emoli\`ere
tunnel would carry --- reach (forward and dual) and height (forward and
dual) --- because the state-space GH assembly of Paper~38 (Lemma~L5
there) uses exactly the two forward ones.
...
Combining the forward estimates (the state-space GH assembly of
Paper~38, Lemma~L5; the max over the four terms is what the would-be
tunnel length~\eqref{eq:propinquity_def_general} would also use):
\begin{equation*}
   \Lambda_{\mathrm{prop}}(\Tcal_\Lammax, \Tcal_G)
