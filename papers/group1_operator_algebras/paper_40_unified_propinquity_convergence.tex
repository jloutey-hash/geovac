% =====================================================================
% Paper 40 (standalone): Unified state-space GH convergence on compact
% semisimple Lie groups with bi-invariant metric (retitled 2026-09-02, PI
% direction; the Zenodo record minted earlier carries the pre-retitle title).
%
% Generalizes Paper 38 (SU(2)) to compact connected semisimple Lie groups
% (tori excluded: a circle factor carries the constant 2/pi).
% Companion to Gaudillot-Estrada & van Suijlekom 2025 (IMRN; arXiv:2310.14733):
% proves state-space GH convergence with explicit rate on the Dirac/spinor
% substrate (unconditional for SU(2); for general G conditional on the
% per-group spin-window lifted-state decomposition, rem:correction_L2;
% NOT Latremoliere propinquity, which remains a named gap) and identifies
% the universal rate constant 4/pi on the semisimple class.
%
...
spectral truncations of compact semisimple Lie groups \\
...
equipped with a bi-invariant Riemannian metric, and let
...
$\Tcal_\Lammax$ converges to $\Tcal_G$ in van~Suijlekom's state-space
Gromov--Hausdorff distance for metric spectral triples, with
\begin{equation*}
...
the dual-Coxeter normalisation of the bi-invariant inner product.  The
value of the constant scales with the metric normalisation (a distance
moment is linear in the metric scale):\ for $\SU(2)$, $4/\pi$ is the value
in the dual-Coxeter normalisation and $2/\pi$ on the unit round $S^3$.
At rank~1 the two conventions are now reconciled (2026-09-03):\
$\mathrm{Cas}(\mathrm{ad}) = h^\vee = 2$ on $\mathfrak{su}(2)$ fixes
$\langle X, Y\rangle = -2\,\mathrm{tr}(XY)$, whose geodesic distance from
the identity is the rotation angle --- exactly the distance Paper~38
integrates against --- so Paper~38 is the rank-1 case of this theorem in
the same normalisation, and its constant $4/\pi$ is this one. Paper~38
\cite{paper38} is recovered as the rank-$1$ case $G = \SU(2)$. The proof
proceeds via the same lemma chain (L1$'$~chirality-doubled operator
...
Peter--Weyl), closed by the lifted-state route.  The approximation pair
of \S\ref{sec:L5} is recorded but \emph{does not supply an independent
proof}:\ its dual reach is a named gap and both height constituents are
refuted (Lemma~\ref{lem:L5}, corrected 2026-09-04). The
universality of $4/\pi$ identifies the state-space GH rate with the
...
class-wide result (rigorous at rank~$1$; numerically established at
rank~$\ge 2$, where the robust content is the discrimination of $4/\pi$
from the Weyl-formula alternative and a fully rank-uniform analytical
proof is a named gap, see Remark~\ref{rem:correction_L2} and the status
note in \S\ref{sec:L2}). The result supplies an explicit
Dirac-controlled rate for the state-space Gromov--Hausdorff convergence
proved by Gaudillot-Estrada and van~Suijlekom~\cite{gaudillot_estrada_vs2025}.
The strictly stronger Latr\'emoli\`ere propinquity is \emph{not}
claimed:\ its dual-reach step is a named gap
(Remark~\ref{rem:correction_L2}), exactly as for the rank-1 case
(Paper~38). Moreover the general-$G$ lifted-state construction
underlying even the state-space~GH statement is verified only for
$\SU(2)$, so for general $G$ the convergence is conditional on that
per-group spin-window decomposition;\ the closed-form rate
$\gamma_\Lammax(G)$ and the universal $4/\pi$ constant (the genuine new
...
distance, compact semisimple Lie group, Kostant cubic Dirac operator, Peter--Weyl
...
Toeplitz/Fej\'er reconstruction. Leimbach and
van~Suijlekom~\cite{leimbach_vs2024} extended the analysis to flat
tori $T^d$, proving
state-space Gromov--Hausdorff convergence of truncated tori with
...
work~\cite{rieffel2023, hawkins2000} treats Berezin--Toeplitz
reconstruction on $S^2$ (matrix algebras converging to the coadjoint
orbit), using the K\"ahler structure of the coadjoint orbit. All of these results address flat abelian or K\"ahler
structures.
...
\)
and asymptotic constant $4/\pi = \Vol(S^2)/\pi^2$, a quotient of
unit-radius sphere volumes (not the Hopf fibration's base-to-total ratio;
label corrected 2026-09-03, see Paper~38's volume-ratio note). The proof proceeded
through a lemma chain L1$'$--L4 plus an assembly step (the fifth,
\S\ref{sec:L5}, has since been reduced to a record rather than a proof
--- dual reach a named gap, both heights refuted), in which the
principal new ingredients beyond the abelian case were (i)~a central spectral Fej\'er
...
We close the gap between Paper~38 and~\cite{gaudillot_estrada_vs2025} on
the semisimple sub-class of Class~1 (compact connected semisimple Lie
groups; a central torus factor carries the circle constant $2/\pi$ and
is excluded, see the Scope note inside
Corollary~\ref{cor:general_compact_connected}).

\begin{theorem}[Main theorem; cf.~\S\ref{sec:main_theorem}]
\label{thm:main_intro}
Let $G$ be a compact connected semisimple Lie group of rank $r \ge 1$
with bi-invariant Riemannian metric (dual-Coxeter normalised). The
truncated metric spectral triples $\Tcal_\Lammax$ converge to $\Tcal_G$
in van~Suijlekom's state-space Gromov--Hausdorff distance, with
explicit rate
...
\emph{Scope of the hypothesis:} the statement is unconditional for
$G = \SU(2)$;\ for general $G$ it is conditional on the per-group
spin-window decomposition underlying the lifted-state construction
(Remark~\ref{rem:correction_L2}), verified only in the rank-one case.
The
closed-form rate and the universality of $4/\pi$ --- the new content
here --- do not depend on that hypothesis. The stronger
Latr\'emoli\`ere propinquity is not claimed at any rank
(Remark~\ref{rem:correction_L2}).
\end{theorem}
...
class-wide property of compact semisimple Lie groups with bi-invariant
metric (a central $U(1)$ factor carries the circle constant $2/\pi$
instead; see the scope note inside
Corollary~\ref{cor:general_compact_connected}).

The universality of $4/\pi$ is rigorous at rank~$1$ ($\SU(2)$:\
Paper~38~\cite{paper38}, with the closed-form sum-rule of \S\ref{sec:L2}
verified to $4/\pi$ by the doubling estimator in
\texttt{geovac/central\_fejer\_su2.py}). For general $G$ the
Plancherel-weight $\times$ Vandermonde-Jacobian cancellation theorem
(Theorem~\ref{thm:universal_constant_analytical}) supplies the
\emph{mechanism} as a proof sketch (\S\ref{sec:L2}), confirmed
numerically at ranks~$1$--$2$ by direct Weyl integration
($\SU(2), \SU(3), \Sp(2), G_2$; backing test
\texttt{tests/test\_paper40\_universal\_rate.py}), with an additional
rank-$3$ datapoint on $\SU(4)$. The numerical leading-constant
extraction is \emph{fit-sensitive} --- the raw extracted leading
constant scatters with the fit order and the $\min L$ cut --- so what is
\emph{robust} is the discrimination of Reading~A ($c = 4/\pi$) from
the Weyl-formula Reading~B ($c = 2|W|/\pi^r$), decisive at $G_2$
(extracted $c$ near $4/\pi \approx 1.27$, far from
$24/\pi^2 \approx 2.43$). A fully rank-uniform analytical proof is a
named gap. In parallel, the
...
$4/\pi = 2\Vol(S^2)/\Vol(\SU(2)) = \Vol(S^2)/\pi^2$ is the Hopf-base
volume quotient $2\Vol(S^2)/\Vol(S^3)$ at unit radius (not the Hopf fibration's base-to-total ratio; Paper~38, volume-ratio note), and
...
that this M1 signature is rank-invariant across the semisimple class:\
whatever the group, the central spectral Fej\'er kernel sees the
rank-$1$ ($\SU(2)$) asymptotic analysis of Paper~38 through
Peter--Weyl rather than through the global geometry of the
maximal-torus quotient $G/T$. (The constant is that of the rank-$1$
\emph{group}, not of its maximal torus:\ the classical Fej\'er kernel
on the circle has first-moment constant $2/\pi$, half the $\SU(2)$
value --- Paper~38, Remark ``Connection to the circle Fej\'er
estimate''; see Section~\ref{sec:why_universal}.)
...
Casimir-cutoff form. Section~\ref{sec:lemmas} proves lemmas L1$'$--L4
and records the fifth:
...
state-space GH assembly via the approximation pair. Section~\ref{sec:main_theorem}
...
Throughout, $G$ denotes a compact connected \emph{semisimple} Lie group
of rank $r \ge 1$, with Lie algebra $\mathfrak g$ and chosen maximal
torus $T \subset G$ of dimension $r$.  (Semisimplicity is where the
dual-Coxeter normalisation below is defined; a central $U(1)$ factor
would carry the circle constant $2/\pi$ rather than $4/\pi$, see the
scope note inside Corollary~\ref{cor:general_compact_connected}.) We equip $G$ with a bi-invariant Riemannian metric
...
\end{equation}
We call this the \emph{dual-Coxeter normalisation} throughout.
\emph{Attribution note (2026-09-03):} it is \emph{not} the normalisation
standard in representation theory and conformal field theory.  That one fixes
$(\theta \mid \theta) = 2$ on the highest root, under which
$\Cas(\mathrm{ad}) = (\theta \mid \theta + 2\rho) = 2 + 2(h^\vee - 1) =
2h^\vee$~\cite{kac1990};\ the rule~\eqref{eq:dual_coxeter} is that form scaled
by $2$.  The choice is immaterial to the universality statement --- what makes
``universal across the class'' well-posed is that the rule is fixed
independently of the answer, not which rule it is --- but it is material to the
\emph{value}:\ on $\SU(2)$, \eqref{eq:dual_coxeter} gives the round sphere of
radius $2$ and the constant $4/\pi$, while $(\theta\mid\theta) = 2$ gives radius
$\sqrt2$ and $2\sqrt2/\pi$, and the unit sphere gives $2/\pi$.  Earlier drafts
called $4/\pi$ canonical;\ it is the value in the declared rule. For
...
\subsection{Distance convention}\label{sec:propinquity_convention}

Throughout this paper, $\Lambda_{\mathrm{prop}}(\Tcal_1, \Tcal_2)$
denotes van~Suijlekom's \emph{state-space Gromov--Hausdorff distance}
between metric spectral triples~\cite{vs2021_jgp} --- the object every
bound in this paper controls; the symbol is retained from the drafting
lineage of Paper~38 (2026-09-02 correction:\ an earlier version of this
subsection defined the symbol as Latr\'emoli\`ere's propinquity). The
strictly stronger Gromov--Hausdorff propinquity for metric spectral
triples of Latr\'emoli\`ere~\cite{latremoliere_metric_st_2017} --- the
spectral-triple-level extension of the dual
propinquity~\cite{latremoliere2016} and of the quantum compact metric
space propinquity~\cite{latremoliere2018}, computed as an infimum over
tunnels --- is \emph{not} claimed:\ the approximation pair of
\S\ref{sec:L5} would be the data of such a tunnel, but its dual-reach
hypothesis is a named gap (Remark~\ref{rem:correction_L2}).
...
bounded compact perturbation, and the state-space GH bound of
...
the rate-controlled state-space GH proof, which uses the truthful
...
universality of $4/\pi$ is \emph{rigorous at rank~$1$}
($\SU(2)$, Paper~38) and \emph{numerically established} across
$\SU(3)$, $\Sp(2)$, $G_2$ (and a rank-$3$ datapoint $\SU(4)$);
Theorem~\ref{thm:universal_constant} below. For general $G$ the
\emph{Plancherel-weight $\times$ Vandermonde-Jacobian cancellation
theorem} (Theorem~\ref{thm:universal_constant_analytical} below)
supplies the \emph{mechanism} as a proof sketch; a fully rank-uniform
analytical proof at the rigor level of Paper~38 Appendix~A is a named
gap. The numerical extraction is fit-sensitive, so the robust content
at rank~$\ge 2$ is the discrimination of Reading~A ($4/\pi$) from the
Weyl-formula Reading~B (decisive at $G_2$); see the backing test
\texttt{tests/test\_paper40\_universal\_rate.py}.
...
value $2/(n_{\max} + 1)$; it plays no role in the L5 bounds.
...
mass-distribution maximum $2/(\Lammax+1)$ enters none of the L5
...
(ii) the approximation-pair assembly of \S\ref{sec:L5} follows
Paper~38~\cite{paper38}; the device is van~Suijlekom's state-space
Gromov--Hausdorff framework, and the tunnel vocabulary there is
orientation only (2026-09-02 reframing).
...
\begin{equation}\label{eq:universal_rate_analytical}
   ...
   \;+\; O\!\bigl(1/\Lammax\bigr),
\end{equation}
with the leading coefficient $4/\pi$ universal across all $G$. (Tier:\
rigorous at rank~$1$, Paper~38; at rank~$\ge 2$ this is established as a
proof sketch supplying the mechanism, the fully rank-uniform write-up
being a named gap --- see the proof below and the status note in
\S\ref{sec:L2}.)
\end{theorem}

\begin{proof}[Proof sketch (mechanism; a fully rank-uniform
write-up is a named gap)]
The argument proceeds in three steps; at rank~$1$ it reduces to the
rigorous Paper~38 Appendix~A computation, and at rank~$\ge 2$ it is
given here at sketch level (the detailed Abel--Plana bookkeeping is
not reproduced).
...
rank-$1$ first-moment integrals of the $\SU(2)$ type of Paper~38 ---
the $\sqrt{2j+1}$-weighted kernel against the $\sin^2(\chi/2)$
conjugacy-class measure --- one per positive-root direction. (These
are \emph{not} the classical circle Fej\'er integrals
$\int_{\T^1}F_n|\theta|$, whose probability-normalised constant is
$2/\pi$, half the $\SU(2)$ value; Paper~38, Remark ``Connection to
the circle Fej\'er estimate''.)
...
positive roots). What remains is the rank-$1$ ($\SU(2)$) constant
$4/\pi$ of Paper~38, Appendix~A, from each positive-root direction's
Euler--Catalan asymptote $\sum_{d \text{ odd}} 1/d^2 = \pi^2/8$.
...
standard but tedious at general rank; carrying it out to the rigor
level of the rank-$1$ Stein--Weiss analysis in Paper~38 Appendix~A,
uniformly in rank, is a named gap. At rank~$\ge 2$ the cancellation
is corroborated numerically (recovered Weyl-integration drivers, see
the backing test); the extraction is fit-sensitive, so the robust
conclusion is Reading~A over Reading~B rather than the exact value.
\end{proof}
...
\begin{corollary}[Extension to compact connected semisimple
...
connected simple $G$ to compact connected semisimple $G$ (finite
quotients of products of simple factors) via factor-wise
cancellation.
...
of factors in the additive distance squared). Iterating across the
simple factors gives the universal rate constant on the full
semisimple product.
\end{proof}

\emph{Scope (2026-09-02).} An earlier version of this corollary
extended the statement to $G_{\mathrm{ss}} \times T^k$ with a central
torus factor, on the ground that ``the rank-$1$ constant is $4/\pi$ on
each circle''. That ground is false:\ on a circle factor the central
Fej\'er kernel reduces to the classical Fej\'er kernel, whose
probability-normalised first moment is $(2/\pi)\log n / n$ --- half the
$\SU(2)$ value (Paper~38, Remark ``Connection to the circle Fej\'er
estimate''; \texttt{tests/test\_trunk\_qa\_fejer\_4\_over\_pi.py}). Under
the factor-wise argument above the product constant would then depend
on the mix of simple and circle factors rather than being universal,
so the torus-factor extension is withdrawn; no value is asserted here
for $c(G_{\mathrm{ss}} \times T^k)$.
\end{corollary}

\begin{remark}[Relation to the circle Fej\'er constant]
\label{rem:stein_weiss_general}
The constant $4/\pi$ in~\eqref{eq:universal_rate} is \emph{twice} the
constant of the classical estimate for the probability-normalised
Fej\'er kernel on the circle~\cite{zygmund2002}:\
$\frac{1}{2\pi}\int_{\T^1} F_n(\theta)\,|\theta|\,d\theta \sim
(2/\pi)\,\log n / n$ (Paper~38, Remark ``Connection to the circle
Fej\'er estimate'', where the circle constant is computed in closed
form and measured against the $\SU(2)$ value:\
\texttt{tests/test\_trunk\_qa\_fejer\_4\_over\_pi.py}). The
$\sqrt{\dim V_\pi}$ Plancherel weighting in
...
\Lammax$ rate (rather than the $1/\Lammax$ rate of the abelian torus
in~\cite{leimbach_vs2024}). \textbf{[OBSERVATION]} Both constants are
of the form $2\Vol(\text{base})/\Vol(\text{group})$
($2\Vol(S^0)/\Vol(S^1) = 2/\pi$; $2\Vol(S^2)/\Vol(\SU(2)) = 4/\pi$);
the universality across the semisimple class is the statement that
every simple factor sees the rank-$1$ ($\SU(2)$) constant through the
Plancherel $\times$ Vandermonde cancellation, discussed further in
\S\ref{sec:universal}. (An earlier version of this remark stated the
circle constant as $4/\pi$ and the $\SU(2)$ constant as ``preserved
across this transition by the underlying real-line Stein--Weiss
analysis''; withdrawn 2026-09-02.)
\end{remark}
...
\begin{lemma}[L3: Lipschitz comparison on compact semisimple Lie groups]\label{lem:L3}
...
(Corollary~\ref{cor:L3_closure_at_all_ranks}) establishes the
\emph{asymptotic-tight} $C_3(G) = 1$ leg rigorously at all ranks (via
the PRV-summand bound of Kumar, which is genuinely rank-uniform) on
every compact connected simple Lie group $G$. The \emph{interior} (INT)
leg of Lemma~\ref{lem:L3} is proved analytically and verified by
exhaustive numeric panels at the tested ranks ($A_2, A_3, C_2, G_2$);
its rank-uniform case-bookkeeping for the higher exceptional ranks
($F_4, E_6, E_7, E_8$) carries through the same analytical skeleton but
is a named follow-on (Remark~\ref{rem:numerical_certification_interior}).
...
in~\cite{mathieu1989, polo1994}). Part~(b) follows from the standard
dominance-order fact~\cite[\S13]{humphreys1972}:\ for
...
of inner products that is non-negative by the standard Weyl-dominance
lemma (the dominant Weyl-orbit representative maximises
$\langle\cdot,\rho\rangle$; Bourbaki~\cite{bourbaki_lie_8}) and the
...
standard dominance-order fact~\cite[\S13]{humphreys1972}
...
common skeleton:\ Lemma~1 (weight-containment), the Weyl-dominance
lemma (the dominant orbit representative maximises
$\langle\cdot,\rho\rangle$; Bourbaki~\cite{bourbaki_lie_8}), and the
dual-Coxeter positivity of the inverse-Cartan matrix. The Brauer--Klimyk cancellation only adds
...
and not required for the state-space GH convergence theorem.
\end{remark}
...
% ---------------------------------------------------------------------
\subsection{Lemma L5: state-space GH assembly via the approximation pair}\label{sec:L5}
% ---------------------------------------------------------------------

We bound van~Suijlekom's state-space Gromov--Hausdorff distance
$\Lambda_{\mathrm{prop}}(\Tcal_\Lammax, \Tcal_G)$
(\S\ref{sec:propinquity_convention}) with an approximation pair
$(B_\Lammax, P_\Lammax)$, $B_\Lammax: \Tcal_G \to \Tcal_\Lammax$ a
unital completely positive (UCP) map and $P_\Lammax$ the spectral
compression, exactly as Paper~38's Lemma~L5 does for $\SU(2)$. For
orientation only:\ in Latr\'emoli\`ere's framework the same pair would
be the data of a tunnel $\tau$ between the two triples, and the
propinquity would be
\begin{equation}\label{eq:propinquity_def_general}
   \inf_\tau \mathrm{length}(\tau), \qquad
   \mathrm{length}(\tau) = \max(\mathrm{reach}(\tau),
   \mathrm{height}(\tau));
\end{equation}
that quantity is \emph{not} what is proved here (the dual-reach
hypothesis is the named gap). The four tunnel terms are recorded below
for what a Latr\'emoli\`ere tunnel would require;\ the state-space GH
bound itself is \emph{not} obtained from this pair (see the proof), but
from the lifted-state route of Remark~\ref{rem:correction_L2}.

\begin{lemma}[L5: state-space GH bound from the approximation pair]\label{lem:L5}
The pair $(B_\Lammax, P_\Lammax)$ with $B_\Lammax$ from
Definition~\ref{def:berezin_general} and $P_\Lammax$ the spectral
projection of~\eqref{eq:casimir_cutoff} is an approximation pair between
$\Tcal_G$ and $\Tcal_\Lammax$. The state-space GH bound
\begin{equation}\label{eq:L5_bound_general}
   \Lambda_{\mathrm{prop}}(\Tcal_\Lammax, \Tcal_G)
   \;\le\; C_3(G) \cdot \gamma_\Lammax(G),
\end{equation}
with $C_3(G) \le 1$ from Lemma~\ref{lem:L3} and $\gamma_\Lammax(G)$
from Lemma~\ref{lem:L2}(d), holds unconditionally for $G = \SU(2)$
(Paper~38, \texttt{thm:main\_unconditional}) and, for general $G$,
conditionally on the per-group lifted-state decomposition of
Remark~\ref{rem:correction_L2}. \emph{The pair does not supply an
independent proof} (corrected 2026-09-04):\ of its four tunnel
constituents, $\mathrm{reach}_B \le \gamma_\Lammax(G)$ is proved
(Lemma~\ref{lem:L4}(c)), $\mathrm{reach}_P$ is a named gap, and both
height constituents are refuted (\eqref{eq:height_one_general}).
\end{lemma}

\begin{proof}
We record the four quantities a Latr\'emoli\`ere tunnel would carry ---
reach (forward and dual) and height (forward and dual). Paper~38's
Lemma~L5 organised its assembly (since withdrawn) by the two forward ones;\
what survives of that lemma is the record below, not a proof.
...
\emph{Height.} The height-type quantity (Latr\'emoli\`ere's height for
metric spectral triples~\cite{latremoliere_metric_st_2017}) measures the
Lipschitz-distortion of the pair's maps with respect to the Lipschitz
seminorms,
...
[retracted 2026-09-04: l5-height-bound-achieved].\ The text printed here
until that date concluded $\mathrm{height}_B \le \gamma_\Lammax(G)$ (from
the good-kernel estimate) and $\mathrm{height}_P = 0$ (from orthogonality
of $P_\Lammax$). Both are false, at \emph{every} rank, by the witness
Paper~38~\cite{paper38} records for the rank-one case, whose argument is
independent of $G$:\ $B_\Lammax$ is a finite-band reconstruction, so the
unit-Lipschitz ball contains an $f$ with $B_\Lammax(f) = 0$, for which the
quantity in \eqref{eq:height_B_general} equals $\Lipnorm{f} = 1$. Hence
\begin{equation}\label{eq:height_one_general}
   \mathrm{height}_B \;\equiv\; 1
   \quad\text{at every cutoff and every rank,}
\end{equation}
and $\mathrm{height}_B \le \gamma_\Lammax(G)$
[retracted 2026-09-04: l5-height-bound-achieved] fails whenever
$\gamma_\Lammax(G) < 1$;\ the same witness gives
$\mathrm{height}_P \ne 0$.

\emph{The conclusion \eqref{eq:L5_bound_general} is unaffected}, for the
reason it is unaffected in Paper~38:\ the state-space GH assembly bounds
$\Lambda_{\mathrm{prop}}$ using reach-type estimates alone, with no height
constituent. What is withdrawn is the height leg of the four-term
organisation above --- retained because it names what a Latr\'emoli\`ere
tunnel would additionally require --- and not the bound. Whether a
\emph{height-controlled} assembly exists for this pair at general rank is
open, and Theorem~\ref{thm:main} must not be cited as height-controlled.

\emph{Assembly.} Until 2026-09-04 this proof closed with
$\Lambda_{\mathrm{prop}} \le \max(\gamma, \gamma, \gamma, 0) = \gamma$
[retracted 2026-09-04: l5-height-bound-achieved], whose third and fourth arguments are the two refuted
height constituents, and declared ``all other constituents proved
above''. Withdrawn:\ with the heights refuted and $\mathrm{reach}_P$ a
named gap, the pair establishes nothing toward
\eqref{eq:L5_bound_general} on its own. The bound holds by the
lifted-state route --- Paper~38's \texttt{thm:main\_unconditional} at
$G = \SU(2)$, which uses neither $B_\Lammax$ nor any dual reach, and its
per-group extension of Remark~\ref{rem:correction_L2} at general $G$,
conditional on the spin-window decomposition there --- which is
precisely the scope Theorem~\ref{thm:main} states. Whether a
height-controlled or dual-reach-controlled assembly of this pair exists
is open.
\end{proof}
...
\begin{theorem}[Unified GH-convergence on compact semisimple Lie groups]\label{thm:main}
Let $G$ be a compact connected semisimple Lie group of rank $r \ge 1$
with bi-invariant Riemannian metric in the dual-Coxeter normalisation, and
...
van~Suijlekom's state-space Gromov--Hausdorff distance (unconditionally
for $\SU(2)$; for general $G$ conditional on the per-group spin-window
lifted-state decomposition of Remark~\ref{rem:correction_L2}):
...
dual-Coxeter normalisation. (Tier:\ rigorous at rank~1; for general
$G$ a proof-sketch mechanism numerically pinned at ranks~1--2 plus a
rank-3 datapoint, the fully rank-uniform analytical proof being a
named gap --- see Theorem~\ref{thm:universal_constant_analytical} and
\S\ref{sec:L2}.)
\end{enumerate}
\end{theorem}
...
the four properties a Latr\'emoli\`ere tunneling pair would require; and---in
the \emph{withdrawn} route only---Lemma~\ref{lem:L5} the assembly into the
state-space GH bound.

\emph{Which route establishes this.}\ Not the L5 assembly:\ its dual reach is a
named gap and both height constituents are refuted
(\S\ref{sec:L5}, corrected 2026-09-04), so the approximation pair supplies no
independent proof.  The bound~\eqref{eq:L5_bound_general} holds on the
lifted-state route inherited from Paper~38 --- UCP compression plus the
conjugation average, using no Berezin map, no partial inverse and hence no
height term --- and combining it with the asymptotic
...
The state-space Gromov--Hausdorff limit of the sequence
$(\Tcal_\Lammax)_{\Lammax > 0}$ (in van~Suijlekom's distance; the
strictly stronger Latr\'emoli\`ere propinquity limit is not claimed) is
the round-$G$ canonical metric
...
\Tcal_G) \to 0$, so the limit object exists in the state-space GH
...
\begin{equation}\label{eq:su2_4_over_pi}
   ...
   \;=\; 2\Vol(S^2)/\Vol(\SU(2)),
\end{equation}
where the last expression is a quotient of unit-radius sphere volumes.
(Paper~38 originally read this as the Hopf fibration's base-to-total ratio;\
that label was withdrawn on 2026-09-03 --- the Hopf base is $S^2(r/2)$ and its
ratio is $1/(2\pi)$ --- without affecting the identity above.)  Two natural
rank-$r$ generalisations
were proposed in the supporting scoping
memo~\cite{unified_gh_scoping_memo}:

\paragraph{Reading A (rank-invariant).}
The constant is rank-invariant:\ $c(G) = 4/\pi$ for every compact
connected semisimple Lie group $G$ in the dual-Coxeter normalisation,
...
Universality is not coincidental. The following cancellation theorem
(Theorem~\ref{thm:universal_constant_analytical} above) is the
proposed mechanism --- rigorous at rank~$1$ (Paper~38 Appendix~A) and
given as a proof sketch at rank~$\ge 2$, where it is corroborated
numerically (the fully rank-uniform write-up is a named gap).
...
What is left is the rank-$1$ ($\SU(2)$) constant $4/\pi$ from each
...
Paper~38's rank-$1$ sum-rule calculation for $\SU(2)$
\cite[App.~A]{paper38}. The rank-$1$ asymptotic
\begin{equation*}
   \gamma_n^{\SU(2)}
   \;\sim\; \frac{4}{\pi} \cdot \frac{\log n}{n}
\end{equation*}
is then applied to each of the $N_+$ positive-root directions, with the
...
than to a rank-dependent product. (The constant is that of the
rank-$1$ \emph{group} $\SU(2)$, not of a circle:\ the classical
Fej\'er kernel on $\T^1$ has probability-normalised first moment
$(2/\pi)\log n/n$, half the $\SU(2)$ value --- Paper~38, Remark
``Connection to the circle Fej\'er estimate''. An earlier version of
this paragraph displayed the circle asymptotic with constant $4/\pi$;
withdrawn 2026-09-02.)

\paragraph{Structural reading.} The central spectral Fej\'er kernel
sees the rank-$1$ ($\SU(2)$) asymptotic analysis through
Peter--Weyl, \emph{not} through the global geometry of $G/T$.
Whichever compact semisimple Lie group you build the kernel on, the Plancherel
...
$2$ would close this in a bounded sub-sprint. The structural
...
(Theorem~\ref{thm:universal_constant_analytical}) supplies the
\emph{mechanism} for $c(G) = 4/\pi$ at all ranks --- rigorous at
rank~$1$, a proof sketch at rank~$\ge 2$ (the fully rank-uniform
write-up is a named gap). Empirically, we have verified this at
...
$G/K$ (Class~2). The state-space GH convergence theorem at full Class~2
...
convention, and may admit a different convergence theory entirely. The
...
should extend from Paper~39 along its \emph{lifted-state} route, which
uses neither a Berezin map nor a partial inverse and so has no
manifold-specific step to generalise. We have not written out the details.
\emph{(Corrected 2026-09-07:\ this paragraph previously proposed extending
``with the Pythagorean Leibniz constant generalising to a
Cartan-product-type bound''. That refinement is \textbf{withdrawn} in
Paper~39 --- graded anticommutation does not give operator-norm
orthogonality --- so the extension was aimed at an abandoned route. The
$C_3^{(2)} \le \sqrt2$ constant of that route survives with its value
unchanged but is not a rate constant of Paper~39's theorem.)}
...
appearing as the leading Szeg\"o-type asymptotic (on the semisimple
class). The detailed
generalisation is open and would constitute a Szeg\"o companion to
the present convergence theorem.
...
Latr\'emoli\`ere, Matilde Marcolli, Eva-Maria Hekkelman, Edward McDonald,
...
\newblock \emph{Space and time dimensions of algebras with applications
...
\newblock preprint arXiv:1811.10843, 2018.
...
\bibitem[Humphreys(1972)]{humphreys1972}
J.~E.~Humphreys,
\newblock \emph{Introduction to Lie Algebras and Representation Theory},
\newblock Graduate Texts in Mathematics \textbf{9}, Springer, New York, 1972
(the dominance partial order:\ \S10.1, \S13.2).
