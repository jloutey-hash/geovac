...
program (Sprint L3b\footnote{Sprint/track designations are anchors into the project's internal chronicle (\texttt{CHANGELOG.md} in the repository).}, see~\S~\ref{sec:open}) would build. Honest scope:
...
Leimbach--van~Suijlekom~2024~\cite{leimbach_vs2024} are strictly
Riemannian.
\end{abstract}
...
of May 2026:\
\begin{enumerate}\setlength{\itemsep}{2pt}
\item[(a)] \textbf{Riemannian state-space GH convergence on spectral
...
Hekkelman--McDonald~\cite{hekkelman_mcdonald2024b},
...
The propagation number~\cite{connes_vs2021} (their \S 2.7, ``Propagation number as an invariant of operator systems'') of an
...
Connes--vS~\cite{connes_vs2021}~(\S 4) establishes that the
...
four lemmas (L1$'$ chirality-doubled operator system, L2 central
spectral Fejér kernel, L3 Lipschitz spinor bound, L4 Berezin
reconstruction) together with the lifted-state construction.  A fifth
(L5, the Latrémolière propinquity assembly) belonged to the original
architecture and was \emph{withdrawn} on 2026-09-03 when its height leg
was refuted;\ it is not available as a template. The
...
Riemannian, as are
Hekkelman--McDonald~\cite{hekkelman_mcdonald2024b} and
Farsi--Latr\'emoli\`ere~\cite{farsi_latremoliere2024,
...
an explicit joint rate. This reduces L3b to a tractable bounded
sprint (\texttt{debug/l3\_scoping\_memo.md}~\S~6). The
compactified-temporal target is then the natural setting for a
Lorentzian-propinquity assembly that inherits the surviving Paper~38
lemmas factor-by-factor, modulo Krein-positivity conventions at the
Berezin reconstruction step.

\emph{Status (2026-09-07).}\ This recommendation was written in May 2026
and is \textbf{superseded} in two ways.  First, the fifth Paper~38 lemma
it would have inherited is withdrawn, so an assembly route can no longer
inherit a reach/height bookkeeping.  Second, and more decisively, the
compactification it recommends is now understood to be the obstruction
rather than the device:\ Paper~45's $K^{+}$ annihilation theorem shows
the natural Krein-positive compression fails structurally, and the
truncated Bisognano--Wichmann boost has \emph{integer} modular spectrum,
so the finite-cutoff modular flow is a compact $\beta = 2\pi$ circle and
every Lipschitz/boost seminorm built on it is signature-blind.  Replacing
$\R_t$ by $S^1_T$ therefore buys tractability at the cost of the very
structure being sought.  The general form of the obstruction:\ signature
is a non-compact phenomenon, while quantum-metric convergence is a
finite-dimensional-approximant phenomenon, and finite-dimensionality
forces discrete spectrum --- so \emph{every} finite truncation
compactifies the modular flow.  The remaining routes are not better
seminorms:\ they are moving signature to the state/modular level, or
adopting a genuinely Lorentzian distance notion (time-separation /
Lorentzian length spaces) rather than a metric one.
...
bounded extension of the present module.
...
\cite{connes_vs2021}~\S 4). Targeting the full
...
finite-cutoff closures with the continuum problem --- a substantial
open program --- named explicitly and left open.
...
$\sthree = \SU(2)$ at the state-space
Gromov--Hausdorff level (unconditional, rate constant $4/\pi$); Paper~40~\cite{paper40_unified} closed it on the universal
...
applications to Lorentzian noncommutative geometry and quantum
electrodynamics},
...
\newblock \emph{Lorentzian metric spaces and Gromov--Hausdorff convergence: the unbounded case},
...
tensor-product Camporesi--Higuchi spectral triples on $\sthree \times \sthree$},
\newblock GeoVac internal preprint, papers/standalone (2026),
DOI via Zenodo.
