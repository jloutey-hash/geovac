% Paper 29: Finite-Size Graph-RH for the GeoVac Hopf Graphs
...
\title{Finite-Size Graph-RH for the GeoVac Hopf Graphs:\\
Ihara Zeta, Bound-Crossing, and a Scope Boundary on Selberg-on-Hydrogen}
...
Dirac-$S^3$ Rule B $n_{\max}=3$ graph under $m_j\to-m_j$; this magnetic
reflection is a fixed-point-free (Kramers) graph automorphism, whereas
its chirality companion $\chi\to-\chi$ is not a graph symmetry at all
--- the two spinor-bundle chiralities differing in size by the discrete
Dirac index $n_{\max}(n_{\max}+1)$, which explains the failure of
relativistic $\mathbb{Z}_2$ tapering.
...
the irreducible $S_{\min}\approx 2.47994$~\cite{paper28}.
...
Paper~25~\cite{paper25}.  They are the GeoVac instance of the
Marcolli--van~Suijlekom gauge-network graph-spectral-triple
framework~\cite{marcolli_vs2014} (with the Perez-Sanchez
correction~\cite{perez_sanchez2025} on the continuum
limit).  We summarize their combinatorial data.
...
\begin{observation}
\label{obs:ramanujan}
At the small (closed-form) cutoffs of this section ($n_{\max} \le 4$ on
$S^3$, $N_{\max} \le 3$ on $S^5$), the largest non-trivial
Hashimoto eigenvalue is strictly smaller than the Kotani--Sunada bound:\
these graphs are strictly sub-Ramanujan, their spectral gap exceeding
the optimal Ramanujan gap of an equivalent-$q_{\max}$ regular graph.
This is a \emph{finite-size} statement:\ the larger-cutoff sweep of
\S\ref{sec:open_questions} (RH-D) shows three of four families
(including $S^3$ Coulomb at $n_{\max} = 5$) \emph{cross} the bound, so
the property does not persist asymptotically.
\end{observation}
...
bound asymptotically are in this sense \emph{optimal}.  The closed-form
data of this section do not by themselves establish the asymptotic
behavior of the deviation; the larger-cutoff sweep in
Section~\ref{sec:open_questions} (RH-D) resolves it --- the signed
deviation crosses the bound for three of four families, so the graphs
are \emph{not} asymptotically Ramanujan (the graph-RH here is a
...
non-trivial Ihara content lives in the $\ell=1$ component}, a connected
graph on $6$ vertices (the $m\in\{-1,0,1\}$ states of the $n=2,3$ shells)
with $7$ edges --- consistent with the degree-$14$ inverse zeta
($\deg\zeta_G^{-1} = 2E = 14$) and first Betti number
$\beta_1 = E - V + 1 = 2$ (the two independent cycles visible in the
factorisation).  The cyclotomic
factors $s^2 \pm s + 1$ in~(\ref{eq:s3_closed}) have zeros at primitive
sixth roots of unity (magnitude 1); the irreducible cubics
$2s^3 \pm s^2 + s \pm 1$ have
zeros at $|s|\approx 0.739$ (real) and $|s|\approx 0.823$ (complex),
corresponding to longer closed walks through the component.
...
give the Ihara polynomial degree $34+46=80$; the Hashimoto matrix has
size $2|E|=84$, the remaining $4$ being zeros at $s=\infty$ from the
degree-1 leaves.
...
Observation~\ref{obs:12_22} is validated by direct restriction of
$M(s)$ to each reflection block in exact sympy rational arithmetic
(\texttt{tests/test\_hopf\_u1\_block.py}, Track RH-E):\ each block
determinant equals the stated factor polynomial, and the product
$\det(M_{\text{sym}})\cdot\det(M_{\text{antisym}})$ equals the full
$\det(M)$ symbolically.

\begin{observation}[Magnetic reflection is Kramers; chirality reflection is Dirac-index--obstructed]
\label{obs:chirality_obstruction}
The $m_j\to-m_j$ reflection of Observation~\ref{obs:12_22} is a
\emph{fixed-point-free graph automorphism} of the Dirac-$S^3$ graph:
its adjacency depends only on $|\Delta m_j|$, and half-integer $m_j$
never vanishes, so every node is paired with a distinct partner --- the
combinatorial signature of Kramers degeneracy.  Consequently the flat
$\mathbb{Z}_2$ connections (spin structures, $H^1(G;\mathbb{F}_2)$)
left invariant by this reflection form a large subspace of the cycle
space (dimension $40$ of $79$ at $n_{\max}=3$).

The companion \emph{chirality} reflection $\chi\to-\chi$ --- equivalently
$\kappa\mapsto$ its opposite-sign partner at fixed $\ell$ --- is by
contrast not a graph automorphism at all, indeed not even a node
bijection.  At each orbital block $(n,\ell)$ the positive-chirality
multiplet $\bigl(\kappa=-(\ell+1),\ j=\ell+\tfrac12\bigr)$ carries
$2\ell+2$ states while the negative-chirality multiplet
$\bigl(\kappa=+\ell,\ j=\ell-\tfrac12\bigr)$ carries $2\ell$ --- a
mismatch of $+2$ in \emph{every} block, never cancelling.  The total
chirality imbalance is
\[
  \#\{\chi=+1\}-\#\{\chi=-1\}
  \;=\;\sum_{(n,\ell)} 2 \;=\; n_{\max}(n_{\max}+1),
\]
the discrete spectral asymmetry (Dirac index) of the
Camporesi--Higuchi spinor bundle.  No permutation can swap sectors of
unequal size, so the genuine-spin $\mathbb{Z}_2$ does not exist on the
substrate, while the magnetic $m_j$ reflection thrives.
\end{observation}

Observation~\ref{obs:chirality_obstruction} was verified in exact
$\mathbb{F}_2$ arithmetic
(\texttt{tests/test\_spin\_structure\_obstruction.py},
\texttt{debug/spin\_structure\_moduli\_memo.md}): the $m_j$ permutation
is a fixed-point-free automorphism at $n_{\max}=2,3$, and the
$(2\ell+2)$-vs-$2\ell$ block mismatch holds at $n_{\max}=2,3,4$ with
total $n_{\max}(n_{\max}+1)$.  It supplies the substrate-level reason
behind the negative relativistic-$\mathbb{Z}_2$ tapering results
(\texttt{debug/sprint\_mj\_parity\_rotated\_memo.md}): a chirality
sign-count cannot stabilize a Hamiltonian when the underlying
reflection is not a graph symmetry to begin with.
...
\begin{corollary}[Integer-algebraicity of the reciprocal Ihara zeros]
\label{cor:int_alg}
Every non-trivial \emph{reciprocal} Ihara zero of a GeoVac Hopf graph
--- i.e.\ every nonzero eigenvalue of the Hashimoto edge operator $T$
--- is an algebraic integer (a root of a \emph{monic}
$\mathbb{Z}$-coefficient polynomial). The Ihara zeros themselves are
algebraic over~$\mathbb{Q}$ (Corollary~\ref{cor:pi_free}) but are
\emph{not} in general algebraic integers.
\end{corollary}

This sharpens Corollary~\ref{cor:pi_free} from ``algebraic over~$\mathbb{Q}$''
to ``algebraic integer'' \emph{for the reciprocal zeros}, and is direct from
the Hashimoto formulation: $T$ is a $0/1$ edge-adjacency matrix, so its
characteristic polynomial $\det(\lambda I - T)$ is \emph{monic} with integer
coefficients, and its eigenvalues (the reciprocal Ihara zeros) are therefore
algebraic integers. The strengthening does \emph{not} pass to the Ihara zeros
themselves:\ they are the roots of the Ihara--Bass determinant
$\det(I - sA + s^2 Q)$, which has integer coefficients but leading coefficient
$\det Q = \prod_v (\deg v - 1) \neq \pm 1$ in general --- so the zeros are
algebraic over~$\mathbb{Q}$, not algebraic integers. (Concretely, the
closed-form factor $2s^2 + 1$ of Section~\ref{sec:closed_forms} has Ihara zeros
$\pm i/\sqrt{2}$, algebraic but not algebraic integers; their reciprocals
$\mp i\sqrt{2}$, roots of the monic $s^2 + 2$, are the $T$-eigenvalues and
\emph{are} algebraic integers.)
...
\item Cyclotomic factors $\Phi_3(s), \Phi_4(s), \Phi_6(s)$ appearing
  ...
  $C_3 \times P_2$ ladder).
\item Ramanujan-boundary factors $(2s^2+1)^2$ (scalar $S^5$
...
of irregularity, not a pathology of our graphs.  The correct $\Xi$-completion for the
irregular case is expected to be a product over the degree strata of
the graph, rather than a single regular-graph correction --- the
natural generalisation of the Stark--Terras regular-graph functional
equation along the lines of the irregular Ihara--Bass machinery of
Kotani--Sunada~\cite{kotani2000}.  Making that completion explicit for
our two graphs is a useful computational exercise but is not required
to state the Ramanujan property, which is already secured by
...
dense families.  The finite-size sign flip (Ramanujan at small size,
crossing at $V\approx 30$--$60$) is recomputed from the live graph
spectra in \texttt{tests/test\_paper29\_bound\_crossing.py}; full data
at \texttt{debug/data/alon\_boppana\_sweep.json}.
...
cases.  The Matsuura--Ohta Kazakov--Migdal / Ihara-zeta program on
generic finite graphs~\cite{matsuura2025,matsuura_ohta2024_phases}
provides a parallel direction in which gauge-theoretic duality
structures on the graph may shadow-match the Hopf-$U(1)$ block
decomposition recorded here; the connection is noted as a forward
pointer, not pursued in the present work.
...
(Sprint~4, RH-P).}  The depth-1 identity
of Paper~28~\cite{paper28} has no depth-2 analog for the CG-weighted irreducible
...
\bibitem{paper1} GeoVac Project.  \emph{Paper 1: The Geometric Atom:
Atomic Structure as a Packing Problem.}  GeoVac Project, ongoing.
\bibitem{paper7} GeoVac Project.  \emph{Paper 7: The Dimensionless
Vacuum: Recovering the Schr\"odinger Equation from Scale-Invariant
Graph Topology.}  GeoVac Project, ongoing.  18 symbolic proofs of the
$S^3$ conformal equivalence.
\bibitem{paper14} GeoVac Project.  \emph{Paper 14: Structurally Sparse Qubit Hamiltonians from
Spectral Graph Theory.}  GeoVac Project, ongoing.
\bibitem{paper18} GeoVac Project.  \emph{Paper 18: Spectral-Geometric
Exchange Constants: Projecting Discrete Spectra onto Continuous
Manifolds.}  GeoVac Project, ongoing.
\bibitem{paper24} GeoVac Project.  \emph{Paper 24: The Bargmann--Segal
Lattice: $\pi$-Free Discretization of the Harmonic Oscillator on
$S^5$.}  GeoVac Project, 2026.
\bibitem{paper25} GeoVac Project.  \emph{Paper 25: The Hopf Graph as a Lattice Gauge
Structure.}  GeoVac Project, April 2026.
...
\bibitem{bander1966} M. Bander and C. Itzykson.  Group Theory and the
Hydrogen Atom (I).  \emph{Rev.\ Mod.\ Phys.}\ \textbf{38}, 330 (1966).
...
on graphs and the Ihara zeta function.  \emph{JHEP}\ \textbf{09}
(2022) 178;
...
\bibitem{matsuura_ohta2024_phases} S. Matsuura and K. Ohta.  Phases
and duality in the fundamental Kazakov--Migdal model on the graph.
arXiv:2403.07385 (2024).  Extends the Matsuura--Ohta
Kazakov--Migdal / Ihara-zeta program to a duality structure on
generic finite graphs; the duality may shadow-match the
Hopf-$U(1)$ block decomposition documented in
Section~\ref{sec:closed_forms}.

\bibitem{yakaboylu2024} E. Yakaboylu.  Hamiltonian for the
Hilbert--P\'olya conjecture.
\emph{J.\ Phys.\ A: Math.\ Theor.}\ \textbf{57}, 235204 (2024);
arXiv:2309.00405.  A current-generation Hilbert--P\'olya candidate;
...
\bibitem{hmy2024} J. Huang, T. McKenzie, and H.-T. Yau.  Ramanujan
...
\emph{J.~Geom.~Phys.}~\textbf{75}, 71--91 (2014); arXiv:1301.3480.

\bibitem{perez_sanchez2025} C.~I.~Perez-Sanchez.
