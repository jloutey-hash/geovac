...
= (4/\pi + o(1)) \log \nmax / \nmax
\)
in the rotation-angle normalisation of the $\SU(2)$ metric (equivalently $2/\pi$ on the unit round $\sthree$; the constant scales with the metric, 2026-09-03 convention note)
\(
\)
...
$\gamma_n \sim (4/\pi)\log n / n$ (rotation-angle normalisation; $2/\pi$ on the unit $\sthree$);\ (L3) \textbf{[PANEL-VERIFIED]} a
...
\begin{equation}\label{eq:main_rate}
   ...
   \;\le\; C_3 \cdot \gamma_{\nmax}, \qquad
   \gamma_{\nmax} = \frac{4 \log \nmax}{\pi\,\nmax} + O\!\bigl(1/\nmax\bigr)
   \quad\text{(rotation-angle normalisation; $2/\pi$ on the unit $\sthree$)},
\end{equation}
...
\textbf{[OBSERVATION]} The asymptotic constant reads
$4/\pi = \Vol(S^2)/\pi^2$ in the rotation-angle normalisation and
$2/\pi = \Vol(S^2)/\Vol(\SU(2))$, the plain $\SU(2)/U(1)$ Haar ratio,
on the unit $\sthree$ (\emph{metric convention, 2026-09-03:} $\gamma_{\nmax}$ as computed in Lemma~\ref{lem:L2} weights the kernel by the $\SO(3)$ rotation angle $\chi \in [0, 2\pi]$, which is \emph{twice} the geodesic distance $\theta = \chi/2$ on the unit round $\sthree$; the constant is $4/\pi$ in that rotation-angle normalisation --- the round $\sthree$ of radius~2 --- and $2/\pi$ on the unit $\sthree$ of \S\ref{sec:setup}, the two differing by the metric scale, which enters a distance moment linearly; the theorem holds in either normalisation, \texttt{tests/test\_p38\_metric\_convention.py}).
\end{theorem}
...
in the rotation-angle normalisation the constant is $4/\pi = \Vol(S^2) /
\pi^2$, where $\Vol(S^2) = 4\pi$ is the standard round volume of the Hopf
base $S^2 = \SU(2)/U(1)$ and $\pi^2 = \Vol(\SU(2))/2$; on the unit
$\sthree$ the constant is exactly the Haar-normalisation ratio
$\Vol(S^2)/\Vol(\SU(2)) = 2/\pi$ --- the factor $2$ between the two is the
metric scale (rotation angle $= 2 \times$ geodesic distance), not a
geometric signature; \textbf{[OBSERVATION]} the same ratio $2/\pi$ is the
...
rotation-angle normalisation) identifies as the $k=0$ sub-case (M1,
...
\;=\; \frac{4}{\pi}\,\frac{\log \nmax}{\nmax} + O(1/\nmax)
\end{equation}
in the rotation-angle normalisation of Lemma~\ref{lem:L2}, i.e.\
...
\begin{equation}\label{eq:gamma_def}
   ...
   \;:=\; \int_{\SU(2)} \KS_{\nmax}(g)\, \chi(g)\, dg,
\end{equation}
where $\chi(g) \in [0, 2\pi]$ is the class (rotation) angle of
$g$, i.e.\ $\chi(g) = 2\, d_{\mathrm{round}}(e, g)$ on the unit
$\sthree$ ($g = \cos\theta + i \sin\theta\, \hat n \cdot \sigma$ has
eigenphases $\pm\theta$ and geodesic distance $\theta = \chi/2$ from
$e$; the characters below are $\sin((2j{+}1)\chi/2)/\sin(\chi/2)$).
\emph{Convention correction (2026-09-03):} the definition previously
wrote $d_{\mathrm{round}}(e, g)$ in place of $\chi(g)$ while every
computation (the sum rule, the module, the estimators) used $\chi$; the
moment against the unit-$\sthree$ distance is exactly $\gamma_{\nmax}/2$
at every $\nmax$ ($\KS_1 \equiv 1$ makes $\gamma_1 = \pi$, the diameter,
while the Haar-mean distance is $\pi/2$;
\texttt{tests/test\_p38\_metric\_convention.py}).  All constants below
are stated in the $\chi$ (rotation-angle) normalisation; halve them for
the unit $\sthree$.
Then:
\begin{enumerate}\setlength{\itemsep}{1pt}
...
\textbf{[OBSERVATION]} In the rotation-angle normalisation the $\SU(2)$
...
coincide at $2/\pi = \Vol(\text{base})/\Vol(\text{group})$ on both
($\Vol(S^0)/\Vol(S^1) = 2/(2\pi)\cdot 2$, hmm --- $2\Vol(S^0)/\Vol(S^1)
= 4/(2\pi) = 2/\pi$; $\Vol(S^2)/\Vol(\SU(2)) = 4\pi/(2\pi^2) = 2/\pi$), the
...
\textbf{[OBSERVATION]} The asymptotic constant identifies as
\begin{equation*}
   \frac{4}{\pi}
   \;=\; \frac{\Vol(S^2)}{\pi^2}
   \;=\; \frac{4\pi}{\pi^2}.
...
Cesaro/Fej\'er theory. \textbf{[OBSERVATION]} The asymptotic constant
$4/\pi$ is twice the circle Fej\'er first-moment constant $2/\pi$
(Remark~\ref{rem:circle_fejer}); we read the $\SU(2)$ value as the
Hopf-base measure factor $2\Vol(S^2)/\Vol(\SU(2))$ in the
$\SU(2)/U(1)$ Haar normalisation. (An earlier version of this
...
above:\ the rate constant $4/\pi$ (rotation-angle normalisation) is reported \emph{universal} across
the
...
behaviour. \textbf{[OBSERVATION]} Both constants are of the form
$2\Vol(\text{base})/\Vol(\text{group})$ --- $2\Vol(S^0)/\Vol(S^1) =
2/\pi$ and $2\Vol(S^2)/\Vol(\SU(2)) = \Vol(S^2)/\pi^2 = 4/\pi$ on the
$\SU(2)/U(1)$ Haar quotient --- but no derivation of the factor $2$
from the circle constant is given. (An earlier version of this
...
(rotation-angle normalisation; rigorous at rank~1, rank-uniform proof a
