...
$4/\pi = \Vol(S^2)/\pi^2$ in the rotation-angle normalisation, $2/\pi =
\Vol(S^2)/\Vol(\SU(2))$ on the unit $\sthree$ --- exhibiting the expected one-log slowdown of the
...
at every $\nmax$ ($\KS_1 \equiv 1$ makes $\gamma_1 = \pi$, exactly twice the
unit-$\sthree$ Haar-mean distance $\pi/2$ --- a mean, not a diameter;
...
\begin{remark}[Hopf-base measure interpretation of the constant]
\textbf{[OBSERVATION]} In the rotation-angle normalisation the asymptotic
constant identifies as
\begin{equation*}
   \frac{4}{\pi}
   \;=\; \frac{\Vol(S^2)}{\pi^2}
   \;=\; \frac{4\pi}{\pi^2},
\end{equation*}
and on the unit $\sthree$ as $2/\pi = \Vol(S^2)/\Vol(\SU(2))$.
The numerator $4\pi$ is the round volume of the Hopf base
$S^2 = \SU(2)/U(1)$. The denominator $\pi^2$ is half the round volume
of $\SU(2)$ in the bi-invariant metric (the factor of two being the
metric scale, 2026-09-03). The constant therefore lives
...
cutoff ($0.667$, $0.808$, $0.878$, $0.913$ at $\nmax = 2, 3, 4, 5$) while
$\gamma_{\nmax}$ falls, so it crosses the bound near $\nmax \approx 6$ ---
...
Cesaro/Fej\'er theory. \textbf{[OBSERVATION]} In the rotation-angle normalisation the
...
$2\Vol(S^2)/\Vol(\SU(2))$ in the rotation-angle normalisation. (An earlier version of this
...
rotation-angle normalisation; no common ``$2\Vol(\text{base})/\Vol(\text{group})$''
