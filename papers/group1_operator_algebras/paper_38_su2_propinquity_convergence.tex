...
Supporting structure is organized in five lemmas (the fifth since withdrawn 2026-09-03):\ (L1$'$) a truncated operator
...
contractive, an approximate identity, and L3-compatible;\ (L5, since \emph{withdrawn} 2026-09-03: the bound is established by the
unconditional lifted-state theorem, not this approximation-pair assembly).
\textbf{[OBSERVATION]} The
asymptotic constant is a quotient of sphere volumes ---
$4/\pi$ in the dual-Coxeter normalisation of \S\ref{sec:ch_triple},
$2/\pi = \Vol(S^2)/\Vol(\sthree)$ at unit radius, and \emph{not} the Hopf
fibration's base-to-total ratio (\S\ref{sec:L2}) --- exhibiting the expected one-log slowdown of the
...
$\le \gamma_{\nmax}$ outright and does not invoke $C_3$;\ the main
theorem's display now states that bound (until 2026-09-04 it carried a
$C_3 = 1$ factor inherited from the withdrawn Lemma~\ref{lem:L5} route).
...
dual-Coxeter normalisation) sits in the $k=0$ sub-case (M1, the
volume slot) of that classification; the same $1/\pi^2$ factor reappears
...
basis. Section~\ref{sec:lemmas} proves lemmas L1$'$--L4;\ Lemma~\ref{lem:L5}
is \emph{withdrawn} [retracted 2026-09-05: l5-assembly-listed-as-live-contribution].
Each surviving lemma is self-contained:\ Lemma~\ref{lem:L1prime} treats the
chirality-doubled operator system substrate; Lemma~\ref{lem:L2} the central
spectral Fej\'er kernel and its quantitative rate; Lemma~\ref{lem:L3} the
Lipschitz comparison bound with $C_3 = 1$; Lemma~\ref{lem:L4} the
Berezin reconstruction. Section~\ref{sec:main_theorem} assembles the
main theorem via the lifted-state route. Section~\ref{sec:discussion} discusses the role of the
...
($\le 3 \times
10^{-16}$;\ the $\nmax=2$ scalar window is frozen in
\texttt{tests/test\_p38\_action\_seminorm.py}, the across-bands sprint check
...
\;\le\; \gamma_{\nmax}
\;=\; \gvq{l2_rate_4_over_pi}{\frac{4}{\pi}}\,\frac{\log \nmax}{\nmax} + O(1/\nmax)
\end{equation}
...
metric uniformly repairs the pairing in the main theorem:\ the distance
$d$, the seminorms $L$ and $L_{\nmax}$, the moment $\gamma_{\nmax}$ and the
Monge--Kantorovich distances all scale together (a seminorm inversely, a
distance moment directly), so the inequality is unchanged in content ---
and, since what was printed before paired a \emph{unit}-metric seminorm
with a dual-Coxeter moment, the uniform statement is a factor-$2$
\emph{strengthening} of the printed bound rather than a restatement of
equal force.  Two scope limits:\ Lemmas~\ref{lem:L3}--\ref{lem:L5} remain
stated in the unit metric (Eq.~\eqref{eq:seminorm_normalisation}
converts), and the panel \emph{height} of \S\ref{sec:L5} is
scale-\emph{invariant} rather than degree one --- a difference of
seminorms measured on that seminorm's own unit ball --- so the panel
comparison there is normalisation-dependent and is not part of the
theorem.)  Identify $\sthree$
...
\end{equation}
This is a unital, \emph{odd}, real spectral triple of metric
dimension $3$ in the sense of Connes~\cite{connes1995}, with KO-dimension
$3 \pmod 8$.  \emph{Parity corrected 2026-09-03:} the text read ``even''
here, which Connes' table forbids --- the grading sign $\varepsilon''$ is
defined only in even KO-dimension, so KO-$3$ carries no chirality, and on
$\sthree$ the label $\mathrm{diag}(+1,-1)$ is $\operatorname{sign}(D)$, which
\emph{commutes} with $D$ and is therefore not a $\mathbb{Z}_2$ grading.
Paper~32 already carries this reading (``KO-3 is the label of the odd
continuum GV factor;\ $\sthree$ carries no chirality'') from the 2026-09-02
adjudication;\ that sweep did not reach this paper.  The unitary real
structure $J$ given by charge conjugation and the Lipschitz seminorm enter as
expected.  Because
...
\begin{equation}\label{eq:seminorm_normalisation}
   \Lipnorm{f} \;:=\; \opnorm{[\DCH, \Mat_f]} \;=\; \norm{\nabla_1 f}_{L^\infty},
   \qquad
   L(f) \;=\; \mathrm{Lip}_d(f) \;=\; \tfrac{1}{2}\,\Lipnorm{f},
\end{equation}
so $\Lipnorm{\cdot}$ denotes throughout the \emph{unit}-metric Dirac
seminorm, while $L$ --- the dual-Coxeter Lipschitz seminorm, equivalently
the translation seminorm of Definition~\ref{def:translation_seminorm} ---
is what metrizes every state space in the main theorem.  Lemmas
\ref{lem:L3}--\ref{lem:L5} below are stated in the unit metric (their
gradients are the unit-metric $\nabla_1$, as their polar-coordinate form
shows), and the conversion between the two is
Eq.~\eqref{eq:seminorm_normalisation};\ note that the good-kernel product
$\mathrm{Lip}_d(f)\,\gamma_d$ is invariant under the metric scale, since a
Lipschitz seminorm and a distance moment scale inversely, so an estimate
of that shape is normalisation-independent \emph{provided both factors are
...
\[
  ...
  \end{array}
\]
Normalised as printed, the panel entries rescale into the
unit-Lipschitz ball, so they are a subset of the set quantified over and a
lower bound on the supremum:\ an entry exceeding $\gamma_{\nmax}$ contradicts
the withdrawn bound.  The analytic witness of
\eqref{eq:height_identically_one} settles the question independently of the
panel.  The proof
...
direction;\ \textbf{[REFUTED AS STATED --- withdrawn 2026-09-03]}]\label{lem:L5}
...
\emph{(withdrawn:\ false by the same finite-band witness as
$\mathrm{height}_B$, see below)}.
\end{enumerate}
...
\end{equation}
\textbf{[REFUTED AS STATED --- withdrawn 2026-09-03.]}  The argument
printed here until that date concluded $\mathrm{height}_B \le
\gamma_{\nmax}$ and $\mathrm{height}_P = 0$.  Both are false, by a
one-line witness.  $B_{\nmax}$ is a finite-band reconstruction --- its
multiplier envelope is $N \le 2\nmax - 1$ --- so at every finite cutoff the
unit-Lipschitz ball contains $f$ with $B_{\nmax}(f) = 0$, and for any such $f$
the quantity in \eqref{eq:height_B_definition} equals $\Lipnorm{f} = 1$.
Hence
\begin{equation}\label{eq:height_identically_one}
   \mathrm{height}_B \;\equiv\; 1 \quad\text{at every } \nmax,
\end{equation}
and $\mathrm{height}_B \le \gamma_{\nmax}$ fails whenever $\gamma_{\nmax} < 1$,
i.e.\ for every $\nmax \ge 6$ ($\gamma_6 = 0.98958$, $\gamma_7 = 0.88277$).
The same witness gives $\mathrm{height}_P \ne 0$.  Two steps of the withdrawn
chain are independently invalid:\ the inequality
$\Lipnorm{f} - \opnorm{[\DCH, \Mat_{\KS_{\nmax}*f}]} \le \Lipnorm{f} -
\Lipnorm{\KS_{\nmax}*f}$ runs the wrong way, $\Mat$ being the \emph{compressed}
multiplier of \S\ref{sec:peter_weyl} and compression contracting;\ and the
good-kernel estimate bounds $\norm{\KS_{\nmax}*f - f}_\infty$, not a difference
of Lipschitz seminorms, which would require a second-order modulus.

\emph{Scope of the withdrawal.}  What falls is this lemma and the L5 route to
Theorem~\ref{thm:main}.  Theorem~\ref{thm:main_unconditional} is untouched:\ it
uses only reach-type estimates and no height constituent, so the convergence
statement --- and with it the WH1 keystone --- stands on that route.  A
height-controlled assembly for this pair is an open problem.

\emph{The assembly this proof used to close with is withdrawn with the
height constituents.}  It read
\begin{equation*}
   \Lambda(\Tcal_{\nmax}, \Tcal_{\sthree})
   \;\le\; \max\bigl(\gamma_{\nmax}, \gamma_{\nmax},
   \gamma_{\nmax}, 0\bigr)
   \;=\; C_3 \cdot \gamma_{\nmax},
\end{equation*}
whose third and fourth arguments are the refuted $\mathrm{height}_B \le
\gamma_{\nmax}$ and $\mathrm{height}_P = 0$.  \textbf{The conclusion
\eqref{eq:L5_bound} is therefore not established by this route.}  What
survives is Theorem~\ref{thm:main_unconditional}, which bounds the same
state-space GH distance by $\gamma_{\nmax}$ using reach-type estimates alone
and no height constituent --- so the \emph{statement} of
Theorem~\ref{thm:main} stands, by that route, and the WH1 keystone with it.
\end{proof}
...
\le 2\pi$ (from L4(b) plus the $\sthree$ diameter in the dual-Coxeter
metric) is not the relevant
...
triple version taken as the model here. Until 2026-09-03 the reduction
of the height to $\gamma_{\nmax}$ [retracted 2026-09-04: l5-height-bound-achieved] via the
Lipschitz-comparison form was presented as what gives the rate-controlled
bound of Theorem~\ref{thm:main};\ that reduction is refuted
(\eqref{eq:height_identically_one}), and the bound comes from
Theorem~\ref{thm:main_unconditional}.
\end{remark}
...
\begin{equation}\label{eq:main_thm}
   ...
   + O\!\bigl(1/\nmax\bigr).
\end{equation}
\textbf{[OBSERVATION]} The asymptotic constant is the sphere-volume
quotient $2\Vol(S^2)/\Vol(\sthree)$ at unit radius (numerically
$\Vol(S^2)/\pi^2$);\ it is \emph{not} the Hopf fibration's base-to-total
ratio, whose base carries half the radius (\S\ref{sec:L2}, volume-ratio
note). \textbf{[PANEL-VERIFIED]} A uniform bound
$\gamma_{\nmax} \le 6 \log \nmax / \nmax$
holds for all $\nmax \ge 2$.
\end{theorem}
...
tunneling pair would require; and---in the \emph{withdrawn} route only---Lemma~\ref{lem:L5} the assembly into the
state-space GH bound.

\emph{Which route establishes this.}  Not the L5 assembly:\ its two height
constituents are refuted (\eqref{eq:height_identically_one}), so
\eqref{eq:L5_bound} is \emph{not} established by that route --- as the
withdrawal in Lemma~\ref{lem:L5} states, and as an earlier version of this
proof failed to say, hedging instead only that the route ``inherits the
panel scope of Lemma~\ref{lem:L3}''.  The bound comes from
Theorem~\ref{thm:main_unconditional}, which bounds the same state-space GH
distance by $\gamma_{\nmax}$ using reach-type estimates alone and no height
constituent, and which uses neither Lemma~\ref{lem:L3} nor
Lemma~\ref{lem:L5}.  Combining that bound with the asymptotic and uniform
rates of Lemma~\ref{lem:L2}(d.ii)--(d.iii) gives \eqref{eq:main_thm}.
\end{proof}
...
2026-09-03); we read the $\SU(2)$ value as the sphere-volume
quotient $\Vol(S^2)/\Vol(\sthree)$ at unit radius, i.e.\
$2\Vol(S^2)/\Vol(\sthree)$ in the dual-Coxeter normalisation --- both
spheres of radius~$1$, so this is not the Hopf base-to-total ratio. (An earlier version of this
