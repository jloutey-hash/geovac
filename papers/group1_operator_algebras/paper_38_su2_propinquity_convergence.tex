...
being the metric scale rather than a geometric signature.  \emph{Normalisation caveat (2026-09-03).}  This identity is not a base-to-group Haar ratio in either normalisation:\ with the metric declared uniformly, $\Vol(\text{Hopf base})/\Vol(\SU(2))$ is $1/(4\pi)$ in the dual-Coxeter metric and $1/(2\pi)$ on the unit $\sthree$ (the Hopf map is a Riemannian submersion $\sthree(r) \to S^2(r/2)$, so the base radius scales with the total space).  The printed form pairs $\Vol(S^2) = 4\pi$, which is the base of the \emph{dual-Coxeter} sphere, with $\pi^2 = \Vol_1(\SU(2))/2$, a \emph{unit}-metric quantity.  It is therefore a numerical coincidence across two normalisations, not a geometric ratio, and the Hopf-base reading of the constant is correspondingly weaker than earlier drafts recorded.  Accordingly; \textbf{[OBSERVATION]} the same ratio $2/\pi$ is the
...
\begin{remark}[Hopf-base numerology of the constant]
...
\begin{equation*}
   ...
   \;=\; \frac{4\pi}{\pi^2}.
\end{equation*}
\emph{Normalisation caveat (2026-09-03).}  This identity is not a base-to-group Haar ratio in either normalisation:\ with the metric declared uniformly, $\Vol(\text{Hopf base})/\Vol(\SU(2))$ is $1/(4\pi)$ in the dual-Coxeter metric and $1/(2\pi)$ on the unit $\sthree$ (the Hopf map is a Riemannian submersion $\sthree(r) \to S^2(r/2)$, so the base radius scales with the total space).  The printed form pairs $\Vol(S^2) = 4\pi$, which is the base of the \emph{dual-Coxeter} sphere, with $\pi^2 = \Vol_1(\SU(2))/2$, a \emph{unit}-metric quantity.  It is therefore a numerical coincidence across two normalisations, not a geometric ratio, and the Hopf-base reading of the constant is correspondingly weaker than earlier drafts recorded.
Read as numerology rather than as a ratio, the constant still points
at the Hopf-fibration data, and is one of
