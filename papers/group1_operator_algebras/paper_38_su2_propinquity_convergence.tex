...
degenerates at finite cutoff;\ \textbf{[MEASURED]} a remark quantifies
...
$k \in \{0, 1, 2\}$~\cite{paper18}. \textbf{[OBSERVATION]} The constant
$4/\pi$ in
...
Krein wedge documented in the companion Lorentzian-extension paper. \textbf{[OBSERVATION]} The
...
metric spaces of \cite{latremoliere2018} (Trans.~AMS \textbf{368}
...
\eqref{eq:connes_distance_def}. \textbf{[MEASURED]} Numerical
verification at
...
\ldots, 1000\}$;\ no proof of monotonicity for all $n$ is given),
...
$4/\pi$ is identical
on both sides, which we read as a common Hopf-base origin in the
...
Marcolli--van~Suijlekom mechanism (unpublished supporting computation;\
no backing artifact is cited here), but the present paper restricts
...
\emph{Update (2026-05-16):} \textbf{[MEASURED]} the value $b$ has been
refined to $22$
...
above:\ the rate constant $4/\pi$ is reported \emph{universal} across
the
class, by a Plancherel-weight $\times$ Vandermonde-Jacobian
cancellation theorem (Paper~40~\cite{loutey_paper40} \S 3.2) --- rigorous at rank~$1$
...
Stein--Weiss / Abel--Plana derivation
of the asymptotic
$\lim_{n\to\infty} n\gamma_n / \log n = 4/\pi$ that appears as a
sketch in the proof of Lemma~\ref{lem:L2}.  Step~3(b) below is
asserted by analogy with Step~3(a) rather than executed;\ the
independent Euler--Maclaurin pinning of the two sub-coefficients, and
the numerical corroboration of their net, are what close the
constant. The argument here is a
