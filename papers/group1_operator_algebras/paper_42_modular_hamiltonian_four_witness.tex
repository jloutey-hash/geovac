...
the \emph{operator-level} period lift $e^{i \cdot 2\pi \cdot \beta D_W}
= -I$ at $\beta = 2\pi$ --- the global $-1$ of the spinor
double cover --- whereas the boost-class $K_\alpha^W$ has odd-integer
spectrum and $e^{i \cdot 2\pi \cdot K_\alpha^W} = +I$, the
...
2,3,4$). The distinction is therefore \emph{not} at the flow level but
