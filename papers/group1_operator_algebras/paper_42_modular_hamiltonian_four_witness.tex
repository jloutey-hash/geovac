...
intrinsic Camporesi--Higuchi Dirac, restricted to the wedge, lifts the
$\beta = 2\pi$ period to the spinor double-cover $-I$ at the operator
level rather than the identity operator $+I$ of $K_\alpha^W$ (the
conjugation flow still closes for both, the global $-I$ cancelling),
isolating $H_{\mathrm{local}} = K_\alpha^W / \beta$ as a
...
the present paper (\S\ref{sec:lorentzian_closure}).  Its \emph{continuum}
...
branch.  The mechanism is visible in this paper's own result~(i):\ the
truncated modular Hamiltonian has \emph{integer} spectrum, so the
finite-cutoff modular flow is a compact $\beta = 2\pi$ circle rather
than a non-compact boost, and every Lipschitz/boost seminorm built on
it is signature-blind.  The finite-cutoff closures reported below stand
as computed;\ what does not follow from them is a metric statement
about \emph{signature}.
...
therefore complements the Paper~38 state-space Gromov--Hausdorff convergence at the
operator level:\ Paper~38 gives a qualitative-rate state-space-GH bound
...
This paper closes the principal Sprint L1\footnote{Sprint/track designations are anchors into the project's internal chronicle (\texttt{CHANGELOG.md} in the repository).} falsifier named in
...
\S\ref{sec:lorentzian_closure} (the operator-system four-witness period
closure $\sigma_{2\pi} = \mathrm{id}$ on the Krein wedge); the
...
Zhang~\emph{et~al.}~\cite{zhu_casini2020}; the present
...
signature of the metric-side state-space GH convergence.
...
Sprint L1 falsifier ahead of the $(3, 1)$ extension (itself beyond
sprint scale); the present paper closes exactly that falsifier.
...
isolating \eqref{eq:H_local_def} as a derived structural finding. We
return to this point in Section~\ref{sec:unified_closure}.
...
state-space-GH bound would predict at finite cutoff; the closure does not
...
spinor double cover --- whereas the boost-class $K_\alpha^W$ has
...
truncated operator system. The above shows that, while either choice
generates a $2\pi$-periodic conjugation flow, only the boost-class
generator $K_\alpha^W$ realises the canonical BW vacuum with the
identity-operator period lift at $\beta = 2\pi$; the wedge-restricted
Dirac $D_W$ carries the spinor double-cover sign instead. The two play
complementary roles:\
...
$2\pi$ that appears in the M1 volume factor $\Vol(S^2)/\pi^2$
of the Paper~38 state-space-GH rate constant
\eqref{eq:paper38_main} (a quotient of unit-radius sphere volumes ---
\emph{not} the Hopf base-to-total ratio, misnomer retired 2026-09-03)
also drives the modular period closure of
...
the lattice realisations of Zhang et al., but the construction lives on
...
framework documentation:\ a substantial open program dominated by the
...
state-space Gromov--Hausdorff convergence~\eqref{eq:paper38_main}, the
present closure
...
obstruction --- a deep frontier --- is the absence of a Lorentzian
propinquity in the published literature.
...
signature) is the named Sprint L2-E target, still open beyond
...
closed at finite cutoff.  The \emph{metric}-level finite-cutoff question
is now understood structurally (Paper~45~\S\,open~Q1,
2026-06):\ the truncated Bisognano--Wichmann boost is compact (integer
spectrum, $e^{2\pi i K} = I$), so the finite-cutoff quantum metric is
Euclidean and the Lorentzian signature is the Wick rotation of the KMS
circle --- a convention, with the genuine reverse-triangle signature
confined to the non-compact continuum limit.
...
same $2\pi$ that drives the Paper~38 state-space-GH rate
...
Paper~43 (\texttt{papers/group1\_operator\_algebras/paper\_43\_lorentzian\_extension\_outline.md},
companion).
...
The headline reading (re-scoped 2026-09-03; the Lorentzian-signature
promotion recorded here before was withdrawn on 2026-07-04):\ the
four-witness Wick-rotation theorem (Hartle--Hawking + Sewell +
Bisognano--Wichmann + Unruh) closes at every tested Krein cutoff, but
the closure is \emph{signature-blind} --- the truncated boost is compact
(integer spectrum, $e^{2\pi i K} = \mathbb{1}$), the period closure is
the compact KMS $\beta = 2\pi$ circle, and the $(3,1)$ label is a
carrier choice the closure cannot distinguish from the Euclidean one
(Paper~45's $K^+$ theorem; WH7).  It is an operator-system-level
closure at finite cutoff, not a Lorentzian identification.

\subsection{Riemannian-limit identification (bit-identical at $N_t = 1$)}
\label{sec:riemannian_limit_identification}

The Sprint L2 architecture is the $N_t$-carrier \emph{extension}
(signature-blind at finite cutoff, 2026-09-03) of the Riemannian closure of \S\S\ref{sec:setup}--\ref{sec:four_witness_collapse},
...
Hekkelman--McDonald 2024 a/b~\cite{hekkelman_mcdonald2024b}) covers only Riemannian / Hilbert-space
structures.  No published Lorentzian propinquity construction exists.

\item Constructing a Lorentzian quantum-metric convergence theory was
the Sprint L3 target; the framework's attempt was DESCOPED on
2026-06-09 (Paper~45's $K^+$ annihilation theorem: the truncated boost
is compact, so no Lorentzian metric statement is available at finite
cutoff). It remains an open problem in the literature.  The
...
finite-cutoff statement, and a signature-blind one:\
Theorem~\ref{thm:krein_four_witness} is an operator-system closure
whose Lorentzian label is a carrier choice (2026-09-03);\ the
Lorentzian rate-level extension was descoped (Paper~45).

\subsection{References and pointer to standalone writeup}
\label{sec:lorentzian_closure_references}

The construction recipe is van~den~Dungen~2016~\cite{vandungen2016}
Proposition~4.1:\ Wick-rotate $g \to g_r$ (positive-definite),
...
outlined in
\texttt{papers/group1\_operator\_algebras/paper\_43\_lorentzian\_extension\_outline.md}
...
$4/\pi = \Vol(S^2)/\pi^2$ carries the same $k = 0$ M1 volume content
that drives the modular period closure of the present
Theorems~\ref{thm:bw_alpha}--\ref{thm:bw_gamma}.
