...
\begin{proof}[Proof sketch]
The argument is a structural composition of three established facts:

\textbf{Fact 1 (Master Mellin engine case-exhaustion,
Theorem~\ref{thm:pi_source_case_exhaustion} above).}  Every transcendental appearing
in a GeoVac observable is classified into the $M_1$, $M_2$, or $M_3$
Mellin sub-ring of $\mathcal{M}[\mathrm{Tr}(D^k \cdot e^{-tD^2})]$ at
$k \in \{0, 1, 2\}$.  Distinct sub-rings correspond to distinct
operator-order $\times$ bundle-type combinations and do not
inter-translate within $\mathcal{A}$.

\textbf{Fact 2 (Three independent spectral homes, Paper 2 §IV
+ Phases 4B--4I).}  $B$, $F$, $\Delta$ live in three structurally
distinct spectral sectors with no shared generator:\
$B$ is the Casimir trace at $m = 3$ (finite scalar sum);
$F$ is the Fock-degeneracy infinite Dirichlet sum at the packing
exponent $d = d_{\max} = 3$;
$\Delta$ is the Dirac mode-degeneracy count at $n = 3$ on the
spinor bundle.

\textbf{Fact 3 (Twelve mechanism eliminations).}  Phases 4B--4I
(April 2026) eliminated nine candidate generation mechanisms;
Sprint A $\alpha$-SP (Sprint A, 2026-04-18) eliminated the
Connes--Chamseddine direct spectral-action derivation;
Sprint K-CC (2026-05-03) eliminated three further mechanisms
(PSLQ at 100 dps against 244-element basis; natural $\Lambda$ search
from $S^3$ spectral data; unified CC heat-kernel approach blocked by
Theorem T9 algebraic obstruction).

Composition.  $\mathcal{A}$ contains no morphism that simultaneously
spans the three Mellin sub-rings of $B$, $F$, $\Delta$ and produces
the specific combination $K = \pi(B + F - \Delta)$.  Twelve mechanisms
have been empirically tested for such a morphism and all are
eliminated.  Therefore the combination rule remains a numerical
coincidence between three independent spectral homes at the 8.8 $\times$
10$^{-8}$ precision of Paper 2 §I.D, without a single-mechanism
generator in the current axiom set. $\hfill\square$
\end{proof}
