...
Order zero $[a, JbJ^{-1}]$
  ...
  & $\le 6.8\%$ finite-resolution (sample-of-3) \\
Order one $[[D,a], JbJ^{-1}]$
  & 5--20\% finite-resolution
  & $\le 10.2\%$ finite-resolution (sample-of-3) \\
\bottomrule
\end{tabular}
...
products);\ \textbf{[MEASURED]} it is constructed as an explicit
Connes-style real-space multi-particle spectral triple, and its
Hamiltonian is validated against the analytic $21\,$cm
singlet--triplet hyperfine gap at $1.62 \times 10^{-7}$ Ha.
...
times semisimple. The long-range Stage-2 Tannakian construction reduces
...
which $J_F$ leaves unchanged; in Connes' table~\cite{connes1995} it is
...
KO-4 columns of Connes' table share it; KO-3 is the label of the odd
...
column (measured 2026-09-02; pinned in the same test).  We therefore
...
\texttt{tests/test\_almost\_commutative.py} (54 passing as of
2026-09-02);\ data
...
\textbf{[PANEL-VERIFIED]} All six Connes axioms ($J^2 = -I$, $JD = +DJ$, $D = D^\dagger$,
...
order-one imposes no further constraint), exactly $128$ real parameters
in the matter sector. (This $128$ is the matter-sector parameter count
and mirrors the matter-sector restriction stated in
Theorem~\ref{thm:forced_count}; the full-axiom
$D_F$ moduli dimension at $n_{\max}=2$ is $260$, not $128$ --- see that
theorem's proof. The Yukawa non-selection conclusion is independent of
this count.) For
...
:\; 2048 \xrightarrow{\textnormal{Herm.}} 1024
     ...
     \xrightarrow{\text{order-one}} 260 .
\end{equation}
Restricted to the $16$-dimensional matter sector, the corresponding
(matter-sector) parameter count is
$512 \xrightarrow{\textnormal{Herm.}} 256
 \xrightarrow{\{\gamma_F, D_F\}=0} 128
 \xrightarrow{[J_F, D_F]=0\,+\,\text{order-one}} 128$;
every ``$128$'' quoted in this paper is this matter-sector count, not
the full-axiom dimension $260$.  The physical-slice dimension under
...
($512 \to 272$); the order-one condition removes a further twelve
parameters ($272 \to 260$).  The matter-sector restriction stated
after the chain ($512 \to 256 \to 128$, with $J$-reality and
order-one imposing no further matter-sector constraint) is the
original H1 sprint accounting:\ on the $16$-dimensional matter
sector the lepton and quark off-chirality blocks each satisfy
order-one without further reduction (because $\mathcal{A}_F$ acts
block-diagonally in lepton/quark and $M_3(\mathbb{C})$ commutes with
the Yukawa flavour structure); it is pinned as a projection-rank
check in the same test module (the matter blocks of the $272$- and
$260$-dimensional solution spaces each span the full $128$-dimensional
matter-sector space; 2026-09-02).  Each constructed-triple
...
\item \textbf{Forced data:}  the moduli space $\mathcal{M}(D_F)$ and
  its dimension (matter-sector count:\ $128\,N_{\mathrm{gen}}^2$ with
  generation mixing, $8$ per generation on the diagonal-Yukawa slice;
  full-axiom:\ $260$ at $N_{\mathrm{gen}} = 1$ --- see
  Theorem~\ref{thm:forced_count}).
...
\item $\Delta = 1/40 = c^2(4,3)$ (Paper~7's $(n, l)$ convention) — Dirac mode degeneracy $g_3^{\rm Dirac}$;
...
Coincidence on the Hopf Fibration,'' Paper~2 (2025; observation status, May~2026).
