...
hypothesis register (\texttt{CLAUDE.md}~\S 1.7, WH1 ``PROVEN'' --- unconditional as of 2026-06-10, Paper~38).
...
angles between the two subspaces---for LiH at $R_{\mathrm{eq}}$ the three
...
threshold.  What the molecular data adds is that the three-center
...
The contrast is quantitative: on the menu $l = 0, 1, 2$ the $\mathfrak{su}(2)$
...
Two invariants must be kept separate.  (i)~The \emph{categorical}
...
resolution.  (ii)~The bonding-dependent content is instead the
...
Geometry sharpens the picture.  At \emph{exactly} linear geometry the
...
face of the Renner--Teller mechanism.  As an internal observation the
...
noted there.  Because the $\sigma$ orbitals are real, $F$ is real-symmetric
...
electronic Hamiltonian.  On the linear-stretch plane $(d_1, d_2)$ the
...
Each intersection is the real (equatorial) section of a charge-$\tfrac12$
...
mode), with nonzero winding.  The $\mathbb{Z}_2$ (rather than $U(1)$)
...
monopole at its equator, recovering the $\pi$ holonomy.  This is an internal
...
four-center analogue of an elliptic modulus.  First, a \emph{threshold}: for
...
four-subspace ($\tilde D_4$) problem.  Second, over molecular shape $W$ is a
...
lift.  The object is the $*$-representation (non-abelian holonomy) analogue
...
three-center integral in Paper~59.  (Regression-pinned in
...
\]
Hence $F \simeq G$---\emph{similar}, not merely cospectral.  Every
...
basis} over nuclear geometry.  Verified across the whole construction:
...
Two consequences follow, one positive and one negative.  \emph{(i)~An
...
bit-exactly.  \emph{(ii)~It is not a conditioning statement.}  The
...
(Connes--Marcolli 2007, arXiv:math/0409306) --- a substantial open
...
condition) carries, after the matter-sector reductions $1024 \to 512$
...
and mirrors the chain of Theorem~\ref{thm:forced_count}; the full-axiom
...
structure).  Each constructed-triple axiom-consistency check passes at
...
chain.  A direct parameter-count of the constraint solution space
(\texttt{tests/test\_trunk\_qa\_forced\_count\_moduli.py}) gives, on the
full $32$-dimensional finite geometry,
$2048 \to 1024\,(\text{Herm.}) \to 512\,(\{\gamma_F, D_F\}{=}0)
\to 272\,(J\text{-real}) \to 260\,(\text{order-one})$---so the chain's
``$128$'' intermediates hold only under the restricted $16$-dimensional
\emph{matter-sector} count ($512 \to 256 \to 128$), not the full-axiom
count ($260$).  The Forced-Count statement should be read as the
matter-sector parameter count, with the $\{\gamma_F, D_F\}{=}0$ step
relabelled accordingly.  The
...
Paper~57 \S 6.1 (the Forced/Free Seam observations paper) records
...
\item \textbf{Forced data:}  the moduli space $\mathcal{M}(D_F)$ and
  its dimension ($128$ per generation).
...
from the multi-cutoff renormalization sub-sector.  A general
...
the cosmic-Galois layer $U^*$ constructed in Paper~56.  MvS 2014
...
an observation.  The Coulomb/HO asymmetry of Paper~31 is the
...
Schr\"odinger Equation from Scale-Invariant Graph Topology,'' Paper~7 (2025).
