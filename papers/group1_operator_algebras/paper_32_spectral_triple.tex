...
self-contained but together suggesting one parent structure.
\textbf{[INTERNAL THEOREM]} This
...
restricted to the truncation, and $D_{\mathrm{GV}}$ the diagonal
Camporesi--Higuchi Dirac operator~\cite{camporesi_higuchi1996} on the
truncated spinor space of unit~$S^3$ (no graph form is attached to
$D_{\mathrm{GV}}$; Remark~\ref{rem:D_GV_no_graph_form}).  This is not a
...
precedent; what is not duplicated is the $\alpha$ observation at
...
$S^3$ is odd-dimensional, the spectral triple is \emph{odd}).
\textbf{[OBSERVATION]} The four-way $S^3$ coincidence---Fock projection image,
Hopf fibration base $S^2$ (the graph carries the total space $\sthree$), Camporesi--Higuchi spin carrier,
...
finite-truncation caveats, and which remain open.  \textbf{[PANEL-VERIFIED]}
The real-structure question---whether the GeoVac Dirac operator $D_{\mathrm{GV}}$ (the diagonal spectral form) carries a real
structure matching the topological $\mathrm{KO}$-dimension of $S^3$
exactly at finite $n_{\max}$ or only in the continuum limit---is
\emph{resolved} in favour of finite-$n_{\max}$ exactness;\ the
residual open items are catalogued in Sec.~\ref{sec:open}.
\end{abstract}
...
Across the thirty-two papers (0--31) preceding this one, the GeoVac framework
has carried four distinct strands.  Strand~1 is the Fock projection
$p_0 = \sqrt{-2E_n}$ that maps the hydrogenic Hamiltonian onto the
free Laplacian on the unit three-sphere, whose continuum eigenvalues
are $\lambda_n = -(n^2 - 1)$; the graph nodes carry the quantum-number
labels $(n, l, m_l)$, not that spectrum (Paper~7).  Strand~2 is the Hopf bundle
...
\end{itemize}
\textbf{[OBSERVATION]} Working hypothesis~WH4 in the project register flags this as the
...
vansuijlekom_book2015}.

\paragraph{What it is not.}  A derivation of $\alpha$.
\textbf{[OBSERVATION]} Paper~2 stays
...
question is phrased without promoting it beyond that observation.
...
spectral triple of \cite{chamseddine_connes1997_2008} is rank~$\ge 3$ with
...
spectral triple that is an \emph{instance} of Marcolli--van Suijlekom 2014~\cite{marcolli_vs2014} (with Perez-Sanchez 2025 continuum-limit correction~\cite{perez_sanchez2025}) at the data level and a GeoVac-original \emph{extension} at the structural level (forced graph from Bertrand $\times$ Fock projection, master Mellin engine, explicit Hopf algebra $\mathcal{H}_{\mathrm{GV}}$, Tannakian dual $U^*$), \textbf{[OBSERVATION]} with the empirically interesting
...
hypothesis register (kept as \S 1.7 of the repository's \texttt{CLAUDE.md}; the sprint, track and door codenames used throughout this paper are chronicle anchors into that file and the repository \texttt{CHANGELOG.md}; WH1 ``PROVEN'' --- unconditional as of 2026-06-10, Paper~38~\cite{loutey_paper38}).
...
D_F)$~\cite{vansuijlekom_book2015,chamseddine_connes1997_2008}, the
...
graph, with the convergence to $C^{\infty}(M)$ carried by the
operator-system representative of Theorem~\ref{thm:gh_convergence}
(the diagonal algebra itself is the gauge-network convention; see
Remark~\ref{rem:operator_system}).  This
...
Perez-Sanchez~\cite{perez_sanchez2025}: replace the
...
\begin{remark}[$\mathcal{A}_{\mathrm{GV}}$ as the diagonal representative of an operator system]
\label{rem:operator_system}
\textbf{[OBSERVATION]} In the parallel reading of GeoVac through the spectral-truncation
framework of Connes and van~Suijlekom~\cite{connes_vs2021},
Definition~\ref{def:A_GV} describes
$\mathcal{A}_{\mathrm{GV}}^{(n_{\max})} = \mathbb{C}^{V_{\mathrm{Fock}}}$
as the diagonal (gauge-network) representative attached to the operator
system (its $C^*$-envelope is $M_N(\mathbb{C})$, since $\mathrm{prop} = 2$
gives $\mathcal{O}_{n_{\max}}^2 = M_N(\mathbb{C})$; corrected 2026-09-03)
\[
...
triple's axiom-level verifications (the finite-triple axioms of
Theorem~\ref{thm:GV_triple}; KO-dimension audit; Hodge structure) are
unaffected by which representative (the diagonal algebra or the operator
system) is taken;
the continuum-limit clause is \emph{not} representative-blind and is
stated for $\mathcal{O}_{n_{\max}}$ (2026-09-03).
\end{remark}

\begin{proposition}[Propagation number of $\mathcal{O}_{n_{\max}}$]
\label{prop:propagation_number}
\textbf{[PANEL-VERIFIED]} Let $\mathcal{O}_{n_{\max}} = P_{n_{\max}} C^\infty(S^3) P_{n_{\max}}$
...
even at this level: it separates the Toeplitz construction from its
dual.  Eq.~\eqref{eq:prop_two} states that the Fock-projected $S^3$
truncation is in the same propagation class as the Toeplitz $S^1$ at
the level of a quantitative Connes--van~Suijlekom invariant.
\textbf{[OBSERVATION]} The value should not be over-read:\ $\mathrm{prop}
= 2$ is the \emph{generic} value for a $*$-closed unital subspace of
$M_N(\mathbb{C})$ of the same complex dimension (random such subspaces
of dimension $14$ in $M_5$ and $55$ in $M_{14}$ all have $\mathrm{prop}
= 2$; 2026-09-03), so the invariant certifies that $\mathcal{O}_{n_{\max}}$
is not an abelian or otherwise degenerate comparator, not that it is
special among truncations.  The strongest quantitative alignment between
GeoVac and the published Connes--van~Suijlekom framework is the
convergence theorem in van~Suijlekom's own distance
(Theorem~\ref{thm:gh_convergence}), not this invariant.
...
\begin{corollary}[$\mathrm{prop} = 2$ separates $\mathcal{O}_{n_{\max}}$ from abelian comparators]
\label{cor:structural_specificity}
\textbf{[PANEL-VERIFIED]} The propagation value
$\mathrm{prop}(\mathcal{O}_{n_{\max}}) = 2$ in
Proposition~\ref{prop:propagation_number} separates the
Connes--van~Suijlekom spectral truncation of $S^3$ from the abelian
(point-evaluation) discretizations of $S^3$; it does \emph{not} single
it out among non-abelian truncations, for which $\mathrm{prop} = 2$ is
generic (retitled 2026-09-03 from ``structural specificity'').
...
\textbf{[MEASURED]} The Connes distance on operator systems
...
\item No comparison to the round-$S^3$ continuum geodesic distance is
   ...
   mapping).  The scalar case is published:\
   Gaudillot-Estrada--van~Suijlekom~\cite{gaudillot_estrada_vs2025}
   prove spectral-truncation convergence for all compact metric
   groups, including $\mathrm{SU}(2) \cong S^3$; the spinor
   (chirality-doubled Camporesi--Higuchi) case is the contribution
   of Theorem~\ref{thm:gh_convergence} below and Paper~38.
\end{itemize}
...
\textbf{[OBSERVATION]} Two of the three caveats above have been closed in subsequent
...
Connes \& van~Suijlekom 2021 three times).  The $S^3$ GH
convergence itself is no longer unpublished:\ the scalar case is
closed by
Gaudillot-Estrada--van~Suijlekom~\cite{gaudillot_estrada_vs2025}
for all compact metric groups including $\mathrm{SU}(2)$, and the
spinor (chirality-doubled) case by
Theorem~\ref{thm:gh_convergence} and Paper~38; the
Kantorovich--Wasserstein monotonicity question remains open.
\end{remark}

\begin{remark}[Two-sided structural alignment]
\label{rem:two_sided_alignment}
\textbf{[OBSERVATION]} Connes \& van~Suijlekom's worked $S^1$ example exhibits the
...
\end{array}
\]
The two-sided match separates spectral truncation from abelian
discretization on both manifolds; it does not foreclose the generic
non-abelian value (Remark above).  Together with the
SO(4)-isometry-preservation property of Sec.~\ref{sec:axiom_audit}
below (the property Connes \& van~Suijlekom themselves identify as
distinguishing spectral truncation from circulant discretization),
it is a consistency check on the construction; the load-bearing
alignment is Theorem~\ref{thm:gh_convergence}.
\end{remark}
...
relativistic angular momentum $\kappa \in \{-(l+1), +(l+1)\}$ for each
$0 \le l \le n_{\mathrm{D}}$ --- equivalently the fixed
$j = l + \tfrac12$ carrying a $\pm 1$ chirality label --- and total angular
momentum projection $m_j \in \{-j, \ldots, j\}$ where $j = |\kappa| - 1/2$.
\emph{Corrected 2026-09-03:} this read $\kappa \in \{-l-1, +l\}$, which is
arithmetically inconsistent with the node count below --- that set gives
$2, 10, 28$ at $n_{\max} = 1, 2, 3$ against the formula's $4, 16, 40$ --- and
is not what \texttt{geovac/full\_dirac\_operator\_system.py} builds;\ its
labels carry an explicit chirality, and its own documentation rejects the
$j = l - \tfrac12$ reading.  The total
node count is
\[
...
$l(\kappa) = |\kappa| - 1$ for both signs (corrected 2026-09-04 together
with the label set:\ the earlier ``$-1-\kappa$ if $\kappa<0$,
$\kappa$ if $\kappa>0$'' returns $l+1$ for $\kappa = +(l+1)$, which maps
top-shell nodes outside $V_{\mathrm{Fock}}$ and breaks the order-zero
argument as printed), and $m_l$ via the standard
...
by truncation of its spectral decomposition.

\begin{definition}[$D_{\mathrm{GV}}$]
\label{def:D_GV_spectral}
$D_{\mathrm{GV}}$ is the diagonal operator on
...
This is the operator used everywhere in the paper:\ every axiom
verification below, the real structure $J_{\mathrm{GV}}$ of
Proposition~\ref{prop:reality}, and every finite-truncation spectral-action
computation are carried out on this diagonal $D_{\mathrm{GV}}$
(\texttt{geovac/full\_dirac\_operator\_system.py},
\texttt{geovac/real\_structure.py}; backing tests
\texttt{tests/test\_full\_dirac\_operator\_system.py},
\texttt{tests/test\_real\_structure.py}).  The $\pi$-free certificate
of the Dirac sector (Paper~28~\S graph\_native\_qed.6) is a statement
about this spectrum:\ every eigenvalue is a half-integer rational,
every degeneracy is a positive integer.

\begin{remark}[No separate ``graph form'' of $D_{\mathrm{GV}}$]
\label{rem:D_GV_no_graph_form}
Earlier drafts of this section defined a second, ``graph form''
$D_{\mathrm{GV}}^{\mathrm{graph}}$ -- a first-order hopping operator
on a Camporesi--Higuchi edge set ``with weights chosen so that the
spectrum matches'' Definition~\ref{def:D_GV_spectral} -- and stated
their unitary equivalence as a proposition.  That proposition was
tautological (two Hermitian operators with the same spectrum and
multiplicities are unitarily equivalent by the spectral theorem, and
the weights had been chosen to make the spectra agree), and the module
it cited, \texttt{geovac/dirac\_matrix\_elements.py}, holds closed-form
angular (Szmytkowski) and radial (hydrogenic) matrix elements in the
$(\kappa, m_j)$ basis, not a hopping operator.  The paper therefore
carries one Dirac operator, the diagonal one above.  The
Dirac-sector \emph{graph} that appears elsewhere in the corpus
(\texttt{geovac/dirac\_lattice.py}, the E1-dipole adjacency of
Paper~28's graph-native QED) is a different operator, $H = D + W$,
whose adjacency encodes the photon-vertex selection rules rather than
$D_{\mathrm{GV}}$ itself.  Withdrawn at the 2026-09-02 trunk
certification run (\texttt{CHANGELOG.md}, v5.3.0); the module contents
asserted here are pinned by
\texttt{tests/test\_paper32\_dirac\_module\_contents.py}.
\end{remark}
...
commutators.  \textbf{[INTERNAL THEOREM]} The continuum limit
$n_{\max} \to \infty$ recovers the canonical spectral triple of the unit
Riemannian spin three-sphere $S^3$ with the Camporesi--Higuchi Dirac
operator \emph{for the operator-system representative}
$\mathcal{T}_{n_{\max}} = (\mathcal{O}_{n_{\max}}, \mathcal{H}, D)$ of
Theorem~\ref{thm:gh_convergence} (van~Suijlekom's state-space
Gromov--Hausdorff distance); the diagonal representative
$\mathcal{A}_{\mathrm{GV}} = \mathbb{C}^{V_{\mathrm{Fock}}}$ used for the
axiom audit above is the gauge-network convention of
Remark~\ref{rem:operator_system} and is \emph{not} the object that
converges (Corollary~\ref{cor:structural_specificity} places it on the
comparator side of the Connes--van~Suijlekom distinction; scope clause
added 2026-09-03).
\end{theorem}
...
for $\mathcal{O}_{n_{\max}}$; no Cauchy-sequence statement for the
diagonal algebra is made (a sentence to that effect was withdrawn
2026-09-03).
...
\textbf{[SYMBOLIC PROOF]} Holds by construction from
Definitions~\ref{def:A_GV}--\ref{def:H_GV} and
Eq.~\eqref{eq:representation}:\ $\pi(f)$ acts diagonally in the
$(n_{\mathrm{D}}, \kappa, m_j)$ basis, so $\pi$ is multiplicative and
$\pi(f^*) = \pi(f)^\dagger$, and it is faithful because the
quantum-number bridges of Eq.~\eqref{eq:representation} map
$V_{\mathrm{Dirac}}$ onto $V_{\mathrm{Fock}}$.  No numerical
certificate is involved (an earlier citation of
\texttt{tests/test\_dirac\_matrix\_elements.py} here covered the
$\kappa \leftrightarrow (l, j)$ bridges, not the representation
axiom, and is withdrawn).
...
|n_{\mathrm{D}}, \kappa, -m_j\rangle$ up to a phase --- \emph{$m_j$ only,
with the chirality label held fixed} --- whose antilinear-squaring gives
$-\mathbb{1}$ on the quaternionic 2-spinor representation).
\emph{Corrected 2026-09-03:} the construction printed here sent
$\kappa \mapsto -\kappa$, which under the label set above flips chirality;\ since $D_{\mathrm{GV}} = \chi\,(n
+ \tfrac12)$ that would give $\varepsilon' = -1$ and contradict this
proposition's own $J D = +D J$.  The implemented
$J$ (\texttt{geovac/real\_structure.py}) maps $m_j$ alone, as \S IV describes
and as the real-structure tests verify.  The restriction therefore inherits the signs.
\end{proof}
...
(44 tests passing); raw audit data
...
result).  By the proven GH convergence on $S^3$
(Theorem~\ref{thm:gh_convergence}; Track-TS-A R2.5 keystone) the
residuals should decay in the
...
``whether the GeoVac Dirac operator $D_{\mathrm{GV}}$ (the diagonal spectral form) carries a real structure
...
\textbf{[PANEL-VERIFIED]} All standard axioms are satisfied in the
continuum limit, and every axiom except the two finite-truncation
artifacts listed above holds at each $n_{\max}$ audited (the rows
marked $\forall\, n_{\max}$ additionally follow from the construction of
Proposition~\ref{prop:reality}); the triple is
real, odd, and of $\mathrm{KO}$-dimension~$3$ in the continuum limit.  The
...
\textbf{[PANEL-VERIFIED]} verify four BBB-predicted-sign axioms bit-exactly at
...
Order zero $[a, JbJ^{-1}]$
  & $7.9\%$ finite-resolution (full basis, $n_{\max}=3$)
  & $7.9\%$ (full basis, bit-identical to the Riemannian column;
    sample-of-3 $\le 6.8\%$) \\
Order one $[[D,a], JbJ^{-1}]$
  & $20.3\%$ finite-resolution (full basis, $n_{\max}=3$)
  & $20.3\%$ (full basis, bit-identical; sample-of-3 $\le 10.2\%$) \\
\bottomrule
\end{tabular}
...
\textbf{[PANEL-VERIFIED]} Four BBB-predicted-sign axioms hold bit-exactly across the
...
\textbf{[PANEL-VERIFIED]} The universal BBB Sec~5(v) axiom $\chi D = -D \chi$ fails bit-exactly
...
$\gamma^5$ rather than anticommuting with it.  \textbf{[SYMBOLIC PROOF]} These two facts together
...
The continuum limit of the GeoVac spectral triple (operator-system
representative, Theorem~\ref{thm:gh_convergence}) is the canonical
...
\textbf{[INTERNAL THEOREM]} This is a structural theorem; no fitting
parameters or empirical
...
$\kappa$, transcendental content, rigidity, $\alpha$ observation).  In
...
\bottomrule
\end{tabular}
\caption{\textbf{[OBSERVATION]} Sub-sector identification of the major framework papers as
...
products);\ \textbf{[PANEL-VERIFIED]} it is constructed as an explicit
Connes-style real-space multi-particle spectral triple, and its
Hamiltonian reproduces the singlet--triplet hyperfine pattern
$(-3A/4, +A/4)$ for the input coupling $A_{\rm hf}$ set to the $21\,$cm
value ($1.62 \times 10^{-7}$ Ha for the $3A/4$ shift) --- a structural
check of the Clebsch--Gordan pattern, not a measurement against an
external reference, since the coupling is the input (2026-09-03).
...
extends Paper~38's convergence to the KO-dim-$6$ graded tensor product
via the \emph{lifted-state} construction (leading constant $1$):\ Paper~38's
unconditional-theorem facts, lifted factor-wise and metrized by the
translation seminorm, with the limit the graded (Clifford-doubled) tensor
product of the two odd Camporesi--Higuchi real spectral triples --- the
naive $\gamma_a \otimes D_b$ being unrealisable ($\gamma_a = 0$ on
$\sthree$).  (An earlier $(B,P)$-pair route via a Pythagorean refinement of
the triangle inequality gave only a weaker conditional bound with constant
$2$ (its internal Lipschitz comparison constant being $C_3^{(2)} \le \sqrt2$);\ its height leg was refuted and it is superseded by the
lifted-state route [retracted 2026-09-05: p39-pythagorean-height-route].) This is
...
\times \sthree)$ in van~Suijlekom's state-space Gromov--Hausdorff
distance (the Latr\'emoli\`ere propinquity strengthening is the same
open gap as for the single $\sthree$ factor; Remark~\ref{rem:gh_status_caveat}). The two
...
\begin{remark}[Multi-center composition: the reducible--irreducible transition]
\label{rem:multicenter_composition}
The multi-focal program above concerns two electrons at distinct focal
lengths on a single $\sthree$; the complementary \emph{multi-center}
composition concerns the orthogonal projections $P_A$ onto the
single-center orbital subspaces of a polyatomic molecule in the shared
Hilbert space with overlap metric $G$.  The natural setting is the
\emph{$*$-representation} theory of orthogonal projections.  For a
\emph{diatomic} the pair $\{P_A, P_B\}$ is \emph{tame} (Type~I): by the
two-projection theorem (Halmos~\cite{halmos1969}, with the principal-angle
norm and block calculus catalogued in B\"ottcher--Spitkovsky~\cite{bottcher_spitkovsky2010})
the generated $*$-algebra decomposes
into blocks of dimension $\le 2$, completely classified by the principal
angles between the two subspaces---\textbf{[MEASURED]} for LiH at $R_{\mathrm{eq}}$ the three
angles $7.6^\circ / 44.7^\circ / 67.3^\circ$, the middle one at the
maximal-non-commutativity ceiling
$\lVert [P_A, P_B] \rVert = \tfrac12$.  The $*$-algebra generated by
\emph{three or more} projections is, by contrast, $*$-wild in general;
the passage from two to three centers is precisely this tame-to-wild
threshold.  \textbf{[PANEL-VERIFIED]} What the molecular data adds is that the three-center
configuration lands on an \emph{irreducible} (Schurian) point of that
wild family: for linear $\mathrm{BeH}_2$ (the $\sigma$ pair
$\{1s, 2p_0\}$ per center) the three center-projections generate the
\emph{full} matrix algebra $M_6$ and act irreducibly (commutant
$= \mathbb{C}\,\mathbb{1}$), welding the whole space into a single
indecomposable block that admits no decomposition; every pair remains
reducible.

Tameness is not composability, and conflating the two misreads the
obstruction.  The diatomic pair is completely classified, but it
emphatically does not commute---$\lVert [P_A, P_B] \rVert = \tfrac12$ is
the maximal possible value---and it is that \emph{two}-projection
non-commutativity, not the three-projection wildness, which constitutes
the multi-focal composition wall recorded above.  The obstruction
accordingly has two distinct rungs.  At two centers the projections fail
to commute while remaining completely classified by their principal
angles: the obstruction is quantitative, and the cost of removing it is
known in closed form.  At three centers the classification itself fails,
and no complete list of invariants exists.  Forcing commutation at the
first rung---L\"owdin orthogonalization---is the framework's
sparsity-destroying move; at the second rung there is no corresponding
normal form available to force.

The wildness is specific to the subspace configuration and is \emph{not}
a generic consequence of non-commutativity.  The framework's other
non-commutative angular structure---$SO(3)/SU(2)$ coupling, the Gaunt and
$6j$ machinery---is the representation theory of a \emph{compact} group,
hence type~I by Peter--Weyl, hence tame however non-abelian it may be.
\textbf{[MEASURED]} The contrast is quantitative: on the menu $l = 0, 1, 2$ the $\mathfrak{su}(2)$
generators satisfy $\lVert [J_a, J_b] \rVert = 2$ yet their commutant is
three-dimensional (one dimension per irreducible constituent, and
$\sum_\lambda m_\lambda^2$ in general---four for $l\!=\!1$ at multiplicity
two, five for $l\!=\!1 \oplus l\!=\!2 \oplus l\!=\!2$), whereas the three
center-projections, at the \emph{smaller} non-commutativity
$\lVert [P_A, P_B] \rVert \approx 0.49$, have commutant
$\mathbb{C}\mathbb{1}$.  Non-abelianness and wildness are independent
properties.  This is the structural reason the framework's angular
sparsity is graded, finite and potential-independent (Paper~22), and the
reason no Gaunt-style graded reduction should be expected to rescue
multi-center composition: arbitrary configurations of subspaces are not
group representations and escape Peter--Weyl.

Two invariants must be kept separate.  \textbf{[PANEL-VERIFIED]} (i)~The \emph{categorical}
statement---pairs reducible, triple irreducible---is robust: it survives
enriching the per-center $\sigma$ menu
$\{1s, 2p_0\} \!\to\! \{1s, 2s, 2p_0\}$ ($M_6 \!\to\! M_9$, irreducible
at both structural $Z=1$ and physical $Z=2$ charges) and holds at real
molecular geometries.  It is, however, a statement about center
\emph{count}: three coupled projections in general position are
irreducible at essentially any bonding-relevant separation, the
commutant reverting to reducible only once residual overlaps fall below
resolution.  \textbf{[MEASURED]} (ii)~The bonding-dependent content is instead the
\emph{magnitude} of the irreducible three-body coupling, measured by the
nested commutator $\lVert [[P_A, P_B], P_C] \rVert$ (which cannot even
be formed for a diatomic): $\approx 0.35$ at bonding, decaying smoothly
to zero as the centers separate.

\textbf{[PANEL-VERIFIED]} Geometry sharpens the picture.  At \emph{exactly} linear geometry the
in-plane basis $\{s, p_x, p_z\}$ per center decouples by axial symmetry
into a $\sigma$ and a $\pi$ sector and the nine-dimensional algebra is
reducible; \emph{any} bend mixes the sectors and welds the full space
into one irreducible $M_9$ block (bit-checked across
$160^\circ \!\to\! 90^\circ$ and at the $\mathrm{H_2O}$ geometry
$104.5^\circ$, for apex charges $Z = 1, 2, 4$)---the operator-algebra
face of the Renner--Teller mechanism.  \textbf{[OBSERVATION]} As an internal observation the
configuration operator $F = \sum_i P_i$ carries, over molecular
geometry, a lattice of conical intersections each with a $\pi$
($\mathbb{Z}_2$) Berry phase and, under bending, a structural
Renner--Teller promotion; this is the \emph{generic} topology of a
symmetric-triatomic degeneracy, distinct from the molecule's physical
electronic conical intersection, not an identity with it.  (Computed and
regression-pinned in \texttt{tests/test\_paper32\_composition.py} on
validated two-center overlaps, with the commutant routine self-validated
on cases of known answer; chronicle in CHANGELOG v5.1.0.)
\end{remark}

\begin{remark}[The configuration operator's Berry curvature: a flat $\mathbb{Z}_2$ bundle over nuclear geometry]
\label{rem:config_berry_curvature}
The configuration operator $F = \sum_i P_i$ of the previous remark varies
over the nuclear-geometry manifold $\mathcal{M}$, and its geometric-phase
structure is the finished form of the ``lattice of conical intersections''
noted there.  \textbf{[SYMBOLIC PROOF]} Because the $\sigma$ orbitals are real, $F$ is real-symmetric
at every geometry, so its Berry connection is \emph{flat} with only a
$\mathbb{Z}_2$ ($\pm$) holonomy: the curvature is not a smooth field but a
sum of $\pi$-flux Dirac deltas at the conical intersections,
\[
  \Omega \;=\; \sum_{\text{CI}} \pi\,\delta^2(m - m_{\text{CI}}),
\]
the Longuet--Higgins sign-change / molecular Aharonov--Bohm structure
(Longuet-Higgins; Berry; Mead--Truhlar) realized here for $F$ rather than the
electronic Hamiltonian.  \textbf{[PANEL-VERIFIED]} On the linear-stretch plane $(d_1, d_2)$ the
Fukui--Hatsugai--Suzuki lattice flux is exactly $0$ or $\pi$ on every
plaquette (machine-exact), with three $\pi$-sources---the central
intersection at $(2.45, 2.45)$ and an antisymmetric-stretch mirror pair---
carried by the two crossing bands and absent on the other four.  The holonomy
is the $\mathbb{Z}_2$ representation of $\pi_1(\mathcal{M} \setminus
\mathrm{CI})$ sending a loop to $\pi \cdot (\#\,\text{enclosed} \bmod 2)$,
cross-checked by direct parallel transport ($\pi$ around one intersection and
around all three).

\textbf{[MEASURED]} Each intersection is the real (equatorial) section of a charge-$\tfrac12$
Berry monopole: in a fixed real frame the local $2\times2$ model is
$F_0 + a_z \sigma_z + a_x \sigma_x$ with the $\sigma_y$ coefficient
\emph{identically zero}---the real family lives on the sphere's equator---and
branching vectors $g = \nabla a_z$ along the symmetric stretch (the tuning
mode) and $h = \nabla a_x$ along the antisymmetric stretch (the coupling
mode), with nonzero winding.  \textbf{[SYMBOLIC PROOF]} The $\mathbb{Z}_2$ (rather than $U(1)$)
character is \emph{forced by time-reversal symmetry}---the reality of the
Coulomb overlaps---not by planarity: real orbitals give real overlaps in any
geometry, so a non-coplanar real center leaves $F$ real-symmetric and does not
lift the phase.  Breaking time reversal does.  \textbf{[PANEL-VERIFIED]} A magnetic (Peierls) phase
$\phi$ threading the $\mathrm{H}$--$\mathrm{Be}$--$\mathrm{H}$ loop makes $F$
complex-Hermitian, opens the empty $\sigma_y$ axis, and turns each intersection
into a genuine $U(1)$ Berry monopole of integer charge: the first Chern number
over a small sphere enclosing $(d^*, d^*, 0)$ in $(d_1, d_2, \phi)$ is $\pm1$
(opposite on the two crossing bands), and the real $\phi = 0$ plane cuts the
monopole at its equator, recovering the $\pi$ holonomy.
\textbf{[OBSERVATION]} This is an internal
observation about the geometry of the composition, distinct from the
molecule's physical electronic conical intersection.  (Regression-pinned in
\texttt{tests/test\_paper32\_berry\_curvature.py}; chronicle in CHANGELOG
v5.1.0.)
\end{remark}

\begin{remark}[Four centers: a continuous four-body modulus (the $\tilde D_4$ analog)]
\label{rem:four_center_modulus}
The three-center geometric phase of Remark~\ref{rem:config_berry_curvature}
is discrete ($\mathbb{Z}_2$ conical intersections).  A \emph{fourth} center
adds a genuinely new, \emph{continuous} four-body invariant.  With
$S_{ij}=Q_i^{\!\top}Q_j$ the metric-orthonormal overlaps between center
subspaces, the loop holonomy of this overlap connection around the
$4$-cycle,
$W=\operatorname{Tr}(S_{12}S_{23}S_{34}S_{41})=\operatorname{Tr}(P_1P_2P_3P_4)$,
is invariant under the per-center gauge $Q_i\mapsto Q_iO_i$ ($O_i\in O(r)$)
and is therefore a physical configuration invariant.  Two facts make it the
four-center analogue of an elliptic modulus.  \textbf{[PANEL-VERIFIED]} First, a \emph{threshold}: for
four rank-one (single-orbital) subspaces $W$ is a function of the pairwise
and triple traces---the $4\times4$ Gram is the entire configuration, so
there is no four-body degree of freedom---whereas for rank $\ge2$ it is not
(a local Jacobian-nullspace test gives residual $\gtrsim0.3$).  A genuine
four-body invariant appears exactly when each center carries a
$\sigma$--$\pi$ pair, mirroring the rank-one$\to$rank-two step in the tame
four-subspace ($\tilde D_4$) problem.  \textbf{[MEASURED]} Second, over molecular shape $W$ is a
\emph{smooth continuous modulus}: for a planar rhombus with $\{1s,2p\}$ per
center it runs monotonically $0.79\!\to\!0.04$ as the diagonal ratio varies
---a one-parameter family of inequivalent four-center configurations, the
continuous freedom the three-center case lacks.  For real orbitals $W$ is
real (its phase is $\mathbb{Z}_2$); a magnetic (Peierls) flux through the
$4$-cycle makes $\arg W$ grow continuously from zero---the four-body $U(1)$
lift.  \textbf{[OBSERVATION]} The object is the $*$-representation (non-abelian holonomy) analogue
of the $\tilde D_4$ cross-ratio: four \emph{orthogonal} projections are
$*$-wild rather than tame, so the modulus is a matrix loop holonomy, not a
single scalar cross-ratio.  It is a geometry-axis invariant---a function of
nuclear shape---distinct from the density-axis cross-ratio of the
three-center integral in Paper~59~\cite{loutey_paper59}.  (Regression-pinned in
\texttt{tests/test\_paper32\_four\_center\_modulus.py}; chronicle in
CHANGELOG v5.1.0.)
\end{remark}

\begin{remark}[The configuration operator is the composed basis's overlap metric]
\label{rem:config_operator_is_metric}
The three preceding remarks treat $F = \sum_i P_i$ as an abstract
configuration operator.  It is not abstract.  Let $G$ be the Gram matrix
of the union of the per-center bases, with blocks
$G_{ij} = \langle \chi_i | \chi_j \rangle$, and suppose each
\emph{intra}-center block is the identity---automatic when each center's
own functions are orthonormal, and exact for the Shibuya--Wulfman metric
of Paper~60~\cite{paper60}.  Put $X = \operatorname{chol}(G)^{\mathsf T}$,
so that $X^{\mathsf T} X = G$ and the columns of $X$ are the basis vectors
expressed in an orthonormal ambient frame; let $W_k$ collect the columns
belonging to center $k$.  Then $W_k^{\mathsf T} W_k = G_{kk} = I$, so
$W_k$ has orthonormal columns, the pseudoinverse in $P_k = W_k W_k^{+}$
collapses to $W_k^{\mathsf T}$, and
\[
   F \;=\; \sum_k W_k W_k^{\mathsf T} \;=\; X X^{\mathsf T}
     \;=\; X\,(X^{\mathsf T} X)\,X^{-1} \;=\; X\,G\,X^{-1}.
\]
\textbf{[SYMBOLIC PROOF]} Hence $F \simeq G$---\emph{similar}, not merely cospectral.  Every
spectral statement in the preceding remarks (the conical intersections,
the band gaps, the $\mathbb{Z}_2$ holonomy of the eigenvectors) is
therefore a statement about the \emph{overlap metric of the composed
basis} over nuclear geometry.  \textbf{[MEASURED]} Verified across the whole construction:
linear $\mathrm{BeH_2}$, bent $\mathrm{BeH_2}$ (including the
$\mathrm{H_2O}$ angle $104.5^\circ$), and the four-center configuration,
with $\max|\operatorname{spec} F - \operatorname{spec} G| \le
3\times10^{-15}$ and $\lVert FX - XG \rVert_{\max} \le 9\times10^{-16}$.
The intra-center hypothesis is load-bearing rather than decorative:
perturbing one intra-center block off the identity by $0.3$ breaks the
identity outright (spectral discrepancy $6.7\times10^{-2}$).  The
four-center holonomy of Remark~\ref{rem:four_center_modulus} is the same
statement read blockwise,
$\operatorname{Tr}(P_1P_2P_3P_4) =
\operatorname{Tr}(G_{12}G_{23}G_{34}G_{41})$ (residual
$3\times10^{-16}$).

Two consequences follow, one positive and one negative.  \textbf{[MEASURED]} \emph{(i)~An
orthogonalization criterion.}  Since the intersections are degeneracies
of the metric, the eigenvectors of $G$ are double-valued around one:
parallel transport of the near-degenerate eigenvector around a small loop
enclosing the central intersection returns it with overlap $-1.0000$,
against $+1.0000$ on a CI-free control.  Any orthogonalization that
\emph{selects} metric eigenvectors---canonical orthogonalization with a
drop threshold, the standard remedy for near-linear-dependence---is
therefore path-dependent along a geometry scan through such a point,
whereas symmetric (L\"owdin) orthogonalization $G^{-1/2}$, being a
function of the matrix rather than of its eigenvectors, returns
bit-exactly.  \textbf{[MEASURED]} \emph{(ii)~It is not a conditioning statement.}  The
intersections are \emph{mid}-spectrum degeneracies (crossing eigenvalue
$\approx 0.71$ against $\lambda_{\min} \approx 0.13$), and
$\operatorname{cond}(G)$ passes through $d^*$ monotonically
($19.9 \to 16.2 \to 15.0$); the metric-conditioning growth of Paper~60 is
a different functional of the same object, and no kink at $d^*$ should be
expected or claimed.  The distinction drawn in the preceding remarks
between $F$ and the molecule's \emph{physical electronic} conical
intersection is unchanged; what changes is that $F$ is an invariant of the
framework's own encoding rather than a generic operator introduced for the
analysis.  (Regression-pinned in
\texttt{tests/test\_paper32\_config\_operator\_metric.py}; chronicle in
CHANGELOG v5.1.2.)
\end{remark}
...
$K = \pi(B + F - \Delta) = 1/\alpha$ at $4.77 \times 10^{-7}$ (raw;\ $8.8 \times 10^{-8}$ after the Sprint-A $\alpha^2$ correction), with
\[
   B = \gvq{b_casimir}{42}, \quad F = \gvq{f_fock_dirichlet}{\tfrac{\pi^2}{6}}, \quad \Delta = \gvq{delta_dirac}{\tfrac{1}{40}},
\]
and the Phase~4B--4I sprint series identified three independent
spectral homes for the three pieces (project development-frontier log):
\begin{itemize}\setlength{\itemsep}{0pt}
...
\item $\Delta^{-1} = g_3^{\mathrm{D}} = 2(3+1)(3+2) = 40$, the total
(both-sign) Camporesi--Higuchi Dirac degeneracy at level $k = 3$,
$|\lambda| = k + 3/2 = 9/2$ (Paper~7's $g_3^{\mathrm{Dirac}}$; Phase~4H SM-D).
\end{itemize}
...
equals $g_3^{\mathrm{D}}$, the total multiplicity (both signs) of the
Camporesi--Higuchi level $k = 3$, $|\lambda_3^{\mathrm{D}}| = 9/2$.
\end{itemize}
...
and \textbf{[PANEL-VERIFIED]} Phases 4B--4I exhausted nine attempts to find a common spectral
generator that produces all three simultaneously, without success
(twelve counting Sprint~A and Sprint~K-CC; see the E6
no-single-mechanism theorem below).
...
\label{rem:sprint_A}
\textbf{[MEASURED]} Sprint~A (April~2026, working-hypothesis register) verified at 80~dps that the
...
Paper~31~\cite{paper31} formalized a seven-layer asymmetry (six of which
are tabulated here; the seventh, the CM Hodge structure, is Paper~24
\S 5) between the
...
\bottomrule
\end{tabular}
\caption{\textbf{[OBSERVATION]} The Coulomb/HO asymmetry from Paper~31, in spectral-triple
...
\textbf{[OBSERVATION]} In spectral-triple language the asymmetry is structural: the Coulomb
...
artifact of the GeoVac construction.  The seven asymmetry layers (corrected 2026-09-03 from ``five'')
...
\textbf{[PANEL-VERIFIED]} The empirical principle of Paper~35~\cite{paper35} (Prediction~1,
...
mechanisms of the present triple.  \textbf{[CONDITIONAL]} We state the
case-exhaustion below and give its proof sketch;\ the
(i)$\Leftrightarrow$(ii) leg rests on the empirical Paper~35
verification rather than on a proof.
...
\mathrm{Vol}(S^2)/4$ via the Hopf fibration $S^1 \to S^3 \to S^2$
(corrected 2026-09-03 from ``$S^3 \to S^2 \times S^1$'', which misstates the
fibration and asserts a product $S^3$ is not),
...
The M1 sub-mechanism (the $k = 0$ volume slot of the master Mellin engine,
historically named the ``Hopf-base measure'' after the factor
$\mathrm{Vol}(S^2)/4 = \pi$ of Paper~25's $S^3 \to S^2$ bundle;\ that name is
a misnomer for Paper~38's $4/\pi$, which is \emph{twice} the
unit-radius sphere-volume quotient $\mathrm{Vol}(S^2)/\mathrm{Vol}(S^3) =
2/\pi$ and not a base-to-total ratio (corrected 2026-09-03;\ the ``$4/\pi$ is
a quotient of unit-radius sphere volumes'' wording printed here until then was
itself wrong, since $\mathrm{Vol}(S^3) = 2\pi^2$, and it contradicted
Paper~38))
produces period values in the localised pure-Tate sub-ring
$\mathbb{Q}[\pi, \pi^{-1}]$ of mixed Tate periods over $\mathbb{Q}$,
at depth $0$ and Tate weight $\in \mathbb{Z}$.  \textbf{[PANEL-VERIFIED]} Every M1 closed form
witnessed across the corpus (Paper 25 Hopf base
$\mathrm{Vol}(S^2)/4 = \pi$; Paper 38 §L2 asymptotic state-space GH
rate $4/\pi = \mathrm{Vol}(S^2)/\pi^2$; Paper 40~\cite{loutey_paper40} Thm 1 universal
$4/\pi$ across compact connected semisimple Lie groups (rigorous at
rank~1, with an analytical universality mechanism at proof-sketch tier
and numerical support at rank~2 plus a noisier rank-3 datapoint; a
circle factor carries $2/\pi$); Paper 43 §10.2 Pythagorean
orthogonality $1/\pi^2$; Paper 50~\cite{loutey_paper50} §3 F-theorem $-3\zeta(3)/(16\pi^2)$
...
sub-case $a(t) \equiv 1$ on $\mathbb{R} \times \sthree$). \textbf{[PANEL-VERIFIED]} The GeoVac
...
Deligne~\cite{deligne2010} (proven for $N \in \{2,3,4,6,8\}$) and the
...
(the $N' = 2 \mid N = 4$ instance of Glanois~\cite{glanois2015} Cor.~1.2) realised on the framework's
...
\textbf{[OBSERVATION]} Paper~2's combination $K = \pi(B+F-\Delta) \approx 1/\alpha$ matches
its target at $8.8 \times 10^{-8}$ (post-correction;\ $4.77\times10^{-7}$ raw).  Under
...
\textbf{[OPEN]} A counter-example to Theorem~\ref{thm:pi_source_case_exhaustion}
...
A three-track follow-up sprint (MR-A: Dirac state-space GH rate;
...
GH rates. The L2 sprint (2026-05-06) measured the
asymptotic constant of the SU(2) scalar central Fej{\'e}r kernel as
$4/\pi$ (dual-Coxeter normalisation; $2/\pi$ on the unit $S^3$,
Paper~38, 2026-09-03).  The constant is the sphere-volume
quotient $\mathrm{Vol}(S^2)/\mathrm{Vol}(S^3) = 2/\pi$ for unit-radius
spheres, twice that in the dual-Coxeter normalisation;\ it is not the Hopf
fibration's base-to-total ratio, whose base carries half the radius
(Paper~38, volume-ratio note).
...
for M3 from a half-integer-only Peter--Weyl truncation (the
state-space GH machinery of Theorem~\ref{thm:gh_convergence}) is a
...
$(4/\pi)\log(n)/n$ decay (dual-Coxeter normalisation; $2/\pi$ on the unit $S^3$) is an integer$\times$half-integer
...
extracted the L2 next-order constant $c = 4.10932146748987562042\ldots$
to $22$ verified decimal digits (an earlier Richardson-extrapolated
$\geq 80$-digit string was contaminated from digit~$12$ by
float-precision panel points; see Paper~38~\S\,Open questions) but
found no PSLQ identification in $M_1 \cup M_2 \cup M_3 \cup \{\gamma_E, \log 2,
...
enrichment program of Paper~55~\cite{loutey_paper55}~\S~subsec:open\_m2\_m3 correctly
...
\textbf{[PANEL-VERIFIED]} On the truncated Camporesi--Higuchi spectral triple at $n_{\max}
...
$\mathrm{Vol}(S^2)/\pi^2 = 4/\pi$ (dual-Coxeter; $2/\pi$ at unit radius) in Paper~38~\S~VIII as the L2
...
open question, reachable from Karamata~\cite{karamata1949} and
...
in Remark~\ref{rem:q5p_strict_strong_drift};\ the deep NCG-mathematics
sub-question on the published Connes 1994 Ch.~IV / Loday \S~1.4 gap
remains open, beyond sprint scale, independent of the Stage-1 closure here.
...
\S\ref{sec:pi_source_theorem}, respectively). With primitive coproduct
...
sector-disjoint (no proper sub-sector nesting). The natural
continuation toward Stage-2's full Tannakian construction
(Connes--Marcolli 2004, arXiv:math/0409306) --- a substantial open
program --- is to enrich the coproduct
...
Karamata~\cite{karamata1949} (foundational), Korevaar~\cite{korevaar2004}
...
A first scoping step of the nested-Hopf-tower substrate
enrichment of Remark~\ref{rem:q5p_stage2_hopf_substrate} (a
substantial open program) replaces the
...
the Mellin partition but is abelian. The natural continuation, beyond
sprint scale, is a synthesis combining both --- either (a) tensor product
...
The first scoping step of the cross-shell off-diagonal Dirac
perturbation enrichment --- itself a substantial open program ---
(second of three structural ingredients flagged
...
The first scoping step of the long-range Cuntz-extension enrichment
...
Fields Inst. Commun. 12), genuinely long-range. The Cuntz route is
...
Connes--Marcolli motivic Galois machinery (Connes--Marcolli 2004
arXiv:math/0409306;\ Connes--Marcolli 2008 book~\cite{connes_marcolli2008}
Ch.~1~\S 7.3, ``Equivariant principal bundles and the group $G^* = G \rtimes \mathbb{G}_m$''):\ pro-unipotent
times the multiplicative group (reductive, not semisimple). The long-range Stage-2 Tannakian construction reduces
to (a) full Tannakian closure of the pro-unipotent factor, and (b)
verification that the cosmic-Galois $U^*$ of Connes--Marcolli 2004
...
sprint-scale decision in the long-range Stage-2 program. See
Paper~55~\S~subsec:open\_m2\_m3.
\end{remark}
...
cutoff. Long-range follow-ons (closed-form $T_{\mathrm{path}}(n_{\max})$,
...
The first long-range scoping step of the algebra-closure dimension at
...
fill-fraction identification. Long-range:\ closed-form per-block rank
...
of the cosmic-Galois $U^*$ Hopf algebra (Connes--Marcolli 2004,
...
The first long-range scoping step of combining the T3a Peter--Weyl
...
\]
exactly the long-range synthesis target predicted by
...
structure as load-bearing data and constitutes a long-range continuation.
See Paper~55~\S~subsec:open\_m2\_m3.
\end{remark}
...
Connes--Marcolli motivic-Galois shape. The canonical long-range next
...
sprint-scale prerequisite to the long-range
...
\begin{remark}[Q5' L2 substrate extended to $j_{\max} = 3/2$:\ sprint-scale prerequisite for the long-range smash-product construction closed, Sprint Q5'-FO1 (v3.66.0), June 2026]
...
long-range smash-product construction
...
group structure is $j_{\max}$-invariant.  This unblocks the long-range
...
long-range (A) follow-ons. The L1 follow-on (a) Tannakian closure of
the pro-unipotent factor in the Connes--Marcolli machinery remains
the canonical long-range next step.
\end{remark}
...
+\zeta(-3)/2$ ($B_4$ rung); the $\alpha$-observation's
...
\item \emph{Controls behave}: $\kappa = -1/16$ (numerically
coincident with the Fock Jacobian factor $1/16$; an Observation,
Paper~0~\cite{paper0}) has no clean $\zeta$ form; the Dirac Casimir
...
$\sthree$ ($4/\pi = 2\,\mathrm{Vol}(S^2)/\mathrm{Vol}(\sthree)$ at unit radius,
numerically $\mathrm{Vol}(S^2)/\pi^2$, the L2 state-space GH rate constant in
the dual-Coxeter normalisation --- \emph{not} a quantity on the Hopf base,
whose radius is half the total space's;\ label corrected 2026-09-03; Sprint
MR-A/B in Remark~\ref{rem:master_mellin_domain})
...
(Hartle--Hawking~\cite{hartle_hawking1976};
 ...
 Bisognano--Wichmann~\cite{bisognano_wichmann1975,bisognano_wichmann1976};
 reviewed in~\cite{witten2018_rmp}),
...
in Track~D \S 3.1; estimated beyond sprint scale; priority MEDIUM-HIGH.
...
$\beta_{\mathrm{Rindler}} = 2\pi/a$) is a pendant follow-up
flagged in Track~D \S 5.4; not pursued in the sprint window.

\textbf{Unruh pendant (May 2026).}\ The Track~D~\S 5.4 follow-up was
executed as a pendant
...
identification); same falsifier; same MEDIUM-HIGH priority and scale.

\textbf{Sprint L1 closure (2026-05-16):\ literal identification at the
operator-system level (Riemannian).}\ \textbf{[SYMBOLIC + MEASURED]} The principal falsifier named
above is now closed positively at the operator-system level.\ Sprint
...
$4.8 \times 10^{-16}$ at $n_{\max}=3$ ($\dim_H = 40$; numerically equal
to $g_3^{\mathrm{Dirac}} = \Delta^{-1} = 40$ but a different object ---
the cumulative node count $4 + 12 + 24$ over levels $k \le 2$ versus the
single-level degeneracy at $k = 3$),
...
(long-range scale per Sprint L0 audit
...
5\}$.  Result:\ \emph{UNIFIED\_STRONG closure} (a bit-exact $\sigma_{2\pi}=\mathrm{id}$ compact-period closure --- a compact-flow feature of the integer modular spectrum, signature-blind, not a metric strengthening) at all tested cutoffs
...
in van~Suijlekom's state-space Gromov--Hausdorff distance (the
strengthening to the Latr{\'e}moli{\`e}re quantum propinquity remains an
open named gap; see Remark~\ref{rem:gh_status_caveat}):
$$
   d_{\mathrm{GH}}(\mathcal{T}_{n_{\max}}, \mathcal{T}_{S^3})
   \;\le\; \gamma_{n_{\max}}
   \;\xrightarrow[n_{\max}\to\infty]{}\; 0,
$$
where (matching Paper~38's corrected unconditional display, whose bound is
$\le \gamma$ outright; the abandoned-route Lipschitz constant was $C_3 = 1$)
$\gamma_{n_{\max}}$ is the L2 mass-concentration moment of the central
spectral Fej{\'e}r kernel on SU(2).  The asymptotic rate
$\gamma_{n_{\max}} = (4/\pi)\log n_{\max}/n_{\max} + O(1/n_{\max})$ (dual-Coxeter normalisation, $\mathrm{Cas}(\mathrm{ad}) = h^\vee$;\ equivalently $2/\pi$ on the unit $S^3$ this paper declares, and $2\sqrt2/\pi$ under the field-standard Kac form --- the constant scales with the metric)
is proven unconditionally in Paper~38~\cite{loutey_paper38}
(main theorem, translation-seminorm metrization, 2026-06-10); the
small-$n$ closed forms are consistent with it.
\end{theorem}

\begin{proof}[Proof sketch]
The theorem is established by Paper~38's~\cite{loutey_paper38}
lifted-state unconditional route in the van~Suijlekom state-space
Gromov--Hausdorff distance (\emph{not} a Latr{\'e}moli{\`e}re
propinquity --- that strengthening is the open named gap of
Remark~\ref{rem:gh_status_caveat}). The historical five-lemma UCP-pair
assembly of WH1 sprint~R2.5 (April--May~2026) is recorded below for
provenance; its lemma~L5 is withdrawn (2026-09-03,
Remark~\ref{rem:gh_status_caveat}):
...
Connes-vS-compatible metric sub-structure on which the state-space
Gromov--Hausdorff distance is defined (the strengthening to the
Latr{\'e}moli{\`e}re propinquity remains the open named gap of
Remark~\ref{rem:gh_status_caveat}).
...
Connes--van~Suijlekom 2021 Fej{\'e}r--Riesz reconstruction on
$S^1$ (their \S 4.2, Lemma~4.4), equivalently the non-K{\"a}hler analog
of the Hawkins~1999 Berezin map for K{\"a}hler coadjoint orbits (Comm.
Math.\ Phys.\ 202).
...
Bo{\.z}ejko--Fendler; \emph{the dual estimate is a withdrawn named
gap}), height contributions bounded constants (L4(b) contractivity +
$\mathrm{height}_P = 0$ --- \emph{both withdrawn 2026-09-03: L4(b) is not the
relevant height, and $\mathrm{height}_P = 0$ is false by the same finite-band
witness as $\mathrm{height}_B$}), giving
$d_{\mathrm{GH}}(\mathcal{T}_{n_{\max}}, \mathcal{T}_{S^3}) \le
C_3 \cdot \gamma_{n_{\max}} \to 0$ --- the state-space distance of the
theorem statement, not the Latr{\'e}moli{\`e}re propinquity $\Lambda$,
whose label remains an open named gap.
\end{itemize}

The module's test panel does \emph{not} verify Lemma~L5 (measured
2026-09-03):\ the Lipschitz-normalised panel height rises with the
cutoff ($0.667$, $0.808$, $0.878$, $0.913$, $0.937$, $0.952$ at
$n_{\max} = 2, \ldots, 7$) while $\gamma_{n_{\max}}$ falls, so the margin
shrinks from $+1.41$ to $+0.05$ and turns negative at $n_{\max} = 7$
($-0.07$; measured 2026-09-03).  Normalised as printed (height over $\|f\|_{\mathrm{Lip}}$) the panel
entries rescale into the unit-Lipschitz ball, so they are a \emph{subset} of
the set L5's supremum ranges over and therefore a lower bound on it:\ an entry
exceeding $\gamma_{n_{\max}}$ \emph{contradicts} the bound rather than leaving
it open.  \textbf{Lemma~L5 is accordingly withdrawn as stated}
(2026-09-03):\ Paper~38 gives a one-line witness --- $B_{n_{\max}}$ is
finite-band, so the unit-Lipschitz ball contains $f$ with $B_{n_{\max}}(f) =
0$ and $\mathrm{height}_B \equiv 1$ at every cutoff --- and the crossing is
simply where $\gamma_{n_{\max}}$ falls below~$1$.  This paragraph read ``this
neither confirms nor contradicts L5\ldots\ an open check'' until then.
\textbf{Scope:} Paper~38's unconditional theorem uses no height constituent
and is untouched, so the convergence statement and the WH1 keystone stand.  What the module reports is therefore the bound itself is $C_3\gamma_{n_{\max}} = 2.075, 1.610,
1.322$ (these are the L2 moments $\gamma_2, \gamma_3, \gamma_4$, not
independent measurements; ratio $0.637$).  Module
\texttt{geovac/gh\_convergence.py} packages the assembly (48 unit
tests, of which the panel-inequality checks added 2026-09-03 are the
ones that can fail --- the earlier version reported the bound as
$\gamma$ by construction), integrating with the L1${}'$--L4 modules verbatim.
\end{proof}

\begin{remark}[Status and named gaps (2026-06-09)]
\label{rem:gh_status_caveat}
Three gaps in the five-lemma proof chain behind
Theorem~\ref{thm:gh_convergence} are named here.
(i)~The L2 cb-norm statement conflated the kernel's Plancherel
\emph{mass distribution} with its convolution-\emph{multiplier symbol}:
the smoothing map is unital completely positive with cb-norm exactly
$1$, and the quantity $2/(n_{\max}+1)$ is the mass-distribution maximum,
not a cb-norm; the rate content (the mass-concentration moment
$\gamma_{n_{\max}}$, its closed forms, and the $4/\pi$ asymptote) is
unaffected.  (ii)~The $\mathrm{reach}_P$ dual estimate in L5 is a
\emph{named gap}: a forward cb-norm controls no partial inverse; the
correct mechanism is an antiderivative-transference lemma in the style
of Leimbach--van~Suijlekom (Adv.~Math.\ \textbf{439} (2024) 109496);
for scalar sectors on compact metric groups the conclusion is covered by
Gaudillot-Estrada--van~Suijlekom~\cite{gaudillot_estrada_vs2025}.
(iii)~The distance proved is van~Suijlekom's state-space
Gromov--Hausdorff distance (arXiv:2005.08544 lineage); the
Latr{\'e}moli{\`e}re propinquity label is an open named gap --- the
cited Latr{\'e}moli{\`e}re framework builds tunnels from quantum
isometries, not UCP pairs.
(iv)~The kernel condition $L(a) = 0 \iff a \in \mathbb{C}\mathbf{1}$
holds for the engineered offdiag Camporesi--Higuchi Dirac (kernel
dimension $1/14$ at $n_{\max} = 2$, $1/55$ at $n_{\max} = 3$, verified)
but \emph{fails} for the truthful chirality-diagonal CH Dirac
...
partial inverse or transference estimate remains.
\textbf{[INTERNAL THEOREM]} The convergence
theorem is unconditional in van~Suijlekom's state-space framework at
rate $(4/\pi + o(1))\log n_{\max}/n_{\max}$ in the dual-Coxeter
normalisation of $\gamma_{n_{\max}}$ ($2/\pi$ on the unit $S^3$).
\end{remark}

\begin{remark}[Limit identification]
\label{rem:gh_limit_id}
A companion proposition (Kantorovich--Rubinstein duality on the
van~Suijlekom state-space distance) identifies the limit
...
Theorem~\ref{thm:gh_convergence} closes the WH1 keystone (working-hypothesis register).
\textbf{[INTERNAL THEOREM]} The structural claim of WH1 --- that GeoVac is an almost-commutative
spectral triple with Connes--van Suijlekom truncations converging to
the round-$S^3$ spectral triple --- is now \emph{proved} with the
explicit rate $(4/\pi)\log n_{\max}/n_{\max} + O(1/n_{\max})$ (dual-Coxeter normalisation, $\mathrm{Cas}(\mathrm{ad}) = h^\vee$;\ equivalently $2/\pi$ on the unit $S^3$ this paper declares, and $2\sqrt2/\pi$ under the field-standard Kac form --- the constant scales with the metric) (Paper~38).  The ``torsion-and-curvature correction''
...
coincidence; the order-zero and order-one conditions of the real
structure at finite $n_{\max}$ --- the KO sign pair itself is settled,
Sec.~\ref{sec:axiom_audit}) are
downstream of this theorem.  The proof chain was corrected in June 2026
...
transcendental in the residual is $1/\pi^2$ (corrected 2026-09-03 from
``$= 1/\mathrm{Vol}(S^1)^2$'', which is false arithmetic:\
$\mathrm{Vol}(S^1) = 2\pi$, so that quantity is $1/(4\pi^2)$ --- the displayed
equation above carries the correct form),
...
state-space GH-rate observables, M2 owns heat-kernel residuals, M3 owns
...
Paper~24~\S 5.3 Coulomb/HO asymmetry (seven layers in the current count; the structural
finding that the Pythagorean orthogonality is $S^3$-specific to
the construction --- the $S^5$ Hardy sector has no clean analog,
joining the modular-Hamiltonian level as a further Coulomb/HO
...
with the structural-skeleton-scope pattern (project development-frontier log, sprints
...
full Connes--Chamseddine cigar route) remains the longer-route
...
sense~\cite{chamseddine_connes1997_2008}.

\subsubsection*{Three Wilson constructions, one kinetic coefficient}

\textbf{[PANEL-VERIFIED]} Three independent sprints have produced gauge-invariant, finite,
...
\texttt{tests/test\_su3\_wilson\_s5.py}, $47$ pass and $2$ skipped
of $49$ as of 2026-09-02) constructs
...
$\mathrm{tr}(T^a T^b) = \tfrac{1}{2} \delta^{ab}$.  \textbf{[SYMBOLIC + MEASURED]} The three rows
...
$\mathfrak{su}(3)$).  \textbf{[OBSERVATION]} This is the single mathematical identity
...
asymmetry and adds to it (the current count is seven layers, Paper~31):\ to the prior
asymmetries (spectrum-computing role of $L_0$; calibration~$\pi$;
Wilson gauge-with-natural-matter) one adds a further layer at
...
and the project's rhetoric rule forbids treating it as the
...
in an almost-commutative algebra.  Per Perez-Sanchez 2025~\cite{perez_sanchez2025}, the
...
$\mathrm{SU}(2)$ are co-located on the same $S^3$ graph (the co-location Sprint~G3 tested and closed in the negative, \S\ref{sec:g3};\ until 2026-09-04 this parenthesis still called it ``the most concrete near-term unification target''), but
...
most directly tied to the (seven-layer) Coulomb/HO asymmetry of
...
$\mathcal{A}_F = \mathbb{C} \oplus \mathbb{H} \oplus M_3(\mathbb{C})$,
bounded sprint sequence, predicted positive-thin) and G4b (genuine
...
\subsubsection*{Unification targets after G3: what remains}

Of the four gaps, G4 (cross-manifold) is the most consequential and --- noting that G3 is \emph{not} among the live ones, Sprint~G3
having closed it in the negative (\S\ref{sec:g3}:\ identification is
structurally impossible by tensor-product factorisation) --- the only sprint-scale path among the three that remain is G4a (\S\ref{sec:g3}, matching the assessment there);\ G1 has no current selecting mechanism (\S\ref{sec:unified_gauge}).
\emph{Corrected 2026-09-04:} this sentence inserted the G3 hedge and left the
predicate ``G3 (electroweak unification) is the most reachable'' standing, so
it announced a proven negative as the nearest-term target in the same breath as
recording it closed.  Paper~25's
...
weak-isospin is \emph{not} open:\ Sprint~G3 closed it in the negative on $S^3$
alone (\S\ref{sec:g3}), and the text here presented that proven negative as
``the second open structural question'' until 2026-09-03.
\emph{That open question} --- the almost-commutative lift of Paper~30's
$\mathrm{SU}(2)$ Wilson instance, \emph{not} the chirality-grading question
Sprint~G3 closed --- is a clean $S^3$-only target that does not require any
progress on the $S^5$ side. It is not by itself an electroweak
construction:\ what it would deliver is a non-chiral
$\mathrm{SU}(2) \times \mathrm{U}(1)$ Wilson product on $S^3$;\ chiral
electroweak co-location on $S^3$ is what Sprint~G3 closed
(\S\ref{sec:g3}), and the remaining electroweak-unification path is G4a
(Connes SM on the $S^3$ triple alone, below). (Clarified 2026-09-04:\ an
earlier version of this paragraph labelled the instance
$\mathrm{SU}(2)_L$ --- the weak-isospin identification G3 refuted --- and
promised ``unified electroweak Wilson construction on $S^3$''.)
...
direction (the almost-commutative lift of Paper~30's $\mathrm{SU}(2)$
Wilson instance is the most concrete $S^3$-only target, and G4a the
remaining electroweak-unification path;\ the chirality-grading route is
closed in the negative, \S\ref{sec:g3}).  Per the project's rhetoric rule, no claim is made that GeoVac
...
Ch.~1~\S 13 (the finite noncommutative
geometry)~\cite{connes_marcolli2008}, the finite electroweak Hilbert
...
truthful CH Dirac.  \textbf{[SYMBOLIC + MEASURED]} The combined real structure is $J = J_{\mathrm{GV}}
...
and $J_F D_F = +D_F J_F$ (the KO-dim~6 signs of $\mathcal{A}_F$ in the
Connes--Marcolli convention).  The signs of the combined $J$ follow
factor by factor:\ $J^2 = J_{\mathrm{GV}}^2 \otimes J_F^2 =
-\mathbb{1}$, and since $\gamma_{\mathrm{GV}} =
\mathrm{sign}(D_{\mathrm{GV}})$ is a function of $D_{\mathrm{GV}}$,
$J D J^{-1} = D$, i.e.\ $J D = +D J$.  The combined sign pair
$(\varepsilon, \varepsilon') = (-, +)$ is the sign pair of the GV factor,
which $J_F$ leaves unchanged; in Connes' sign table~\cite{connes1995}
(in the three-sign convention of~\cite{vansuijlekom_book2015}) it is
shared by the KO-2, KO-3 and KO-4 columns, KO-3 being the odd continuum
factor's own label.  It is \emph{not} the KO-dimension $3 + 6
\equiv 1 \pmod 8$ that the additive rule for products of real spectral
triples would assign:\ KO-1 has $(\varepsilon, \varepsilon') = (+, -)$,
and the additive rule is a theorem about the product whose Dirac
operator carries the grading of the even factor, $D_{\mathrm{GV}}
\otimes \gamma_F + \mathbb{1} \otimes D_F$
(the $\widetilde{\mathcal{D}}$ of
Dabrowski--Dossena~\cite{dabrowski_dossena2011}, eq.~(18), which their
\S 4.2 prescribes when the first factor is odd; their eq.~(10) is the
classical Cartesian-product analogue); for that product their Table~5
(row $3_+$) shows that Connes' KO-6 real structure on the finite factor
(column $6_+$) admits no consistent $\varepsilon'$ at all, and the
alternative real structure $J_F \gamma_F$ (column $6_-$) gives KO-1
(entry $1_-$) with $J^2 = +\mathbb{1}$, $J D = -D J$.  Our $D$ is the ungraded sum above, so the
additive rule does not apply to it, and we label it by its signs
(derived factor by factor above and confirmed numerically).  Both
signs are verified to machine precision at $n_{\max} \le
3$ (\texttt{tests/test\_almost\_commutative.py},
\texttt{test\_J\_combined\_sign\_pair}).  \textbf{[PANEL-VERIFIED]} The
pair $(-, +)$ alone does not fix a KO-dimension:\ the KO-2, KO-3 and
KO-4 columns of Connes' table~\cite{connes1995,vansuijlekom_book2015} share it; KO-3 is the label of the odd
continuum GV factor ($S^3$ carries no chirality).  At finite cutoff the chirality-doubling grading
$\gamma_{\mathrm{GV}} = \sigma_x \otimes \mathbb{1}$
(\texttt{geovac/chirality\_grading.py}), extended as
$\gamma_{\mathrm{GV}} \otimes \mathbb{1}_F$, squares to one,
anticommutes with the combined $D$ and commutes with $J$ bit-exactly
at $n_{\max} \le 3$, so the finite combined triple is \emph{even} with
$(\varepsilon, \varepsilon', \varepsilon'') = (-, +, +)$, the KO-4
column, i.e.\ the sign triple of that column (measured 2026-09-02; pinned in the same test).  We therefore
attach \emph{no} KO-dimension label to the finite truncation and
record the measured sign triple $(-, +, +)$ instead (PI direction,
2026-09-02):\ the continuum factor is KO-3 and the finite grading reads
as KO-4, so any single label at finite cutoff is a convention, not a
result.  (An earlier version of this paragraph carried the label KO-3;
before 2026-09-02 the paper carried KO-1, from the additive rule that
does not apply here.)
...
GV and weak-isospin chiralities (G3 stays open).  Per the project's
rhetoric rule, no claim is made that GeoVac contains the Higgs:\ the
...
\texttt{tests/test\_almost\_commutative.py} (56 passing as of
2026-09-03);\ data
...
(Paper~35, tensor-product verification section), and the inner
$\mathrm{Tr}_{\mathrm{inner}}(Y_{\mathrm{diag}}^2)$ is a $k=2$
Dirichlet sum in the Yukawa eigenvalues
(Paper~18~\S~IV.6; $\eta$-trivialization-theorem-style
...
deliver a specific value at each.  \textbf{[CONDITIONAL]} Three
further candidates are \emph{forbidden} by the $\eta$-trivialization
theorem (Paper~18~\S~IV.6) on any Krajewski-class even spectral
triple --- a positive structural prediction, conditional on the
observable being carried by the $\eta$-trivialised inner factor:\
axial chemical potential thermodynamics, parity-odd transport, and
finite-$T$ EDMs vanish identically on the $\eta$-trivialised inner
...
hydrides (Paper~19~\cite{loutey_paper19}, Sec.~\texttt{sec:w1c\_residual}). The bridge sprint
...
three to four orders of magnitude below the bond-energy scale (maximum
differential $2.7 \times 10^{-4}$ of $D_e = 0.075$~Ha in the sprint
memo's own table). Ruled out kernel-shape
...
\mathbf{0.0007}]$ --- a three-order-of-magnitude change ($1.0000 \to
0.0007$) in dominant
...
with revised five-sub-layer hierarchy), Paper~17~\cite{loutey_paper17} \S6.10 (composed-architecture
...
($\beta$-update, 2026-05-23).} Three diagnostic sprints in immediate succession
...
that requires beyond-sprint-scale architectural lift} (Schmidt
orthogonalization at the basis-construction level, DMRG-class
beyond-FCI methods, or explicit correlation factors --- all are
beyond-sprint-scale architectural investments). W1e characterizes a
...
candidates are beyond-sprint-scale commitments rather than sprint-scale
...
the broader multi-focal-composition wall taxonomy of the working-hypothesis register
...
of the diagnostic-before-engineering discipline (working-hypothesis register)
...
Paper~19 \S\texttt{sec:w1e\_refinement\_f4\_f6} (refined
...
correlation Day-1 diagnostics (2026-05-23).} The two beyond-sprint-scale
...
closure candidates are substantial architectural commitments. The W1e wall is the
...
\S\texttt{sec:w1e\_schmidt\_core\_correlation} and Paper~34
\S\texttt{sec:conv\_w1e\_cross\_domain\_wall}.
...
The finite factor therefore carries the KO-dim~6 signs
$(\varepsilon_F, \varepsilon'_F, \varepsilon''_F) = (+, +, -)$;
\textbf{[SYMBOLIC + MEASURED]} the
combined real structure on H1's combined Dirac carries the sign pair
$(\varepsilon, \varepsilon') = (-, +)$, the GV factor's pair --- with
$\varepsilon'' = +1$ under the finite grading, and no finite-cutoff
KO label attached (\S\ref{sec:higgs_h1}).  The additive rule $3 + 6 \equiv 1 \pmod 8$
does not apply to H1's ungraded sum $D_{\mathrm{GV}} \otimes
\mathbb{1}_F + \gamma_{\mathrm{GV}} \otimes D_F$; it governs the
graded product of Dabrowski--Dossena~\cite{dabrowski_dossena2011},
...
The product works as an involution; the \emph{difference} does not.
$\gamma_5$ is a well-defined $\mathbb{Z}_2$ operator, but it is a
\emph{grading of $D$} only for the production $\gamma_{\mathrm{GV}} =
\sigma_x \otimes \mathbb{1}$ at vanishing Yukawa: with
$\gamma_{\mathrm{GV}} = \mathrm{sign}(D_{\mathrm{GV}})$ one has
$\|\{\gamma_5, D\}\|_F = 17, 52, 110$ at $n_{\max} = 1, 2, 3$, and with
the production grading $4.7, 9.3, 14.8$ at $Y \ne 0$ (two distinct
operators carried the name $\gamma_{\mathrm{GV}}$ in this section;
disambiguated 2026-09-03).  $\gamma_{\mathrm{GV}}$ and $\gamma_F$ do not
...
\texttt{tests/test\_almost\_commutative.py} (56 passing,
...
Connes SM finite algebra is reachable as a bounded sprint-scale extension
...
Paper~24~\S 5.3 records these layers of this asymmetry (seven in the
current count, Paper~31), all Coulomb-specific; the first four:\ (i)~spectrum-computing role of $L_0$;\
...
\emph{sibling} of these layers, not a lone additional layer:\ the
...
the sense of the project's rhetoric rule:\ the framework is consistent with a
...
\textbf{[PANEL-VERIFIED]} The axioms $J^2 = -I$, $JD = +DJ$, $D = D^\dagger$
and $\gamma_F^2 = I$ hold at machine zero at every tested $n_{\max}$.
\emph{Scope correction (2026-09-03):} the order-zero and order-one
residuals were also machine zero, but for a restricted reason --- the
module's finite algebra acted on the matter sector only while
$J b J^{-1}$ acts on antimatter, so the two commute by disjoint support
(and the module's quark action $\mathrm{kron}(ew, m)$ was not a linear
$*$-representation of the direct sum; corrected in
\texttt{geovac/standard\_model\_triple.py}).  With the corrected
representation the finite-algebra order-zero and order-one residuals are
exactly zero, while the combined-triple residuals ($1.49$ and $1.26$ at
$n_{\max} = 2$) are the GV factor's finite-resolution residual (the operator-system
multipliers do not commute; cf.\ the order-zero/one rows of
Table~\ref{tab:axiom_audit_lorentzian}), factorising through the
operator-system commutator with residual $0$ (\texttt{tests/test\_standard\_model\_triple.py});
the gauge census still returns $\mathrm{U}(1) \times \mathrm{SU}(2)
\times \mathrm{SU}(3)$ at $n_{\max} = 2$ (colour now read from the
antiquark block); and the combined grading
$\gamma_{\mathrm{GV}} \otimes \gamma_F$ built from $\gamma_{\mathrm{GV}} =
\mathrm{sign}(D_{\mathrm{GV}})$ does \emph{not} anticommute with $D$
($\|\{\gamma, D\}\|_F = 34, 104, 220$ at $n_{\max} = 1, 2, 3$), so the
``no finite-resolution degradation'' reading is withdrawn; with the
production grading $\gamma_{\mathrm{GV}} = \sigma_x \otimes \mathbb{1}$
...
exactly.  \textbf{[PANEL-VERIFIED]} The predicted POSITIVE-THIN verdict is confirmed:
...
\textbf{[PANEL-VERIFIED]} The spectral-action Mexican-hat potential is confirmed:
$\Tr(D_F^2) > 0$ and $\Tr(D_F^4) > 0$ give the standard CCM
sign structure $V(\Phi) = -\mu^2 |\Phi|^2 + \lambda |\Phi|^4$.

\begin{theorem}[H1 Yukawa non-selection]
\label{thm:h1_yukawa_non_selection}
Within the almost-commutative spectral triple
$(\mathcal{A}_{\mathrm{GV}} \otimes \mathcal{A}_F, \mathcal{H}_{\mathrm{total}}, D)$
with $\mathcal{A}_F = \mathbb{C} \oplus \mathbb{H} \oplus M_3(\mathbb{C})$,
the space of admissible finite Dirac operators $D_F$ satisfying all
Connes axioms (Hermiticity, chirality grading, $J$-reality, order-one
condition) carries, after the matter-sector reductions $512 \to 256$
(Hermiticity) $\to 128$ (chirality grading) $\to 128$ ($J$-reality;
order-one imposes no further constraint), $128$ real parameters in the
matter sector \emph{under the degenerate algebra sample used before
2026-09-03}; with a linear $*$-representation of $\mathbb{C} \oplus
\mathbb{H} \oplus M_3(\mathbb{C})$ the matter-sector projection of the
full-axiom moduli space has rank $16$ and the full-axiom dimension is
$32$ --- see Theorem~\ref{thm:forced_count}. The Yukawa non-selection
conclusion is independent of this count.) For
one generation, $Y = \mathrm{diag}(y_\nu, y_e, y_u, y_d)$ accounts
for $8$ of these. The gauge sector
$\mathrm{U}(1) \times \mathrm{SU}(2) \times \mathrm{SU}(3)$ is
\textbf{forced}; the Yukawa values are \textbf{not selected} by any
morphism in $\mathcal{A}_{\mathrm{GV}}$.
\end{theorem}

\begin{proof}[Proof sketch]
The parameter count follows from direct enumeration on the
chirality-graded $J$-real subspace of Hermitian operators on
$\mathcal{H}_F$. The non-selection of the Yukawa is the G3
structural finding (Sprint G3, \S\ref{sec:g3}):
$\gamma_{\text{GV}}$ and $\gamma_F$ are independent commuting
$\mathbb{Z}_2$'s on $\mathcal{H}_{\mathrm{GV}} \otimes \mathcal{H}_F$,
so no GeoVac-side data couples to the $\gamma_F$ flip that carries
the Yukawa. The full fluctuated Dirac $D_A = D + A + JAJ^{-1}$ was
constructed at $n_{\max} \in \{2, 3\}$
($\dim \mathcal{H}_{\text{total}} = 512, 1280$) and the inner
fluctuations decompose cleanly into the forced gauge piece (matching
G4a) plus the Higgs piece (complex $\mathrm{SU}(2)$ doublet $\Phi$ in
the L--R off-diagonal block, non-trivial iff $Y \ne 0$). The
spectral-action Mexican-hat potential is confirmed:
$\Tr(D_F^2) > 0$ and $\Tr(D_F^4) > 0$ give the standard CCM sign
structure $V(\Phi) = -\mu^2 |\Phi|^2 + \lambda |\Phi|^4$. See
\texttt{debug/h1\_higgs\_inner\_fluctuation\_memo.md}.
\end{proof}

\paragraph{The forced/free seam (forcing-catalogue Door~4, 2026-06-01).}
\textbf{[OBSERVATION]} The Yukawa non-selection theorem admits a sharper structural reading.
The almost-commutative factorization $D^2 = D_{\mathrm{GV}}^2 \otimes 1
+ 1 \otimes D_F^2$ (the cross term vanishes by chirality
anticommutation \emph{for the graded product} $D = D_{\mathrm{GV}} \otimes
1 + \gamma_{\mathrm{GV}} \otimes D_F$ with $\{\gamma_{\mathrm{GV}},
D_{\mathrm{GV}}\} = 0$; for the module's $\gamma_{\mathrm{GV}} =
\mathrm{sign}(D_{\mathrm{GV}})$ the cross term is $2|D_{\mathrm{GV}}|
\otimes D_F \ne 0$, norms $4.3, 13.2, 28.0$ at $n_{\max} = 1, 2, 3$;
scoped 2026-09-03) factorizes the master Mellin engine into an
...
result, leaves the inner Yukawa ring sharing no generator with any forced
...
rings sharing no generator, and the Yukawa is free \emph{because}
...
there. \textbf{[PANEL-VERIFIED]} A finite-algebra enumeration shows that among all finite
...
$\mathrm{SU}(2)$) might select $\mathbb{H}$ over $M_2(\mathbb{C})$. \textbf{[SYMBOLIC PROOF]} A
...
$J = J_{\mathrm{GV}} \otimes J_F$. The combined $J^2$ and the combined
$(\varepsilon, \varepsilon')$ signs are
...
$(\mathcal{H}_F, D_F, J_F, \gamma_F)$ data) sharing no generator with
any forced outer ring (an Observation-level reading, \S\ref{sec:g4}
below; ``provably disjoint'' withdrawn 2026-09-03). Second, Paper~0~\S VII:\ the packing construction is
...
Paper~0 does not contain --- recorded as the open (long-range) direction
...
sub-sprints.  \textbf{[SYMBOLIC + MEASURED]} Stated as a theorem it makes explicit what the
...
D_{\mathrm{GV}} \otimes 1 + \gamma_{\mathrm{GV}} \otimes D_F,\; J,\; \gamma)$ be the
GeoVac almost-commutative spectral triple of
Sec.~\ref{sec:construction} (the graded product the modules build;
the count below concerns $D_F$ alone), assuming:
\begin{enumerate}
...
dimension forced by these inputs alone, with the full-axiom reduction
chain at $N_{\mathrm{gen}} = 1$ (on the full $32$-dimensional finite
geometry):
\begin{equation}
\label{eq:forced_count_chain}
\dim_{\mathbb{R}}\, \mathcal{M}(D_F)
:\; 2048 \xrightarrow{\textnormal{Herm.}} 1024
     \xrightarrow{\{\gamma_F, D_F\}=0} 512
     \xrightarrow{[J_F, D_F]=0} 272
     \xrightarrow{\text{order-one}} 32 .
\end{equation}
\textbf{[SYMBOLIC + MEASURED]} The endpoint $32$ is computed with a linear
$*$-representation of $\mathcal{A}_F$ on $\mathbb{C}^{32}$ (particles
colour-blind, $M_3(\mathbb{C})$ acting on antiquarks; three independent
linear-algebra routes agree, singular-value gap $12.4$ vs $10^{-13}$;
\texttt{tests/test\_trunk\_qa\_forced\_count\_moduli.py}).  Its projection
onto the $16$-dimensional matter sector has rank $16$ --- the \emph{full}
lepton $2\times2$ and quark-flavour $2\times2 \otimes \mathbb{1}_3$ blocks, of
which the four colour-diagonal Yukawas $y_\nu, y_e, y_u, y_d$ with their
$L \leftrightarrow R$ placements are $4$ complex, i.e.\ \emph{half}
(corrected 2026-09-04:\ the statement named only those four, which the proof
below already calls false and which is $8$ real, not $16$) and onto the matter--antimatter (Majorana) block rank $16$.
\emph{Correction (2026-09-03):} the endpoint printed before, $260$ (with a
matter-sector chain $512 \to 256 \to 128 \to 128$), was an artefact of a
degenerate algebra sample --- eighteen of the twenty-four basis elements
were identically zero and the remaining six had no quark block, because
the module's quark action $\mathrm{kron}(ew, m)$ vanished at $ew = 0$ or
$m = 0$ --- so the order-one condition added almost no constraint; every
``$128$'' quoted earlier in this paper as a matter-sector moduli count
refers to that degenerate sample.  The physical-slice dimension under
restriction to diagonal Yukawa
$Y = \mathrm{diag}(y_\nu, y_e, y_u, y_d)$ is $8$ per generation.  The
$N_{\mathrm{gen}}$-generation count with full generation-mixing Yukawa
matrices is not re-derived here (the $128 \cdot N_{\mathrm{gen}}^2$
stated before 2026-09-03 was the degenerate-sample figure).
\end{theorem}

\begin{proof}[Proof sketch]
The full-axiom chain in eq.~\eqref{eq:forced_count_chain} is the
direct parameter-count of the constraint solution space
(\texttt{tests/test\_trunk\_qa\_forced\_count\_moduli.py}) on the full
$32$-dimensional finite geometry at $n_{\max} = 2$,
$N_{\mathrm{gen}} = 1$:\ Hermiticity halves the free parameters;
$\{\gamma_F, D_F\} = 0$ kills the chirality-diagonal blocks;
$J$-reality identifies the matter and antimatter sectors
($512 \to 272$); the order-one condition, imposed with a linear
$*$-representation of $\mathcal{A}_F$, removes $240$ further
parameters ($272 \to 32$): with $M_3(\mathbb{C})$ acting on the
antiquark colour index, $[[D_F, a], J b J^{-1}] = 0$ forces the quark
Yukawa blocks to be colour-diagonal and colour-independent, and strongly
constrains the matter$\to$antimatter block $P$ (it does \emph{not} annihilate
it --- the rank-$16$ real Majorana slice survives, as the next paragraph details).

\emph{What survives, measured (corrected 2026-09-03).}  The sketch printed
here previously said the condition ``kills every lepton--quark and off-flavour
entry'' and described the rank-$16$ matter projection as the four
colour-diagonal Yukawas with their $L \leftrightarrow R$ placements.  Both
descriptions are false, and the support pattern of the solution space is the
theorem's actual content.  The matter block (rank $16$ real $=8$ complex) is
\emph{full} on the lepton $2\times2$ --- including $\nu_L \to e_R$ and
$e_L \to \nu_R$ --- and full on the quark flavour $2\times2 \otimes \mathbb{1}_3$,
including $u_L^c \leftrightarrow d_R^c$;\ the four named Yukawas are $4$
complex, i.e.\ \emph{half} of it.  The Majorana block (rank $16$ real) carries
$2$ complex $\nu_R$ entries and \emph{six} complex
$\text{lepton}_R \leftrightarrow \text{antiquark-}u_R$ entries --- so twelve of
its sixteen real moduli are precisely the lepton--quark operators the old
sentence said were killed.  The endpoint $32$ and the two rank-$16$ projections
are unchanged.  Each constructed-triple
axiom-consistency check passes at
$n_{\max} \in \{2, 3\}$ in module
\texttt{geovac/standard\_model\_triple.py} (backed by
\texttt{tests/test\_standard\_model\_triple.py}, 47 tests).  \emph{QA
correction (2026-06-14):} those 47 tests verify the axiom consistency of
one constructed $D_F$, \emph{not} the moduli-dimension counts of the
chain; the counts above are the direct enumeration of
\texttt{tests/test\_trunk\_qa\_forced\_count\_moduli.py}.
\emph{Statement relabelling (2026-09-01):}
eq.~\eqref{eq:forced_count_chain} originally printed the mixed chain
$1024 \to 512 \to 128 \to 128$, which matched neither the full-axiom
nor the matter-sector convention; it now states the full-axiom count
with the matter-sector restriction labelled explicitly, applying the
relabelling this QA correction directed.  The
...
\begin{itemize}\setlength{\itemsep}{2pt}
\item \textbf{Door 4 seam reading (Yukawa-value wall):}\ the
  $\eta$-trivialization plus tensor factorization plus the G3
  commuting-$\mathbb{Z}_2$ result place the $D_F$-entries inside
  $\mathcal{M}(D_F)$ in a Dirichlet ring sharing no generator with any
  forced outer ring, so the framework does not derive them from the
  forced rings (an Observation-level reading).  \textbf{[PANEL-VERIFIED]}
  Sprint Yukawa-PSLQ (2026-06-03) added empirical confirmation:\ a
...
\item \textbf{Direction 2 packing-reach theorem:}\
  ...
  \texttt{debug/seam\_packing\_scoping\_memo.md}).
\end{itemize}
\textbf{[CONDITIONAL]} Theorem~\ref{thm:forced_count} thus crystallises the \emph{full}
...
% -------------------------------------------------------------------
% N_gen non-selection theorem (Sprint C3 closure, 2026-06-08).
% Upgrades the Direction 2 / Read 2 scoping verdicts to a formal
% structural theorem conditional on canonical CCM representation.
% -------------------------------------------------------------------

\begin{theorem}[$N_{\mathrm{gen}}$ non-selection under canonical CCM representation]
\label{thm:n_gen_non_selection}
Let $\mathcal{A}$ denote the framework's current axiom set
$\{$Paper 0 packing axiom; standard real-spectral-triple axioms
(Hermiticity, $J$-reality, KO-dim chirality, order-one); Hopf-tower
truncation (Hurwitz); Bertrand's classical theorem; Upgrade B
(sphere-Lie-group axiom)$\}$, and fix the canonical Chamseddine--Connes
SM representation of $\mathcal{A}_F = \mathbb{C} \oplus \mathbb{H}
\oplus M_3(\mathbb{C})$ on the inner Hilbert space.

For every integer $N \ge 1$, let $\mathcal{T}_N$ denote the GeoVac
spectral triple with inner Hilbert-space multiplicity $N$ (i.e.,
$\dim \mathcal{H}_F = 32 N$ under the canonical rep).  Then
$\mathcal{T}_N$ satisfies every axiom in $\mathcal{A}$.  Consequently,
$\mathcal{A}$ contains no morphism that selects a specific value
of $N$.
\end{theorem}

\begin{proof}[Proof sketch]
Each axiom in $\mathcal{A}$ is independent of $N$:
\begin{itemize}\setlength{\itemsep}{2pt}
\item The Paper 0 packing axiom outputs $(n, l, m, m_s)$ labels for a
  single $S^3$ outer graph;  $N$ is not in this output.
\item The standard real-spectral-triple axioms constrain the relations
  $(D, J, \gamma)$ on $\mathcal{H} = \mathcal{H}_{\mathrm{GV}} \otimes
  \mathcal{H}_F$.  $\dim \mathcal{H}_F$ scales linearly with $N$, but
  Hermiticity, $J$-reality, KO-dim chirality, and order-one are
  structural relations independent of multiplicity.
\item Hopf-tower truncation (Hurwitz, Doors 4b/4d/4e) operates on
  $\mathcal{A}_F$;  the algebra $\mathcal{A}_F = \mathbb{C} \oplus
  \mathbb{H} \oplus M_3(\mathbb{C})$ is fixed independent of $N$.
\item Bertrand's classical theorem selects the outer manifold $S^3$;
  $N$-independent.
\item Upgrade B constrains the inner-algebra structure via the
  sphere-Lie-group identification of Door 4e;  $N$-independent.
\item The canonical CCM SM representation distributes $32$ fermion
  degrees of freedom per generation across the three algebra factors;
  the multiplicity $N$ enters as an external Hilbert-space multiplicity
  rather than as a constraint emergent from the rep structure.
\end{itemize}

The Read 2 scoping
(\texttt{debug/sprint\_read2\_n\_gen\_scoping\_memo.md}, 2026-06-03)
provides three independent obstructions to any candidate
Hopf-tower-to-representation shortcut that would correlate $N$ with
the algebra factor count under canonical representation:\ the SM
fermion DOF count per generation distributes across all three algebra
factors;  the Yukawa-Higgs cross-generation coupling structure
required for CKM/PMNS phenomenology prevents algebra-factor
reorganisation;  and the alternative ``3 factors $=$ 3 generations''
assignment gives $12$ DOF per gen versus the $32$ DOF of the SM.
Each obstruction independently closes the candidate shortcut.

Therefore $\mathcal{T}_N$ is a valid GeoVac spectral triple for every
$N \ge 1$ under canonical representation;  no axiom in $\mathcal{A}$
has been exhibited that selects a specific value. $\hfill\square$
\end{proof}

\begin{remark}[Scope and the unconditional question]
\label{rem:n_gen_scope}
Theorem~\ref{thm:n_gen_non_selection} is conditional on the canonical
CCM SM representation choice.  The unconditional question --- whether
any SM-phenomenologically-consistent representation of $\mathcal{A}_F$
on $\mathcal{H}_F$ could correlate $N$ with the algebra factor count
under an alternative rep --- remains the open long-range structural-
research target named by the Read 2 sharpest falsifier:\ exhibit a
representation in which the $32$ fermion DOF per generation are
distributed across the three algebra factors in a way that makes
the three generations the three rep blocks of the algebra, and the
cross-generation Yukawa-Higgs coupling (CKM/PMNS) is naturally
encoded as the algebra cross-factor coupling.  Absent such a
construction, Theorem~\ref{thm:n_gen_non_selection} stands as the
conditional impossibility result corresponding to the Read 2
NO-GO scoping verdict.

The Theorem identifies $N_{\mathrm{gen}}$ as Class 1 calibration data
(external input on which the framework's outer-factor mechanism is
structurally silent), formalising the Direction 2 packing-reach
and Read 2 representation-level NO-GO scopings into a single
structural statement.  It is the multiplicity-side analogue of the
H1 Yukawa non-selection theorem:\ both are non-selection results for
inner-factor content, both conditional on the canonical rep, and
together with the Forced-Count Theorem~(\ref{thm:forced_count}) they
characterise the full free-side content of the inner-factor
structural-skeleton boundary at the canonical-representation level.

\textbf{[CONDITIONAL]} Paper~57 \S 6.1 (the Forced/Free Seam observations paper) records
this theorem as the C3 closure of the empirical Direction 2 / Read 2
NO-GO scopings, and as one of three theorem-grade non-selection
results in the corpus (alongside H1 Yukawa non-selection and the
Forced-Count Theorem).
\end{remark}

% -------------------------------------------------------------------
% Inner KO-dim non-selection theorem (Sprint F3 closure, 2026-06-08).
% Companion to thm:n_gen_non_selection; same Direction 2 NO-GO basis,
% but the structural argument is more direct: packing-is-kinematic per
% Paper 0 §VII.B, and KO-dim is real-structure data.
% -------------------------------------------------------------------

\begin{theorem}[Inner KO-dim is external calibration data under canonical-rep convention]
\label{thm:ko_dim_non_selection}
Let $\mathcal{A}$ denote the framework's current axiom set
(as defined in \emph{Theorem~\ref{thm:n_gen_non_selection}}).
The Paper~0 packing axiom is kinematic
(Paper~0~\S VII.B ``What the construction does not provide''):  it
outputs the labels $(n, l, m, m_s)$ and the $S^3$ graph topology, but
neither a Dirac operator, nor a real structure, nor a chirality
grading.  The inner KO-dimensional signature is a property of the
real-structure data $(D_F, J_F, \gamma_F)$, none of which is in
the packing axiom's output.

The remaining axioms in $\mathcal{A}$ (standard real-spectral-triple
axioms, Hopf-tower truncation, Bertrand's classical theorem,
Upgrade~B) constrain the consistency of $(D_F, J_F, \gamma_F)$ with
the outer GeoVac triple's KO-dimension~$3$, but do not uniquely
select an inner KO-dim signature.  Different choices of
$(D_F, J_F, \gamma_F)$ that are consistent with these constraints
yield different inner KO-dimensional signatures, each producing a
valid GeoVac spectral triple.  The canonical Chamseddine--Connes
SM representation (inner KO-dim~$6$) is one such choice, supplied as
external input to the framework.
\end{theorem}

\begin{proof}[Proof sketch]
The argument is a direct composition of two established facts:

\textbf{Fact 1 (Paper~0 \S VII.B, explicit scope statement).} The
packing construction is \emph{kinematic}:\ it produces labels and
graph topology, and explicitly does \emph{not} produce operators,
real structures, couplings, or fiber-Hilbert-space multiplicities.

\textbf{Fact 2 (Door~4f T1, Door~4b Q3).} The inner KO-dimensional
signature is defined as a property of the real-structure data
$(D_F, J_F, \gamma_F)$. The chirality-grading axiom
$\{\gamma_F, D_F\} = 0$ and the $J_F$-reality conditions
$J_F^2 = \pm \mathbf{1}$, $J_F D_F = \pm D_F J_F$, $J_F \gamma_F =
\pm \gamma_F J_F$ specify the residue classes mod~$8$ that are
consistent, but do not uniquely select one in the absence of
additional constraints (such as the phenomenological constraints
imposed by SM fermion doubling and Majorana neutrino-mass
structure).

Composition.  $\mathcal{A}$ produces no $(D_F, J_F, \gamma_F)$ data
autonomously (Fact 1), and the inner KO-dim is a function of that
data (Fact 2).  Therefore $\mathcal{A}$ does not autonomously select
an inner KO-dim signature.  The canonical CCM SM rep supplies a
specific $(D_F, J_F, \gamma_F)$ triple (KO-dim~$6$);  alternative
specifications consistent with the outer KO-dim~$3$ and the standard
real-spectral-triple axioms produce alternative KO-dim signatures,
each consistent with $\mathcal{A}$. $\hfill\square$
\end{proof}

\begin{remark}[The full inner-factor structural-skeleton boundary]
\label{rem:full_inner_factor_boundary}
Theorems~\ref{thm:forced_count}, the H1 Yukawa non-selection
(\S\ref{sec:open}), \ref{thm:n_gen_non_selection}, and
\ref{thm:ko_dim_non_selection} together characterise the full
free-side content of the inner-factor structural-skeleton boundary
at the canonical-representation level:
\begin{itemize}\setlength{\itemsep}{2pt}
\item \textbf{Forced data:}  the moduli space $\mathcal{M}(D_F)$ and
  its dimension (full-axiom:\ $32$ at $N_{\mathrm{gen}} = 1$, matter-sector
  projection rank $16$, $8$ per generation on the diagonal-Yukawa slice;
  the $128\,N_{\mathrm{gen}}^2$ matter count quoted before 2026-09-03 was a
  degenerate-sample artefact --- see Theorem~\ref{thm:forced_count}).
\item \textbf{Free data:}  (i) the point in $\mathcal{M}(D_F)$
  (Yukawa values --- H1 non-selection),
  (ii) the Hilbert-space multiplicity ($N_{\mathrm{gen}}$ ---
  Theorem~\ref{thm:n_gen_non_selection}),
  (iii) the real-structure signature (inner KO-dim ---
  Theorem~\ref{thm:ko_dim_non_selection}).
\end{itemize}

These four theorems together formalise the structural-skeleton scope
pattern at the inner-factor level for the GeoVac framework's current
axiom set.  Each is conditional on the canonical CCM SM
representation.  The unconditional versions --- whether alternative
phenomenologically-consistent representations could correlate any of
the three free data with framework-derivable structure --- remain
long-range structural-research targets;  Read~2 sharpest falsifier
names the candidate construction for $N_{\mathrm{gen}}$, and
analogous targets exist for KO-dim (a non-canonical rep that admits
a different KO-dim while retaining SM phenomenology) and for Yukawa
values (a packing-derivable structure on $\mathcal{A}_F$ that selects
within the diagonal slice).

Paper~57 \S 6.3 (Forced/Free Seam observations paper) records the
trio plus Forced-Count Theorem as the C3 closure of the inner-factor
non-selection arc.
\end{remark}

% -------------------------------------------------------------------
% Single-cutoff spectral action theorem (Sprint E7/E8 closure,
% 2026-06-08).  Covers E7, E8, G3, G4 catalogue entries:  the
% renormalization sub-sector of the multi-focal-composition wall.
% -------------------------------------------------------------------

\begin{definition}[Multi-cutoff spectral action structure]
\label{def:multi_cutoff}
A multi-cutoff spectral action on a real spectral triple
$\mathcal{T} = (\mathcal{A}, \mathcal{H}, D)$ is a functional of the
form
\[
  S^{\mathrm{multi}}(D, \{\Lambda_i\}_{i=1}^k, \{f_i\}_{i=1}^k)
  \;=\; \sum_{i=1}^{k} c_i \, \mathrm{Tr}\, f_i(D/\Lambda_i)
\]
or more generally any functional of $D$ depending on $k \ge 2$
independent energy scales $\Lambda_1, \ldots, \Lambda_k$ in a manner
not reducible to a function of a single scale by reparametrization.
\end{definition}

\begin{theorem}[Single-cutoff spectral action]
\label{thm:single_cutoff_spectral_action}
The framework's current axiom set $\mathcal{A}$ (as defined in
\emph{Theorem~\ref{thm:n_gen_non_selection}}) specifies the
Chamseddine--Connes spectral action $S(D, \Lambda) = \mathrm{Tr}\,
f(D/\Lambda)$ as a single-cutoff functional in the sense of
\emph{Definition~\ref{def:multi_cutoff}}.  No morphism derivable
from $\mathcal{A}$ produces a multi-cutoff spectral action structure.
\end{theorem}

\begin{proof}[Proof sketch]
The argument is a structural inspection of each axiom in $\mathcal{A}$:

\begin{enumerate}\setlength{\itemsep}{2pt}
\item \textbf{Spectral action axiom (CCM).}  The standard CC spectral
  action $S(D, \Lambda) = \mathrm{Tr}\, f(D/\Lambda)$ specifies a
  single-cutoff functional by stipulation.  The argument $D/\Lambda$
  is a dimensionless ratio against a single energy scale.
\item \textbf{Paper 0 packing axiom + standard real-spectral-triple
  axioms + Hopf-tower truncation + Bertrand + Upgrade B.}  These
  axioms are algebraic, kinematic, and topological:\ they specify
  $(\mathcal{A}_F, \mathcal{H}_F, D_F, J_F, \gamma_F)$ and the outer
  manifold geometry, but introduce no energy-scale data.
\item \textbf{Inner fluctuations $D \to D + A$ for $A = a[D, b]$.}
  These preserve the single-Dirac structure of the underlying
  spectral triple.  The fluctuated operator $D' = D + A$ enters the
  spectral action as $S(D', \Lambda)$, still a single-cutoff
  functional.
\item \textbf{Tensor products $\mathcal{T}_1 \otimes \mathcal{T}_2$
  (Paper 39).}  These produce a new spectral triple with combined
  Dirac $D_1 \otimes 1 + 1 \otimes D_2$, on which the spectral
  action $S(D_1 \otimes 1 + 1 \otimes D_2, \Lambda)$ remains
  single-cutoff with the new combined $D$.  No second cutoff is
  introduced by the tensor-product construction.
\end{enumerate}

No axiom in $\mathcal{A}$ introduces a second independent energy
scale, and no morphism derivable from $\mathcal{A}$ produces a
functional of the form
$S^{\mathrm{multi}}(D, \Lambda_1, \Lambda_2, \ldots, \Lambda_k, \ldots)$
with $k \ge 2$ independent scales.  Therefore the framework's
spectral action machinery is single-cutoff. $\hfill\square$
\end{proof}

\begin{corollary}[Multi-loop renormalization counterterms are external calibration data]
\label{cor:multi_loop_renormalization_wall}
Multi-loop QED renormalization counterterms (wavefunction
renormalization $Z_2$, mass counterterm $\delta m$, vertex
counterterm $Z_1$, etc.) require a multi-cutoff structure
$(\Lambda_{UV}, \mu_R)$ where $\Lambda_{UV}$ is the ultraviolet
cutoff for the bare integrand and $\mu_R$ is the renormalization
scale at which finite subtraction is performed. By
\emph{Theorem~\ref{thm:single_cutoff_spectral_action}},
$\mathcal{A}$ contains no morphism producing such a structure.
Therefore the multi-loop QED counterterms are external calibration
data, supplied to the framework rather than derived from it.

Consequently the following catalogue entries from
Paper~57~\cite{paper57} are classified as multi-cutoff
renormalization calibration data:
\begin{itemize}\setlength{\itemsep}{2pt}
\item \textbf{E7:} two-loop self-energy counterterms $Z_2, \delta m$.
\item \textbf{E8:} multi-loop QED corrections beyond the one-loop
  LS-7 closure.
\item \textbf{G3:} HF-5 multi-loop QED on hyperfine.
\item \textbf{G4:} LS-8a two-loop SE renormalization (identical to E7
  under the cross-domain unity of Paper~57~\S 4).
\end{itemize}

The LS-8a sprint (May 2026) empirically confirmed this structural
verdict:\ the framework's iterated Chamseddine--Connes spectral
action reproduces the two-loop UV-divergent integrand at the right
prefactor, sign, and divergence rate $\sim N^{3.79}$ (Paper 36
LS-8a), but cannot autonomously generate the $Z_2, \delta m$
counterterms required for finite multi-loop closure.
Theorem~\ref{thm:single_cutoff_spectral_action} formalises the
structural reason for the LS-8a wall:\ single-cutoff machinery
cannot produce multi-cutoff observables.
\end{corollary}

\begin{remark}[Honest scope]
\label{rem:single_cutoff_scope}
Theorem~\ref{thm:single_cutoff_spectral_action} is theorem-grade
within the framework's current axiom set $\mathcal{A}$.  Two scope
clarifications:

\textbf{(i) The theorem does not claim ``no multi-cutoff structure
exists.''}  It claims that no morphism derivable from $\mathcal{A}$
\emph{as currently stated} produces a multi-cutoff structure.  A
sibling axiom adding renormalization-group flow machinery would
extend the axiom set and could in principle produce multi-cutoff
observables.  Such a sibling axiom is the analog of the
$N_{\mathrm{gen}}$ sibling-axiom direction
(\emph{Theorem~\ref{thm:n_gen_non_selection}} unconditional version):
a logically-possible escape, named here, that requires new primitive
content beyond $\mathcal{A}$.

\textbf{(ii) The theorem does not address the spatial-composition
sub-sector of the multi-focal-composition wall.}  The catalogue
entries G1 (HF-3 recoil cross-register $V_{eN}(\hat r_e, \hat R_N)$),
G2 (HF-4 Zemach magnetization density), and G5 (W1e chemistry
inner-region correlation) lie outside this theorem's scope:\ they
are spatial $\times$ spatial composition walls, structurally distinct
from the multi-cutoff renormalization sub-sector.  \textbf{[OPEN]} A general
multi-focal-composition wall theorem unifying both sub-sectors
remains the named open structural-research target.

Paper~57~\S 4 (cross-domain unity) and Paper~57~\S 6.3 (Sprint E7/E8
closure) record this theorem as the renormalization sub-sector
closure of the multi-focal-composition wall.
\end{remark}

% -------------------------------------------------------------------
% Spatial-composition wall theorem (Sprint G1/G2/G5 closure,
% 2026-06-08).  Covers the remaining catalogue entries in the
% multi-focal-composition wall family:  spatial composition sub-
% sector.  Completes the multi-focal-composition wall closure
% together with thm:single_cutoff_spectral_action.
% -------------------------------------------------------------------

\begin{theorem}[Tensor-product radial coupling does not autonomously match physical two-body profiles]
\label{thm:spatial_composition_radial_wall}
Let $\mathcal{T}_a = (\mathcal{A}_a, \mathcal{H}_a, D_a^{\mathrm{CH}})$ and
$\mathcal{T}_b = (\mathcal{A}_b, \mathcal{H}_b, D_b^{\mathrm{CH}})$ be two
GeoVac spectral triples on the same outer manifold $S^3$ at distinct
focal lengths $\lambda_a \neq \lambda_b$, equipped with the
Camporesi--Higuchi spinor Dirac operators.  The framework's natural
tensor-product spectral triple
$\mathcal{T}_a \otimes \mathcal{T}_b$~\cite{paper39} produces composed
two-body operators whose:
\begin{enumerate}\setlength{\itemsep}{1pt}
\item \textbf{Angular structure} is forced:  Gaunt selection rules,
  $m$-conservation, and monopole structure are all autonomously
  reproduced by the composed algebra
  (Paper~54~\cite{paper54}~Thm 3).
\item \textbf{Radial coupling profile} is determined autonomously by
  the combined Dirac structure $D = D_a^{\mathrm{CH}} \otimes 1 +
  \gamma_a \otimes D_b^{\mathrm{CH}}$, but does not autonomously
  coincide with the physical 1/r Coulomb form, the dipole-magnetization
  form, or other specific two-body radial profiles required by
  empirical observables.
\end{enumerate}
The framework's axiom set $\mathcal{A}$ contains no morphism enforcing
that the composed radial coupling profile matches a specific physical
two-body observable; matching a given observable requires external
specification.
\end{theorem}

\begin{proof}[Proof sketch]
The two assertions of the theorem stand on independent structural
content:

\textbf{(1) Angular structure forcing.}  Paper~54 Thm~3
\cite{paper54} establishes that the composed algebra structure on
$\mathcal{A}_a \otimes \mathcal{A}_b$ acts on the two-body Hilbert
space through Wigner~3j coefficients in a way that exactly reproduces
the angular selection rules (Gaunt, $m$-conservation, monopole
hierarchy) of physical two-body Coulomb-style interactions.  This is
a direct consequence of the SU(2) representation-theoretic structure
of the angular momentum operators on $S^3$, which the framework
inherits via the Fock projection (Paper~7) and the Hopf-tower truncation.

\textbf{(2) Radial coupling non-reproduction.} The structural reason
is the non-commutativity of the Fock-projection conformal factor with
the tensor-product construction:

\begin{enumerate}\setlength{\itemsep}{1pt}
\item Paper~7's chordal-distance identity maps continuum $-Z/r$ on
  $\mathbb{R}^3$ to the Laplace--Beltrami operator on the unit $S^3$
  (whose discretisation is the graph Laplacian) via a focal-length-
  dependent stereographic conformal factor $c(\lambda)$.  Two
  different focal lengths $\lambda_a, \lambda_b$ give two different
  conformal factors $c_a, c_b$.
\item The tensor-product spectral triple
  $\mathcal{T}_a \otimes \mathcal{T}_b$ has continuum limit on the
  product manifold $S^3_a \times S^3_b$.  Under Fock projection, this
  corresponds to a six-dimensional space
  $\mathbb{R}^3_a \times \mathbb{R}^3_b$ parametrised by two
  independent position vectors, with combined conformal factor
  $c_{ab} \neq c_a \cdot c_b$ in general.
\item Physical two-body observables (V$_{eN}(\hat r_e, \hat R_N)$,
  Zemach magnetization, chemistry inner-region $V_{ee}(r_{12})$) are
  functions of the \emph{relative} coordinate $\vec r_a - \vec r_b$
  on $\mathbb{R}^3$ (three dimensions, not six).  The reduction from
  the six-dimensional product manifold to the three-dimensional
  relative manifold is not in the framework's axiom set.
\item Empirical verification:  Sprint resolvent\_two\_body
  (2026-06-01, \texttt{debug/sprint\_resolvent\_two\_body\_memo.md})
  tested four weightings of the resolvent
  $(D_a^{\mathrm{CH}})^{-2}$ against the physical 1/r Coulomb profile
  and found Pearson correlations $\le 0.81$ decreasing with
  $n_{\max}$.  Paper~54 verified the angular selection rules but
  reported only partial radial matching (75\% connected fraction).
  Together these establish that the framework's natural composition
  mechanisms (tensor product, resolvent, gauged spectral action) do
  not autonomously match physical two-body radial profiles.
\end{enumerate}

No axiom in $\mathcal{A}$ supplies the relative-coordinate reduction
from the six-dimensional product manifold to the three-dimensional
relative manifold, nor enforces a specific composed radial profile
on the tensor-product spectral triple.  Therefore the framework
specifies the angular structure of two-body operators autonomously
(via the SU(2) representation theory of $S^3$) but does not
autonomously match the radial structure of any specific physical
two-body observable. $\hfill\square$
\end{proof}

\begin{corollary}[Spatial-composition catalogue entries are calibration data]
\label{cor:spatial_composition_wall}
The following catalogue entries from Paper~57~\cite{paper57} are
classified as spatial-composition calibration data:
\begin{itemize}\setlength{\itemsep}{1pt}
\item \textbf{G1:} HF-3 recoil cross-register
  $V_{eN}(\hat r_e, \hat R_N)$ (Track NI cross-register, Sprint HF
  2026-05-07);
\item \textbf{G2:} HF-4 Zemach magnetization density (Sprint HF
  2026-05-07);
\item \textbf{G5:} W1e chemistry inner-region overattraction (Sprint
  W1c arc 2026-05-23).
\end{itemize}
Each requires a specific physical radial coupling profile that
Theorem~\ref{thm:spatial_composition_radial_wall} establishes is not
autonomously produced by the framework's tensor-product composition
mechanism.  The angular content is forced; the radial content is
external calibration.
\end{corollary}

\begin{remark}[Multi-focal-composition wall fully theorem-bound]
\label{rem:multi_focal_wall_fully_characterized}
Together with
Theorem~\ref{thm:single_cutoff_spectral_action} +
Corollary~\ref{cor:multi_loop_renormalization_wall} (E7, E8, G3, G4),
Theorem~\ref{thm:spatial_composition_radial_wall} +
Corollary~\ref{cor:spatial_composition_wall} (G1, G2, G5) close seven
catalogue entries of the multi-focal-composition wall family
(Paper~57~\S 4 cross-domain unity) at theorem-grade level under two
structurally distinct theorems:

\begin{itemize}\setlength{\itemsep}{1pt}
\item \textbf{Renormalization sub-sector} (E7, E8, G3, G4): closed by
  Theorem~\ref{thm:single_cutoff_spectral_action} — spectral action
  is single-cutoff; no multi-cutoff structure in $\mathcal{A}$.
\item \textbf{Spatial-composition sub-sector} (G1, G2, G5): closed by
  Theorem~\ref{thm:spatial_composition_radial_wall} — tensor-product
  composition reproduces angular structure but not radial structure;
  no relative-coordinate-reduction morphism in $\mathcal{A}$.
\end{itemize}

An eighth catalogue entry (G6) is the H1 Yukawa non-selection, closed
by Theorem~\ref{thm:n_gen_non_selection}'s companion result
(\S~\ref{sec:open}).  The entries are finer than the instances:\ all
six independent multi-focal-composition wall instances of
Paper~57~\S 4 (H1, LS-8a, HF-3/4/5, W1e; G6 $=$ F1 is not independent)
are now characterised at theorem-grade level by one of the three
theorems.

The multi-focal-composition wall is theorem-bound at the canonical-
representation level.  Together with the four inner-factor non-
selection theorems
(Theorems~\ref{thm:forced_count},
\ref{thm:n_gen_non_selection}, \ref{thm:ko_dim_non_selection},
plus H1), the corpus now carries six theorem-grade non-selection
results covering the inner-factor structural-skeleton boundary, the
multi-loop QED renormalization sub-sector, and the spatial-
composition sub-sector.

Paper~57~\S 6.3 (Sprint G1/G2/G5 closure) records this completion.
\end{remark}

% -------------------------------------------------------------------
% E6 — Combination rule K = π(B + F − Δ) no-single-mechanism theorem
% (Sprint E6 closure, 2026-06-08).
% -------------------------------------------------------------------

\begin{theorem}[No single-mechanism generation of the combination rule $K = \pi(B + F - \Delta)$]
\label{thm:no_single_mechanism_K}
Under the framework's axiom set $\mathcal{A}$ and the master Mellin
engine case-exhaustion classification, the three spectral homes that
appear in the empirical combination rule $K = \pi(B + F - \Delta)$
for $\alpha^{-1}$ (Paper 2) live in two distinct Mellin sub-rings:
\begin{itemize}\setlength{\itemsep}{2pt}
\item $B = 42$ — Casimir trace on the scalar/representation sector at
  $m = 3$;  $M_2$ sub-ring (heat-kernel period).
\item $F = \pi^2/6$ — Fock-degeneracy Dirichlet at the packing exponent
  $d = 3$ (the $\zeta(2)$ value);  $M_2$ sub-ring.
\item $\Delta = 1/40 = \bigl(g_3^{\rm Dirac}\bigr)^{-1}$, the inverse Dirac mode degeneracy (numerically also the composite $(2/5)/\Omega^4(0)$ at $(n,l) = (4,3)$, Paper~7, an Observation);
  $M_1$ sub-ring (Hopf-base counting).
\end{itemize}

No morphism in $\mathcal{A}$ produces the combination $K = \pi(B + F -
\Delta)$ as an output of one Mellin mechanism, nor as a composition of
two or three Mellin mechanisms with the specific weights $(+1, +1, -1)$
and the external $\pi$ prefactor.  Twelve candidate mechanisms have
been empirically eliminated for single-principle generation of $K$
(Paper 2 §IV.G; Phases 4B--4I, April 2026; Sprint A $\alpha$-SP,
2026-04-18; Sprint K-CC, 2026-05-03).
\end{theorem}

\begin{proof}[Proof sketch]
The non-selection statement follows from a \emph{structural} argument
(Facts~1+2); Fact~3 is independent empirical corroboration that does
not enter the proof but is recorded as a robustness witness.

\textbf{Structural argument.}

\textbf{Fact 1 (Master Mellin engine case-exhaustion,
Theorem~\ref{thm:pi_source_case_exhaustion}).}  Every transcendental
appearing in a GeoVac observable is classified into the $M_1$, $M_2$,
or $M_3$ Mellin sub-ring of
$\mathcal{M}[\mathrm{Tr}(D^k \cdot e^{-tD^2})]$ at $k \in \{0, 1, 2\}$.
The three sub-rings correspond to distinct operator-order $\times$
bundle-type combinations.  Under $\mathcal{A}$, the master Mellin
engine is the unique mechanism producing transcendental output, and a
morphism producing output simultaneously in two distinct sub-rings
would require simultaneously evaluating $\mathcal{M}[\mathrm{Tr}(D^k
\cdot e^{-tD^2})]$ at two distinct values of $k$, which is excluded.

\textbf{Fact 2 (Sub-ring partition of $B$, $F$, $\Delta$, Paper 2 §IV
+ Phases 4B--4I).}  $B = 42$ is a finite Casimir trace in the
representation sector ($M_2$); $F = \pi^2/6$ is a Fock-degeneracy
Dirichlet sum at the packing exponent $d = 3$ ($M_2$); $\Delta = 1/40$
is the Dirac mode-degeneracy count $g_3^{\rm Dirac}$ ($M_1$).  The
combination $B + F - \Delta$ therefore mixes outputs of $M_2$ ($k=2$)
and $M_1$ ($k=0$).

\textbf{Composition.}  Under $\mathcal{A}$, by Fact~1 no single
morphism produces output in both $M_1$ and $M_2$ simultaneously.  By
Fact~2 the combination $K = \pi(B + F - \Delta)$ mixes $M_1$ and $M_2$
output.  Therefore no morphism in $\mathcal{A}$ generates $K$ as a
single-mechanism output.  The non-selection conclusion follows
structurally; the specific weights $(+1, +1, -1)$ and the external
$\pi$ prefactor enter the empirical observation but not the
non-selection argument.

\textbf{Empirical corroboration (Fact 3, not load-bearing).}  Twelve
candidate single-principle generators of $K$ have been independently
tested and eliminated:\ Phases 4B--4I (April 2026, nine mechanisms);
Sprint~A $\alpha$-SP (2026-04-18, Connes--Chamseddine direct
spectral-action derivation); Sprint K-CC (2026-05-03, three further
mechanisms via PSLQ at 100~dps against a 244-element basis, a natural
$\Lambda$ search from $S^3$ spectral data, and a unified CC
heat-kernel route blocked by the algebraic obstruction
Theorem~T9 of Paper 28).  These eliminations confirm the structural
result empirically but are not used to establish it. $\hfill\square$
\end{proof}

\begin{remark}[E6 scope and the observation-label discipline]
\label{rem:e6_scope}
The Theorem identifies the structural reason for the empirical
12-mechanism elimination: the three spectral homes live in distinct
Mellin sub-rings with no shared generator in $\mathcal{A}$.  It does
not derive $K$ (which would contradict the empirical eliminations).
It does not claim $K$ is mathematically impossible to generate from
some extended axiom set; sibling axioms producing $K$ from a single
mechanism remain logically possible but none is currently exhibited.

Per the project's standing hard prohibition, the Paper 2 combination
rule is labeled an \emph{Observation} (not a conjecture or derivation),
a status protected at the rule level and preserved by this Theorem:\
the structural impossibility result strengthens the empirical
non-derivability, but does not promote $K$ from observation to
theorem.  Paper 2's status as Observations-folder paper (post
2026-05-02 curve-fit-audit migration) is unchanged.

Catalogue entry E6 (the combination rule for $\alpha^{-1}$) is now
classified as no-single-mechanism calibration data:\ the framework
provides three structural homes for the ingredients but no single
principle for their combination.
\end{remark}

% -------------------------------------------------------------------
% D5/D6 — Cutoff function and CC fine-tuning external-selection
% theorem (Sprint D5/D6 closure, 2026-06-08).
% -------------------------------------------------------------------

\begin{theorem}[Cutoff function $f$ and its Mellin moments are external calibration data]
\label{thm:cutoff_function_external}
The framework's axiom set $\mathcal{A}$ specifies the
Chamseddine--Connes spectral action functional $S(D, \Lambda, f) =
\mathrm{Tr}\, f(D/\Lambda)$ on a real spectral triple.  The cutoff
function $f$ enters as a Schwartz-class test function on $[0, \infty)$
(Paper~51) but is not constrained by any axiom in $\mathcal{A}$.
Consequently the Mellin moments
\[
  \varphi(s) \;=\; \int_0^\infty x^{s-1} f(x) \, dx
\]
are external calibration data:\ no morphism in $\mathcal{A}$ selects
the moment values $\varphi(0), \varphi(1), \varphi(2), \ldots$.

The Wald two-term-exactness $\Rightarrow$ pure Einstein
$\Rightarrow$ action-$G \equiv$ entropy-$G$ argument (Paper~51 \S G7)
forces \emph{relations} among the moments via the sector-wise Mellin
moment map (tip $\leftrightarrow \varphi(0)$, Einstein--Hilbert
$\leftrightarrow \varphi(1)$, $\Lambda_{cc} \leftrightarrow \varphi(2)$);
these relations are theorem-grade within $\mathcal{A}$.  The moment
\emph{values} are not constrained by $\mathcal{A}$.
\end{theorem}

\begin{corollary}[CC fine-tuning is external moment selection]
\label{cor:cc_fine_tuning_external}
The observed cosmological-constant problem in the framework's
sector-wise Mellin moment formulation (Paper~51 \S G4-5) requires
\[
  \Lambda_{cc} \cdot G_{\rm eff} \;\approx\; 36\pi \cdot
  \frac{\varphi(2)}{\varphi(1)^2}
  \;\approx\; 10^{-122}
\]
(dimensionless), which is consistent with the framework's structural
relations between $G_N$, $\Lambda_{cc}$, and $S_{\rm BH}$
(Wald, theorem-grade) provided $\varphi(2)/\varphi(1)^2 \approx
10^{-124}$ with $\varphi(0), \varphi(1) = O(1)$.  No natural cutoff
function $f$ produces this fine-tuned moment ratio, and no axiom in
$\mathcal{A}$ constrains the moment values to it.

Catalogue entries D5 (cutoff function moments $\varphi(0), \varphi(1),
\varphi(2)$) and D6 (cosmological constant value $\Lambda_{cc}$) are
external calibration data inheriting from the cutoff function
selection.
\end{corollary}

\begin{remark}[D5/D6 scope and the sibling-axiom direction]
\label{rem:d5_d6_scope}
Theorem~\ref{thm:cutoff_function_external} formalises the precise
location of the cosmological-constant problem in the GeoVac framework:\
the gravitational \emph{relations} are forced (Wald, Paper~51 \S G7);
the cutoff-function \emph{moments} are external.  The CC problem in
this framing is a moment-selection problem on the cutoff function
$f$, structurally similar to the Higgs--Yukawa selection problem
(\emph{Theorem}~H1) but on a different calibration object.

A sibling axiom that constrains the cutoff function or its moments
(e.g., a renormalisation-group flow axiom, a regulator-independence
principle, an asymptotic-safety constraint) would extend $\mathcal{A}$
and could in principle select the moment values.  None is currently
exhibited;  the long-range structural-research direction is named
here as the natural analog of the inner-factor sibling-axiom
directions.

Paper~57~\S 6.3 records this theorem as the gravity sector closure
of the C-arc.
\end{remark}

% -------------------------------------------------------------------
% Chemistry-side η-trivialization analog — open structural-research
% target (Sprint Chemistry-Analog closure, 2026-06-08).
% -------------------------------------------------------------------

\begin{remark}[Chemistry-side $\eta$-trivialization analog: open structural-research target]
\label{rem:chemistry_eta_analog}
The SM-side $\eta$-trivialization theorem (Paper~18 \texttt{thm:eta\_trivialization})
forces the inner-factor period ring to $M_1 \cup M_2$ on any
Krajewski-class even finite spectral triple satisfying the chirality
grading axiom $\{\gamma_F, D_F\} = 0$:\ for all odd $k$, $\mathrm{Tr}(D_F^k
\cdot e^{-tD_F^2}) \equiv 0$, so $M_3$ (vertex-parity Hurwitz / Catalan $G$ /
Dirichlet $L$) vanishes on the inner factor.  This is the structural
reason F1 (Yukawa values) and F4--F7 (Higgs VEV, CKM, PMNS, neutrino
masses) live outside the master Mellin engine's structural skeleton:\
the inner-factor period ring is forced to a sub-ring categorically
disjoint from where the framework's outer-factor mechanism could
generate values.

The chemistry-side analog would be a structural property of the
FrozenCore $Z_{\rm eff}(r)$ pipeline that mirrors $\{\gamma_F, D_F\} =
0$ on the chemistry inner-factor side:\ a chirality-style grading on
the Clementi--Raimondi $Z_{\rm eff}(r)$ structure that forces a
similar period-ring restriction on chemistry inner-factor data.  Such
a property is not currently exhibited.  The most concrete candidate
direction would identify a discrete grading on the FrozenCore radial
profile pipeline (e.g., parity of multi-zeta exponents, partition of
core orbital basis into chirality-graded blocks) that produces an
$\eta$-trivialization-style theorem for chemistry inner-factor data.

Sprint-scale path is unclear:\ unlike the SM-side
$\eta$-trivialization, which follows directly from a single axiom
($\{\gamma_F, D_F\} = 0$, established in Krajewski 1998 and
canonically adopted in CCM 2010), the chemistry-side analog would
need to identify the analog grading axiom from scratch.  This is a
substantial NCG-research target.

Catalogue entries H6 (Clementi--Raimondi $Z_{\rm eff}$ exponents) and
H7 (multi-zeta coefficients in second-row valence basis) remain
empirical-only pending this structural-research target's completion.
The empirical-grade content (W1e period-class diagnostic,
\texttt{debug/sprint\_w1e\_period\_class\_memo.md}, 2026-06-04)
establishes that H6, H7 are not in $M_1 \cup M_2 \cup M_3$ at audit
ceiling 100;  the open question is whether they sit in a structurally
distinct chemistry-side ring forced by a hypothetical analog axiom.

Paper~57~\S 6.3 records this remark as the named substantial
structural-research target for the chemistry sector; the C-arc's
sprint-scale run closes at v3.103.0 with this target named but not
attempted.
\end{remark}

% -------------------------------------------------------------------
% Final C-arc closure remark (2026-06-08).
% -------------------------------------------------------------------

\begin{remark}[The C-arc terminal state]
\label{rem:c_arc_terminal_state}
After Sprints C2 + C3 + F3 + E7/E8 + G1/G2/G5 + E6 + D5/D6 +
Chemistry-Analog (all 2026-06-08, v3.97.0--v3.103.0), the corpus
carries \emph{eight} theorem-grade non-selection results across the
following sectors:

\textbf{Inner-factor sector} (closed at canonical-rep level):
\begin{enumerate}\setlength{\itemsep}{1pt}
\item Forced-Count Theorem (\ref{thm:forced_count}).
\item H1 Yukawa non-selection (\S \ref{sec:open}).
\item $N_{\mathrm{gen}}$ non-selection (\ref{thm:n_gen_non_selection}).
\item Inner KO-dimension non-selection (\ref{thm:ko_dim_non_selection}).
\end{enumerate}

\textbf{Multi-focal-composition wall} (fully closed under two
sub-sector theorems):
\begin{enumerate}\setlength{\itemsep}{1pt}\setcounter{enumi}{4}
\item Single-cutoff spectral action (\ref{thm:single_cutoff_spectral_action}).
\item Spatial-composition radial wall (\ref{thm:spatial_composition_radial_wall}).
\end{enumerate}

\textbf{$\alpha$ combination rule sector} (single-mechanism
generation closed):
\begin{enumerate}\setlength{\itemsep}{1pt}\setcounter{enumi}{6}
\item No-single-mechanism $K = \pi(B + F - \Delta)$
  (\ref{thm:no_single_mechanism_K}).
\end{enumerate}

\textbf{Gravity sector} (cutoff function external-selection closed):
\begin{enumerate}\setlength{\itemsep}{1pt}\setcounter{enumi}{7}
\item Cutoff function $f$ and its moments are external
  (\ref{thm:cutoff_function_external}).
\end{enumerate}

The chemistry-side $\eta$-trivialization analog
(Remark~\ref{rem:chemistry_eta_analog}) is the named substantial
structural-research target;  catalogue entries H6, H7 remain
empirical-only pending its completion.

The remaining empirical-only catalogue entries (F4--F7 Higgs VEV /
CKM / PMNS / neutrino mass values, structurally inheriting from F1
and F4;  I-entries foundational calibration) are subsidiary residuals
inheriting from theorems above (Higgs admission B7 + H1 Yukawa
non-selection) or sitting outside the structural-skeleton scope
(Born rule via Gleason).

The full forced/free seam (Paper~57) is therefore in a stable terminal
state for the path-\#1 vein C arc:\ eight theorem-grade non-selection
results across four sectors, one named substantial follow-on (chemistry
$\eta$-trivialization analog), all subsidiary residuals accounted for.
\end{remark}
...
H1 sprint memo
\texttt{debug/h1\_higgs\_inner\_fluctuation\_memo.md}.
Sprint C3 N$_{\mathrm{gen}}$ non-selection theorem memo
\texttt{debug/sprint\_c3\_n\_gen\_non\_selection\_memo.md}
(2026-06-08).  G4b is recorded as an open structural question.
...
\textbf{[OBSERVATION]} The five walls catalogued in the multi-focal-composition audit
...
FCI; project development-frontier log).  Sprint~7b made partial progress for the
...
\textbf{[OBSERVATION]} These three walls are tooling-addressable inside the present
...
sprint at bounded scale.  Their existence does not undermine
...
GeoVac-specific.  The Fathizadeh--Marcolli periods-and-motives analysis of
Robertson--Walker spectral actions~\cite{fathizadeh_marcolli2016}
...
Farsi--Latr\'emoli\`ere--Packer 2024 inductive-sequence work
(Adv.\ Math.\ 437, 109442).  This is the structural content of the G4b cross-manifold
...
SU(2) state-space GH convergence theorem.
\end{itemize}
...
forecast is that closing W1a/b/c is internal-effort plausible as a
substantial program per wall;\ closing W2a or W2b would be a
...
observation at $8.8 \times 10^{-8}$).  The W1 walls remain GeoVac-side
...
a Sprint L2 consistency check (a substantial program, gated on blocker B2; see
Paper~31~\S~8.5).
...
Riemannian results.  Sprint L1 (beyond sprint scale, MEDIUM-HIGH) would
...
propinquity, long-range) substantially if applicable.  The current
...
space~\eqref{eq:l2b_krein_space} following the $i^t$ Krein-triple pattern of van~den~Dungen
Proposition~4.1~\cite{vandungen2016} (whose operator is $i^t$ times the
\emph{pseudo}-Riemannian Dirac of $(M, g)$, with the Riemannian metric
$g_r$ entering only through completeness and the positive-definite
inner product):\ Wick-rotate the Lorentzian metric $g$ on $M = S^3
\times \mathbb{R}$ to its Riemannian counterpart $g_r$, construct the
standard Riemannian Dirac $\not{\!\!D}_{g_r}$ on $(M, g_r)$, and define
$D_L = i \not{\!\!D}_{g_r}$.  The Krein self-adjointness of this $D_L$
is verified directly ($\{\gamma^0, D_{\mathrm{GV}}\} = 0$, $\partial_t$
anti-Hermitian; \texttt{geovac/lorentzian\_dirac.py}), not inherited
from the proposition; the identification of $i\not{\!\!D}_{g_r}$ with van
den Dungen's $i\not{\!\!D}_g$ on the static product is not established
here (citation corrected 2026-09-03).
...
$\{J, \eta\} = 0$, $\{\eta, \chi\} = 0$.  \textbf{[SYMBOLIC + MEASURED]} At the bare Cl$(3, 1)$
...
(vii) order-one $[[D_L, a], J_L b J_L^{-1}] = 0$ & $0$
  & $\le 0.1014$ finite-resolution (sample-of-3) \\
\bottomrule
\end{tabular}
\end{center}

Axioms (i)--(iv) hold bit-exactly at every cell.  Axioms (vi)--(vii)
exhibit the same 5--10.2\% finite-resolution residuals as the
Riemannian-side audit (Table~\ref{tab:axiom_audit}, ``Order zero
$[a, J b J^{-1}] = 0$''); over the full $55$-element multiplier basis
at $n_{\max} = 3$ the maxima are $0.0785$ and $0.2026$ (measured
2026-09-02), i.e.\ the sample-of-3 figures understate the order-one
residual by a factor of two; the residuals are sample-of-3 estimates on
...
\verb|tests/test_connes_axiom_audit_31.py| (87 cases, all pass).
...
% -------------------------------------------------------------------
\subsection{Krein-level four-witness compact-period closure (signature-blind) at finite
...
Verdict at every cell:\ \textsc{compact\_period\_closure} (signature-blind;\ the closure cannot distinguish the $(3,1)$ carrier from the Euclidean one --- see the retraction note below).
...
\paragraph{Theorem (Krein-level four-witness compact-period closure
(signature-blind) at finite cutoff).}\label{thm:krein_four_witness_paper32}
...
$+$ BW $+$ Unruh) closes at every Krein cutoff --- but the closure is
\emph{signature-blind} (withdrawn 2026-09-03, per the 2026-07-04
retirement recorded in the C16 registry):\ the truncated BW boost is
compact (integer spectrum, $e^{2\pi i K} = \mathbb{1}$), the period
closure is the compact KMS $\beta = 2\pi$ circle, and the $(3,1)$ label
is a carrier choice that the closure cannot distinguish from the
Euclidean one (Papers 45/47; WH7).  An earlier version of this
paragraph read the closure as a lift ``to literal identification at
the operator-system level (Lorentzian, finite cutoff)''; that reading
is retired.
...
load-bearing falsifier that ensures Sprint L2-E is the $N_t$-carrier
\emph{extension} of Paper~42~\S~6 (the Tomita BW-$\gamma$
construction)~\cite{paper42}, not a parallel-but-different construction;
it does not make the extension Lorentzian in any metrically visible sense
(withdrawn reading, 2026-09-03).
...
continuum (GH-limit) limit.  No published Lorentzian quantum-metric
convergence construction exists as of May~2026
(Latr\'emoli\`ere 2017/2026 and Hekkelman--McDonald 2024 are strictly
Riemannian), and the framework's own attempt was DESCOPED on
2026-06-09 by the $K^+$ annihilation theorem (Paper~45):\ the truncated
boost is compact, so no Lorentzian metric statement is available at
finite cutoff (updated 2026-09-03; an earlier version named this as the
...
companion document; Paper~43 has since been drafted as
\texttt{paper\_43\_lorentzian\_extension.tex});\ Paper~43 sits alongside Papers~38,
...
\textbf{[PANEL-VERIFIED]} The Sprint L1 falsifier named in
...
The \textsc{strong\_identification} verdict labels a bit-exact $\sigma_{2\pi}=\mathrm{id}$ compact-period closure --- a compact-flow feature of the integer modular spectrum (signature-blind, not a metric strengthening;\ cf.\ the retraction note in \S\ref{sec:l2e_krein_modular_hamiltonian}).

The $n_{\max} = 3$ cutoff is the one at which the cumulative node
count $\dim_H = 4 + 12 + 24 = 40$ numerically coincides with Paper~2's
$g_3^{\mathrm{Dirac}} = \Delta^{-1} = 40$, the single-level degeneracy at
$k = 3$ (different objects:\ $N_{\mathrm{Dirac}}(n_{\max}) = n_{\max}\,
g^{\mathrm{D}}_{n_{\max}}/3$, so they agree only at $n_{\max} = 3$).  The L1 closure at this
...
of Theorem~\ref{thm:gh_convergence} (although it is consistent with it
via the state-space GH-rate cross-check: residual / $\gamma_{n_{\max}}$
ratio is $O(\varepsilon_{\mathrm{machine}})$, several orders below the
rate prediction).  The structural reason is the
...
level, not just at the metric-functional level.  The $\sigma_{2\pi}$ period
closure holds at the operator level, but it is \emph{signature-blind} (a
compact-flow feature of the integer modular spectrum, not a metric
strengthening):\ the Lorentzian boost-orbit reading of that period continues
to run through the published Wick-rotation chain, the closure being unable to
distinguish the $(3,1)$ carrier from the Euclidean one.
...
\emph{Result (UNIFIED\_STRONG --- a bit-exact $\sigma_{2\pi}=\mathrm{id}$ compact-period closure, signature-blind, not a metric strengthening):}\ at every tested $n_{\max} \in \{2, 3,
...
has half-integer spectrum, so its operator-level period lift is
$e^{i 2\pi D_W} = -I$ (the spinor double cover) rather than the $+I$ of
the odd-integer $K_{\alpha}^W$;\ the conjugation flow still closes for
both (the global $-I$ cancels), but only $K_{\alpha}^W$ lifts the period
to the identity operator, so the framework's intrinsic Dirac is not the
canonical local Hamiltonian for the wedge KMS state at $\beta = 2\pi$.  The L1-tighten unified closure is for the
...
new tests at the time (the file now holds 70 tests as of 2026-09-02:\
66 pass, 4 slow-marked and skipped by default; 0 regressions of the
41 L1 tests).
...
% ===================================================================
\section{Marcolli--van~Suijlekom Placement: Instance + GeoVac-Original Extension}
\label{sec:lineage}
% ===================================================================

The construction of Sec.~\ref{sec:construction} is an \emph{instance}
of the gauge networks framework of Marcolli and
van~Suijlekom~\cite{marcolli_vs2014} (with the Perez-Sanchez
correction~\cite{perez_sanchez2025} that the continuum limit is
Yang--Mills without Higgs) at the data level, together with three
GeoVac-original structural extensions:\ the forced-graph claim
(Bertrand's theorem $\times$ Fock projection forces $S^3$), the
master Mellin engine $\mathcal{M}[\mathrm{Tr}(D^k e^{-tD^2})]$ at
$k \in \{0,1,2\}$ classifying the framework's transcendentals, and
the cosmic-Galois layer $U^*$ constructed in Paper~56~\cite{loutey_paper56}.  MvS 2014
does not predict a Tannakian dual;\ the $U^*$ sits on top of the
gauge-network scaffolding as additional structure, not as an MvS
corollary.  The key parallels at the data level:
...
(\cite{krajewski1998,paschke_sitarz1998,cacic2013}), graph spectral
...
the Standard Model spectral action (\cite{chamseddine_connes1997_2008}).
What is not duplicated in the literature is the $\alpha$ observation at
$8.8 \times 10^{-8}$ from a discrete spectral construction; that
remains the original empirical claim of Paper~2.
\end{observation}

This is part of what the working hypothesis WH1 in the project register
rests on (working-hypothesis register, WH1 ``PROVEN'' as of 2026-06-10): the structural
framing is in the published lineage; the empirical claim is not
duplicated.
...
\paragraph{Q1.  Order one beyond rank 1.}  \textbf{[OPEN]}  The order-one condition
...
in~\cite{chamseddine_connes1997_2008}).  Whether GeoVac admits a natural
...
Sprint~5 (project development-frontier log) with negative outcome; that is one negative
...
\paragraph{Q2.  Real structure at finite $n_{\max}$ (resolved).}
\textbf{[PANEL-VERIFIED]}
Proposition~\ref{prop:reality} establishes a real structure on the
continuum spinor bundle that restricts to the truncation.  The axiom
audit of Sec.~\ref{sec:axiom_audit} pins the finite-$n_{\max}$ behaviour
down to machine precision: the restricted $J_{\mathrm{GV}}^{(n_{\max})}$
satisfies $J^2 = -\mathbb{1}$ (KO-dimension~3, \emph{not} $+\mathbb{1}$)
bit-exactly at every $n_{\max}$ examined in the audit
(\texttt{test\_real\_structure}), so the finite-versus-limit question is
closed in favour of finite-$n_{\max}$ exactness.

\paragraph{Q3.  KO-dimension and the four-way coincidence.}  \textbf{[OPEN]}  Working
...
\paragraph{Q4.  Spectral-action selection of $\Lambda$.}  \textbf{[OPEN]}  Paper~28's
...
\textbf{[INTERNAL THEOREM]} This paper has constructed an explicit finite spectral triple
$(\mathcal{A}_{\mathrm{GV}}, \mathcal{H}_{\mathrm{GV}},
...
(Observation~\ref{obs:three_sector}), without changing its status as
an observation.  \textbf{[OBSERVATION]} The Coulomb/HO asymmetry of Paper~31 is the
...
\bibitem{dabrowski_dossena2011}
L.~Dabrowski and G.~Dossena, ``Product of real spectral triples,''
\textit{Int.~J.~Geom.~Methods Mod.~Phys.}\ \textbf{8}, 1833--1848
(2011), arXiv:1011.4456.

\bibitem{chamseddine_connes1997_2008}
...
published version \textit{J.~Number Theory} \textbf{160},
334--384 (2016).
...
\textit{Lett.~Math.~Phys.}\ \textbf{112}, 20 (2022),
arXiv:2111.13865.
...
\bibitem{karamata1949}
J.~Karamata,
\textit{Theory and Applications of Stieltjes Integrals},
SANU Posebna izdanja CLIV, Belgrade, 1949.
...
Packing and the Universal Angular Cross-Section,'' Paper~0 (2026).

\bibitem{paper2}
GeoVac Project, ``The Fine Structure Constant as a Spectral
Coincidence on the Hopf Fibration,'' Paper~2 (2026; observation status, May~2026).

\bibitem{paper7}
GeoVac Project, ``The Dimensionless Vacuum: Recovering the
Schr\"odinger Equation from Scale-Invariant Graph Topology,'' Paper~7 (2026).

\bibitem{paper14}
GeoVac Project, ``Structurally Sparse Qubit Hamiltonians from Spectral
Graph Theory,'' Paper~14 (2026).

\bibitem{paper18}
GeoVac Project, ``Spectral-Geometric Exchange Constants: Projecting
Discrete Spectra onto Continuous Manifolds,'' Paper~18
(2026).

\bibitem{paper22}
GeoVac Project, ``Potential-Independent Angular Sparsity for Qubit
Hamiltonians in Spherical Fermion Systems,''
Paper~22 (2026).

\bibitem{paper23}
GeoVac Project, ``Nuclear Shell Model Hamiltonians on the
Hyperspherical Lattice,'' Paper~23 (2026).
...
\bibitem{paper31}
GeoVac Project, ``The Universal/Coulomb Partition of the GeoVac
Spectral Triple,'' Paper~31 (2026).

\bibitem{paper34}
GeoVac Project, ``GeoVac as a Two-Layer Framework: Projection Taxonomy
and the Empirical Matches Catalog,'' Paper~34 (2026).
...
\bibitem{paper36}
GeoVac Project, ``Bound-State QED on $S^3$: One-Loop Closure of the
Hydrogen Lamb Shift,'' Paper~36 (2026; observation status).
...
spectral triples: four-witness Wick-rotation literal identification at
finite cutoff} (Paper~42 of the GeoVac series, Zenodo, 2026).

\bibitem{paper43}
J.~Loutey, \emph{Lorentzian extension of the four-witness
Wick-rotation theorem on truncated $S^3 \times \mathbb{R}$ spectral
triples at finite cutoff} (Paper~43 of the GeoVac
series, Zenodo, 2026).

\bibitem{paper51}
J.~Loutey, \emph{Gravity from the GeoVac spectral action: continuum
results and the discrete-substrate program} (Paper~51 of the GeoVac series, Zenodo, 2026).

\bibitem{paper54}
J.~Loutey, \emph{Two-body interactions from the tensor-product
spectral triple: Selection rules from the algebra, coupling strengths
from the metric} (Paper~54 of the GeoVac series, Zenodo, 2026).

\bibitem{paper57}
J.~Loutey, \emph{Forced and Free Content in the GeoVac Framework:
An Observations Paper} (Paper~57 of the GeoVac series, Zenodo, 2026).

\bibitem{paper60}
J.~Loutey, \emph{The Generalized-Sturmian Secular Equation Block-Encodes Without a
Metric} (Paper~60 of the GeoVac series, Zenodo, 2026).

\bibitem{latremoliere2021dual}
F.~Latr\'emoli\`ere, ``The Dual Modular Gromov--Hausdorff Propinquity and Completeness,''
...
J.~Loutey,
``State-space Gromov--Hausdorff convergence of spectral truncations of compact semisimple Lie groups with bi-invariant metric,''
GeoVac Paper~40 (2026).
\bibitem{loutey_paper50}
J.~Loutey,
``Boundary CFT$_{3}$-on-$S^3$ observables on the truncated GeoVac spectral triple: partition function, wedge KMS entropy, and Casini--Huerta--Myers modular Hamiltonian,''
GeoVac Paper~50 (2026).
...
J.~Loutey,
``The Tannakian substrate of \textsc{GeoVac},''
GeoVac Paper~56 (2026).
\bibitem{loutey_paper59}
J.~Loutey,
``The Three-Center Electron-Repulsion Integral Is an Elliptic Bessel Moment: Genus One at the Third Center in Momentum-Space Slater Theory,''
GeoVac Paper~59 (2026).
\bibitem{halmos1969}
P.~R.~Halmos,
``Two subspaces,''
\textit{Trans.\ Amer.\ Math.\ Soc.}\ \textbf{144}, 381 (1969).
\bibitem{bottcher_spitkovsky2010}
A.~B\"ottcher and I.~M.~Spitkovsky,
``A gentle guide to the basics of two projections theory,''
\textit{Linear Algebra Appl.}\ \textbf{432}, 1412 (2010).
\end{thebibliography}
