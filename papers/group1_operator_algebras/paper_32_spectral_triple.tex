...
finite-truncation caveats, and which remain open.  The real-structure
question---whether the GeoVac Dirac graph operator carries a real
...
spectral triple that is an \emph{instance} of Marcolli--van Suijlekom 2014~\cite{marcolli_vs2014} (with Perez-Sanchez 2024/2025 correction~\cite{perez_sanchez2024,perez_sanchez2025}) at the data level and a GeoVac-original \emph{extension} at the structural level (forced graph from Bertrand $\times$ Fock projection, master Mellin engine, explicit Hopf algebra $\mathcal{H}_{\mathrm{GV}}$, Tannakian dual $U^*$), with the empirically interesting
...
hypothesis register (\texttt{CLAUDE.md}~\S 1.7, WH1 ``PROVEN'' --- unconditional as of 2026-06-10, Paper~38~\cite{loutey_paper38}).
...
artifacts listed above holds at each $n_{\max}$ audited; the triple is
...
products);\ \textbf{[MEASURED]} it is an explicit Connes-style real-space multi-particle
...
lift the phase.  Breaking time reversal does.  A magnetic (Peierls) phase
...
\label{rem:sprint_A}
\textbf{[MEASURED]} Sprint~A (April~2026, CLAUDE.md~\S 1.7) verified at 80~dps that the
...
$\mathrm{tr}(T^a T^b) = \tfrac{1}{2} \delta^{ab}$.  \textbf{[SYMBOLIC PROOF]} The three rows
...
\emph{forbidden} by the $\eta$-trivialization theorem
(Paper~18~\S~IV.6) on any
...
sub-sprints.  \textbf{[PANEL-VERIFIED]} Stated as a theorem it makes explicit what the
...
the Yukawa flavour structure).  Each constructed-triple
axiom-consistency check passes at
$n_{\max} \in \{2, 3\}$ in module
\texttt{geovac/standard\_model\_triple.py} (45 tests).  \emph{QA
...
\item \textbf{Door 4 seam theorem (Yukawa-value wall):}\ the
  ...
  \textbf{[PANEL-VERIFIED]} Yukawa-PSLQ (2026-06-03) added empirical confirmation:\ a
...
\item \textbf{Forced data:}  the moduli space $\mathcal{M}(D_F)$ and
  its dimension (matter-sector count:\ $128$ per generation;
  full-axiom:\ $260$ --- see Theorem~\ref{thm:forced_count}).
...
\item Paper~7's chordal-distance identity maps continuum $-Z/r$ on
  $\mathbb{R}^3$ to the unit-$S^3$ graph Laplacian via a focal-length-
...
Corollary~\ref{cor:spatial_composition_wall} (G1, G2, G5) close all
six catalogue instances of the multi-focal-composition wall family
...
The seventh wall instance (G6) is the H1 Yukawa non-selection, closed
by Theorem~\ref{thm:n_gen_non_selection}'s companion result
(\S~\ref{sec:open}).  All six unique multi-focal-composition wall
instances (G6 $=$ F1 is not independent) are now characterised at
theorem-grade level by one of the three theorems.
...
\item $\Delta = 1/40 = c^2(3,2)$ — Dirac mode degeneracy $g_3^{\rm Dirac}$;
...
Per CLAUDE.md \S 13.5, the Paper 2 combination
...
$\{J, \eta\} = 0$, $\{\eta, \chi\} = 0$.  \textbf{[SYMBOLIC PROOF]} At the bare Cl$(3, 1)$
...
rests on (CLAUDE.md~\S 1.7, WH1 ``PROVEN'' as of 2026-06-10): the structural
...
J.~Loutey,
``State-space Gromov--Hausdorff convergence of spectral truncations of compact Lie groups with bi-invariant metric,''
