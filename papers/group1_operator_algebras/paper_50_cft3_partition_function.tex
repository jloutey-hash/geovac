...
GH-convergence rate inherited from~\cite{paper38} (and the
signature-agnostic product-carrier rate of~\cite{paper45}). The
...
The F-theorem values themselves are due
to~\cite{klebanov_pufu_safdi2011} and are not new; the present paper's
contributions are (i) their bit-exact reproduction from the discrete
truncated spectral triple, (ii) the orthogonal M2/M3 dual-basis
decomposition (Theorem~\ref{thm:mellin_decomp}), and (iii) the
finite-cutoff identification of the framework's modular Hamiltonian
with the CHM hemisphere operator at GH-convergence rate inherited
from~\cite{paper38} (signature-agnostic product-carrier rate
in~\cite{paper45}).
The bulk-side identifications
...
$\sigma_{2\pi}(\Op) = \Op$ closes bit-exactly. The Lorentzian
\emph{propinquity} extension to signature $(3,1)$ is descoped:\
Paper~45~\cite{paper45} proves a degeneracy theorem (the natural
Lorentzian-propinquity seminorm vanishes identically, closing that
route) and the strong-form construction~\cite{paper46} is
correspondingly descoped on the natural substrate, so a Lorentzian
quantum-metric convergence remains an open target. What survives is
the norm-resolvent non-compact-carrier closure~\cite{paper47} at the
spectral level and Paper~45's signature-agnostic product-carrier rate.
The Krein--Mondino--S\"amann bridge~\cite{paper48} and the
...
$g_{n}^{\mathrm{Dirac}} = 2(n+1)(n+2)$ for the single (3d) Dirac
fermion (the standard $\sthree$ massless Dirac, whose $F_{D}$ is
reproduced below; in odd dimension there is no Weyl projection),
$4(n+1)(n+2)$ for the doubled four-component Dirac. Both carry the
same Hurwitz-zeta structure under
...
$\Op_{\nmax} \to \Op_{\sthree}$ in van~Suijlekom's state-space
Gromov--Hausdorff distance holds at rate $\gamma_{\nmax} \sim (4/\pi) \log(\nmax) /
...
\end{align}
for the conformally coupled scalar and the free massless
Dirac fermion (no Weyl projection in odd dimension), respectively. Both values are expressed in terms of the
...
of~\cite{paper35}, the modular-residual closed form of Sprint MR-B\footnote{Sprint/track designations are anchors into the project's internal chronicle (\texttt{CHANGELOG.md} in the repository).}
...
GeoVac-internal observables to the organization of
independently-published CFT$_{3}$ data on the round $\sthree$ at
finite cutoff --- a bit-exact \emph{reproduction} of the KPS values,
not an independent prediction of CFT physics.
\end{remark}
...
\rhoW]$ (recomputed from the live BW construction, the slope-$2$
scaling, the $\coth 1 - \log(2\sinh 1)$ constant, and the area-law
rejection, in \texttt{tests/test\_paper50\_wedge\_kms.py}). Sprint BH-Phase0
...
to ``the state-side constant admits no integer relation with the
spectral-zeta ring up to coefficient ceiling $10^{8}$'' (a strong
numerical disjointness, not a proof of ring non-membership; the
structural argument that $S$ is a log of an integer, hence ring-free,
is the independent support).
...
bit-exactly at every tested $\nmax$~\cite{paper42}. The odd-integer
spectrum, the multiplicities, and the $2\pi$ closure are verified
from the live BW construction in \texttt{tests/test\_paper50\_wedge\_kms.py}.
...
\infty$ holds in the van~Suijlekom state-space Gromov--Hausdorff
distance at rate
\[
d_{\mathrm{GH}}(\Tcal_{\nmax}, \Tcal_{\sthree}) \le C_{3} \cdot \gamma_{\nmax}
\]
with $C_{3} \to 1$ and $\gamma_{\nmax} \sim (4/\pi) \log(\nmax) /
\nmax$ inherited from Paper~38~\cite{paper38} (the signature-agnostic
product-carrier rate of~\cite{paper45} extends it to the temporal
factor).
\end{itemize}
\end{theorem}
...
allowed by the truncation. Convergence in the state-space
Gromov--Hausdorff distance follows from~\cite{paper38}.
\end{proof}
...
AdS$_{4}$ infrastructure would be a major structural extension,
beyond sprint scale and outside the present paper's scope.
...
in the van~Suijlekom state-space Gromov--Hausdorff distance
per~\cite{paper38} (with the signature-agnostic product-carrier rate
of~\cite{paper45} on the temporal factor).
...
partition functions of supersymmetric
CFTs~\cite{hama_hosomichi_lee2011} use deformed half-integer
...
\item \textbf{State-space-GH-rate convergence statement.} Theorem~\ref{thm:chm_identification}
states convergence at the state-space Gromov--Hausdorff level
(inherited from Paper~38). A quantitative
...
named major structural extension that would reach these; it is
beyond sprint scale.
\end{enumerate}
...
Free Dirac (CH) on $\sthree$ & KPS~\cite{klebanov_pufu_safdi2011} & DONE Sec.~\ref{sec:partition} & M2 + M3 \\
Free scalar (conformal) on $S^{5}$ & Beccaria-Tseytlin~\cite{beccaria_tseytlin2017} & DONE Sec.~\ref{sec:s5_extension} & M2 + M3 extended \\
Free Dirac (CH) on $S^{5}$ & Beccaria-Tseytlin~\cite{beccaria_tseytlin2017} & DONE Sec.~\ref{sec:s5_extension} & M2 + M3 extended \\
Free scalar (conformal) on $S^{7}$ & --- (framework-generated) & DONE (in-ring $\mathcal{R}_{7}$, $\geq 200$ dps; Erratum to Rem~\ref{rem:general_d_conjecture}) & M2 + M3 extended \\
\midrule
...
A structural classification shared with the AdS/CFT literature that the framework
obtains directly --- without additional computation --- is the M2-sector
placement of the holographic Weyl anomaly and the
...
\textbf{Structural classification.} Both the Henningson--Skenderis
...
heat-kernel ring $\sqrt{\pi}\cdot\Q \oplus \pi^{2}\cdot\Q$). This is a
shared heat-kernel \emph{classification}, not a computed identity: both
are heat-kernel-trace observables ($\mathcal{M}[\mathrm{Tr}\,e^{-tD^{2}}]$
at operator order $k = 2$), the HS quantity computed on the bulk-gravity
side and the CC quantity on the finite spectral triple. The framework
does not compute the HS holographic anomaly; the content is the
taxonomic placement of both in the M2 ring. The framework's
discrete-cutoff realization (Paper~38~\cite{paper38} state-space GH
convergence at rate $4/\pi$) is the operator-system-level home of the
CC side.
...
placed in the same heat-kernel ring as the framework's spectral action
on $\sthree$ --- a structural classification, not a reproduction of the
...
\textbf{GeoVac} (this paper) & \textbf{Discrete spectral triple on $\sthree$} & \textbf{State-space GH convergence at rate $4/\pi$} \\
\bottomrule
\end{tabular}
\end{center}

The framework's analog of the holographic correspondence is the
Paper~38~\cite{paper38} state-space GH convergence theorem
...
\item the ``correspondence'' replaced by state-space GH convergence in
...
state-space-GH-convergence ``correspondence'' between operator-algebraic
...
state-space GH convergence machinery to all simple compact Lie groups. A natural
...
where the simple ring fails (like the squashed-$\sthree$ deformed-Hurwitz
case, not in the MR-B Jacobi-$\vartheta_{2}$ closed form). Each outcome is paper-worthy
...
(\texttt{docs/retired\_papers.md}). The present paper is the
...
\emph{Paper~22: Potential-Independent Angular Sparsity for Qubit
Hamiltonians in Spherical Fermion Systems},
GeoVac internal preprint, 2026.

\bibitem{paper23} J.~Loutey,
\emph{Paper~23: Nuclear Shell Model Hamiltonians on the Hyperspherical Lattice},
GeoVac internal preprint, 2026.
...
\emph{Paper~35: Time as Projection: The Klein-Gordon Spectrum on
$S^3 \times \mathbb{R}$ and the Algebraic / Observation Split},
GeoVac internal preprint, 2026.

\bibitem{paper38} J.~Loutey,
\emph{Paper~38: State-space Gromov--Hausdorff convergence of truncated Camporesi--Higuchi spectral triples on $S^3$},
GeoVac internal preprint, 2026.

\bibitem{paper28} J.~Loutey,
\emph{Paper~28: QED on $S^3$: Spectral Geometry of Perturbative
Transcendentals},
GeoVac internal preprint, 2026.

\bibitem{paper40} J.~Loutey,
\emph{Paper~40: State-space Gromov--Hausdorff convergence of spectral truncations of compact semisimple Lie groups with bi-invariant metric},
GeoVac internal preprint, 2026.

\bibitem{paper42} J.~Loutey,
\emph{Paper~42: Tomita--Takesaki modular structure on truncated $SU(2)$ spectral triples: four-witness Wick-rotation literal identification at finite cutoff},
GeoVac internal preprint, 2026.

\bibitem{paper43} J.~Loutey,
\emph{Paper~43: Lorentzian extension of the four-witness Wick-rotation theorem on truncated $S^3 \times \mathbb{R}$ spectral triples at finite cutoff},
GeoVac internal preprint, 2026.

\bibitem{paper44} J.~Loutey,
\emph{Paper~44: Operator-system extension of the BBB Krein spectral triple at finite cutoff: propagation number and envelope dependence on the Camporesi--Higuchi spinor bundle},
GeoVac internal preprint, 2026.

\bibitem{paper45} J.~Loutey,
\emph{Paper~45: Lorentzian propinquity on truncated $SU(2) \times U(1)_{T}$ Krein spectral triples: a degeneracy theorem},
GeoVac internal preprint, 2026.

\bibitem{paper46} J.~Loutey,
\emph{Paper~46: Strong-form Lorentzian propinquity on truncated $SU(2) \times U(1)_{T}$ Krein spectral triples},
GeoVac internal preprint, 2026.

\bibitem{paper47} J.~Loutey,
\emph{Paper~47: Norm-resolvent convergence to the non-compact Lorentzian Krein
spectral triple on $S^3\times\mathbb{R}_t$ and the two-rate hybrid limit},
GeoVac internal preprint, 2026.

\bibitem{paper48} J.~Loutey,
\emph{Paper~48: Krein-pointed proper quantum metric spaces and the bridge to
Mondino--S\"amann Lorentzian pre-length spaces, with application to the
GeoVac hemispheric wedge},
GeoVac internal preprint, 2026.

\bibitem{paper49} J.~Loutey,
\emph{Paper~49: Operator-system Lorentzian pre-length spaces and the
...
\bibitem{hama_hosomichi_lee2011} N.~Hama, K.~Hosomichi, and S.~Lee,
\emph{SUSY gauge theories on squashed three-spheres},
JHEP \textbf{05} (2011) 014, arXiv:1102.4716.

\bibitem{beccaria_tseytlin2017} M.~Beccaria, X.~Bekaert, and A.~A.~Tseytlin,
\emph{Partition function of free conformal higher spin theory},
JHEP \textbf{08} (2014) 113, arXiv:1406.3542.

\bibitem{lei_van_leuven2024} Y.~Lei and S.~van Leuven,
...
$T^{2}$ deformation}, JHEP \textbf{03} (2019) 004, arXiv:1807.11401.
