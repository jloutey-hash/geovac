...
(2026-05-22)~\cite{debug:bh_phase0} computed $S(\rhoW)$ at $\nmax
\in \{2, 3, 4, 5, 6, 7\}$, obtaining the log-linear fit
\begin{equation}
S(\rhoW) \approx 1.94 \cdot \log(\nmax) + 0.56, \qquad R^{2} = 0.99991,
\label{eq:S_W_fit}
\end{equation}
with the slope $\approx 2$ identifiable as the wedge boundary
dimension $\dim(\partial W) = \dim(S^{2}_{\mathrm{equator}}) = 2$.
...
$(2n + 4)(n + 1)(n + 2)(n + 3)/6$ for $n = 0, 1, 2, \dots$, and the
...
= -\frac{\log 2}{32} - \frac{\zeta(3)}{32\pi^{2}} + \frac{15\,\zeta(5)}{64\pi^{4}}
\approx -0.02297.
\label{eq:Fs_S5}
\end{equation}
\end{theorem}
...
\begin{equation}
\zeta_{S^{5}}^\text{conf}(s) = \frac{1}{3}\sum_{k \ge 0} g_{k}(s) [\zeta_{R}(2s + 2k - 4) - \zeta_{R}(2s + 2k - 2)]
\end{equation}
with $g_{k}(s) = \binom{s + k - 1}{k}/4^{k}$ follows from the
re-indexed multiplicity structure $v^{2}(v^{2} - 1)/3$ at $v = n + 2$.
At $s = 0$, only $g_{0}(0) = 1$ survives, giving
$\zeta_{S^{5}}^\text{conf}(0) = 0$; the derivative
\begin{equation}
\zeta'_{S^{5}}(0) = \frac{2}{3}[\zeta_{R}'(-4) - \zeta_{R}'(-2)]
+ \frac{1}{3}\sum_{k \ge 1} \frac{\zeta_{R}(2k - 4) - \zeta_{R}(2k - 2)}{k \cdot 4^{k}}
\end{equation}
converges geometrically in $k$. PSLQ at 200~dps and $k_{\max} = 100$
finds the integer relation $[32, -2, -2, 15]$ bit-exactly identifying
\begin{equation}
\zeta'_{S^{5}}(0) = \frac{\log 2}{16} + \frac{\zeta(3)}{16\pi^{2}}
- \frac{15\,\zeta(5)}{32\pi^{4}}.
\end{equation}
...
\begin{align}
\zeta(3)/\pi^{2}\text{ coefficient} = 0 \;&\Longrightarrow\; -4a - 5b = 0 \;\Longrightarrow\; a = -5b/4,\\
\zeta(5)/\pi^{4}\text{ coefficient} = 0 \;&\Longrightarrow\; 60a - 15b = 0 \;\Longrightarrow\; a = b/4,
\end{align}
which are inconsistent unless $b = 0$.
\end{proposition}

\begin{remark}[Conjecture for general odd $d$ --- partially falsified at $S^{7}$]
\label{rem:general_d_conjecture}
The structural-specificity of Theorem~\ref{thm:mellin_decomp} to
$S^{3}$ has a natural conjectural generalization. Round $S^{d}$ for
odd $d \ge 3$ is \emph{conjectured} to have $F$ in a ring of dimension
$(d - 1)/2 + 1$ generated by $\{\log 2, \zeta(3)/\pi^{2},
\zeta(5)/\pi^{4}, \dots, \zeta(d - 2)/\pi^{d - 3}\}$, and the dual-basis
projection on free CFT species would require $(d - 1)/2 + 1$
independent species: scalar plus $(d - 1)/2 - 1$ higher-spin CFTs.

\textbf{Status:} The conjecture is verified bit-exactly at $S^{3}$
(Sec.~\ref{sec:partition}) and $S^{5}$
(Theorems~\ref{thm:scalar_S5},~\ref{thm:dirac_S5}). \textbf{It does
not hold in this simple form at $S^{7}$.} The framework's value
$\zeta'_{S^{7}, \text{conf}}(0) \approx -0.00159466$ (computed to 30+
digits via the same Hurwitz machinery) does \emph{not} admit a PSLQ
integer relation in any of the natural candidate bases tested.
Specifically, no integer relation is found at tolerance $10^{-30}$ to
$10^{-80}$ with $\mathrm{maxcoeff}$ up to $10^{16}$ for the following
fifteen-plus bases:

\begin{itemize}
\item Simple M3-extended ring: $\{\log 2, \zeta(3)/\pi^{2}, \zeta(5)/\pi^{4},
\zeta(7)/\pi^{6}\}$ ($\pm$ constants $\{1\}$, $\{\log \pi\}$, $\{\log 3\}$,
$\{\zeta(9)/\pi^{8}\}$);
\item Unscaled separation: $\{\log 2, \zeta(3), \zeta(5), \zeta(7),
\pi^{2}, \pi^{4}, \pi^{6}\}$;
\item Log-of-primes extension: $\{\log 2, \log 3, \log 5, \log 7,
\zeta(3)/\pi^{2}, \zeta(5)/\pi^{4}, \zeta(7)/\pi^{6}\}$ (testing the
S$^{5}$-minimal-scalar log $3$-log $5$ pollution pattern documented
in this session's v3.2.3 finding);
\item Dirichlet $L$-class additions: $\{\log 2, \log 3, G, \zeta(3)/\pi^{2},
\zeta(5)/\pi^{4}, \zeta(7)/\pi^{6}\}$ (Catalan's $G$);
\item Polygamma at half-integer: $\{\log 2, \zeta(3)/\pi^{2}, \zeta(5)/\pi^{4},
\zeta(7)/\pi^{6}, \psi(1/2), \psi'(1/2)\}$;
\item Weight-8 MZV products: $\{\log 2, \zeta(3)/\pi^{2}, \zeta(5)/\pi^{4},
\zeta(7)/\pi^{6}, \zeta(3)^{2}/\pi^{6}, \zeta(3)\,\zeta(5)/\pi^{6}\}$ and variants;
\item Composite logarithmic combinations: $\{\log 2, \log(15/4),
\zeta(3)/\pi^{2}, \zeta(5)/\pi^{4}, \zeta(7)/\pi^{6}, \zeta(9)/\pi^{8}\}$.
\end{itemize}

\textbf{Computation cross-check.} The same Hurwitz machinery reproduces
the $S^{3}$ and $S^{5}$ bit-exact values to 30+ digits with zero
residual (Hurwitz machinery is correct across the dimensional family).
The $S^{7}$ non-match is therefore \emph{structural, not computational}.
The framework's spectral data on $S^{7}$ converges to a real algebraic
or transcendental number that does not lie in any of the simple
M-engine-extended rings tested. Open question whether the value is
expressible in a richer ring (e.g., weight-9 multiple zeta values, or
specific polynomial combinations of $\log p$ for $p$ prime up to $7$),
or whether the structural identification fails outright. Resolving this
is the natural next step in extending the master Mellin engine
classification beyond $d \le 5$.

Completing the dual basis on $S^{5}$ via a third free CFT species (the
original motivation for the conjecture's extension) remains the
natural next investigation. The $S^{7}$ negative result documents a
clean boundary: the simple $\{\log 2, \zeta(\text{odd})/\pi^{\text{even}}\}$
ring is verified at $S^{3, 5}$ and does not extend naively to $S^{7}$.
\end{remark}
