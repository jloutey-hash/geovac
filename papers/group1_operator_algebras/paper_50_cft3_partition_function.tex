...
\begin{remark}[Conjecture for general odd $d$]
\label{rem:general_d_conjecture}
The structural-specificity of Theorem~\ref{thm:mellin_decomp} to
$S^{3}$ has a natural generalization. Round $S^{d}$ for odd $d \ge 3$
is conjectured to have $F$ in a ring of dimension $(d - 1)/2 + 1$
generated by $\{\log 2, \zeta(3)/\pi^{2}, \zeta(5)/\pi^{4}, \dots,
\zeta(d - 2)/\pi^{d - 3}\}$, and the dual-basis projection on free
CFT species requires $(d - 1)/2 + 1$ independent species: scalar
plus $(d - 1)/2 - 1$ higher-spin CFTs. Completing the dual basis on
$S^{5}$ requires computing $F$ for a third free CFT species --- a
natural candidate is the Maxwell ($p = 1$ form) gauge field with
proper ghost subtraction, or a higher-rank antisymmetric tensor.
\end{remark}
