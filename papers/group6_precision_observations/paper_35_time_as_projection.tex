...
Beyond the verification sprints $\mathrm{KG}\text{-}1$,
$\mathrm{KG}\text{-}2$, $\mathrm{KG}\text{-}3$ and the closed forms
reported in Sections~\ref{sec:casimir} and~\ref{sec:tx_b}
(among them the $S^5$ conformal $-31/60480$ and the Bargmann--Segal
harmonic-oscillator $-17/3840$, both computed here), this paper
...
empirically -- the Hopf bundle for the $\alpha$ Observation, the
...
The verification computations (KG-1, KG-2,
KG-3)\footnote{Sprint/track designations are anchors into the
...
appear in $n \in [1, 50]$ contains 136 distinct integers ranging from
$2$ to $10401$, all in $\mathbb{Q}[\sqrt{d}]$ for some $d$.
(Recounted 2026-08-28; the previously quoted ``134\ldots from $1$''
was wrong in both the count and the lower endpoint --- $1$ is not a
generator in this set.)
...
At $\beta = 1$, $m = 0$, the first seven eigenvalues sorted by
ascending $\omega^2$ are (listing $\omega$): $0$, $\sqrt{3}$, $2\sqrt{2}$, $\sqrt{15}$,
$2\sqrt{6}$, $\sqrt{35}$, $2\pi$.  The first six are in
$\mathbb{Q}[\sqrt{d}]$; the seventh is the first $\pi$-bearing one.
(Corrected 2026-08-28: an earlier version listed five and placed
$2\pi$ fifth, omitting the $n=4,5$ spatial modes $2\sqrt{6} \approx
4.899$ and $\sqrt{35} \approx 5.916$, both below $2\pi \approx 6.283$.
The load-bearing claim --- that $2\pi$ is the \emph{first}
$\pi$-bearing eigenvalue --- is unaffected.)  No earlier step in the construction injects $\pi$.
\end{observation}
...
Layer-2-presence bound of Paper~34 (this is a zero-Layer-2-input,
rational-analytic chain), a single-projection match of this kind
should sit at the framework's basis-quality limit -- here exact -- and
indeed it is.
...
Paper~34's projection dictionary, in the form current when this sprint
ran, did not separately index ``introduce a mass parameter
into the spectrum'' from ``compactify the temporal direction for
finite-time observation.''  In practice these had been bundled with
...
= 11.10 \cdot Q$ rule across 35 composed molecules is integer/rational
at every $Q$.
\item \textbf{Nuclear magic numbers} (Paper~23): $\{2, 8, 20, 28, 50,
82, 126\}$ from HO + spin-orbit graph eigenvalue counting; all integers.
\item \textbf{Hydrogenic atomic eigenvalues via the Fock/$S^3$ projection}
(Paper~7): $\lambda_n = -(n^2 - 1)$ for the Fock-projected Coulomb
operator (a continuum $S^3$ Laplace--Beltrami property, not a bare-graph
one); pure integers at every $n$.
...
$\pi$.  The most natural place to look is the Paper~2 Observation
...
fifteen then-catalogued projections through the spectral-triple lens of
...
The case-exhaustion theorem is proven \emph{by exhaustion over the
Paper~34 projections \S\,III.1--\S\,III.15} (the fifteen then
catalogued), for which ``the converged value contains $\pi$'',
``the chain contains a continuous spectral/temporal integration'', and
``the chain engages one of M1/M2/M3'' are shown equivalent.
\emph{Within that scope}, Prediction~\ref{pred:pi_iff_temporal} is
therefore a structural corollary of the GeoVac spectral triple's three
operating modes (Hopf decomposition of the spinor bundle, heat kernel
of $D^2$, vertex-parity selection on the Camporesi--Higuchi spectrum),
rather than a bare empirical regularity.  For observables outside the
theorem's enumerated scope (the projections added since, and the
general biconditional over \emph{all} GeoVac observables) the reading
remains the \emph{structural Observation} of this paper --- WH7 is
registered, not proven --- with the 208/208 confirmation as its
empirical foundation; the theorem supplies the structural ``why''
where it applies.
...
(Paper~34~§III.15), instantiated at
...
explicit verification is a sprint-scale pendant follow-up flagged in
Track~D \S 5.4.
...
$2\pi$ (this is the principal falsifier, a follow-up beyond sprint
scale); a
Lorentzian-signature spectral triple (a major NCG structural
extension, Track~B
...
\textbf{Metric-level dual (Paper~45~§\,open~Q1, 2026-06).}\ The
temporal-compactification picture here has a metric-level counterpart:\
the truncated Bisognano--Wichmann boost is \emph{compact} (integer
spectrum, $e^{2\pi i K} = I$ at every finite cutoff, the same period-$2\pi$
modular closure), so the finite-cutoff quantum metric on the truncated
Krein triple is Euclidean (forward-triangle), and the Lorentzian
signature is the Wick rotation of the compact $2\pi$-circle --- a
convention, with the genuine reverse-triangle signature confined to the
non-compact continuum limit. Just as compactification is the one step
that injects $\pi$ into the \emph{spectrum} (this paper), it is the one
step that fixes the \emph{signature} as a Wick-rotation convention:\ the
same compactness $\Rightarrow$ discreteness $\Rightarrow$ $\pi/\beta$
reading on both faces.
...
Paper~45~§\,open~Q1 for the metric-level dual above.
...
(\texttt{debug/unruh\_pendant\_memo.md}, May~2026, a sprint-scale
pendant follow-up to Track~D) documents the construction.
...
lifts the structural correspondence on all three.  A follow-up
beyond sprint scale; priority MEDIUM-HIGH.
...
Bytsenko et al.~\cite{bytsenko}~\S~4.5, and
Dowker--Altaie~1978 (Phys.\ Rev.\ D \textbf{17}, 417), the
vacuum-averaged stress-energy tensor for spinor fields in the Einstein
universe, which gives $\langle T_{00}\rangle = 17/(1920\pi^2 a^4)$
(a value that paper attributes to Ford); integrated over
$\mathrm{Vol}(S^3) = 2\pi^2 a^3$ this is $E = 17/(960a)$, matching the
$17/960$ single-Weyl-chirality convention line quoted below --- the
density numeral $1920$ and the energy numeral $960$ differ by the
volume factor and should not be identified.  (Its
finite-temperature companion, Altaie--Dowker, Phys.\ Rev.\ D
\textbf{18}, 3557, was cited here between 2026-08-28 and this correction;
the zero-temperature member of the pair is the right one for a Casimir
energy, and the author order differs between them.)  Verified to 40 dps numerically.
(Attribution corrected 2026-08-28: this sentence previously cited
Kennedy--Critchley--Dowker~1980, which is Ann.\ Phys.\ \textbf{125}, 346
--- finite-temperature \emph{scalar} field theory \emph{with boundaries},
neither spinorial nor on the boundaryless $S^3$.  The section number was
also inconsistent with this paper's own bibliography entry.)
...
\item \textbf{Where does this leave the K combination rule? (Sprint K-CC,
...
distance $1.86\times 10^{-4}$ from $\Lambda_\infty$, $\sim$three orders of
magnitude looser than the K residual itself ($4.8\times 10^{-7}$ for
the plain sum; $8.8\times 10^{-8}$ via Paper~2's self-consistency root
$1/\alpha + \alpha^2 = K$),
...
absolute error vs the LS-1 one-loop ceiling ($-32.78$~MHz); vs LS-3
($-28.85$~MHz) the reduction is $5.1\times$.  Sprint LS-7 (May 2026) made a first pass at
...
$\sim +4.4$~MHz of the residual must be non-loop physics; the itemized
non-loop entries are recoil
$-2.40$~MHz, finite nuclear size $+0.138$~MHz (the 2S value; the
retired $+1.18$ is the 1S shift), hyperfine averaging
$\sim +5.0$~MHz.  These three sum to $\sim +2.74$~MHz; adding the
$\sim +1.20$~MHz multi-loop term gives $\sim +3.94$~MHz for the four
itemized contributions against the $-5.65$~MHz residual --- $\sim
70\%$ closure, not a match to within the literature precision.
(Corrected 2026-08-29: an earlier correction note printed
$+3.78$/$+4.98$ --- sums still computed with the retired $+1.18$~MHz
finite-size entry; the $+0.138$~MHz 2S value gives $+2.74$/$+3.94$,
in agreement with Paper~34's Lamb autopsy.  Cross-check: $5.65 -
3.94 = 1.71$~MHz reproduces Paper~34's tabulated $+1.72$~MHz
Lamb-autopsy residual --- the unattributed remainder IS the autopsy
residual.)
...
spectral-action-type statements in disguise.  Sprint~K-CC has since
\emph{ruled out} the single-projection reading at the framework's
current resolution (no single Chamseddine--Connes heat-kernel
expansion contains $B$, $F$ and $\Delta$ as terms); the live readings
are the revised multi-projection one and the coincidence one, per
WH5.  The open step is therefore to discriminate \emph{those two},
a structural follow-on to Sprint~A (2026-04-18) and the WH1
Marcolli--van~Suijlekom upgrade.  (Updated 2026-08-28: the earlier
wording proposed as a next step a question this paper's own
Section~\ref{sec:open} reports closed.)
