...
(inner-factor input data) and CLAUDE.md~\S~1.7 WH5 (calibration as
external input):\ alongside the Yukawa couplings, the fine-structure
...
candidate:\ \texttt{BH-Phase0-route-A} ($\sim 2$--$4$ days)
...
alternative (8--14 weeks).  See Paper~32~\S~VIII
...
\item \textbf{W1e-deep-correlation-residual:} ${\sim}65$--$90\%$ of
  ...
  multi-week+ architectural investments.
\end{itemize}
...
candidates not yet tested are multi-week+ architectural lifts
...
recognition. Two Day-1 diagnostics tested the two multi-week+ candidate
...
$\sim 50\times$. Remaining candidates are multi-month
architectural commitments (self-consistent $Z_\text{eff}$
...
inner-factor input data; CLAUDE.md~\S~1.7 WH5; the
