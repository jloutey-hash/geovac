...
and at $-211$~ppm; depth-4 chains exist at $+2$~ppm and at
...
Apart from Remarks~\ref{rem:minimal_presentation} and
\ref{rem:ee_partial_split}, which report new measurements with
dedicated backing tests, this paper introduces no new computation.  It is a structural
...
Its spectral data are integers or half-integers: the Casimir label
$n^2 - 1$ on the Coulomb $S^3$ graph and $n + 3/2$ on Dirac $S^3$
(in both cases the \emph{continuum} operator spectra that the graph
encodes as labels --- the bare graph Laplacian itself is a distinct,
positive-semidefinite object), and the total quanta $N$ on the
...
\begin{remark}[The minimal presentation: two primitives, and the
geometry--dynamics split]
\label{rem:minimal_presentation}
Observation~\ref{obs:single_origin} admits a sharper packaging.  The
framework's verified content is generated by \emph{two} primitive
kinds of data: (P1)~the Layer-1 skeleton (Definition~\ref{def:layer1});
and (P2)~the projection family (Definition~\ref{def:layer2}), each
member carrying its scale parameter and its tagged transcendental.
Operators that the framework's presentation treats as separate
objects keep collapsing into these primitives.  Four verified
instances:
(i)~the chemistry Dirac operator equals the one-body Hamiltonian
bit-exactly under the Marcolli--van~Suijlekom gauge-network
assembly --- \emph{measured but not yet promoted to a paper}, so it is
recorded here as supporting rather than as one of the paper-backed
instances;
(ii)~the $N$-electron isoenergetic Sturmian secular equation is a
pure-number matrix plus a single scale $p_\kappa$, with energies as
eigenvalues --- an \emph{atomic (single-centre)} property; two centres
make $T^0$ non-diagonal and $p_\kappa$-dependent (Paper~60);
(iii)~the multi-center configuration operator $F=\sum_i P_i$ is
similar to the union-basis Gram matrix \emph{given intra-centre
orthonormality} --- a load-bearing hypothesis, not a technicality
(Paper~32, \texttt{rem:config\_operator\_is\_metric});
(iv)~the atomic one-electron Hamiltonian in the shared-$k$
Coulomb--Sturmian basis is
$h_1 = k^2\bigl(\mathbb{1} - S/2\bigr) - Zk\,\mathrm{diag}(1/n)$
--- a textbook Sturmian identity (the Sturmian equation plus
potential-weighted orthonormality
$\langle\chi_m|r^{-1}|\chi_n\rangle = (k/n)\,\delta_{mn}$), re-read
here as an information statement and verified against an
independent quadrature route at relative deviation
$4\times10^{-9}$ at the backing test's $N_g = 500$ grid
($6\times10^{-10}$ at $N_g = 800$; the ratio $6.7 \approx 1.6^4$
over $500\to800$ pins the truncation order at $O(N_g^{-4})$)
(\texttt{tests/test\_paper34\_minimal\_presentation.py}).

Instance~(iv) exhibits the split cleanly: \emph{all geometric
content sits in the integer labels and the metric $S$; dynamics
enters only through the single scale} $k = p_0$, which is the Fock
focal length ($p_0^2 = -2E$) --- the energy scale \emph{is} the
projection parameter.  The same split holds in
instances~(i)--(iii).

The boundary of the reduction is equally sharp, and is measured in
the same test file: the two-electron tensor is \emph{not}
metric-generated (best fit by Kronecker words in $S$ leaves a
$39\%$ residual, against $\sim\!10^{-12}$ for a genuinely
metric-generated control), and the multi-center composition algebra
and the Class-1 calibration inputs are established
skeleton-external elsewhere in the corpus.  The derived/independent
boundary for the atomic sector sits exactly at the
electron--electron channel.

\emph{Tier:} instances~(ii)--(iv) are individually proven or
measured \emph{at the citations given}; instance~(i) is measured but
carries no paper citation and is therefore not load-bearing here.  The
claim that the two-primitive set is \emph{minimal} is an Observation.  Its
falsifier: a verified operator within the one-center, one-electron
sector that carries information beyond (labels, metric, scale).
\end{remark}

\begin{remark}[The electron--electron boundary admits an exact partial
split]
\label{rem:ee_partial_split}
The boundary stated in Remark~\ref{rem:minimal_presentation} --- the
two-electron tensor is not metric-generated --- is sharper than a
residual: the tensor splits \emph{exactly}, and the irreducible part
is a single low-rank object.  Each multipole kernel obeys the
elementary min/max identity
\[
  \frac{r_<^{L}}{r_>^{L+1}}
  \;=\;
  \underbrace{\tfrac12\bigl[\varphi(r_1)\psi(r_2)
      + \varphi(r_2)\psi(r_1)\bigr]}_{\text{separable, smooth}}
  \;-\;
  \underbrace{\tfrac12\bigl|\varphi(r_1)\psi(r_2)
      - \varphi(r_2)\psi(r_1)\bigr|}_{W_L \,\ge\, 0}
  \qquad (\varphi = r^{L},\ \psi = r^{-(L+1)}),
\]
so the channel-$L$ tensor is a \emph{rank-two} head
$\tfrac12(A \otimes B + B \otimes A)$, with $A = \langle r^{L}\rangle$
(bandwidth $L+1$, measured) and $B = \langle r^{-(L+1)}\rangle$, minus
a remainder built from $W_L$.  At $L=0$ the head is pure
labels~$\times$~metric ($A = S$, $B = (k/n)\,\delta_{mn}$), and the
full dynamical dependence is one power of the focal length,
$g(k) = k\,g(1)$ with $g(1)$ rational
($\langle 1s^2|g|1s^2\rangle = 5k/8$ exactly); at $L \ge 1$ the head
is still rank-two one-body~$\times$~one-body but
$B$ is dense --- a \emph{product}, hence genuinely two-body, so the
fold-into-$h_1$ below is available only in the $L=0$ channel.  Verified properties of the split
(\texttt{tests/test\_paper34\_ee\_split.py}): (i)~in the $L=0$ channel the separable head
supports \emph{no correlation} --- its full-CI equals the dressed
one-body problem $h_1 + \tfrac12(N{-}1)V$ bit-exactly for $N = 2,3$,
so all correlation lives in $W$ (at $L \ge 1$ the head is a product
with dense $B$ and is genuinely two-body, per the preceding
sentence, so the fold does not apply); (ii)~$W$ carries the kernel's entire
non-smoothness at electron coalescence; (iii)~$W$ is not small
($\langle 1s^2|W\rangle/\langle 1s^2|g\rangle = 3/5$ exactly) but is
\emph{compressible at basis-independent rank}: full-CI is reproduced
to $1$\,mHa at $W$-rank $3$--$4$ and to $0.01$\,mHa at rank $5$--$7$
across $n_s = 3$--$6$ (while $n_s^2$ grows $9$--$36$), the
compressibility improving with $L$ (Frobenius tail after four terms:
$1.6\%,\,0.7\%,\,0.1\%,\,0.03\%$ for $L = 0$--$3$).  The payoff
survives angular coupling: in a Gaunt-coupled $s{+}p$ full-CI with the
$L = 0,1,2$ channels all active, truncating every $W_L$ at rank $3$--$4$
reproduces the full energy to under $1$\,mHa, flat as the radial-pair
space grows from $10$ to $28$.  \emph{Resource consequence
($L=0$; measured and bounded):} in the $L=0$ channel the head is exactly
one-body (its kernel $\tfrac12(r_1^{-1}+r_2^{-1})$ sums over pairs to
$\tfrac12(N-1)\sum_i r_i^{-1}$), so it folds into $h_1$ with no
approximation, lowering the Jordan--Wigner LCU $1$-norm by
$15$--$17\%$ at bit-identical energy, and beating a plain density-fitting
control (eigen-truncation of $g$ itself at matched rank) by $13$--$17\%$ at
matched accuracy.  The advantage is a \emph{constant factor that degrades
with basis size} --- $\lambda_{\text{split}}/\lambda_{\text{full}} = 0.83,
0.85, 0.89, 0.91, 0.94$ at $n_s = 4, 6, 8, 10, 12$, monotone toward unity ---
so it changes no exponent and is not claimed as a scaling result.

\emph{Lineage:} numerical low-rank structure of electron-repulsion
integrals is classical --- it is the basis of the Cholesky-decomposition
approach of Beebe--Linderberg~\cite{beebe_linderberg1977} and of
density fitting --- and the compressibility per se is not claimed as
new.  What this remark adds is the placement in the minimal
presentation: an \emph{exact} rank-two labels-$\times$-metric head, the
bit-exact no-correlation property of that head, the localization of
the coalescence non-smoothness in $W$, and the measured flatness of
the remainder's rank in basis size.  \emph{Tier:} the split identity
and the rank-two head are exact (machine-verified); the
no-correlation property is exact for fixed $N$ in the $L=0$ channel
(there, and only there, a one-body operator in disguise); the rank-flatness is MEASURED over $n_s \le 6$ one-center $s$-only,
and separately in a Gaunt-coupled $s{+}p$ full-CI with $L = 0,1,2$
active, where rank $3$--$4$ holds to under $1$\,mHa as the radial-pair
space grows from $10$ to $28$.

Two sharpenings
(\texttt{tests/test\_paper34\_ee\_split.py}, second block).
\emph{(a) The $2n{-}1$ rank law.}  The $s$-wave pair densities
$R_m R_n r^2 e^{-2kr}$ are polynomials of degree $\le 2n{-}2$ (on the
$r^2$ floor) times one fixed exponential, a function space of
dimension exactly $2n{-}1$.  Hence the matricization of the
two-electron tensor has exact rank $\le 2n{-}1$ --- \emph{linear} in
$n$, for \emph{every} radial kernel --- with explicit integer
dependencies among the densities
($-\rho_{11} + 2\rho_{12} - 3\rho_{13} + 2\rho_{22} \equiv 0$ at
$n=3$, verified symbolically) and total nullity $(n{-}1)^2$.
Measured: $g$, $W$, and a kinked control kernel all saturate the
ceiling at every $n_s \le 6$, while very smooth kernels may sit below
it; the separable head stays rank $2$.  The flat accuracy-rank
$3$--$7$ of the previous paragraph sits under this exact linear
ceiling.
\emph{(b) The closed-form question, resolved negative.}  At $k=1$
every entry is exactly rational
($\langle 1s^2|W|1s^2\rangle = 3/8$), but the active characteristic
polynomials are \emph{irreducible} over $\mathbb{Q}$ (degree $2n{-}1$;
at $n=2$ the cubic $131072\lambda^3 - 50688\lambda^2 - 25200\lambda -
675$, pinned in the backing test), so the finite eigenpairs are
algebraic numbers of full degree with no low-degree closed form; and
the L\"owdin-compressed $W$ spectrum \emph{grows} with $n_s$ (the
kernel operator is unbounded), so the finite eigenvectors do not
converge to fixed continuum eigenfunctions.  The skeleton-form
characterization that survives is differential: in reciprocal radius
$u = 1/r$ the Coulomb kernel is $\min(u_1, u_2)$ --- the Green's
function of $-d^2/du^2$ with Dirichlet boundary at $u=0$, equivalently
the manifestly positive resolution
$1/r_> = \int_0^\infty \mathbf{1}[r_1{<}R]\,\mathbf{1}[r_2{<}R]\,
dR/R^2$ --- and $W$'s kernel $\tfrac12|u_1 - u_2|$ obeys
$d^2 K/du^2 = \delta(u - u')$, so eigenfunctions of any weighted
compression satisfy the second-order ODE
$\lambda f''(u) = w(u)\, f(u)$ (grid-verified at $10^{-5}$).
\emph{Tier:} the rank law, rationality, integer dependencies, and
irreducibility are exact (machine-verified over $\mathbb{Q}$); the
spectral growth is MEASURED.
\end{remark}
...
baseline $-3.10\%$ at $N \to \infty$ --- the pre-LS-6a convention,
superseded by the LS-6a fix at $-0.534\%$; and per Sprint LS-7 the
residual is not primarily an $\alpha^5$ multi-loop ceiling).  The projection is structurally a
...
vs Fee~1993 $1{,}233{,}607{,}216.4(3.2)$ MHz; the Layer-2 $\alpha^4$
Breit input (the standard coefficient $-(11/48)$; Ps theory per
Karshenboim~2005~\S~9 --- the literal fraction is the framework's own
reduction, verified numerically at $0.57\%$, its textbook source an
open verification item;
complete Ps theory at $m\alpha^6$ from Pachucki--Karshenboim~1998,
PRL~80, 2101) at the $\sim 80$ GHz scale closes the residual to sub-MHz.\\
...
Karplus--Klein~1952, Pachucki--Karshenboim~1998, Czarnecki--Melnikov--Yelkhovsky~1999,
Karshenboim~2005~\S~9) --- but the framework's catalogue
...
infrastructure target.  The $\sim 0.57\%$ match between the
calibration-by-construction Layer-2 input ($-79{,}861.9$ MHz $=$
$-0.2279 \times m_e c^2 \alpha^4/h$) and the canonical
Karshenboim~2005~\S~9 closed-form $-(11/48) = -0.2292$ is consistency
...
contribution from $r_p$ at the $\sim +0.138$ MHz level (corrected
2026-08-28 from $+1.18$, which is the 1S value; Sprint LS-7
...
$r_Z = 1.045(20)$ fm (Eides--Grotch--Shelyuto 2007 atomic compilation) or $r_Z =
...
mismatch between Krauth~2017, Eides--Grotch--Shelyuto~2007~\cite{eides_grotch_shelyuto2007}, and Antognini-Krauth-Pohl~2015,
...
transcendental is introduced at any step \emph{when both densities sit
on the same centre}.

\emph{Two-centre scope (added 2026-08-13).}  The ring-preserving
statement above is a one-centre statement, and it is worth saying
exactly where it stops.  When the two densities sit on
\emph{different} centres the same projection is carried out in prolate
spheroidal coordinates with the Neumann ordered-$\xi_</\xi_>$ split
(\S~\ref{sec:proj_sturmian}; Paper~12, Paper~58), and there the radial
factor is no longer a Slater integral.  A native closed-form
evaluation of all four two-centre classes shows what enters, and it is
a short list: the Level-2 Stieltjes seed $e^a E_1(a)$ (Paper~18
\S~Level~2), the logarithm --- inherited from the Neumann kernel
$Q_0(\xi) = \tfrac12 \ln\!\big[(\xi{+}1)/(\xi{-}1)\big]$, so it is the
\emph{same} embedding object seen from the kernel side rather than the
moment side --- and the Euler--Mascheroni constant $\gamma_E$, which
arrives inseparably with $E_1$ through
$E_1(z) = -\gamma_E - \ln z + \mathrm{Ein}(z)$.  All three sit in the
\textbf{embedding} tier of Paper~18's taxonomy, the same tier as the
$1/r_{12}$ cusp content: they are what the continuum two-centre
geometry costs, not what the graph contains.  The seed set grows with
class difficulty --- none for $(AA|BB)$, $\{E_1, \ln\}$ for the hybrid
classes, $\{E_1, \ln, \gamma_E\}$ for exchange --- but the
\emph{weight} does not: every class closes at transcendence weight
one.

Two negatives carry more structural content than the list itself.
First, \textbf{no $\pi$ survives}: it enters through the spherical
normalisations and cancels identically against the $4\pi$ of the
Coulomb kernel, so the calibration tier is untouched and the
master-Mellin sub-mechanisms $M_1/M_2/M_3$ (\S~\ref{sec:proj_hopf},
\S~\ref{sec:proj_spectral_action}) are simply not in this chain.
Second, \textbf{no weight-two object appears} --- no $\mathrm{Li}_2$,
no $\zeta(2)$ --- even though the ordered simplex integral is formally
a period of exactly the type that produces dilogarithms.  The two
together are what make the $(AA|BB)$ entries \emph{decidable}: the
closed form reduces to $A_0(R) + \sum_j A_j(R) e^{-\lambda_j R}$ with
$A_j$ rational and $\lambda_j$ rational, so Lindemann--Weierstrass
decides vanishing exactly rather than by threshold (Paper~58, decided
census: $195/195$ permitted entries nonzero, zero accidental
zeros).  Had $\pi$ or a
dilogarithm survived, that decision procedure would not exist.  The
chain is Fock conformal (\S~\ref{sec:proj_fock}) $\to$ Sturmian
(\S~\ref{sec:proj_sturmian}) $\to$ the present multipole projection
evaluated across two centres; three-axis tag $(L, \text{dimension-preserving},
\{E_1, \ln, \gamma_E\}$ at weight one$)$.

\emph{Three-centre continuation (added 2026-08-16).}  Carried to a
\emph{third} centre---the two-body three-centre integral $(XY|XZ)$, two
two-centre densities sharing one centre on different axes---the same
Fock~$\to$~Sturmian~$\to$~multipole chain, evaluated in momentum space, leaves
the two densities on Fock spheres of \emph{different} radii, and the two-centre
negatives do not survive.  The radial kernel lives on the elliptic curve
$y^2 = (c_1 k^2 + 1)(c_2 k^2 + 1)$, whose $D=0$ period is a complete elliptic
integral $K$ (modulus $m = 1 - c_{\min}/c_{\max}$ over the Feynman family,
degenerate only at $c_1 = c_2$).  The transcendental axis is thus genus-one
elliptic---an elliptic polylogarithm, a new ring within the embedding
tier---so the embedding tier is genus-graded: genus zero at two centres, genus
one at three, the third centre leaving the polylogarithm/MZV tower rather than
climbing it.  Three-axis tag $(L, \text{dimension-preserving}, \{$elliptic
$K$ / elliptic polylog$\}$ at genus one$)$; the elliptic period is verified
numerically to $31$ digits, the closed form not derived (Paper~18
\S~Level~2, three-centre genus grading).\\
...
exactly $Q$-linear composed Pauli count
($N_{\mathrm{Pauli}} = 27.90 \times Q$ at fixed basis) of
Paper~14: every claim about ERI density,
...
$0.022\%$ raw error (properly variational; $0.004\%$ after a
...
QED coefficients (Eides Tab.~7.3; the $\alpha^4$ Breit coefficient
$-(11/48)$ from Karshenboim 2005~\S~9, with the complete Ps theory at
$m\alpha^6$ from Pachucki--Karshenboim 1998), Yukawa couplings (per CLAUDE.md
...
requires the $\alpha^4$ Breit input (canonical coefficient
$-(11/48)$, Karshenboim~2005~\S~9) as Layer-2~content.
\end{itemize}
\end{observation}
...
$\alpha^2{\cdot}\mathbb{Q}$ & $-1102$ ppm ($\approx -a_e$) \\
...
$(m_\text{red}/m_e)^3 (r_d/r_p)^2 = 7.577$ reproduced to 4 digits) \\
...
D 1S hyperfine $\nu_\text{HFS}(D) = 327.2350$ MHz Bohr--Fermi strict
(sprint precision-catalogue 2026-05-08, $+$ external $g_d$) & spinor
lift (Fermi-contact vertex) $\circ$ Fock $\circ$ rest-mass projection
(universal $m_e/m_p$, $g_d$); $I=1$ multiplicity $3/2$ and the
$1/2$ convention divisor on
$A_\text{hf}\,\hat{I}\cdot\hat{S}$ & $\alpha, g_d, m_e/m_p$ & frequency &
$\alpha^2{\cdot}\mathbb{Q}$ & $-456$ ppm vs Wineland--Ramsey 1972
$327.384352522(2)$ MHz \\
...
external $\alpha^4$ Breit, Karshenboim~2005~\S~9) & Fock $\circ$ rest-mass
...
(consistent with the Layer-2-presence bound) \\
...
$4.8\!\times\!10^{-7}$ (sum; $8.8\!\times\!10^{-8}$ via P2 root) \\
\bottomrule
...
by 13 orders of magnitude.  The Layer-2-presence bound of
...
$0.022\%$ at $\ell_\text{max}=7$ & T & Track DI Sprint 1; cusp-corrected
...
\\
H$_2$O $R_{\text{eq}}$ & composed + lone pairs & $19.4\%$ & B &
...
LS-1 baseline $-3.10\%$ at $N\to\infty$ (pre-LS-6a convention;
superseded by LS-6a at $-0.534\%$).  Sprint LS-4. \\
...
the clean result. The $\sim +32.78$~MHz residual that had been
...
reduction in absolute error vs the LS-1 one-loop ceiling
($-32.78$~MHz); vs LS-3 ($-28.85$~MHz) the reduction is $5.1\times$.
(Baseline label corrected 2026-08-28: the factor $5.8$ is the ratio
to LS-1, not to LS-3.)  \textbf{Sprint LS-7 reframed
...
$+0.138$, hyperfine averaging $\sim +5.0$; these three sum to
$\sim +2.74$, and together with the $\sim +1.20$~MHz multi-loop term
the four itemized contributions total $\sim +3.94$~MHz against the
$-5.65$~MHz residual, i.e.\ $\sim 70\%$ closure rather than the
$\sim 88\%$ implied by the retired $+1.18$~MHz finite-size entry,
which is the 1S value; see \S\ref{sec:autopsy_lamb}).  The
...
$-0.20\%$) are in \S~\ref{sec:catalog}; this small interval has the
...
projection ($m_p$, proton) & $-1102$ ppm ($\approx -a_e$) & A & Bohr-Fermi level;
$g_e = 2$ (no anomalous moment).  Residual is $\approx a_e \approx
...
Paper~28 (curvature coefficients).  Residual $+58$ ppm is
...
Layer-2 input $r_Z = 1.045$~fm & $-7689$ ppm vs Krauth full theory
...
dipole-dipole contribution ($4/12$ of $m_e c^2 \alpha^4/h$; the LO
HFS total is $7/12$).  The
...
2026-05-08. \\
D 1S hyperfine $\nu_\text{HFS}(D) = 327.3153$ MHz (BF $+$ recoil
...
& $-211$ ppm vs Wineland--Ramsey 1972 $327.384352522(2)$ MHz & A
(framework-native) $+$ S (LS-8a wall, multi-loop QED) $+$ S (deuteron
polarizability, QCD-internal NN dynamics)
& Strict Bohr-Fermi at $-456$ ppm verifies $I=1$ cross-register angular
...
Cumulative residual $-211$ ppm (corrected 2026-08-28; see
\S\ref{sec:autopsy_d_hfs}).  The framework chain now sits $\sim 72$~ppm
\emph{below} the Pachucki--Yerokhin~2010 / Karshenboim~2005 total theory
($327.339$~MHz), the expected direction for a chain that omits deuteron
polarizability (total TPE $44.5$~kHz $= +136$~ppm, QCD-internal;
corrected 2026-08-29 from a $+44$~ppm unit slip), multi-loop QED ($\sim$few ppm,
LS-8a wall), recoil NLO and finite-size charge --- but the itemization
does \emph{not} close at the $\pm 25$~ppm level and is an open item.
...
to $\sim 1.45 \times 10^{-14}\%$ of LO shift; cumulative $-210.9$~ppm
chain residual (the previously reported $+285.6$~ppm, and its match to
the PY~2010 budget magnitude, are \textbf{withdrawn} --- they rested on
a baseline that was high by $496.5$~ppm).  Polarizability
...
(basis-quality: Zemach scalar $r_Z(t) = 1.762$~fm \textbf{[input under
review, 2026-08-28: this coincides with the triton \emph{charge} radius
$1.7591$~fm to $0.2\%$; the proton ratio $r_Z/r_\text{charge}=1.243$ puts
the Zemach radius nearer $2.2$~fm.  Ab-initio nuclear calculations give
$r_Z({}^3\text{H}) = 2.27(3)$~fm (GFMC) and $2.30(3)$~fm (VMC)
(King \emph{et al.}, arXiv:2606.11153 (2026), Tab.~2; the earlier
impulse-approximation values of Nevo~Dinur \emph{et al.}, Phys.\ Rev.\ C
\textbf{99}, 034004 (2019) --- the journal-published Zemach moments of
$^{2,3}$H and $^{3,4}$He --- are $2.35(1)$/$2.37(1)$~fm), which
would move
this component from $-66.6$ to $\approx -86$~ppm and the stated residual
from $-3.0$ to $\approx -22$~ppm.  The Carlson~2008 attribution is also wrong: PRA 78,
022517 is a \emph{proton} structure paper]},
convention drift claim (formerly ``$\pm 5$~ppm Carlson vs Sick~2014'',
source unidentified); sub-ppm absolute
LS-8a $+$ W1a $+$ W3 budget) & \textbf{[provisional, $r_Z(t)$ under
review] Cleanest LS-8a multi-loop QED
isolation in the hadronic-nucleus catalogue.}  Tritium polarizability
$\sim +0.7$~ppm is $\sim 300\times$ smaller than deuteron's
$\sim +340$~ppm (re-sourced 2026-08-29) because the 2n+1p binding
($B(t) = 8.482$~MeV) is
...
inside$^{\ddagger}$ the projected Karshenboim 2005 Layer-2 budget
$+12 \pm 5$~ppm (LS-8a $\sim$$+6$ ppm $+$ W1a recoil NLO $\sim$$+3$ ppm
$+$ W3 polarizability $\sim$$+0.7$ ppm $+$ finite-size $\sim$$+1.5$ ppm,
sum $\sim$$+12$ ppm, framework gives $-3$~ppm$^{\ddagger}$ --- provisional:
at the ab-initio $r_Z(t) = 2.27(3)$~fm the residual moves to
$\approx -22$~ppm, \emph{outside} this band).  The claimed $\pm 5$~ppm
$r_Z(t)$ ``value drift'' has no identified source: the Carlson~2008
attribution is wrong (a proton-structure paper) and Sick~2014 covers
${}^{3,4}$He, not ${}^3$H. \\
...
Friar moment leading-order at $4.3\%$.  $-0.92\%$ residual decomposes to within $0.20$~meV into missing K\"all\'en--Sabry two-loop VP ($+1.51$ meV),
...
from Karshenboim's $+1085.84$ MHz SE itemization by approximately the
recoil NLO ($11.71$ vs $11.30$~MHz)).  Naively adding Karshenboim's recoil to the framework total
...
$-1.42\%$) & A & \textbf{Z-scaling test of Paper~36 architecture
(Z=1 $\to$ Z=2)}.  Framework's projection-chain machinery $Z$-portable:
three Z-scaling structural tests pass (Lamb$_\text{SE}$ ratio $13.21$
matches Eides bracket prediction $13.18$; Lamb$_\text{VP}$ ratio $15.71$
against $Z^4 = 16$ contact-density (kernel-mixed, see autopsy); Lamb$_\text{FNS}$ ratio $63.60$
match to $0.12\%$ against $Z^4 \times (r_E({}^4\text{He})/r_p)^2 = 63.52$).  The
$-1.42\%$ framework-native residual is structurally the Z-amplified analog
...
catalogue: ${}^4$He polarizability $+0.06$~MHz / $4.3 \times 10^{-6}$ of the 2S
Lamb shift (tightly bound nucleus suppresses virtual photonuclear
...
\textbf{Cleanest LS-8a isolation in the catalogue}
\textbf{[provisional: rests on $r_Z(t) = 1.762$~fm, under review ---
see \S\ref{sec:autopsy_t_hfs}]}: leptonic nucleus
...
this match).  The Layer-2 $\alpha^4$ Breit input (Karshenboim~2005~\S~9;
complete Ps theory at $m\alpha^6$ from Pachucki--Karshenboim~1998,
...
accurate than absolute energies ($4.4\times$ ratio: $\sim 12\%$ on splitting
...
Layer-2) and Eides--Grotch--Shelyuto~2007~\cite{eides_grotch_shelyuto2007} Tab.~7.3 (used for H~21cm Layer-2), with
...
$|\chi_d^\text{tot}| = 135.0$~kHz ($-40\%$ on magnitude; the $-2/5$
rotational sign carried by the $J{=}1$ column is not resolved against
the experimental sign convention by this OoM walk).  Bare-nuclear-only
...
= 1.965$~fm \textbf{[under review: this is the He-3 \emph{charge}
radius; Sick~2014 gives $r_Z = 2.528(16)$~fm]} at Slater-screened
$Z_\text{eff,2s} = 1.15$; Track-3,
2026-05-18) &
...
\midrule
Li-7 $2{}^2S_{1/2}$ HFS $\nu_\text{HFS}({}^7\text{Li}) = 289.13$ MHz
...
$-64.0\%$ vs Beckmann--Boklen--Elke 1974 $803.504086(34)$~MHz &
...
factor $\sim 2.78\times$ on $A_\text{hf}$ from hydrogenic-$Z_\text{eff}$
underestimating $|\psi_{2s}(0)|^2$ vs Clementi--Roetti 1974 SCF
($\sim 2.8\times$ enhancement; the retired $\sim 8\times$ figure came with
the retired baseline and is excluded by experiment, which caps the
enhancement at $2.78\times$) plus \S\ref{sec:proj_phillips_kleinman} PK
orthogonality and \S\ref{sec:proj_bipolar_drake} bipolar core
polarization (1--2\%).  Effective $Z_\text{eff}$ for contact density
$\approx 1.80$ vs Slater 1.3 (orbital extent screened, contact density
...
\subsubsection{Hydrogen $2S_{1/2}-2P_{1/2}$ Lamb shift autopsy}
...
\textbf{Residual} & $+1.72$ & (was $+0.68$ at the retired FNS value;
the ``within quoting precision'' reading no longer follows) & \\
\bottomrule
\end{tabular}
\caption{Hydrogen $2S_{1/2}-2P_{1/2}$ Lamb shift Roothaan autopsy.
\textbf{Corrected 2026-08-28:} the FNS row previously read $+1.18$~MHz,
which is the \emph{1S} finite-size shift (at $r_p \approx 0.868$~fm); the
$2S$ value is $+0.138$~MHz at $r_p = 0.8409$~fm, an $n^3 = 8$ factor
smaller.  The error was visible inside this paper: the He$^+$ autopsy
(\S\ref{sec:autopsy_he_plus_lamb}) uses a Lamb$_\text{FNS}$ ratio of
$63.599$, which implies an H reference of $8.7913/63.599 = 0.13823$~MHz.
Each component is
...
measurement).  \textbf{Layer-2 net:} $+1.20 + 0.138 - 2.40 \approx
-1.06$ MHz.  \textbf{The Layer-2 inputs do \emph{not} cancel in this
observable.}  Until 2026-08-28 this paragraph reported a net of
$\approx -0.02$~MHz and called it an ``empirical near-cancellation''; that
reading was an artifact of the retired $+1.18$~MHz FNS entry and is
\textbf{withdrawn}.  The framework-native components alone still reproduce
the measurement at $\approx 6 \times 10^{-4}$ precision --- that claim is
independent of the FNS row, which is a Layer-2 entry and does not enter
the framework-native subtotal.
...
Layer-2 input.
\item Component 6 (FNS) is \emph{framework-native at the
operator level} (the \S\ref{sec:proj_charge_density} promotion);
the remaining Layer-2 input is the scalar $r_p$ value, not the
operator structure consuming it.  (This bullet previously called it
``the cleanest beneficiary'' of that promotion; at $+0.138$~MHz rather
than the retired $+1.18$~MHz it is a small contributor, and the
showcase claim is withdrawn.)
\end{itemize}
...
(renormalization, Component 5; two-body kinematics, Component 7; FNS,
Component 6) netting to $-1.06$~MHz --- they do not cancel, per the
withdrawal note above --- and FNS now stated in projection-chain
...
\textbf{Residual} & $+0.026$ & $= +18.4$~ppm; marginally above the Eides Tab.~7.3 itemized $+12$--$+18$~ppm band & \\
\bottomrule
\end{tabular}
...
($+18.4$~ppm above measurement).  \textbf{Layer-2 net:} $-39.495 + 18.37
...
analog.
\item Convention drift on $r_Z$ (Eides $1.045$ vs Karshenboim
$1.054$~fm) shifts the Zemach line by $0.34$~ppm ($\approx 0.5$~kHz)
--- small against the $+18.4$~ppm residual [corrected 2026-08-29: an
earlier version stated $\pm 5$~ppm and ranked this the largest
class-(a) sensitivity; at $0.34$~ppm it is among the smallest].
\end{itemize}

(The itemized entries above account for $\approx 13.5$ of the
$+18.4$~ppm; the remainder is unitemized higher-order content.)
...
ppm residual attributable in large part ($\approx 73\%$ itemized) to
LS-8a (multi-loop QED) $+$ W1a-D (Roothaan recoil) $+$ W3 (nuclear
polarizability), as classified in CLAUDE.md \S~1.7.  The structural-skeleton scope
statement is verified: the framework reproduces the observable to
$\sim 2 \times 10^{-5}$ precision via four named projections, and
the itemized $\approx 73\%$ of the residual budget decomposes into
already-named walls without surfacing any new class~(a) literature convention mismatch
...
profile's $\langle r^2\rangle / \langle r\rangle^2 = 3\pi/8 \approx
1.178$ factor ($4.33/3.68 = 1.1766$, matching to $0.1\%$; an earlier
version quoted the two-dimensional constant $4/\pi \approx 1.27$,
which fails by $8\%$) --- a profile-convention issue between calibrating to
...
= 1.168 \times 6.487 = 7.577
\end{equation}
is reproduced by the framework's bare Eides calculation to 4 digits.
...
\begin{itemize}\setlength\itemsep{0pt}
\item Lamb$_\text{SE}$ ratio = $13.211$ (vs naive $Z^4 = 16$).  The
...
\item Lamb$_\text{VP}$ ratio = $15.71$ from the printed rows
($-426.2096$ full-kernel He$^+$ over $-27.13$ contact-form H; vs naive
$Z^4 = 16$).  The $1.8\%$ deviation mixes the $\beta$-dependent kernel
integral structure with a contact-vs-full-kernel mismatch between the
two rows [corrected 2026-08-29: an earlier version quoted
$15.878$/$0.8\%$, which does not reproduce from the printed rows].

\item Lamb$_\text{FNS}$ ratio = $63.599$ (vs predicted $Z^4 \times
(r_E({}^4\text{He})/r_p)^2 = 16 \times (1.9925)^2 = 63.52$).
Matches to $0.12\%$, confirming the
...
$-200.02$ MHz / $-1.42\%$ below experiment, which is structurally the
...
${}^4$He polarizability contributes $+0.06$ MHz / $4.3 \times 10^{-6}$
of the 2S Lamb shift [corrected 2026-08-29: an earlier version stated
$4\times10^{-9}$, inconsistent with its own $+0.06$~MHz row by three
orders], the smallest W3 contribution in the catalogue (cf.\
$\mu$d Lamb where deuteron polarizability is $+1.69$ meV, the
dominant Layer-2 line).  Mechanism: ${}^4$He is tightly bound (binding
energy $\sim 28.3$~MeV total, $7.1$ per nucleon, vs the deuteron's
$2.2$~MeV total), so virtual
...
contamination at the few-ppm level --- the regime where the
...
the SE bracket's logarithmic Z-dependence reproduced to $0.3\%$
(a matching test, not an exactness claim; the VP ratio is
kernel-mixed, see above).
Second, the $-1.42\%$ native residual is the Z-amplified analog of
...
QED-at-Z=2 test, with nuclear-structure Layer-2 inputs at the
few-ppm level ($4.3\times10^{-6}$ of the 2S Lamb shift).  This is the canonical Z-scaling validation of the Paper~36
...
\textbf{Residual} & $-0.069$ & $= -210.9$~ppm; itemization does not
close (see below) & \\
\bottomrule
\end{tabular}
...
327.3153$~MHz ($99.979\%$ of measurement).  \textbf{Layer-2 net:}
$-210.9$~ppm of $\nu_F$ (the framework chain \emph{under}shoots).
\textbf{Correction, 2026-08-28:} this autopsy previously reported a
$+285.6$~ppm residual from a Bohr--Fermi baseline of $327.397464$~MHz.
That baseline was high by $496.5$~ppm: the $g^\text{atomic} = 2\mu_I/\mu_N$
convention is twice the CODATA $g$-factor
$g_\text{CODATA} = \mu_I/(I\mu_N)$, which is what the $A$-constant
requires; the drivers kept the doubled form and additionally used
$m_e/m_N$ where the Fermi-contact mass factor is $m_e/m_p$ for every
nucleus (the nuclear magneton is defined with the proton mass).  The net
is $2(m_p/m_N)$, and at $I=1$ the deuteron's $m_d/m_p = 1.99900750$
nearly cancels the doubling --- which is why the error left the baseline
looking right to three digits.  H~21cm and T~1S~HFS were unaffected
because their drivers used $g_\text{CODATA}$ and $m_e/m_p$ directly;
the escape is a property of those drivers, not of $I=1/2$ (He-3 is also
$I=1/2$ and \emph{is} affected, by the factor $0.6682$ recorded in
\S\ref{sec:autopsy_he3_hfs}).  \emph{Correction 2026-08-28:} an earlier
version of this note gave the divisor as $2I$ rather than $2$; the two
agree only at $I=1$, the case it was derived from.
Cross-anchoring the three baselines against each other now reproduces T
to $0.02$~ppm.  \textbf{The prior claim that the $+285.6$~ppm residual
matched the $-286$~ppm PY 2010 Layer-2 budget magnitude is withdrawn};
the itemization does not close at the $\pm 25$~ppm level.  What replaces
it is a sharper structural statement.

\medskip\noindent\textbf{Why it does not close: the deuteron's
nuclear-structure correction has the opposite sign from the proton's.}
Dividing each chain by its own Fermi value $\nu_F$ (baseline $\times$
reduced mass) and removing the Schwinger $a_e$ factor isolates the net
nuclear-structure correction each observable requires:
...
T 1S     & $-63.2$~ppm & $-66.6$~ppm$^{\dagger}$ & $+3.3$~ppm$^{\dagger}$ \\
\textbf{D 1S} & $\mathbf{+112.9}$~\textbf{ppm} & $-98.0$~ppm &
$\mathbf{+210.9}$~\textbf{ppm} \\
\bottomrule
\end{tabular}
\\[2pt]
{\footnotesize $^{\dagger}$ computed at the catalogue's current
$r_Z(t) = 1.762$~fm, an input under review (see text).
The Gap column is computed from each autopsy's \emph{component} rows
(baseline, reduced mass, $a_e$, Zemach) and equals minus that chain's
residual.  For H it reads $-16.4$~ppm against the $+18.4$~ppm printed
in \S\ref{sec:autopsy_lamb}'s sibling 21\,cm table: the printed
\emph{final} there ($1420.4318$) is itself $\approx 2$~ppm above what
its own components give ($1420.4289$), a pre-existing inconsistency this
table exposes rather than introduces.  The sign argument does not depend
on it.}
\end{center}

For the tightly bound $I = 1/2$ nuclei the required correction is
\emph{negative} and the leading Zemach term carries the right sign,
capturing $71\%$ of it for hydrogen.  (The tritium row is quoted at the
$r_Z(t)$ value this catalogue currently uses, $1.762$~fm; that input is
under review --- it coincides with the triton \emph{charge} radius
$1.7591$~fm to $0.2\%$, and the proton's $r_Z/r_\text{charge} = 1.243$
would put the Zemach radius nearer $2.2$~fm, moving the applied shift to
$\approx -86$~ppm and the gap to $\approx +23$~ppm.  The sign of the
required correction, which is what the comparison rests on, does not
depend on $r_Z$ at all.)  For the deuteron the required
correction is \emph{positive}: the deuteron is bound by only
$2.2$~MeV and is correspondingly polarizable, so its polarizability
contribution to the hyperfine splitting outweighs the negative Zemach
term and reverses the sign of the total.  A leading-Zemach-only
treatment is therefore not merely incomplete at $I = 1$ --- it points
the wrong way.  The $+210.9$~ppm gap is everything the chain omits ---
non-Zemach nuclear structure (expected to dominate), multi-loop QED,
recoil NLO and finite-size charge --- not nuclear structure alone.

This is a boundary of the \S\ref{sec:proj_magnetization_density}
projection, not a bookkeeping error: the projection consumes a single
scalar $r_Z$, which is sufficient where Zemach dominates (H, T) and
structurally insufficient where polarizability does (D).  Two
consequences follow.  (i)~The $+44$~ppm deuteron-polarizability figure
quoted elsewhere in this catalogue is \textbf{withdrawn as a unit
slip}: the primary literature's total two-photon-exchange (TPE)
nuclear-structure correction for electronic D~1S is $44.5(1.1)$
\emph{kHz} (Bonilla \emph{et al.}, arXiv:2508.18776, Tab.~IV), which on
a $327.384$~MHz splitting is $+135.9$~ppm --- the numeral $44$ was
carried across from the kHz total, and it is a \emph{total}, not a
polarizability.  (ii)~Closing the D
itemization needs a polarizability channel in the projection dictionary,
not a better $r_Z$ --- which is the honest open item, and a sharper one
than the coincidental magnitude match it replaces.

\textbf{What the channel has to deliver (2026-08-29, sourced).}  The
primary decomposition is now available and confirms the mechanism this
subsection infers from the sign contrast.  Bonilla \emph{et al.}\ split
the electronic-D TPE into elastic $-41.50(11)$~kHz, polarizability
$+110.16(38)$~kHz, single-proton $-33.14(7)$~kHz and single-neutron
$+8.95(93)$~kHz, totalling $+44.5(1.1)$~kHz.  In fractions of the
splitting: the inelastic (polarizability) channel is $+336.5$~ppm and
\emph{all} elastic-class pieces together are $-200.6$~ppm, so
polarizability outweighs the elastic terms and flips the sign of the
total to $+135.9$~ppm.  That is precisely the mechanism argued above,
with magnitudes now sourced rather than inferred.

Two quantitative consequences.  First, the framework requires
$+112.9$~ppm of total nuclear structure and the literature supplies
$+135.9$~ppm; the $+23.0$~ppm difference is what this chain omits in
\emph{higher-order QED} (it carries Schwinger $a_e$ and reduced mass
only).  Second --- and this is the check that makes the reading
non-circular --- the H~21cm chain is architecturally identical and,
with its own nuclear-structure input consumed, lands at $+18.4$~ppm
(\S\ref{sec:autopsy_21cm}).  So a D chain consuming the sourced TPE
would sit at $+23.0$~ppm: the same sign, the same tens-of-ppm class,
and the same missing-QED explanation as hydrogen.  The $I=1$ anomaly
is then not an anomaly at all --- it is the absence of a channel,
quantified.  \emph{Tier:} MEASURED (the framework side) against a
cited Layer-2 evaluation; the $+23.0$~ppm attribution to higher-order
QED is an inference from the H parallel, not an itemization.  The Layer-2 inputs are the scalar
$r_Z(D)$ value; the PY 2010 budget itemization
\textbf{[2026-08-28: listed here historically, but the chain does not
consume it --- the only Layer-2 scalar entering is $r_Z(D)$.  This
matters for Prediction~\ref{pred:l2_presence}: with $r_Z(D)$ alone as
the input, the corrected $-210.9$~ppm residual is not bounded by
$\max(\varepsilon_\text{basis}, |L_2|)$ on the ppm-class reading, so this
row is an open case for that Prediction rather than a supporting one.]}
...
residual was itemized per Pachucki--Yerokhin~2010 as follows.
\textbf{[Withdrawn 2026-08-28: these entries sum to $\approx +285$~ppm,
the retired residual.  The corrected residual is $-210.9$~ppm and the
itemization does not close; the list is retained only as the historical
budget it was matched against.  The $+200$~ppm polarizability entry
below has since been re-sourced (2026-08-29): its near-coincidence with
the $+210.9$~ppm requirement was accidental, and the primary value is
$+336.5$~ppm with the \emph{total} TPE at $+135.9$~ppm.]}
\begin{itemize}\setlength\itemsep{0pt}
\item Deuteron polarizability ($\sim +200$~ppm) is the dominant
component, $\sim 30\times$ the proton's contribution.
\textbf{[Re-sourced 2026-08-29: the primary value is
$+110.16(38)$~kHz $= +336.5$~ppm (Bonilla \emph{et al.},
arXiv:2508.18776); the $\sim +200$ figure is neither that nor the
$+135.9$~ppm total.  Dominance and sign stand; the magnitude does not.]}  This is
...
at the same operator-level precision.  \textbf{[2026-08-28: a new
convention defect \emph{was} subsequently found --- the Bohr--Fermi
$g$/mass-slot error identified in \S\ref{sec:autopsy_he3_hfs}. The
sentence below refers only to the itemization axis surveyed here.]}
No new convention mismatch
...
\textbf{Residual} & $-0.005$ & $= -3.0$~ppm$^{\ddagger}$; inside Karshenboim 2005
Layer-2 budget $+12 \pm 5$ ppm & \\
\bottomrule
\end{tabular}
\caption{($^{\ddagger}$ provisional: rests on $r_Z(t) = 1.762$~fm, under review --- see the status note below.)  Tritium 1S hyperfine Roothaan autopsy.  Each component
tagged to its \S\ref{sec:layer2} projection chain. Status: FN $=$
framework-native; L2 $=$ Layer-2 input.  Source memo:
...
with $r_Z(t) = 1.762$~fm (source under review; the Carlson~2008 attribution is wrong --- PRA 78, 022517 is a proton-structure paper).  The bilinear matrix
element $\langle\hat{r}_Z\rangle$ on the Sturmian register at L=0
multipole reduces to $M_1[\rho_M] = r_Z$ to machine precision; the
...
$r_Z(t)$ value ($1.762$~fm; the Carlson~2008 attribution is wrong --- that
paper is proton-structure --- and the true source is unidentified) and the Karshenboim
...
\textbf{[Status 2026-08-28: this subsection's $-3.0$~ppm residual and the
``cleanest isolation'' reading below both rest on $r_Z(t) = 1.762$~fm,
an input flagged as under review in \S\ref{sec:offprecision} --- it
coincides with the triton \emph{charge} radius, and at the ab-initio
Zemach value $2.27(3)$~fm the residual moves to $\approx -22$~ppm,
\emph{outside} the quoted $+12\pm5$~ppm band.  Both claims are
provisional pending that re-sourcing.]}
...
deuteron's $\sim +340$~ppm (re-sourced 2026-08-29) because the tritium
2n+1p binding
...
\item Convention drift (claimed $\pm 5$~ppm; source unidentified ---
neither Carlson~2008, a proton paper, nor Sick~2014, a ${}^{3,4}$He
paper, covers ${}^3$H)
...
$m_N \neq m_p$ (hadronic) & \textbf{T 1S HFS ($-3$~ppm$^{\ddagger}$)} & D 1S HFS ($-211$~ppm) \\
...
1S HFS the cleanest LS-8a multi-loop QED isolation point$^{\ddagger}$ for an
...
claimed $r_Z(t)$ value drift, whose source is unidentified (neither
cited work covers ${}^3$H).

\noindent{\footnotesize $^{\ddagger}$ Provisional: computed at
$r_Z(t) = 1.762$~fm, which coincides with the triton \emph{charge}
radius; at the ab-initio Zemach value $2.27(3)$~fm the residual moves
to $\approx -22$~ppm, outside the quoted band.  See
\S\ref{sec:autopsy_t_hfs}.}
...
$+$ Zemach $r_Z(\text{He-3})=1.965$~fm \textbf{[under review; see
\S\ref{sec:autopsy_he3_hfs}]} via
...
He-3 nucleus, with $r_Z(\text{He-3}) = 1.965$~fm \textbf{[input under
review, 2026-08-28: this coincides with the He-3 \emph{charge} radius
$1.9661$~fm to three significant figures; Sick~2014 (PRC \textbf{90},
064002) reports $r_Z({}^3\text{He}) = 2.528(16)$~fm; quantum Monte Carlo
gives $2.50(3)$~fm (King \emph{et al.}, arXiv:2606.11153, Tab.~2), and a hyperfine determination gives $2.608(24)$~fm,
which differs from Sick's value by $2.8\sigma$ (a documented tension, not
a corroboration).  All three exceed the value used here by a wide margin.  The value
used here is therefore low by $\sim\!21$--$25\%$, and this component and
the residual built on it are provisional]}.
...
\textbf{Convention finding (new \S\ref{sec:convention_exposures}-class candidate).}
...
g-factor convention ($g_d^\text{atomic} = 2\mu_d/I$).  \textbf{Corrected
2026-08-28:} this paragraph previously stated that the combination
``happens to give the correct splitting by a \emph{factor cancellation}
specific to I=1''.  \textbf{The cancellation is not exact.}  The
convention yields $\text{correct} \times 2(m_p/m_N)$, and at $I=1$ that
is $2(m_p/m_d) = 1.0004965$ --- near unity, but high by $496.5$~ppm,
which is the error retracted in \S\ref{sec:autopsy_d_hfs}.  The near-miss
is why it survived: $m_d/m_p = 1.99900750$ is close enough to $2$ that
the D baseline looked right to three digits.

The same one-line formula accounts for every known instance:
$2(m_p/m_d) = 1.000496$ (D, high by $496.5$~ppm),
$2(m_p/m_{{}^3\!\text{He}}) = 0.6682$ (He-3, low by $3/2$ --- the
discrepancy reported below), and
$2(m_p/m_{{}^7\!\text{Li}}) = 0.287204$ (Li-7, low by $3.4818\times$ ---
the error corrected in \S\ref{sec:autopsy_li7_hfs}, where the observed
deficit matches this ratio to six digits).  Its mass-slot half
independently produced the alkali-cliff table's suppression
(\S\ref{sec:conv_alkali_hfs}).  Tritium was unaffected: its driver used
the standard convention.  For I=1/2 nuclei (He-3, H, Mu, T) no
cancellation occurs at all; the standard convention
($g_N = \mu/I$, $m_p$ in mass slot) is required throughout.
...
convention generalizes correctly across all I-values.  This is not
merely a candidate \S\ref{sec:convention_exposures} class-(i) exposure:
it is the \textbf{identified root cause of a defect class} that produced
one retracted baseline (D), one corrected baseline (Li-7) and one
corrected resource table (the alkali cliff sequence).  The cross-anchor
guarding against its recurrence is
\texttt{tests/test\_paper34\_autopsy\_baselines.py}.
...
$+2$~ppm ($\mu$H) to $-211$~ppm (D). Multi-electron HFS (this entry;
...
Table~\ref{tab:offprecision}.  This entry is a \emph{structural} negative
...
$+0.065$~mHa), $E(2^1\!P) = -2.123744$~Ha (vs Drake $-2.123843$,
...
C2: Screened $|\psi_{6s}(0)|^2$ via [Xe] \texttt{FrozenCore}
  $Z_\text{eff}(r)$ (\S\ref{sec:proj_fock} $+$ \S\ref{sec:proj_phillips_kleinman})
...
L: Multi-loop QED $\sim +100$~ppm (\S\ref{sec:proj_breit_retardation})
  & $+0.1$ & & L2 \\
\midrule
\textbf{Framework-native total} & $\mathbf{1219.0}$ & $\mathbf{-47.0\%}$ & \\
...
\textbf{Residual} & $-514.4$ & $= -64.0\%$
($\sim 2.78\times$ cliff factor) & \\
\bottomrule
\end{tabular}
\caption{Lithium-7 $2{}^2S_{1/2}$ hyperfine Roothaan autopsy.
\textbf{Corrected 2026-08-28:} the Bohr--Fermi baseline previously read
$+82.977$~MHz.  That value carried the same Track-5 convention error as
the deuterium autopsy (\S\ref{sec:autopsy_d_hfs}): the $g^\text{atomic} =
2\mu_I/\mu_N$ convention combined with a nuclear-mass magneton gives
$\text{correct} \times 2(m_p/m_N)$, and $82.977/288.913 = 0.287204 =
2(m_p/m_{{}^7\!\text{Li}})$ to six digits --- a pure mass-and-convention
ratio with no $Z_\text{eff}$ content, so screening cannot account for it.
The same formula reproduces deuterium's $+496.5$~ppm excess and the
factor-$3/2$ discrepancy noted in \S\ref{sec:autopsy_he3_hfs}.  Each
component tagged to its \S\ref{sec:layer2} projection chain.  The
framework-native chain at hydrogenic-$Z_\text{eff}$ sits $\sim 2.78\times$
below experiment --- a large cliff, NOT the
...
The framework-native chain sits at $-64.0\%$ residual.  This is
...
$Z_\text{eff}^\text{effective} \approx 1.80$
\end{itemize}

The effective $Z_\text{eff}$ for Li 2s \emph{contact density} sits
near $Z=1.8$, not $Z=1.3$, despite the orbital's spatial extent being
...
$|\psi_{2s}(0)|^2$ exceeds the hydrogenic-$Z_\text{eff}=1.279$ value;
experiment caps the net enhancement at $\sim 2.8\times$ [corrected
2026-08-29: the retired $\sim 8\times$ figure came with the retired
baseline], and \S\ref{sec:proj_bipolar_drake} bipolar-harmonic core
...
$-211$~ppm) hit \emph{nuclear-structure} Layer-2 walls (W3 inner-factor
...
The Z$=3$ cliff manifests as a $-64.0\%$ undershoot on Li HFS because
the 2s valence is the \emph{first} shell beyond the closed core, where
PK orthogonality matters acutely.  \textbf{Corrected 2026-08-28:} this
passage previously read ``$\sim 90\%$'' (the retired baseline) and
concluded ``much larger magnitude at Z$=3$ than at Z$=55$''.  Both are
withdrawn --- see \S\ref{sec:conv_alkali_hfs}.  The $-47\%$ quoted for Cs
is a \texttt{FrozenCore} number and the $-64.0\%$ for Li a bare-hydrogenic
one; compared like for like the cliff \emph{deepens} monotonically with
$Z$ ($-64.0$ to $-98.4\%$).  The shared-mechanism reading stands; the
inverse-$Z$ magnitude reading does not.
...
$\sim 2.8\times$ on $A_\text{hf}$ for Li-7, growing monotonically with
$Z$ ($-64.0\%$ at Li to $-98.4\%$ at Cs) --- not a common factor
across the alkali valence HFS series.  See \S\ref{sec:conv_alkali_hfs}.
...
in the module's convention
$R^k(a,b;c,d) = \int\!\!\int P_{ab}(r_1)\, P_{cd}(r_2)\, K(r_1,r_2)$
--- the first pair of labels is electron~1's density, the second pair
electron~2's.  \textbf{Corrected 2026-08-28:} the middle two rows of
this table previously carried their labels swapped relative to that
convention, so the label $(1s,2s;1s,2s)$ appeared here with the
log-bearing value while \S\ref{sec:proj_breit_retardation} gives it as
$4/81$.  Both \emph{values} were and remain correct; only the label
assignment was inconsistent between the two locations, and the
convention itself was never stated.  Pinned by
\texttt{tests/test\_paper34\_projection\_spot\_checks\_batch1.py}:
...
$\alpha^4$ Breit residual (calibrated against the Pachucki--Karshenboim
1998 complete theoretical value) gives fraction
$-79{,}861.9 / (m_e c^2 \alpha^4 / h) = -0.2279$, vs canonical
Karshenboim 2005 \S~9 closed-form $-(11/48) = -0.2292$; agreement
$0.57\%$.  Match consistent with the calibration-by-construction
...
the $-8$ to $-9$~MHz scale, summing to $-17.8$, while Component 2 is at
the $-80{,}000$ MHz scale --- over three orders of magnitude
separation); (iii) the wall is at
...
\S\ref{sec:proj_3j}) & $+337.61$ & $\mathbf{-135.04}$ \\
\midrule
Experimental & $-$ & $+224.54$ \\
...
in magnitude ($|{-}195|$ vs $224$~kHz; the $-2/5$ rotational sign
carried by the $J{=}1$ column is not resolved against the experimental
sign convention by this OoM walk); the 40\% residual is the absence of molecular
...
the dominant LO contribution to within 13\% of experiment in
magnitude, with the right size and the right angular structure (the
overall EFG sign convention is unresolved; see the table caption) (J=1 rigid-rotor Wigner-3j reduction is exact and
...
et al., \emph{Nature} \textbf{589}, 527; muonic
...
$\sim 0.13$ meV on a 3.68 meV leading --- a $\sim 3.5\%$ fraction at
both $Z$, i.e.\ the higher-Friar fraction is essentially
$Z$-independent between these two systems.
...
closes the remaining $+64$~ppm to NIST.}
\label{tab:autopsy_lyman_alpha}
\begin{tabular}{cllll}
\toprule
\# & Component & Value & Layer & Projection \\
\midrule
1 & Bohr energy gap $E_2 - E_1$ & $3/8\, E_h$ exact & 1 & \S\ref{sec:proj_fock} \\
2 & Radial dipole $\langle R_{2p}|r|R_{1s}\rangle$ & $128\sqrt{6}/243\,a_0$ & 1 & Algebraic Hydrogenic integration \\
3 & Angular factor $\langle Y_{10}|\cos\theta|Y_{00}\rangle$, $\sum_{m_f}$ & $1/\sqrt{3} \times 3$ & 1 & \S\ref{sec:proj_3j} \\
...
gives a residual of $+64$~ppm, attributable to relativistic Dirac
...
\# & Component & Value & Layer & Projection \\
\midrule
1 & Camporesi--Higuchi spinor Dirac on $S^3$, $|\lambda_n^D| = n + 3/2$ & Spectrum exact & 1 & \S\ref{sec:proj_spinor} \\
2 & Fock projection $p_0 = Z/n$, maps to H bound states & Exact & 1 & \S\ref{sec:proj_fock} \\
...
cm$^{-1}$ $= 0.029257\,E_h = 192.51$ THz.  The singlet sits above
...
of $\sim 3$ between best and worst.  The autopsy structure is
...
fail by factors of $1.1$ (with $\kappa$ adjacency) or $3.1$ (with
explicit $r_{12}$ correlation).
...
--- single-exponent bases fail by factors of $3$ or $1.1$ depending
...
(folded into BF) &
\S\ref{sec:proj_restmass} & FN \\
$+$ Zemach $r_Z = 1.045$~fm (Eides--Grotch--Shelyuto 2007) & $\times(1 - 7340\text{~ppm})$ &
\S\ref{sec:proj_fock} $\circ$ \S\ref{sec:proj_spinor} $\circ$
\textbf{\S\ref{sec:proj_magnetization_density}} & FN at op-level $+$ L2 ($r_Z$) \\
\midrule
...
versus $\sim 0.3$~ppm $\approx 0.5$~kHz at electronic H 21cm
[corrected 2026-08-29: an earlier version printed $56$~kHz, the full
Zemach line rather than the drift].  \textbf{The
...
$0.34$~ppm on H 21cm, far below the $+18$~ppm framework residual;
...
$A_\text{exp}$ (MHz) & Cliff & $|\psi(0)|^2_\text{needed}/|\psi(0)|^2_\text{CR67}$ \\
\midrule
...
(\S\ref{sec:proj_phillips_kleinman}).  \textbf{[Diagnosed
2026-08-29: the enhancement is \emph{not} a power law in $Z$ --- a
fitted $\sim Z^{1.2}$ misses Na by $126\%$, because the CR67 estimate
itself is non-monotone in $Z$ (K dips below Na).  The closed form is
given below the table.]}  \textbf{Corrected 2026-08-28:}
the source driver evaluated the Fermi-contact mass factor as $m_e/m_N$;
the nuclear magneton $\mu_N = e\hbar/(2m_p)$ is defined with the
\emph{proton} mass for every nucleus, so each entry was suppressed by
$m_N/m_p$ ($7.0\times$ at Li-7 rising to $131.9\times$ at Cs-133).  The
tell was the three identical $0.3$~MHz entries for K, Rb and Cs.  This is
the mass-slot half of the same convention defect corrected in
\S\ref{sec:autopsy_d_hfs} and \S\ref{sec:autopsy_li7_hfs}; the corrected
Li-7 row now agrees with that autopsy's independently corrected residual,
which it did not before.  Source:
\texttt{debug/muh\_hfs\_and\_na\_cliff.py}.}
\label{tab:alkali_cliff_sequence}
\end{table}

The rightmost column is the quantitative measure of the PK
contact-enhancement cliff: $2.8\times$ at Li (matching the
\S\ref{sec:autopsy_li7_hfs} finding), growing to $63\times$ at Cs.

\textbf{The cliff in closed form (2026-08-29).}  With the corrected
table it becomes possible to ask what the sequence actually measures,
and the answer is not a power law in $Z$.  Two observations fix it.
First, the \emph{needed} contact density is smooth and monotone
($0.233, 0.743, 1.106, 2.335, 3.896$ in units of $a_0^{-3}$) while the
CR67 \emph{estimate} is not (K's $0.050$ falls below Na's $0.186$,
because CR67's $Z_\text{eff} = 2.162$ for the K $4s$ is smaller than
Na's $2.507$).  The jump in the ratio at K is therefore an artifact of
the denominator, not a feature of the physics --- which is why a
power-law fit through the ratio column is ill-founded.
Second, the needed density is the textbook Fermi--Segr\'e value
$|\psi_{ns}(0)|^2 = Z/(\pi a_0^3 \nu^3)$, with $\nu = \sqrt{\mathrm{Ry}/E_I}$
the effective quantum number read off the measured ionization energy
and $\mathrm{d}\nu/\mathrm{d}n$ taken as unity.  Including the standard
relativistic contact enhancement $F_\text{rel} = [\gamma(2\gamma-1)]^{-1}$,
$\gamma = \sqrt{1-(Z\alpha)^2}$, it reproduces the needed density to
$\leq 10\%$ across the whole series.  The cliff is then the ratio of
that value to the framework's hydrogenic estimate:
\begin{equation}\label{eq:alkali_cliff}
  \frac{|\psi(0)|^2_\text{needed}}{|\psi(0)|^2_\text{CR67}}
  \;=\; \underbrace{\frac{Z}{Z_\text{eff}^3}}_{\text{charge}}
  \;\cdot\; \underbrace{\Bigl(\frac{n}{\nu}\Bigr)^{3}}_{\text{quantum defect}}
  \;\cdot\; F_\text{rel}(Z\alpha),
\end{equation}
giving $2.9, 4.4, 22.3, 41.5, 56.9$ against the tabulated
$2.8, 4.0, 22.0, 43.1, 63.0$ --- errors $2, 10, 1, 4, 10\%$, versus
$126\%$ for the retired power law.

\textbf{What drives it.}  Decomposing the growth from Li to Cs: the
charge factor $Z/Z_\text{eff}^3$ spans only $2.7\times$ and is
non-monotone; $F_\text{rel}$ spans $1.3\times$; the quantum-defect
factor $(n/\nu)^3$ spans $17\times$ and is monotone.  The cliff is
therefore dominated by a single structural fact: the alkali valence
electron has $\nu = 1.59, 1.63, 1.77, 1.80, 1.87$ along the series ---
\emph{essentially constant near $1.7$} --- while the framework labels
it with the integer principal quantum number $n = 2,\ldots,6$.  The
quantum defect $\delta = n - \nu$ grows by almost exactly one per added
core $s$ shell ($0.41, 1.37, 2.23, 3.20, 4.13$), which is the
correct form of the ``inner shells'' reading: the shells do not pile
amplitude at the origin directly, they shift $\nu$ --- and it is $n/\nu$,
cubed, that the contact density sees.  The existing Cs
...
core penetration); the $-98.4\%$ here uses the bare
hydrogenic-$Z_\text{eff}$ contact density (which captures \emph{none}).
The gap between $-47\%$ and $-98.4\%$ at Cs quantifies the partial
...
compilations (Eides--Grotch--Shelyuto~2007~\cite{eides_grotch_shelyuto2007}, Karshenboim~2005, Krauth~2017,
...
structurally.  This subsection consolidates the nine exposures
catalogued to date (V.D.1--V.D.4 from the 2026-05-09 multi-track
Roothaan autopsy sprint; V.D.5--V.D.6 from the 2026-05-10
V.D-prediction sprint; V.D.7--V.D.9 subsequently);
...
\begin{table}[h]
\centering\small
\caption{Literature convention exposures catalogued to date (V.D.1--V.D.6
through 2026-05-10; V.D.7--V.D.9 subsequently).
...
Eides--Grotch--Shelyuto~2007~\cite{eides_grotch_shelyuto2007} Tab.~7.3 vs Pachucki--Yerokhin~2010 &
...
$\sim 11$ MHz at $\mu$ Lamb (match within bracket-evaluation precision, $0.4$~MHz, between framework
...
$\langle r^2\rangle/\langle r\rangle^2 = 3\pi/8 \approx 1.178$
$\to$ $+17.7\%$ on Friar moment (vs Eides; $+12.7\%$ vs Antognini) &
...
Hydrogenic $Z_\text{eff}$ misses $|\psi_{ns}(0)|^2$ by $\sim 2.8\times$
for Li 2s (growing to $\sim 63\times$ at Cs); \emph{architecture-class (iii) exposure}, not a
...
$\sim 2.8\times$ on $A_\text{hf}$ at Z=3, growing monotonically with
$Z$ (to $\sim 63\times$ at Z=55) &
...
$\nu_\text{HFS}(D) = 327\,384\,352.522$~Hz (Wineland--Ramsey 1972),
...
\textbf{Source compilations.}  Eides--Grotch--Shelyuto~2007~\cite{eides_grotch_shelyuto2007} Tab.~7.3 (used as the
...
and (c) the cumulative chain residual, corrected on 2026-08-28 to
$-210.9$~ppm.  \textbf{The previously reported match between a
$+285.6$~ppm residual and the $-286$~ppm PY 2010 Layer-2 budget
magnitude is withdrawn}: it rested on a Bohr--Fermi baseline that was
high by $496.5$~ppm (\S\ref{sec:autopsy_d_hfs}).  With the corrected
baseline the residual changes sign and the budget itemization does not
close; whether it closes under either compilation's convention is now
an open item.  No new convention mismatch surfaced at the operator level.  This sharpens the $\sim 0.01\%$ /
...
existing HFS Zemach use case).  For a three-dimensional Gaussian profile, the two
calibrations differ by exactly the factor
$\langle r^2\rangle / \langle r\rangle^2 = 3\pi/8 \approx 1.178$
[corrected 2026-08-29: an earlier version quoted the two-dimensional
constant $4/\pi \approx 1.27$].

\textbf{Magnitude.}  $+17.7\%$ overshoot on the Friar moment vs the
Eides closed form (operator-level $-4.33$~meV vs $-3.68$~meV at fixed
$r_p$; $+12.7\%$ vs the Antognini itemization value $-3.84$~meV).
This is $(3\pi/8)\times$ the closed-form value to $0.1\%$ relative
precision.
...
$+12.7\%$, with the ratio between the two paths equal to the
three-dimensional Gaussian profile factor $3\pi/8$ to $0.13\%$
[corrected 2026-08-29: an earlier version said $4/\pi$ to sub-percent
precision; $4/\pi$ is the two-dimensional constant and fails by $8\%$].
...
\textbf{Observable.}  Deuterium 1S hyperfine splitting
$\nu_\text{HFS}(D) = 327\,384\,352.522$~Hz, with focus on the
\emph{deuteron polarizability} sub-component of the Layer-2 budget
entering the framework's $-210.9$~ppm chain residual
...
Bonilla et al.~2025 chiral-EFT deuteron nuclear-structure update
(arXiv:2508.18776).
...
disagreement reaches operational consequences: Bonilla et al.
...
term in the PY itemization \textbf{[note 2026-08-29: the chiral-EFT
evaluation of Bonilla \emph{et al.}\ gives the polarizability channel
as $+110.16(38)$~kHz $= +336.5$~ppm and the \emph{total} TPE as
$44.5(1.1)$~kHz $= +135.9$~ppm; the spread between these itemizations
is exactly the convention exposure this entry documents, and the
\emph{total} is the convention-independent quantity]}; the convention split (bundled-vs-separated) is
...
addressing the dominant Layer-2 budget item.  \textbf{Closure verdict \textnormal{(corrected 2026-08-29)}:}
the framework's projection chain does \emph{not} consume any
polarizability input --- the only Layer-2 scalar entering the D chain
is $r_Z(D)$ (\S\ref{sec:autopsy_d_hfs}), which is why the residual is
$-210.9$~ppm and why this row is an open case for
Prediction~\ref{pred:l2_presence}.  The convention choice documented
here therefore does not yet propagate into the residual; it becomes
live the moment a polarizability channel is added, and at that point
the itemization spread ($+336.5$~ppm inelastic vs the $+135.9$~ppm
total) is the leading systematic on the input.  Currently anchored to Pachucki--Yerokhin~2010+
...
$|\psi_{2s}(0)|^2$ enhanced by a factor of $\sim 2.8\times$
\textbf{[corrected 2026-08-28: the retired $0.665$ / $\sim 8\times$ pair
came with the suppressed baseline and is excluded by experiment, which
caps the enhancement at $2.78\times$]}.  The mechanism is the
...
\textbf{Magnitude.}  $\sim 2.8\times$ on $A_\text{hf}$ for Li-7
($A_\text{hf}^\text{framework, hydro-} = 144.6$ MHz vs
$A_\text{hf}^\text{exp} = 401.75$ MHz), growing with $Z$ to
$\sim 63\times$ at Cs-133 (Table~\ref{tab:alkali_cliff_sequence}).
\textbf{Corrected 2026-08-28:} this paragraph previously gave
$41.5$~MHz and $\sim 10\times$, and asserted ``the same
factor-$\sim 10\times$ undershoot \emph{for all} alkali valence HFS
observables''.  Those came from the suppressed driver (the value
$41.5 = 144.6/3.4818$), and the uniformity claim is refuted by the
corrected table: the undershoot is \emph{not} a common factor but
grows monotonically with $Z$.  What survives is the structural
prediction that the single-zeta hydrogenic architecture undershoots
every alkali valence HFS, with the deficit increasing along the series.
...
as a $-47\%$ undershoot on $A_\text{hf}$ at $Z=55$ \emph{using the
\texttt{FrozenCore} wavefunction}; the Li cliff manifests as a
$-64.0\%$ undershoot at $Z=3$ on the bare hydrogenic basis.  The two
cliffs are not independent walls but two manifestations of the same
structural limitation.  \textbf{Corrected 2026-08-28:} this passage
previously read that the cliff scales ``with $Z$ inversely to
expectation,'' the $Z=3$ case being more affected than $Z=55$.  That
comparison set a bare-hydrogenic Li against a \texttt{FrozenCore} Cs.
On a like-for-like bare-hydrogenic basis the corrected sequence is
\emph{monotonically increasing} in $Z$ ($-64.0$, $-75.2$, $-95.5$,
$-97.7$, $-98.4\%$ for Li through Cs), i.e.\ in the expected direction:
more inner shells to orthogonalize against, deeper cliff.  The
inverse-scaling reading is \textbf{withdrawn}; the shared-mechanism
reading stands.
...
the W1c Layer~3 finding (Paper~19 (W1c-residual), 2026-05-23
...
Paper~19 (W1c-residual wall + F1-F2-F3 arc
...
extension, sprint-scale). Refines the methodological lesson of
...
Cross-references Paper~19 (W1e refinement, F4--F6)
...
Cross-references Paper~19's W1e Schmidt / core-correlation closeout section
...
The exposures partition into \emph{four} structural classes.
...
\S\ref{sec:conv_d_polarizability_mud_lamb}):
six of the nine exposures
are between Friar--Payne~2005, Eides--Grotch--Shelyuto~2007~\cite{eides_grotch_shelyuto2007}, Krauth~2017, Karshenboim~2005,
...
the alkali valence; the cliff is $\sim 2.8\times$ for Li 2s (growing to $\sim 63\times$
at Cs) and same
...
HF for high-$Z$), not the projection-chain dictionary.  Eq.~\eqref{eq:alkali_cliff}
makes the target precise: what the extended basis has to reproduce is
the \emph{effective} quantum number $\nu \approx 1.7$ of the valence
orbital, not a larger $Z_\text{eff}$ and not more shells.  A
single-zeta hydrogenic function carrying an integer label $n$ and a
screened charge cannot produce $\nu \approx 1.7$ at $n = 6$ by any
choice of $Z_\text{eff}$ --- the deficit is in the radial scale, which
is what makes this an architecture-class exposure rather than a
parameter-fitting one.  \emph{Honest scope:} Eq.~\eqref{eq:alkali_cliff}
is a diagnosis, not a framework prediction --- it consumes $\nu$ from
the measured ionization energy, so it is a Layer-2-fed account of the
cliff's size, and its value is that it identifies \emph{which}
structural quantity the basis is missing.
All four classes vindicate the directive's finding types (class~(iv),
the architectural-absence family, is tagged row-by-row in the table):
...
Eides--Grotch--Shelyuto~2007~\cite{eides_grotch_shelyuto2007}, Karshenboim~2005, Krauth~2017, Pachucki--Yerokhin~2010,
...
[section anchor under re-verification 2026-08-29: the review's TOC
places positronium in \S~9 and \S~6 covers light hydrogen-like HFS;
cf.\ the \S~4$\to$\S~9 correction]
...
6/6 of the \emph{then-catalogued} \S~V.D entries (V.D.1--V.D.6; the
2026-08 catalogue has nine, and the later architecture/wall-class
additions are not spectroscopy observables, so Pattern~C is a statement
about the original six) are precision-spectroscopy class observables
...
match.  It is placed in \S~\ref{sec:catalog} because it accumulates
...
(Lorentzian propinquity, deep original NCG-mathematics
...
multi-sprint), L2 (BBB Krein lift, deep), and L3
(Lorentzian propinquity, deep, possibly skippable; see
Paper~31~\S~8.5).
...
tighten to tens of ppm or sub-ppm --- with the known exception of the
corrected D 1S HFS row at $-210.9$~ppm (\S\ref{sec:autopsy_d_hfs}),
an open case for this class, and one now diagnosed: the D chain is
missing a nuclear-structure channel rather than mis-consuming its
input, and a chain consuming the sourced two-photon-exchange total
would land at $+23.0$~ppm, inside this class --- (H 21cm HF-4 with Zemach $r_Z$ at
...
with it, with one known exception introduced by the 2026-08-28
correction: the D 1S HFS row (\S\ref{sec:autopsy_d_hfs}), whose
corrected $-210.9$~ppm residual against a single $r_Z(D)$ Layer-2
input is not bounded on the ppm-class reading, and is therefore an
open case for this Prediction rather than support for it.
\emph{Status (2026-08-29):} the exception is now diagnosed rather
than merely recorded.  It is a \emph{missing-channel} failure, not a
counterexample to the bound's logic: the D chain omits the
polarizability channel entirely (\S\ref{sec:autopsy_d_hfs}), and a
chain consuming the sourced total two-photon-exchange correction
($44.5(1.1)$~kHz $= +135.9$~ppm; Bonilla \emph{et al.},
arXiv:2508.18776) would sit at $+23.0$~ppm --- the same sign and
class as the architecturally identical H~21cm row at $+18.4$~ppm.
That is a \emph{prediction} of what building the channel should
produce, and it is the sharpest available test of this Prediction:
if the channel is built and the row does not land in the
tens-of-ppm class, the Prediction is falsified on a row that was
chosen for it in advance.  The bound,
the depth-linear falsification below, and the
three exact L2-count~$=0$ anchors (the spatial $S^3$ Casimir
$\tfrac{1}{240} = \tfrac12\zeta_R(-3)$, the Stefan--Boltzmann
$\pi^2/90$, and the H~$1S$ polarizability $\tfrac{9}{2}a_0^3$ via a
Dalgarno--Lewis symbolic solve, each landing at residual~$0$) are
pinned in
\texttt{tests/test\_paper34\_l2\_presence\_bound.py} (the polarizability
by a genuine symbolic solve there; the two zeta anchors as identities
whose from-scratch derivations live in
\texttt{tests/test\_paper35\_kg\_panel.py}).  Tests for the
prediction:
\begin{itemize}\setlength\itemsep{0pt}
...
($-456$~ppm; the $+40$~ppm quoted here until 2026-08-28 was an artifact
of the retired baseline), and the He oscillator strength ($+3.4\%$).  At depth~4
...
Plus one candidate surfaced by the Paper~60 molecular arc (2026-09-12):
...
slot rather than a mechanism.  \emph{Caution if promoted:}\ the reading of $j_0$ as the
...
ceiling $-3.10\%$ (LS-4, pre-LS-6a convention); the LS-6a fix supersedes this with $-0.534\%$.  Under
Prediction~\ref{pred:l2_presence} the $-0.534\%$ is the
...
$1/\alpha$ at $4.8 \times 10^{-7}$ relative residual (the tighter
$8.8 \times 10^{-8}$ figure is the residual of Paper~2's
self-consistency root $1/\alpha + \alpha^2 = K$ against CODATA, not of
the plain sum).  Under
...
prediction is consistent with current data --- with the one known
exception recorded in \S\ref{sec:prediction}, the corrected D 1S HFS
row --- including the 208/208-confirmation panel of Paper~35
Prediction~1 (Sprint TX-B, May~2026), and refinable by future
calculations.

Apart from Remarks~\ref{rem:minimal_presentation} and
\ref{rem:ee_partial_split}, added after certification, this paper
introduces no new computational result.  Its contribution
...
% ===================================================================
\section*{Citation status of the Layer-2 inputs}
\label{sec:citation_status}

The Layer-2 values this catalogue consumes come from the external precision
literature, and until 2026-08-29 most were carried as inline
``Author~Year'' attributions with no bibliography entry.  That layer is
where every external-citation defect found by the 2026-08 review arc
actually lived: a phantom edition (``Eides~2024'' --- no such edition
exists; the work is Eides--Grotch--Shelyuto~2007), a section anchor wrong
at eight loci (positronium is \S~9 of Karshenboim~2005, not \S~4), ab-initio
Zemach radii credited to a paper that does not contain them (they are
King~\emph{et al.}~2026), and a coefficient characterised as a source's
``closed form'' that does not appear in that source's text.

\textbf{Resolved.}  Thirteen works are now in the bibliography with
volume/page/year verified against the primary text:
Eides--Grotch--Shelyuto~2007~\cite{eides_grotch_shelyuto2007},
Karshenboim~2005~\cite{karshenboim2005},
Antognini~2013~\cite{antognini2013},
Friar--Payne~2005~\cite{friar_payne2005},
Clementi--Roetti~1974~\cite{clementi_roetti1974},
Wineland--Ramsey~1972~\cite{wineland_ramsey1972},
Lundeen--Pipkin~1981~\cite{lundeen_pipkin1981},
Sick~2014~\cite{sick2014},
Nevo~Dinur~\emph{et al.}~2019~\cite{nevo_dinur2019},
King~\emph{et al.}~2026~\cite{king2026},
Bonilla~\emph{et al.}~2025~\cite{bonilla2025},
Pachucki--Karshenboim~1998~\cite{pachucki_karshenboim1998} and
Carlson--Nazaryan--Griffioen~2008~\cite{carlson_nazaryan_griffioen2008}.
These carry the experimental anchors and the nuclear-structure evaluations
that the autopsies of \S\ref{sec:autopsies} actually consume.

\textbf{Not yet resolved, and tracked.}  The remaining inline attributions
still lack bibliography entries.  Two deserve naming because they are
load-bearing and ambiguous rather than merely unlisted.  (i)~The
``Pachucki--Yerokhin~2010'' itemization invoked for the deuterium
hyperfine budget: that author pair's 2009/2010 papers are helium
fine-structure calculations, and the modern deuterium-HFS itemization is
Kalinowski--Pachucki--Yerokhin~2018 (and now
Bonilla~\emph{et al.}~2025~\cite{bonilla2025}), so the attribution needs
re-sourcing rather than a bibitem transcribed from it.  (ii)~The
$-(11/48)$ positronium Breit coefficient: the section anchor is now
correct, but the literal fraction does not appear in that section's text
and is the framework's own reduction, verified numerically at $0.57\%$
(\S\ref{sec:offprecision}).

The unresolved set is not left to memory: the deterministic check
\texttt{debug/qa/check\_inline\_attributions.py} enumerates every inline
attribution, marks those with no resolvable \verb|\bibitem|, and holds the
current set as a ratchet baseline --- a \emph{new} unresolved attribution
fails the check by name, while the existing backlog is recorded rather
...
\bibitem{eides_grotch_shelyuto2007}
M.~I. Eides, H. Grotch and V.~A. Shelyuto,
\emph{Theory of Light Hydrogenic Bound States},
Springer Tracts in Modern Physics \textbf{222} (Springer, Berlin, 2007);
earlier review: Phys.\ Rep.\ \textbf{342}, 63 (2001).
[The ``Tab.~7.x'' pointers throughout this paper are to the 2007 volume.
Corrected 2026-08-29: cited here as ``Eides~2024'' until that date; no
2024 edition exists.]
...
Phys.\ Rep.\ \textbf{422}, 1--63 (2005), hep-ph/0509010.
[Positronium is \S~9; the hydrogen Lamb shift is \S~4.]

\bibitem{antognini2013}
A. Antognini \emph{et al.},
``Proton structure from the measurement of $2S$--$2P$ transition
frequencies of muonic hydrogen,''
Science \textbf{339}, 417 (2013).

\bibitem{friar_payne2005}
J.~L. Friar and G.~L. Payne,
``Nuclear corrections to hyperfine structure in light hydrogenic atoms,''
Phys.\ Rev.\ C \textbf{72}, 014002 (2005).

\bibitem{clementi_roetti1974}
...
atoms, $Z \leq 54$,''
At.\ Data Nucl.\ Data Tables \textbf{14}, 177 (1974).

\bibitem{wineland_ramsey1972}
D.~J. Wineland and N.~F. Ramsey,
``Atomic deuterium maser,''
Phys.\ Rev.\ A \textbf{5}, 821 (1972).

\bibitem{lundeen_pipkin1981}
S.~R. Lundeen and F.~M. Pipkin,
``Measurement of the Lamb shift in hydrogen, $n = 2$,''
Phys.\ Rev.\ Lett.\ \textbf{46}, 232 (1981).

\bibitem{sick2014}
I. Sick,
``Zemach moments of $^3$He and $^4$He,''
Phys.\ Rev.\ C \textbf{90}, 064002 (2014).

\bibitem{nevo_dinur2019}
N. Nevo~Dinur \emph{et al.},
``Zemach moments and radii of $^{2,3}$H and $^{3,4}$He,''
Phys.\ Rev.\ C \textbf{99}, 034004 (2019).

\bibitem{king2026}
G.~B. King, S. Bacca, G. Chambers-Wall, A. Gnech, S. Pastore, M. Piarulli
and R.~B. Wiringa,
``Quantum Monte Carlo calculations of Zemach moments in $A \leq 9$ nuclei,''
arXiv:2606.11153 (2026).

\bibitem{bonilla2025}
Bonilla \emph{et al.},
``Improved nuclear-structure corrections to the hyperfine splitting of
electronic and muonic deuterium,''
arXiv:2508.18776 (2025).

\bibitem{pachucki_karshenboim1998}
K. Pachucki and S.~G. Karshenboim,
``Complete results for positronium energy levels at order $m\alpha^6$,''
Phys.\ Rev.\ Lett.\ \textbf{80}, 2101 (1998).

\bibitem{carlson_nazaryan_griffioen2008}
C.~E. Carlson, V. Nazaryan and K. Griffioen,
``Proton structure corrections to electronic and muonic hydrogen hyperfine
splitting,''
Phys.\ Rev.\ A \textbf{78}, 022517 (2008).
[Cited only to record that it is \emph{not} a tritium source; the
attribution it was carrying is withdrawn.]

\bibitem{beebe_linderberg1977}
N.~H.~F. Beebe and J. Linderberg,
``Simplifications in the generation and transformation of two-electron
integrals in molecular calculations,''
Int. J. Quantum Chem. \textbf{12}, 683--705 (1977).
