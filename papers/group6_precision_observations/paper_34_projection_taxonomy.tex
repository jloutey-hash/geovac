...
eigenvalues (Paper~60);
(iii)~the multi-center configuration operator $F=\sum_i P_i$ is
similar to the union-basis Gram matrix (Paper~32,
\texttt{rem:config\_operator\_is\_metric});
...
\emph{Tier:} the four collapse instances are individually
proven or measured at the citations given; the claim that the
two-primitive set is \emph{minimal} is an Observation.  Its
...
is a single small object.  Each multipole kernel obeys the
...
$B$ is dense.  Verified properties of the remainder
(\texttt{tests/test\_paper34\_ee\_split.py}): (i)~the separable head
supports \emph{no correlation} --- its full-CI equals the dressed
one-body problem $h_1 + \tfrac12(N{-}1)V$ bit-exactly for $N = 2,3$,
so all correlation lives in $W$; (ii)~$W$ carries the kernel's entire
...
space grows from $10$ to $28$.  \emph{Resource consequence, measured and
bounded:} because the head is exactly one-body it can be folded into $h_1$
with no approximation, lowering the Jordan--Wigner LCU $1$-norm by
...
0.85, 0.89, 0.92, 0.94$ at $n_s = 4, 6, 8, 10, 12$, monotone toward unity ---
...
no-correlation property is exact for fixed $N$ (one-body operator in
disguise); the rank-flatness is MEASURED over $n_s \le 6$, $s$-only,
one center.

Two exact sharpenings
...
$\sim$4--8 of the previous paragraph sits under this exact linear
ceiling.
...
candidate \S\ref{sec:convention_exposures} class-(i) inter-track itemization
convention exposure.
...
with it.  The bound, the depth-linear falsification below, and the
...
\texttt{tests/test\_paper34\_l2\_presence\_bound.py}.  Tests for the
prediction:
\begin{itemize}\setlength\itemsep{0pt}
