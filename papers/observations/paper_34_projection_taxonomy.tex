...
\subsubsection{Hydrogen 21 cm hyperfine autopsy (placeholder)}
\label{sec:autopsy_21cm}

The 21 cm hyperfine $A_\text{HF} = 1420$ MHz decomposes into
Bohr--Fermi (HF-1) $+$ Schwinger $a_e$ via Parker--Toms (HF-2)
$+$ Zemach ($r_Z$ Layer-2 input via
\S\ref{sec:proj_magnetization_density}, HF-4), with the multi-loop
QED $+$ recoil NLO budget at Eides Tab.~7.3 levels.  Sprint HF
(2026-05-07) decomposed into a four-component analysis producing the
$+18$ ppm Eides-consistent residual.  The autopsy table is
structurally parallel to \S\ref{sec:autopsy_lamb} but at hyperfine
focal lengths and with \S\ref{sec:proj_magnetization_density}
playing the role \S\ref{sec:proj_charge_density} plays in the
Lamb-shift autopsy.  Full table deferred to a follow-on autopsy
memo; source: \texttt{debug/sprint\_hf\_track\{1..5\}.\{py,\_memo.md\}}.

\subsubsection{Muonic hydrogen 2$S$--2$P$ Lamb shift autopsy (placeholder)}
\label{sec:autopsy_muh_lamb}

The $\mu$H 2$S$--2$P$ Lamb shift $202.37$ meV (CREMA 2010) decomposes
into Uehling (Antognini-style $+205.0074$ meV at $<1$ ppm vs theory
via full kernel integration), SE Bethe-log gap ($24\%$), Friar moment
($4.3\%$ leading via \S\ref{sec:proj_charge_density}),
K\"all\'en--Sabry two-loop VP, and deuteron polarizability.  The
autopsy is structurally parallel to \S\ref{sec:autopsy_lamb} but in
the muonic regime ($\beta = 2 m_e a_\mu = 1.475$, full Uehling kernel
rather than contact form).  Full table deferred to a follow-on
autopsy memo; source: \texttt{debug/sprint\_mh\_track\_a\_memo.md}.
