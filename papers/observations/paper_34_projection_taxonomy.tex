...
We catalogue sixteen projection mechanisms and tag each one along
...
We enumerate sixteen projection mechanisms.  For each: source and
...
\S~\ref{sec:offprecision}): both take the framework's
...
dimension.  Of the sixteen projections, nine add a dimension and
all nine also add the variable that carries it (perfect correlation
...
Fock $\circ$ rest-mass projection (equal-mass limit
$\lambda_a = \lambda_b = 0.5$) $\circ$ spectral action (Eides) &
$\alpha, m_e$ & frequency & $\alpha^2{\cdot}\mathbb{Q}$ & $+64.75$ ppm vs
Fee~1993 $1{,}233{,}607{,}216.4(3.2)$~MHz \\
\midrule
...
sixteen projections by axis-flag pattern, no projection adds a
dimension without also adding the variable that carries it (perfect
V/D correlation, 9/9); zero projections fall into the
...
Sixteen projections are listed, each tagged by variables, dimension,
