...
$2\pi$ and the Lorentzian-side modular-flow / boost-orbit period.  The
...
Unruh corollary $T_U = a/(2\pi)$ on a Rindler wedge is a 1--2 day
follow-up flagged in Track~D \S 5.4.  See Paper~32~§VIII Remark on the
...
chain residual matches PY~2010 Layer-2 budget magnitude. \\
...
ppm.  Follow-up sprint: deeper D HFS Roothaan autopsy beyond the
\S~\ref{sec:conv_rZG_DHFS} closure.
...
Follow-up sprint: positronium HFS five-component autopsy at
$\alpha^5$.
...
class-(a) literature-itemization (D.1, D.2, D.3 of the existing
catalogue) versus class-(ii) closed-form-vs-operator-level (D.4 of
the existing catalogue).  Future sprints adding entries should tag
which class the new entry falls under.

\textbf{What would falsify Pattern C.}  If the next three sprints
produce \S~V.D entries from electronic Lamb shift (where
Eides--Pachucki--Yerokhin itemizations are well-aligned at the
framework's precision) or from non-precision atomic spectra (where
the framework operates in larger residual ranges), Pattern C is
HFS-specific rather than a structural prediction about which
observable types surface convention exposures.  In that case the
predictive refinement above is data-availability artefact, not
structural.
...
\item \textbf{Closure of the projection list.}  Nineteen projections
...
constructions during the H 21cm and D 1S HFS autopsies.

Are there projections in the framework that are still not yet in
this list?  Two candidates that may need to be added on review:
...
(Paper~28 \S~vq\_sprint).  Decision pending.  See also the
Lorentz-boost candidate twentieth projection at the bottom of this
section (verdict: REQUIRES-EXTENSION at the framework level; not
adding now).
...
\item \textbf{Lorentz boost as a candidate twentieth projection.}
% Source: Sprint Lorentz-Boost-Scoping (May 2026, Track B);
% refines synthesis pass §7 Hunt 4 REQUIRES-EXTENSION verdict with
...
spectral triple is built (multi-month structural extension),
Lorentz boosts become a candidate twentieth projection at the
metric level; pre-Minkowski, only the Bisognano--Wichmann reading
of $\beta$-circle physics is available.}  No near-term track.
...
REQUIRES-EXTENSION verdict on the Lorentz-boost candidate twentieth
projection itself;\ it closes the documentation loop on the
