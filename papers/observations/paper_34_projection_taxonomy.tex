...
dimension.  Of the fifteen projections, eight add a dimension and
all eight also add the variable that carries it (perfect correlation
on the V/D side); zero projections add a dimension without a
variable.  The transcendental axis is the only one that admits an
...
$E_\text{Cas}^{S^3} = 1/240$ (conformal scalar) & Fock + conformal
coupling & none & dimensionless & rational & exact symbolic \\
...
Bargmann--Segal $+$ Casimir/$\zeta$-reg & none & dimensionless &
...
$n_\text{max} = 2, 3$ & Fock $+$ discrete Green's (no continuous
...
Fock $+$ observation/temporal-window $+$ Casimir/$\zeta$-reg & $T$ or
$\beta$ & energy density & $\pi^2 \cdot \mathbb{Q}$ & exact symbolic \\
Uehling shift $-27.13$ MHz at 2$S$ & spectral action $\circ$ Fock &
$Z, \alpha$ & energy & $\pi^2{\cdot}\mathbb{Q}$ & $<1\%$ \\
21~cm hyperfine $A_\text{HF} = 1418.84$ MHz Bohr-Fermi (HF-1) & spinor
$\circ$ Fermi contact $\circ$ Fock & $\alpha, g_p, m_e/m_p$ & energy &
$\alpha^2{\cdot}\mathbb{Q}$ & $-1102$ ppm (= $-a_e$) \\
21~cm hyperfine $A_\text{HF} = 1420.488$ MHz $+$ Parker--Toms $a_e$ (HF-2)
& spinor + Fermi contact + Fock + spectral action curvature expansion &
$\alpha, g_p, m_e/m_p$ & energy & $\alpha^2{\cdot}\mathbb{Q}, c_1
\cdot \mathbb{Q}$ & $+58$ ppm \\
21~cm hyperfine $A_\text{HF} = 1420.432$ MHz $+$ external Zemach $r_Z$
(HF-4) & HF-2 chain $+$ rest-mass projection $+$ Zemach Layer-2 input &
$r_Z$ & energy & calibration & $+18$ ppm (Eides Tab.~7.3 consistent) \\
$\mu$H 1S Bohr--Fermi $\nu_F(\mu p) = 182.4433$ meV (Sprint MH Track B)
& spinor $\circ$ Fermi contact $\circ$ Fock $\circ$ rest-mass projection
($m_e \to m_\mu$) & $\alpha, g_p, m_\mu/m_e, m_e/m_p$ & energy &
$\alpha^2 \cdot \mathbb{Q}$ & $+2$ ppm vs Eides pure-QED $\nu_F = 182.443$ meV \\
...
(sprint precision-catalogue 2026-05-08) & spinor $\circ$ Fermi contact
$\circ$ Fock; $I=1$ multiplicity $3/2$ on $A_\text{hf}\,\hat{I}\cdot\hat{S}$ &
$\alpha, g_d, m_e/m_d$ & frequency &
...
muonium triple closure) & spinor $\circ$ Fermi contact $\circ$ Fock $\circ$
rest-mass projection (lepton-unchanged, nucleus $m_p \to m_\mu$); leptonic
nucleus, no QCD content & $\alpha, m_e, m_\mu/m_e$ & frequency &
...
2026-05-08) & spinor $\circ$ Wigner 6j $\circ$ Wigner 3j (Gaunt) $\circ$
Fock; internal multi-focal (1s, 2p at distinct $Z_\text{eff}$),
bipolar $(k_1{=}0,k_2{=}2)$ direct + $(k_1{=}1,k_2{=}1)$ exchange &
$\alpha, Z$ & frequency & $\alpha^2{\cdot}\mathbb{Q}$ &
$-0.014\%$ vs NIST $+29{,}616.951(6)$ MHz; below LS-8a $\alpha^3/\pi$
budget \\
...
Spectral action $+$ proper-time integration $+$ vector-photon (one loop)
...
spinor $\circ$ Wigner 6j (Drake) $\circ$ Wigner 3j (Gaunt) $\circ$ Fock &
...
21~cm $A_\text{HF} = 1418.84$ MHz (HF-1 reduced-mass) & Fock + Fermi
contact + reduced-mass & $-1102$ ppm (= $-a_e$) & A & Bohr-Fermi level;
$g_e = 2$ (no anomalous moment).  Residual is exactly $a_e \approx
\alpha/(2\pi)$, addressed by HF-2.  Sprint HF, 2026-05-07. \\
21~cm $A_\text{HF} = 1420.488$ MHz (HF-2, $+a_e$ Schwinger) & HF-1
chain $+$ graph-native $F_2$ on Dirac-$S^3$ & $+58$ ppm & A & Schwinger
...
$\circ$ Fermi contact $\circ$ Fock $\circ$ rest-mass projection
...
framework $+$ Layer-2 annihilation) & spinor $\circ$ Fermi contact
...
double-counts.  Sprint precision-catalogue, 2026-05-08. \\
Mu 1S hyperfine framework Bohr--Fermi $+$ Schwinger one-loop
$4464.1905$ MHz vs Liu~1999 $4463.302765(53)$ MHz & spinor $\circ$
Fermi contact $\circ$ Fock $\circ$ rest-mass projection (lepton-unchanged,
nucleus $m_p \to m_\mu$) & $+199$ ppm & A &
...
$S^3$ with half-integer charges).  The metadata-level test correctly
flags this as the unique non-commuting pair in the worked examples.

\begin{theorem}[Composition Algebra]
\label{thm:composition_algebra}
The 15$\times$15 composition table classifies each ordered pair
$(P_1, P_2)$ as one of: \emph{commute} (admissible in both orders,
disjoint variables); \emph{one-way} (admissible in only one order,
constrained by Layer transitions); \emph{not composable};
\emph{idempotent} (Layer-1 self-composition); \emph{self-compose}
(non-trivial diagonal); \emph{variable conflict} (same variable name
added by both); \emph{inverse pair} (one is the inverse of the
other).  Of $15^2 = 225$ cells:
\begin{itemize}
\setlength\itemsep{0pt}
\item $182$ commute (80.9\%);
\item $22$ one-way pairs (constrained by Layer transitions);
\item $6$ variable conflicts (energy-scale-conflating compositions
of Fock, Bargmann--Segal, and spectral action);
\item $3$ idempotent projections on the diagonal (Sturmian,
Wigner~$3j$, Drake--Swainson);
\item $3$ inverse pairs (stereographic, Wigner~$3j$, Wigner~$D$).
\end{itemize}
The most informative entry is that \texttt{drake\_swainson} is the
unique projection with input\_layer = output\_layer = 2; all $22$
one-way cells in the table are drake\_swainson's row and column.
...
fifteen projections by axis-flag pattern, no projection adds a
dimension without also adding the variable that carries it (perfect
V/D correlation, 8/8); zero projections fall into the
...
\item \textbf{Closure of the projection list.}  Fifteen projections
...
Matsubara).  Are there projections in the framework that are still
not yet in this list?  Two candidates that may need to be added on
review: (a)~the Mellin / heat-kernel evaluation at fractional~$s$
(currently treated as part of spectral-action), and (b)~the
direction-resolved Hodge decomposition of vector-photon edges
(Paper~28 \S~vq\_sprint).  Decision pending.
...
Fifteen projections are listed, each tagged by variables, dimension,
...
signature level (with the strong form failing on bundle-non-trivial
spinor-lift compositions), and the 15$\times$15 composition table
flags Drake--Swainson as the unique projection with asymmetric
composition relations, accounting for all 22 one-way cells.  The
