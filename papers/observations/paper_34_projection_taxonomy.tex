...
1--2 day follow-up to Track~D) closes the Track~D~\S 5.4 flag with
...
triple) is the principal falsifier flagged in Track~D \S 3.1, with
...
$\mathbb{Q}[\log 2, \log 3]$ (Paper~18 embedding-log content;
\emph{e.g.\,} $R^0_\text{BP}(1s,1s;1s,1s) = -5 + 8\log 2$,
$R^0_\text{BP}(1s,2s;1s,2s) = -4\log 2 - 19/9 + 9\log(3)/2$).
...
sprint \texttt{HD-EFG-position-space} (4-8 weeks; analog of the
...
named Sprint L2 follow-up (6--12 month scale per Sprint L0 audit).
...
\textbf{Sprint L2-E (2026-05-17):\ literal identification at the Krein
operator-system level (finite cutoff).}\ The Lorentzian-side analog of
the Sprint L1 closure above is now closed at the strongest level.
...
Krein-level four-witness Wick-rotation theorem closes bit-exactly at
...
construction (Frobenius residual exactly $0.0$), confirming Sprint L2-E
is the genuine Lorentzian \emph{extension} of Paper~42, not a parallel
construction.  Substantive structural finding:\ Paper~42~\S~7.2 open
...
widens with temporal cutoff).  \textbf{Net status:} the Wick-rotation
projection now \textbf{lifts from ``structural correspondence at the
metric-functional level'' to ``literal identification at the Krein
operator-system level (finite cutoff)''} at every tested
$(n_{\max}, N_t)$.  The continuum / Lorentzian-propinquity-rate-level
extension (Sprint L3) remains open as a separate frontier;\ no
published Lorentzian propinquity construction exists as of
May~2026.  Cross-manifold W2b
...
in \texttt{papers/standalone/paper\_43\_lorentzian\_extension\_outline.md})
...
falsifier, 4--8 week follow-up. \\
Unruh temperature $T_U = a/(2\pi)$ on Rindler wedge &
Fock $\circ$ observation/temporal-window
(\S~\ref{sec:proj_observation}) at $\beta_U = 2\pi/a$ &
machinery-witness only & C &
Unruh pendant (\texttt{debug/unruh\_pendant\_memo.md}, May~2026,
1--2 day pendant follow-up to Track~D) reproduces $\beta_U = 2\pi/a$
...
(4--8 weeks; analog of the W1a-D Roothaan recoil and
...
ratio at $u=0$ converges $0.29 \to 0.49$ toward Kato $0.5$).  However,
the $2{}^1\!P \to 1{}^1\!S$ oscillator strength is \emph{deferred} to a
follow-on sprint: it requires extending the Hylleraas basis to L=1 P-state
trial functions (Schwartz 1961 form $(z_1 \pm z_2)\,\chi_\pm(s, t, u)$),
a $\sim$2-3 week structural extension out of scope for the Track 1
S-state implementation.  The single-exponent Hylleraas trial does not
close the asymmetric-Z$_{\text{eff}}$ excited-state issue at modest
basis size; the named structural closure is
\textbf{Hylleraas--Eckart double-zeta} (Hylleraas--Eckart 1933;
implemented in Drake's HYLA), which adds a second exponent $\beta$
to the trial $\Psi = \sum_{lmn}(a_{lmn} e^{-\alpha s - \beta t} +
b_{lmn} e^{-\alpha s + \beta t}) s^l t^{2m+1} u^n$ to represent the
inner $1s$ at $Z_a \approx 2$ and outer $2p$ at $Z_b \approx 1$.
Source: \texttt{debug/hylleraas\_r12\_track1\_memo.md}; data:
\texttt{debug/data/hylleraas\_r12\_track1\_results.json}.
...
\subsubsection{HD molecule $J=1$ rotational hyperfine (first molecular precision-catalogue entry; SCOPING)}
...
a scoping autopsy plus order-of-magnitude estimate, the first
...
has one OoM-level scoping autopsy with a named follow-on track for
...
PDF-level diagnostic ($\sim 3$-$5$ days) on Karshenboim 2005 \S~6.2-6.5
...
27 & Wick rotation / signature change & W.M.F. & H & THE BRIDGE; literal identification at Krein level (finite cutoff): Sprint L2-B/C/D/E closed bit-exact (2026-05-16/17). \\
...
\paragraph{Sprint L2-E closure of the Wick-rotation projection at the
Krein-operator-system level (2026-05-17).}
...
BW choice $H_{\mathrm{local}} = K_L^{\alpha, W}/\beta$, the Krein-level
four-witness Wick-rotation theorem closes bit-exactly:\ BW-$\alpha$
...
closure~\cite{paper42}, confirming Sprint L2-E is the Lorentzian
\emph{extension} of Paper~42, not a parallel construction.  The
headline structural finding:\ Paper~42~\S~7.2 ``$H_{\mathrm{local}}
...
the round-$S^3$ Riemannian truncation.  The Wick-rotation projection
in \S~\ref{sec:proj_wick_rotation} therefore promotes from
``structural correspondence at the metric-functional level'' to
``literal identification at the Krein operator-system level (finite
cutoff).''  The continuum / Lorentzian-propinquity-rate-level extension
(Sprint L3) remains open as a separate frontier;\ no published
...
(6--12 month scale per the Sprint L0 audit blocker-B1 estimate; the
...
Franco--Eckstein~2014~\cite{franco_eckstein2014} temporal Lorentzian
...
without a Minkowski-signature spectral triple (multi-month NCG
work, dominated by 6--12 months of original Lorentzian-propinquity
...
spectral triple is built (multi-month structural extension), the
...
dramatically and the multi-month extension becomes feasible.
...
May~2026) recommended a 1-week documentation deliverable
...
falsifier, 4--8 week follow-up at MEDIUM-HIGH priority).  Paper~32
...
1--2 day pendant follow-up to Track~D) added the flat-space face
...
N.~Franco and M.~Eckstein, ``Temporal Lorentzian spectral triples,''
...
A.~Devastato, F.~Lizzi, and P.~Martinetti, ``Lorentz signature and
twisted spectral triples,'' \textit{JHEP}\ \textbf{03}, 089 (2018).
arXiv:1710.04965.

\bibitem{nieuviarts2025a}
B.~Nieuviarts, ``Emergence of Lorentz symmetry from an
almost-commutative twisted spectral triple,'' arXiv:2502.18105
(2025).

\bibitem{nieuviarts2025b}
B.~Nieuviarts, ``Emergence of Time from a Twisted Spectral Triple
in Almost-Commutative Geometry,'' arXiv:2512.15450 (2025).

\bibitem{paper38}
J.~Loutey, ``SU(2) Propinquity Convergence on the Camporesi--Higuchi
Spectral Triple,'' GeoVac project, May 2026.
...
Cutoff,'' GeoVac project, May 2026.
