...
\end{itemize}
The most informative entry is the mutual one-way relation between
\texttt{spectral\_action} and \texttt{observation\_window}: each is
a Layer~1~$\to$~Layer~2 projection that consumes the bare graph
independently, so neither feeds the other.  The metadata correctly
encodes the field-theoretic UV/IR ordering anomaly --- the thermal
limit $\beta \to \infty$ does not commute with the UV limit
$\Lambda \to \infty$ in continuum QFT, and signature arithmetic
captures the structural content of this asymmetry.
\end{theorem}
...
encodes the field-theoretic UV/IR ordering anomaly through the
mutual one-way relation between spectral action and
observation/temporal-window.  Of the three axes, only the
