...
\subsection{General-$s$ closed form for
$D_{\mathrm{even}} - D_{\mathrm{odd}}$}
\label{sec:D_diff_closed}

The $s=4$ identities above generalize to an exact closed form at every
integer $s \ge 2$. Two elementary Hurwitz identities suffice:
$\zeta(s, 5/4) = \zeta(s, 1/4) - 4^s$ (shift recurrence) and
$\zeta(s, 3/4) = \zeta(s, 1/4) - 4^s\,\beta(s)$ (the Dirichlet-beta
definition). Substituting both into the Hurwitz forms of
$D_{\mathrm{even}}(s)$ and $D_{\mathrm{odd}}(s)$, the
$\zeta(s, 1/4)$ pieces cancel and the $2^{2s-1}$ prefactors match across
the $s$ and $s-2$ contributions, leaving
\begin{equation}
...
\end{equation}
verified symbolically (sympy residual zero) at
$s \in \{2, 3, \ldots, 10, 15\}$ and by PSLQ at 100-digit precision at
every integer $s$ in the tested range.
Equation~\eqref{eq:D_diff_closed} upgrades the qualitative statement
``vertex parity acts as $\chi_{-4}$ on the mode sum'' to a quantitative
identity at every $s$: the $\chi_{-4}$ structure enters the
spectral Dirichlet difference $D_{\mathrm{even}}(s) - D_{\mathrm{odd}}(s)$
as the exact linear combination
$\beta(s) - \beta(s - 2)$, with integer prefactor $2^{s-1}$.
The $s=3$ case is the Hurwitz-pole-regularized limit giving
$D_{\mathrm{diff}}(3) = \pi^3/8 - \pi$ (cf. the residue subtraction in the
appendix of Ref.~\cite{rh_j_memo}).

At $s=4$, (\ref{eq:D_diff_closed}) reproduces
$D_{\mathrm{even}}(4) - D_{\mathrm{odd}}(4) = 8(\beta(4) - G)$
(since $G = \beta(2)$), consistent with
Eqs.~\eqref{eq:D_even}--\eqref{eq:D_odd}.

Identity~\eqref{eq:D_diff_closed} is a concrete spectral-zeta to
Dirichlet-$L$ bridge on $S^3$. It does not by itself constrain the
zeros of $\beta(s)$ or of $D_{\mathrm{even}}, D_{\mathrm{odd}}$
individually: a zero of $\beta$ at $s = \rho$ propagates to zeros of
$D_{\mathrm{diff}}$ at both $\rho$ and $\rho + 2$, which is compatible
with but does not enforce $\Re\rho = 1/2$. The identity is a structural
realization of the vertex-parity-as-$\chi_{-4}$ mechanism rather than
an RH-type statement.
...
correction:
\begin{equation}
  S_{\min} = 2.47993\,69380\,34222\,55441\,35795\,00083\ldots
