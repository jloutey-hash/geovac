...
inter-nuclear compression.  A Fock-weighted kinetic correction, whose
\emph{form} is derived from the energy-shell constraint $p_0^2 = Z^2/n^2$
of Paper~7 but whose overall scale $\lambda$ is a single fitted parameter
(retained as a labeled qualitative diagnostic, not a zero-parameter
result), partially restores the equilibrium by concentrating
orthogonalization repulsion on compact core orbitals, achieving
$R_{\mathrm{eq}}\approx 3.8$~bohr and $D_e^{\mathrm{CP}}\approx 0.046$~Ha.  A 29-version diagnostic arc
...
retaining its atom-centered conformal structure and recovering, under the
Fock projection, the hydrogenic spectrum $\varepsilon_{n} = -Z^2/(2n^2)$
(the bare graph Laplacian is positive-semidefinite; these negative
energies are its continuum image, not graph-Laplacian eigenvalues).  At $n_{\max}=3$, the lithium
...
The diagonal blocks carry the hydrogenic eigenvalues
$\varepsilon_n = -Z^2/(2n^2)$ (the continuum image of the graph spectrum
under the Fock projection, not graph-Laplacian eigenvalues) supplemented
by the Mulliken
...
\noindent\emph{Composition note.}  The tabulated fragment energies
$E_{\mathrm{Li}}^{\mathrm{mol}} = -7.392$ and
$E_{\mathrm{H}}^{\mathrm{mol}} = -0.500$~Ha are the free-atom
(own-basis) energies; substituted directly into Eq.~(\ref{eq:cp})
they yield the \emph{raw} binding $D_e^{\mathrm{raw}} = 0.226$~Ha.
The counterpoise correction evaluates the same fragments in the full
dimer (ghost) basis---$E_{\mathrm{Li}}^{\mathrm{ghost}} = -7.497$,
$E_{\mathrm{H}}^{\mathrm{ghost}} = -0.510$~Ha---so that
Eq.~(\ref{eq:cp}) gives the counterpoise-corrected
$D_e^{\mathrm{CP}} = D_e^{\mathrm{raw}} - \Delta E_{\mathrm{BSSE}}
= 0.226 - 0.115 = 0.110$~Ha.
...
sharing a single momentum parameter $p_0$, the one-electron Hamiltonian
and overlap obey the two-term identity
$H_{ij} = -\tfrac12 p_0^2 S_{ij} + (1-\beta_j)V_{ij}$ and share a single
orthogonal $SO(4)$ Wigner-$D$ congruence $D^{(n)}(\gamma(R))$ (so $H$ is
block-, not scalar-, proportional to $S$).  The generalized eigenvalues
are therefore independent of the bond angle $\gamma$, hence of $R$,
yielding pure $1/R$ nuclear repulsion with no electronic binding.  Recovering
...
alternative is therefore falsified \emph{in this single-$n$, shared-$p_0$
form}.  The scope matters:\ the genuine cross-$n$ Coulomb--Sturmian overlap
couples $n$ and escapes the theorem, and it binds (Paper~8, Remark).  What is
ruled out is this approximation, not the discrete framework.

\paragraph{Cross-$n$ hybridization (v0.9.37).}
A further hypothesis attributed the geometric error to the $D$-matrix's
block-diagonality in $n$: the Shibuya--Wulfman formula connects only
same-$n$ orbitals (Li~$1s \leftrightarrow$ H~$1s$, Li~$2s \leftrightarrow$
H~$2s$), giving zero off-diagonal coupling for the chemically relevant
Li~$2s \leftrightarrow$ H~$1s$ pair.  Overlap-based bridges using the
Shibuya--Wulfman energy scale $-(Z_{\mathrm{eff}}/p_0)\sin\gamma$ were
implemented, producing Li~$2s$--H~$1s$ coupling of 0.052~Ha (40\% of
the same-$n$ $D$-matrix coupling).  However, the PES worsened: $D_e$
increased from 7.1 to 10.7~eV because the cross-$n$ coupling adds
binding at short~$R$ where the overlap is largest, reinforcing the
existing overbinding.  Cross-shell hybridization is eliminated as a
mechanism for the geometry error.

\paragraph{Level~4 locked-core (v0.9.37).}
The Level~4 molecule-frame hyperspherical solver (Paper~15) was extended
to LiH by locking the Li~$1s^2$ core and treating the two active
electrons in hyperspherical coordinates with effective charges
$Z_{\mathrm{eff}}$.  Four core models were tested (bare $Z$, integer
screening, Clementi--Raimondi, and Core Hartree with penetration); all
produce monotonically attractive PES with $R_{\mathrm{eq}} \lesssim
2.5$~bohr and $D_e > 12$~eV.  The failure has the same root cause:
the core-to-H-nucleus attraction $V_{\mathrm{cross}} = -2Z_B\langle
1s^2_Z | 1/r_B | 1s^2_Z \rangle$ behaves as $-2/R$ at large $R$ but
saturates exponentially at short $R$ (exp$(-6R)$ decay for Li $1s$),
providing insufficient repulsion to establish a minimum at
$R \approx 3$~bohr.

\medskip

Having falsified BSSE as a geometric driver, eliminated integral
incompleteness, ruled out orbital exponent relaxation, mathematically
disproven the Sturmian alternative, excluded cross-$n$ hybridization
and locked-core hyperspherical approaches, the $O(V)$
Topological LCAO remains, among the alternatives surveyed here, the
only sound discrete framework for heteronuclear FCI.  The absence of an equilibrium geometry in the
...
achieves $R_{\mathrm{eq}} = 2.001$~bohr (0.21\% error) at the
finite-difference level, while the spectral Laguerre radial solver
reaches $0.0002\%$ energy error, both with zero fitted
...
a qualitatively correct weighting for kinetic repulsion reinforces the physical
...
v0.9.31 & MO Sturmian FCI (full)
  & --- & --- & Unbound/overbinding & Shared $SO(4)$ congruence, eigenvalues $\gamma$-independent \\
...
\bibitem{paper0}
J.~Loutey, ``Angular Momentum as Information Geometry: Isotropic Packing
and the Universal Angular Cross-Section,'' GeoVac Paper~0 (2026).

\bibitem{paper1}
J.~Loutey, ``The Geometric Atom: Atomic Structure as a Packing Problem,''
GeoVac Paper~1 (2026).
...
\bibitem{paper8}
J.~Loutey, ``Bond Sphere: Diatomic Molecules as $S^3$ and their Natural
Sturmian Basis,'' GeoVac Papers~8--9 (2026).
