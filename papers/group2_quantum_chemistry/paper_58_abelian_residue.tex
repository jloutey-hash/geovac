...
atom-centered Coulomb-Sturmian blocks whose angular matrix elements are
generated from quantum-number labels by Gaunt selection, with no spatial
quadrature.  This yields structurally sparse qubit Hamiltonians---334
Pauli terms for LiH at $Q = 30$, $O(Q^{2.5})$ scaling (Paper~14)---at
the cost of a documented binding failure for second-row diatomics
(the W1e wall, Papers~19/20).

We settle the question of whether that trade was necessary.  Using exact
rational two-center machinery (Mulliken--Ruedenberg auxiliary functions
...
cross-center evaluation.  Only the axial $m$-rule does.  Electron-repulsion
density inflates $13.8\times$ over the builder at $n_{\max} = 2$, rising to
$15.0\times$ at $n_{\max} = 3$, and cross-center one-body entries measure
$0.19$--$0.80$~Ha---bond scale, not perturbative.
...
requires the full sphere and does not.  Surviving label-generated
sparsity is therefore governed by the largest common abelian symmetry,
which is computable from the molecular point group in advance.

Two re-readings follow.  First, the $17.9\times$ Pauli inflation
previously attributed to L\"owdin orthogonalization
(v4.73.1) was never orthogonalization's price: it is within
measurement of the genuine tensor's own density, so L\"owdin was paying
an honest bill.  Second \textbf{[OBSERVATION]}, four independent attempts
to relocate the non-orthogonality---into the operator, into a dual basis,
into a high-symmetry embedding, into device-measured many-body
overlaps---each moved the cost without reducing it, because the cost
measures the physical asymmetry of a two-center problem.
...
integrals are genuine end-to-end.  This localizes W1e to the Hamiltonian
specification rather than the correlation treatment, by construction.
\end{abstract}
...
Paper~14~\cite{loutey_paper14}: composed LiH at 334 Pauli terms on
$Q = 30$ qubits, $O(Q^{2.5})$ Pauli scaling against $O(Q^{3.9\text{--}4.3})$
for Gaussian baselines, and a $190\times$ reduction against cc-pVDZ.
...
(Papers~19/20; CHANGELOG v2.19.4--v3.87.0).  The wall was localized to
...
rationals via the Mulliken--Ruedenberg auxiliary functions
$A_n$/$B_n$~\cite{mulliken1949,ruedenberg1951}, evaluated in prolate
spheroidal coordinates where the two-center problem
...
\end{equation}
holds as an \emph{exact rational equality} on every entry, which is a
machinery check no fortuitous cancellation survives.  Four independent
...
The electron-repulsion tensor is $29.4\%$ dense genuinely against
$2.1\%$ for the builder.  At $n_{\max} = 3$ ($M = 28$) the comparison is
$114{,}280$ nonzeros ($18.6\%$) against $7{,}600$ ($1.2\%$): an inflation
factor of $13.8\times$ rising to $15.0\times$, i.e.\ \emph{increasing}
with basis size.

Two further measurements matter for interpretation.  First, per-side
Gaunt feasibility removes less than $6\%$ of entries beyond what the
$m$-rule already removes, and the three cross classes
$(AA|AB) + (AB|AB) + (AB|BB)$ account for $80\%$ of the genuine tensor.
The residual $l$-selection is therefore confined to the same-center
$(AA|AA)$ and $(BB|BB)$ blocks.  Second, cross-center magnitudes are
large: overlaps $0.08$--$0.52$, one-body entries $0.19$--$0.80$~Ha.  For
...
different centers.
\item[(iii)] The surviving label-generated sparsity is exactly that
generated by the largest symmetry group common to both centers, which is
the axial $U(1)$ and is abelian.
\end{enumerate}
\end{theorem}
...
the $<6\%$ residual measured in Sec.~\ref{sec:census}.)
...
\begin{prediction}[Water, untested]
\label{pred:water}
$\mathrm{H_2O}$ has point group $C_{2v}$, which is order four and
\emph{abelian}, isomorphic to $\mathbb{Z}_2 \times \mathbb{Z}_2$.  All
its irreducibles are one-dimensional, so it admits a clean two-bit
grading and exactly two qubits of symmetry-based reduction---not a
multiplicative factor.  This sharpens the qualitative expectation that
the framework's angular advantage degrades for non-linear molecules into
a specific count, and is falsifiable on the existing $\mathrm{H_2O}$
build.
\end{prediction}
...
constant factor---not the $O(Q^{2.5})$ scaling story the framework's
qubit results rest on.
...
retrofitting L\"owdin orthogonalization onto the balanced LiH integrals
...
genuine tensor's own inflation over the builder is $\sim 15\times$,
within measurement of the L\"owdin figure.  Orthogonalization was not
destroying sparsity that the untransformed tensor possessed; the
untransformed tensor never possessed it.  ``L\"owdin destroys sparsity''
...
Four independent attempts to move the non-orthogonality out of the way
...
comparably, and by theorem rather than
happenstance~\cite{artacho1991};
...
theorem: it is a pattern across four instances with a plausible cause,
not a proof that no fifth currency exists.  The underlying obstruction
...
end-to-end integrals should bind NaH without any change to the treatment
of correlation.  It does.
...
fragment-native orbitals, and general-$N$ non-orthogonal Slater--Condon
evaluation by L\"owdin cofactors.  Orbital exponents were optimized on
...
a fit.  Machinery validation: rotational invariance at
$1.1 \times 10^{-13}$, and the 91-determinant complete space reproduces
bitstring FCI with a span identity of exactly zero.

\begin{table}[b]
\caption{NaH, all-electron 12e, $M = 7$.  Percentages are of in-basis FCI
binding.  Experiment: $R_{\rm eq} = 3.566\,a_0$, $D_e = 1.961$~eV.}
\label{tab:nah}
\begin{ruledtabular}
\begin{tabular}{lrrr}
...
$D_e$ deficit ($55\%$ of experiment) is minimal-basis incompleteness---no
polarization, no diffuse functions---and reproduces the pattern of the
four-electron LiH case at $53\%$; it is not an integral-fidelity effect.
...
first and second row, with and without $p$ orbitals.  It generalizes to
systems not yet run and is cheap to falsify.
...
obstruction, because the census closed the last one available.
...
\begin{table}[b]
\caption{Claim $\to$ backing.  Test backfill from the exploratory drivers
into \texttt{tests/} is a precondition for this paper's release; see
Sec.~\ref{sec:scope}.}
\label{tab:backing}
\begin{ruledtabular}
\begin{tabular}{lll}
claim & tier & backing (\texttt{tests/}) \\
\hline
Table~\ref{tab:census} & MEASURED & \texttt{..p58\_census} \\
Eq.~\eqref{eq:braket_certificate} & SYMBOLIC & \texttt{..p58\_certificate} \\
Thm.~\ref{thm:abelian_residue} & INTERNAL & \texttt{..p58\_m\_rule} \\
Pred.~\ref{pred:water} & UNTESTED & --- \\
Obs.~\ref{obs:cost_conservation} & OBSERVATION & CHANGELOG v4.73.x \\
Table~\ref{tab:nah} & PANEL-VER. & \texttt{..p58\_nah} \\
Obs.~\ref{obs:truncation} & MEASURED & \texttt{..p58\_nah} \\
\end{tabular}
\end{ruledtabular}
\end{table}
...
J.~Loutey, ``Prolate Spheroidal Natural Geometry for Two-Center
One-Electron Systems,'' Paper 11 in the Geometric Vacuum series.

\bibitem{loutey_paper14}
J.~Loutey, ``Qubit Encodings from Natural Geometry,'' Paper 14 in the
Geometric Vacuum series.

\bibitem{mulliken1949}
R.~S.~Mulliken, C.~A.~Rieke, D.~Orloff, and H.~Orloff, ``Formulas and
numerical tables for overlap integrals,''
\textit{J.\ Chem.\ Phys.}\ \textbf{17}, 1248 (1949).

\bibitem{ruedenberg1951}
K.~Ruedenberg, ``A study of two-center integrals useful in calculations
on molecular structure,''
\textit{J.\ Chem.\ Phys.}\ \textbf{19}, 1459 (1951).

\bibitem{artacho1991}
E.~Artacho and L.~M.~del~Bosch, ``Nonorthogonal basis sets in quantum
mechanics: Representations and second quantization,''
\textit{Phys.\ Rev.\ A}\ \textbf{43}, 5770 (1991).
