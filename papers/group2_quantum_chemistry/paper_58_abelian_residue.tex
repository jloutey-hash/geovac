...
\emph{Provenance, stated precisely because the three rows differ.} The
$S$ and $h$ rows are \emph{computed}: every entry is evaluated in exact
rational arithmetic by the machinery of
Sec.~\ref{sec:census}\,A, so both the zeros and the non-zeros are decided
rather than inferred.  The $g$ row is \emph{counted}: it is a
...
permitted-dense genuinely against $2.1\%$ for the builder.  At $n_{\max} = 3$ ($M = 28$) the comparison is
...
condition on $l$ whatsoever.  The $m$-rule is hard-coded because it
...
inflates the Pauli count, in every configuration tested
...
\emph{Documentation discrepancy.} The docstring of
\texttt{extended\_tapered\_from\_spec} states that atom-swap ``saves
...
\begin{table}[b]
\caption{Claim $\to$ backing.  \textsc{live} $=$ passing test in
\texttt{tests/test\_paper58\_abelian\_residue.py}.  \textsc{owed} $=$
...
artifact; see Sec.~\ref{sec:scope}.}
\label{tab:backing}
\begin{ruledtabular}
\begin{tabular}{lll}
claim & tier & backing \\
...
Table~\ref{tab:census} $g$ & COUNTED & \textsc{owed} \\
...
Table~\ref{tab:nah} & PANEL-VER. & \textsc{owed} \\
Obs.~\ref{obs:truncation} & MEASURED & \textsc{owed} \\
\end{tabular}
\end{ruledtabular}
\end{table}
