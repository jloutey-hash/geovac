...
\emph{The integrated integral is an elliptic multiple modular value.}  The full
parameter integral pulls back, via Eq.~\eqref{eq:lambda_rho}, to an iterated
integral over $X(2)$ of period-weighted forms---a multiple modular value for
$\Gamma(2)$ in the sense of Brown, i.e.\ an elliptic polylogarithm.  A dedicated
evaluator (a fixed Gauss--Legendre tensor grid with a $\sin^{2}$ substitution that
removes the endpoint non-analyticity, replacing the divergent-cost adaptive
quadrature) gives the collinear value \textbf{[MEASURED, $\sim\!17$ digits]}
$0.39535576590171392\ldots$; a weight-graded search finds it is \emph{not} a
low-height combination of the central fibre's period $K(\tfrac12)$, its
quasi-period $E(\tfrac12)$, and $\pi$, nor of the classical polylogarithms,
consistent with a genuine (non-classical, multi-fibre) elliptic multiple modular
value.  \textbf{[OBSERVATION]} This places the three-center integral inside the
...
for~\cite{loutey_paper56}---the concrete elliptic period the mixed-Tate substrate
could not itself supply.  \textbf{[OPEN]} Its explicit realization in the
$\Gamma(2)$ iterated-Eisenstein basis is the natural next step, of a piece with the
finite closed form of Sec.~\ref{sec:obstruction}.  We note the level is
