...
$-2.8963\,\text{Ha}$ (0.26\% error versus exact) at $n_{\max}=5$;
for lithium, to $-7.4007\,\text{Ha}$ (1.03\% error) at $n_{\max}=5$.
Helium surpasses PySCF/STO-3G accuracy on the same system (the lithium
STO-3G comparison is not computed).
For beryllium (4 electrons), the solver achieves $E = -14.563\,\text{Ha}$
(0.71\% error) at $n_{\max}=4$ with $487{,}635$ determinants---the
first four-electron calculation in this framework.
For lithium hydride (4 electrons, first heteronuclear molecule), the
CP-corrected \emph{vertical} binding energy at the fixed experimental
geometry is $D_e^{\text{CP}} = 0.110\,\text{Ha}$ (19\% error versus
experiment); the $R$-independence of the graph-Laplacian kinetic energy
makes the potential-energy surface monotonically attractive with no
equilibrium geometry, so this is a fixed-$R$ binding energy rather than
an optimized $D_e$.  Basis truncation
at $n_{\max}=3$ is identified as the dominant error source via systematic
diagnostic decomposition, and basis set superposition error (0.115~Ha)
is quantified and removed via Boys--Bernardi counterpoise correction.
...
length).
Exact non-relativistic energies are $E_{\text{He}} = -2.9037\,\text{Ha}$~\cite{Pekeris1958}
...
Exact non-relativistic energies: $E_{\text{Be}} = -14.6674\,\text{Ha}$~\cite{ChakravortyDavidson1993}.}
\begin{ruledtabular}
\begin{tabular}{lcccc}
...
\colrule
He ($Z\!=\!2$) & GeoVac $n_{\max}\!=\!5$ & $-2.8963$ & 0.26 & 5995 \\
               & PySCF/STO-3G & $-2.8078$ & 3.30 & 1 \\
\colrule
Li ($Z\!=\!3$) & GeoVac $n_{\max}\!=\!4$ & $-7.3987$ & 1.06 & 34\,220 \\
               & GeoVac $n_{\max}\!=\!5$ & $-7.4007$ & 1.03 & 215\,820 \\
               & PySCF/STO-3G & --- & --- & --- \\
\colrule
Be ($Z\!=\!4$) & GeoVac $n_{\max}\!=\!3$ & $-14.558$ & 0.75 & 20\,475 \\
               & GeoVac $n_{\max}\!=\!4$ & $-14.563$ & 0.71 & 487\,635 \\
\end{tabular}
\end{ruledtabular}
\end{table}
...
GeoVac achieves 0.26\% error for helium, 1.03\% for lithium
(at $n_{\max}=5$), and 0.71\% for beryllium (at $n_{\max}=4$),
...
0.50\% ($n_{\max}=2$) $\to$ 0.37\% ($n_{\max}=3$) $\to$ 0.29\%
...
5.00\% ($n_{\max}=2$) $\to$ 1.12\% ($n_{\max}=3$) $\to$ 1.06\%
($n_{\max}=4$).
The lithium error at $n_{\max} \geq 3$ (1.06--1.12\%) remains
\emph{above} the Hartree--Fock limit of $0.61\%$, so---unlike
helium---the lithium FCI does not recover correlation energy beyond
mean-field theory at the basis sizes reached here.
...
The CP-corrected binding energy increases monotonically as $R$
decreases over the sampled range (the deepest computed point is
$D_e^{\text{CP}} = 0.171$~Ha at $R = 2.5$~bohr, the smallest $R$
sampled---not an interior minimum).  Consistent with the
graph-concatenation analysis of the companion molecular paper, the
balanced Hamiltonian produces a \emph{monotonically attractive}
surface with no equilibrium geometry: the $R$-independent
graph-Laplacian kinetic energy, not basis incompleteness, is the
structural cause (Sec.~\ref{sec:limitations}).
...
\colrule
2 & 2 &        45 &        203 & -2.889303 & 0.50 & $<$1 \\
2 & 3 &       378 &      6\,066 & -2.892932 & 0.37 & $<$1 \\
2 & 4 &     1\,770 &    87\,550 & -2.895227 & 0.29 & $<$1 \\
2 & 5 &     5\,995 &   777\,019 & -2.896298 & 0.26 & 1.6 \\
\colrule
3 & 2 &       120 &        604 & -7.104490 & 5.00 & $<$1 \\
3 & 3 &     3\,276 &    111\,192 & -7.394597 & 1.12 & $<$1 \\
3 & 4 &   34\,220 & 4\,385\,956 & -7.398718 & 1.06 & 7.6 \\
3 & 5 &  215\,820 & 78\,025\,400 & -7.400705 & 1.03 & 116 \\
\colrule
4 & 3 &   20\,475 &           1\,224\,507 & -14.558052 & 0.75 & 2.9 \\
4 & 4 &  487\,635 & 118\,132\,111 & -14.562659 & 0.71 & 357 \\
\end{tabular}
\end{ruledtabular}
\end{table}
...
basis sizes, converging monotonically from 5.00\% to 1.03\%.
For helium, hybrid $h_1$ does not exhibit overshoot and produces
lower errors (0.29\% versus 1.99\% at $n_{\max}=4$ with exact $h_1$),
...
The helium error of 0.26\% at $n_{\max}=5$, lithium error of
...
Diagonal: $\varepsilon_k = \tfrac12 p_0^2 - p_0^2/\beta_k$ (equals
$-p_0^2/2$ only in the degenerate case $\beta_k=1$).  Off-diagonal:
$D$-matrix elements weighted by nuclear charges (same-$m$ block-diagonal).
...
0.26\% accuracy for helium, 1.03\% for lithium, and 0.71\% for
...
\emph{vertical} binding energy at fixed geometry ($D_e^{\text{CP}} =
0.110$~Ha, 19\% error) confirms that the molecular bridge architecture
transfers correctly to asymmetric systems---though, as in the
homonuclear case, the $R$-independent graph-Laplacian kinetic energy
yields a monotonically attractive surface with no equilibrium geometry.
A systematic diagnostic decomposition---zeroing cross-nuclear,
...
J.~Chem.~Phys.\ \textbf{51}, 2657 (1969).
...
graph-native achieves 0.25\% error, essentially matching the
...
0.23\% at $n_{\max} = 6$, and at $n_{\max} = 7$ reaches 0.216\%
