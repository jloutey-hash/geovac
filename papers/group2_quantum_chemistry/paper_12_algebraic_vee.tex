...
$-7.932$ analytically:\ every reduced element matches its Monte-Carlo target
below $1\%$ (e.g.\ $\bar F=3.570$, $\sigma^2=0.128$, $h=-0.103$) and the
assembled correlation lowering is $-33$~mHa, or $-29$~mHa (matching the
Monte-Carlo $-29.5$~mHa) once the single grid-limited Yukawa covariance
$\mathrm{Cov}[F,\sum e^{-\gamma r}/r]$---the long-range-geminal ill-conditioning,
amplified by $\bar F=3.570$---is taken at converged precision.  The genuinely
...
Backing:\ test \texttt{tests/test\_paper12\_r12.py} and the reduction of
Eq.~\eqref{eq:r12_kinetic_collapse} above;\ the two-center bridge, the triangle
reduction, and the full analytic $2\times2$ energy are validated against
variational Monte-Carlo in the sprint tree and chronicled in
\texttt{CHANGELOG.md} (v5.15.10--v5.15.13).  (The prototype engine migrates from
the sprint tree into \texttt{geovac/}, with dedicated regression tests, in a
follow-on;\ it is a proof-of-concept, not yet a production path.)
