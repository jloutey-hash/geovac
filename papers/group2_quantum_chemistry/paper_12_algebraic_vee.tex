...
closed-shell blocks---and that general path has so far been checked only as a
Monte-Carlo--versus--Monte-Carlo factorization test at the $5\%$ level.  Every
deterministic validation above uses the block-factorized closed-shell reference
$|\Phi_0|^2=D(1,2)\,D(3,4)$, which is the enabling assumption of the
spectator-integration bookkeeping.  \emph{Reference generality.}  Read at the
...
reference is precisely the test that the general, non-isotropic-leaf path still
lacks.
