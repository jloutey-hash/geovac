...
multipole sum terminates at $L\le 2\,l_{\rm bridge}$ (measured
$s\!\to\!0,\,p\!\to\!2,\,d\!\to\!4$, bounded by the finite orbital angular
content, not by the Coulomb), and the Legendre addition theorem factorizes the
chain across the bridge into a finite $(L,M)$ contraction of vertex kernels---the
chain analogue of the triangle contraction.  A native evaluation was validated
against brute-force $12$-dimensional Monte-Carlo integration to $2\times10^{-4}$
on the Be-relevant four-electron integral, including with the antisymmetric
determinant (exchange).  As a proof of concept it was exercised inside a
...
by the \emph{same} construction---an isotropic one-dimensional leaf dressing
followed by a two-center prolate Neumann Coulomb between the dressed
densities---and reproduces an independent twelve-dimensional Monte-Carlo
evaluation to $\sim\!2\times10^{-5}$ for \emph{every} azimuthal channel
($\sigma$, $\pi$, $\delta$; the $\pi$ channel, which dominates valence
correlation, included), with the self-Coulomb step anchored to the closed form
$5\alpha/8$.  \emph{Second}, the same machinery evaluates the many-body matrix
