...
Legendre moment $B_l$.  (The $|m| \ge 1$ channels keep the same kernel;
their ordered-$\xi$ integral is available both from a two-dimensional
spectral quadrature and---generalising the same $A_l$, $B_l$, $X_l$
recurrences to associated Legendre functions
(Sec.~\ref{sec:azimuthal})---from a moment-recurrence engine that is
\emph{stable} in the $\delta$ ($|m| = 2$) sector where the
differentiation route diverges.  That engine is recurrence-stable and
quadrature-seeded rather than fully quadrature-free.)
...
variationally to $\gvq{p12_rebased_de_pct}{99.77}\%$ of $D_e$
($\gvq{p12_rebased_err_mha}{0.41}$~mHa, inside chemical accuracy) at a
larger truncation (Sec.~\ref{sec:recondition}).  \textbf{[WITHDRAWN]} This paper
...
relations and closed forms, with the sole exception of the
log-singular second-kind moment $B_l$, which uses a single
one-dimensional adaptive quadrature (Sec.~III.D).  The
...
relations; the log-singular second-kind moment $B_l$ requires
a single one-dimensional adaptive quadrature (below).
...
$V_{ee}$ angular series terminates exactly, with only the single
$B_l$ moment carried by a 1D quadrature).
...
$(2,2)$ $\delta$ subspace being the smaller).  One honest limit
remains:\
the low-$l$ $B_l^{m,s}$ seeds still use a single one-dimensional
quadrature---as the $\sigma$-sector $B_l$ does---and the moment tables
are built in extended precision, so the engine is recurrence-stable and
quadrature-seeded rather than fully quadrature-free.  Backing:\
...
tested.  Every matrix entry is therefore built in extended precision,
...
\caption{H$_2$ at $R = 1.4011$~bohr, re-based to orthogonal polynomials
  ...
  extended-precision build and unit-normalized solve of the text.
...
energy agrees to $<1$~$\mu$Ha at every truncation) at a normalized
condition number two to three orders of magnitude smaller
($9.1\times10^4$ vs $9.4\times10^{10}$ at $(5,5)+\delta$), which is what
keeps the downcast solve trustworthy at large truncation.  It does
...
chemistry in which the two-electron integrals are reduced to
recurrence relations on quantum number labels, the sole residual
...
\begin{enumerate}
  \item \textbf{92.4\% of exact $D_e$} with algebraic $V_{ee}$ (a single 1D
    quadrature for the log-singular $B_l$ moment) and zero fitted
    parameters.
  ...
  \item \textbf{99.1\% of exact $D_e$} (stability envelope
    ...
    stability envelope.
  \item \textbf{$\gvq{p12_rebased_de_pct}{99.77}\%$ of exact $D_e$}
    ($\gvq{p12_rebased_err_mha}{0.41}$~mHa, chemical accuracy) once that
    $99.1\%$ monomial basis is re-conditioned:\ its $\xi^j$ radial factor
...
recurrence relations on quantum number labels replace the
six-dimensional quadrature for electron-electron repulsion, leaving
only a single one-dimensional quadrature for the log-singular $B_l$
moment---one such seed per kernel order $m$ once the same recurrences
are carried to the associated Legendre functions of the $|m| \ge 1$
channels (Sec.~\ref{sec:azimuthal}).  Just as
