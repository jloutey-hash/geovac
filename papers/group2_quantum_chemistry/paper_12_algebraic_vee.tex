...
identity to a heteronuclear diatomic (HeH$^+$) and to the many-body matrix
elements of a two-center four-electron explicit-$r_{12}$ CI, where the
genuinely four-body Coulomb-bridge term is evaluated exactly \emph{inside} a
real correlation quantity---placing the classical exactness boundary at a
two-center three-body integral (Sec.~\ref{sec:r12}).
\end{abstract}
...
$s(d)=(1-e^{-\gamma d})/d$.)  The remaining boundary is now sharper than
``$N=4$'':\ completing the diagonal $\langle\chi^2 V_{ee}\rangle$ requires a
two-center \emph{three-body triangle}
$\langle f_{13}f_{23}\,r_{12}^{-1}\rangle$ (two correlation edges sharing a
vertex, closed by a Coulomb), whose down-integration yields a \emph{non-separable}
two-point kernel $H_2(1,2)=\int\rho(3)f(r_{13})f(r_{23})\,d^3r_3$;\ a low-rank
expansion of $H_2$ followed by a per-mode Neumann Coulomb is the identified next
reduction, and the full analytic assembly of the CI energy awaits it.  Until
then the assembled energy is available from the variational Monte-Carlo route
alone, which places the explicit-$r_{12}$ correction correctly signed and above
the exact value.

Backing:\ test \texttt{tests/test\_paper12\_r12.py} and the reduction of
Eq.~\eqref{eq:r12_kinetic_collapse} above;\ the two-center bridge and the
many-body matrix-element reductions are validated against variational
Monte-Carlo in the sprint tree and chronicled in \texttt{CHANGELOG.md}
(v5.15.10--v5.15.12).  (The prototype engine migrates from the sprint tree into
\texttt{geovac/}, with dedicated regression tests, when the triangle reduction
closes the analytic energy.)
