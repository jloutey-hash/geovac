...
geometry for diatomic molecules, solving H$_2^+$ to machine precision
(spectral Laguerre solver) with zero free parameters.  The extension to two electrons requires
...
prolate spheroidal FE-DVR treatment of Tao, McCurdy and Rescigno, an
accurate two-electron H$_2$ calculation in these same coordinates.  \textbf{[MEASURED]}
An explicit-$r_{12}$ (James--Coolidge) extension of the algebraic engine,
in which every $r_{12}$ matrix element reduces to the same
$A\,K_0 - B\,K_1$ Neumann kernels as $V_{ee}$ (no quadrature), reaches
$\gvq{p12_r12_err_mha}{0.053}$~mHa above the exact value---a sixfold
reduction of the CI residual---confirming constructively that ordinary
integer powers of $r_{12}$, not non-analytic terms, close the gap
(Sec.~\ref{sec:r12}).  The same reduction extends without a resolution of
identity to a heteronuclear diatomic (HeH$^+$) and through the \emph{complete}
many-body matrix elements of a two-center four-electron explicit-$r_{12}$ CI:\
the genuinely four-body Coulomb bridge and the two-center three-body triangle
are both reduced RI-free---the latter through a low-rank expansion of its
non-separable two-point kernel contracted mode-by-mode with the general-$m$
prolate Neumann Coulomb---so the entire $2\times2$ CI energy assembles
analytically, matching a same-geminal variational Monte-Carlo reference to
sub-mHa and lying above the exact value (variational) (Sec.~\ref{sec:r12}).
\end{abstract}
...
molecule H$_2^+$, reaching machine precision (spectral Laguerre
...
$r_{12}$ dependence anywhere in it, carry out an accurate
two-electron H$_2$ calculation in these same coordinates.  (Their odd-$m$ radial functions do carry an explicit
...
\textbf{[MEASURED 2026-09-18]} \emph{Two condition numbers appear in this
section and they answer different questions;\ conflating them is a live
trap.} The raw $\mathrm{cond}$ figures above ($1.8\times10^{16}$,
$4.1\times10^{26}$) are the right metric for the $\|C\|^2$
congruence-corruption argument, which is about entry error amplified by
the change of basis. They are \emph{not} the metric the eigensolve sees:
that is the unit-normalized $\mathrm{cond}$, and the two can differ by
six orders on the same matrix. Measured at $(3,3)$, $|m|\le1$:\ raw
$3.0\times10^{15}$ against normalized $1.1\times10^{11}$. The practical
consequence is not academic---a raw comparison was used to conclude that
a two-exponent radial set carried a $47\times$ conditioning penalty,
whereas on the governing metric the same basis is $544\times$
\emph{better} conditioned than the single-exponent one, because two
distinct decay rates make the functions less linearly dependent rather
than more. Quote raw $\mathrm{cond}$ when arguing about a congruence and
normalized $\mathrm{cond}$ when arguing about a solve.  With that pipeline the energy climbs monotonically and
...
\textbf{[MEASURED 2026-09-18]} \emph{What the remaining residual is not.} Three
knobs of this basis have now been measured against the $|m|\le1$ ladder, one axis
at a time, and none of them reaches it:\ radial degree $+1$ buys $34\,\mu$Ha,
angular degree $+1$ buys $8\,\mu$Ha, and re-optimizing the single exponent buys
$0.056$~mHa---together about $9\%$ of that rung's $1.096$~mHa residual. A
\emph{two-block} exponent set (low degrees at $a_1$, high degrees at $a_2$, at
equal function count) does not reach it either:\ $+0.005$~mHa at $(4,4)$ and
$-0.009$~mHa at $(3,3)$, and the $(4,4)$ optimum is a \emph{plateau} rather than a
point---$1.107$, $1.108$, $1.110$~mHa across three quite different
$(a_1,a_2)$ pairs, flat to $3\,\mu$Ha---which is direct evidence that a second
exponent has no work to do at this basis size. That last result also disposes of
the natural reading of the drift in $\alpha_{\mathrm{opt}}$ with basis size
($\sim\!1.20$ at $(4,4)$, $1.40$ at $(5,5)$):\ the drift is real but it is not
relievable strain. We state this as the \emph{elimination} of three knobs, not as a
measurement of the cusp:\ what is established is that more polynomial degree and
more exponents do not close the gap in this family, which is weaker than---and
should not be quoted as---a demonstration that the remainder \emph{is} the
...
\textbf{[MEASURED]} What the residual $\sim\!0.4$~mHa is \emph{not} is
more azimuthal channels:\ $\varphi$ buys under $+0.06$~pp on top of
$\delta$.  What it \emph{is} remains unattributed, and per the
elimination statement above it must not be quoted as the
electron--electron cusp:\ radial completeness and the cusp are the two
standing candidates, both converging as the slow $L^{-3}$ partial-wave
crawl, and the knob elimination does not separate them.  A literal
$99.9\%$ therefore needs a much larger basis or explicit correlation
(geminals);\ what is established is that a \emph{radial} re-basing
within this family does not reach it.
...
\subsection{Explicit correlation, evaluated algebraically}
\label{sec:r12}

The knob-elimination above closes with an explicit alternative:\ a literal
$99.9\%$ ``needs a much larger basis or explicit correlation (geminals).''
The second route is available inside this paper's own machinery, and it
does not leave the algebraic (quadrature-free) discipline of
Sec.~\ref{sec:neumann}.  Multiply each product function by a power of the
interelectronic distance, $\Phi = g(\xi_1,\eta_1;\xi_2,\eta_2)\,r_{12}^{\,p}$
with $p\in\{0,1\}$ (a James--Coolidge basis in prolate coordinates), and
every resulting matrix element---overlap, $V_{ne}$, $V_{ee}$, and
kinetic---reduces to the \emph{same} algebraic objects already built for
$V_{ee}$.

\textbf{[MEASURED 2026-09-20]} The reduction is exact.  Writing
$r_{12}^2 = (R/2)^2(A - B\cos\Delta\varphi)$ with $A,B$ the standard
prolate combinations, every \emph{even} power of $r_{12}$ is a polynomial
in the separated coordinates (its azimuthal average is a finite sum of the
one-electron moments of Sec.~\ref{sec:neumann}), and every \emph{odd} power is
$r_{12}^{2s+1} = (R/2)^{2s+2}(A - B\cos\Delta\varphi)^{s+1}/r_{12}$, whose
azimuthal average is the same $A\,K_0 - B\,K_1$ combination of Neumann
kernels---the $m=0$ and $m=1$ channels of the general-$m$ $V_{ee}$ table
of Sec.~\ref{sec:neumann}---that the Coulomb term itself uses.  The one
element that looks harder, the kinetic coupling between a $p=0$ and a
$p=1$ function, collapses:\ integrating
$\tfrac12\langle\nabla g_u\!\cdot\!\nabla(g_v r_{12})\rangle$ by parts
cancels the gradient-of-$r_{12}$ term against half of itself and leaves
\begin{equation}
  T_{p_0\times p_1} \;=\; -\tfrac12\!\int r_{12}\,g_v\,(\nabla^2 g_u)\,dV,
  \label{eq:r12_kinetic_collapse}
\end{equation}
a pure odd-$r_{12}^{1}$ element (no $\nabla r_{12}$, no non-analytic term)
with $\nabla^2 g_u$ a polynomial numerator, routed through the same
kernels.  Each block was validated against direct quadrature to
$\sim\!10^{-4}$ (grid-limited).

\textbf{[MEASURED 2026-09-20]} On the $\sigma$ ($\mu=0$) gerade basis this
reaches $\gvq{p12_r12_de_pct}{99.97}\%$ of $D_e$
($\gvq{p12_r12_err_mha}{0.053}$~mHa) at $(j_{\max},l_{\max})=(3,4)$,
$n=416$, $\alpha = 1.0$---a sixfold reduction of the residual from the
re-based CI's $\gvq{p12_rebased_err_mha_aopt}{0.32}$~mHa, obtained with
exact algebraic integrals (no quadrature at any stage).  The energy
descends monotonically from above along nested truncations
($-20.7,\,-0.167,\,-0.079$~mHa at $n = 18,\,90,\,160$), and the extended
precision of Sec.~\ref{sec:recondition} is again load-bearing:\ by
$n\simeq 400$ the overlap conditioning reaches $\sim\!10^{19}$ and the
double-precision solve discards genuine directions
($16\,\mu$Ha above the extended-precision energy), while the
mpf-orthogonalized solve retains them.

\textbf{[MEASURED 2026-09-20]} This constructively settles what the
Sec.~\ref{sec:recondition} residual \emph{is}.  It is
electron--electron correlation, captured by ordinary integer powers of
$r_{12}$---not the non-analytic $r_{12}^{1/2}$, $r_{12}\ln r_{12}$ terms
this paper once invoked (Abstract, \textbf{[WITHDRAWN]}).  The
explicit-$r_{12}$ energy then \emph{plateaus} near $0.05$~mHa:\ at $p\le 1$
and a single $\alpha$ the overlap becomes linearly dependent
($\mathrm{cond}\sim\!10^{19}$ with essentially all directions retained),
i.e.\ that space is nearly spanned;\ a variational $\alpha$ scan gains only
$\sim\!2\,\mu$Ha (its optimum is $\alpha\approx 1.0\text{--}1.2$, and the
CI optimum $\alpha = \gvq{p12_rebased_alpha_opt}{1.40}$ is \emph{worse}
here, because explicit $r_{12}$ already supplies the correlation a
contracted exponent was compensating for).  The microhartree regime is
therefore not an $\alpha$ or radial-degree question but a higher-$r_{12}$-power
($p\ge 2$) one, which the same $A\,K_0 - B\,K_1$ reduction extends to
(with additional $m\ge 2$ channels).  We report the $p\le 1$ result and its
measured ceiling, not a microhartree claim.

\textbf{[MEASURED]} The construction is not specific to H$_2$.  Because the
$r_{12}$ kernels touch only the electron coordinates, the whole reduction is
charge-independent:\ the sole term that changes for a \emph{heteronuclear}
two-electron diatomic is $V_{ne}$, whose per-electron form
$(2/R)[(Z_A+Z_B)\xi + (Z_B-Z_A)\eta]/(\xi^2-\eta^2)$ stays polynomial after the
Jacobian (the $(Z_B-Z_A)\,\eta$ term breaks the $\eta\!\to\!-\eta$ symmetry, so
both angular parities enter and $V_{ne}$ couples them).  Applied to HeH$^+$
at the reference geometry $R_e = 1.463\,a_0$ the same engine converges
variationally to $\gvq{p12_r12_heh_err_mha}{0.9}$~mHa of the Kolos--Peek
reference~\cite{KolosPeek1976} (well depth $16456$~cm$^{-1}$, $E_{\rm BO}(R_e)
= -2.9787$~Ha) at a modest basis, and---consistently with the H$_2$ plateau
above---the lever there is the \emph{angular} basis, not the exponent (a
two-block exponent gains $\sim\!0.3$~mHa, one more angular shell $\sim\!11$~mHa).
Scanning $R$ locates the bond length as well:\ $R_{\rm eq}$ converges from
$-4.5\%$ at $l_{\max}=2$ to $-0.5\%$ at $l_{\max}=3$ ($1.456$ vs $1.463\,a_0$),
by the \emph{same} angular lever---so the engine reproduces the heteronuclear
\emph{geometry}, not only the energy, and the residual is heteronuclear
polarization (an angular effect).  This is the two-center, two-different-charge
analogue of the H$_2$ convergence, and the precedent for the frozen-core LiH
extension.

\textbf{[MEASURED 2026-09-21]} The reduction is also not specific to two
electrons, and this places the natural boundary of the quadrature-free,
\emph{resolution-of-identity--free} discipline.  For a James--Coolidge ansatz
$\Psi = F\,\Phi$ with $F=\sum_{p<q}f_{pq}$ multiplicative, the variational
kernel $\langle\Phi|F H F|\Phi\rangle$ stays at most three-body through
$N=3$:\ every term of the operator product closes with closed-form angular
rules (a shared-vertex rule fixing scalar legs to $L=0$, and the triangle rule
$\langle P_a(12)P_b(13)P_c(23)\rangle = \delta_{abc}/(2a+1)^2$), so the
three- and four-electron integrals that force a resolution of identity in
Gaussian $R_{12}/F_{12}$ theory are here evaluated \emph{exactly}, with no such
approximation.  At $N=4$ a genuinely four-body term first appears---the
disjoint-pair Coulomb chain $f_{12}\,f_{34}\,r_{13}^{-1}$, with the two
correlation edges bridged by one Coulomb, four distinct electrons (only the
two-body Coulomb can bridge disjoint pairs; the kinetic gradients are
pair-local and $V_{ne}$ is one-body).  This term is nonetheless still
\emph{exact}, still RI-free, and still \emph{reducible}:\ the bridging-Coulomb
multipole sum terminates, bounded by the finite orbital angular content and not
by the Coulomb:\ at $L\le 2\,l_{\rm bridge}$ for $s$-type leaves (measured
$s\!\to\!0,\,p\!\to\!2,\,d\!\to\!4$), and in general at
$L\le 2\,(l_{\rm bridge}+l_{\rm leaf})$---for $p$-type leaves the $L=4$ channel
that the $s$-leaf bound would discard is measurably nonzero (a $24\sigma$
Monte-Carlo detection, 2026-09-22), so the general bound must be used whenever
the leaves are not isotropic;\ the sum remains finite, so the RI-free property
is untouched---and the Legendre addition theorem factorizes the
chain across the bridge into a finite $(L,M)$ contraction of vertex kernels---the
chain analogue of the triangle contraction.  A native evaluation (deterministic
quadrature on the reduced form, not a closed formula) was validated against
brute-force $12$-dimensional Monte-Carlo integration to $2\times10^{-4}$ on the
Be-relevant four-electron integral ($2p$ bridge, $1s$ leaves), and, with the
antisymmetric determinant (exchange chains), at $L=0$ with all-$s$ leaves
(relative $9\times10^{-5}$ on a 2026-09-22 re-run;\ higher-$L$ exchange chains
have not been run).  As a proof of concept it was exercised inside a
minimal-basis Be $R_{12}$-CI, where a single short-range geminal captures
$\sim\!19\%$ of the correlation energy variationally with the four-body term
evaluated by this reduction.  The residual hard walls are on separate
axes---an explicit four-body correlation operator is a four-body Pauli string
in a qubit encoding, and the non-Hermitian transcorrelated route carries
vector legs that do not reduce this way---but the classical exactness axis, on
which Gaussian $F_{12}$ resorts to a resolution of identity, extends past $N=3$
rather than stopping there.  This is a diagnostic result at proof-of-concept
resolution, not a spectroscopic $N=4$ claim.

\textbf{[MEASURED 2026-09-21]} Two extensions sharpen this boundary and carry
the RI-free discipline from a standalone integral into a genuine many-electron
correlation calculation.  \emph{First}, the four-body bridge is not tied to the
single-center atom:\ evaluated between two-center prolate densities it reduces
by the \emph{same} construction---for isotropic ($1s$-type) leaves a
one-dimensional radial leaf dressing, in general the two-center Neumann
potential of the leaf density, followed by a two-center prolate Neumann Coulomb
between the dressed densities---with the self-Coulomb step anchored to the
closed form $5\alpha/8$.  The $\sigma$ channel reproduces an independent
twelve-dimensional Monte-Carlo evaluation to $1.5\times10^{-4}$ (the Monte-Carlo
scatter, not the reduction, sets this figure);\ the $\pi$ and $\delta$ channels
were validated to $\sim\!2\times10^{-5}$ on an azimuthally modulated model
density $\rho_{1s}(1+b_1\cos\phi+b_2\cos2\phi)$, which exercises the general-$m$
Neumann bridge but is not an $m\neq0$ orbital density;\ the assembled energies
below are $\sigma$-only.  \emph{Second}, the same machinery evaluates the many-body matrix
elements of an explicit-$r_{12}$ CI on a two-center four-electron reference
($\Phi_0=|1s_A^2\,1s_B^2|$, a proof-of-concept ionic reference, not a
spectroscopic LiH) with no resolution of identity, each element checked against
an independent variational Monte-Carlo ground truth.  The entire off-diagonal
coupling $h=h_T+h_{V_{ne}}+h_{V_{ee}}$ reduces to grid integrals of dressed
one-electron densities and matches the Monte-Carlo values to within their
statistical error:\ the kinetic part via an integration-by-parts identity that
collapses the gradient coupling $\nabla F\!\cdot\!\nabla\Phi_0$ to a
\emph{scalar} screened-Coulomb (Yukawa) expectation
($\nabla^2 f=\gamma^2 f-2\gamma\,e^{-\gamma r}/r$), the Coulomb part via the
two-center Neumann \emph{potential} of the transition density;\ the diagonal
one-body moment $\langle\chi^2 V_{ne}\rangle$ ($\chi=F-\bar F$) reduces by a
block decomposition $F=S+I$ (intra/inter) of the reference density with all
three ($\langle S^2\rangle$, $\langle SI\rangle$, $\langle I^2\rangle$) parts
Monte-Carlo--validated.  Crucially, the four-body bridge appears \emph{inside}
the diagonal $V_{ee}$ moment as
$\langle f_{12}f_{34}\,r_{13}^{-1}\rangle=\tfrac14\,I_{\rm coul}[S^f,S^f]$---the
Coulomb self-interaction of the $f$-dressed one-electron slice, via the same
Neumann potential---and again matches Monte-Carlo to within statistical error,
so the RI-free treatment of the genuinely four-body content is demonstrated not
merely as an isolated integral but inside a real correlation quantity.  (A
technical enabler:\ the screened-Coulomb dressing fields needed here are
obtained without a Yukawa-specific Green's function, as
$\Psi^Y=\Psi^{\rm coul}-\Psi^{\rm s}$ with the non-singular kernel
$s(d)=(1-e^{-\gamma d})/d$.)

\textbf{[MEASURED 2026-09-21]} The diagonal $\langle\chi^2 V_{ee}\rangle$ closes
the analytic energy.  Its one genuinely non-separable class is a two-center
\emph{three-body triangle} $\langle f_{i k}f_{j k}\,r_{ij}^{-1}\rangle$ (two
correlation edges sharing a vertex $k$, closed by a Coulomb on the opposite
edge), whose down-integration yields a non-separable two-point kernel
$H_2(1,2)=\int\mu(3)f(r_{13})f(r_{23})\,d^3r_3$.  It is reduced RI-free by
resolving $H_2$ into azimuthal Fourier components $H_2^{(m)}$, low-rank
expanding each ($H_2^{(m)}=\sum_r\lambda_r\,v_r\!\otimes\!v_r$), and contracting
mode-by-mode with the general-$m$ prolate Neumann Coulomb---the same $\pi/\delta$
machinery built above.  Both block placements (the Coulomb on the intra edge,
so the shared vertex carries the marginal density;\ or on an inter edge, so the
shared vertex carries an AO-pair component) reduce identically and reproduce
Monte-Carlo to $0.2$--$0.4\sigma$.  Summed over all $216$ interaction triples
with their correct multiplicities, the analytic $\langle F^2 V_{ee}\rangle$
matches Monte-Carlo to $\sim\!10^{-3}$ across all six intra/inter product
groups, giving $g_{V_{ee}}=\langle\chi^2 V_{ee}\rangle=+0.538$ ($+0.536$
Monte-Carlo).  The kinetic diagonal needs no vector $\nabla f$ dressing:\ the
same integration-by-parts identity collapses $g_{T2}+g_{T3}$ to
$-\gamma^2\sigma^2+2\gamma\,\mathrm{Cov}[F,\textstyle\sum_{i<j}e^{-\gamma r_{ij}}/r_{ij}]$,
leaving only a drift$^2$ moment (a grid-consistent block reduction over the
gradient density), so $g_T=+1.394$ ($+1.386$ Monte-Carlo).

Assembling the fully analytic, RI-free $2\times2$ then gives, for the geminal
$f=e^{-\gamma r}$, $E_{R12}=-7.942$~Ha---below the reference $E_0=-7.888$ and
above the exact $-8.070$ (variational), and matching the same-geminal
Monte-Carlo $2\times2$ to $0.8$~mHa.  The framework is geminal-agnostic---only
the kernels change---and the \emph{cusp-correct} geminal $f=r\,e^{-\gamma r}$
(whose $\nabla^2 f=\gamma^2 f-4\gamma e^{-\gamma r}+2e^{-\gamma r}/r$ adds one
term to the IBP-collapsed $g_T$) reproduces the headline proof-of-concept energy
analytically:\ every reduced element matches its Monte-Carlo target below $1\%$
(e.g.\ $\bar F=3.570$, $\sigma^2=0.128$, $h=-0.103$), giving an assembled
correlation lowering of $-29$~mHa (Monte-Carlo $-29.5$~mHa) and
$E_{R12}=-7.917$~Ha.  The one element that initially missed---the Yukawa
covariance $\mathrm{Cov}[F,\sum e^{-\gamma r}/r]$, a small difference amplified
by the long-range-geminal cancellation $\bar F=3.570$---is closed by evaluating
the isotropic screened-Coulomb dressings from the exact closed-radial Yukawa
potential rather than the Neumann-minus-smooth field, exactly the distinction
the Coulomb sector already draws between its closed Hartree potential and the
two-center Neumann potential;\ this carries the covariance from $-0.079$ to
$-0.103$ (Monte-Carlo $-0.102$), so the cusp-correct energy is reached fully
analytically and RI-free with no residual quadrature caveat.  The genuinely
four-body and three-body content thus lives, RI-free, \emph{inside} a
two-center four-electron correlation energy---exactly the boundary the classic
$F_{12}$ methods treat with a resolution of identity.  This remains a
proof-of-concept (ionic single-$\zeta$ reference, not a spectroscopic LiH):\
the demonstration is the RI-free reduction, not the correlation recovered.

\textbf{[AUDITED 2026-09-22]} Two clarifications fix the standing of these
reductions.  \emph{Tier.}  The bridge factorization is a \emph{validated
reduction}, not a theorem:\ an exact re-expression of one operator term through
three standard identities (the addition theorem or the Neumann expansion across
the bridge, and the radial convolution of each leaf), stated here and checked
numerically at the levels quoted, but never written out as a derivation, and
carrying no stated hypotheses under the name ``bridge addition theorem'' that
earlier sprint records attached to it.  Its extension beyond four electrons is
conjectural.  Note that at the linear-in-$F$ ansatz every term of
$\langle\Phi|FHF|\Phi\rangle$ carries at most three correlation edges, so no
term is ever more than four-body-connected for \emph{any} $N$;\ what a larger
system or a many-determinant reference changes is the \emph{density}---leaves
that are no longer isotropic, a reference that no longer factorizes into
closed-shell blocks---and at the time of the audit that general path had been
checked only as a Monte-Carlo--versus--Monte-Carlo factorization test at the
$5\%$ level (it has since been checked directly;\ see the build note below).
Every deterministic validation of the audit used the block-factorized
closed-shell reference $|\Phi_0|^2=D(1,2)\,D(3,4)$, which was the enabling
assumption of its spectator-integration bookkeeping.  \emph{Reference generality.}  Read at the
level of the code rather than the proof-of-concept, the reductions are not tied
to the two-orbital reference:\ the bridge, the triangle kernel, the two-center
Neumann potential, and the integration-by-parts identities each act on a single
separable product of one-electron pair densities supplied as an argument, and
the density of any four-electron CI is a finite sum of such products, so the
reference enters only through a coefficient tensor over pair indices.  The
many-determinant extension is therefore unbuilt bookkeeping and a $K^4$
contraction cost, not an obstruction---and running it on a multi-orbital
reference is precisely the test of the general, non-isotropic-leaf path, which
the build note below now reports.

\textbf{[BUILD IN PROGRESS 2026-09-22]}  The coefficient-tensor bookkeeping of
the preceding paragraph has since been constructed for a twelve-orbital,
$\sigma$-only CI reference (the core-enriched prolate CI of Paper~19, bond
$(J,L)=(2,1)$ plus three Li core exponents, $E=-8.02335$~Ha), rotated to its
natural orbitals and truncated at $|c|>10^{-3}$ to 74 determinants
($+0.44$~mHa above the full CI).  The pair-index tensor over the 78 unordered
orbital pairs has 38{,}786 nonzero entries, and its spectator traces reproduce
the one- and two-particle reduced density matrices of the truncated vector to
$2\times10^{-16}$, so the spectator-integration step that the closed-shell
validations took from $|\Phi_0|^2=D(1,2)\,D(3,4)$ is now exact for a general
CI.  The same construction exposed a quadrature defect that the closed-shell
reference had masked:\ the proof-of-concept engine's prolate--Neumann Coulomb
potential integrates the ordered kernel $P_l(\xi_<)Q_l(\xi_>)$ across its
diagonal kink with cumulative Gauss--Legendre sums, an $O(N_\xi^{-2})$ error
that is $+5\times10^{-3}$ relative on the Li $1s$ self-Coulomb and $+8$~mHa on
$\langle V_{ee}\rangle$ of the CI reference at the engine's 72 $\xi$-nodes
(the kinetic and nuclear-attraction grid integrals agree with the integral
engine to $10^{-7}$~Ha, and the $\zeta=4.5$ core density is resolved to
$10^{-11}$, so the defect is the kernel's quadrature, not the grid's
resolution of the density).  The closed-shell engine is only mildly exposed
because its isotropic pieces use a closed-form Hartree potential;\ the
general-density path is fully exposed.  The remedy is an exact
ordered-integral operator on the same grid:\ the $\eta$-moments of every
$\sigma$ density are entire in $\xi$ and are carried by the barycentric
interpolant through the 72 Gauss--Legendre nodes, so the two partial
integrals against $P_l^m(\xi')$ and $Q_l^m(\xi')$ can be taken exactly
relative to that interpolant (one Gauss rule for the polynomial $P$ side,
geometrically graded panels toward the logarithmic endpoint on the $Q$ side),
and the result is a precomputed $72\times72$ operator per $(l,m)$.  It
reproduces the $5\zeta/8$ self-Coulomb of an isotropic $1s$ density to
$2\times10^{-13}$ at $\zeta=1.0$, $1.6$, $2.6875$ and $4.5$ (the legacy sum
is $+1.7\times10^{-3}$ to $+8.5\times10^{-3}$ off), the two-center Hartree
closed forms to $7\times10^{-14}$, and solid-harmonic closed forms for
$m=0$--$4$ to $3\times10^{-13}$;\ with it, the grid kinetic, nuclear-attraction
and electron-repulsion energies of the 74-determinant reference agree with the
integral engine to $10^{-6}$~mHa, and the legacy path is bit-identical to the
committed engine (fast guard \texttt{tests/test\_lih\_r12ci\_neumann\_exact.py}).
On the corrected kernel the closed-shell proof-of-concept energies move by
$-0.4$~mHa (cusp-correct geminal, to $-7.9172$~Ha, whose correlation energy
$-29.4$~mHa now sits closer to the Monte-Carlo $-29.5$) and by $-1.6$~mHa
(exponential geminal, to $-7.9436$~Ha, now $2.4$~mHa below the same-geminal
Monte-Carlo $2\times2$ value of $-7.9412$):\ the $0.8$~mHa agreement quoted
above for the exponential geminal was in part a cancellation between this
Coulomb-kernel bias and the remaining coarse-grid errors of the $f$-kernel
dressings, which the engine's own source recorded at the time and which the
many-determinant build quantifies next.  The banked closed-shell numbers are
left on the legacy path, where the regression tests pin them.

\textbf{[VALIDATED 2026-09-23]}  The generality test of the audit paragraph
has now been run.  On the exact operator, the four-body bridge integral
$\langle\rho_1\rho_2\rho_3\rho_4\,f_{12}f_{34}/r_{13}\rangle$ was evaluated for
two \emph{real, non-isotropic} pair densities of the 74-determinant reference
against brute-force Monte-Carlo:\ a signed $p$-like core$\times$bond transition
density on the leaves (bridge $=$ the bond density) and a strongly directional
bond density on the leaves (bridge $=$ the core).  In both the reduction
reproduces the Monte-Carlo value---for the signed leaf a six-dimensional
route that isolates the Coulomb bridge reaches $\sigma_{\rm MC}/|I|=5\times
10^{-4}$ and gives $|I_{\rm reduction}-I_{\rm MC}|=1.1\times10^{-8}$ against a
tolerance $1.7\times10^{-8}$ (the twelve-dimensional estimator floors at
$1.5\times10^{-3}$ on the signed density and agrees at $0.3\sigma$);\ for the
directional leaf the unbiased twelve-dimensional integral confirms the
reduction to $1.8\times10^{-4}$.  Both dressed leaves carry Neumann channels
beyond the $2\,l_{\rm bridge}$ (isotropic-leaf) bound---an $l=4$ contribution
at the level of $10^{-3}$ of the integral---so the general termination bound
$2(l_{\rm bridge}+l_{\rm leaf})$ is the operative one, as the audit noted.  The
production $f$-kernel dressing itself contributes only $\sim\!10^{-6}$ relative
to these integrals, so it is adequate for sub-mHa energies without the
exact-integration treatment the Coulomb kernel required.  This upgrades the
four-body bridge reduction from validated-on-model-densities to
validated-on-real, non-isotropic densities under a non-block-factorized
reference:\ the reference-generality reading of the audit paragraph is
confirmed at the integral level.  The assembled many-determinant \emph{energy}
followed:\ all seven energy enumerators were generalized to contractions over
the coefficient tensor and run on a $74$-determinant reference, giving a
deterministic explicit-$r_{12}$ LiH energy of $-8.0372$~Ha that an independent
variational Monte-Carlo of the same trial function confirms to
$\lesssim0.3$~mHa (Paper~19, the 2026-09-23 marriage note).  Backing:\ fast guard
\texttt{tests/test\_lih\_r12ci\_bridge.py} (the reduction versus its frozen
Monte-Carlo value, with built-in fire tests that reject the isotropic-leaf
shortcut and the $L\le2$ truncation on the $p$-like leaf).

Backing:\ tests \texttt{tests/test\_paper12\_r12.py} (the H$_2$ engine) and
\texttt{tests/test\_lih\_r12ci.py} (the two-center LiH engine), and the reduction
of Eq.~\eqref{eq:r12_kinetic_collapse} above;\ the two-center bridge, the triangle
reduction, and the full analytic $2\times2$ energy are validated against
variational Monte-Carlo and chronicled in \texttt{CHANGELOG.md}
(v5.15.10--v5.15.15).  The proof-of-concept engine has been migrated from the
sprint tree into the deterministic, resolution-of-identity--free
\texttt{geovac.lih\_r12ci} package (an \texttt{energy(geminal)} API reproducing
both the $-7.942$ and $-7.917$ energies);\ it remains a proof-of-concept (ionic
single-$\zeta$ reference), not a production chemistry path.
...
\begin{enumerate}
  ...
  \item \textbf{Two-center, one-electron} (H$_2^+$):
    ...
    Accuracy: machine precision (spectral Laguerre)~\cite{loutey_paper11}.
  \item \textbf{Two-center, two-electron} (H$_2$):
    ...
    $\sigma$ sector, 99.1\% once the azimuthal channels are
    restored (Sec.~\ref{sec:azimuthal}), and $99.81$--$99.97\%$ after
    re-conditioning and the explicit-$r_{12}$ extension
    (Secs.~\ref{sec:recondition},~\ref{sec:r12}).  \emph{This paper.}
\end{enumerate}
...
different solver class, so at matched $m_{\max}$ neither claims an
advantage over the other.  Beyond that matched comparison, re-basing the
$|m|\le1$ span onto an orthogonal-polynomial family
(Sec.~\ref{sec:recondition}) takes this paper's H$_2$ to $99.81\%$ of
$D_e$, and the explicit-$r_{12}$ extension (Sec.~\ref{sec:r12}) to
$99.97\%$ ($0.053$~mHa)---which settles constructively that the residual
is electron correlation captured by ordinary integer powers of
$r_{12}$, not an inability of these coordinates to represent the cusp.  The defensible statement is the negative one:\
...
\begin{enumerate}
  ...
  \item \textbf{$\gvq{p12_rebased_de_pct_aopt}{99.81}\%$ of exact $D_e$}
    ...
    (Sec.~\ref{sec:recondition}).  What the residual is \emph{not} is more
    azimuthal channels;\ radial completeness and the $e$--$e$ cusp remain
    candidates that the knob elimination does not separate.
\end{enumerate}
...
reach chemical accuracy---$0.053$~mHa with the explicit-$r_{12}$
extension---and an accurate grid-based prolate spheroidal treatment in
the literature~\cite{tao_mccurdy_rescigno2010} independently shows these
coordinates are not cusp-limited.
...
\bibitem{KolosPeek1976}
W.~Ko\l{}os and J.~M. Peek,
``New \textit{ab initio} potential curve and quasibound states of HeH$^+$,''
Chem. Phys. \textbf{12}, 381--386 (1976).
