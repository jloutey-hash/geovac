...
different from both---and not a like-for-like comparison:\ that
$99.97\%$ is $l_{\max}=6$ with a full $m$-sum on a radial space roughly
two orders larger than the basis here, while
Ref.~\cite{tao_mccurdy_rescigno2010}'s own $l_{\max}=2$ point is
$-1.1732$~Ha ($1.22$~mHa, a larger error than is reached here), so the
gap is the angular order and basis size their radial representation
affords, not the coordinate system.  The defensible statement is the negative one:\
