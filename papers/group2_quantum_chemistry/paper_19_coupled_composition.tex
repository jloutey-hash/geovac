...
(Track~BQ, v2.0.32).\footnote{Sprint/track designations are anchors into the project's internal chronicle (\texttt{CHANGELOG.md} in the repository).}  Six attempts to modify PK have failed
...
for the $l\le 1$ orbitals) and to the radial quadrature
($n_{\rm grid}=2000$--$16000$).  Equivalently, at the true minimum the computed
electronic energy gradient is $9.0\%$ weaker than the
$+Z_A Z_B/R^2$ required to balance nuclear repulsion, a deficit that does not
move under angular or quadrature refinement---so the well-shape error is an
...
\item \textbf{F3-extend: $l>0$ cross-block h1 + non-axial geometry
  (a major structural extension).} Current F3 cross-block h1 is s-s only; mixed angular
...
whose magnitude depends on the Reading A vs Reading B verdict.} The three sub-sub-mechanisms
