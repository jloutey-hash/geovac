...
$R_{\rm eq} = +0.2\%$ (the corpus's best LiH geometry) and reaches a
near-chemical energy ($\approx -8.062$~Ha, an additive-F12 estimate), locating the
balanced drift specifically in the frozen/parameter-free core rather than in the
coupled architecture itself.
\end{abstract}
...
residual gap.  This last figure is an \emph{additive estimate, not a variational
bound}:\ the explicit-$r_{12}$ machinery is welded to a two-orbital reference, so
the fully rigorous variational number requires either an
F12-on-correlated-reference re-derivation or a VMC-over-FCI build (scoped,
deferred);\ a feasibility study confirms the estimate is defensible to a few mHa
and that the rigorous value lands near it, not overturning it
(\texttt{tests/test\_prolate\_route\_c\_lih.py};\ chronicled in
\texttt{CHANGELOG.md} v5.15.18--20).  The two enabling pieces---a sparse
connected-pair FCI and a float64 two-center ERI assembly, each validated
energy-exact against the high-precision reference---made the $M>16$ orbital ceiling
and the cusp study reachable.
