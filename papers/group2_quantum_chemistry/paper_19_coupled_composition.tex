...
This produces structurally sparse qubit Hamiltonians---a Pauli count
exactly linear in the qubit count across molecules at fixed basis,
$N_{\mathrm{Pauli}} = 27.90 \times Q$, with $54\times$--$317\times$
fewer terms than equal-qubit Gaussian baselines (Paper~14; the
retired pair-diagonal rule gave $O(Q^{2.5})$ and
51--1{,}712$\times$)---but PK limits accuracy to 5.3--19.4\%
equilibrium bond length error.  PK has resisted improvement:\ six
...
We observe that these missing integrals are closely related to the
...
balanced Hamiltonian has 2{,}726 Pauli terms (Conjecture~1 holds in
its ratio form only; see Sec.~\ref{sec:conjecture_verdict}).
...
confirming convergence through $n_{\max}=3$;\ at $n_{\max}=4$ the
model crosses \emph{past} exact (over-binding), so the fixed-geometry
error is not monotonically improvable either.  The $R_{\rm eq}$ drift is structural to the
block decomposition ($+0.053$ then $+0.023$~bohr at the two available steps---
decelerating, not a constant rate; $3\times$
smaller than PK's $+0.15$--$0.22$~bohr/$l_{\max}$), not numerical.  It is,
however, not a physics wall:\ a companion all-electron prolate-spheroidal
treatment with an $R$-accurate analytic atom-centered core resolves it to
$R_{\rm eq} = +0.2\%$ (the corpus's best LiH geometry) and, with a rigorous
VMC-over-FCI treatment (a two-body Jastrow on the analytic-orbital FCI state),
reaches a \emph{variational} energy of $-8.047$~Ha---$18$~mHa below the $-8.032$
orbital ceiling and safely above the exact $-8.0705$ (an additive-F12 estimate
places the near-chemical target at $\approx-8.062$~Ha, which a fuller three-body
Jastrow would approach)---locating the balanced drift specifically in the
frozen/parameter-free core rather than in the coupled architecture itself.
\end{abstract}
...
BeH$_2$ gives 11.7\%; H$_2$O gives 19.4\%.  These errors are
...
(Track~BQ, v2.0.32;\ sprint and track designations are anchors into the project's internal chronicle, \texttt{CHANGELOG.md} in the repository).  Six attempts to modify PK have failed
...
block-diagonal, yielding a Pauli count exactly linear in the qubit
count across molecules at fixed basis
($N_{\mathrm{Pauli}} = 27.90 \times Q$,
...
The resulting qubit Hamiltonian has 2{,}702 Pauli terms (3.22$\times$ the
composed value of 838) and non-identity 1-norm 80.5~Ha (2.10$\times$ the
composed value of 38.3~Ha).  However, the FCI energy tells the real story:
...
\item Cross-block ERIs are included, subject to Gaunt selection rules.
  ...
  ERI count (214 vs.~321 for LiH at $n_{\max} = 2$).
...
the coupled Hamiltonian should have $\lesssim 4.5\times$ the composed
count for LiH at $n_{\max} = 2$, remaining below the full Level~4N
count of 3{,}288 and potentially below the Gaussian STO-3G count of 907
after the PK one-body terms are removed.  (As originally framed under
the retired pair-diagonal rule this read $\lesssim 1{,}500$ against a
composed count of 334; the \emph{ratio} is the rule-independent
content, and the absolute figure followed from it.)
\end{conjecture}
...
$Q = 30$.  All values from Jordan--Wigner encoding, re-measured
2026-09-01 under the exact global-$M_L$ rule.  Pauli counts include
the identity term; 1-norms exclude it.  The Gaussian
STO-3G baseline ($Q = 12$) is included for reference.  Retired
pair-diagonal figures: $334/37.3/21$, $878/74.1/87$, $854/85.7/88$.}
\label{tab:resources}
\begin{ruledtabular}
\begin{tabular}{lddd}
...
The balanced Hamiltonian has 2{,}726 Pauli terms, confirming the
ratio form of Conjecture~\ref{conj:sparsity} ($3.25\times$ the composed
838, under the conjectured $4.5\times$) but not its absolute form:
2{,}726 exceeds both the $\sim 1{,}500$ the ratio implied at the
retired composed count and the STO-3G count of 907.  The 1-norm of the balanced
Hamiltonian (75.2~Ha) is \emph{lower} than the coupled CB value
(80.5~Ha) because the attractive cross-center $V_{ne}$ partially
cancels the repulsive cross-block $V_{ee}$.  The QWC group count
(629; 87 under the retired pair-diagonal rule) no longer sits below
the Gaussian baseline (273).
...
Hamiltonian has 127{,}855 Pauli terms and a 436.9~Ha 1-norm
...
\hline
Grid       & 2  & 30 & 2726   & 75.2  & -7.924 & 1.8\%  \\
Analytical & 2  & 30 & 2726   & 75.2  & -7.929 & 1.7\%  \\
Grid       & 3  & 84 & 127855 & 436.9 & -8.048 & 0.28\% \\
Analytical & 3  & 84 & 127855 & 436.9 & -8.055 & 0.20\% \\
\end{tabular}
\end{ruledtabular}
\end{table}
...
\subsection{Fixed-geometry energy versus well shape}

The convergent energy and the persistent outward $R_{\rm eq}$ drift are two
independent read-outs of the same residual: the energy error is its magnitude
at a fixed geometry, whereas $R_{\rm eq}$ is fixed by its \emph{slope} at the
true minimum.  The well \emph{curvature} is a third, independent read-out.  A
harmonic fit to the $n_{\max}=2$ balanced PES about its minimum gives
$\omega_e \approx 2040$~cm$^{-1}$ for LiH, some $45\%$ above the experimental
$1406$~cm$^{-1}$ (force constant $0.14$ vs.\ $0.066$~Ha/bohr$^2$).  The balanced
construction---the more accurate of the two architectures on \emph{energy}
($0.20\%$ at $n_{\max}=3$)---therefore misplaces both the position \emph{and}
the curvature of the minimum: its residual is concentrated in the \emph{shape}
of the potential well, not in the fixed-geometry energy (the well depth $D_e$
itself is a separate, distinct read-out and is not extracted here).  This
shape residual is localized to the one-particle orbital basis: the tilt
$\varepsilon'$ and curvature (both evaluated at the true minimum) are
\emph{bit-invariant} to the cross-center $V_{\rm ne}$ angular multipole order
(identical across $L=2$--$8$, the multipole series Gaunt-terminating at $L=2$
for the $l\le 1$ orbitals) and---separately---insensitive to the
radial quadrature.  We state that second leg precisely, because the two
quadrature knobs are not alike:\ the cross-center $V_{\rm ne}$ is evaluated
analytically (incomplete gamma), so it carries no quadrature to converge at
all;\ the one live radial quadrature is the cross-block electron-repulsion
trapezoid, and refining it moves the tilt by $0.15\%$ over $n_{\rm grid}=2000$--$8000$
and $0.87\%$ over $1000$--$8000$.  Equivalently, at the true minimum the computed
electronic energy gradient is $8.8\%$ weaker than the
$+Z_A Z_B/R^2$ required to balance nuclear repulsion---a deficit an order of
magnitude larger than the largest quadrature movement, so the well-shape error is an
orbital-basis-completeness effect, not an integral-evaluation or angular-%
truncation effect (LiH $n_{\max}=2$; a measured result).

\emph{Term-resolved, this deficit has a single home.}  A Hellmann--Feynman
force decomposition---freezing each operator's $R$-dependence at the minimum and
reading off $F_T = \varepsilon'_{\rm full} - \varepsilon'_{\rm frozen\,T}
= \langle\psi|\,dT/dR\,|\psi\rangle$---shows that of the entire balanced
Hamiltonian only two operators carry any $R$-dependence: the nuclear repulsion
$V_{NN} = Z_A Z_B / R$ and the cross-center $V_{\rm ne}$.  The within-block
one-body term, the within-block electron repulsion, and the cross-block electron
repulsion are \emph{bit-exactly} $R$-independent (each $F_T$ numerically zero,
$\lesssim 10^{-13}$~Ha/bohr).  The LiH well is therefore a strict two-term
balance---$V_{NN}$ pushing the nuclei apart ($F = -0.331$~Ha/bohr) against the
cross-center $V_{\rm ne}$ drawing them together ($F = +0.302$~Ha/bohr), summing
to the residual outward tilt $-0.029$~Ha/bohr.  The $8.8\%$ deficit is thus
specifically the $R$-slope of the cross-center $V_{\rm ne}$: its \emph{magnitude}
at any fixed geometry is basis-accurate (hence the convergent energy), while its
\emph{slope}---which encodes the bonding density contracting toward the bond as
the nuclei approach---is under-delivered, because the fixed, unpolarized
hydrogenic bond orbital ($Z_{\rm orb} = 1$) carries no such $R$-adaptive
relaxation (LiH $n_{\max} = 2$; a measured result).

\emph{The mechanism, demonstrated.}  The required inward force $Z_A Z_B/R^2$ is
exactly the fully-contracted (point-charge) limit of the cross-attraction
$R$-slope.  For a $1s$ orbital of exponent $z$ attracted to the partner nucleus
of charge $Z$ at separation $R$,
$\langle\phi_z|{-}Z/r|\phi_z\rangle = -(Z/R)\,[\,1-(1+zR)\,e^{-2zR}\,]$, whose
slope $dV/dR \to +Z/R^2$ as $z\to\infty$: a point density exerts precisely the
classical Coulomb force that cancels $dV_{NN}/dR$.  The finite-extent factor
$[\,1-(1+zR)e^{-2zR}\,] < 1$ softens that slope, and at the fixed
$Z_{\rm orb}=1$ value it delivers only $93.9\%$ of the limit ($+0.310$ vs
$+0.330$~Ha/bohr at $R=3.015$, $Z=Z_{\rm Li}$)---a $6.1\%$ deficit, of the same
sign and comparable magnitude to the measured $8.8\%$ (the remainder being the
more diffuse $2s/2p$ admixture and the weaker Li$\to$H cross term).  The deficit
is thus the bond orbital's \emph{finite extent}, and full contraction would
recover the exact force balance.  Crucially, that contraction \emph{cannot be
expressed} within the parameter-free construction: its one-body term is the
hydrogenic baseline $-Z_{\rm orb}^2/(2n^2)$, so $Z_{\rm orb}$ is tied to the
nuclear charge; forcing $Z_{\rm orb}>1$ on the $Z=1$ H centre deepens $h_1$
unphysically and drives the energy below the exact floor ($-9.6$ vs $-8.07$~Ha at
$Z_{\rm orb}=2.3$).  A genuine contracting bond orbital requires $h_1$ from
physical kinetic-plus-nuclear integrals---i.e.\ a variational
(non-parameter-free) two-centre treatment, the prolate-spheroidal route of
Paper~12 (LiH $n_{\max}=2$; a measured closed-form model).  This
mirrors the composed/PK result (Paper~17), where the \emph{parameter-free}
pseudopotential barrier reproduces $\omega_e$ to $4.6\%$ but caps the attainable
energy accuracy.  For LiH the two architectures thus trade energy accuracy
against curvature accuracy, and neither reproduces the full well; universality
of this trade across hydrides is untested (a measured result; the curvature
quoted here is the $n_{\max}=2$ fine-grid value for LiH).  For single-geometry applications
(fixed-nucleus qubit Hamiltonians) only the energy read-out enters, which is why
the balanced construction remains well matched to that use despite its
well-shape errors.

\emph{The prolate-spheroidal fix, tested---and further localized to the core.}
Building that variational two-centre treatment for LiH---a frozen Li~$1s^2$ core
(closed-form Hartree screening plus a Huzinaga core-orthogonality level-shift,
$\lambda$-plateau-verified) with an explicitly-correlated ($r_{12}$)
prolate-spheroidal valence CI, the same engine that reproduces the HeH$^+$
geometry---narrows the drift but does \emph{not} close it:\ $R_{\rm eq}$ is
$+3.4\%$ at $l_{\max}=2$ and $+5.5\%$ at $l_{\max}=3$ (still outward, still
\emph{growing} with the angular basis), against this balanced construction's
$+8.8\%$ (the composed $l$-dependent-PK recipe of Paper~17 reaches $5.3\%$ at
$l_{\max}=2$;\ the frozen-core prolate PoC is a mechanism diagnostic, not a bid
to better that number).
The control is decisive:\ the identical $r_{12}$-CI applied to HeH$^+$---two
active electrons, \emph{no} frozen core---instead \emph{converges} its geometry,
$-4.5\%\to-0.5\%$ across the same $l_{\max}=2\to3$.  The one structural
difference between the drifting and converging cases is the frozen core, so the
finite-extent rigidity of the mechanism above lives in the \emph{core}'s static
screening, not only in the valence bond orbital;\ a variational \emph{valence}
contraction (which the prolate CI supplies) and the valence--valence correlation
lever $r_{12}$ are both insufficient---$r_{12}$ absorbs part of the angular
over-polarization (the $l_{\max}=3$ drift falls $+7.1\%\to+5.5\%$ when it is
switched on) but cannot reach a core degree of freedom.  Full closure therefore
requires a variational \emph{core}---an all-electron ($N=4$) two-centre
CI---which meets the four-body explicit-$r_{12}$ wall for full correlation
(a measured result;\ frozen-core prolate LiH at $\lambda=1000$, $l_{\rm neu}=20$,
$R$-grid minimum;\ rests on the Paper~12 HeH$^+$ prolate convergence as control).

\emph{The all-electron variational-core test brackets $R_{\rm eq}$ from the
opposite side.}  A determinant CI with \emph{all four} electrons active and
R-adaptive (a multi-exponent prolate FCI; $V_{ee}$ pairwise, so it avoids the
four-body $r_{12}$ wall) removes the frozen-core rigidity---and its $R_{\rm eq}$
error changes \emph{sign}, collapsing \emph{inward} rather than drifting outward.
The two errors thus bracket the true value ($\text{inward}\leftarrow 3.015\to
+5.5\%$ frozen $\to +22\%$ over-corrected), and a variational-core all-electron
treatment is the mechanism that removes the outward drift---consistent with the
fact that standard all-electron quantum chemistry reproduces LiH's bond length.
The inward collapse is itself a \emph{numerical} artifact, not physics:\ an
\emph{isolated} Li$^{2+}$ $1s^2$ core (whose energy must be $R$-independent)
acquires a spurious $0.36$~Ha $R$-dependence in the two-center prolate grid,
because the real-space resolution near the tight core is tied to $R$ through
$r_A=(R/2)(\xi+\eta)$;\ confirmed a grid artifact (the spread shrinks under
refinement), with a co-dominant contribution from the $R$-dependent completeness
of the two-center basis for a tight atomic core.  A clean prolate all-electron
$R_{\rm eq}$ is thus a well-characterized \emph{numerical} target (a dedicated
atom-centered core), not a physics obstruction (a measured result;\ rests on the
Paper~12 prolate-CI engine).

\emph{That dedicated atom-centered core has now been built---and it closes the
geometry.}  The cure (``Route~C'') replaces the $R$-tied grid core with an
\emph{analytic} atom-centered tight core---every core-involving integral in closed
form, $R$-accurate to ${\sim}10^{-50}$ on the isolated Li$^{2+}$ $1s^2$ core, so
the $0.36$~Ha grid artifact vanishes---with all four electrons active in the
prolate natural geometry.  With the angular ($\pi$) polarization lever (the same
lever decisive for HeH$^+$), the equilibrium bond length converges to experiment:\
$R_{\rm eq} = +0.2\%$ ($\pi$-converged, $1\pi \equiv 2\pi$ shells), the best LiH
geometry in the GeoVac corpus and a decisive improvement over both this balanced
construction's $+8.8\%$ and the composed $l$-dependent-PK $5.3\%$ (Paper~17).  The
outward drift of the mechanism above is thus \emph{not} a physics wall:\ once the
core is variational \emph{and} numerically clean, the geometry is recovered
(\texttt{tests/test\_prolate\_allelectron\_fci.py};\ chronicled in
\texttt{CHANGELOG.md} v5.15.16;\ rests on the Paper~12 prolate-CI engine and its
HeH$^+$ convergence control).

\emph{The energy is near-chemical, by estimate.}  The pure-orbital prolate
all-electron energy plateaus at $-8.032$~Ha ($38$~mHa above the exact $-8.0705$)
once the angular and radial ladders saturate---a determinant-limited orbital
ceiling, already the corpus's best LiH energy---because a finite orbital basis
cannot represent the electron--electron cusp.  Adding that cusp through an
additive-F12 correction (the $r_{12}$ energy an isolated He-like pair gains beyond
a radial-converged orbital pair:\ $30.1$~mHa for the Li core, validated against
exact He to $1.4$~mHa) brings the total to $\approx-8.062$~Ha ($\approx9$~mHa above
exact, near-chemical), the smaller valence cusp being budget-consistent with the
residual gap.  This $-8.062$ figure is an \emph{additive estimate, not a
variational bound}.  The fully rigorous number has now been obtained by a
\emph{VMC-over-FCI} build (\texttt{debug/lih\_vmc.py}):\ the analytic-orbital FCI
ground state $\Psi_{\rm CI}$ (the orbital-ceiling wavefunction, evaluated in real
space from the analytic prolate orbitals and the CI eigenvector) multiplied by a
two-body Pad\'e Jastrow $J=\exp\sum_{i<j}u(r_{ij})$, $u=A_{\rm spin}\,
r/(1+\gamma r)$ (exact electron--electron cusp, hence a finite-variance local
energy), sampled by Metropolis with the standard $E_L=V-\tfrac12\nabla^2\Psi/\Psi$
estimator.  The build is validated by the mandatory gate---VMC of $\Psi_{\rm CI}$
at $J=1$ reproduces the FCI energy to within statistics ($-8.0285\pm0.005$ vs
$-8.0290$ for the $M=16$ basis).  The resulting rigorous variational energy is
$\mathbf{-8.047\pm0.001}$~Ha ($\gamma\simeq1.5$, four independent seeds):\ $18$~mHa
below the orbital ceiling and safely variational ($>-8.0705$).  A two-body Jastrow
recovers $\approx19$~mHa of the $41$~mHa ceiling-to-exact gap; the remaining
correlation---including the electron--electron--nucleus (three-body) content that
the additive-F12 rich Hylleraas basis ($\{u,u^2,ut^2,t^2,s\}$) captured but a pure
$u(r_{12})$ cannot---requires a nucleus-coupled three-body correlation term:\
either a three-body Jastrow in the Monte-Carlo route, or, on the analytic side,
an $r_{12}\,t^2$-type geminal evaluated by the same
resolution-of-identity--free reductions of Paper~12, whose reference generality
(they act on arbitrary pair-density products, so a many-determinant reference
enters only through a coefficient tensor) was established by audit on
2026-09-22 and demonstrated at the integral level on 2026-09-23:\ the
four-body bridge reduction reproduces brute-force Monte-Carlo on the real,
non-isotropic pair densities of a 74-determinant reference (Paper~12, the
2026-09-23 build note).  The additive $-8.062$ estimate is thus a target a fuller trial
function would approach, consistent with (not overturned by) the two-body
variational upper bound $-8.047$;\ a two-body geminal on the analytic route is
expected to land near the same value, supplying a deterministic cross-check of
it rather than a lower number.  Two enabling
pieces---a sparse connected-pair FCI and a float64 two-center ERI assembly, each
validated energy-exact against the high-precision reference---made the $M>16$
orbital ceiling, the cusp study, and the real-space VMC reachable (chronicled in
\texttt{CHANGELOG.md} v5.15.18--22).

\textbf{[MARRIAGE, 2026-09-23]}  The deterministic analytic route promised above
has now been assembled and run.  The resolution-of-identity--free multibody
reductions of Paper~12 were generalized from the two-orbital reference to a
many-determinant one---every enumerator ($\bar F$, $\sigma^2$, $h$ and $g$ for
kinetic, nuclear and electron--electron terms, and the reference energy itself)
rewritten as a contraction over a coefficient tensor on the orbital-pair
indices---and applied to a $74$-determinant, $\sigma$-only core-enriched CI
reference (natural-orbital truncation of the $E=-8.02335$~Ha CI, reference
energy $E_{\rm trunc}=-8.02292$~Ha).  A regression gate confirmed that feeding
the two-orbital reference back through the general enumerators reproduces the
banked $-7.917$/$-7.943$ per piece to $<0.1$~mHa.  On the many-determinant
reference the analytic explicit-$r_{12}$ energy is
$E_{R_{12}}=\mathbf{-8.0372}$~Ha for both geminals (linexp $-8.03724$,
exponential $-8.03709$, agreeing to $0.15$~mHa), variational and $\sim\!5$~mHa
below the $M=16$ orbital ceiling.  It is cross-validated by an independent
variational Monte-Carlo of the \emph{same} trial function
$(1+c\,(F-\bar F))\Psi_{\rm CI}$ (standard local energy, analytic Jastrow
Laplacian):\ $E_{\rm VMC}=-8.03755\pm0.00041$ (linexp) and
$-8.03704\pm0.00003$ (exponential), agreeing with the analytic values to
$0.31$ and $0.05$~mHa.  This is the first deterministic, resolution-of-identity--free
LiH energy from the many-determinant explicit-$r_{12}$ engine, and it confirms
the reference-generality of the reductions at the level of a real,
independently-checked energy.  It is \emph{not} the corpus-best LiH number:\ the
$-8.037$ sits above the VMC-over-FCI $-8.047$ because the $\sigma$-only reference
carries no $\pi$ orbitals and tops out $\sim\!9$~mHa above the $M=16$ ceiling---the
two-body correlation the marriage recovers ($\approx14$~mHa) matches the
two-body Pad\'e Jastrow ($\approx15$~mHa), so the shortfall is the reference, not
the correlation treatment.  Reaching the $-8.047$ window is the same construction
on a $\pi$-containing $M=16$ reference (a larger, azimuthally-resolved
computation);\ reaching the $-8.062$ additive-F12 estimate additionally needs the
nucleus-coupled $r_{12}t^2$ three-body geminal, which the same reductions are
built to carry.  Backing:\ \texttt{tests/test\_lih\_r12ci\_bridge.py} (the
four-body reduction on real densities) and \texttt{tests/test\_lih\_vmc.py} (the
VMC stack);\ chronicle \texttt{debug/sprint\_lih\_marriage\_memo.md}.

\textbf{[MEASURED]} The banked $n_{\max}=4$ decider has now been run---a
matrix-free (Davidson) direct-CI engine
(\texttt{geovac/balanced\_direct\_ci.py};\ ${\sim}1600\times$ faster than the
assembled-matrix path at $n_{\max}=3$, validated to $\le 7\times10^{-15}$~Ha
against a brute-force Fock-space FCI and to $1.8\times10^{-15}$~Ha against the
live library at $n_{\max}=2$) solved the $16{,}040{,}025$-determinant
$n_{\max}=4$ sector at six $R$ points.  The verdict is decisive for the
\emph{irreducible-wall} reading:\ the \emph{per-shell increments} of energy,
curvature and tilt at the true minimum all shrink geometrically at a common
ratio ($\approx 0.36$--$0.38$)---the basis is converging---but every
observable, the energy included, converges to a \emph{wrong} limit.
$E_{\rm tot}(R_{\rm true})$ runs $-7.9297\to-8.0548\to-8.1020$~Ha at
$n_{\max}=2/3/4$ (shipped convention, frozen-core offset $-7.2799$~Ha;\ the
corrected sign shifts each value by $\approx+4$~mHa), \emph{crossing} the exact
$-8.0705$ between $n_{\max}=3$ and $4$ and extrapolating to $\approx-8.13$:\ the
balanced model over-binds
past exact, so its excellent energy convergence is an approach-from-above
statement valid through $n_{\max}=3$ only.  The curvature ratio at the true minimum runs
$2.245\times\to2.219\times\to2.210\times$ the true force constant at
$n_{\max}=2/3/4$, extrapolating to $2.204\times$ ($99.5\%$ of the gap
surviving, robust across geometric, power-law and deliberately-slow $1/n$
extrapolation models);\ the tilt worsens toward $-0.0399$;\ $R_{\rm eq}$
extrapolates to $+10.1\%$ and worsening;\ $\omega_e$ at the well's own minimum
falls $2040\to1946\to1901$~cm$^{-1}$ ($+45.1\to+38.4\to+35.3\%$, extrapolating
to $\approx+32\%$---the earlier ``stays $\sim+45\%$'' reading was an
$n_{\max}=2$-only fit), still far from $1406$.  Model-free statement:\ closing
the curvature gap at the observed per-shell step would need over a hundred
further shells with no decay in step size.  The well-shape residual is
therefore an \emph{irreducible} property of the balanced architecture's
orbital-basis limit, not slow basis healing
(\texttt{tests/test\_balanced\_direct\_ci.py}).

\emph{Reproducibility note.}  In the course of building the engine, a global
sign error was found (and fixed) in the shipped assembled-matrix path's
same-spin double-excitation phase:\ it contributed a nearly
$n_{\max}$-independent spurious over-binding of ${\sim}+4$~mHa on balanced
LiH while leaving the geometry observables essentially unchanged
($<0.2$~pp).  \emph{Scope of the correction, stated precisely because the two conventions
are easy to conflate.}\ The energies tabulated in this paper are on the
\emph{faithful} (pre-fix) convention:\ at the fixed geometry $R=3.015$ the
banked values are $-7.9297$, $-8.0548$, $-8.1020$~Ha at $n_{\max}=2,3,4$,
while the corrected phase gives $-7.9256$, $-8.0509$, $-8.0982$~Ha---a shift
of $+4.13$, $+3.85$, $+3.79$~mHa.  So the $n_{\max}=3$ energy error reads
$0.20\%$ on the faithful convention---the figure quoted throughout this paper---
and $0.24\%$ on the corrected one (both against the exact $-8.0705$~Ha).  The
earlier statement that ``no number in this paper changes'' was \emph{wrong}:\ it
compared the corrected energy at the well's own minimum ($R_{\rm eq}=3.2776$,
a different offset) against the faithful energy at $R=3.015$, and the two
agreed only by cancellation.  What genuinely does not move is the geometry:\
$R_{\rm eq}$ shifts by $\le0.13$~pp and the qualitative conclusions---binding,
the stiffness excess, and the $n_{\max}=4$ crossing---are unchanged under
either convention.  The corrected phase is pinned element-by-element against
a brute-force Fock-space FCI in the same test file.

\subsection{Summary of conjecture status}
\label{sec:conjecture_verdict}

Conjecture~\ref{conj:sparsity} (Pauli count): \textbf{partially confirmed}.
The balanced Hamiltonian has 2{,}726 Pauli terms.  The conjecture's
rule-independent content --- $\lesssim 4.5\times$ the composed count
--- holds at $3.25\times$.  Its absolute legs do not: 2{,}726 exceeds
the ${\sim}1{,}500$ that ratio implied at the retired composed count
of 334, and exceeds the STO-3G count of 907.  It stays below the
Level~4N count of 3{,}288, but by 17\% rather than the $3.7\times$
the retired figures showed.
...
$n_{\max} = 3$.  The drift is $+0.053$~bohr from $n_{\max}=2$ to 3 and $+0.023$~bohr
from 3 to 4---decelerating, consistent with the extrapolated limit below
($3\times$ smaller than PK's $+0.15$--$0.22$~bohr/$l_{\max}$) but in
the same outward direction.  Energy converges excellently through
$n_{\max}=3$: $1.8\% \to 0.20\%$ ($9\times$ improvement) from $n_{\max} = 2$
to 3---and then crosses \emph{past} exact at $n_{\max}=4$ (over-binding;\ the
well-shape subsection), so the fixed-geometry error is not monotonically
improvable either.
...
geometry determination---indicated at $n_{\max}=4$ (a three-point,
model-robust extrapolation;\ and, model-free, closing the curvature gap would
need well over a hundred further shells at an undecaying step) to be an
irreducible limit of the architecture, not slow basis healing (the well-shape
subsection above).
...
\item \textbf{Balanced coupled Hamiltonian.}  \emph{COMPLETE.}
  ...
  interaction types) gives 2{,}726 Pauli terms, 75.2~Ha non-identity
  1-norm, and 629 QWC groups (Table~\ref{tab:resources}).
...
\item \textbf{Scaling study.}  \emph{COMPLETE.}  $n_{\max} = 3$
  ...
  Pauli scaling exponent 3.74, 1-norm exponent 1.71.
  $R_{\rm eq} = 3.280$~bohr (8.8\% error), confirming structural drift
  of $+0.053$ then $+0.023$~bohr per step (decelerating, not a constant rate; $3\times$ smaller than PK's
  $+0.15$--$0.22$~bohr/$l_{\max}$).  Grid error contributes only
  0.007~bohr to $R_{\rm eq}$ (negligible).  Classification: MIXED---energy
  converges excellently through $n_{\max}=3$ (then crosses past exact;\ the
  well-shape subsection), $R_{\rm eq}$ drifts structurally.
...
BeH$_2$ & 3 & 50 & 1,396 & 8,868 & 6.35$\times$ & 374.9 & 323.4 \\
H$_2$O & 5 & 70 & 1,954 & 19,742 & 10.10$\times$ & 372.5$^\dagger$ / 28,061.9 & 1,612.0 \\
\hline
\end{tabular}
\caption{Balanced coupled resource census, re-measured 2026-09-01
under the exact global-$M_L$ rule with each molecule at its own
equilibrium geometry.  Pauli counts include the identity term and
1-norms are identity-inclusive (the composed-with-PK total is only
available that way).  $^\dagger$H$_2$O composed electronic-only (PK
partitioned).  Retired pair-diagonal figures: LiH $334/878/37.3/74.1$,
BeH$_2$ $556/2{,}652/354.9/304.7$, H$_2$O $778/5{,}798/361/28{,}053/1{,}509.3$.}
...
decoupled.  The balanced 1-norm (323.4~Ha) is \emph{lower} than the
composed 1-norm (374.9~Ha), confirming that PK adds more 1-norm than
...
$3.25\times$ (2-block LiH) $\to$ $6.35\times$ (3-block BeH$_2$)
...
is an upper bound rather than the measured trend.
...
projection;\ Paper~17 gives the same-center case. The implementation
...
compat preserved (LiH 2{,}726-Pauli regression bit-identical at machine
...
options remain:
\begin{enumerate}
  \item \textbf{Physical-$n$ hydrogenic relabeling (sprint-scale)}:
  ...
  \item \textbf{Strict-Schmidt orthogonalization (beyond sprint scale)}:
...
\begin{enumerate}
  \item \textbf{NaH $n_{\max}=3$ test} (sprint-scale, highest priority).
  ...
  \item \textbf{Cross-$V_{ne}$ kernel-shape substitution}
    (sprint-scale, parallel).  Track~3's diagnostic (2026-05-09)
  ...
  \item \textbf{W1c + multi-zeta combination by removing
    NotImplementedError} (bundled with item~1).
...
production extension: five matrix elements computed via converged
...
\item \textbf{F3-extend: $l>0$ cross-block h1 + non-axial geometry
  (beyond sprint scale).} Current F3 cross-block h1 is s-s only; mixed angular
...
Three diagnostic sprints in immediate succession (F4, F5, F6) tested all three
...
closes W1e on its own;\ individually they account for at most
${\sim}10\%$ and ${\sim}26\%$ of the wall and jointly for ${\sim}36\%$, leaving the
${\sim}65$--$90\%$ itemized below---the spread reflecting whether one or both
closable mechanisms are counted---as
genuinely-deep correlation requiring architectural lifts beyond
sprint scale.}
...
\item \textbf{W1e-deep-correlation-residual:} ${\sim}65$--$90\%$ of
  ...
  orthogonalization at the basis-construction level (beyond sprint
  scale), explicit treatment of [Ne] core as fully correlated
  (a major structural extension: requires occupancy-restriction FCI
  infrastructure not present in framework), or acceptance that the framework's
...
The two named beyond-sprint-scale candidates from \S\ref{sec:w1e_refinement_f4_f6}
...
sprint scale are major architectural commitments
...
framework (CLAUDE.md \S1.7; Paper~34's projection catalogue). The five previous
...
\emph{closer at hand} than UV-completion physics --- substantial
architectural commitments beyond sprint scale rather than a fundamental
UV completion outside the framework.

\textbf{Closure-prospect classification: (a-with-caveat)
sprint-scale exhausted but tractable for components as a
substantial open program, with a calibration-external residual
whose magnitude depends on the Reading A vs Reading B verdict (since
resolved: Reading B;\ Sprint B.1 note below).} The three sub-sub-mechanisms
...
whether implementation beyond sprint scale pushes this to $\sim 85$--$95\%$
...
\textbf{Explicit-core 12-electron HF on NaH is the
...
(W1e is active-space deficit; a well-defined CCSD-class post-HF closure
path exists beyond sprint scale). If HF still gives monotone-descending PES
$\rightarrow$ \textbf{Reading B confirmed}.  \emph{(Since run:\ Sprint B.1,
2026-06-07---the 12-electron explicit-core RHF still descends monotonically;\
Reading B is the confirmed outcome.)} (W1e is structurally tied
...
1-norm 32.6~Ha).  The sparsity advantage in Pauli count may be
...
rules, the composed builder's structural sparsity retained (a Pauli
count exactly linear in $Q$ across molecules at fixed basis,
Paper~14), but without PK and with significantly better accuracy.  Paper~17's composed architecture
...
is excellent through $n_{\max}=3$: 1.8\% $\to$ 0.20\% ($9\times$ improvement) as the
basis grows from $Q = 30$ to $Q = 84$, with the variational bound
respected at these basis sizes (at $n_{\max}=4$ the model crosses past
exact;\ the well-shape subsection).  This rules out the PK-like divergence scenario (where
...
\bibitem{hoggan2011}
P.~E.~Hoggan, ``Slater-type Orbital Basis Sets:\ Reliable and Rapid
  Solution of the Schr\"odinger Equation for Accurate Molecular
  Properties,'' in \emph{Solving the Schr\"odinger Equation: Has
  Everything Been Tried?}, edited by P.~Popelier (Imperial College
  Press, 2011).
...
F.~Latr\'emoli\`ere, ``The Modular Gromov--Hausdorff Propinquity,''
