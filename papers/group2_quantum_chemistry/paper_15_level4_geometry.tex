...
recovers 96.0\% of the exact dissociation energy $D_e$ at
$l_{\max}=6$ with $\sigma$ and $\pi$ channels and a Schwartz cusp correction
(61~channels, CBS extrapolation ${\sim}97\%$;\ the pure-variational
...
dependence, whereas the Level~4 figure includes $\pi$ channels.  Nor
...
The framework extends to heteronuclear diatomics (HeH$^+$,
$Z_A=2$, $Z_B=1$) via a charge-center origin shift and removal of the
gerade constraint; the adiabatic solver recovers 93.1\% of the exact
$D_e$ at $l_{\max}=4$ ($\sigma{+}\pi$, 57~channels), but---unlike
H$_2$---a consistent 2D variational treatment at the same truncation
recovers only ${\sim}5\%$, the absolute energy error ($2.4\%$) being
comparable to the small $0.075$~Ha binding.  The He-like ($Z=2$) core
and charge asymmetry thus converge substantially more slowly than
H$_2$, and the adiabatic value is a non-variational over-binding.
...
\item[Level~2] \emph{Two-center, one-electron} (H$_2^+$).  In prolate
  ...
  Paper~11~\cite{paper11} reaches machine precision for H$_2^+$
...
restored in the same basis, take it to 99.1\%.
...
We demonstrate that this approach recovers 96.0\% of $D_e$ at
...
This is the structural difference of Level~4 coordinates: the cusp is
...
cusp sits;\ as the scope note accompanying the comparison with Paper~12
...
\caption{Convergence with partial-wave truncation.
  ...
  $D_e^{\rm exact} = 0.174475$~Ha~\cite{kolos1968}.}
...
\paragraph{Extended $l_{\max}$ convergence study.}
Table~\ref{tab:extended_convergence} (plotted in
Fig.~\ref{fig:convergence}) extends the convergence analysis
through $l_{\max} = 6$ using the 2D solver with Schwartz cusp correction
(Sec.~\ref{sec:cusp_correction}).  Two features are immediately apparent:
(i)~a pronounced even-odd staircase in the $\sigma$-only column, and
(ii)~a constant ${\sim}6.6$~percentage-point additive offset from
$\pi$~channels, independent of $l_{\max}$.

\begin{table}[h]
\caption{Extended convergence through $l_{\max} = 6$ (2D+cusp solver).
  $\sigma$-only uses $m_{\max} = 0$;
  $\sigma{+}\pi$ uses $m_{\max} = 1$ with $\pi$ channels frozen at
  ...
  discrepancy.
  $\delta$~channels ($m_{\max} = 2$) are shown at $l_{\max} = 4$ only.
  $\Delta_{\rm step}$ is the gain from each $l_{\max}$ increment.
  CBS extrapolation via $(l+1/2)^{-2}$ fit to $l_{\max} = 4$--$6$.}
\label{tab:extended_convergence}
\begin{ruledtabular}
\begin{tabular}{cdddd}
$l_{\max}$
  & \multicolumn{1}{c}{$\sigma$ (\%)}
  & \multicolumn{1}{c}{$\Delta_{\rm step}$}
  & \multicolumn{1}{c}{$\sigma{+}\pi$ (\%)}
  & \multicolumn{1}{c}{$N_{\rm ch}$} \\
\hline
1 &  37.2 & ---   & 43.7 &   2 \\
2 &  79.5 & +42.3 & 86.2 &   9 \\
3 &  80.2 & +0.66 & 86.8 &  18 \\
...
6 &  88.8 & +1.69 & 96.0 &  61 \\
\hline
CBS &  \multicolumn{1}{c}{---}  & & \multicolumn{1}{c}{${\sim}97$} & \\
\hline
4 ($\delta$) & 87.6 & \multicolumn{1}{c}{$+0.65^{\dagger}$} & --- & 37 \\
\end{tabular}
\end{ruledtabular}
{\footnotesize ${}^\dagger$$\delta$-channel ($m_{\max}=2$) gain
relative to $\sigma$-only at same $l_{\max} = 4$.  Cost increase:
$8.7\times$.}
\end{table}
...
\hline
Paper~12 (Neumann $V_{ee}$, $\sigma$-only)   & 0.1612 & 92.4 \\
...
The $\sigma$-only result (87.0\%) falls below Paper~12's
...
basis is $\varphi$-independent and spans only $m_1 = m_2 = 0$.  Nor
...
calculation in the literature~\cite{tao_mccurdy_rescigno2010} treats
two-electron H$_2$ accurately in these coordinates, which
settles the narrower point that those coordinates are not cusp-limited
without ordering the two solvers here.  What the table below
...
is recovered; adding $\pi$ channels brings this to 93.1\% in the
\emph{adiabatic} treatment.  This is a non-variational over-binding
figure---the consistent two-dimensional variational treatment recovers
only $\sim\!5\%$ of $D_e$, so it is not parity with the variational
H$_2$ result.
...
\subsection{The even-odd staircase}
\label{sec:staircase}

The extended convergence data
(Table~\ref{tab:extended_convergence}) reveals a pronounced
even-odd staircase: even $l_{\max}$ increments yield large
$D_e$ gains ($l_{\max} = 2$: $+42.3$~pp; $l_{\max} = 4$:
$+6.78$~pp; $l_{\max} = 6$: $+1.69$~pp), while odd
increments yield near-zero gains ($l_{\max} = 3$: $+0.66$~pp;
$l_{\max} = 5$: $+0.08$~pp).  This is not a numerical artifact
but a direct consequence of the nuclear coupling selection rules.

The nuclear coupling matrix [Eq.~(\ref{eq:Wnuc})] is diagonal in
one electron's angular momentum via the Kronecker delta:
$\delta_{l_2' l_2}$ for the first term and $\delta_{l_1' l_1}$
for the second.  The ground channel $(0,0)$ therefore couples
\emph{directly} only to channels of the form $(0,L)$ or $(L,0)$,
where $L$ is determined by the Gaunt integral selection rules.
The gerade constraint $l_1 + l_2 = \text{even}$
[Eq.~(\ref{eq:gerade})] forces $L$ to be even.  Consequently:
\begin{itemize}
  \item At even $l_{\max}$, new channels $(0, l_{\max})$ and
    $(l_{\max}, 0)$ couple \emph{directly} to the ground channel
    and produce immediate $D_e$ improvement.
  \item At odd $l_{\max}$, the new channels
    (e.g., $(1,3)$, $(3,1)$, $(3,3)$ at $l_{\max} = 3$) have no
    direct nuclear coupling to $(0,0)$---both Kronecker deltas are
    zero---and contribute only through weak electron-electron
    indirect coupling.
\end{itemize}

The $\sigma$-only column in Table~\ref{tab:extended_convergence}
reproduces the staircase identically, confirming that $\pi$
channels do not participate in the mechanism.  A $\sigma$-only
test with the $e$--$e$ off-diagonal coupling removed shows that
the residual odd-$l_{\max}$ gains ($+0.66$~pp at $l_{\max} = 3$)
come from indirect coupling through the electron-electron
multipole expansion, not from nuclear coupling.
...
This improvement is essential.  It is also, as Paper~12 has since
...
the offset is 6.7~pp at $l_{\max} = 2$, 6.6~pp at $l_{\max} = 3$,
6.6~pp at $l_{\max} = 4$, 6.6~pp at $l_{\max} = 5$, and
7.2~pp at $l_{\max} = 6$.
The offset is near-constant over $l_{\max} = 2$--$5$ and rises to
...
\paragraph{$\delta$-channel cost.}
At $l_{\max} = 4$, adding $\delta$ channels ($m_{\max} = 2$) yields a
...
The cost increase is $8.7\times$ due to the larger matrix dimension.
$\delta$ channels are therefore not cost-effective for production
calculations.

\subsection{The remaining gap and CBS extrapolation}

At $l_{\max} = 6$ with $\sigma{+}\pi$ channels (61~channels,
${\sim}754$~seconds per PES point), the 2D+cusp solver recovers
96.0\% of the exact $D_e$.  The CBS extrapolation from $l_{\max} =
4$--$6$ using the $(l + 1/2)^{-2}$ partial-wave convergence formula
yields ${\sim}97\%$.

The remaining ${\sim}3\%$ gap has two identified sources:
\begin{enumerate}
  \item \textbf{Adiabatic--2D gap.}  A constant ${\sim}10$~percentage-point
    gap persists between adiabatic and 2D results across all $l_{\max}$,
    reflecting the kinetic energy cost of non-adiabatic coupling.  The
    2D solver captures this correctly; the adiabatic solver overestimates
    $D_e$ by this amount.
  \item \textbf{Higher $m$ channels.}  $\delta$ channels ($m_{\max} = 2$)
...
Reaching $> 99\%$ along this route would need higher $m$ channels
...
\paragraph{Dissociation limit.}
The hyperradial solver does not converge at large $R$
($R_{e,\max} = 15$~bohr is too small for fully separated atoms),
but this does not affect $D_e$ because the dissociation energy is
computed using the exact separated-atom energy
$E_{\rm atoms} = 2E_{\rm H} = -1.0$~Ha rather than a large-$R$
numerical limit.
...
The 93.1\% recovery quoted above is an \emph{adiabatic} value
(Table~\ref{tab:heh_convergence}).  A consistent 2D variational
treatment---the same solver that yields H$_2$'s 94.1\% at this
truncation---instead gives $E_{\rm total} = -2.9077$~Ha for HeH$^+$
at $l_{\max}=4$, i.e.\ only ${\sim}5\%$ of $D_e$: the variational
energy lies $2.4\%$ ($0.071$~Ha) above exact, an error comparable to
the entire $0.075$~Ha binding.  The adiabatic solver's apparent
$93.1\%$ is therefore a non-variational over-binding coincidence
(the same mechanism that produces the spurious He adiabatic
``$0.05\%$'' of Paper~13), not a genuine partial-wave convergence.
Contrary to a naive H$_2$ parity, the heteronuclear $Z=2$ system
converges \emph{substantially more slowly} than H$_2$: the charge
asymmetry spreads angular weight across many channels (only $26\%$
in $(0,0)$ for a $6{:}1$ ratio) and the He-like core demands both
higher $l_{\max}$ and a finer hyperradial grid than the
H$_2$-matched parameters used here.
...
68\%, with 20\% in odd-$l$ channels.  This drift reflects the single-channel angular truncation for the
bare, asymmetric Level~4 system.  It must not be conflated with the
$R_{\rm eq}$ divergence of the \emph{composed} geometry: there the
variational 2D solver produces the \emph{same} drift
(Paper~17~\cite{paper17}), so that divergence is structural to the
PK/composed architecture, not a single-channel adiabatic artifact
removable by the 2D solver.
...
\subsection{Schwartz cusp correction}
\label{sec:cusp_correction}

The electron-electron cusp at $\alpha = \pi/4$, $\theta_{12} = 0$
is resolved as a boundary condition in Level~4 coordinates, but the
partial-wave expansion converges algebraically in $l_{\max}$, not
exponentially.  Schwartz~\cite{schwartz1962} showed that the
partial-wave energy increment $\Delta E_l$ for the $e$--$e$ cusp
converges as
\begin{equation}
  \Delta E_l \sim -\frac{A}{(l + \tfrac{1}{2})^4}
  \label{eq:schwartz}
\end{equation}
where $A = (10/\pi)\langle\delta^3(\bm{r}_{12})\rangle$ depends on
the coalescence density.  The cumulative correction
\begin{equation}
  \Delta E_{\rm cusp}(l_{\max})
    = -\sum_{l > l_{\max}} \frac{A}{(l + \tfrac{1}{2})^4}
  \label{eq:cusp_sum}
\end{equation}
can be applied as a post-processing correction to the variationally
computed energy at any $l_{\max}$.

For He (Level~3), the cusp correction breaks through the
0.19--0.20\% adiabatic floor: at $l_{\max} = 2$, the corrected
error is 0.10\% (from 0.24\% uncorrected).  At $l_{\max} = 0$,
the correction overcorrects (1.6\% vs 1.1\% uncorrected)
because the adiabatic approximation error dominates.

For H$_2$ (Level~4), the correction is $R$-dependent: largest at
short $R$ (${\sim}2.5$~mHa at $R = 1.0$~bohr) and vanishing at
dissociation, where the coalescence density goes to zero.  The
$D_e$ differential correction is ${\sim}1.7$~mHa, contributing
${\sim}1.0$~percentage point of $D_e$ recovery.

\paragraph{Solver independence.}
A comparison of the cusp correction applied to the adiabatic and 2D
variational solvers at the same $l_{\max}$ confirms that the
correction magnitude is solver-independent: both solvers produce
identical correction values at each $l_{\max}$.  The correction
scales as ${\sim}1/(l_{\max} + 1/2)^4$, consistent with Eq.~(\ref{eq:schwartz}).
At $l_{\max} = 4$, the correction differential (between $R_{\rm eq}$ and
...
\paragraph{Transcorrelated alternative (explored; dead end).}
A complementary approach attempts to remove the cusp algebraically rather than
correcting it post hoc.  The transcorrelated (TC) transformation
$H_{\mathrm{TC}} = e^{-J} H\, e^{J}$ with Jastrow factor
$J = \tfrac{1}{2} r_{12}$ replaces $1/r_{12}$ with a smooth
gradient operator.  This transformation is incompatible with the
adiabatic solver (the gradient operator is $O(1)$ while $V_{ee}$ is
$O(R)$ in the angular eigenvalue problem; Track~BX-2).  An initial
second-quantized benchmark (Track~BX-3) appeared to show the TC qubit
Hamiltonian converging to 3.3--3.6\% error for He while the standard
encoding diverged ($5.3\%\to8.2\%$), but this was a \emph{false
positive}: the benchmark diagonalized the full qubit (Fock) space,
including wrong-particle-number sectors, which for a non-Hermitian TC
Hamiltonian admits artifactually low eigenvalues below the variational
bound.  Under correct particle-number-projected full CI (Track~TC-V),
the standard encoding does \emph{not} diverge---it converges to
${\sim}2.0\%$---while TC plateaus at ${\sim}3.4\%$, i.e.\ \emph{worse}
than the untransformed pipeline.  The cusp is therefore an
energy-evaluation problem, not a wavefunction problem, and TC is not
pursued further; the Schwartz post-hoc correction above remains the
operative method.

\paragraph{Irreducibility of the per-$\rho$ diagonalization.}
The angular eigenvalues $\mu(\rho)$ are computed by pointwise
diagonalization because the nuclear coupling $V_{\rm nuc}(\rho)$
depends on~$\rho$ through split-region factors
$\min(s,\rho)/\max(s,\rho)$, which are piecewise-smooth in~$\rho$
with derivative discontinuities at $s = \rho$.  Three approaches
to eliminating this per-$\rho$ diagonalization were investigated
(Track~Z):
\begin{enumerate}
  \item \emph{Algebraic blow-up}: resolving $|s - \rho|$ by
    introducing sheet variables produces smooth functions within
    each sheet, but the sheet boundaries move with~$\rho$ (at
    $\alpha = \arccos\rho$ and $\arcsin\rho$), creating
    transcendental (not polynomial) matrix elements.  No global
    characteristic polynomial $P(\rho,\mu) = 0$ is possible.
  \item \emph{Lie algebra elevation}: the function
    $\min(s,\rho)/\max(s,\rho)$ is $C^0$ but not~$C^1$ at $s = \rho$.
    Matrix elements of finite-dimensional Lie group representations
    are real-analytic, so this function cannot arise as a matrix
    element of any group containing SO(6).
  \item \emph{$S^3 \times S^3$ hybrid}: each electron's nuclear
    attraction is smooth on its own Fock $S^3$, but two-center
    nuclear attraction cannot be smooth on any single compact manifold
    (the multi-center obstruction of Papers~8--9).  Furthermore,
    $1/r_{12}$ does not factorize on the product space.
\end{enumerate}
The piecewise structure is therefore irreducible: it reflects the
physical geometry of two nuclei at finite separation in
hyperspherical coordinates.  The spectral Jacobi solver
(Table~\ref{tab:spectral_angular_l4}) is the optimal approach---it
reduces the per-$\rho$ cost from $1000 \times 1000$ to
$50 \times 50$ while preserving eigenvalue accuracy.
...
\begin{enumerate}
  ...
  \item The framework extends to heteronuclear diatomics (HeH$^+$)
    ...
    $D_e$ at $l_{\max} = 4$ with $\sigma{+}\pi$ channels in the
    \emph{adiabatic} treatment---a non-variational over-binding figure
    (the consistent 2D-variational value is only $\sim\!5\%$ of $D_e$,
    not parity with the variational H$_2$ result).
\end{enumerate}
...
4 & H$_2$   & Mol.-frame hypersp. & 96.0\% \\
4 & HeH$^+$ & Mol.-frame hypersp. & 93.1\% (adiabatic; ${\sim}5\%$ variational) \\
\bottomrule
\end{tabular}
\end{center}

The extended convergence study through $l_{\max} = 6$ with
$\sigma{+}\pi$ channels and Schwartz cusp correction achieves 96.0\%
$D_e$ (61~channels), with CBS extrapolation to ${\sim}97\%$.
The even-odd staircase (Sec.~\ref{sec:staircase}) reflects the
gerade--quadrupole constraint on nuclear coupling.
$\pi$ channels contribute a near-constant ${\sim}6.6$~pp offset
...
\paragraph{Level~4N generalization.}
The Level~4 framework generalizes to $N > 2$ electrons by replacing
SO(6) with SO($3N$) and the gerade constraint with full $S_N$
antisymmetry.  For LiH ($N=4$), this ``Level~4N'' solver operates
on $S^{11}$ with SO(12) symmetry and $S_4$ singlet ($[2,2]$)
projection.  The full 4-electron solver produces molecular equilibrium
without a pseudopotential at $l_{\max} \geq 2$ (Paper~17, Sec.~VIII),
validating the composed geometry (Level~5) as an approximation to the
exact $N$-electron treatment with $144{\times}$ angular compression.

\paragraph{Note added v2.0.24.}
The 2D variational solver for the full 4-electron system (Track~AR,
v2.0.23) has confirmed that the adiabatic $D_e$ overcounting
($0.49$~Ha at $l_{\max} = 2$, $5.3{\times}$ the exact value) was an
artifact: the 2D solver gives $D_e = -0.19$~Ha (unbound), with
$E_{\min} = -7.79$~Ha (variational bound respected, above the exact
$-8.07$~Ha).  The $l_{\max} = 2$ $S_4$ $[2,2]$ angular basis (12
channels) is genuinely insufficient for LiH binding.  All three
radial solvers---adiabatic, coupled-channel, and 2D
variational---are now exhausted for the 4-electron system.  The
bottleneck is the angular basis, not the radial treatment.  The
composed geometry (Level~5) at the same $l_{\max} = 2$ achieves
5.3\% $R_{\rm eq}$ error (vs 63.5\%) because its angular
pre-optimization via core/valence separation captures the dominant
physics that the raw SO(12) basis cannot represent at tractable
$l_{\max}$.  The classical solver investigation across the natural
geometry hierarchy is now complete (v2.0.24).
...
from Scale-Invariant Graph Topology'' (Paper~7 in the Geometric Vacuum series).

\bibitem{paper11}
J.~Loutey,
``The Molecular Fock Projection: Diatomic Molecules as Prolate
Spheroidal Lattices'' (Paper~11 in the Geometric Vacuum series).
...
Channel Graphs'' (Paper~13 in the Geometric Vacuum series).
