...
\paragraph{Transcorrelated alternative (v2.0.35).}
A complementary approach removes the cusp algebraically rather than
...
gradient operator, converting the algebraic partial-wave convergence
to exponential.  This transformation is incompatible with the
adiabatic solver (the gradient operator is $O(1)$ while $V_{ee}$ is
$O(R)$ in the angular eigenvalue problem; Track~BX-2), but succeeds
in the second-quantized qubit pipeline (Track~BX-3), where integrals
are computed directly without adiabatic separation.  For He, the TC
qubit Hamiltonian converges to 3.3--3.6\% error across
$n_{\max} = 1$--$3$, eliminating the basis divergence seen in the
standard encoding (5.3\%$\to$8.2\%).  The Gaunt angular selection
rules are preserved (the Jastrow is rotationally invariant), with
a constant $1.68\times$ Pauli term overhead from non-Hermiticity.
See Paper~14, Sec.~IV for composed-molecule TC benchmarks.
