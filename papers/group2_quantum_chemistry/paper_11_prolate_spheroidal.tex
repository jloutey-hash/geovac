...
can be computed algebraically from three-term recurrence relations,
eliminating numerical quadrature entirely (the default path uses
Gauss--Laguerre quadrature, which agrees to machine precision).  For $\pi$ and $\delta$
states ($m \neq 0$), an associated Laguerre basis absorbs the
centrifugal $1/x$ singularity via partial-fraction decomposition and
Stieltjes integral recurrence, reducing the non-algebraic content to
a single transcendental seed value $e^a E_1(a)$.
...
$E_{\min} = -0.602634214494936$~Ha, reproducing the two-center
...
with a shared momentum scale $p_0$, the one-electron Hamiltonian and
overlap obey the two-term identity
$H_{ij} = -\tfrac12 p_0^2 S_{ij} + (1-\beta_j)V_{ij}$ and share a single
orthogonal $SO(4)$ Wigner-$D$ congruence (so $H$ is block-, not scalar-,
proportional to $S$), rendering the generalized eigenvalues independent
of $R$.  Binding in this
...
circumvents this, achieving a vertical binding energy
$D_e^{\text{CP}} = 0.110$~Ha for LiH at nmax$= 3$ ($\sim 20\%$ error
vs experiment)~\cite{loutey_lcao_fci}, but carries intrinsic BSSE and
an $R$-independent kinetic energy, producing a monotonically attractive
potential-energy surface with no equilibrium geometry.
...
radial solver that reaches machine precision.
...
accuracy.  This section describes a spectral alternative that reaches
machine precision with $250\times$ fewer degrees of freedom.
...
\hline
  $E_{\mathrm{total}}$ error & 1.01\% & \multicolumn{1}{c}{machine precision} \\
  $R_{\mathrm{eq}}$ (bohr)   & \sim 2.01 & 1.9973 \\
  $R_{\mathrm{eq}}$ error    & 0.65\% & 0.013\% \\
...
The spectral solver achieves a $250\times$ dimension reduction and a
$270\times$ speedup over the FD solver, and reaches machine
precision.  The FD solver's 1.01\% error was a discretization
...
\subsection{Algebraic matrix elements}
\label{sec:algebraic}

\subsubsection{\texorpdfstring{$\sigma$}{Sigma} states (\texorpdfstring{$m = 0$}{m=0})}
...
\subsubsection{\texorpdfstring{$\pi$, $\delta$}{Pi, delta} states (\texorpdfstring{$m \neq 0$}{m != 0}): associated Laguerre basis}
\label{sec:algebraic_associated}

For $m \neq 0$, the centrifugal term $m^2/(\xi^2 - 1)$ produces
a $1/x$ singularity in the Laguerre variable $x = 2\alpha(\xi - 1)$.
In the \emph{ordinary} Laguerre basis, the required integral
$\int_0^\infty L_i\,L_j\,e^{-x}/x\,dx$ diverges at $x = 0$---it
has no finite Laguerre moment expansion.

The obstruction is removed by switching to the \emph{associated
Laguerre basis} $L_n^{(\lvert m\rvert)}(x)$ with weight
$x^{\lvert m\rvert} e^{-x}$.  The basis functions
\begin{equation}
  \varphi_n(\xi) = (\xi - 1)^{\lvert m\rvert/2}\,
    e^{-\alpha(\xi-1)}\,L_n^{(\lvert m\rvert)}\bigl(2\alpha(\xi-1)\bigr)
  \label{eq:associated_basis}
\end{equation}
absorb the $(\xi-1)^{\lvert m\rvert/2}$ prefactor that enforces the
Dirichlet condition at $\xi = 1$, converting the $1/x$ singularity
into a \emph{lowered moment} $M_{-1}^{(\alpha_L)}$ that converges for
$\alpha_L = \lvert m\rvert \geq 1$.

The centrifugal matrix element decomposes via partial fractions:
\begin{equation}
  \frac{m^2}{\xi^2 - 1} = \frac{m^2}{(x/\beta)(x/\beta + 2)}
  = \frac{m^2\beta}{2}\left(\frac{1}{x} - \frac{1}{x + 2\beta}\right),
  \label{eq:partial_fraction}
\end{equation}
where $\beta = 2\alpha$.  The first term gives the lowered moment
matrix $M_{-1}^{(\alpha_L)}$, computed algebraically from the
summation identity (DLMF~18.9.13):
\begin{equation}
  L_n^{(\alpha)}(x) = \sum_{k=0}^{n} L_k^{(\alpha-1)}(x)
  \quad\Longrightarrow\quad
  M_{-1}[i,j] = \sum_{p=0}^{\min(i,j)} h_p^{(\alpha_L - 1)},
  \label{eq:lowered_moment}
\end{equation}
where $h_p^{(\alpha)} = \Gamma(p + \alpha + 1)/p!$ is the
orthogonality constant.

The second term in~\eqref{eq:partial_fraction} gives a
\emph{Stieltjes integral matrix}:
\begin{equation}
  J^{(\alpha_L)}_{ij}(a) = \int_0^\infty
    \frac{x^{\alpha_L}\,e^{-x}\,L_i^{(\alpha_L)}(x)\,L_j^{(\alpha_L)}(x)}
    {x + a}\,dx,
  \label{eq:stieltjes}
\end{equation}
with $a = 2\beta$.  This is evaluated by a three-term recurrence
seeded by the single transcendental value
\begin{equation}
  S_0^{(0)}(a) = e^a\,E_1(a),
  \label{eq:stieltjes_seed}
\end{equation}
where $E_1$ is the exponential integral.  Higher orders
$S_0^{(k)}(a) = \Gamma(k) - a\,S_0^{(k-1)}(a)$ are rational in~$a$
for $k \geq 1$, and column entries follow from the associated
Laguerre three-term recurrence applied to the denominator.
Miller's backward recurrence is used to ensure numerical stability
of the minimal solution.

The overlap, potential, and kinetic matrices in the associated
basis are assembled from $M_0^{(\alpha_L)}$, $M_1^{(\alpha_L)}$,
$M_2^{(\alpha_L)}$ (band matrices from the associated three-term
recurrence), $M_{-1}^{(\alpha_L)}$, and $J^{(\alpha_L)}(2\beta)$.
The kinetic matrix uses a derivative kernel with coefficients
$G_0[n,n] = \lvert m\rvert/2 + n$, $G_0[n,n-1] = -(n + \alpha_L)$
and $G_1[n,n] = -1/2$, combined with weighted moment matrices
$P_k = M_k + 2\beta\,M_{k-1}$.

\emph{Qualitative distinction from $m = 0$:}  The $m = 0$ algebraic
construction is \emph{fully rational}---every matrix element is a
finite combination of integers and the Laguerre scale $\alpha$.
For $m \neq 0$, the single transcendental seed $e^a E_1(a)$
in~\eqref{eq:stieltjes_seed} propagates through the Stieltjes
matrix.  All other matrix elements remain algebraic.

Table~\ref{tab:associated_convergence} shows the convergence
behavior.  The associated basis converges faster than the ordinary
basis with quadrature for $\lvert m\rvert = 1$ (stable by
$N = 10$ vs.\ non-monotonic at $N = 40$), and both bases agree
to $\sim 5 \times 10^{-9}$ for $\lvert m\rvert = 2$.

\begin{table}[h]
\caption{\label{tab:associated_convergence}
  H$_2^+$ $1\pi_u$ state ($m = 1$) at $R = 2.0$~bohr.
  Energies from the associated Laguerre (algebraic) and ordinary
  Laguerre (quadrature) spectral bases, compared to the FD
  reference (extrapolated).  The associated basis converges by
  $N = 10$; ordinary Laguerre is non-monotonic and slower.}
\begin{ruledtabular}
\begin{tabular}{rdd}
  $N$ & \multicolumn{1}{c}{$E_{\mathrm{assoc}}$ (Ha)}
      & \multicolumn{1}{c}{$E_{\mathrm{ord}}$ (Ha)} \\
\hline
...
bottleneck, reaching machine precision with $N_b = 20$.

\subsection{Potential energy surface}

Table~\ref{tab:pes} gives the H$_2^+$ potential energy surface
...
obtain $E_{\mathrm{total}} = -1.512$~Ha (converged spectral solver,
$n_{\mathrm{basis}} \ge 20$; the FD-era $-1.475$ was under-resolved).  The heteronuclear case
...
\paragraph{How the $\eta$-label drifts with $R$.}
\textbf{[MEASURED]} The separation parameter $A$ reduces to $l(l+1)$ in
the united-atom limit, so the ground $\eta$-eigenvector's participation
deficit $D = 1 - \max_l |v_l|^2$ in the Legendre basis measures how far
the state has drifted from a pure united-atom $l$-label.  The two cases
turn on at different orders.  For H$_2^+$ ($b = 0$) the $c^2\eta^2$ term
couples $l \leftrightarrow l \pm 2$ and the deficit rises as $R^{4.0}$;\
for HeH$^{2+}$ the $b\eta$ term couples $l \leftrightarrow l \pm 1$ and
the deficit rises as $R^{2.1}$---first order in the coupling, the
heteronuclear mixing being a factor $R$ earlier.  The large-$R$ behavior
differs too:\ the homonuclear deficit saturates near $\tfrac12$ (gerade
mixing of two Legendre components), whereas the heteronuclear deficit
keeps rising ($0.44$ at $R = 2$, $0.71$ at $R = 8$, $0.75$ at $R = 12$)
as $\langle\eta\rangle \to -1$:\ the label is lost to \emph{localization}
on the He focus, not to mixing.  Both curves are smooth in $R$.  This is
the one-electron, continuous face of the abelian residue of
Paper~58~\cite{loutey_paper58}---$m$ exact at every $R$, $l$ replaced by
a parameter that deforms rather than switches.  Backing:\
\texttt{tests/test\_paper11\_eta\_mixing\_onset.py} (the two onset
exponents, solver-free);\ the large-$R$ values are the solver's, chronicled
in CHANGELOG v5.10.2.
...
$R$-dependent binding \emph{in the single-$n$, block-diagonal approximation}: the one-electron $(H,S)$ obey the two-term
identity $H_{ij} = -\tfrac12 p_0^2 S_{ij} + (1-\beta_j)V_{ij}$ and
share a single $SO(4)$ Wigner-$D$ congruence, making the eigenvalues
geometry-independent.  The
...
While it gives a vertical binding energy ($D_e^{\text{CP}} = 0.110$~Ha
for LiH, $\sim 20\%$ error, no equilibrium geometry), it has inherent
limitations:
\begin{enumerate}
...
reaching machine precision with $N_b = 20$ basis
functions, at $250\times$ smaller matrix dimension than the FD solver.  For $\sigma$ states, all matrix elements
...
energy to machine precision and the equilibrium bond length to
$0.013\%$ accuracy with zero free parameters.  The spectral solver
...
spheroidal CI achieves 58\% of the exact binding energy with a single
variationally optimized orbital exponent and no empirical parameters.  Future work will pursue larger
...
wavefunction's asymptotic decay, reaching machine precision
with $N_b = 20$.
...
Residue of Multi-Center Bonding,'' GeoVac Paper~58 (2026).
