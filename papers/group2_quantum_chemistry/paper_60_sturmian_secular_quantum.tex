...
the same to encode, and at the largest basis where both roots were computed, $K=452$, the
...
the $s$-only ladder at $K=105$ the posing cost falls by $4.3\times$, $3.0\times$ and $2.6\times$
across the first three rungs of the $^{1}S$ ladder.  The result is therefore a measured trade-off between cost growth,
...
the $s$, $+p$ and $spdf$ sectors---past the locked $s$-sector value $\gvq{p60_he_chain_s_k55}{-2.8745}$~Ha to
$\gvq{p60_he_chain_spdf_k164}{-2.8964}$~Ha, toward the exact non-relativistic $-2.90372$~Ha), and measure the decisive quantity: the block-encoding $1$-norm of the
...
inter-center coupling.  \textbf{[PRIOR ART]} The growth law is not new:\ the band-limited
...
one-electron, not $N$-electron.  \textbf{[MEASURED]} A third lever removes the conditioning cost outright
...
unrotated frame the same band is $106\times$ \emph{worse} than no treatment at all.  \textbf{[RESOURCE MODEL]} A direct block-encoding of the preconditioned metric then takes the
penalty from $n^3$ to $n$ (the circulant-embedded circuit is analysed, not compiled).  \textbf{[OPEN]} The atomic case is a clean,
...
shared-scale bases at $M=2$ and non-collinear $M=3$, the \emph{locality} cost is capped by the symbol's other
pole, and the $\ell$-block sparsity loss is structural and not a conditioning
effect at all.  \textbf{[MEASURED]} Its remaining
...
sublinear behavior.  The real molecular savings are the metric levers (the band-Toeplitz
preconditioner, the gerade sector, large separation, one-electron-only metric),
not a sublinear matrix.  We do not
...
\begin{equation}
\lambda_{\text{hydrogenic}}\sim Q^{\gvq{p60_l2_inflation_hydrogenic}{1.19}},
\qquad
\lambda_{\text{Sturmian}}\sim Q^{\gvq{p60_l2_inflation_sturmian}{3.33}},
\label{eq:blowup}
\end{equation}
...
instability:\ a condition number of $32$ is not ill-conditioned.  This is a currency-change of the
...
domain [retracted 2026-09-07: p60-l2-metric-diverges] (Appendix~\ref{app:box}).  On a converged domain
...
\textbf{[PRIOR ART]} The classical momentum-space literature met this and
...
\[
-2.8477\ (1s^2)\;\to\;\gvq{p60_he_chain_s_k55}{-2.8745}\ (s,\,K{=}55)\;\to\;-2.8947\ (+p,\,K{=}100)\;\to\;\gvq{p60_he_chain_spdf_k164}{-2.8964}\ (spdf,\,K{=}164),
\]
converging toward the exact non-relativistic $-2.90372$~Ha.  Every rung beyond the leading $1s^2$ single configuration (a $1\times1$ matrix) is taken at the same $n_{\max}=10$ family, so that part of the sequence is a pure $\ell_{\max}$ ladder;\ values are rounded to four decimals.  \textbf{[MEASURED]}
...
same comparison is $\gvq{p60_span_deficit_sonly_free}{0.15}$~mHa against $\gvq{p60_span_deficit_sonly_locked}{4.40}$~mHa at $K=136$.  The residual is
...
matches instead our scale-optimized ladder ($1.64$~mHa at $K=100$).
\textbf{[OBSERVATION]} Consultations of the primary canon, relayed,
...
against Eq.~\eqref{eq:scale_lock} and withdrawn.  The machinery is correct, including the higher-$\ell$ angular
...
\emph{total} exponent rises but stays below $1$ (local slopes
$0.867\to0.911$ across the rungs from $K=35$ to $K=220$;\ the first
slope lying wholly inside $K=56$--$220$ is $0.879$, and a global fit
over that window is $0.893$).  A superlinear block and a rising total are therefore both
real for a different growth rule;\ but no measured full-shell point makes the
total itself superlinear, and we do not claim one.  What Eq.~\eqref{eq:sublinear} measures is the fixed-$l_{\max}$
...
in its atomic special case.}  Hence the kinetic matrix at unit scale
...
($s$-only, where $-2.879029$~Ha is known independently) against $\gvq{p60_span_deficit_sonly_locked}{4.40}$~mHa
...
step needs $E_{\rm iso}$ to be identified as the lowest root in advance.  And freeing the scale forfeits the
...
$\mathrm{cond}(S)\sim K^{0.94}$ --- a factor $5\times10^{3}$ at $K=136$.  \emph{Metric-free posing and accuracy floor are
...
asymptote moves with the window is not asserting an asymptote.  In the state-dependence
...
mechanism rather than unexplored ground.  And whether the
...
$1$'' [retracted 2026-09-08: p60-tprime-superlinear].  Those were computed on a fixed $60$-bohr radial domain, which truncates
...
\textbf{[INTERNAL THEOREM]} \emph{The general-$V_0$ form, and what the metric
...
(cond $5.8$, growing with basis), so the within-center disaster of
...
$\mathrm{cond}(S)\sim N^{\gvq{p60_sw_cond_exponent}{1.85}}$, essentially the same rate as the $L^2$ overlap's
$N^{\gvq{p60_l2_overlap_exponent}{1.71}}$ (decisively power-law, not exponential), with the largest eigenvalue
...
\textbf{[SYMBOLIC + MEASURED]} The conditioning law has an exact derivation rather than being a fit
...
reading.  Because the intra-center block is
...
theory~\cite{eijkhout_vassilevski1991}.  In the
...
proof we can cite, not a claim on our side of the line).  It is the
...
with symbol $j_0(kR\cot(\chi/2))$.  It is \emph{not} the reading of the
Shibuya--Wulfman operator as a translation:\ that is prior art on three
counts, and earlier versions of this paper claimed it.  Shibuya and Wulfman already
...
quotation here is withdrawn as unverified rather than asserted.)\ Wulfman and Takahata gave the explicit
...
basis~\cite{red_weatherford2004}.  That the
angular average $j_0(pR)$ becomes trivial at $p=0$ is then a property of the
symbol, which is the part we are entitled to.  The physical reading follows:\ a basis
...
\textbf{[MEASURED]} One reading of this degeneracy should be resisted, and
...
the coupling's participation ratio stays small.  It also qualifies the motivation offered below for this basis:\
...
change of sign.  A genuine $O(1/n)$ term survives in both conventions.  Whether a weaker
hypothesis covers this symbol class is left open.

\textbf{[SYMBOLIC + MEASURED]} The two transcendentals of this section are
...
(Richardson $\gvq{p60_window_richardson_pi2}{9.86949}$ against $\pi^2=9.86960$), the near-null direction attains
that minimum with r.m.s.\ spread $\pi/n$ (Richardson $\gvq{p60_window_rms_richardson_pi}{3.14158}$), and
...
\textbf{[MEASURED + PRIOR ART]} Reading the symbol as a quotient yields a
...
constant to $\gvq{p60_weighted_collapse_control}{0.828}$ while leaving the exponent at $-1.97$, so the agreement
...
is $\|[P_A,P_B]\|=\max_k\sigma_k\sqrt{1-\sigma_k^{2}}$ --- a one-line
...
therefore inverse-closed.  What is ours is
...
block-diagonalizes $S$, isolating a gerade sector whose conditioning is flat
...
coverage-versus-linear-dependence trade-off; the Coulomb--Sturmians soften it rather than escaping it, because they are a
...
\textbf{[MEASURED]} A third conditioning lever exists;\ it is stronger than
...
$\mathrm{H}_2^+$ the gerade route remains the cheaper one.  Because the ill-conditioning is a symbol \emph{zero} of
...
$n=160$ untreated.  This escapes the $\Theta(\kappa)$ degree floor for QSVT matrix inversion
...
than the polynomial approximated on it.  The statement that the metric cost is ``mitigated, not
...
leaves the other exactly where it was.  The localization class is
...
Jaffard's class~\cite{jaffard1990}, and inversion there preserves the decay \emph{provided the
...
\textbf{[MEASURED]} \textbf{The lever reaches the polyatomic case the gerade
lever cannot}, and the structural reason is that the degeneracy's \emph{direction}
is geometry-independent, and the degeneracy itself is a known one:\ a kernel
...
radial-basis-function literature~\cite{driscoll_fornberg2002,barthelme_usevich2021}.  What is ours is
...
fixed vector set that does not move with geometry or with basis size.
...
For water's $A_1$ block, $A(\chi)=\bigl[\begin{smallmatrix}1&\sqrt2 a_{\rm OH}\\
...
($N^{\gvq{p60_water_a1_exponent}{1.96}}$ here);\ the preconditioned column is bounded, with increments
...
basis-dependent---is gone.  Two controls separate what is doing the work, and the first cannot do the
...
establish is that banding \emph{alone} is not the lever --- $194\to42008$
...
draft printed it as the low endpoint, which reproduced a spurious $N^{0.98}$.)  The discriminating control is the
...
\textbf{[RESOURCE MODEL]} Priced end to end the lever is smaller than the
...
preconditioned, $G$ direct    & $125.5$  & $24.3$            & $3.1\times10^{3}$\\
\bottomrule
\end{tabular}
\end{center}
...
the floor it would be $n$.  The third row is not hypothetical:\ the direct
...
above under Table~\ref{tab:resource}'s cost model, and the direct construction
...
the first three rungs of the $^{1}S$ ladder.  Read as a ground-state route to chemical accuracy it is not
...
with basis size.  Two qualifications lift this from a caveat to a payoff.  The first is
...
the metric untreated.  Established on $s$-sector shared-scale bases at $M=2$ and non-collinear
$M=3$;\ the collinear case is open and is not claimed.  And the penalty does not compound with electron number: the
...
(Sec.~\ref{sec:manyelectron})---the metric cost is one-electron, not $N$-electron.  The atomic case is a result;\ the molecular case is a frontier that is
...
not exhaustive of the paper's smaller observations):\ the scale-lock identity
...
variational bound for the fixed-scale metric-free problem.  Developed with an AI-augmented workflow under
...
$\sim\!10^{-4}$;\ $c=3$ gives $2.15$--$2.16\times10^{-7}$, converged.  Two independent quadrature
...
earlier version of this sentence gave only the order of magnitude.  Exponents are converged to
...
\bibitem{monkhorst_jeziorski1979}
H.~J.~Monkhorst and B.~Jeziorski, ``No linear dependence and multi-center integral
problems in momentum space quantum chemistry,'' \textit{J.\ Chem.\ Phys.}\
\textbf{71}(12), 5268--5269 (1979);\ doi:10.1063/1.438337.

\bibitem{wulfman_takahata1967}
...
\bibitem{batenkov_demanet_goldman_yomdin2019}
...
systems,'' \textit{Math.\ Comp.}\ \textbf{66}, 651 (1997).
